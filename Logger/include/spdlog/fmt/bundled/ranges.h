...
// Check for integer_sequence
    ...
    class is_tuple_formattable_<T, C, true> {
        template<size_t... Is>
        static auto all_true(index_sequence<Is...>, integer_sequence<bool, (Is >= 0)...>) -> std::true_type;
        static auto all_true(...) -> std::false_type;

        template<size_t... Is>
        static auto check(index_sequence<Is...>) -> decltype(all_true(
            index_sequence<Is...> {},
            integer_sequence<bool, (is_formattable<typename std::tuple_element<Is, T>::type, C>::value)...> {}));
    ...
    {
        for_each(tuple_index_sequence<remove_cvref_t<Tuple>>(), std::forward<Tuple>(t), std::forward<F>(f));
    }
    ...
    {
        for_each2(
            tuple_index_sequence<remove_cvref_t<Tuple1>>(),
    ...
    struct range_format_kind_ :
        std::integral_constant<
            range_format,
            std::is_same<uncvref_type<T>, T>::value ? range_format::disabled
...
{
private:
    decltype(detail::tuple::get_formatters<Tuple, Char>(detail::tuple_index_sequence<Tuple>())) formatters_;
...
public:
    ...
    auto format(const Tuple& value, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        detail::for_each2(formatters_, value, detail::format_tuple_element<FormatContext> { 0, ctx, separator_ });
...
struct is_range
{
    static constexpr const bool value = detail::is_range_<T>::value && !detail::has_to_string_view<T>::value;
};
...
namespace detail {
    ...
    struct is_formattable_delayed : is_formattable<uncvref_type<maybe_const_range<R>>, Char>
    {
    };
} // namespace detail
...
private:
    ...
    template<typename Output, typename It, typename Sentinel, typename U = T, FMT_ENABLE_IF(std::is_same<U, Char>::value)>
    auto write_debug_string(Output& out, It it, Sentinel end) const -> Output
    {
        ...
        return detail::write<Char>(out, basic_string_view<Char>(buf.data(), buf.size()), specs);
    }
...
struct formatter<
    ...
    enable_if_t<conjunction<
        bool_constant<
            ...
            range_format_kind<R, Char>::value != range_format::map && range_format_kind<R, Char>::value != range_format::string &&
...
public:
    ...
    FMT_CONSTEXPR formatter()
    {
        if (detail::const_check(range_format_kind<R, Char>::value != range_format::set)) {
            return;
        }
        range_formatter_.set_brackets(detail::string_literal<Char, '{'> {}, detail::string_literal<Char, '}'> {});
    }

    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return range_formatter_.parse(ctx); }
...
private:
    ...
    decltype(detail::tuple::get_formatters<element_type, Char>(detail::tuple_index_sequence<element_type>())) formatters_;
    ...
    auto format(map_type& map, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        for (auto&& value : map) {
            ...
            detail::for_each2(
                formatters_,
                value,
                detail::format_tuple_element<FormatContext> { 0, ctx, detail::string_literal<Char, ':', ' '> {} });
            ++i;
        }
...
public:
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return underlying_.parse(ctx); }

    template<typename FormatContext>
    auto format(range_type& range, FormatContext& ctx) const -> decltype(ctx.out())
    {
        auto out = ctx.out();
        if (detail::const_check(range_format_kind<R, Char>::value == range_format::debug_string)) {
            *out++ = '"';
        }
        out = underlying_.format(string_type { detail::range_begin(range), detail::range_end(range) }, ctx);
        if (detail::const_check(range_format_kind<R, Char>::value == range_format::debug_string)) {
            *out++ = '"';
        }
        return out;
...
private:
    ...
    using view =
        conditional_t<std::is_copy_constructible<It>::value, const join_view<It, Sentinel, Char>, join_view<It, Sentinel, Char>>;

public:
    using nonlocking = void;

    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return value_formatter_.parse(ctx); }
...
template<typename Char, typename Tuple>
struct formatter<tuple_join_view<Char, Tuple>, Char, enable_if_t<is_tuple_like<Tuple>::value>>
{
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return do_parse(ctx, std::tuple_size<Tuple>()); }
...
private:
    ...
    FMT_CONSTEXPR auto do_parse(parse_context<Char>& ctx, std::integral_constant<size_t, 0>) -> const Char*
    {
        return ctx.begin();
    }

    template<size_t N>
    FMT_CONSTEXPR auto do_parse(parse_context<Char>& ctx, std::integral_constant<size_t, N>) -> const Char*
    {
        auto end = ctx.begin();
    ...
    template<typename FormatContext>
    auto do_format(const tuple_join_view<Char, Tuple>&, FormatContext& ctx, std::integral_constant<size_t, 0>) const ->
    ...
    auto do_format(const tuple_join_view<Char, Tuple>& value, FormatContext& ctx, std::integral_constant<size_t, N>) const ->
        typename FormatContext::iterator
    {
        using std::get;
        auto out = std::get<std::tuple_size<Tuple>::value - N>(formatters_)
