 ...
 */
    ...
    constexpr auto parse_replacement_field_then_tail(S fmt)
    {
        ...
        if constexpr (c == '}') {
            return parse_tail<Args, END_POS + 1, NEXT_ID>(field<char_type, typename field_type<T>::type, ARG_INDEX>(), fmt);
        }
        else if constexpr (c != ':') {
    ...
    constexpr auto compile_format_string(S fmt)
    {
        ...
        if constexpr (str[POS] == '{') {
            ...
            if constexpr (str[POS + 1] == '{') {
                ...
                return parse_replacement_field_then_tail<get_type<ID, Args>, Args, POS + 1, ID, next_id>(fmt);
            }
            else {
                ...
                if constexpr (arg_id_result.kind == arg_id_kind::index) {
                    ...
                    constexpr auto arg_index = get_arg_index_by_name(arg_id_result.arg_id.name, Args {});
                    if constexpr (arg_index >= 0) {
                        ...
                        return parse_tail<Args, arg_id_end_pos + 1, ID>(
                            runtime_named_field<char_type> { arg_id_result.arg_id.name },
                            fmt);
                    }
                    else if constexpr (c == ':') {
    ...
    template<typename... Args, typename S, FMT_ENABLE_IF(is_compiled_string<S>::value)>
    constexpr auto compile(S fmt)
    {
        ...
        if constexpr (str.size() == 0) {
            ...
            constexpr auto result = detail::compile_format_string<detail::type_list<Args...>, 0, 0>(fmt);
            return result;
        }
    }
...
template<typename S, typename... Args, FMT_ENABLE_IF(is_compiled_string<S>::value)>
...
template<typename OutputIt, typename S, typename... Args, FMT_ENABLE_IF(is_compiled_string<S>::value)>
FMT_CONSTEXPR OutputIt format_to(OutputIt out, const S&, Args&&... args)
{
    constexpr auto compiled = detail::compile<Args...>(S());
    if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>, detail::unknown_format>()) {
        return fmt::format_to(out, static_cast<basic_string_view<typename S::char_type>>(S()), std::forward<Args>(args)...);
    }
    else {
...
template<typename OutputIt, typename S, typename... Args, FMT_ENABLE_IF(is_compiled_string<S>::value)>
auto format_to_n(OutputIt out, size_t n, const S& fmt, Args&&... args) -> format_to_n_result<OutputIt>
{
