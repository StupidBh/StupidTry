...
#define FMT_NOMACRO
    ...
    void write_codecvt(codecvt_result<CodeUnit>& out, string_view in, const std::locale& loc)
    {
        ...
        auto result = f.in(mb, in.begin(), in.end(), from_next, std::begin(out.buf), std::end(out.buf), out.end);
    ...
    auto write_encoded_tm_str(OutputIt out, string_view in, const std::locale& loc) -> OutputIt
    {
        if (const_check(detail::use_utf8) && loc != get_classic_locale()) {
            ...
            auto u = to_utf8<code_unit, basic_memory_buffer<char, unit_t::max_size * 4>>();
    ...
    template<
        ...
        typename FromRep,
        typename FromPeriod,
        FMT_ENABLE_IF(std::is_integral<FromRep>::value&& std::is_integral<typename To::rep>::value)>
    auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To
    {
        ...
        using common_rep = typename std::common_type<FromRep, typename To::rep, decltype(factor::num)>::type;

        int ec = 0;
        auto count = safe_duration_cast::lossless_integral_conversion<common_rep>(from.count(), ec);
        if (ec) {
        ...
        auto to = To(safe_duration_cast::lossless_integral_conversion<typename To::rep>(count, ec));
        if (ec) {
            throw_duration_error();
        }
        return to;
    ...
    template<
        ...
        typename FromRep,
        typename FromPeriod,
        FMT_ENABLE_IF(std::is_floating_point<FromRep>::value&& std::is_floating_point<typename To::rep>::value)>
    auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To
    {
#if FMT_SAFE_DURATION_CAST
    ...
    template<
        ...
        typename FromRep,
        typename FromPeriod,
        FMT_ENABLE_IF(!is_similar_arithmetic_type<FromRep, typename To::rep>::value)>
    auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To
    {
    ...
    auto to_time_t(sys_time<Duration> time_point) -> std::time_t
    {
        ...
        // overflow.
        return detail::duration_cast<std::chrono::duration<std::time_t>>(time_point.time_since_epoch()).count();
    }
...
FMT_DEPRECATED inline auto localtime(std::time_t time) -> std::tm
{
    struct dispatcher
    {
        ...
        inline dispatcher(std::time_t t) :
            time_(t)
        {
        }
...
namespace detail {
    ...
    FMT_CONSTEXPR auto parse_chrono_format(const Char* begin, const Char* end, Handler&& handler) -> const Char*
    {
        ...
        while (ptr != end) {
            ...
            case 'e': handler.on_day_of_month(numeric_system::standard, pad_type::space); break;
            // Hour, minute, second:
    ...
    {
        static constexpr const char* full_name_list[] = { "January", "February", "March",     "April",   "May",      "June",
    ...
    {
        if (!std::is_unsigned<Int>::value && (value < 0 || to_unsigned(value) > to_unsigned(upper))) {
    ...
    void write_fractional_seconds(OutputIt& out, Duration d, int precision = -1)
    {
        ...
        const auto subseconds = std::chrono::treat_as_floating_point<typename subsecond_precision::rep>::value
                                    ? fractional.count()
    ...
    void write_floating_seconds(memory_buffer& buf, Duration duration, int num_fractional_digits = -1)
    {
        ...
        if (num_fractional_digits < 0) {
            ...
            using namespace std;
            num_fractional_digits = count_fractional_digits<Duration::period::num, Duration::period::den>::value;
        ...
        fmt::format_to(
            ...
            FMT_STRING("{:.{}f}"),
            std::fmod(
                val * static_cast<rep>(Duration::period::num) / static_cast<rep>(Duration::period::den),
                static_cast<rep>(60)),
    ...
    private:
        ...
        void write_year(long long year, pad_type pad) { write_year_extended(year, pad); }
    ...
    public:
        ...
        FMT_CONSTEXPR void on_text(const Char* begin, const Char* end) { out_ = copy<Char>(begin, end, out_); }
        ...
        void on_dec1_week_of_year(numeric_system ns, pad_type pad)
        {
            if (is_classic_ || ns == numeric_system::standard) {
                auto wday = tm_wday();
                write2((tm_yday() + days_per_week - (wday == 0 ? (days_per_week - 1) : (wday - 1))) / days_per_week, pad);
            }
            else {
                format_localized('W', 'O');
            }
    ...
    inline auto get_milliseconds(std::chrono::duration<Rep, Period> d) -> std::chrono::duration<Rep, std::milli>
    {
        // This may overflow and/or the result may not fit in the target type.
#if FMT_SAFE_DURATION_CAST
        using common_seconds_type = typename std::common_type<decltype(d), std::chrono::seconds>::type;
        auto d_as_common = detail::duration_cast<common_seconds_type>(d);
        auto d_as_whole_seconds = detail::duration_cast<std::chrono::seconds>(d_as_common);
    ...
    auto format_duration_value(OutputIt out, Rep val, int precision) -> OutputIt
    {
        ...
        specs.set_type(precision >= 0 ? presentation_type::fixed : presentation_type::general);
    ...
    public:
        ...
        inline operator const std::locale&() const { return has_locale_ ? locale_ : get_classic_locale(); }
    };
    ...
    struct duration_formatter
    {
        ...
        auto hour() const -> Rep { return static_cast<Rep>(mod((s.count() / 3600), 24)); }
        ...
        auto minute() const -> Rep { return static_cast<Rep>(mod((s.count() / 60), 60)); }
        ...
        void on_text(const Char* begin, const Char* end) { copy<Char>(begin, end, out); }
        ...
        void on_second(numeric_system ns, pad_type pad)
        {
            ...
            if (ns == numeric_system::standard) {
                if (std::is_floating_point<rep>::value) {
                    ...
                    write(second(), 2, pad);
                    write_fractional_seconds<Char>(out, std::chrono::duration<rep, Period>(val), precision);
                }
                return;
            }
            auto time = tm();
...
public:
    ...
    auto format(std::chrono::duration<Rep, Period> d, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        if (begin == end || *begin == '}') {
            ...
            auto f = detail::duration_formatter<Char, Rep, Period>(out, d, ctx.locale());
...
private:
    format_specs specs_;
    detail::arg_ref<Char> width_ref_;
    basic_string_view<Char> fmt_ = detail::string_literal<Char, '%', 'F', ' ', '%', 'T'>();
...
protected:
    ...
    FMT_CONSTEXPR auto do_parse(parse_context<Char>& ctx, bool has_timezone) -> const Char*
    {
        ...
        end = detail::parse_chrono_format(it, end, detail::tm_format_checker(has_timezone));
    ...
    auto do_format(const std::tm& tm, FormatContext& ctx, const Duration* subsecs) const -> decltype(ctx.out())
    {
        ...
        auto w = detail::tm_writer<basic_appender<Char>, Char, Duration>(loc, out, tm, subsecs);
...
{
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return this->do_parse(ctx, true); }
    ...
    auto format(sys_time<Duration> val, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        if (detail::const_check(period::num == 1 && period::den == 1 && !std::is_floating_point<typename Duration::rep>::value)) {
            detail::set_tm_zone(tm, detail::utc());
            return formatter<std::tm, Char>::format(tm, ctx);
        }
        Duration epoch = val.time_since_epoch();
        Duration subsecs = detail::duration_cast<Duration>(epoch - detail::duration_cast<std::chrono::seconds>(epoch));
        if (subsecs.count() < 0) {
...
struct formatter<utc_time<Duration>, Char> : formatter<sys_time<Duration>, Char>
{
    ...
    {
        return formatter<sys_time<Duration>, Char>::format(detail::utc_clock::to_sys(val), ctx);
    }
};
...
{
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return this->do_parse(ctx, false); }

    template<typename FormatContext>
    auto format(local_time<Duration> val, FormatContext& ctx) const -> decltype(ctx.out())
    {
        auto time_since_epoch = val.time_since_epoch();
        auto seconds_since_epoch = detail::duration_cast<std::chrono::seconds>(time_since_epoch);
        // Use gmtime to prevent time zone conversion since local_time has an
        // unspecified time zone.
        std::tm t = gmtime(seconds_since_epoch.count());
        using period = typename Duration::period;
        if (period::num == 1 && period::den == 1 && !std::is_floating_point<typename Duration::rep>::value) {
            return formatter<std::tm, Char>::format(t, ctx);
        }
        auto subsecs = detail::duration_cast<Duration>(time_since_epoch - seconds_since_epoch);
