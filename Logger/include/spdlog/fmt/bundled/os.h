...
FMT_API std::system_error vwindows_error(int error_code, string_view fmt, format_args args);
...
private:
    ...
    inline explicit buffered_file(FILE* f) :
        file_(f)
    {
    }
...
public:
    inline buffered_file(buffered_file&& other) noexcept :
        file_(other.file_)
    {
        other.file_ = nullptr;
    }
    ...
    inline void print(string_view fmt, const T&... args)
    {
        ...
        detail::is_locking<T...>() ? fmt::vprint_buffered(file_, fmt, vargs) : fmt::vprint(file_, fmt, vargs);
    }
};
