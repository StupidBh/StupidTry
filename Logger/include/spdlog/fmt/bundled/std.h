...
#include "ostream.h"
    ...
    // Check FMT_CPLUSPLUS to suppress a bogus warning in MSVC.
        ...
        #if FMT_HAS_INCLUDE(<filesystem>) && (!defined(FMT_CPP_LIB_FILESYSTEM) || FMT_CPP_LIB_FILESYSTEM != 0)
            #include <filesystem>
        #endif
...
namespace detail {
        ...
        const std::basic_string<PathChar>& native)
    {
        if constexpr (std::is_same_v<Char, char> && std::is_same_v<PathChar, wchar_t>) {
            ...
            write_escaped_string<std::filesystem::path::value_type>(std::back_inserter(quoted), native);
        }
        else {
...
public:
    ...
    {
        auto specs = specs_;
        auto path_string = !path_type_ ? p.native() : p.generic_string<std::filesystem::path::value_type>();
...
struct formatter<
    std::expected<T, E>,
    Char,
    std::enable_if_t<(std::is_void<T>::value || is_formattable<T, Char>::value) && is_formattable<E, Char>::value>>
{
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return ctx.begin(); }

    template<typename FormatContext>
    auto format(const std::expected<T, E>& value, FormatContext& ctx) const -> decltype(ctx.out())
    {
        auto out = ctx.out();
...
struct formatter<std::source_location>
{
    ...
    template<typename FormatContext>
    auto format(const std::source_location& loc, FormatContext& ctx) const -> decltype(ctx.out())
    {
        auto out = ctx.out();
...
namespace detail {
    ...
    class is_variant_formattable_ {
        ...
        static std::conjunction<is_formattable<std::variant_alternative_t<Is, T>, C>...> check(std::index_sequence<Is...>);
    ...
    std::enable_if_t<std::conjunction_v<is_variant_like<Variant>, is_variant_formattable<Variant, Char>>>>
{
    ...
    auto format(const Variant& value, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        FMT_TRY
        {
            std::visit([&](const auto& v) { out = detail::write_escaped_alternative<Char>(out, v); }, value);
        }
...
namespace detail {
    ...
    auto write_demangled_name(OutputIt out, const std::type_info& ti) -> OutputIt
    {
        ...
        std::unique_ptr<char, void (*)(void*)> demangled_name_ptr(
            abi::__cxa_demangle(ti.name(), nullptr, &size, &status),
            &std::free);
        ...
        if (demangled_name_ptr) {
            ...
            if (demangled_name_view.starts_with("std::")) {
                ...
                char* to = begin + 5; // std::
                for (char *from = to, *end = begin + demangled_name_view.size(); from < end;) {
...
struct formatter<
    std::type_info,
    Char // DEPRECATED! Mixing code unit types.
    >
{
public:
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char* { return ctx.begin(); }
    ...
    auto format(const std::type_info& ti, Context& ctx) const -> decltype(ctx.out())
    {
...
namespace detail {
    ...
    struct is_bit_reference_like
    {
        static constexpr const bool value =
            std::is_convertible<T, bool>::value && std::is_nothrow_assignable<T, bool>::value && has_flip<T>::value;
    };
...
struct formatter<BitRef, Char, enable_if_t<detail::is_bit_reference_like<BitRef>::value>> : formatter<bool, Char>
{
    template<typename FormatContext>
...
template<typename T, typename Char>
struct formatter<std::atomic<T>, Char, enable_if_t<is_formattable<T, Char>::value>> : formatter<T, Char>
{
    template<typename FormatContext>
...
{
    template<typename FormatContext>
    auto format(const std::atomic_flag& v, FormatContext& ctx) const -> decltype(ctx.out())
    {
...
private:
    ...
    FMT_CONSTEXPR auto
        do_format(const std::complex<T>& c, detail::dynamic_format_specs<Char>& specs, FormatContext& ctx, OutputIt out) const
        -> OutputIt
    {
        if (c.real() != 0) {
            *out++ = Char('(');
...
public:
    FMT_CONSTEXPR auto parse(parse_context<Char>& ctx) -> const Char*
    {
        ...
        return parse_format_specs(ctx.begin(), ctx.end(), specs_, ctx, detail::type_constant<T, Char>::value);
    }
    ...
    auto format(const std::complex<T>& c, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        return detail::write<Char>(ctx.out(), basic_string_view<Char>(buf.data(), buf.size()), outer_specs);
    }
};
