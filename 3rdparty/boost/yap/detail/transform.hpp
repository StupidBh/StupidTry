    ...
    namespace yap {
        namespace detail {
            ...
            struct placeholder_transform_t
            {
                ...
                constexpr decltype(auto) operator()(expr_tag<expr_kind::terminal>, boost::yap::placeholder<I>) const
                {
                    static_assert(
                    ...
                    return as_expr<minimal_expr>(rvalue_mover<!std::is_lvalue_reference<nth_type>::value> {}(
...
#define BOOST_YAP_UNARY_OPERATOR_CASE(op, op_name)                                 \
    ...
    {                                                                              \
        return op transform(as_expr<minimal_expr>(static_cast<T&&>(t)), *this);    \
    }
...
//]
                ...
                template<typename T, typename U>
                constexpr decltype(auto) operator()(expr_tag<expr_kind::subscript>, T&& t, U&& u) const
                {
                    return transform(
                        as_expr<minimal_expr>(static_cast<T&&>(t)),
                        *this)[transform(as_expr<minimal_expr>(static_cast<U&&>(u)), *this)];
                }

#undef BOOST_YAP_BINARY_OPERATOR_CASE

                template<typename T, typename U, typename V>
                constexpr decltype(auto) operator()(expr_tag<expr_kind::if_else>, T&& t, U&& u, V&& v) const
                {
                    return transform(as_expr<minimal_expr>(static_cast<T&&>(t)), *this)
                               ? transform(as_expr<minimal_expr>(static_cast<U&&>(u)), *this)
                               : transform(as_expr<minimal_expr>(static_cast<V&&>(v)), *this);
                }
...
//]
            ...
            template<bool Strict, typename Expr, typename TransformTuple, int I, expr_arity Arity, typename = void_t<>>
            ...
            template<
                ...
                typename OldTuple,
                typename NewTuple>
            constexpr auto make_expr_from_tuple(ExprTemplate<Kind, OldTuple> const& expr, NewTuple&& tuple)
            {
            ...
            constexpr decltype(auto) transform_nonterminal(Expr const& expr, Tuple&& tuple, TransformTuple transforms)
            {
                auto transformed_tuple = hana::transform(static_cast<Tuple&&>(tuple), [&](auto&& element) {
                    using element_t = decltype(element);
                    auto const kind = remove_cv_ref_t<element_t>::kind;
                    ::boost::yap::detail::transform_impl<false, 0, kind == expr_kind::expr_ref> xform;
            ...
            struct default_transform<false, false, false>
            {
                ...
                constexpr decltype(auto) operator()(Expr&& expr, TransformTuple transforms) const
                {
                    return transform_nonterminal(expr, std::move(expr.elements), transforms);
                }
            };
            ...
            template<bool Strict, typename Expr, typename TransformTuple, int I, bool NextTransformExists>
            struct next_or_default_transform
            {
                constexpr decltype(auto) operator()(Expr&& expr, TransformTuple transforms) const
                {
                    ...
                    return detail::transform_impl<Strict, I + 1, kind == expr_kind::expr_ref> {}(
            ...
            template<bool Strict, typename Expr, typename TransformTuple, int I, typename = detail::void_t<>>
            struct transform_expression_expr
            {
                constexpr decltype(auto) operator()(Expr&& expr, TransformTuple transforms) const
                {
                    ...
                    return next_or_default_transform<
                        ...
                        TransformTuple,
                        I,
                        I + 1 < decltype(hana::size(std::declval<TransformTuple>()))::value> {}(
                        static_cast<Expr&&>(expr),
                        transforms);
                }
            };
            ...
            struct transform_expression_expr<
                ...
                TransformTuple,
                I,
                void_t<decltype((*std::declval<TransformTuple>()[hana::llong<I> {}])(std::declval<Expr>()))>>
            {
                constexpr decltype(auto) operator()(Expr&& expr, TransformTuple transforms) const
                {
            ...
            template<bool Strict, typename Expr, typename TransformTuple, int I, expr_arity Arity, typename>
            struct transform_expression_tag
            {
                constexpr decltype(auto) operator()(Expr&& expr, TransformTuple transforms) const
                {
            ...
            struct transform_call_unpacker
            {
                template<long long... I>
                constexpr auto
                    operator()(Expr&& expr, Transform& transform, std::integer_sequence<long long, I...>) const
                    -> decltype(transform(
                        expr_tag<expr_kind::call> {},
                        terminal_value(::boost::yap::get(static_cast<Expr&&>(expr), hana::llong_c<I>))...))
                {
                    return transform(
                        expr_tag<expr_kind::call> {},
                        terminal_value(::boost::yap::get(static_cast<Expr&&>(expr), hana::llong_c<I>))...);
            ...
            struct transform_expression_tag<
                ...
                I,
                expr_arity::n,
                void_t<decltype(transform_call_unpacker<
                                Expr,
            ...
            struct transform_impl
            {
                ...
                constexpr decltype(auto) operator()(Expr&& expr, TransformTuple transforms) const
                {
                    ...
                    return detail::
                        transform_expression_tag<Strict, Expr, TransformTuple, I, detail::arity_of<kind>()> {}(
                            static_cast<Expr&&>(expr),
                            transforms);
                }
            };
