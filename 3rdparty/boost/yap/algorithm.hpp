...
namespace boost {
    namespace yap {
        ...
        namespace detail {
            ...
            struct deref_impl
            {
                constexpr decltype(auto) operator()(Expr&& expr) { return std::move(*expr.elements[hana::llong_c<0>]); }
            };

            template<typename Expr>
            struct deref_impl<Expr, false>
            {
                constexpr decltype(auto) operator()(Expr&& expr) { return *expr.elements[hana::llong_c<0>]; }
            };
        }
        ...
        {
            static_assert(is_expr<Expr>::value, "deref() is only defined for expressions.");
            ...
            return detail::deref_impl<
                Expr,
                std::is_rvalue_reference<Expr>::value && !std::is_const<std::remove_reference_t<Expr>>::value> {}(
        ...
        namespace detail {
            ...
            struct lvalue_ref_ith_element : std::is_lvalue_reference<decltype(std::declval<Tuple>()[hana::llong<I> {}])>
            {
            };
            ...
            {
                constexpr decltype(auto) operator()(T&& x) { return x.elements[hana::llong_c<0>]; }
            };

            template<typename T, bool ValueOfTerminalsOnly>
            struct value_expr_impl<T, false, ValueOfTerminalsOnly, true, false>
            {
                constexpr decltype(auto) operator()(T&& x) { return std::move(x.elements[hana::llong_c<0>]); }
            };
            ...
            {
                constexpr decltype(auto) operator()(T&& x) { return static_cast<T&&>(x); }
            };
                ...
                constexpr decltype(auto) operator()(T&& x)
                {
                    ...
                    return value_expr_impl<
                        ...
                        std::is_lvalue_reference<T>::value ||
                            detail::lvalue_ref_ith_element<decltype(x.elements), 0>::value> {}(static_cast<T&&>(x));
                }
            };
            ...
            constexpr decltype(auto) value_impl(T&& x)
            {
                return detail::value_impl_t<T, is_expr<T>::value, ValueOfTerminalsOnly> {}(static_cast<T&&>(x));
            }
            ...
            constexpr decltype(auto) value_impl(T&& x)
            {
                if constexpr (is_expr<T>::value) {
                    ...
                    if constexpr (kind == expr_kind::expr_ref) {
                        ...
                        kind == expr_kind::terminal || (!ValueOfTerminalsOnly && arity == expr_arity::one)) {
                        if constexpr (
                            std::is_lvalue_reference<T>::value ||
                            detail::lvalue_ref_ith_element<decltype(x.elements), 0> {}) {
        ...
        namespace detail {
            ...
            {
                constexpr decltype(auto) operator()(Expr&& expr, hana::llong<I> i) { return expr.elements[i]; }
            };
        ...
        */
        ...
        constexpr decltype(auto) get(Expr&& expr, I const& i)
        {
            static_assert(is_expr<Expr>::value, "get() is only defined for expressions.");
            static_assert(hana::IntegralConstant<I>::value, "'i' must be an IntegralConstant");

            constexpr expr_kind kind = detail::remove_cv_ref_t<Expr>::kind;

            static_assert(
                kind == expr_kind::expr_ref || (0 <= I::value && I::value < decltype(hana::size(expr.elements))::value),
                "In get(expr, I), I must be a valid index into expr's tuple "
                "elements.");

#ifdef BOOST_NO_CONSTEXPR_IF
            return detail::
                get_impl<I::value, Expr, kind == expr_kind::expr_ref, std::is_lvalue_reference<Expr>::value> {}(
                    static_cast<Expr&&>(expr),
                    i);
#else
            using namespace hana::literals;
        ...
        */
        ...
        constexpr decltype(auto) left(Expr&& expr)
        {
            ...
            constexpr expr_kind kind = detail::remove_cv_ref_t<Expr>::kind;
            static_assert(
                kind == expr_kind::expr_ref || detail::arity_of<kind>() == detail::expr_arity::two,
        ...
        */
        ...
        constexpr decltype(auto) callable(Expr&& expr)
        {
            ...
            constexpr expr_kind kind = detail::remove_cv_ref_t<Expr>::kind;
            static_assert(
                kind == expr_kind::expr_ref || detail::arity_of<kind>() == detail::expr_arity::n,
        ...
        */
        ...
        constexpr decltype(auto) argument(Expr&& expr, hana::llong<I> i)
        {
            ...
            static_assert(
                kind == expr_kind::expr_ref || (0 <= I && I < decltype(hana::size(expr.elements))::value - 1),
        ...
        constexpr auto make_expression(T&&... t)
        {
            ...
            static_assert(
                (arity == detail::expr_arity::one && sizeof...(T) == 1) ||
                    ...
                    (arity == detail::expr_arity::three && sizeof...(T) == 3) || arity == detail::expr_arity::n,
        ...
        */
        ...
        {
            static_assert(!is_expr<T>::value, "make_terminal() is only defined for non expressions.");
        ...
        namespace detail {
            ...
            struct as_expr_impl
            {
                constexpr decltype(auto) operator()(T&& t) { return static_cast<T&&>(t); }
            };

            template<template<expr_kind, class> class ExprTemplate, typename T>
            struct as_expr_impl<ExprTemplate, T, false>
            {
                constexpr decltype(auto) operator()(T&& t) { return make_terminal<ExprTemplate>(static_cast<T&&>(t)); }
            };
        }
        ...
        */
        ...
        {
#ifdef BOOST_NO_CONSTEXPR_IF
            return detail::as_expr_impl<ExprTemplate, T, is_expr<T>::value> {}(static_cast<T&&>(t));
#else
            if constexpr (is_expr<T>::value) {
        ...
        */
        ...
        {
            static_assert(is_expr<Expr>::value, "make_expression_function() is only defined for expressions.");
            using stored_type = detail::operand_type_t<detail::expression_function_expr, Expr&&>;
            return expression_function<stored_type> { detail::make_operand<stored_type> {}(static_cast<Expr&&>(expr)) };
        }
    }
}
...
namespace boost {
    namespace yap {
        ...
        */
        ...
        {
            static_assert(is_expr<Expr>::value, "evaluate() is only defined for expressions.");
            return transform(static_cast<Expr&&>(expr), replacements(static_cast<T&&>(t)...));
        }
        ...
        */
        ...
        {
            static_assert(is_expr<Expr>::value, "evaluate() is only defined for expressions.");
            return transform(static_cast<Expr&&>(expr), evaluation(static_cast<T&&>(t)...));
        }
        ...
        namespace detail {
            ...
            struct transform_
            {
                ...
                constexpr decltype(auto) operator()(Expr&& expr, Transform& transform, Transforms&... transforms) const
                {
                    auto transform_tuple = detail::make_transform_tuple(transform, transforms...);
                    constexpr expr_kind kind = detail::remove_cv_ref_t<Expr>::kind;
                    return detail::transform_impl<Strict, 0, kind == expr_kind::expr_ref> {}(
                        static_cast<Expr&&>(expr),
                        transform_tuple);
                }
            };
        ...
        */
        template<typename Expr, typename Transform, typename... Transforms>
        constexpr decltype(auto) transform(Expr&& expr, Transform&& transform, Transforms&&... transforms)
        {
            static_assert(is_expr<Expr>::value, "transform() is only defined for expressions.");
            return detail::transform_<false> {}(static_cast<Expr&&>(expr), transform, transforms...);
        }
        ...
        */
        template<typename Expr, typename Transform, typename... Transforms>
        constexpr decltype(auto) transform_strict(Expr&& expr, Transform&& transform, Transforms&&... transforms)
        {
            static_assert(is_expr<Expr>::value, "transform() is only defined for expressions.");
            return detail::transform_<true> {}(static_cast<Expr&&>(expr), transform, transforms...);
        }
