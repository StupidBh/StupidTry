...
namespace boost {
    namespace spirit {
        namespace karma {
            ...
            inline bool generate(OutputIterator& sink, Expr const& expr)
            {
                return detail::generate_impl<Expr>::call(sink, expr);
            }

            template<typename OutputIterator, typename Expr>
            inline bool generate(OutputIterator const& sink_, Expr const& expr)
            {
            ...
            inline bool
                generate(detail::output_iterator<OutputIterator, Properties>& sink, Expr const& expr, Attr const& attr)
            {
            ...
            inline bool generate(OutputIterator& sink_, Expr const& expr, Attr const& attr)
            {
                // Report invalid expression error as early as possible.
                // If you got an error_invalid_expression error message here,
                // then the expression (expr) is not a valid spirit karma expression.
            ...
            template<typename OutputIterator, typename Expr, typename Attr>
            inline bool generate(OutputIterator const& sink_, Expr const& expr, Attr const& attr)
            {
                OutputIterator sink = sink_;
                return karma::generate(sink, expr, attr);
            }
            ...
            {
                return detail::generate_delimited_impl<Expr>::call(sink, expr, delimiter, pre_delimit);
            }
                ...
                Attribute const& attr)
            {
                ...
                return compile<karma::domain>(expr).generate(sink, context, delimiter_, attr);
            }
            ...
            inline bool generate_delimited(
                ...
                Attribute const& attr)
            {
                return karma::generate_delimited(sink, expr, delimiter, delimit_flag::dont_predelimit, attr);
            }
