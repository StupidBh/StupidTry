...
namespace boost {
    namespace spirit {
        namespace traits {
            ...
            {
                typedef mpl::
                    eval_if<fusion::traits::is_sequence<T>, traits::detail::value_at_c<T, 0>, detail::add_const_ref<T>>
                        sequence_type;
                typedef typename mpl::eval_if<traits::is_container<T>, traits::container_value<T>, sequence_type>::type
                    type;
            ...
            struct symbols_lookup<Attribute, Attribute>
            {
                ...
                static type call(Attribute const& t) { return t; }
            };
            ...
            struct symbols_value<Attribute, Attribute>
            {
                ...
                static type call(Attribute const&) { return unused; }
            };
        }
    }
}
...
namespace boost {
    namespace spirit {
        namespace karma {
            ...
            struct symbols_lookup : mpl::if_<traits::not_is_unused<T>, std::map<Attribute, T>, std::set<Attribute>>
            {
            };
            ...
            namespace detail {
                ...
                {
                    typedef typename proto::terminal<tag::char_code<Tag, CharEncoding>>::type encoding_type;
            ...
            struct symbols :
                proto::extends<
                    typename proto::terminal<reference<symbols<Attribute, T, Lookup, CharEncoding, Tag>>>::type,
                    symbols<Attribute, T, Lookup, CharEncoding, Tag>>,
                primitive_generator<symbols<Attribute, T, Lookup, CharEncoding, Tag>>
            {
                ...
                template<typename Attr, typename T_>
                friend adder const& operator+=(symbols& sym, std::pair<Attr, T_> const& p)
                {
                    return sym.add(p.first, p.second);
                }

                template<typename Attr>
                friend remover const& operator-=(symbols& sym, Attr const& attr)
                {
                ...
                template<typename Attr, typename T_>
                friend adder const& operator+=(symbols& sym, std::pair<Attr, T_>& p)
                {
                    return sym.add(p.first, p.second);
                }
                ...
                // non-const version needed to suppress proto's -= kicking in
                template<typename Attr>
                friend remover const& operator-=(symbols& sym, Attr& attr)
                {
                    return sym.remove(attr);
                }
    #else
                ...
                {
                    typename Lookup::iterator it = lookup->find(traits::symbols_lookup<Attr, Attribute>::call(attr));
                    if (it == lookup->end()) {
                        return false;
                    }
                ...
                void name(std::string const& str) { name_ = str; }
        ...
        #pragma warning(disable: 4512) // assignment operator could not be generated.
                ...
                struct adder
                {
                    ...
                    template<typename Attr>
                    adder const& operator()(Attr const& attr, T const& val = T()) const
                    {
                    ...
                    template<typename Attr>
                    adder const& operator,(Attr const& attr) const
                    {
            ...
            struct symbols<Attribute, unused_type, Lookup, CharEncoding, Tag> :
                proto::extends<
                    typename proto::terminal<
                        spirit::karma::reference<symbols<Attribute, unused_type, Lookup, CharEncoding, Tag>>>::type,
                    symbols<Attribute, unused_type, Lookup, CharEncoding, Tag>>,
                spirit::karma::generator<symbols<Attribute, unused_type, Lookup, CharEncoding, Tag>>
            {
                ...
                template<typename CharEncoding_, typename Tag_>
                symbols& operator=(symbols<Attribute, unused_type, Lookup, CharEncoding_, Tag_> const& rhs)
                {
                    *lookup = *rhs.lookup;
                    name_ = rhs.name_;
                    return *this;
                ...
                template<typename Attr>
                friend adder const& operator+=(symbols& sym, Attr const& attr)
                {
                    return sym.add(attr);
                }
                ...
                // non-const version needed to suppress proto's += kicking in
                template<typename Attr>
                friend adder const& operator+=(symbols& sym, Attr& attr)
                {
                    return sym.add(attr);
                }
        ...
        #pragma warning(pop)
            ...
            struct make_primitive<reference<symbols<Attribute, T, Lookup, CharEnconding, Tag>>, Modifiers>
            {
                ...
                typedef reference<symbols<Attribute, T, Lookup, CharEnconding, Tag>> reference_;
                ...
                typedef typename mpl::if_c<
                    ...
                    reference_>::type result_type;

                result_type operator()(reference_ ref, unused_type) const { return result_type(ref.ref.get()); }
            };
        }
    }
}
