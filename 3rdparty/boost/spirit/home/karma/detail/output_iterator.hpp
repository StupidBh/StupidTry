    ...
    namespace spirit {
        namespace karma {
            namespace detail {
                ...
                struct position_policy
                {
                    ...
                    std::size_t get_out_count() const { return track_position_data.get_count(); }

                    // return the current line in the output
                    std::size_t get_line() const { return track_position_data.get_line(); }

                    // return the current column in the output
                    std::size_t get_column() const { return track_position_data.get_column(); }
                ...
                public:
                    counting_sink(OutputIterator& sink_, std::size_t count_ = 0, bool enabled = true) :
                        count(count_),
                        initial_count(count),
                        prev_count(0),
                        sink(sink_)
                    ...
                    void update_count(std::size_t c) { count += c; }
                ...
                public:
                    ...
                    counting_policy(counting_policy const& rhs) :
                        count(rhs.count)
                    {
                    }

                    // functions related to counting
                    counting_sink<OutputIterator>* chain_counting(counting_sink<OutputIterator>* count_data)
                    {
        ...
        #pragma warning(pop)
                ...
                public:
                    ...
                    buffering_policy(buffering_policy const& rhs) :
                        buffer(rhs.buffer)
                    {
                    }
                ...
                template<typename OutputIterator, typename Properties, typename Derived = unused_type>
                ...
                struct make_output_iterator
                {
                    ...
                    typedef typename mpl::if_<
                        traits::not_is_unused<Derived>,
                        Derived,
                        output_iterator<OutputIterator, Properties, Derived>>::type most_derived_type;

                    static const generator_properties::enum_type properties =
                        static_cast<generator_properties::enum_type>(Properties::value);

                    typedef typename mpl::if_c<
                        (properties & generator_properties::tracking) ? true : false,
                        position_policy,
                        no_position_policy>::type tracking_type;
                    ...
                    typedef typename mpl::if_c<
                        ...
                        counting_policy<most_derived_type>,
                        no_counting_policy>::type counting_type;

                    typedef typename mpl::if_c<
                        (properties & generator_properties::disabling) ? true : false,
                        disabling_output_iterator<buffering_type, counting_type, tracking_type>,
                        output_iterator_base<buffering_type, counting_type, tracking_type>>::type type;
                };
                ...
                ///////////////////////////////////////////////////////////////////////////
                template<typename OutputIterator, typename Properties, typename Derived>
                class output_iterator : public make_output_iterator<OutputIterator, Properties, Derived>::type {
                private:
                    // base iterator type
                    typedef typename make_output_iterator<OutputIterator, Properties, Derived>::type base_iterator;
        ...
        #pragma warning(pop)
                ...
                class output_iterator<karma::ostream_iterator<T, Elem, Traits>, Properties> :
                    public output_iterator<
                        karma::ostream_iterator<T, Elem, Traits>,
                        Properties,
                ...
                struct enable_counting
                {
                    ...
                    std::size_t count() const { return count_data.get_count(); }
                ...
                struct enable_buffering
                {
                    enable_buffering(OutputIterator& sink_, std::size_t width = std::size_t(-1)) :
                        sink(sink_),
                        prev_buffer(NULL),
                        enabled(false)
                    {
                    ...
                    bool buffer_copy(std::size_t maxwidth = std::size_t(-1), bool disable_ = true)
                    {
                        if (disable_) {
                            disable();
                        }
                    ...
                    std::size_t buffer_size() const { return buffer_data.buffer_size(); }
                    ...
                    template<typename OutputIterator_>
                    bool buffer_copy_to(OutputIterator_& sink, std::size_t maxwidth = std::size_t(-1)) const
                    {
                ...
                struct disable_output
                {
                    disable_output(OutputIterator& sink_) :
                        sink(sink_),
                        prev_do_output(sink.do_output)
                    {
                        sink.do_output = false;
                    }

                    ~disable_output() { sink.do_output = prev_do_output; }
