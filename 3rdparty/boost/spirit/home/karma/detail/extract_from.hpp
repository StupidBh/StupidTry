    ...
    namespace spirit {
        namespace traits {
            ...
            namespace detail {
                ...
                template<typename T>
                struct add_const_ref : add_reference<typename add_const<T>::type>
                {
                };

                template<typename T, int N>
                struct value_at_c : add_const_ref<typename fusion::result_of::value_at_c<T, N>::type>
                {
                };
            }
            ...
            struct extract_from_attribute_base<Attribute, Exposed, true>
            {
                typedef typename remove_const<
                    typename remove_reference<typename fusion::result_of::at_c<Attribute, 0>::type>::type>::type
                    elem_type;
            ...
            struct extract_from_attribute : extract_from_attribute_base<Attribute, Exposed>
            {
            };
            ...
            {
                typedef typename traits::container_value<Attribute const>::type value_type;
                typedef typename is_convertible<value_type, Exposed>::type is_convertible_to_value_type;

                typedef typename mpl::if_<
                    mpl::or_<is_same<value_type, Exposed>, is_same<Attribute, Exposed>>,
                    Exposed const&,
                    Exposed>::type type;
                ...
                static type call(Attribute const& attr, Context&, mpl::false_, mpl::true_)
                {
                    ...
                    Exposed result;
                    append_to_string(result, traits::get_begin<char_type>(attr), traits::get_end<char_type>(attr));
                    return result;
                }
                ...
                static type call(Attribute const& attr, Context& ctx)
                {
                    typedef typename mpl::and_<traits::is_string<Exposed>, traits::is_string<Attribute>>::type
                        handle_strings;

                    // return first element from container
                    return call(attr, ctx, is_convertible_to_value_type(), handle_strings());
                }
            };
            ...
            template<typename Exposed, typename Attribute, typename Context>
            inline typename spirit::result_of::extract_from<Exposed, Attribute>::type extract_from(
                Attribute const& attr,
                Context& ctx
    #if (defined(__GNUC__) && (__GNUC__ < 4)) || (defined(__APPLE__) && defined(__INTEL_COMPILER))
                ,
            ...
            )
            {
                ...
                return detail::extract_from<Exposed>(attr, ctx, is_not_wrapped_container());
            }
