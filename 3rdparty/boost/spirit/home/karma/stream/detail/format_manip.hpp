...
namespace boost {
    namespace spirit {
        ...
        #pragma warning(disable: 4512) // assignment operator could not be generated.
                ...
                struct format_manip
                {
                    ...
                    format_manip(Expr const& xpr, Delimiter const& d, Attribute const& a) :
                        expr(xpr),
                        delim(d),
                        pre(delimit_flag::dont_predelimit),
                        attr(a)
                    {
                    }
        ...
        #pragma warning(pop)
                ...
                struct format<Expr, typename enable_if<traits::matches<karma::domain, Expr>>::type>
                {
                ...
                struct format_delimited<Expr, Delimiter, typename enable_if<traits::matches<karma::domain, Expr>>::type>
                {
                    typedef format_manip<Expr, mpl::false_, mpl::false_, Delimiter> type;

                    static type
                        call(Expr const& expr, Delimiter const& delimiter, BOOST_SCOPED_ENUM(delimit_flag) pre_delimit)
                    {
                        // Report invalid expression error as early as possible.
                        // If you got an error_invalid_expression error message here,
                ...
                inline std::basic_ostream<Char, Traits>&
                    operator<<(std::basic_ostream<Char, Traits>& os, format_manip<Expr, CopyExpr, CopyAttr> const& fm)
                {
