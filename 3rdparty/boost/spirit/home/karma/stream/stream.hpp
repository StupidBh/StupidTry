...
namespace boost {
    namespace spirit {
        ...
        namespace karma {
            ...
            template<typename Char = char>
            struct stream_generator : spirit::terminal<tag::stream_tag<Char>>
            {
            };
        }
        ...
        struct use_terminal<
            karma::domain // enables stream(...)
            ,
            terminal_ex<tag::stream, fusion::vector1<A0>>> : mpl::true_
        {
        };

        template<typename A0>
        struct use_terminal<
            karma::domain // enables wstream(...)
            ,
            terminal_ex<tag::wstream, fusion::vector1<A0>>> : mpl::true_
        {
        };
        ...
        struct use_terminal<karma::domain, tag::stream_tag<Char>> : mpl::true_
        {
        };

        template<typename Char, typename A0>
        struct use_terminal<karma::domain, terminal_ex<tag::stream_tag<Char>, fusion::vector1<A0>>> : mpl::true_
        {
        };
...
namespace boost {
    namespace spirit {
        ...
        #pragma warning(disable: 4512) // assignment operator could not be generated.
    #endif
                template<typename OutputIterator, typename Char, typename CharEncoding, typename Tag>
                struct psbuf : std::basic_streambuf<Char>
                {
                    psbuf(OutputIterator& sink) :
                        sink_(sink)
                    {
                    }
                ...
                protected:
                    typename psbuf::int_type overflow(typename psbuf::int_type ch) BOOST_OVERRIDE
                    {
                        ...
                        return detail::generate_to(sink_, psbuf::traits_type::to_char_type(ch), CharEncoding(), Tag())
                                   ? ch
                                   : psbuf::traits_type::eof();
                    }
        ...
        #pragma warning(pop)
            ...
            struct any_stream_generator : primitive_generator<any_stream_generator<Char, CharEncoding, Tag>>
            {
                ...
                static bool generate(
                    karma::detail::output_iterator<karma::ostream_iterator<T, Char, Traits>, Properties>& sink,
                    Context& context,
                    Delimiter const& d,
                    Attribute const& attr)
                {
                    typedef karma::detail::output_iterator<karma::ostream_iterator<T, Char, Traits>, Properties>
                        output_iterator;
            ...
            struct lit_stream_generator : primitive_generator<lit_stream_generator<T, Char, CharEncoding, Tag>>
            {
                ...
                template<
                    ...
                    typename Attribute>
                bool generate(
                    karma::detail::output_iterator<karma::ostream_iterator<T1, Char, Traits>, Properties>& sink,
                    Context&,
                    Delimiter const& d,
                    Attribute const&) const
                {
                    typedef karma::detail::output_iterator<karma::ostream_iterator<T1, Char, Traits>, Properties>
                        output_iterator;
            ...
            struct make_primitive<tag::stream, Modifiers> : make_stream<char, Modifiers>
            {
            };

            // wstream
            template<typename Modifiers>
            struct make_primitive<tag::wstream, Modifiers> : make_stream<wchar_t, Modifiers>
            {
            };

            // any_stream_generator<char_type>
            template<typename Char, typename Modifiers>
            struct make_primitive<tag::stream_tag<Char>, Modifiers> : make_stream<Char, Modifiers>
            {
            };
