    ...
    namespace spirit {
        namespace karma {
            ...
            struct real_inserter
            {
                ...
                static bool call(OutputIterator& sink, U n, Policies const& p = Policies())
                {
                    if (traits::test_nan(n)) {
                        return p.template nan<CharEncoding, Tag>(sink, n, p.force_sign(n));
                    }
                    else if (traits::test_infinite(n)) {
                        return p.template inf<CharEncoding, Tag>(sink, n, p.force_sign(n));
                    }
        ...
        #pragma warning(disable: 4267) // conversion from 'size_t' to 'unsigned int', possible loss of data
                ...
                ///////////////////////////////////////////////////////////////////////
                template<typename OutputIterator, typename U>
                static bool call_n(OutputIterator& sink, U n, Policies const& p)
                {
                    ...
                    if (precexp_offset) {
                        fractional_part = floor((fractional_part * precexp + U(0.5)) * U(10.)) / U(10.);
                    }
                    else {
                    ...
                    if (!p.trailing_zeros(n)) {
                        ...
                        if (0 != long_frac_part) {
                            ...
                            while (0 != prec && 0 == traits::remainder<10>::call(long_frac_part)) {
                        ...
                        if (precision != prec) {
                            long_frac_part = frac_part_floor / spirit::traits::pow10<U>(precision - prec);
                        }
                    }

                    // call the actual generating functions to output the different parts
                    if ((force_sign || sign_val) && traits::test_zero(long_int_part) &&
