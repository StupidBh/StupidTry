...
namespace boost {
    namespace spirit {
        ...
        // enables center[]
        template<>
        struct use_directive<karma::domain, tag::center> : mpl::true_
        {
        };
        ...
        template<typename T>
        struct use_directive<karma::domain, terminal_ex<tag::center, fusion::vector1<T>>> : mpl::true_
        {
        };

        // enables *lazy* center(d)[g], where d provides a generator
        template<>
        struct use_lazy_directive<karma::domain, tag::center, 1> : mpl::true_
        {
        };
        ...
        // a maximum width
        template<>
        struct use_lazy_directive<karma::domain, tag::center, 2> : mpl::true_
        {
        };
        ...
        namespace karma {
            ...
            namespace detail {
                ...
                template<
                    ...
                    typename Embedded,
                    typename Padding>
                inline static bool center_generate(
                    OutputIterator& sink,
                    Context& ctx,
                    Delimiter const& d,
            ...
            struct simple_center_alignment : unary_generator<simple_center_alignment<Subject, Width>>
            {
                ...
                {
                    return detail::center_generate(sink, ctx, d, attr, subject, width, compile<karma::domain>(' '));
                }
            ...
            struct padding_center_alignment : unary_generator<padding_center_alignment<Subject, Padding, Width>>
            {
                typedef Subject subject_type;
                ...
                bool generate(OutputIterator& sink, Context& ctx, Delimiter const& d, Attribute const& attr) const
                {
                    return detail::center_generate(sink, ctx, d, attr, subject, width, padding);
                }
