...
namespace boost {
    namespace spirit {
        ...
        // enables maxwidth[]
        template<>
        struct use_directive<karma::domain, tag::maxwidth> : mpl::true_
        {
        };

        // enables maxwidth(w)[g], where w provides a maxwidth
        template<typename T>
        struct use_directive<karma::domain, terminal_ex<tag::maxwidth, fusion::vector1<T>>> : mpl::true_
        {
        };

        // enables *lazy* maxwidth(w)[g], where w provides a maxwidth
        template<>
        struct use_lazy_directive<karma::domain, tag::maxwidth, 1> : mpl::true_
        {
        };
        ...
        struct use_directive<karma::domain, terminal_ex<tag::maxwidth, fusion::vector2<T, RestIter>>> : mpl::true_
        {
        };
        ...
        template<>
        struct use_lazy_directive<karma::domain, tag::maxwidth, 2> : mpl::true_
        {
        };
    ...
    namespace spirit {
        namespace karma {
            ...
            namespace detail {
                ...
                bool buffer_copy_rest(
                    detail::enable_buffering<OutputIterator>& buff,
                    std::size_t start_at,
                    RestIterator& dest)
                {
                ...
                template<
                    ...
                    typename Embedded,
                    typename Rest>
                inline static bool maxwidth_generate(
                    OutputIterator& sink,
                    Context& ctx,
                    Delimiter const& d,
                    ...
                    Rest& restdest)
                {
                    ...
                    return r && buffering.buffer_copy(maxwidth) && buffer_copy_rest(buffering, maxwidth, restdest);
                }
            }
            ...
            struct maxwidth_width : unary_generator<maxwidth_width<Subject, Width, Rest>>
            {
                ...
                {
                    return detail::maxwidth_generate(sink, ctx, d, attr, subject, width, rest);
                }
            ...
            template<typename T, typename RestIter, typename Subject, typename Modifiers>
            struct make_directive<terminal_ex<tag::maxwidth, fusion::vector2<T, RestIter>>, Subject, Modifiers>
            {
                ...
                result_type operator()(Terminal const& term, Subject const& subject, unused_type) const
                {
                    return result_type(subject, fusion::at_c<0>(term.args), fusion::at_c<1>(term.args));
                }
            };
...
namespace boost {
    namespace spirit {
        namespace traits {
            ...
            struct has_semantic_action<karma::maxwidth_width<Subject, Width, Rest>> : unary_has_semantic_action<Subject>
            {
            };
