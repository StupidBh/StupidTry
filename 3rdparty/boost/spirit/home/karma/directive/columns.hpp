...
namespace boost {
    namespace spirit {
        ...
        template<typename T>
        struct use_directive<karma::domain, terminal_ex<tag::columns, fusion::vector1<T>>> : mpl::true_
        {
        };

        // enables *lazy* columns(c)[g]
        template<>
        struct use_lazy_directive<karma::domain, tag::columns, 1> : mpl::true_
        {
        };
        ...
        template<>
        struct use_lazy_directive<karma::domain, tag::columns, 2> : mpl::true_
        {
        };
...
namespace boost {
    namespace spirit {
        ...
        #pragma warning(pop)
            ...
            struct columns_generator : unary_generator<columns_generator<Subject, NumColumns, ColumnsDelimiter>>
            {
                typedef Subject subject_type;
                typedef ColumnsDelimiter delimiter_type;

                typedef mpl::int_<subject_type::properties::value | delimiter_type::properties::value> properties;
                ...
                columns_generator(Subject const& subject, NumColumns const& cols, ColumnsDelimiter const& cdelimiter) :
                    subject(subject),
                    numcolumns(cols),
                    column_delimiter(cdelimiter)
                {
                ...
                bool generate(OutputIterator& sink, Context& ctx, Delimiter const& delimiter, Attribute const& attr)
                    const
                {
                    //  The columns generator dispatches to the embedded generator
                    //  while supplying a new delimiter to use, wrapping the outer
                    //  delimiter.
                    typedef detail::columns_delimiter<Delimiter, ColumnsDelimiter> columns_delimiter_type;
            ...
            {
                typedef typename result_of::compile<karma::domain, eol_type, Modifiers>::type columns_delimiter_type;
                typedef columns_generator<Subject, detail::default_columns, columns_delimiter_type> result_type;
    ...
    #endif
                    return result_type(subject, detail::default_columns(), compile<karma::domain>(eol));
                }
            };
            ...
            {
                typedef typename result_of::compile<karma::domain, eol_type, Modifiers>::type columns_delimiter_type;
                typedef columns_generator<Subject, T, columns_delimiter_type> result_type;
                ...
                result_type operator()(Terminal const& term, Subject const& subject, unused_type) const
                {
                    ...
                    eol_type const eol = eol_type();
    #endif
                    return result_type(subject, fusion::at_c<0>(term.args), compile<karma::domain>(eol));
                }
            };
            ...
            struct make_directive<
                ...
                typename enable_if<mpl::and_<
                    ...
                    mpl::not_<mpl::bool_<integer_traits<T>::is_integral>>>>::type>
            {
                typedef typename result_of::compile<karma::domain, T, Modifiers>::type columns_delimiter_type;
                typedef columns_generator<Subject, detail::default_columns, columns_delimiter_type> result_type;
            ...
            {
                typedef typename result_of::compile<karma::domain, T2, Modifiers>::type columns_delimiter_type;
                typedef columns_generator<Subject, T1, columns_delimiter_type> result_type;
...
namespace boost {
    namespace spirit {
        namespace traits {
            ...
            struct has_semantic_action<karma::columns_generator<Subject, T1, T2>> : unary_has_semantic_action<Subject>
            {
            };
