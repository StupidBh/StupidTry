...
namespace boost {
    namespace spirit {
        ...
        // enables right_align[]
        template<>
        struct use_directive<karma::domain, tag::right_align> : mpl::true_
        {
        };
        ...
        template<typename T>
        struct use_directive<karma::domain, terminal_ex<tag::right_align, fusion::vector1<T>>> : mpl::true_
        {
        };

        // enables *lazy* right_align(d)[g], where d provides a generator
        template<>
        struct use_lazy_directive<karma::domain, tag::right_align, 1> : mpl::true_
        {
        };
        ...
        // is a maximum width
        template<>
        struct use_lazy_directive<karma::domain, tag::right_align, 2> : mpl::true_
        {
        };
        ...
        namespace karma {
            ...
            namespace detail {
                ...
                template<
                    ...
                    typename Embedded,
                    typename Padding>
                inline static bool right_align_generate(
                    OutputIterator& sink,
                    Context& ctx,
                    Delimiter const& d,
            ...
            struct simple_right_alignment : unary_generator<simple_right_alignment<Subject, Width>>
            {
                typedef Subject subject_type;

                typedef mpl::int_<generator_properties::countingbuffer | subject_type::properties::value> properties;
                ...
                {
                    return detail::right_align_generate(
                        sink,
                        ctx,
                        d,
                        attr,
            ...
            struct padding_right_alignment : unary_generator<padding_right_alignment<Subject, Padding, Width>>
            {
                typedef Subject subject_type;
                ...
                padding_right_alignment(Subject const& subject, Padding const& padding, Width width = Width()) :
                    subject(subject),
                    padding(padding),
                    width(width)
                {
                ...
                bool generate(OutputIterator& sink, Context& ctx, Delimiter const& d, Attribute const& attr) const
                {
                    return detail::right_align_generate(sink, ctx, d, attr, subject, width, padding);
                }
