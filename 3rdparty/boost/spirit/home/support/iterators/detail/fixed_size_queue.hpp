...
//  file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
    ...
    ///////////////////////////////////////////////////////////////////////////////
    #define BOOST_SPIRIT_ASSERT_FSQ_SIZE BOOST_ASSERT(((m_tail + N + 1) - m_head) % (N + 1) == m_size % (N + 1))
...
namespace boost {
    namespace spirit {
        namespace detail {
            ...
            class fsq_iterator :
                public boost::iterator_facade<fsq_iterator<Queue, T, Pointer>, T, std::random_access_iterator_tag> {
            public:
                typedef typename Queue::position_type position_type;
                typedef boost::iterator_facade<fsq_iterator<Queue, T, Pointer>, T, std::random_access_iterator_tag>
                    base_type;

                fsq_iterator() {}

                fsq_iterator(position_type const& p_) :
                    p(p_)
                {
                }
            ...
            private:
                ...
                typename base_type::reference dereference() const { return p.self->m_queue[p.pos]; }
                ...
                bool is_eof() const { return p.self == 0 || p.pos == p.self->size(); }
                ...
                template<typename Q, typename T_, typename P>
                typename base_type::difference_type distance_to(fsq_iterator<Q, T_, P> const& x) const
                {
            ...
            private:
                struct position
                {
                    ...
                    position(const fixed_size_queue* s, std::size_t p) :
                        self(const_cast<fixed_size_queue*>(s)),
                        pos(p)
                    {
                    }
            ...
            public:
                ...
                iterator begin() { return iterator(position(this, m_head)); }

                const_iterator begin() const { return const_iterator(position(this, m_head)); }

                iterator end() { return iterator(position(0, m_tail)); }

                const_iterator end() const { return const_iterator(position(0, m_tail)); }

                std::size_t size() const { return m_size; }

                T& front() { return m_queue[m_head]; }

                const T& front() const { return m_queue[m_head]; }
