...
namespace boost {
    namespace spirit {
        ...
        {
            typename std::iterator_traits<Iterator>::reference ch = *(this->base());
        ...
        template<class Iterator>
        inline iterator_range<Iterator> get_current_line(Iterator lower_bound, Iterator current, Iterator upper_bound);
        /*`Get an `iterator_range` containing the current line. Applicable to any
           iterator. */

        //`[heading get_column]
        template<class Iterator>
        inline std::size_t get_column(Iterator lower_bound, Iterator current, std::size_t tabs = 4);
        ...
        namespace detail {
            ...
            template<class Iterator>
            inline Iterator get_line_start(Iterator lower_bound, Iterator current, std::forward_iterator_tag)
            {
            ...
            template<class Iterator>
            inline Iterator get_line_start(Iterator lower_bound, Iterator current, std::bidirectional_iterator_tag)
            {
        ...
        inline line_pos_iterator<Iterator>
            get_line_start(line_pos_iterator<Iterator> lower_bound, line_pos_iterator<Iterator> current)
        {
        ...
        template<class Iterator>
        inline line_pos_iterator<Iterator>
            get_line_end(line_pos_iterator<Iterator> current, line_pos_iterator<Iterator> upper_bound)
        {
            // No need in line number counting because it will be the same
        ...
        template<class Iterator>
        inline std::size_t get_column(Iterator lower_bound, Iterator current, std::size_t tabs)
        {
            std::size_t column = 1;
            Iterator first = get_line_start(lower_bound, current);
            ...
            for (Iterator i = first; i != current; ++i) {
                switch (*i) {
                case '\t': column += tabs - (column - 1) % tabs; break;
                default  : ++column;
                }
            }
        ...
        inline std::size_t get_column(
            line_pos_iterator<Iterator> lower_bound,
