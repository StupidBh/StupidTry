...
namespace boost {
    namespace spirit {
        namespace traits {
            ...
            BOOST_SCOPED_ENUM_START(clear_mode) { clear_if_enabled, clear_always };
            BOOST_SCOPED_ENUM_END

            template<typename Iterator>
            void clear_queue(Iterator&, BOOST_SCOPED_ENUM(clear_mode) /*mode*/ = clear_mode::clear_if_enabled)
            {
            }
