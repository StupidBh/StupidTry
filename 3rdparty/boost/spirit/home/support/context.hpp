...
==============================================================================*/
    ...
    ///////////////////////////////////////////////////////////////////////////////
        ...
        #define SPIRIT_DECLARE_ATTRIBUTE(z, n, data)                                       \
            typedef phoenix::actor<attribute<n>> BOOST_PP_CAT(BOOST_PP_CAT(_r, n), _type); \
            phoenix::actor<attribute<n>> const BOOST_PP_CAT(_r, n) = BOOST_PP_CAT(BOOST_PP_CAT(_r, n), _type)();
    /***/
...
BOOST_PHOENIX_DEFINE_CUSTOM_TERMINAL(
    ...
    mpl::false_ // is not nullary
    ,
    v2_eval(proto::make<boost::spirit::local_variable<N>()>, proto::call<functional::env(proto::_state)>))
    ...
    namespace spirit {
        ...
        struct context
        {
            ...
            context(typename Attributes::car_type attribute) :
                attributes(attribute, fusion::nil_()),
                locals()
            {
            }

            template<typename Args, typename Context>
            context(typename Attributes::car_type attribute, Args const& args, Context& caller_context) :
                attributes(
                    attribute,
                    fusion::as_list(fusion::transform(args, detail::expand_arg<Context>(caller_context)))),
                locals()
            {
            }
        ...
        template<typename Context>
        struct attributes_of<Context&> : attributes_of<Context>
        {
        };
        ...
        struct attribute
        {
            ...
            {
                typedef
                    typename attributes_of<typename mpl::at_c<typename Env::args_type, 1>::type>::type attributes_type;

                typedef typename fusion::result_of::size<attributes_type>::type attributes_size;

                // report invalid argument not found (N is out of bounds)
                BOOST_SPIRIT_ASSERT_MSG((N < attributes_size::value), index_is_out_of_bounds, ());

                typedef typename fusion::result_of::at_c<attributes_type, N>::type type;
            };

            template<typename Env>
            typename result<Env>::type eval(Env const& env) const
            {
        ...
        struct local_variable
        {
            ...
            struct result
            {
                typedef typename locals_of<typename mpl::at_c<typename Env::args_type, 1>::type>::type locals_type;

                typedef typename fusion::result_of::size<locals_type>::type locals_size;

                // report invalid argument not found (N is out of bounds)
                BOOST_SPIRIT_ASSERT_MSG((N < locals_size::value), index_is_out_of_bounds, ());

                typedef typename fusion::result_of::at_c<locals_type, N>::type type;
            };
        ...
        BOOST_PP_REPEAT_FROM_TO(3, SPIRIT_ATTRIBUTES_LIMIT, SPIRIT_DECLARE_ATTRIBUTE, _)
