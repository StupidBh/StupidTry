    ...
    namespace spirit {
        namespace char_encoding {
            ...
            struct iso8859_1
            {
                ...
                static bool isascii_(int ch) { return 0 == (ch & ~0x7f); }
                ...
                static bool strict_ischar(int ch) { return ch >= 0 && ch <= 255; }

                static bool isalnum(int ch)
                {
                    BOOST_ASSERT(strict_ischar(ch));
                    return (iso8859_1_char_types[ch] & BOOST_CC_ALPHA) || (iso8859_1_char_types[ch] & BOOST_CC_DIGIT);
                }
                ...
                static bool isgraph(int ch) { return ('\x21' <= ch && ch <= '\x7e') || ('\xa1' <= ch && ch <= '\xff'); }
                ...
                static bool isprint(int ch) { return ('\x20' <= ch && ch <= '\x7e') || ('\xa0' <= ch && ch <= '\xff'); }
                ...
                {
                    BOOST_ASSERT(strict_ischar(ch));
                    return isupper(ch) && '\0' != iso8859_1_char_conversion[ch] ? iso8859_1_char_conversion[ch] : ch;
                }

                static int toupper(int ch)
                {
                    BOOST_ASSERT(strict_ischar(ch));
                    return islower(ch) && '\0' != iso8859_1_char_conversion[ch] ? iso8859_1_char_conversion[ch] : ch;
                }
