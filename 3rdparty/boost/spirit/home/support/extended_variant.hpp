...
namespace boost {
    namespace spirit {
        ...
        struct extended_variant
        {
    ...
    #if defined(BOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES)
            typedef boost::variant<BOOST_SPIRIT_EXTENDED_VARIANT_ENUM_PARAMS(T)> variant_type;
            typedef typename variant_type::types types;

            typedef extended_variant<BOOST_SPIRIT_EXTENDED_VARIANT_ENUM_PARAMS(T)> base_type;
    #else
            ...
            variant_type const& get() const { return var; }

            variant_type& get() { return var; }

            void swap(extended_variant& rhs) BOOST_NOEXCEPT { var.swap(rhs.var); }
...
namespace boost {
    ...
    inline T const& get(boost::spirit::extended_variant<BOOST_SPIRIT_EXTENDED_VARIANT_ENUM_PARAMS(T)> const& x)
    {
        return boost::get<T>(x.get());
    }

    template<typename T, BOOST_SPIRIT_EXTENDED_VARIANT_ENUM_PARAMS(typename T)>
    inline T& get(boost::spirit::extended_variant<BOOST_SPIRIT_EXTENDED_VARIANT_ENUM_PARAMS(T)>& x)
    {
        return boost::get<T>(x.get());
    }
    ...
    inline T const& get(boost::spirit::extended_variant<Types...> const& x)
    {
        return boost::get<T>(x.get());
    }

    template<typename T, typename... Types>
    inline T& get(boost::spirit::extended_variant<Types...>& x)
    {
        return boost::get<T>(x.get());
    }
