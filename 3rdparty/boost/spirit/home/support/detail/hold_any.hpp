...
namespace boost {
    namespace spirit {
        struct bad_any_cast : std::bad_cast
        {
            ...
            const char* what() const BOOST_NOEXCEPT_OR_NOTHROW BOOST_OVERRIDE { return "bad any cast"; }
        ...
        namespace detail {
            ...
            struct fxns<mpl::true_>
            {
                ...
                struct type
                {
                    static boost::core::typeinfo const& get_type() { return BOOST_CORE_TYPEID(T); }

                    static void static_delete(void** x) { reinterpret_cast<T*>(x)->~T(); }

                    static void destruct(void** x) { reinterpret_cast<T*>(x)->~T(); }

                    static void clone(void* const* src, void** dest) { new (dest) T(*reinterpret_cast<T const*>(src)); }
                    ...
                    static std::basic_istream<Char>& stream_in(std::basic_istream<Char>& i, void** obj)
                    {
            ...
            struct fxns<mpl::false_>
            {
                ...
                struct type
                {
                    ...
                    static void clone(void* const* src, void** dest) { *dest = new T(*static_cast<T const*>(*src)); }
            ...
            struct get_table
            {
                ...
                {
                    static fxn_ptr_table<Char> static_table = { fxns<is_small>::template type<T, Char>::get_type,
            ...
            template<typename Char>
            inline std::basic_istream<Char>& operator>>(std::basic_istream<Char>& i, empty&)
            {
        ...
        public:
            ...
            explicit basic_hold_any(T const& x) :
                table(spirit::detail::get_table<T>::template get<Char>()),
                object(0)
            {
            ...
            ~basic_hold_any() { table->static_delete(&object); }
            ...
            {
                // are we copying between the same type?
                spirit::detail::fxn_ptr_table<Char>* x_table = spirit::detail::get_table<T>::template get<Char>();
                if (table == x_table) {
            ...
            basic_hold_any& operator=(basic_hold_any const& x) { return assign(x); }
            ...
            boost::core::typeinfo const& type() const { return table->get_type(); }
            ...
            T const& cast() const
            {
                ...
                return spirit::detail::get_table<T>::is_small::value ? *reinterpret_cast<T const*>(&object)
    ...
    // implicit casting is disabled by default for compatibility with boost::any
        ...
        inline T* any_cast(basic_hold_any<Char>* operand)
        {
            if (operand && operand->type() == BOOST_CORE_TYPEID(T)) {
                return spirit::detail::get_table<T>::is_small::value ? reinterpret_cast<T*>(&operand->object)
