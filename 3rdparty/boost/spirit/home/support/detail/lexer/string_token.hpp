    ...
    namespace lexer {
        ...
        struct basic_string_token
        {
            ...
            void remove_duplicates()
            {
                ...
                _charset.erase(std::unique(_charset.begin(), _charset.end()), _charset.end());
            }

            void normalise()
            {
                const std::size_t max_chars_ = sizeof(CharT) == 1 ? num_chars : num_wchar_ts;
            ...
            {
                return _negated < rhs_._negated || (_negated == rhs_._negated && _charset < rhs_._charset);
            }

            bool empty() const { return _charset.empty() && !_negated; }

            bool any() const { return _charset.empty() && _negated; }
            ...
            {
                if ((any() && rhs_.any()) || (_negated == rhs_._negated && !any() && !rhs_.any())) {
        ...
        private:
            ...
            void intersect_charset(basic_string_token& rhs_, basic_string_token& overlap_)
            {
                ...
                else // rhs_._negated == true
                {
                    ...
                    if (!overlap_._charset.empty()) {
                        ...
                        rhs_._charset.erase(
                            std::unique(rhs_._charset.begin(), rhs_._charset.end()),
                            rhs_._charset.end());
                        normalise();
                        overlap_.normalise();
                        rhs_.normalise();
            ...
            void merge(string& src_, string& dest_)
            {
                ...
                std::merge(src_.begin(), src_.end(), dest_.begin(), dest_.end(), tmp_.begin());
                dest_ = tmp_;
            }
        };
    }
