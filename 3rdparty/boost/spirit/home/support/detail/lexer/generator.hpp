    ...
    namespace lexer {
        ...
        public:
            ...
            static void build(const rules& rules_, basic_state_machine<CharT>& state_machine_)
            {
                ...
                detail::internals& internals_ = const_cast<detail::internals&>(state_machine_.data());
                ...
                for (index_ = 0, size_ = internals_._lookup->size(); index_ < size_; ++index_) {
                    internals_._lookup[index_]->resize(sizeof(CharT) == 1 ? num_chars : num_wchar_ts, dead_state_index);

                    if (!rules_.regexes()[index_].empty()) {
                        // vector mapping token indexes to partitioned token index sets
                        index_set_vector set_mapping_;
                        // syntax tree
                        detail::node* root_ = build_tree(rules_, index_, node_ptr_vector_, internals_, set_mapping_);

                        build_dfa(root_, set_mapping_, internals_._dfa_alphabet[index_], *internals_._dfa[index_]);
        ...
        protected:
                ...
                index_set_vector& set_mapping_)
            {
                ...
                const typename rules::string_deque_deque& regexes_ = rules_.regexes();
                ...
                typename rules::string_deque::const_iterator regex_iter_ = regexes_[state_].begin();
                typename rules::string_deque::const_iterator regex_iter_end_ = regexes_[state_].end();
                typename rules::id_vector::const_iterator ids_iter_ = ids_[state_].begin();
                typename rules::id_vector::const_iterator unique_ids_iter_ = unique_ids_[state_].begin();
                typename rules::id_vector::const_iterator states_iter_ = states_[state_].begin();
                const typename rules::string& regex_ = *regex_iter_;
                // map of regex charset tokens (strings) to index
                token_map token_map_;
                const typename rules::string_pair_deque& macrodeque_ = rules_.macrodeque();
                ...
                detail::node* root_ = parser::parse(
                    regex_.c_str(),
                    regex_.c_str() + regex_.size(),
                    *ids_iter_,
                    *unique_ids_iter_,
                    *states_iter_,
                    rules_.flags(),
                ...
                if (internals_._seen_BOL_assertion) {
                    // Fixup BOLs
                    typename detail::node::node_vector::iterator iter_ = tree_vector_.begin();
                    typename detail::node::node_vector::iterator end_ = tree_vector_.end();
                ...
                // join trees
                {
                    ...
                    for (; iter_ != end_; ++iter_) {
                        ...
                        node_ptr_vector_->back() = new detail::selection_node(root_, *iter_);
                ...
                typename charset_list::list::const_iterator iter_ = token_list_->begin();
                typename charset_list::list::const_iterator end_ = token_list_->end();
                std::size_t index_ = 0;

                for (; iter_ != end_; ++iter_, ++index_) {
                    const charset* cs_ = *iter_;
                    typename charset::index_set::const_iterator set_iter_ = cs_->_index_set.begin();
                    typename charset::index_set::const_iterator set_end_ = cs_->_index_set.end();
            ...
            {
                for (typename macro_deque::const_iterator iter_ = macrodeque_.begin(), end_ = macrodeque_.end();
                     iter_ != end_;
                     ++iter_) {
                    const typename rules::string& name_ = iter_->first;
                    const typename rules::string& regex_ = iter_->second;
                    detail::node* node_ = parser::parse(
                        regex_.c_str(),
                        regex_.c_str() + regex_.size(),
                        0,
                        0,
                        ...
                        seen_BOL_assertion_,
                        seen_EOL_assertion_);
                    macro_iter_pair map_iter_ =
                        macromap_.insert(macro_pair(name_, static_cast<const detail::node*>(0)));
            ...
            static void build_dfa(
                ...
                size_t_vector& dfa_)
            {
                typename detail::node::node_vector* followpos_ = &root_->firstpos();
                ...
                closure(followpos_, seen_sets_, seen_vectors_, hash_vector_, dfa_alphabet_, dfa_);
                ...
                for (std::size_t index_ = 0; index_ < seen_vectors_->size(); ++index_) {
                    ...
                    for (typename equivset_list::list::const_iterator iter_ = equiv_list_->begin(),
                                                                      end_ = equiv_list_->end();
                         iter_ != end_;
                         ++iter_) {
                        equivset* equivset_ = *iter_;
                        const std::size_t transition_ = closure(
                            &equivset_->_followpos,
                            seen_sets_,
                            seen_vectors_,
                            hash_vector_,
                            dfa_alphabet_,
                ...
                size_t_vector& dfa_)
            {
                ...
                for (typename detail::node::node_vector::const_iterator iter_ = followpos_->begin(),
                                                                        end_ = followpos_->end();
                     iter_ != end_;
                     ++iter_) {
                    closure_ex(*iter_, end_state_, id_, unique_id_, state_, set_ptr_.get(), vector_ptr_.get(), hash_);
                }

                bool found_ = false;
                typename size_t_vector::const_iterator hash_iter_ = hash_vector_.begin();
                typename size_t_vector::const_iterator hash_end_ = hash_vector_.end();
                typename node_set_vector::vector::const_iterator set_iter_ = seen_sets_->begin();
                        ...
                        while (!r_->empty() && iter_ != end_) {
                            ...
                            if (overlap_->empty()) {
                                ...
                                *l_iter_ = overlap_.release();
                                ...
                                iter_ = lhs_->insert(++iter_, static_cast<charset*>(0));
            ...
            {
                typename parser::tokeniser::token_map::const_iterator iter_ = map_.begin();
                typename parser::tokeniser::token_map::const_iterator end_ = map_.end();
            ...
            static void fill_lookup(const string_token& token_, size_t_vector* lookup_, const std::size_t index_)
            {
                ...
                if (token_._negated) {
                    ...
                    while (curr_ < chars_end_) {
                        ptr_[static_cast<typename Traits::index_type>(*curr_)] = index_ + dfa_offset;
                        ++curr_;
                    }
                }
            }
            ...
            static void
                build_equiv_list(const node_vector* vector_, const index_set_vector& set_mapping_, equivset_list& lhs_)
            {
                equivset_list rhs_;
                        ...
                        while (!r_->empty() && iter_ != end_) {
                            ...
                            if (overlap_->empty()) {
                                ...
                                *l_iter_ = overlap_.release();
                                ...
                                iter_ = lhs_->insert(++iter_, static_cast<equivset*>(0));
                                *iter_ = overlap_.release();
            ...
            static void
                fill_rhs_list(const node_vector* vector_, const index_set_vector& set_mapping_, equivset_list& list_)
            {
                typename node_vector::const_iterator iter_ = vector_->begin();
                typename node_vector::const_iterator end_ = vector_->end();
                    ...
                    if (!node_->end_state()) {
                        ...
                        if (token_ != null_token) {
                            ...
                            if (token_ == bol_token || token_ == eol_token) {
                                ...
                                list_->back() = new equivset(index_set_, node_->greedy(), token_, node_->followpos());
                            }
                            else {
                                list_->back() =
                                    new equivset(set_mapping_[token_], node_->greedy(), token_, node_->followpos());
                            }
                        }
                    }
                }
            ...
            static void fixup_bol(detail::node*& root_, node_ptr_vector& node_ptr_vector_)
            {
                ...
                bool found_ = false;
                typename detail::node::node_vector::const_iterator iter_ = first_->begin();
                typename detail::node::node_vector::const_iterator end_ = first_->end();
                ...
                if (!found_) {
                    ...
                    node_ptr_vector_->push_back(static_cast<detail::selection_node*>(0));
                    node_ptr_vector_->back() = new detail::selection_node(lhs_, rhs_);
                    lhs_ = node_ptr_vector_->back();

                    node_ptr_vector_->push_back(static_cast<detail::sequence_node*>(0));
                    node_ptr_vector_->back() = new detail::sequence_node(lhs_, root_);
                    root_ = node_ptr_vector_->back();
                }
            }
            ...
            static void minimise_dfa(const std::size_t dfa_alphabet_, size_t_vector& dfa_, std::size_t size_)
            {
                ...
                *lookup_ptr_ = 0;
                ...
                for (; first_ < end_; first_ += dfa_alphabet_, ++index_) {
                    for (second_ = first_ + dfa_alphabet_, curr_index_ = index_ + 1; second_ < end_;
                         second_ += dfa_alphabet_, ++curr_index_) {
                        ...
                        if (memcmp(first_, second_, sizeof(std::size_t) * dfa_alphabet_) == 0) {
                ...
                if (!index_set_.empty()) {
                    ...
                    typename index_set::iterator set_end_ = index_set_.end();
