    ...
    namespace lexer {
        ...
        public:
            ...
            public:
                ...
                struct data
                {
                    ...
                    {
                        return dfa == rhs_.dfa && states == rhs_.states && state == rhs_.state &&
                               transitions == rhs_.transitions && transition == rhs_.transition &&
                               end_state == rhs_.end_state && id == rhs_.id && unique_id == rhs_.unique_id &&
                               goto_dfa == rhs_.goto_dfa && bol_index == rhs_.bol_index &&
                               eol_index == rhs_.eol_index && token == rhs_.token && goto_state == rhs_.goto_state;
                    }
                };
                ...
                {
                    return _dfas == rhs_._dfas && _dfa == rhs_._dfa && _states == rhs_._states &&
                           _state == rhs_._state && _transitions == rhs_._transitions &&
            ...
            private:
                ...
                typename detail::basic_char_state_machine<CharT>::state::size_t_string_token_map::const_iterator
                    _token_iter;
                typename detail::basic_char_state_machine<CharT>::state::size_t_string_token_map::const_iterator
                    _token_end;
                ...
                void next()
                {
                    ...
                    if (_transition >= _transitions) {
                        ...
                        if (_state >= _states) {
                            ...
                            if (_dfa >= _dfas) {
                                ...
                                _states = _data.states = _sm->_csm._sm_vector[_dfa].size();
                    ...
                    if (reset_state_) {
                        const typename detail::basic_char_state_machine<CharT>::state* ptr_ =
                            &_sm->_csm._sm_vector[_dfa][_state];
            ...
            bool empty() const
            {
                ...
                return _internals._lookup->empty() && _internals._dfa_alphabet.empty() && _internals._dfa->empty();
            }

            std::size_t size() const { return _internals._dfa->size(); }
            ...
            bool operator==(const basic_state_machine& rhs_) const
            {
                ...
                return _internals._lookup == rhs_._internals._lookup &&
                       ...
                       _internals._seen_BOL_assertion == rhs_._internals._seen_BOL_assertion &&
                       _internals._seen_EOL_assertion == rhs_._internals._seen_EOL_assertion;
            }
            ...
            iterator begin() const
            {
                ...
                if (!_csm.empty()) {
                    const typename detail::basic_char_state_machine<CharT>::state_vector* ptr_ =
                        &_csm._sm_vector.front();

                    iter_._dfas = _csm._sm_vector.size();
                    iter_._states = iter_._data.states = ptr_->size();
                    iter_._transitions = iter_._data.transitions = ptr_->front()._transitions.size();
            ...
            const detail::internals& data() const { return _internals; }
        ...
        private:
            ...
            void human_readable(detail::basic_char_state_machine<CharT>& sm_) const
            {
                ...
                for (std::size_t start_state_index_ = 0; start_state_index_ < start_states_; ++start_state_index_) {
                    const detail::internals::size_t_vector* lu_ = _internals._lookup[start_state_index_];
                    const std::size_t alphabet_ = _internals._dfa_alphabet[start_state_index_] - dfa_offset;
                    std::vector<std::basic_string<CharT>> chars_(alphabet_);
                    const std::size_t states_ = _internals._dfa[start_state_index_]->size() / (alphabet_ + dfa_offset);
                    const std::size_t* read_ptr_ =
                        &_internals._dfa[start_state_index_]->front() + alphabet_ + dfa_offset;

                    sm_._sm_vector[start_state_index_].resize(states_ - 1);

                    for (std::size_t alpha_index_ = 0; alpha_index_ < max_; ++alpha_index_) {
                        const std::size_t col_ = lu_->at(alpha_index_);

                        if (col_ != static_cast<std::size_t>(dead_state_index)) {
                            chars_[col_ - dfa_offset] += static_cast<CharT>(alpha_index_);
                        }
                    }

                    for (std::size_t state_index_ = 1; state_index_ < states_; ++state_index_) {
                        typename detail::basic_char_state_machine<CharT>::state* state_ =
                        ...
                        for (std::size_t col_index_ = 0; col_index_ < alphabet_; ++col_index_, ++read_ptr_) {
                            const std::size_t transition_ = *read_ptr_;

                            if (transition_ != 0) {
                                const std::size_t i_ = transition_ - 1;
                                typename detail::basic_char_state_machine<
                                    CharT>::state::size_t_string_token_map::iterator iter_ =
                                    state_->_transitions.find(i_);

                                if (iter_ == state_->_transitions.end()) {
                                    basic_string_token<CharT> token_(false, chars_[col_index_]);
                                    typename detail::basic_char_state_machine<CharT>::state::size_t_string_token_pair
                                        pair_(i_, token_);
                        ...
                        for (typename detail::basic_char_state_machine<CharT>::state::size_t_string_token_map::iterator
                                 iter_ = state_->_transitions.begin(),
                                 end_ = state_->_transitions.end();
                             iter_ != end_;
                             ++iter_) {
                            std::sort(iter_->second._charset.begin(), iter_->second._charset.end());
