        ...
        public:
            ...
            public:
                ...
                struct data
                {
                    ...
                    {
                        return id == rhs_.id && unique_id == rhs_.unique_id && start == rhs_.start && end == rhs_.end &&
                               state == rhs_.state;
                    }
                };
                ...
                bool operator==(const iterator& rhs_) const { return _data == rhs_._data; }
                ...
                void next_token()
                {
                    ...
                    if (internals_._dfa->size() == 1) {
                        ...
                        _data.id = _input->next(internals_, _data.state, _data.start, _data.end, _data.unique_id);
                    }
            ...
            basic_file_input(
                const basic_state_machine<CharT>* state_machine_,
...
again:
                ...
                for (;;) {
                    ...
                    if (BOL_state_ && (_start_token == _start_buffer || *(_start_token - 1) == '\n')) {
            ...
            std::size_t next(
                const std::size_t* const lookup_,
                const std::size_t dfa_alphabet_,
                const std::size_t* const dfa_,
                const CharT*& start_,
                const CharT*& end_,
                std::size_t& unique_id_)
            {
            ...
            bool reload_buffer(const CharT*& curr_, const bool end_state_, const CharT*& end_token_)
            {
                ...
                if (success_) {
                    ...
                    if (_start_token - 1 == _start_buffer) {
                        ...
                        _stream->read(_start_buffer + old_size_, _buffer_increment);
                        ...
                        _stream->read(_start_buffer, static_cast<std::streamsize>(_buffer.size()));
                        count_ = _stream->gcount();
                    ...
                    if (end_state_) {
                        end_token_ = _start_token + (end_token_ - old_start_token_);
                    }
