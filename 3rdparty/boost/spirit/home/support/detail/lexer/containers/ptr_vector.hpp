    ...
    namespace lexer {
        namespace detail {
            ...
            public:
                ...
                vector* operator->() { return &_vector; }

                const vector* operator->() const { return &_vector; }

                vector& operator*() { return _vector; }

                const vector& operator*() const { return _vector; }

                Type*& operator[](const std::size_t index_) { return _vector[index_]; }

                Type* const& operator[](const std::size_t index_) const { return _vector[index_]; }
