                        ...
                        if (ch_ == '\\') {
                            ...
                            case ')':
                                ...
                                if (state_._paren_count < 0) {
                                    ...
                                    ss_ << "Number of open parenthesis < 0 at index " << state_.index() - 1 << '.';
                            ...
                            case '+':
                                ...
                                break;
                            case '{': open_curly(state_, token_); break;
                            case '|': token_.set(num_token::OR, null_token); break;
                            case '^':
                            ...
                            case '/': throw runtime_error("Lookahead ('/') is not supported yet."); break;
                            default:
                                if ((state_._flags & icase) &&
                                    (std::isupper(ch_, state_._locale) || std::islower(ch_, state_._locale))) {
            ...
            private:
                ...
                static void read_options(state& state_)
                {
                    if (!state_.eos() && *state_._curr == '?') {
                        ...
                        while (!eos_ && ch_ != ':') {
                            switch (ch_) {
                            case '-': negate_ ^= 1; break;
                            case 'i':
                                if (negate_) {
                                    state_._flags = static_cast<regex_flags>(state_._flags & ~icase);
                                }
                                else {
                                    state_._flags = static_cast<regex_flags>(state_._flags | icase);
                                }
                            ...
                            case 's':
                                if (negate_) {
                                    state_._flags = static_cast<regex_flags>(state_._flags | dot_not_newline);
                                }
                                else {
                                    state_._flags = static_cast<regex_flags>(state_._flags & ~dot_not_newline);
                                }
                ...
                static void escape(state& state_, token_map& map_, num_token& token_)
                {
                    ...
                    const CharT* str_ = tokeniser_helper::escape_sequence(state_, ch_, str_len_);

                    if (str_) {
                        state state2_(str_ + 1, str_ + str_len_, state_._flags, state_._locale);
                ...
                static void create_charset_token(
