        ...
        namespace detail {
                    ...
                    bool& seen_EOL_assertion_)
                {
                    ...
                    do {
                        ...
                        case '=':
                            ...
                            tokeniser::next(state_, map_, rhs_token_);
                            break;
                        case '>': reduce(token_stack_, macromap_, node_ptr_vector_, tree_node_stack_); break;
                        default:
                            std::ostringstream ss_;

                            ss_ << "A syntax error occurred: '" << lhs_token_.precedence_string() << "' against '"
                                << rhs_token_.precedence_string() << "' at index " << state_.index() << ".";
                    ...
                    // properly!
                    if (state_._seen_BOL_assertion) {
                        seen_BOL_assertion_ = true;
                    }

                    if (state_._seen_EOL_assertion) {
            ...
            private:
                ...
                static void reduce(
                    token_stack& token_stack_,
                    const macro_map& macromap_,
                    node_ptr_vector& node_vector_ptr_,
                    tree_node_stack& tree_node_stack_)
                {
                    ...
                    case token::REGEX:
                        ...
                        break;
                    case token::OREXP     : orexp(handle_, token_stack_, node_vector_ptr_, tree_node_stack_); break;
                    case token::SEQUENCE  : token_stack_.push(token::OREXP); break;
                    case token::SUB       : sub(handle_, token_stack_, node_vector_ptr_, tree_node_stack_); break;
                    case token::EXPRESSION: token_stack_.push(token::SUB); break;
                    case token::REPEAT    : repeat(handle_, token_stack_); break;
                    case token::CHARSET   : charset(handle_, token_stack_, node_vector_ptr_, tree_node_stack_); break;
                    case token::MACRO:
                        macro(handle_, token_stack_, macromap_, node_vector_ptr_, tree_node_stack_);
                        break;
                    case token::OPENPAREN: openparen(handle_, token_stack_); break;
                    case token::OPT:
                    case token::AOPT:
                        optional(rhs_._type == token::OPT, node_vector_ptr_, tree_node_stack_);
                        token_stack_.push(token::DUP);
                        break;
                    case token::ZEROORMORE:
                    case token::AZEROORMORE:
                        zero_or_more(rhs_._type == token::ZEROORMORE, node_vector_ptr_, tree_node_stack_);
                        token_stack_.push(token::DUP);
                        break;
                    case token::ONEORMORE:
                    case token::AONEORMORE:
                        one_or_more(rhs_._type == token::ONEORMORE, node_vector_ptr_, tree_node_stack_);
                        token_stack_.push(token::DUP);
                        break;
                    case token::REPEATN:
                    case token::AREPEATN:
                        repeatn(rhs_._type == token::REPEATN, handle_.top(), node_vector_ptr_, tree_node_stack_);
                        token_stack_.push(token::DUP);
                        break;
                    default: throw runtime_error("Internal error regex_parser::reduce"); break;
                    }
                }
                ...
                static void orexp(
                    ...
                    tree_node_stack& tree_node_stack_)
                {
                    BOOST_ASSERT(handle_.top()._type == token::OREXP && (handle_.size() == 1 || handle_.size() == 3));
                ...
                static void
                    sub(token_stack& handle_,
                        token_stack& token_stack_,
                        node_ptr_vector& node_ptr_vector_,
                        tree_node_stack& tree_node_stack_)
                {
                    BOOST_ASSERT(handle_.top()._type == token::SUB && (handle_.size() == 1 || handle_.size() == 2));
                ...
                {
                    BOOST_ASSERT(handle_.top()._type == token::REPEAT && handle_.size() >= 1 && handle_.size() <= 3);
                ...
                static void charset(
                    ...
                    tree_node_stack& tree_node_stack_)
                {
                    BOOST_ASSERT(handle_.top()._type == token::CHARSET && handle_.size() == 1);
                    // store charset
                    ...
                    tree_node_stack& tree_node_stack_)
                {
                    ...
                    typename macro_map::const_iterator iter_ = macromap_.find(top_._macro);
                ...
                {
                    BOOST_ASSERT(handle_.top()._type == token::OPENPAREN && handle_.size() == 3);
                    handle_.pop();
                ...
                static void perform_or(node_ptr_vector& node_ptr_vector_, tree_node_stack& tree_node_stack_)
                {
                    // perform or
                ...
                static void sequence(node_ptr_vector& node_ptr_vector_, tree_node_stack& tree_node_stack_)
                {
                    node* rhs_ = tree_node_stack_.top();
                ...
                static void
                    optional(const bool greedy_, node_ptr_vector& node_ptr_vector_, tree_node_stack& tree_node_stack_)
                {
                    // perform ?
                    node* lhs_ = tree_node_stack_.top();
                    ...
                    for (node::node_vector::iterator iter_ = firstpos_.begin(), end_ = firstpos_.end(); iter_ != end_;
                         ++iter_) {
                    ...
                    tree_node_stack& tree_node_stack_)
                {
                    ...
                    if (!(token_._min == 1 && !token_._comma)) {
                        const std::size_t top_ = token_._min > 0 ? token_._min : token_._max;
