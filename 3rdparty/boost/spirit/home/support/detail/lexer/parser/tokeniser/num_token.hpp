    ...
    namespace lexer {
        namespace detail {
            ...
            struct basic_num_token
            {
                ...
                basic_num_token(const type type_ = BEGIN, const std::size_t id_ = null_token) :
                    _type(type_),
                    _id(id_),
                    _min(0),
                    _comma(false),
                ...
                void min_max(const std::size_t min_, const bool comma_, const std::size_t max_)
                {
                    _min = min_;
                    _comma = comma_;
                    _max = max_;
                ...
                char precedence(const type type_) const { return _precedence_table[_type][type_]; }

                const char* precedence_string() const { return _precedence_strings[_type]; }
            };

            template<typename CharT>
            const char basic_num_token<CharT>::_precedence_table[END + 1][END + 1] = {
                //        BEG, REG, ORE, SEQ, SUB, EXP, RPT, DUP,  | , CHR, MCR,  ( ,  ) ,  ? , ?? ,  * , *? ,  + , +?,
                //        {n}?, {n}, END
                /*BEGIN*/ { ' ', '<', '<', '<', '<', '<', '<', ' ', ' ', '<', '<',
                            '<', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', '>' },
                /*REGEX*/ { ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ',
