...
namespace boost {
    namespace lexer {
        template<typename FwdIter, typename Traits = char_traits<typename std::iterator_traits<FwdIter>::value_type>>
        class basic_input {
        public:
            class iterator {
            public:
            ...
            private:
                ...
                void next_token()
                {
                    ...
                    if (internals_._dfa->size() == 1) {
                        if (internals_._seen_BOL_assertion || internals_._seen_EOL_assertion) {
                            _data.id = next(
                        ...
                        if (internals_._seen_BOL_assertion || internals_._seen_EOL_assertion) {
                            _data.id =
                                next(internals_, _data.state, _data.bol, _data.end, _input->_end, _data.unique_id);
                        }
                        else {
                            _data.id = next(internals_, _data.state, _data.end, _input->_end, _data.unique_id);
                        }
                    }
...
again:
            ...
            basic_input(
                const basic_state_machine<typename Traits::char_type>* state_machine_,
                const FwdIter& begin_,
                const FwdIter& end_) :
