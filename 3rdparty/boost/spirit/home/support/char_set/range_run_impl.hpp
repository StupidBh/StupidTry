    ...
    namespace spirit {
        namespace support {
            namespace detail {
                ...
                inline bool try_merge(Run& run, Iterator iter, Range const& range)
                {
                ...
                inline bool range_run<Char>::test(Char val) const
                {
                    if (run.empty()) {
                        return false;
                    }
                    ...
                    typename storage_type::const_iterator iter =
                        std::upper_bound(run.begin(), run.end(), val, range_compare<range_type>());
                ...
                template<typename Char>
                inline void range_run<Char>::swap(range_run& other)
                {
                    run.swap(other.run);
                }

                template<typename Char>
                void range_run<Char>::set(range_type const& range)
                {
                    ...
                    typename storage_type::iterator iter =
                        std::upper_bound(run.begin(), run.end(), range, range_compare<range_type>());
                ...
                template<typename Char>
                void range_run<Char>::clear(range_type const& range)
                {
                    BOOST_ASSERT(is_valid(range));
                    if (!run.empty()) {
                        // search the ranges for one that potentially includes 'range'
