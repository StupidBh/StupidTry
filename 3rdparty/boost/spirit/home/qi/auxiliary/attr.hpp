...
namespace boost {
    namespace spirit {
        ...
        struct use_terminal<qi::domain, terminal_ex<tag::attr, fusion::vector1<A0>>> : mpl::true_
        {
        };

        template<> // enables *lazy* attr()
        struct use_lazy_terminal<qi::domain, tag::attr, 1> : mpl::true_
        {
        };
...
namespace boost {
    namespace spirit {
        namespace traits {
            ...
            struct handles_container<qi::attr_parser<T>, Attr, Context, Iterator> : traits::is_container<Attr>
            {
            };
        }
    }
