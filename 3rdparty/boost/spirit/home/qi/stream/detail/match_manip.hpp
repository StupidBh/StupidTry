...
namespace boost {
    namespace spirit {
        ...
        #pragma warning(disable: 4512) // assignment operator could not be generated.
                ...
                template<
                    typename Expr,
                    typename CopyExpr = mpl::false_,
                    typename CopyAttr = mpl::false_,
                ...
                struct match_manip
                {
                    ...
                    BOOST_SPIRIT_ASSERT_MSG(!CopyExpr::value || !CopyAttr::value, error_invalid_should_not_happen, ());

                    match_manip(Expr const& xpr, Skipper const& s, Attribute& a) :
                        expr(xpr),
                        skipper(s),
                        attr(a),
                        post_skip(skip_flag::postskip)
                    {
                    }

                    match_manip(Expr const& xpr, Skipper const& s, BOOST_SCOPED_ENUM(skip_flag) ps, Attribute& a) :
                        expr(xpr),
                        skipper(s),
                        attr(a),
                        post_skip(ps)
        ...
        #pragma warning(pop)
                ...
                struct match<Expr, typename enable_if<traits::matches<qi::domain, Expr>>::type>
                {
                    typedef match_manip<Expr> type;

                    static type call(Expr const& expr) { return type(expr, unused, unused); }
                };
                ...
                struct phrase_match<Expr, Skipper, typename enable_if<traits::matches<qi::domain, Expr>>::type>
                {
                    typedef match_manip<Expr, mpl::false_, mpl::false_, Skipper> type;

                    static type call(Expr const& expr, Skipper const& skipper, BOOST_SCOPED_ENUM(skip_flag) post_skip)
                    {
                        // Report invalid expression error as early as possible.
                        // If you got an error_invalid_expression error message here,
                ...
                ///////////////////////////////////////////////////////////////////////////
                template<typename Char, typename Traits, typename Expr, typename CopyExpr, typename CopyAttr>
                inline std::basic_istream<Char, Traits>&
                    operator>>(std::basic_istream<Char, Traits>& is, match_manip<Expr, CopyExpr, CopyAttr> const& fm)
                {
                    ...
                    match_manip<Expr, CopyExpr, CopyAttr, Skipper> const& fm)
                {
                    ...
                    input_iterator f(is);
                    input_iterator l;
                    if (!qi::phrase_parse(f, l, fm.expr, fm.skipper, fm.post_skip)) {
                        is.setstate(std::basic_istream<Char, Traits>::failbit);
                    }
                    return is;
                }
                    ...
                    match_manip<Expr, CopyExpr, CopyAttr, Attribute, Skipper> const& fm)
                {
                    ...
                    input_iterator f(is);
                    input_iterator l;
                    if (!qi::phrase_parse(f, l, fm.expr, fm.skipper, fm.post_skip, fm.attr)) {
                        is.setstate(std::basic_istream<Char, Traits>::failbit);
                    }
                    return is;
                }
