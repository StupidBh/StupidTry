...
namespace boost {
    namespace spirit {
        namespace traits {
            ...
            template<typename T, typename AccT>
            inline bool scale(int exp, T& n, AccT acc_n)
            {
                if (exp >= 0) {
            ...
            inline bool scale(int /*exp*/, unused_type /*n*/, unused_type /*acc_n*/)
            {
            ...
            template<typename T, typename AccT>
            inline bool scale(int exp, int frac, T& n, AccT acc_n)
            {
                return scale(exp - frac, n, acc_n);
            }

            inline bool scale(int /*exp*/, int /*frac*/, unused_type /*n*/)
            {
                // no-op for unused_type
                return true;
            }

            inline float negate(bool neg, float n)
            {
                return neg ? (core::copysign)(n, -1.f) : n;
            }

            inline double negate(bool neg, double n)
            {
                return neg ? (core::copysign)(n, -1.) : n;
            }

            inline long double negate(bool neg, long double n)
            {
            ...
            inline unused_type negate(bool /*neg*/, unused_type n)
            {
                // no-op for unused_type
                return n;
            }
        ...
        namespace qi {
            namespace detail {
                ...
                struct real_impl
                {
                    ...
                    template<typename Iterator>
                    static std::size_t ignore_excess_digits(Iterator& first, Iterator const& last, mpl::true_)
                    {
                        return RealPolicies::ignore_excess_digits(first, last);
                    }

                    template<typename Iterator>
                    static std::size_t ignore_excess_digits(Iterator& first, Iterator const& last)
                    {
                    ...
                    static bool parse(Iterator& first, Iterator const& last, Attribute& attr, RealPolicies const& p)
                    {
                        if (first == last) {
                            return false;
                        }
                        ...
                        if (!got_a_number) {
                            ...
                            if (p.parse_nan(first, last, n) || p.parse_inf(first, last, n)) {
