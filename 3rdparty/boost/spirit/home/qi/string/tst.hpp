    ...
    namespace spirit {
        namespace qi {
            ...
            struct tst
            {
                ...
                template<typename Iterator>
                T* add(Iterator first, Iterator last, typename boost::call_traits<T>::param_type val)
                {
            ...
            private:
                ...
                void copy(tst const& rhs) { root = node::clone_node(rhs.root, this); }
                ...
                node* new_node(Char id) { return new node(id); }

                T* new_data(typename boost::call_traits<T>::param_type val) { return new T(val); }

                void delete_node(node* p) { delete p; }

                void delete_data(T* p) { delete p; }
            };
        }
    }
}
