...
namespace boost {
    namespace spirit {
        namespace qi {
            ...
            struct symbols :
                proto::extends<
                    typename proto::terminal<reference<symbols<Char, T, Lookup, Filter>>>::type,
                ...
                primitive_parser<symbols<Char, T, Lookup, Filter>>
            {
                ...
                template<typename Str>
                friend adder const& operator+=(symbols& sym, Str const& str)
                {
                    return sym.add(str);
                }

                template<typename Str>
                friend remover const& operator-=(symbols& sym, Str const& str)
                {
                ...
                template<typename Str>
                friend adder const& operator+=(symbols& sym, Str& str)
                {
                    return sym.add(str);
                }
                ...
                // non-const version needed to suppress proto's -= kicking in
                template<typename Str>
                friend remover const& operator-=(symbols& sym, Str& str)
                {
                    return sym.remove(str);
                }
    #else
                ...
                {
                    return *lookup->add(traits::get_begin<Char>(str), traits::get_end<Char>(str), T());
                }
                ...
                {
                    return find_impl(traits::get_begin<Char>(str), traits::get_end<Char>(str));
                }
            ...
            public:
                    ...
                    Attribute& attr_) const
                {
                    ...
                    if (value_type* val_ptr = lookup->find(first, last, Filter())) {
        ...
        #pragma warning(disable: 4512) // assignment operator could not be generated.
                ...
                struct adder
                {
                    ...
                    template<typename Iterator>
                    adder const& operator()(Iterator const& first, Iterator const& last, T const& val) const
                    {
                    ...
                    template<typename Str>
                    adder const& operator()(Str const& s, T const& val = T()) const
                    {
                        sym.lookup->add(traits::get_begin<Char>(s), traits::get_end<Char>(s), val);
                        return *this;
                    }

                    template<typename Str>
                    adder const& operator,(Str const& s) const
                    {
                        sym.lookup->add(traits::get_begin<Char>(s), traits::get_end<Char>(s), T());
                        return *this;
                    }
                ...
                struct remover
                {
                    ...
                    template<typename Iterator>
                    remover const& operator()(Iterator const& first, Iterator const& last) const
                    {
                    ...
                    template<typename Str>
                    remover const& operator()(Str const& s) const
                    {
                        sym.lookup->remove(traits::get_begin<Char>(s), traits::get_end<Char>(s));
                        return *this;
                    }
        ...
        #pragma warning(pop)
            ...
            struct make_primitive<reference<symbols<Char, T, Lookup, Filter>>, Modifiers>
            {
                ...
                {
                    Char operator()(Char ch) const { return static_cast<Char>(CharEncoding::tolower(ch)); }
                };
                ...
                typedef typename mpl::if_<no_case, symbols<Char, T, Lookup, nc_filter>, reference_>::type result_type;
