            ...
            namespace detail {
                ...
                struct parser_binder
                {
                        ...
                        mpl::false_) const
                    {
                        ...
                        return p.parse(first, last, context, skipper, fusion::at_c<0>(context.attributes));
                    }

                    template<typename Iterator, typename Skipper, typename Context>
                    bool operator()(Iterator& first, Iterator const& last, Context& context, Skipper const& skipper)
                        const
                    {
                ...
                inline parser_binder<Parser, Auto> bind_parser(Parser const& p)
                {
