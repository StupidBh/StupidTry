...
namespace boost {
    namespace spirit {
        namespace qi {
            ...
            struct rule :
                proto::extends<
                    typename proto::terminal<reference<rule<Iterator, T1, T2, T3, T4> const>>::type,
                ...
                parser<rule<Iterator, T1, T2, T3, T4>>
            {
                ...
                typedef typename spirit::detail::extract_component<qi::domain, template_params>::type skipper_type;
                ...
                typedef typename spirit::detail::extract_sig<template_params, encoding_type, qi::domain>::type sig_type;
                ...
                typedef context<fusion::cons<attr_reference_type, parameter_types>, locals_type> context_type;

                typedef function<
                    bool(Iterator& first, Iterator const& last, context_type& context, skipper_type const& skipper)>
                    function_type;

                typedef typename mpl::if_<
                    is_same<encoding_type, unused_type>,
                    unused_type,
                    tag::char_code<tag::encoding, encoding_type>>::type encoding_modifier_type;
                ...
                {
                    lhs.f = detail::bind_parser<Auto>(compile<qi::domain>(expr, encoding_modifier_type()));
                }
    ...
    // VC7.1 has problems to resolve 'rule' without explicit template parameters
                ...
                template<
                    ...
                    typename T4_,
                    typename Expr>
                friend rule<OutputIterator_, T1_, T2_, T3_, T4_>&
                    operator%=(rule<OutputIterator_, T1_, T2_, T3_, T4_>& r, Expr const& expr);
                ...
                template<
                    ...
                    typename Expr>
                friend rule<OutputIterator_, T1_, T2_, T3_, T4_>&
                    operator%=(rule<OutputIterator_, T1_, T2_, T3_, T4_>& r, Expr& expr);
    #endif
                    ...
                    Attribute& attr_param) const
                {
                    ...
                    if (f) {
                        ...
                        // rhs parser
                        typedef traits::transform_attribute<Attribute, attr_type, domain> transform;
                ...
                reference_ alias() const { return reference_(*this); }
    ...
    #if BOOST_WORKAROUND(BOOST_MSVC, < 1400)
            template<typename OutputIterator_, typename T1_, typename T2_, typename T3_, typename T4_, typename Expr>
            rule<OutputIterator_, T1_, T2_, T3_, T4_>&
                operator%=(rule<OutputIterator_, T1_, T2_, T3_, T4_>& r, Expr const& expr)
            {
                // Report invalid expression error as early as possible.
                // If you got an error_invalid_expression error message here,
                ...
                typedef
                    typename rule<OutputIterator_, T1_, T2_, T3_, T4_>::encoding_modifier_type encoding_modifier_type;

                r.f = detail::bind_parser<mpl::true_>(compile<qi::domain>(expr, encoding_modifier_type()));
                return r;
            }

            template<typename Iterator_, typename T1_, typename T2_, typename T3_, typename T4_, typename Expr>
            rule<Iterator_, T1_, T2_, T3_, T4_>& operator%=(rule<Iterator_, T1_, T2_, T3_, T4_>& r, Expr& expr)
            {
                return r %= static_cast<Expr const&>(expr);
            }
    #endif
