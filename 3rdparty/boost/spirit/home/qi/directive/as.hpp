...
namespace boost {
    namespace spirit {
        ...
        // enables as_string[...]
        template<>
        struct use_directive<qi::domain, tag::as_string> : mpl::true_
        {
        };

        // enables as_wstring[...]
        template<>
        struct use_directive<qi::domain, tag::as_wstring> : mpl::true_
        {
        };

        // enables as<T>[...]
        template<typename T>
        struct use_directive<qi::domain, tag::stateful_tag<T, tag::as>> : mpl::true_
        {
        };
    }
}
    ...
    namespace spirit {
        namespace qi {
            ...
            struct as_directive : unary_parser<as_directive<Subject, T>>
            {
                ...
                bool parse(Iterator& first, Iterator const& last, Context& context, Skipper const& skipper, T& attr_)
                    const
                {
                    Iterator i = first;
...
namespace boost {
    namespace spirit {
        namespace traits {
            ///////////////////////////////////////////////////////////////////////////
            template<typename Subject, typename T>
            struct has_semantic_action<qi::as_directive<Subject, T>> : unary_has_semantic_action<Subject>
            {
            };

            ///////////////////////////////////////////////////////////////////////////
            template<typename Subject, typename T, typename Attribute, typename Context, typename Iterator>
            struct handles_container<qi::as_directive<Subject, T>, Attribute, Context, Iterator> : mpl::false_
            {
            };
        }
    }
