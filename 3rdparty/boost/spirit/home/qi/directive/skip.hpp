...
namespace boost {
    namespace spirit {
        ...
        struct use_directive<
            qi::domain,
            terminal_ex<
                tag::skip // enables skip(s)[p]
                ,
                fusion::vector1<T>>> : boost::spirit::traits::matches<qi::domain, T>
        {
        };
    ...
    namespace spirit {
        namespace qi {
            ...
            struct reskip_parser : unary_parser<reskip_parser<Subject>>
            {
                ...
                typename enable_if<detail::is_unused_skipper<Skipper>, bool>::type parse(
                    Iterator& first,
                    Iterator const& last,
                    Context& context,
                    Skipper const& u // --> The skipper is reintroduced
                    ,
                    Attribute& attr_) const
                {
                    return subject.parse(first, last, context, detail::get_skipper(u), attr_);
                }

                template<typename Iterator, typename Context, typename Skipper, typename Attribute>
                typename disable_if<detail::is_unused_skipper<Skipper>, bool>::type parse(
                    Iterator& first,
                    Iterator const& last,
                    Context& context,
            ...
            struct skip_parser : unary_parser<skip_parser<Subject, Skipper>>
            {
                ...
                template<typename Iterator, typename Context, typename Skipper_, typename Attribute>
                bool parse(
                    Iterator& first,
                    Iterator const& last,
                    Context& context,
                    Skipper_ const& // skipper --> bypass the supplied skipper
                    ,
                    Attribute& attr_) const
                {
                    return subject.parse(first, last, context, skipper, attr_);
            ...
            {
                typedef typename result_of::compile<qi::domain, Skipper, Modifiers>::type skipper_type;
...
namespace boost {
    namespace spirit {
        namespace traits {
            ///////////////////////////////////////////////////////////////////////////
            template<typename Subject>
            struct has_semantic_action<qi::reskip_parser<Subject>> : unary_has_semantic_action<Subject>
            {
            };

            template<typename Subject, typename Skipper>
            struct has_semantic_action<qi::skip_parser<Subject, Skipper>> : unary_has_semantic_action<Subject>
            {
            };
            ...
            template<typename Subject, typename Skipper, typename Attribute, typename Context, typename Iterator>
