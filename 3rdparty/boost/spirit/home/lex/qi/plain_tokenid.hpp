...
namespace boost {
    namespace spirit {
        ...
        // enables tokenid
        template<>
        struct use_terminal<qi::domain, tag::tokenid> : mpl::true_
        {
        };
        ...
        struct use_terminal<qi::domain, terminal_ex<tag::tokenid, fusion::vector2<A0, A1>>> :
            mpl::and_<mpl::or_<is_integral<A0>, is_enum<A0>>, mpl::or_<is_integral<A1>, is_enum<A1>>>
        {
        };

        // enables *lazy* tokenid(id)
        template<>
        struct use_lazy_terminal<qi::domain, tag::tokenid, 1> : mpl::true_
        {
        };

        // enables *lazy* tokenid(idmin, idmax)
        template<>
        struct use_lazy_terminal<qi::domain, tag::tokenid, 2> : mpl::true_
        {
        };
    }
}
...
namespace boost {
    namespace spirit {
        namespace qi {
            ...
            struct plain_tokenid : primitive_parser<plain_tokenid<TokenId>>
            {
            ...
            ///////////////////////////////////////////////////////////////////////////
            template<typename TokenId>
            struct plain_tokenid_range : primitive_parser<plain_tokenid_range<TokenId>>
            {
                template<typename Context, typename Iterator>
                struct attribute
                {
            ...
            struct make_primitive<tag::tokenid, Modifiers>
            {
                ...
                result_type operator()(unused_type, unused_type) const { return result_type(std::size_t(~0)); }
            };
...
namespace boost {
    namespace spirit {
        namespace traits {
            ...
            struct handles_container<qi::plain_tokenid<Idtype>, Attr, Context, Iterator> : mpl::true_
            {
            };

            template<typename Idtype, typename Attr, typename Context, typename Iterator>
            struct handles_container<qi::plain_tokenid_range<Idtype>, Attr, Context, Iterator> : mpl::true_
            {
            };
        }
    }
