...
namespace boost {
    namespace spirit {
        namespace lex {
            namespace lexertl {
                ...
                struct make_multi_pass
                {
                    ...
                    typedef iterator_policies::
                        default_policy<ownership_policy, check_policy, input_policy, storage_policy>
                            policy_type;
                ...
                private:
                    typedef typename make_multi_pass<Functor>::functor_data_type functor_type;
                ...
                public:
                    ...
                    iterator(
                        IteratorData const& iterdata_,
                        base_iterator_type& first,
                        base_iterator_type const& last,
                        char_type const* state = 0) :
                        base_type(functor_type(unique_functor_type(), shared_functor_type(iterdata_, first, last)))
                    {
                    ...
                    std::size_t set_state(std::size_t state) { return unique_functor_type::set_state(*this, state); }

                    // get the current state for the underlying lexer object
                    std::size_t get_state() { return unique_functor_type::get_state(*this); }
                    ...
                    {
                        return (0 != statename) ? unique_functor_type::map_state(*this, statename) : 0;
                    }
                };
            }
        }
        ...
        namespace traits {
            template<typename Functor>
            struct is_multi_pass<spirit::lex::lexertl::iterator<Functor>> : mpl::true_
            {
            };
