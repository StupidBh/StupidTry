...
namespace boost {
    namespace spirit {
        ...
        #pragma warning(pop)
                ...
                public:
                    ...
                    functor& operator=(functor const& rhs) { return *this; }
    #endif
                    ...
                    typedef FunctorData<Iterator, SupportsActors, SupportsState, token_value_type> shared;
                    ...
                    static result_type& get_next(MultiPass& mp, result_type& result)
                    {
                        ...
                        for (;;) {
                            ...
                            if (boost::lexer::npos == id) { // no match
                                ...
                                std::cerr << "Not matched, in state: " << state << ", lookahead: >" << next << "<"
                                          << std::endl;
    #endif
    ...
    #if defined(BOOST_SPIRIT_LEXERTL_DEBUG)
                            {
                                ...
                                std::cerr << "Matched: " << id << ", in state: " << state << ", string: >"
                                          << std::basic_string<char_type>(data.get_first(), end) << "<"
                                          << ", lookahead: >" << next << "<" << std::endl;
                                if (data.get_state() != state) {
                                    std::cerr << "Switched to state: " << data.get_state() << std::endl;
                                }
                            }
    #endif
                            ...
                            BOOST_SCOPED_ENUM(pass_flags) pass = data.invoke_actions(state, id, unique_id, end);
    ...
    #if defined(BOOST_SPIRIT_LEXERTL_DEBUG)
                        std::cerr << "Switching state from: " << oldstate << " to: " << state << std::endl;
    #endif
                        return oldstate;
                    }
                ...
                typename functor<Token, FunctorData, Iterator, SupportsActors, SupportsState>::result_type const
                    functor<Token, FunctorData, Iterator, SupportsActors, SupportsState>::eof =
                        typename functor<Token, FunctorData, Iterator, SupportsActors, SupportsState>::result_type();
    #endif
