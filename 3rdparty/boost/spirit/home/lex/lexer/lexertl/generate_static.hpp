        ...
        namespace lex {
            namespace lexertl {
                namespace detail {
                    ...
                    ///////////////////////////////////////////////////////////////////////////
                    template<typename Char>
                    inline bool generate_delimiter(std::basic_ostream<Char>& os_)
                    {
                    ...
                    template<typename Char>
                    inline bool generate_cpp_state_info(
                        ...
                        Char const* name_suffix)
                    {
                        ...
                        typedef
                            typename boost::lexer::basic_rules<Char>::string_size_t_map::const_iterator state_iterator;
                        ...
                        os_ << boost::lexer::detail::strings<Char>::char_name() << " const* const lexer_state_names"
                            << (name_suffix[0] ? "_" : "") << name_suffix << "[" << rules_.statemap().size()
                            << "] = \n{\n";
                        ...
                        for (iterator rit = reverse_state_map.begin(); rit != rend; ++last_id) {
                            ...
                            os_ << "    " << boost::lexer::detail::strings<Char>::char_prefix() << "\"" << (*rit).second
                                << "\"";
                            if (++rit != rend) {
                                os_ << ",\n";
                            }
                        ...
                        os_ << "std::size_t const lexer_state_count" << (name_suffix[0] ? "_" : "") << name_suffix
                            << " = " << rules_.statemap().size() << ";\n\n";
                        return os_.good();
                    }

                    template<typename Char>
                    inline bool generate_cpp_state_table(
                        std::basic_ostream<Char>& os_,
                        Char const* name_suffix,
                        bool bol,
                        bool eol)
                    ...
                    template<typename Char>
                    bool generate_function_body_dfa(
                        std::basic_ostream<Char>& os_,
                        boost::lexer::basic_state_machine<Char> const& sm_)
                    {
                        ...
                        if (dfas_ > 1) {
                            for (std::size_t state_ = 0; state_ < dfas_; ++state_) {
                                ...
                                os_ << "    static std::size_t const lookup" << state_ << "_[" << lookups_
                                    << "] = {\n        ";
                                for (/**/; i_ < count_; ++i_) {
                                ...
                                os_ << "    static const std::size_t dfa" << state_ << "_[" << count_
                                    << "] = {\n        ";
                                count_ /= 8;
                            ...
                            os_ << "    static std::size_t const* lookup_arr_[" << count_ << "] = { lookup0_";
                            for (i_ = 1; i_ < count_; ++i_) {
                                os_ << ", " << "lookup" << i_ << "_";
                            }
                            os_ << " };\n";

                            os_ << "    static std::size_t const dfa_alphabet_arr_[" << count_ << "] = { ";
                    ...
                    {
                        typedef typename boost::lexer::basic_state_machine<Char>::iterator iterator_type;
                    ...
                    ///////////////////////////////////////////////////////////////////////////
                    template<typename Char>
                    bool generate_function_body_switch(
                        std::basic_ostream<Char>& os_,
                        boost::lexer::basic_state_machine<Char> const& sm_)
                    {
                        typedef typename boost::lexer::basic_state_machine<Char>::iterator iterator_type;
                                    ...
                                    while (ptr_ != end2_) {
                                        ...
                                        if (*ptr_ == curr_char_ + 1) {
                                            ...
                                            if (range_) {
                                                ...
                                                os_ << "(ch_ >= '" << get_charlit(start_char_) << "' && ch_ <= '"
                                                    << get_charlit(curr_char_) << "')";
                                                range_ = false;
                                            }
                                            else {
                                                os_ << "ch_ " << ((iter_->token._negated) ? "!=" : "==") << " '"
                                                    << get_charlit(curr_char_) << "'";
                                            }
                                        }
                                    }
                        ...
                        F generate_function_body)
                    {
                        ...
                        typename std::basic_string<Char>::size_type p = guard.find_first_of(L<Char>(": "));
                        ...
                        { // to_upper(guard)
                            ...
                            for (iter_t iter = guard.begin(), last = guard.end(); iter != last; ++iter) {
                        ...
                        os_ << "template<typename Iterator>\n";
                        os_ << "std::size_t next_token" << (name_suffix[0] ? "_" : "") << name_suffix << " (";
                        ...
                        if (!generate_cpp_state_table<Char>(
                                os_,
                                name_suffix,
                                sm_.data()._seen_BOL_assertion,
                                sm_.data()._seen_EOL_assertion)) {
                            return false;
                        }
                    ...
                    F f)
                {
                    ...
                    return detail::generate_cpp(lexer.state_machine_, lexer.rules_, os, name_suffix, f);
                }
                ...
                ///////////////////////////////////////////////////////////////////////////
                template<typename Lexer>
                inline bool generate_static_dfa(
                    Lexer const& lexer,
                    std::basic_ostream<typename Lexer::char_type>& os,
                    typename Lexer::char_type const* name_suffix = detail::L<typename Lexer::char_type>())
                {
                ...
                ///////////////////////////////////////////////////////////////////////////
                template<typename Lexer>
                inline bool generate_static_switch(
                    Lexer const& lexer,
                    std::basic_ostream<typename Lexer::char_type>& os,
                    typename Lexer::char_type const* name_suffix = detail::L<typename Lexer::char_type>())
                {
