...
namespace boost {
    namespace spirit {
        ...
        #pragma warning(disable: 4512) // assignment operator could not be generated.
                ...
                struct lexer_def_ :
                    proto::extends<
                        typename proto::terminal<lex::reference<lexer_def_<LexerDef> const>>::type,
                        lexer_def_<LexerDef>>,
                ...
                public:
                    ...
                    struct attribute
                    {
                        ...
                        typedef fusion::vector2<id_type, iterator_range<iterator_type>> type;
                    };
                        ...
                        Attribute& attr) const
                    {
                        ...
                        if (first != last) {
                            typedef typename std::iterator_traits<Iterator>::value_type token_type;
                ...
                private:
                    ...
                    struct adder
                    {
                        ...
                        adder const& operator()(char_type c, id_type token_id = id_type()) const
                        {
                            if (id_type() == token_id) {
                        ...
                        adder const& operator()(string_type const& s, id_type token_id = id_type()) const
                        {
                            if (id_type() == token_id) {
                            ...
                            id_type token_id = id_type()) const
                        {
                            ...
                            if (id_type() == token_id) {
                                ...
                                BOOST_ASSERT(id_type() == tokdef.id() || token_id == tokdef.id());
                                tokdef.id(token_id);
                            }
                    ...
                    struct pattern_adder
                    {
                        ...
                        pattern_adder const& operator()(string_type const& p, string_type const& s) const
                        {
                ...
                public:
                    ...
                    lexer_def_ operator()(char_type const* state_) const { return lexer_def_(def, state_); }

                    lexer_def_ operator()(char_type const* state_, char_type const* targetstate_) const
                    {
                        return lexer_def_(def, state_, targetstate_);
                    }

                    lexer_def_
                        operator()(string_type const& state_, string_type const& targetstate_ = string_type()) const
                    {
                        return lexer_def_(def, state_, targetstate_);
                    }
        ...
        #pragma warning(pop)
                ...
                inline lexer_def_<LexerDef>& operator+=(lexer_def_<LexerDef>& lexdef, Expr& xpr)
                {
                    // Report invalid expression error as early as possible.
                    // If you got an error_invalid_expression error message here,
                    // then the expression (expr) is not a valid spirit lex
                ...
                template<typename LexerDef, typename Expr>
                inline lexer_def_<LexerDef>& operator+=(lexer_def_<LexerDef>& lexdef, Expr const& xpr)
                {
                    // Report invalid expression error as early as possible.
                    // If you got an error_invalid_expression error message here,
                    // then the expression (expr) is not a valid spirit lex
            ...
            public:
                ...
                iterator_type end() const { return this->lexer_type::end(); }

                std::size_t map_state(char_type const* state) { return this->lexer_type::add_state(state); }
