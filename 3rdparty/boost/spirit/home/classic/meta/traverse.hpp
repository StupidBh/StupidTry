...
namespace boost {
    namespace spirit {
        ...
        struct post_order
        {
            ...
            {
                typedef
                    typename traverse_post_order_return<MetaT, ParserT, traverse_post_order_env<0, 0, 0, 0>>::type type;
            };
            ...
            template<typename MetaT, typename ParserT>
            static typename result<MetaT, ParserT>::type traverse(MetaT const& meta_, ParserT const& parser_)
            {
        ...
        struct unary_identity_policy
        {
            ...
            struct unary_result
            {
                typedef typename unary_identity_policy_return<UnaryT, SubjectT>::type type;
            };

            template<typename UnaryT, typename SubjectT, typename EnvT>
            typename parser_traversal_unary_result<TransformT, UnaryT, SubjectT, EnvT>::type
        ...
        struct action_identity_policy
        {
            ...
            template<typename ActionT, typename SubjectT, typename EnvT>
            typename parser_traversal_action_result<TransformT, ActionT, SubjectT, EnvT>::type
                generate_action(ActionT const& action_, SubjectT const& subject_, EnvT const& /*env*/) const
            {
        ...
        struct binary_identity_policy_return
        {
            ...
            typedef typename parser_generator_t ::template result<LeftT, RightT>::type type;
        };
        ...
        struct binary_identity_policy
        {
            ...
            {
                typedef typename binary_identity_policy_return<BinaryT, LeftT, RightT>::type type;
            };

            template<typename BinaryT, typename LeftT, typename RightT, typename EnvT>
            typename parser_traversal_binary_result<TransformT, BinaryT, LeftT, RightT, EnvT>::type
                generate_binary(BinaryT const&, LeftT const& left_, RightT const& right_, EnvT const& /*env*/) const
            {
                typedef typename BinaryT::parser_generator_t parser_generator_t;
                return parser_generator_t::template generate<LeftT, RightT>(left_, right_);
            }
        };
