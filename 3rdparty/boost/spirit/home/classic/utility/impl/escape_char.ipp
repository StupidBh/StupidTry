...
namespace boost {
    ...
    #pragma warning(disable: 4127)
            ...
            struct escape_char_action_parse
            {
                ...
                static typename parser_result<ParserT, ScannerT>::type parse(ScannerT const& scan, ParserT const& p)
                {
                    ...
                    if (scan.first != scan.last) {
                        ...
                        if (result_t hit = p.subject().parse(scan)) {
                            ...
                            if (*scan.first == '\\') {
                                ...
                                case 'X': {
                                    char_t hex = 0;
                                    char_t const lim = (std::numeric_limits<char_t>::max)() >> 4;
                                ...
                                case '7': {
                                    char_t oct = 0;
                                    char_t const lim = (std::numeric_limits<char_t>::max)() >> 3;
                                    while (scan.first != scan.last) {
                                        char_t c = *scan.first;
                                        if (oct > lim && (c >= '0' && c <= '7')) {
                                            // overflow detected
    ...
    #pragma warning(pop)
            ...
            struct escape_char_parse
            {
                ...
                static typename parser_result<ParserT, ScannerT>::type parse(ScannerT const& scan, ParserT const& /*p*/)
                {
                    typedef uint_parser<CharT, 8, 1, std::numeric_limits<CharT>::digits / 3 + 1> oct_parser_t;
                    typedef uint_parser<CharT, 16, 1, std::numeric_limits<CharT>::digits / 4 + 1> hex_parser_t;

                    typedef alternative<
                        difference<anychar_parser, chlit<CharT>>,
                        sequence<
                            chlit<CharT>,
                            alternative<
                                alternative<oct_parser_t, sequence<inhibit_case<chlit<CharT>>, hex_parser_t>>,
                                difference<difference<anychar_parser, inhibit_case<chlit<CharT>>>, oct_parser_t>>>>
                        parser_t;

                    static parser_t p =
                        ((anychar_p - chlit<CharT>(CharT('\\'))) |
                         (chlit<CharT>(CharT('\\')) >>
                          (oct_parser_t() | as_lower_d[chlit<CharT>(CharT('x'))] >> hex_parser_t() |
                           (anychar_p - as_lower_d[chlit<CharT>(CharT('x'))] - oct_parser_t()))));

                    BOOST_SPIRIT_DEBUG_TRACE_NODE(
                        p,
