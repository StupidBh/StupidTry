...
namespace boost {
    namespace spirit {
        ...
        public:
            ...
            parser_error_base& operator=(parser_error_base const&) { return *this; }
        };
        ...
        struct parser_error : public parser_error_base
        {
            ...
            parser_error& operator=(parser_error const& rhs)
            {
                where = rhs.where;
                descriptor = rhs.descriptor;
                return *this;
            ...
            virtual ~parser_error() BOOST_NOEXCEPT_OR_NOTHROW {}
        ...
        //////////////////////////////////
        template<typename ErrorDescrT, typename IteratorT>
        inline void throw_(IteratorT where, ErrorDescrT descriptor)
        {
            boost::throw_exception(parser_error<ErrorDescrT, IteratorT>(where, descriptor));
        }
        ...
        struct assertive_parser : public unary<ParserT, parser<assertive_parser<ErrorDescrT, ParserT>>>
        {
        ...
        struct error_status
        {
            ...
            error_status(result_t result_ = fail, std::ptrdiff_t length_ = -1, T const& value_ = T()) :
                result(result_),
                length(length_),
                value(value_)
            {
        ...
        struct fallback_parser : public unary<ParserT, parser<fallback_parser<ErrorDescrT, ParserT, HandlerT>>>
        {
        ...
        struct guard_gen : public unary<ParserT, nil_t>
        {
            ...
            {
                return fallback_parser<ErrorDescrT, ParserT, HandlerT>(this->subject(), handler);
            }
        };
        ...
        struct guard
        {
            ...
            template<typename ParserT>
            static guard_gen<ErrorDescrT, ParserT> generate(ParserT const& parser)
            {
                return guard_gen<ErrorDescrT, ParserT>(parser);
            }

            template<typename ParserT>
            guard_gen<ErrorDescrT, ParserT> operator()(ParserT const& parser) const
            {
                return guard_gen<ErrorDescrT, ParserT>(parser);
            }
        };
