            ...
            RT fallback_parser_parse(ParserT const& p, ScannerT const& scan)
            {
                ...
                while (hr.result == error_status_t::retry) {
                    try {
                        ...
                        switch (hr.result) {
                        case error_status_t::fail: return scan.no_match();
                        case error_status_t::accept:
                            return scan.create_match(std::size_t(hr.length), hr.value, save, scan.first);
                        case error_status_t::rethrow: boost::throw_exception(error);
                        default                     : continue;
                        }
                    }
                }
