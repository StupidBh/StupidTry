...
#define BOOST_SPIRIT_ASSERT_FSQ_SIZE BOOST_SPIRIT_ASSERT(((m_tail + N + 1) - m_head) % (N + 1) == m_size % (N + 1))
...
namespace boost {
    ...
    #error "Please use at least Boost V1.31.0 while compiling the fixed_size_queue class!"
            ...
            class fsq_iterator :
                public boost::
                    iterator_adaptor<fsq_iterator<QueueT, T, PointerT>, PointerT, T, std::random_access_iterator_tag> {
            public:
                ...
                fsq_iterator(position const& p_) :
                    p(p_)
                {
                }
            ...
            private:
                ...
                typename base_t::reference dereference() const { return p.self->m_queue[p.pos]; }
        ...
        public:
            ...
            iterator end() { return iterator(position(this, m_tail)); }

            const_iterator end() const { return const_iterator(position(this, m_tail)); }
