    ...
    namespace spirit {
        ...
        namespace fileiter_impl {
            ...
            public:
                ...
                std_file_iterator(const std_file_iterator& iter) { *this = iter; }
                ...
                operator bool() const { return m_file ? true : false; }

                bool operator==(const std_file_iterator& iter) const
                {
                    return (m_file == iter.m_file) && (m_eof == iter.m_eof) && (m_pos == iter.m_pos);
                }

                const CharT& get_cur_char(void) const { return m_curChar; }
...
// mmap_file_iterator, Windows version
            ...
            public:
                ...
                mmap_file_iterator(const mmap_file_iterator& iter) { *this = iter; }
                ...
                // Nasty bug in Comeau up to 4.3.0.1, we need explicit boolean context
                //  for shared_ptr to evaluate correctly
                operator bool() const { return m_mem ? true : false; }

                bool operator==(const mmap_file_iterator& iter) const { return m_curChar == iter.m_curChar; }

                const CharT& get_cur_char(void) const { return *m_curChar; }

                void next_char(void) { m_curChar++; }

                void prev_char(void) { m_curChar--; }

                void advance(std::ptrdiff_t n) { m_curChar += n; }

                std::ptrdiff_t distance(const mmap_file_iterator& iter) const { return m_curChar - iter.m_curChar; }

                void seek_end(void) { m_curChar = m_mem.get() + (m_filesize / sizeof(CharT)); }
...
// mmap_file_iterator, POSIX version
            ...
            private:
                struct mapping
                {
                    ...
                    CharT const* begin() const { return static_cast<CharT*>(data); }

                    CharT const* end() const { return static_cast<CharT*>(data) + size / sizeof(CharT); }

                    ~mapping() { munmap(static_cast<char*>(data), size); }
            ...
            public:
                    ...
                    m_curChar(0)
                {
                    ...
                    if ((fstat(fd, &stat_buf) != 0) ||
                        !S_ISREG(stat_buf.st_mode)) { // if fstat returns an error or if the file isn't a
                ...
                void advance(signed long n) { m_curChar += n; }

                long distance(const mmap_file_iterator& iter) const { return m_curChar - iter.m_curChar; }

                void seek_end(void) { m_curChar = m_mem->end(); }
        ...
        file_iterator<CharT, BaseIteratorT> file_iterator<CharT, BaseIteratorT>::make_end(void)
        {
        ...
        template<typename CharT, typename BaseIteratorT>
        file_iterator<CharT, BaseIteratorT>& file_iterator<CharT, BaseIteratorT>::operator=(const base_t& iter)
        {
