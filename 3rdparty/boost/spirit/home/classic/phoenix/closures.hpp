...
namespace phoenix {
    ...
    namespace impl {
        ...
        struct closure_frame_holder
        {
            ...
            void set(frame_ptr f) { *tsp_frame = f; }
    ...
    public:
        ...
        struct result
        {
            typedef typename tuple_element<N, typename ClosureT::tuple_t>::rtype type;
        };

        template<typename TupleT>
        typename tuple_element<N, typename ClosureT::tuple_t>::rtype eval(TupleT const& /*args*/) const
        {
            using namespace std;
    ...
    public:
        ...
        closure() :
            frame()
        {
            closure_frame_holder_ref(&frame);
        }
...
#ifdef PHOENIX_THREADSAFE
        static boost::thread_specific_ptr<holder_t*>& tsp_frame_instance()
        {
            static boost::thread_specific_ptr<holder_t*> the_instance;
            return the_instance;
        }

        static void tsp_frame_instance_init() { tsp_frame_instance(); }
#endif

        static holder_t& closure_frame_holder_ref(holder_t* holder_ = 0)
        {
#ifdef PHOENIX_THREADSAFE
