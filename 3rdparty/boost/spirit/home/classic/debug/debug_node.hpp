    ...
    namespace spirit {
        ...
        namespace impl {
            ...
            struct token_printer_aux :
                mpl::if_<
                    mpl::and_<is_convertible<CharT, char>, is_convertible<char, CharT>>,
            ...
            inline ResultT& print_closure_info(ResultT& hit, int level, std::string const& name)
            {
                if (!name.empty()) {
        ...
        struct parser_context_linker : public ContextT
        {
            ...
            ResultT& post_parse(ResultT& hit, ParserT const& p, ScannerT& scan)
            {
                ...
                --scan.get_level();
                if (trace_parser(p.derived())) {
                    impl::print_node_info(hit, scan.get_level(), true, parser_name(p.derived()), scan.first, scan.last);
                }
            #endif // BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES
        ...
        struct parser_scanner_linker : public ScannerT
        {
            ...
            int& get_level() { return debug.get_level(); }
