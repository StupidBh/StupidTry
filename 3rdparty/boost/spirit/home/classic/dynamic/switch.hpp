...
namespace boost {
    namespace spirit {
        ...
        struct switch_parser : public unary<CaseT, parser<switch_parser<CaseT, CondT>>>
        {
            ...
            {
                return this->subject().parse(scan, impl::make_cond_functor<CondT>::do_(cond));
            }
        ...
        struct case_parser : public unary<ParserT, parser<case_parser<N, ParserT, IsDefault>>>
        {
            ...
            BOOST_STATIC_CONSTANT(
                bool,
                is_epsilon = (is_default && boost::is_same<self_subject_t, epsilon_parser>::value));
            ...
            typename parser_result<self_t, ScannerT>::type parse(ScannerT const& scan, CondT const& cond) const
            {
                ...
                if (!scan.at_end()) {
                    typedef impl::default_delegate_parse<value, is_default, default_t::value> default_parse_t;

                    typename ScannerT::iterator_t const save(scan.first);
                    return default_parse_t::parse(cond(scan), *this, *this, scan, save);
                }
            ...
            impl::compound_case_parser<self_t, case_parser<N1, ParserT1, IsDefault1>, IsDefault1>
        ...
        inline case_parser<N, ParserT, false> case_p(parser<ParserT> const& p)
        {
            return case_parser<N, ParserT, false>(p);
        }

        ///////////////////////////////////////////////////////////////////////////////
        struct default_parser_gen : public case_parser<BOOST_SPIRIT_DEFAULTCASE_MAGIC, epsilon_parser, true>
        {
            default_parser_gen() :
                case_parser<BOOST_SPIRIT_DEFAULTCASE_MAGIC, epsilon_parser, true>(epsilon_p)
            {
            }

            template<typename ParserT>
            case_parser<BOOST_SPIRIT_DEFAULTCASE_MAGIC, ParserT, true> operator()(parser<ParserT> const& p) const
            {
