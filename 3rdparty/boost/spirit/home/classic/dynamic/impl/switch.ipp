    ...
    namespace spirit {
        ...
        namespace impl {
            ...
            inline typename parser_result<ParserT, ScannerT>::type
                delegate_parse(ParserT const& p, ScannerT const& scan, typename ScannerT::iterator_t const save)
            {
            ...
            {
                template<typename ParserT, typename DefaultT, typename ValueT, typename ScannerT>
                static typename parser_result<ParserT, ScannerT>::type parse(
                    ValueT const& value,
                    ParserT const& p,
                    DefaultT const&,
                    ScannerT const& scan,
            ...
            struct get_default_parser
            {
                ...
                static ResultT get(parser<ParserT> const& p)
                {
                    return default_case<typename ParserT::derived_t::left_t>::get(p.derived().left());
                }
            };
            ...
            struct default_case
            {
                ...
                BOOST_STATIC_CONSTANT(bool, value = (CaseT::is_default || default_case<typename CaseT::left_t>::value));
                ...
                BOOST_STATIC_CONSTANT(
                    bool,
                    is_epsilon =
                        ((CaseT::is_default && CaseT::is_epsilon) || default_case<typename CaseT::left_t>::is_epsilon));
                ...
                typedef typename boost::mpl::if_c<
                    CaseT::is_default,
                    typename CaseT::right_embed_t,
                    typename default_case<typename CaseT::left_t>::type>::type type;
                ...
                template<typename ParserT>
                static type get(parser<ParserT> const& p)
                {
            ...
            {
                template<typename ParserT>
                static nothing_parser get(parser<ParserT> const&)
                {
                    return nothing_p;
                }
            };
            ...
            struct default_case<CaseT, true>
            {
                ...
                BOOST_STATIC_CONSTANT(bool, is_epsilon = (CaseT::is_default && CaseT::is_epsilon));

                typedef typename boost::mpl::if_c<CaseT::is_default, CaseT, nothing_parser>::type type;
            ...
            struct case_chain
            {
                BOOST_STATIC_CONSTANT(int, depth = (case_chain<typename CaseT::left_t>::depth + 1));
            };
            ...
            struct chain_parser
            {
                ...
                static left_t left(CaseT const& p) { return chain_parser<Depth - 1, our_left_t>::left(p.left()); }

                static right_t right(CaseT const& p) { return chain_parser<Depth - 1, our_left_t>::right(p.left()); }
            };
            ...
            struct compound_case_parser :
                public binary<LeftT, RightT, parser<compound_case_parser<LeftT, RightT, IsDefault>>>
            {
                ...
                BOOST_STATIC_CONSTANT(
                    bool,
                    is_epsilon = (is_default && boost::is_same<typename RightT::subject_t, epsilon_parser>::value));
                ...
                typename parser_result<self_t, ScannerT>::type parse(ScannerT const& scan, CondT const& cond) const;

                template<int N1, typename ParserT1, bool IsDefault1>
                compound_case_parser<self_t, case_parser<N1, ParserT1, IsDefault1>, IsDefault1>
                    operator,(case_parser<N1, ParserT1, IsDefault1> const& p) const
                {
                    ...
                    BOOST_STATIC_ASSERT(case_chain<self_t>::depth < BOOST_SPIRIT_SWITCH_CASE_LIMIT);
...
#define BOOST_SPIRIT_PARSE_SWITCH_TYPEDEFS(z, N, _)                                         \
    typedef typename BOOST_PP_CAT(left_t, N)::left_t BOOST_PP_CAT(left_t, BOOST_PP_INC(N)); \
    /**/

#define BOOST_SPIRIT_PARSE_SWITCH_CASES(z, N, _)                                  \
case (long)(BOOST_PP_CAT(left_t, N)::value):                                      \
    return delegate_parse(chain_parser<N, left_t1>::right(p.left()), scan, save); \
    /**/
...
#define BOOST_SPIRIT_PARSE_SWITCHES(z, N, _)                                                                        \
    ...
    {                                                                                                               \
        template<typename ParserT, typename ScannerT>                                                               \
        static typename parser_result<ParserT, ScannerT>::type                                                      \
            do_(ParserT const& p, ScannerT const& scan, long cond_value, typename ScannerT::iterator_t const& save) \
        {                                                                                                           \
            typedef ParserT left_t0;                                                                                \
            BOOST_PP_REPEAT_FROM_TO_##z(0, BOOST_PP_INC(N), BOOST_SPIRIT_PARSE_SWITCH_TYPEDEFS, _)                  \
                                                                                                                    \
                switch (cond_value)                                                                                 \
            {                                                                                                       \
            case (long)(BOOST_PP_CAT(left_t, BOOST_PP_INC(N))::value):                                              \
                return delegate_parse(chain_parser<case_chain<ParserT>::depth, ParserT>::left(p), scan, save);      \
                                                                                                                    \
            BOOST_PP_REPEAT_FROM_TO_##z(1, BOOST_PP_INC(N), BOOST_SPIRIT_PARSE_SWITCH_CASES, _)                     \
                                                                                                                    \
                case (long)(left_t0::value):                                                                        \
            default:                                                                                                \
                typedef default_case<ParserT> default_t;                                                            \
                typedef default_delegate_parse<Value, IsDefault, default_t::value> default_parse_t;                 \
                                                                                                                    \
                return default_parse_t::parse(cond_value, p.right(), default_t::get(p), scan, save);                \
            }                                                                                                       \
        }                                                                                                           \
    };                                                                                                              \
    /**/

            BOOST_PP_REPEAT(BOOST_PP_DEC(BOOST_SPIRIT_SWITCH_CASE_LIMIT), BOOST_SPIRIT_PARSE_SWITCHES, _)
...
#undef BOOST_SPIRIT_PARSE_SWITCHES
            ...
            template<typename LeftT, typename RightT, bool IsDefault>
            template<typename ScannerT, typename CondT>
            inline typename parser_result<compound_case_parser<LeftT, RightT, IsDefault>, ScannerT>::type
                compound_case_parser<LeftT, RightT, IsDefault>::parse(ScannerT const& scan, CondT const& cond) const
            {
                ignore_unused(scan.at_end()); // allow skipper to take effect
                return parse_switch<value, case_chain<self_t>::depth, is_default>::do_(
                    *this,
                    scan,
                    cond(scan),
                    scan.first);
            ...
            struct cond_functor
            {
                ...
                template<typename ScannerT>
                typename condition_result<self_t, ScannerT>::type operator()(ScannerT const& scan) const
                {
                    typedef typename parser_result<ParserT, ScannerT>::type result_t;
            ...
            struct make_cond_functor
            {
                ...
                static cond_functor<typename as_parser_t::type> do_(ParserT const& cond)
                {
                    return cond_functor<typename as_parser_t::type>(as_parser_t::convert(cond));
                }
            };
            ...
            struct cond_actor
            {
                ...
                struct result
                {
                    typedef typename ::phoenix::actor_result<ActorT, ::phoenix::tuple<>>::type type;
                };
            ...
            {
                static cond_actor<::phoenix::actor<ActorT>> do_(::phoenix::actor<ActorT> const& actor)
                {
            ...
            {
                static get_next_token_cond do_(get_next_token_cond const& cond) { return cond; }
            };
