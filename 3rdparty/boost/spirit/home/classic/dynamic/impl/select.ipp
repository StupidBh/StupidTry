    ...
    namespace spirit {
        ...
        namespace impl {
            ...
            struct select_match_gen<ResultT, select_default_no_fail>
            {
                ...
                static ResultT do_(ScannerT const& scan)
                {
            ...
            struct parse_tuple_element
            {
                ...
                template<typename ScannerT>
                static ResultT do_(TupleT const& t, ScannerT const& scan)
                {
                    ...
                    if (result) {
                        return scan.create_match(result.length(), TupleT::length - N, save, scan.first);
                    }
                    scan.first = save; // reset the input stream
                    return parse_tuple_element<N - 1, ResultT, TupleT, BehaviourT>::do_(t, scan);
                }
            };
            ...
            struct parse_tuple_element<1, ResultT, TupleT, BehaviourT>
            {
                ...
                static ResultT do_(TupleT const& t, ScannerT const& scan)
                {
                    ...
                    if (result) {
                        return scan.create_match(result.length(), TupleT::length - 1, save, scan.first);
                    }
                    scan.first = save; // reset the input stream
