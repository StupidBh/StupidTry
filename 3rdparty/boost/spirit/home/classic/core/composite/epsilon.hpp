...
namespace boost {
    namespace spirit {
        ...
        struct condition_parser : parser<condition_parser<CondT, positive_>>
        {
            ...
            condition_parser(CondT const& cond_) :
                cond(cond_)
            {
            }
            ...
            condition_parser<CondT, !positive_> negate() const { return condition_parser<CondT, !positive_>(cond); }
    ...
    BOOST_WORKAROUND(__SUNPRO_CC, <= 0x580)
        ...
        inline condition_parser<CondT, !positive> operator~(condition_parser<CondT, positive> const& p)
        {
            return p.negate();
        }
        ...
        struct empty_match_parser : unary<SubjectT, parser<empty_match_parser<SubjectT>>>
        {
            ...
            explicit empty_match_parser(SubjectT const& p) :
                base_t(p)
            {
            }
        ...
        struct negated_empty_match_parser : public unary<SubjectT, parser<negated_empty_match_parser<SubjectT>>>
        {
            ...
            explicit negated_empty_match_parser(SubjectT const& p) :
                base_t(p)
            {
            }
            ...
            empty_match_parser<SubjectT> negate() const { return empty_match_parser<SubjectT>(this->subject()); }
        };
        ...
        namespace impl {
            ...
            struct epsilon_selector
            {
                ...
                typedef typename mpl::
                    if_<is_parser<subject_t>, empty_match_parser<subject_t>, condition_parser<subject_t>>::type type;
            };
        }
