    ...
    namespace spirit {
        ...
        namespace impl {
            ...
            struct get_param
            {
                typedef typename mpl::if_<
                    ...
                    typename mpl::if_<
                        ...
                        T1,
                        typename mpl::if_<is_base_and_derived<BaseT, T2>, T2, DefaultT>::type>::type>::type type;
            };
            ...
            class rule_base :
                ...
                public context_aux<typename impl::get_context<T0, T1, T2>::type, DerivedT>,
            ...
            public:
                ...
                typename parser_result<DerivedT, ScannerT>::type parse(ScannerT const& scan) const
                {
                    ...
                    BOOST_SPIRIT_CONTEXT_PARSE(scan, *this, linked_scanner_t, linked_context_t, result_t);
                }
        ...
        #pragma warning(disable: 4512) // assignment operator could not be generated
            ...
            struct concrete_parser : abstract_parser<ScannerT, AttrT>
            {
                ...
                typename match_result<ScannerT, AttrT>::type do_parse_virtual(ScannerT const& scan) const BOOST_OVERRIDE
                {
                    return p.parse(scan);
                }

                abstract_parser<ScannerT, AttrT>* clone() const BOOST_OVERRIDE { return new concrete_parser(p); }
        ...
        #define BOOST_SPIRIT_RULE_ENUM_DOPARSE_A(z, N, _)                                           \
            virtual typename match_result<BOOST_PP_CAT(ScannerT, N), AttrT>::type do_parse_virtual( \
                BOOST_PP_CAT(ScannerT, N) const& scan) const = 0;

        #define BOOST_SPIRIT_ENUM_ABSTRACT_PARSERS(z, N, _)                                                   \
            template<BOOST_PP_ENUM_PARAMS_Z(z, BOOST_PP_INC(N), typename ScannerT), typename AttrT>           \
            struct abstract_parser<scanner_list<BOOST_PP_ENUM_PARAMS_Z(z, BOOST_PP_INC(N), ScannerT)>, AttrT> \
            {                                                                                                 \
                abstract_parser() {}                                                                          \
                virtual ~abstract_parser() {}                                                                 \
                                                                                                              \
                BOOST_PP_REPEAT_##z(BOOST_PP_INC(N), BOOST_SPIRIT_RULE_ENUM_DOPARSE_A, _)                     \
                                                                                                              \
                    virtual abstract_parser* clone() const = 0;                                               \
            };

            BOOST_PP_REPEAT_FROM_TO(1, BOOST_SPIRIT_RULE_SCANNERTYPE_LIMIT, BOOST_SPIRIT_ENUM_ABSTRACT_PARSERS, _)
        ...
        #define BOOST_SPIRIT_ENUM_CONCRETE_PARSERS(z, N, _)                                                           \
            template<typename ParserT, BOOST_PP_ENUM_PARAMS_Z(z, BOOST_PP_INC(N), typename ScannerT), typename AttrT> \
            struct concrete_parser<                                                                                   \
                ParserT,                                                                                              \
                scanner_list<BOOST_PP_ENUM_PARAMS_Z(z, BOOST_PP_INC(N), ScannerT)>,                                   \
                AttrT> : abstract_parser<scanner_list<BOOST_PP_ENUM_PARAMS_Z(z, BOOST_PP_INC(N), ScannerT)>, AttrT>   \
            {                                                                                                         \
                concrete_parser(ParserT const& p_) :                                                                  \
                    p(p_)                                                                                             \
                {                                                                                                     \
                ...
                virtual ~concrete_parser() {}                                                                         \
                                                                                                                      \
                BOOST_PP_REPEAT_##z(BOOST_PP_INC(N), BOOST_SPIRIT_RULE_ENUM_DOPARSE_C, _)                             \
                                                                                                                      \
                    virtual abstract_parser<                                                                          \
                        scanner_list<BOOST_PP_ENUM_PARAMS_Z(z, BOOST_PP_INC(N), ScannerT)>,                           \
                        AttrT>* clone() const                                                                         \
            ...
            BOOST_PP_REPEAT_FROM_TO(1, BOOST_SPIRIT_RULE_SCANNERTYPE_LIMIT, BOOST_SPIRIT_ENUM_CONCRETE_PARSERS, _)
