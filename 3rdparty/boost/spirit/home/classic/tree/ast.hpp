    ...
    namespace spirit {
        ...
        struct ast_match_policy :
            public common_tree_match_policy<
                ...
                IteratorT,
                NodeFactoryT,
                ast_tree_policy<ast_match_policy<IteratorT, NodeFactoryT, T>, NodeFactoryT, T>,
                T>
        {
        ...
        struct ast_tree_policy : public common_tree_tree_policy<MatchPolicyT, NodeFactoryT>
        {
            ...
            static void concat(MatchAT& a, MatchBT const& b)
            {
                ...
                if (0 != b.trees.size() && b.trees.begin()->value.is_root()) {
                    ...
                    while (pnon_root_trees->size() > 0 && pnon_root_trees->begin()->value.is_root()) {
                        pnon_root_trees = &pnon_root_trees->begin()->children;
                    }
                    pnon_root_trees->insert(pnon_root_trees->begin(), tmp.begin(), tmp.end());
                }
                    ...
                    std::copy(
                        b.trees.begin(),
                        b.trees.end(),
                        std::back_insert_iterator<container_t>(a.trees.begin()->children));
                }
                else {
#if !defined(BOOST_SPIRIT_USE_LIST_FOR_TREES)
                    a.trees.reserve(a.trees.size() + b.trees.size());
#endif
                    std::copy(b.trees.begin(), b.trees.end(), std::back_insert_iterator<container_t>(a.trees));
                }
            ...
            static void group_match(MatchT& m, parser_id const& id, Iterator1T const& first, Iterator2T const& last)
            {
                ...
                if (m.trees.size() == 1
                    ...
                    while (punset_id->size() > 0 && punset_id->begin()->value.id() == 0) {
                    ...
                    for (cont_iterator_t i = newmatch.trees.begin(); i != newmatch.trees.end(); ++i) {
                        if (i->value.id() == 0) {
                            i->value.id(id);
                        }
                    }
        ...
        namespace impl {
            ...
            {
                typedef ast_tree_policy<ast_match_policy<IteratorT, NodeFactoryT, T>, NodeFactoryT, T> type;
            };
        ...
        template<typename T>
        struct gen_ast_node_parser : public unary<T, parser<gen_ast_node_parser<T>>>
        {
        ...
        struct gen_ast_node_parser_gen
        {
            ...
            template<typename T>
            static gen_ast_node_parser<T> generate(parser<T> const& s)
            {
                return gen_ast_node_parser<T>(s.derived());
            }

            template<typename T>
            gen_ast_node_parser<T> operator[](parser<T> const& s) const
            {
                return gen_ast_node_parser<T>(s.derived());
            }
        };
        ...
        inline tree_parse_info<IteratorT, AstFactoryT> ast_parse(
            ...
            AstFactoryT const& /*dummy_*/ = AstFactoryT())
        {
            ...
            typedef scanner_policies<it_policy_t, ast_match_policy_t> scan_policies_t;
        ...
        inline tree_parse_info<IteratorT> ast_parse(
            IteratorT const& first_,
            IteratorT const& last_,
            parser<ParserT> const& parser,
            SkipT const& skip_)
        ...
        inline tree_parse_info<IteratorT>
            ast_parse(IteratorT const& first_, IteratorT const& last, parser<ParserT> const& parser)
        {
            typedef ast_match_policy<IteratorT> ast_match_policy_t;
            IteratorT first = first_;
            scanner<IteratorT, scanner_policies<iteration_policy, ast_match_policy_t>> scan(first, last);
        ...
        inline tree_parse_info<CharT const*>
            ast_parse(CharT const* str, parser<ParserT> const& parser, SkipT const& skip)
        {
        ...
        inline tree_parse_info<CharT const*> ast_parse(CharT const* str, parser<ParserT> const& parser)
        {
            CharT const* last = str;
            while (*last) {
                last++;
