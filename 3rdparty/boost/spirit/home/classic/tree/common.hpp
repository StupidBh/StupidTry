...
#if defined(BOOST_SPIRIT_DEBUG) && (BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES)
    #include <iostream>
...
namespace boost {
    namespace spirit {
...
#if defined(BOOST_SPIRIT_DEBUG) && (BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES)
        template<typename T>
        inline std::ostream& operator<<(std::ostream& o, tree_node<T> const& n)
        {
            static int depth = 0;
            o << "\n";
            ...
            int c = 0;
            for (typename tree_node<T>::children_t::const_iterator it = n.children.begin(); it != n.children.end();
                 ++it) {
                o << " children[" << c++ << "] = " << *it;
            }
            o << ")";
        ...
        struct node_iter_data
        {
            ...
            IteratorT begin() { return first; }

            IteratorT const& begin() const { return first; }

            IteratorT end() { return last; }

            IteratorT const& end() const { return last; }

            bool is_root() const { return is_root_; }

            void is_root(bool b) { is_root_ = b; }

            parser_id id() const { return parser_id_; }

            void id(parser_id r) { parser_id_ = r; }
        ...
        inline std::ostream& operator<<(std::ostream& o, nil_t const&)
        {
            return o;
        }

        template<typename IteratorT, typename ValueT>
        inline std::ostream& operator<<(std::ostream& o, node_iter_data<IteratorT, ValueT> const& n)
        {
            o << "(id = " << n.id() << " text = \"";
            typedef typename node_iter_data<IteratorT, ValueT>::const_iterator_t iterator_t;
            for (iterator_t it = n.begin(); it != n.end(); ++it) {
                impl::token_printer(o, *it);
            }
            o << "\" is_root = " << n.is_root() << /*" value = " << n.value() << */ ")";
            return o;
        }
#endif
...
#if defined(BOOST_SPIRIT_DEBUG) && (BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES)
        template<typename IteratorT, typename ValueT>
        inline std::ostream& operator<<(std::ostream& o, node_val_data<IteratorT, ValueT> const& n)
        {
            o << "(id = " << n.id() << " text = \"";
            typedef typename node_val_data<IteratorT, ValueT>::const_iterator_t iterator_t;
            for (iterator_t it = n.begin(); it != n.end(); ++it) {
                impl::token_printer(o, *it);
            }
            o << "\" is_root = " << n.is_root() << " value = " << n.value() << ")";
        ...
        public:
            ...
            public:
                ...
                static node_t group_nodes(ContainerT const& nodes)
                {
                    ...
                    for (typename ContainerT::const_iterator i = nodes.begin(); i != i_end; ++i) {
        ...
        public:
            ...
            explicit tree_match(std::size_t length_) :
                match<T>(length_),
                trees()
            {
            }
...
#if defined(BOOST_SPIRIT_DEBUG) && (BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_NODES)
        template<typename IteratorT, typename NodeFactoryT, typename T>
        inline std::ostream& operator<<(std::ostream& o, tree_match<IteratorT, NodeFactoryT, T> const& m)
        {
            typedef typename tree_match<IteratorT, NodeFactoryT, T>::container_t::iterator iterator;
        ...
        struct common_tree_match_policy : public match_policy
        {
            ...
            static const match_t empty_match() { return match_t(0, tree_policy_t::empty_node()); }

            template<typename AttrT, typename Iterator1T, typename Iterator2T>
            static tree_match<IteratorT, NodeFactoryT, AttrT>
...
#endif
                return tree_match<IteratorT, NodeFactoryT, AttrT>(
                    length,
                    val,
...
#if defined(BOOST_SPIRIT_DEBUG) && (BOOST_SPIRIT_DEBUG_FLAGS & BOOST_SPIRIT_DEBUG_FLAGS_TREES)
        ...
        struct common_tree_tree_policy
        {
            ...
            static node_t
                create_node(std::size_t /*length*/, Iterator1T const& first, Iterator2T const& last, bool leaf_node)
            {
                return factory_t::create_node(first, last, leaf_node);
            }

            static node_t empty_node() { return factory_t::empty_node(); }
        ...
        struct leaf_node_parser : public unary<T, parser<leaf_node_parser<T>>>
        {
            ...
            typename parser_result<self_t, ScannerT>::type parse(ScannerT const& scanner) const
            {
                ...
                result_t hit = impl::contiguous_parser_parse<result_t>(
                    this->subject(),
                    scanner.change_policies(policies_t(scanner, match_policy(), scanner)),
                    scanner);

                if (hit) {
                    return result_t(hit.length(), factory_t::create_node(from, scanner.first, true));
                }
                else {
        ...
        struct node_parser : public unary<T, parser<node_parser<T, NodeParserT>>>
        {
            ...
            typename parser_result<self_t, ScannerT>::type parse(ScannerT const& scanner) const
            {
                ...
                if (hit) {
                    impl::tree_policy_selector<typename ScannerT::match_policy_t>::type::apply_op_to_match(
                        NodeParserT(),
                        hit);
                }
                return hit;
        ...
        //////////////////////////////////
        template<typename ParserT, typename ActionT>
        access_match_action::action<ParserT, ActionT>::action(ParserT const& subject, ActionT const& actor_) :
        ...
        typename parser_result<access_match_action::action<ParserT, ActionT>, ScannerT>::type
            access_match_action::action<ParserT, ActionT>::parse(ScannerT const& scan) const
        {
            typedef typename ScannerT::iterator_t iterator_t;
            typedef typename parser_result<self_t, ScannerT>::type result_t;
            if (!scan.at_end()) {
        ...
        //////////////////////////////////
        template<typename ParserT, typename ActionT>
        access_node_action::action<ParserT, ActionT>::action(ParserT const& subject, ActionT const& actor_) :
        ...
        typename parser_result<access_node_action::action<ParserT, ActionT>, ScannerT>::type
            access_node_action::action<ParserT, ActionT>::parse(ScannerT const& scan) const
        {
            typedef typename ScannerT::iterator_t iterator_t;
            typedef typename parser_result<self_t, ScannerT>::type result_t;
            if (!scan.at_end()) {
