    ...
    namespace spirit {
        namespace x3 {
            ...
            struct eps_parser : parser<eps_parser>
            {
                ...
                bool
                    parse(Iterator& first, Iterator const& last, Context const& context, RuleContext&, Attribute&) const
                {
                    x3::skip_over(first, last, context);
                    return true;
                }

                constexpr semantic_predicate operator()(bool predicate) const { return { predicate }; }
