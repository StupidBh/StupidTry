    ...
    namespace spirit {
        namespace x3 {
            ...
            public:
                ...
                {
                    BOOST_STATIC_ASSERT_MSG((is_same<Iterator, Iterator_>::value), "Incompatible iterator used");

                    BOOST_ASSERT_MSG((_content != nullptr), "Invalid use of uninitialized any_parser");
                ...
                std::string get_info() const { return _content ? _content->get_info() : ""; }
            ...
            private:
                struct placeholder
                {
                    ...
                    virtual bool
                        parse(Iterator& first, Iterator const& last, Context const& context, Attribute& attr) const = 0;
                ...
                struct holder : placeholder
                {
                    ...
                    holder* clone() const override { return new holder(*this); }

                    bool parse(Iterator& first, Iterator const& last, Context const& context, Attribute& attr)
                        const override
                    {
                        return _parser.parse(first, last, context, unused, attr);
                    }

                    std::string get_info() const override { return x3::what(_parser); }
            ...
            struct get_info<any_parser<Iterator, Attribute, Context>>
            {
                ...
                std::string operator()(any_parser<Iterator, Attribute, Context> const& p) const { return p.get_info(); }
            };
        }
    }
}
