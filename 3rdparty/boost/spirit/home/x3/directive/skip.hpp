...
namespace boost {
        ...
        namespace x3 {
            ...
            struct reskip_directive : unary_parser<Subject, reskip_directive<Subject>>
            {
                ...
                typename disable_if<has_skipper<Context>, bool>::type
                    parse(Iterator& first, Iterator const& last, Context& context, RContext& rcontext, Attribute& attr)
                        const
                {
                ...
                template<typename Iterator, typename Context, typename RContext, typename Attribute>
                typename enable_if<has_skipper<Context>, bool>::type parse(
                    Iterator& first,
                    Iterator const& last,
            ...
            struct skip_directive : unary_parser<Subject, skip_directive<Subject, Skipper>>
            {
                    ...
                    Attribute& attr) const
                {
                    ...
                    return this->subject.parse(first, last, make_context<skipper_tag>(skipper), rcontext, attr);
                }
    ...
    #if BOOST_SPIRIT_X3_THROW_EXPECTATION_FAILURE
                    return this->subject
                        .parse(first, last, make_context<skipper_tag>(skipper, context), rcontext, attr);
