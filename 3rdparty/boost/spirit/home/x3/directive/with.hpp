...
namespace boost {
    namespace spirit {
        namespace x3 {
            ...
            template<typename Subject, typename Derived, typename T>
            struct with_value_holder<Subject, Derived, T&> : unary_parser<Subject, Derived>
            {
                typedef unary_parser<Subject, Derived> base_type;
                T& val;
            ...
            struct with_directive : with_value_holder<Subject, with_directive<Subject, ID, T>, T>
            {
                ...
                {
                    return this->subject.parse(first, last, make_context<ID>(this->val, context), rcontext, attr);
                }
            };
