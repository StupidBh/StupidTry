            ...
            struct expect_directive : unary_parser<Subject, expect_directive<Subject>>
            {
                    ...
                    Attribute& attr) const
                {
                    ...
                    if (!r) {
                        ...
                        boost::throw_exception(expectation_failure<Iterator>(first, what(this->subject)));
    #else
            ...
            namespace detail {
                ...
                struct parse_into_container_impl<expect_directive<Subject>, Context, RContext>
                {
                    ...
                    static bool call(
                        ...
                        Attribute& attr)
                    {
                        bool const r = parse_into_container(parser.subject, first, last, context, rcontext, attr);

                        if (!r) {
    #if BOOST_SPIRIT_X3_THROW_EXPECTATION_FAILURE
                            boost::throw_exception(expectation_failure<Iterator>(first, what(parser.subject)));
    #else
                            if (!has_expectation_failure(context)) {
