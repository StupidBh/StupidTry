    ...
    namespace spirit {
        namespace x3 {
            ...
            struct bool_parser : parser<bool_parser<T, Encoding, BoolPolicies>>
            {
                ...
                bool parse(Iterator& first, Iterator const& last, Context const& context, unused_type, T& attr) const
                {
            ...
            struct literal_bool_parser : parser<bool_parser<T, Encoding, BoolPolicies>>
            {
                ...
                template<typename Iterator, typename Context>
                bool parse_main(Iterator& first, Iterator const& last, Context const& context, T& attr) const
                {
                    x3::skip_over(first, last, context);
