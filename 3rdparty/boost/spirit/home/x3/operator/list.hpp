            ...
            struct list : binary_parser<Left, Right, list<Left, Right>>
            {
                    ...
                    Attribute& attr) const
                {
                    ...
                    while (this->right.parse(iter, last, context, rcontext, unused) &&
                           detail::parse_into_container(this->left, iter, last, context, rcontext, attr)) {
                        first = iter;
                    }
        ...
        namespace x3 {
            namespace traits {
                ...
                struct attribute_of<x3::list<Left, Right>, Context> :
                    traits::build_container<typename attribute_of<Left, Context>::type>
                {
                };

                template<typename Left, typename Right, typename Context>
                struct has_attribute<x3::list<Left, Right>, Context> : has_attribute<Left, Context>
                {
                };
            }
        }
