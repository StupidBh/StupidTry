    ...
    namespace spirit {
        namespace x3 {
            ...
            struct binary_parser : parser<Derived>
            {
                ...
                static bool const has_action = left_type::has_action || right_type::has_action;
            ...
            namespace extension {
                namespace detail {
                    namespace as_parser_guard {
                        ...
                        template<typename T, typename R = decltype(as_spirit_parser(boost::declval<T const&>()))>
                        struct deduce_as_parser
                        {
                            typedef R type;
                            typedef
                                typename boost::remove_cv<typename boost::remove_reference<R>::type>::type value_type;

                            static type call(T const& v) { return as_spirit_parser(v); }
                        };
                ...
                struct as_parser<unused_type>
                {
                    ...
                    static constexpr type call(unused_type) { return unused; }
                };
                ...
                struct as_parser<Derived, typename enable_if<is_base_of<parser_base, Derived>>::type>
                {
                    ...
                    static constexpr type call(Derived const& p) { return p; }
                };
                ...
                struct as_parser<parser<Derived>>
                {
                    ...
                    static constexpr type call(parser<Derived> const& p) { return p.derived(); }
                };
            }

            template<typename T>
            constexpr typename extension::as_parser<T>::type as_parser(T const& x)
            {
                return extension::as_parser<T>::call(x);
            }

            template<typename Derived>
            constexpr Derived const& as_parser(parser<Derived> const& p)
            {
                return p.derived();
            }
    ...
    namespace spirit {
        namespace x3 {
            namespace traits {
                ...
                struct has_attribute<x3::unary_parser<Subject, Derived>, Context> : has_attribute<Subject, Context>
                {
                };
                ...
                struct has_attribute<x3::binary_parser<Left, Right, Derived>, Context> :
                    mpl::bool_<has_attribute<Left, Context>::value || has_attribute<Right, Context>::value>
                {
                };
            }
        }
