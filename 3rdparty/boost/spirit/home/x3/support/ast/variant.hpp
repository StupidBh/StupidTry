    ...
    namespace spirit {
        namespace x3 {
            ...
            public:
                ...
                forward_ast(forward_ast&& operand) BOOST_NOEXCEPT : p_(operand.p_) { operand.p_ = 0; }
                ...
                ~forward_ast() { boost::checked_delete(p_); }

                forward_ast& operator=(forward_ast const& rhs)
                    BOOST_NOEXCEPT_IF(std::is_nothrow_copy_assignable<T>::value)
                {
                    assign(rhs.get());
                    return *this;
                }
        ...
        #pragma warning(disable: 4521) // multiple copy constructors specified
            ...
            struct variant
            {
                ...
                using non_self_t // used only for SFINAE checks below
                    = std::enable_if_t<!(std::is_base_of<base_type, std::remove_reference_t<T>>::value)>;

                variant() BOOST_NOEXCEPT_IF(std::is_nothrow_default_constructible<variant_type>::value) :
                    var()
                {
                }
                ...
                explicit variant(T const& rhs)
                    BOOST_NOEXCEPT_IF((std::is_nothrow_constructible<variant_type, T const&>::value)) :
                    var(rhs)
                {
                }
                ...
                variant(variant& rhs)
                    BOOST_NOEXCEPT_IF((std::is_nothrow_constructible<variant_type, variant_type&>::value)) :
                    var(rhs.var)
                {
                }
                ...
                variant& operator=(variant const& rhs)
                    BOOST_NOEXCEPT_IF(std::is_nothrow_copy_assignable<variant_type>::value)
                {
                    var = rhs.get();
                    return *this;
                }
                ...
                variant& operator=(variant&& rhs)
                    BOOST_NOEXCEPT_IF(std::is_nothrow_move_assignable<variant_type>::value)
                {
                ...
                variant& operator=(T const& rhs)
                    BOOST_NOEXCEPT_IF((std::is_nothrow_assignable<variant_type, T const&>::value))
                {
                    var = rhs;
                    return *this;
                }
                ...
                variant_type const& get() const BOOST_NOEXCEPT { return var; }

                variant_type& get() BOOST_NOEXCEPT { return var; }

                void swap(variant& rhs) BOOST_NOEXCEPT { var.swap(rhs.var); }
...
namespace boost {
    ...
    inline T const& get(boost::spirit::x3::variant<Types...> const& x) BOOST_NOEXCEPT
    {
        return boost::get<T>(x.get());
    }

    template<typename T, typename... Types>
    inline T& get(boost::spirit::x3::variant<Types...>& x) BOOST_NOEXCEPT
    {
        return boost::get<T>(x.get());
    }
