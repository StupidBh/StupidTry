...
namespace boost {
    namespace spirit {
        namespace x3 {
            ...
            inline namespace BOOST_SPIRIT_X3_EXPECTATION_FAILURE_NS {
                ...
                using expectation_failure_t = std::remove_cv_t<
                    std::remove_reference_t<decltype(x3::get<expectation_failure_tag>(std::declval<Context>()))>>;
                ...
                namespace expectation_failure_helpers {
                    // bare type
                    template<typename Iterator>
                    BOOST_ATTRIBUTE_NODISCARD constexpr decltype(auto)
                        where(expectation_failure<Iterator> const& failure) noexcept
                    {
                    ...
                    template<typename Iterator>
                    BOOST_ATTRIBUTE_NODISCARD constexpr decltype(auto)
                        which(expectation_failure<Iterator> const& failure) noexcept
                    {
                    ...
                    template<typename Iterator>
                    BOOST_ATTRIBUTE_NODISCARD constexpr decltype(auto) where(
                        std::reference_wrapper<std::optional<expectation_failure<Iterator>>> const& failure) noexcept
                    {
                        return failure.get()->where();
                    }

                    template<typename Iterator>
                    BOOST_ATTRIBUTE_NODISCARD constexpr decltype(auto) which(
                        std::reference_wrapper<std::optional<expectation_failure<Iterator>>> const& failure) noexcept
                    {
                        return failure.get()->which();
                    }

                    // boost::optional + std::reference_wrapper
                    template<typename Iterator>
                    BOOST_ATTRIBUTE_NODISCARD constexpr decltype(auto) where(
                        std::reference_wrapper<boost::optional<expectation_failure<Iterator>>> const& failure) noexcept
                    {
                        return failure.get()->where();
                    }
            ...
            } // namespace BOOST_SPIRIT_X3_EXPECTATION_FAILURE_NS
            ...
            namespace detail {
                inline namespace BOOST_SPIRIT_X3_EXPECTATION_FAILURE_NS {
                    ...
                    template<typename Iterator>
                    constexpr bool has_expectation_failure_impl(
                        std::optional<expectation_failure<Iterator>> const& failure) noexcept
                    {
                    ...
                    template<typename Iterator>
                    constexpr bool has_expectation_failure_impl(
                        boost::optional<expectation_failure<Iterator>> const& failure) noexcept
                    {
                        return failure.has_value();
                    }
                    ...
                    template<typename Iterator, typename T>
                    constexpr void
                        set_expectation_failure_impl(std::optional<expectation_failure<Iterator>>& failure, T&& value)
                    {
                        failure = std::forward<T>(value);
                    }
                    ...
                    template<typename Iterator, typename T>
                    constexpr void
                        set_expectation_failure_impl(boost::optional<expectation_failure<Iterator>>& failure, T&& value)
                    {
                        failure = std::forward<T>(value);
                    }
                    ...
                    constexpr void
                        set_expectation_failure_impl(std::reference_wrapper<AnyExpectationFailure>& failure, T&& value)
                    {
                    ...
                    template<typename Iterator>
                    constexpr void
                        clear_expectation_failure_impl(std::optional<expectation_failure<Iterator>>& failure) noexcept
                    {
                        failure.reset();
                    }
                    ...
                    template<typename Iterator>
                    constexpr void
                        clear_expectation_failure_impl(boost::optional<expectation_failure<Iterator>>& failure) noexcept
                    {
                        failure.reset();
                    }
            ...
            inline namespace BOOST_SPIRIT_X3_EXPECTATION_FAILURE_NS {
                template<typename Context>
                BOOST_ATTRIBUTE_NODISCARD constexpr bool has_expectation_failure(Context const& context) noexcept
                {
    #if BOOST_SPIRIT_X3_THROW_EXPECTATION_FAILURE
                ...
                constexpr void
                    set_expectation_failure(Iterator const& where, Subject const& subject, Context const& context)
                {
    #if BOOST_SPIRIT_X3_THROW_EXPECTATION_FAILURE
                    ...
                    if constexpr (std::is_same_v<T, bool>) {
                        ...
                        detail::set_expectation_failure_impl(x3::get<expectation_failure_tag>(context), true);
                    }
                    else {
                ...
                constexpr void
                    set_expectation_failure(AnyExpectationFailure const& existing_failure, Context const& context)
                {
    #if BOOST_SPIRIT_X3_THROW_EXPECTATION_FAILURE
    ...
    #else
                template<typename Context>
                BOOST_ATTRIBUTE_NODISCARD constexpr decltype(auto) get_expectation_failure(Context const& context)
                {
                    using T = expectation_failure_t<Context>;
                    static_assert(
                        !std::is_same_v<T, unused_type>,
