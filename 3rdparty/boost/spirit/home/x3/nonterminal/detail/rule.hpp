...
namespace boost {
        ...
        namespace x3 {
            namespace detail {
                ...
                struct has_on_error<
                    ...
                    Iterator,
                    Context,
                    decltype(void(std::declval<ID>().on_error(
                        std::declval<Iterator&>(),
                        std::declval<Iterator>(),
                        std::declval<expectation_failure<Iterator>>(),
                        std::declval<Context>())))> : mpl::true_
                {
                };
                ...
                struct has_on_success<
                    ...
                    Context,
                    Attribute,
                    decltype(void(std::declval<ID>().on_success(
                        std::declval<Iterator&>(),
                        std::declval<Iterator&>(),
                ...
                Context const& make_rule_context(
                    RHS const& /* rhs */,
                    Context const& context,
                ...
                template<typename ID, typename RHS, typename Context>
                auto make_rule_context(RHS const& rhs, Context const& context, mpl::true_ /* is_default_parse_rule */)
                {
                ...
                struct rule_parser
                {
                        ...
                        mpl::true_ /* Has on_success handler */)
                    {
                        ...
                        bool pass = true;
                        ID().on_success(before, after, attr, make_context<parse_pass_context_tag>(pass, context));
                        return pass;
                    }
                        ...
                        mpl::false_)
                    {
                        ...
                        typedef decltype(parse_rule(
                            detail::rule_id<ID> {},
                            first,
                            last,
                            make_unique_context<ID>(rhs, context),
                            std::declval<Attribute&>())) parse_rule_result;
                        ...
                        bool r = rhs.parse(
                            first,
                            last,
                            make_rule_context<ID>(
                                rhs,
                                context,
                                std::conditional_t<skip_definition_injection, mpl::false_, is_default_parse_rule>()),
                            rcontext,
                            attr);
                        ...
                        mpl::true_ /* on_error is found */)
                    {
                        for (;;) {
                            ...
                            if (has_expectation_failure(context)) {
                                ...
                                case error_handler_result::fail: clear_expectation_failure(context); return false;
                        ...
                        ExplicitAttrPropagation)
                    {
                        ...
                        // rhs parser
                        typedef traits::transform_attribute<ActualAttribute, Attribute, parser_id> transform;
    ...
    #if defined(BOOST_SPIRIT_X3_DEBUG)
                            context_debug<Iterator, transform_attr> dbg(rule_name, first, last, attr_, ok_parse);
    #endif
                            ok_parse = parse_rhs(
                                rhs,
                                first,
