...
namespace boost {
    namespace spirit {
        namespace x3 {
            ...
            template<typename ID, typename Iterator, typename Context, typename ActualAttribute>
            inline detail::default_parse_rule_result parse_rule(
                detail::rule_id<ID>,
                Iterator& first,
                Iterator const& last,
            ...
            {
                static_assert(
                    !is_same<decltype(x3::get<ID>(context)), unused_type>::value,
            ...
            struct rule_definition :
                parser<rule_definition<ID, RHS, Attribute, force_attribute_, skip_definition_injection>>
            {
                ...
                bool parse(Iterator& first, Iterator const& last, Context const& context, unused_type, Attribute_& attr)
                    const
                {
                    return detail::rule_parser<attribute_type, ID, skip_definition_injection>::call_rule_definition(
                        rhs,
                        name,
                        first,
                        last,
            ...
            {
                static_assert(
                    !std::is_reference<Attribute>::value,
                    "Reference qualifier on rule attribute type is meaningless");

                typedef ID id;
                typedef Attribute attribute_type;
                static bool const has_attribute = !std::is_same<std::remove_const_t<Attribute>, unused_type>::value;
                static bool const handles_container = traits::is_container<Attribute>::value;
                static bool const force_attribute = force_attribute_;

    #if !defined(BOOST_SPIRIT_X3_NO_RTTI)
                rule() :
                    name(typeid(rule).name())
                {
                }
    #else
                constexpr rule() :
                ...
                template<typename RHS>
                constexpr rule_definition<ID, typename extension::as_parser<RHS>::value_type, Attribute, true>
                    operator%=(RHS const& rhs) const&
                {
                    return { as_parser(rhs), name };
                }
                ...
                template<typename RHS>
                constexpr rule_definition<ID, typename extension::as_parser<RHS>::value_type, Attribute, true, true>
                    operator%=(RHS const& rhs) const&&
                {
                    return { as_parser(rhs), name };
                }
                ...
                bool parse(Iterator& first, Iterator const& last, Context const& context, unused_type, Attribute_& attr)
                    const
                {
                    static_assert(has_attribute, "The rule does not have an attribute. Check your parser.");

                    using transform = traits::transform_attribute<Attribute_, attribute_type, parser_id>;
                ...
                bool
                    parse(Iterator& first, Iterator const& last, Context const& context, unused_type, unused_type) const
                {
            ...
            namespace traits {
                ...
                struct is_rule<rule_definition<ID, RHS, Attribute, force_attribute, skip_definition_injection>> :
                    mpl::true_
                {
                };
            }
    ...
    #define BOOST_SPIRIT_DECLARE(...)                                                          \
        BOOST_PP_SEQ_FOR_EACH(BOOST_SPIRIT_DECLARE_, _, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)) \
        /***/
        ...
        #define BOOST_SPIRIT_DEFINE_(r, data, rule_name)                                                              \
            ...
            inline bool parse_rule(                                                                                   \
                ...
                Iterator const& last,                                                                                 \
                Context const& context,                                                                               \
                BOOST_PP_CAT(rule_name, _synonym)::attribute_type& attr)                                              \
            {                                                                                                         \
                using rule_t = BOOST_JOIN(rule_name, _synonym);                                                       \
                return ::boost::spirit::x3::detail ::rule_parser<typename rule_t::attribute_type, rule_t::id, true>:: \
                    call_rule_definition(                                                                             \
                        BOOST_JOIN(rule_name, _def),                                                                  \
                        rule_name.name,                                                                               \
                        first,                                                                                        \
                        last,                                                                                         \
                        context,                                                                                      \
        ...
        #define BOOST_SPIRIT_DEFINE_(r, data, rule_name)                                                              \
            ...
            inline bool parse_rule(                                                                                   \
                ...
                Iterator const& last,                                                                                 \
                Context const& context,                                                                               \
                decltype(rule_name)::attribute_type& attr)                                                            \
            {                                                                                                         \
    ...
    #define BOOST_SPIRIT_DEFINE(...)                                                          \
        BOOST_PP_SEQ_FOR_EACH(BOOST_SPIRIT_DEFINE_, _, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)) \
        /***/
