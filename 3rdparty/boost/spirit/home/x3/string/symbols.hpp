    ...
    namespace spirit {
        namespace x3 {
            ...
            struct symbols_parser : parser<symbols_parser<Encoding, T, Lookup>>
            {
                ...
                static bool const has_attribute = !std::is_same<unused_type, attribute_type>::value;
                static bool const handles_container = traits::is_container<attribute_type>::value;
                ...
                symbols_parser(Symbols const& syms, Data const& data, std::string const& name = "symbols") :
                    symbols_parser(name)
                {
                    using std::begin;
                    auto di = begin(data);
                ...
                symbols_parser(std::initializer_list<char_type const*> syms, std::string const& name = "symbols") :
                    symbols_parser(name)
                {
                    for (auto str : syms) {
                        add(str);
                ...
                symbols_parser& operator=(symbols_parser const& rhs)
                {
                    name_ = rhs.name_;
                    lookup = rhs.lookup;
                    return *this;
                ...
                template<typename Str>
                friend adder const& operator+=(symbols_parser& sym, Str const& str)
                {
                    return sym.add(str);
                }

                template<typename Str>
                friend remover const& operator-=(symbols_parser& sym, Str const& str)
                {
                ...
                {
                    return *lookup->add(
                        traits::get_string_begin<char_type>(str),
                ...
                {
                    return find_impl(traits::get_string_begin<char_type>(str), traits::get_string_end<char_type>(str));
                }
            ...
            public:
                    ...
                    const
                {
                    ...
                    if (value_type const* val_ptr = lookup->find(first, last, get_case_compare<Encoding>(context))) {
                ...
                {
                    template<typename Iterator>
                    adder const& operator()(Iterator first, Iterator last, T const& val) const
                    {
                    ...
                    {
                        sym.lookup->add(
                            traits::get_string_begin<char_type>(s),
            ...
            struct get_info<symbols_parser<Encoding, T, Lookup>>
            {
                ...
                result_type operator()(symbols_parser<Encoding, T, Lookup> const& symbols) const
                {
                    return symbols.name();
                }
            };
