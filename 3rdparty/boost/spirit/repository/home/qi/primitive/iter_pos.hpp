...
namespace boost {
    namespace spirit {
        ...
        template<>
        struct use_terminal<qi::domain, repository::qi::tag::iter_pos> : mpl::true_
        {
        };
    }
}
        ...
        namespace repository {
            namespace qi {
                struct iter_pos_parser : boost::spirit::qi::primitive_parser<iter_pos_parser>
                {
                    ...
                    bool parse(Iterator& first, Iterator const& last, Context&, Skipper const& skipper, Attribute& attr)
                        const
                    {
