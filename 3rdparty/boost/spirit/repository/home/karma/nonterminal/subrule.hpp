...
namespace boost {
    namespace spirit {
        namespace repository {
            namespace karma {
                ...
                struct subrule_group_generator : spirit::karma::generator<subrule_group_generator<Defs>>
                {
                    ...
                    struct def_type
                    {
                        ...
                        BOOST_SPIRIT_ASSERT_MSG(
                            (fusion::result_of::has_key<defs_type const, id_type>::type::value),
                            subrule_used_without_being_defined,
                            (mpl::int_<ID>));

                        typedef typename fusion::result_of::at_key<defs_type const, id_type>::type type;
                    };
                    ...
                    struct attribute
                        ...
                        :
                        mpl::identity<typename remove_reference<
                            typename fusion::result_of::front<Defs>::type>::type::second_type::attr_type>
                    {
                    };
                    ...
                    {
                        // Forward to first subrule.
                        return generate_subrule(fusion::front(defs).second, sink, context, delimiter, attr);
                    }
                    ...
                    {
                        // Forward to first subrule.
                        return generate_subrule(fusion::front(defs).second, sink, context, delimiter, attr, params);
                    }

                    template<int ID, typename OutputIterator, typename Context, typename Delimiter, typename Attribute>
                    bool generate_subrule_id(
                        OutputIterator& sink,
                        Context& context,
                        Delimiter const& delimiter,
                        Attribute const& attr) const
                    {
                        return generate_subrule(def<ID>(), sink, context, delimiter, attr);
                    }
                    ...
                    bool generate_subrule_id(
                        ...
                        Params const& params) const
                    {
                        return generate_subrule(def<ID>(), sink, context, delimiter, attr, params);
                    }
                        ...
                        Attribute const& attr) const
                    {
                        ...
                        typedef subrule_context<
                            this_type,
                            fusion::cons<subrule_attr_reference_type, subrule_parameter_types>,
                            subrule_locals_type>
                            context_type;

                        typedef traits::transform_attribute<Attribute const, subrule_attr_type, spirit::karma::domain>
                            transform;
                        ...
                        Params const& params) const
                    {
                        ...
                        context_type context(*this, transform::pre(attr), params, caller_context);
                ...
                struct subrule_group :
                    proto::extends<typename proto::terminal<subrule_group_generator<Defs>>::type, subrule_group<Defs>>
                {
                    ...
                    struct properties
                        ...
                        :
                        remove_reference<
                            typename fusion::result_of::front<Defs>::type>::type::second_type::subject_type::properties
                    {
                    };

                    static size_t const params_size =
                        // Forward to first subrule.
                        remove_reference<typename fusion::result_of::front<Defs>::type>::type::second_type::params_size;
                    ...
                    subrule_group<typename fusion::result_of::as_map<
                        typename fusion::result_of::join<Defs const, Defs2 const>::type>::type>
                        operator,(subrule_group<Defs2> const& other) const
                    {
                        typedef subrule_group<typename fusion::result_of::as_map<
                            typename fusion::result_of::join<Defs const, Defs2 const>::type>::type>
                            result_type;
                    ...
                    template<typename Defs2>
                    friend subrule_group<typename fusion::result_of::as_map<
                        typename fusion::result_of::join<Defs const, Defs2 const>::type>::type>
    ...
    #endif
                    {
                        return static_cast<subrule_group const&>(left).operator,(
                                   static_cast<subrule_group<Defs2> const&>(other));
                    }
    ...
    #include <boost/spirit/home/karma/nonterminal/detail/fcall.hpp>
                ...
                struct subrule_definition
                {
                    ...
                    typedef spirit::karma::detail::generator_binder<Subject, auto_type> binder_type;
                ...
                struct subrule :
                    proto::extends<
                        typename proto::terminal<spirit::karma::reference<subrule<ID_, T1, T2> const>>::type,
                        subrule<ID_, T1, T2>>,
                    spirit::karma::generator<subrule<ID_, T1, T2>>
                {
                    // FIXME should go fetch the real properties of this subrule's definition in the current context,
                    // but we don't
                    //  have the context here (properties would need to be 'template<typename Context> struct
                    //  properties' instead)
                    ...
                    typedef typename spirit::detail::extract_locals<template_params>::type locals_type;

                    // The subrule's encoding type
                    typedef typename spirit::detail::extract_encoding<template_params>::type encoding_type;

                    // The subrule's signature
                    typedef typename spirit::detail::
                        extract_sig<template_params, encoding_type, spirit::karma::domain>::type sig_type;

                    // This is the subrule's attribute type
                    typedef typename spirit::detail::attr_from_sig<sig_type>::type attr_type;
                    ...
                    typedef typename spirit::detail::params_from_sig<sig_type>::type parameter_types;
                    ...
                    struct def_type_helper
                    {
                        ...
                        typedef typename result_of::compile<spirit::karma::domain, Expr>::type subject_type;
                    ...
                    typename group_type_helper<Expr, false>::type operator=(Expr const& expr) const
                    {
                        ...
                        return result_type(
                            fusion::make_map<id_type>(def_type(compile<spirit::karma::domain>(expr), name_)));
                    }

    #define BOOST_SPIRIT_SUBRULE_MODULUS_ASSIGN_OPERATOR(lhs_ref, rhs_ref)                                           \
        template<typename Expr>                                                                                      \
        friend typename group_type_helper<Expr, true>::type operator%=(subrule lhs_ref sr, Expr rhs_ref expr)        \
        {                                                                                                            \
            typedef group_type_helper<Expr, true> helper;                                                            \
            typedef typename helper::def_type def_type;                                                              \
            typedef typename helper::type result_type;                                                               \
            return result_type(fusion::make_map<id_type>(def_type(compile<spirit::karma::domain>(expr), sr.name_))); \
        }                                                                                                            \
                    /**/
    ...
    #undef BOOST_SPIRIT_SUBRULE_MODULUS_ASSIGN_OPERATOR
                    ...
                    bool generate(
                        ...
                        Attribute const& attr) const
                    {
                        return context.group.template generate_subrule_id<ID_>(sink, context, delimiter, attr);
                    }
                        ...
                        Attribute const& /*attr*/) const
                    {
                        ...
                        BOOST_SPIRIT_ASSERT_FAIL(OutputIterator, subrule_used_outside_subrule_group, (id_type));
                    ...
                    bool generate(
                        ...
                        Params const& params) const
                    {
                        return context.group.template generate_subrule_id<ID_>(sink, context, delimiter, attr, params);
                    }
