...
namespace boost {
    namespace spirit {
        ...
        template<>
        struct use_lazy_directive<karma::domain, repository::tag::confix, 2> : mpl::true_
        {
        };
    ...
    namespace spirit {
        namespace repository {
            namespace karma {
                ...
                struct confix_generator : spirit::karma::primitive_generator<confix_generator<Subject, Prefix, Suffix>>
                {
                    ...
                    confix_generator(Subject const& subject, Prefix const& prefix, Suffix const& suffix) :
                        subject(subject),
                        prefix(prefix),
                        suffix(suffix)
                    {
                    ...
                    bool generate(OutputIterator& sink, Context& ctx, Delimiter const& d, Attribute const& attr) const
                    {
                        ...
                        return prefix.generate(sink, ctx, d, unused) && subject.generate(sink, ctx, d, attr) &&
...
namespace boost {
    namespace spirit {
        namespace karma {
            ...
            template<typename Prefix, typename Suffix, typename Subject, typename Modifiers>
            struct make_directive<
                terminal_ex<repository::tag::confix, fusion::vector2<Prefix, Suffix>>,
                Subject,
                Modifiers>
            {
                typedef typename result_of::compile<karma::domain, Prefix, Modifiers>::type prefix_type;
                typedef typename result_of::compile<karma::domain, Suffix, Modifiers>::type suffix_type;

                typedef repository::karma::confix_generator<Subject, prefix_type, suffix_type> result_type;
    ...
    namespace spirit {
        namespace traits {
            ...
            struct has_semantic_action<repository::karma::confix_generator<Subject, Prefix, Suffix>> :
                mpl::or_<has_semantic_action<Subject>, has_semantic_action<Prefix>, has_semantic_action<Suffix>>
            {
            };
        }
    }
