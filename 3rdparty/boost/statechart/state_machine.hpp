    ...
    namespace statechart {
        namespace detail {
            ...
            public:
                ...
                result operator()()
                {
                    return detail::result_utility::make_result(toState_.react_impl(evt_, eventType_));
                }
            ...
            public:
                ...
                static bool found(const void* pFound) { return pFound != 0; }
            ...
            struct state_cast_impl :
                public mpl::if_<is_pointer<Target>, state_cast_impl_pointer_target, state_cast_impl_reference_target>::
                    type
            {
            };
            ...
            public:
                ...
                typename RttiPolicy::id_type history_context_type() const { return historyContextType_; }

                friend bool operator<(const history_key& left, const history_key& right)
                {
                    return std::less<typename RttiPolicy::id_type>()(
                               left.historyContextType_,
                               right.historyContextType_) ||
                           ((left.historyContextType_ == right.historyContextType_) &&
                            (left.historizedOrthogonalRegion_ < right.historizedOrthogonalRegion_));
                }
        ...
        public:
            ...
            void initiate()
            {
...

                {
                    ...
                    detail::result_utility::get_result(
                        translator_(initial_construct_function(*this), exception_event_handler(*this)));
                    guard.dismiss();
                }
            ...
            void terminate()
            {
                terminator guard(*this, 0);
                detail::result_utility::get_result(
                    translator_(terminate_function(*this), exception_event_handler(*this)));
                guard.dismiss();
            }

            bool terminated() const { return pOutermostState_ == 0; }
            ...
            Target state_cast() const
            {
                ...
                for (typename state_list_type::const_iterator pCurrentLeafState = currentStates_.begin();
                     pCurrentLeafState != currentStatesEnd_;
                     ++pCurrentLeafState) {
                    const state_base_type* pCurrentState(get_pointer(*pCurrentLeafState));
...
// typically small compared to the cycles dynamic_cast needs
...
#endif
                        {
                            Target result = dynamic_cast<Target>(impl::deref_if_necessary(pCurrentState));
                ...
                for (typename state_list_type::const_iterator pCurrentLeafState = currentStates_.begin();
                    ...
                    while (pCurrentState != 0) {
                        if (pCurrentState->dynamic_type() == targetType) {
                            return static_cast<Target>(impl::deref_if_necessary(pCurrentState));
                        }
            ...
            public:
                //////////////////////////////////////////////////////////////////////
                explicit state_iterator(typename state_base_type::state_list_type::const_iterator baseIterator) :
                ...
                state_iterator operator++(int) { return state_iterator(baseIterator_++); }

                bool operator==(const state_iterator& right) const { return baseIterator_ == right.baseIterator_; }

                bool operator!=(const state_iterator& right) const { return !(*this == right); }
        ...
        protected:
            ...
            void post_event(const event_base_ptr_type& pEvent) { post_event_impl(pEvent); }

            void post_event(const event_base& evt) { post_event_impl(evt); }
        ...
        public:
            ...
            void clear_shallow_history()
            {
                ...
                typedef typename mpl::at_c<typename HistoryContext::inner_initial_list, orthogonalPosition>::type
                    historized_state;

                store_history_impl(shallowHistoryMap_, history_key_type::make_history_key<historized_state>(), 0);
            }
            ...
            void clear_deep_history()
            {
                ...
                store_history_impl(deepHistoryMap_, history_key_type::make_history_key<historized_state>(), 0);
            }

            const event_base_type* triggering_event() const { return pTriggeringEvent_; }
        ...
        public:
            ...
            typedef typename state_base_type::node_state_base_ptr_type node_state_base_ptr_type;
            ...
            void post_event_impl(const event_base& evt) { post_event_impl(evt.intrusive_from_this()); }
            ...
            outermost_context_type& outermost_context() { return *polymorphic_downcast<MostDerived*>(this); }
            ...
            outermost_context_base_type& outermost_context_base() { return *this; }

            const outermost_context_base_type& outermost_context_base() const { return *this; }
            ...
            void add(const intrusive_ptr<State>& pState)
            {
                ...
                node_state_base_ptr_type pNewOutermostUnstableStateCandidate = add_impl(pState, *pState);

                if (isInnermostCommonOuter_ ||
                    (is_in_highest_orthogonal_region<State>() &&
                     (get_pointer(pOutermostUnstableState_) == pState->State::outer_state_ptr()))) {
                    isInnermostCommonOuter_ = false;
            ...
            void release_events() { eventQueue_.splice(eventQueue_.begin(), deferredEventQueue_); }
            ...
            {
                construct_with_history_impl<DefaultState>(shallowHistoryMap_, pContext);
            }

            template<class HistorizedState, class LeafState>
            void store_deep_history()
            {
                typedef typename detail::make_context_list<typename HistorizedState::context_type, LeafState>::type
                    history_context_list;
                typedef detail::constructor<history_context_list, outermost_context_base_type> constructor_type;
                // 5.2.10.6 declares that reinterpret_casting a function pointer to a
                // different function pointer and back must yield the same value. The
                // following reinterpret_cast is the first half of such a sequence.
                store_history_impl(
            ...
            {
                construct_with_history_impl<DefaultState>(deepHistoryMap_, pContext);
            }
        ...
        private: // implementation
            ...
            {
                InitialState::initial_deep_construct(*polymorphic_downcast<MostDerived*>(this));
            }
            ...
            private:
                //////////////////////////////////////////////////////////////////////
                // avoids C4512 (assignment operator could not be generated)
                initial_construct_function& operator=(const initial_construct_function&);
            ...
            public:
                //////////////////////////////////////////////////////////////////////
                terminate_function(state_machine& machine) :
                    machine_(machine)
                {
                }
            ...
            detail::reaction_result
                handle_exception_event(const ExceptionEvent& exceptionEvent, state_base_type* pCurrentState)
            {
                if (terminated()) {
                ...
                // function is left.
                state_base_type* const pOutermostUnstableState = get_pointer(pOutermostUnstableState_);
                state_base_type* const pHandlingState =
                    pOutermostUnstableState == 0 ? pCurrentState : pOutermostUnstableState;
                ...
                const detail::reaction_result reactionResult =
                    pHandlingState->react_impl(exceptionEvent, exceptionEvent.dynamic_type());
                ...
                if ((reactionResult != detail::do_discard_event) || (get_pointer(pOutermostUnstableState_) != 0)) {
                    throw;
                }
            ...
            public:
                //////////////////////////////////////////////////////////////////////
                exception_event_handler(state_machine& machine, state_base_type* pCurrentState = 0) :
                    machine_(machine),
                ...
                {
                    return detail::result_utility::make_result(
                        machine_.handle_exception_event(exceptionEvent, pCurrentState_));
                }

            private:
                //////////////////////////////////////////////////////////////////////
                // avoids C4512 (assignment operator could not be generated)
                exception_event_handler& operator=(const exception_event_handler&);
            ...
            public:
                //////////////////////////////////////////////////////////////////////
                terminator(state_machine& machine, const event_base* pNewTriggeringEvent) :
                    machine_(machine),
            ...
            detail::reaction_result send_event(const event_base_type& evt)
            {
                ...
                for (typename state_list_type::iterator pState = currentStates_.begin();
                     (reactionResult == detail::do_forward_event) && (pState != currentStatesEnd_);
                     ++pState) {
                    ...
                    reactionResult = detail::result_utility::get_result(translator_(
                        detail::send_function<state_base_type, event_base_type, rtti_policy_type::id_type>(
                            **pState,
                            evt,
                            eventType),
            ...
            void terminate_impl(state_base_type& theState, bool performFullExit)
            {
                ...
                if (get_pointer(pOutermostUnstableState_) != 0) {
                    theState.remove_from_state_list(currentStatesEnd_, pOutermostUnstableState_, performFullExit);
                }
                ...
                // increment it again, if the test failed.
                    ...
                    pState->exit_impl(pState, pOutermostUnstableState_, performFullExit);
                }
                else {
            ...
            node_state_base_ptr_type
                add_impl(const leaf_state_ptr_type& pState, detail::leaf_state<allocator_type, rtti_policy_type>&)
            {
                if (currentStatesEnd_ == currentStates_.end()) {
                    pState->set_list_position(currentStates_.insert(currentStatesEnd_, pState));
                }
                else {
            ...
            node_state_base_ptr_type add_impl(const node_state_base_ptr_type& pState, state_base_type&)
            {
                return pState;
            }
            ...
            static bool is_in_highest_orthogonal_region()
            {
                return mpl::equal_to<
                    ...
                    mpl::minus<
                        ...
                        mpl::integral_c<detail::orthogonal_position_type, 1>>>::value;
            }
            ...
            typedef std::map<
                ...
                typename boost::detail::allocator::
                    rebind_to<allocator_type, std::pair<const history_key_type, void (*)()>>::type>
                history_map_type;
            ...
            {
                typename history_map_type::iterator pFoundSlot =
                    historyMap.find(history_key_type::make_history_key<DefaultState>());

                if ((pFoundSlot == historyMap.end()) || (pFoundSlot->second == 0)) {
                    // We have never entered this state before or history was cleared
                    DefaultState::deep_construct(pContext, *polymorphic_downcast<MostDerived*>(this));
                }
                else {
                    ...
                    (*pConstructFunction)(pContext, *polymorphic_downcast<MostDerived*>(this));
                }
            }

            typedef std::list<
                event_base_ptr_type,
                typename boost::detail::allocator::rebind_to<allocator_type, event_base_ptr_type>::type>
                event_queue_type;
            ...
            typedef std::map<
                ...
                typename boost::detail::allocator::
                    rebind_to<allocator_type, std::pair<const state_base_type* const, event_queue_type>>::type>
                deferred_map_type;
