...
namespace boost {
    ...
    #pragma warning(disable: 4244 4459)
        ...
        inline std::intmax_t umax(std::integral_constant<bool, false> const&)
        {
            ...
            return std::numeric_limits<std::intmax_t>::is_specialized ? (std::numeric_limits<std::intmax_t>::max)()
                                                                      : INT_MAX;
        }
        ...
        inline std::intmax_t umax()
        {
            return umax(
                std::integral_constant<
                    bool,
                    std::numeric_limits<std::intmax_t>::digits >= std::numeric_limits<std::size_t>::digits>());
        }
        ...
        public:
            ...
            void fail(
                regex_constants::error_type error_code,
        ...
        basic_regex_parser<charT, traits>::basic_regex_parser(regex_data<charT, traits>* data) :
            basic_regex_creator<charT, traits>(data),
            m_parser_proc(),
            m_base(0),
            m_end(0),
            m_position(0),
        ...
        void basic_regex_parser<charT, traits>::parse(const charT* p1, const charT* p2, unsigned l_flags)
        {
            ...
            if ((p1 == p2) && (((l_flags & regbase::main_option_type) != regbase::perl_syntax_group) ||
            ...
            case regbase::basic_syntax_group: m_parser_proc = &basic_regex_parser<charT, traits>::parse_basic; break;
            case regbase::literal           : m_parser_proc = &basic_regex_parser<charT, traits>::parse_literal; break;
            default:
                // Oops, someone has managed to set more than one of the main option flags,
                // so this must be an error:
                fail(
                    regex_constants::error_unknown,
                    0,
            ...
            if (!result) {
                fail(
                    ...
                    std::distance(m_base, m_position),
        ...
        template<class charT, class traits>
        void basic_regex_parser<charT, traits>::fail(
            regex_constants::error_type error_code,
            std::ptrdiff_t position,
            std::string message,
            std::ptrdiff_t start_pos)
        {
            ...
            std::ptrdiff_t end_pos =
                (std::min)(position + static_cast<std::ptrdiff_t>(10), static_cast<std::ptrdiff_t>(m_end - m_base));
    ...
    #pragma warning(disable: 4702)
        ...
        bool basic_regex_parser<charT, traits>::parse_basic()
        {
            ...
            case regex_constants::syntax_escape: return parse_basic_escape();
            case regex_constants::syntax_dot   : return parse_match_any();
            ...
            case regex_constants::syntax_open_set: return parse_set();
    ...
    #pragma warning(push)
        ...
        bool basic_regex_parser<charT, traits>::parse_extended()
        {
            bool result = true;
            switch (this->m_traits.syntax_type(*m_position)) {
            case regex_constants::syntax_open_mark : return parse_open_paren();
            case regex_constants::syntax_close_mark: return false;
            case regex_constants::syntax_escape    : return parse_extended_escape();
            case regex_constants::syntax_dot       : return parse_match_any();
            case regex_constants::syntax_caret:
                ++m_position;
                this->append_state(
                    (this->flags() & regex_constants::no_mod_m ? syntax_element_buffer_start
            ...
            case regex_constants::syntax_plus:
                ...
                ++m_position;
                return parse_repeat(1);
            case regex_constants::syntax_open_brace: ++m_position; return parse_repeat_range(false);
            case regex_constants::syntax_close_brace:
                if ((this->flags() & regbase::no_perl_ex) == regbase::no_perl_ex) {
                    fail(
                        regex_constants::error_brace,
                        this->m_position - this->m_base,
            ...
            default: result = parse_literal(); break;
            }
            return result;
        }
#ifdef BOOST_REGEX_MSVC
    ...
    #pragma warning(pop)
        ...
        bool basic_regex_parser<charT, traits>::parse_literal()
        {
            ...
            if (((this->flags() & (regbase::main_option_type | regbase::mod_x | regbase::no_perl_ex)) !=
                 regbase::mod_x) ||
        ...
        bool basic_regex_parser<charT, traits>::parse_open_paren()
        {
            ...
            if (((this->flags() & (regbase::main_option_type | regbase::no_perl_ex)) == 0) ||
                ((this->flags() & (regbase::main_option_type | regbase::emacs_ex)) ==
                 (regbase::basic_syntax_group | regbase::emacs_ex))) {
            ...
            if (0 == (this->flags() & regbase::nosubs)) {
                ...
                if (this->flags() & regbase::save_subexpression_location) {
                    this->m_pdata->m_subs.push_back(
                        std::pair<std::size_t, std::size_t>(std::distance(m_base, m_position) - 1, 0));
                }
            }
            ...
            if (m_has_case_change) {
                ...
                static_cast<re_case*>(this->append_state(syntax_element_toggle_case, sizeof(re_case)))->icase =
                    opts & regbase::icase;
            }
            this->flags(opts);
        ...
        bool basic_regex_parser<charT, traits>::parse_basic_escape()
        {
            ...
            case regex_constants::syntax_or:
                ...
                break;
            case regex_constants::syntax_digit: return parse_backref();
            ...
            default:
                if (this->flags() & regbase::emacs_ex) {
                    bool negate = true;
                    switch (*m_position) {
                    case 'w': negate = false; BOOST_REGEX_FALLTHROUGH;
                    case 'W': {
                    ...
                    case 's': negate = false; BOOST_REGEX_FALLTHROUGH;
                    case 'S': return add_emacs_code(negate);
                    case 'c':
                    case 'C':
                        // not supported yet:
                        fail(
                            ...
                            m_position - m_base,
                            "The \\c and \\C escape sequences are not supported by POSIX basic regular expressions: "
                            "try the Perl syntax instead.");
                        return false;
                    default: break;
                    }
                }
        ...
        bool basic_regex_parser<charT, traits>::parse_extended_escape()
        {
            ...
            switch (this->m_traits.escape_syntax_type(*m_position)) {
            case regex_constants::escape_type_not_class: negate = true; BOOST_REGEX_FALLTHROUGH;
...
escape_type_class_jump:
            ...
            case regex_constants::escape_type_Z:
                ...
                break;
            case regex_constants::escape_type_Q: return parse_QE();
            case regex_constants::escape_type_C: return parse_match_any();
            ...
            case regex_constants::escape_type_G:
                ...
                break;
            case regex_constants::escape_type_not_property: negate = true; BOOST_REGEX_FALLTHROUGH;
            ...
            case regex_constants::escape_type_reset_start_mark:
                if (0 == (this->flags() & (regbase::main_option_type | regbase::no_perl_ex))) {
                    re_brace* pb =
                        static_cast<re_brace*>(this->append_state(syntax_element_startmark, sizeof(re_brace)));
                    pb->index = -5;
                    pb->icase = this->flags() & regbase::icase;
                    this->m_pdata->m_data.align();
                    ++m_position;
            ...
            case regex_constants::escape_type_extended_backref:
                if (0 == (this->flags() & (regbase::main_option_type | regbase::no_perl_ex))) {
                    ...
                    if ((syn == regex_constants::syntax_open_brace) ||
                        ...
                        have_brace = true;
                        switch (syn) {
                        case regex_constants::syntax_open_brace: syn_end = regex_constants::syntax_close_brace; break;
                        case regex_constants::escape_type_left_word:
                            syn_end = regex_constants::escape_type_right_word;
                            break;
                        default: syn_end = regex_constants::escape_type_end_buffer; break;
                        }
                    }
                    ...
                    if (((i < hash_value_mask) && (i > 0)) ||
                        ((i >= hash_value_mask) && (this->m_pdata->get_id((int)i) > 0))) {
                        m_position = pc;
                        re_brace* pb =
                            static_cast<re_brace*>(this->append_state(syntax_element_backref, sizeof(re_brace)));
                        pb->index = (int)i;
                        pb->icase = this->flags() & regbase::icase;
            ...
            case regex_constants::escape_type_control_v:
                ...
                BOOST_REGEX_FALLTHROUGH;
            default: this->append_literal(unescape_character()); break;
            }
            return true;
        }
        ...
        bool basic_regex_parser<charT, traits>::parse_match_any()
        {
            ...
            //
            ++m_position;
            static_cast<re_dot*>(this->append_state(syntax_element_wild, sizeof(re_dot)))->mask =
                static_cast<unsigned char>(
                    this->flags() & regbase::no_mod_s ? BOOST_REGEX_DETAIL_NS::force_not_newline
                    : this->flags() & regbase::mod_s  ? BOOST_REGEX_DETAIL_NS::force_newline
                                                      : BOOST_REGEX_DETAIL_NS::dont_care);
            return true;
        }
        ...
        bool basic_regex_parser<charT, traits>::parse_repeat(std::size_t low, std::size_t high)
        {
            ...
            //
            if ((m_position != m_end) && ((0 == (this->flags() & (regbase::main_option_type | regbase::no_perl_ex))) ||
                                          ((regbase::basic_syntax_group | regbase::emacs_ex) ==
                                           (this->flags() & (regbase::main_option_type | regbase::emacs_ex))))) {
                ...
                if ((m_position != m_end) &&
                    (this->m_traits.syntax_type(*m_position) == regex_constants::syntax_question)) {
                    greedy = false;
                    ++m_position;
                }
            ...
            if (this->m_last_state->type == syntax_element_endmark) {
                ...
                (this->m_last_state->type == syntax_element_literal) &&
                (static_cast<re_literal*>(this->m_last_state)->length > 1)) {
                ...
                lit = static_cast<re_literal*>(
                    this->append_state(syntax_element_literal, sizeof(re_literal) + sizeof(charT)));
                lit->length = 1;
            ...
            //
            re_repeat* rep =
                static_cast<re_repeat*>(this->insert_state(insert_point, syntax_element_rep, re_repeater_size));
            rep->min = low;
            rep->max = high;
            rep->greedy = greedy;
            rep->leading = false;
            ...
            if (possessive) {
                        ...
                        if (m_position != m_end) {
                            ...
                            case regex_constants::syntax_open_mark:
                                ...
                                if ((m_position + 2 < m_end) &&
                                    this->m_traits.syntax_type(*(m_position + 1)) == regex_constants::syntax_question &&
                                    this->m_traits.syntax_type(*(m_position + 2)) == regex_constants::syntax_hash) {
                                    while ((m_position != m_end) && (this->m_traits.syntax_type(*m_position++) !=
                ...
                re_brace* pb = static_cast<re_brace*>(
                    this->insert_state(insert_point, syntax_element_startmark, sizeof(re_brace)));
                pb->index = -3;
                pb->icase = this->flags() & regbase::icase;
                jmp = static_cast<re_jump*>(
                    this->insert_state(insert_point + sizeof(re_brace), syntax_element_jump, sizeof(re_jump)));
                this->m_pdata->m_data.align();
        ...
        bool basic_regex_parser<charT, traits>::parse_alt()
        {
            ...
            //
            re_alt* palt =
                static_cast<re_alt*>(this->insert_state(this->m_alt_insert_point, syntax_element_alt, re_alt_size));
        ...
        bool basic_regex_parser<charT, traits>::parse_set()
        {
            static const char incomplete_message[] =
                "Character set declaration starting with [ terminated prematurely - either no ] was found or the set "
                "had no content.";
            ++m_position;
            if (m_position == m_end) {
            ...
            while (m_position != m_end) {
                ...
                default: parse_set_literal(char_set); break;
                }
            }
        ...
        {
            static const char incomplete_message[] =
                "Character class declaration starting with [ terminated prematurely - either no ] was found or the set "
                "had no content.";
            //
    ...
    #pragma warning(disable: 4127)
            ...
            switch (this->m_traits.escape_syntax_type(*m_position)) {
            case regex_constants::escape_type_control_a  : result = charT('\a'); break;
            case regex_constants::escape_type_e          : result = charT(27); break;
            case regex_constants::escape_type_control_f  : result = charT('\f'); break;
            case regex_constants::escape_type_control_n  : result = charT('\n'); break;
            case regex_constants::escape_type_control_r  : result = charT('\r'); break;
            case regex_constants::escape_type_control_t  : result = charT('\t'); break;
            case regex_constants::escape_type_control_v  : result = charT('\v'); break;
            case regex_constants::escape_type_word_assert: result = charT('\b'); break;
            ...
            case regex_constants::escape_type_hex:
                ...
                if (this->m_traits.syntax_type(*m_position) == regex_constants::syntax_open_brace) {
                    ...
                    if ((m_position == m_end) || (i < 0) ||
                        ((std::numeric_limits<charT>::is_specialized) &&
                         (i > (std::intmax_t)(std::numeric_limits<charT>::max)())) ||
                        (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_close_brace)) {
                        // Rewind to start of escape:
                        --m_position;
                        while (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_escape) {
                    ...
                    std::ptrdiff_t len =
                        (std::min)(static_cast<std::ptrdiff_t>(2), static_cast<std::ptrdiff_t>(m_end - m_position));
            ...
            default: result = *m_position; break;
            }
            ++m_position;
            return result;
#ifdef BOOST_REGEX_MSVC
    #pragma warning(pop)
        ...
        bool basic_regex_parser<charT, traits>::parse_backref()
        {
            ...
            if ((i == 0) || (((this->flags() & regbase::main_option_type) == regbase::perl_syntax_group) &&
                             (this->flags() & regbase::no_bk_refs))) {
    ...
    #pragma warning(pop)
        ...
        bool basic_regex_parser<charT, traits>::parse_perl_extension()
        {
            ...
            //
            switch (this->m_traits.syntax_type(*m_position)) {
            case regex_constants::syntax_or: m_mark_reset = m_mark_count; BOOST_REGEX_FALLTHROUGH;
            case regex_constants::syntax_colon:
                //
            ...
            case regex_constants::syntax_digit: {
                ...
                if ((v < 0) || (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_close_mark)) {
                    ...
                    fail(
                        regex_constants::error_perl_extension,
                        m_position - m_base,
...
insert_recursion:
                pb->index = markid = 0;
                re_recurse* pr =
                    static_cast<re_recurse*>(this->append_state(syntax_element_recurse, sizeof(re_recurse)));
            ...
            case regex_constants::syntax_open_mark: {
                ...
                if (*m_position == charT('R')) {
                    ...
                    re_brace* br =
                        static_cast<re_brace*>(this->append_state(syntax_element_assert_backref, sizeof(re_brace)));
...
named_capture_jump:
                ...
                if (0 == (this->flags() & regbase::nosubs)) {
                    ...
                    if (this->flags() & regbase::save_subexpression_location) {
                        this->m_pdata->m_subs.push_back(
                            std::pair<std::size_t, std::size_t>(std::distance(m_base, m_position) - 2, 0));
                    }
                }
                pb->index = markid;
                const charT* base = ++m_position;
...
option_group_jump:
            ...
            if (markid == -4) {
                ...
                if (b->type != syntax_element_alt) {
                    re_alt* alt = static_cast<re_alt*>(
                        this->insert_state(expected_alt_point, syntax_element_alt, sizeof(re_alt)));
                    alt->alt.i = this->m_pdata->m_data.size() - this->getoffset(alt);
                }
                else if (
                    ((std::ptrdiff_t)this->m_pdata->m_data.size() >
                     (static_cast<re_alt*>(b)->alt.i + this->getoffset(b))) &&
                    (static_cast<re_alt*>(b)->alt.i > 0) &&
                    this->getaddress(static_cast<re_alt*>(b)->alt.i, b)->type == syntax_element_alt) {
                ...
                if ((b->type != syntax_element_assert_backref) && (b->type != syntax_element_startmark)) {
                    // Rewind to start of (? sequence:
                    --m_position;
                    while (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_open_mark) {
                        --m_position;
        ...
        bool basic_regex_parser<charT, traits>::parse_perl_verb()
        {
            ...
            case 'C':
                ...
                if (match_verb("OMMIT")) {
                    ...
                    ++m_position;
                    static_cast<re_commit*>(this->append_state(syntax_element_commit, sizeof(re_commit)))->action =
                        commit_commit;
    ...
    #pragma warning(pop)
        ...
        bool basic_regex_parser<charT, traits>::add_emacs_code(bool negate)
        {
            ...
            static const charT s_punct[5] = {
                'p', 'u', 'n', 'c', 't',
            };

            switch (*m_position) {
            case 's':
            case ' ': char_set.add_class(this->m_mask_space); break;
            case 'w': char_set.add_class(this->m_word_mask); break;
            case '_':
                ...
                char_set.add_single(digraph<charT>(charT('>')));
                break;
            case '.': char_set.add_class(this->m_traits.lookup_classname(s_punct, s_punct + 5)); break;
            case '(':
                char_set.add_single(digraph<charT>(charT('(')));
                char_set.add_single(digraph<charT>(charT('[')));
                char_set.add_single(digraph<charT>(charT('{')));
            ...
            case '\'':
                ...
                char_set.add_single(digraph<charT>(charT('#')));
                break;
            case '<': char_set.add_single(digraph<charT>(charT(';'))); break;
            case '>':
                char_set.add_single(digraph<charT>(charT('\n')));
                char_set.add_single(digraph<charT>(charT('\f')));
                break;
            default: fail(regex_constants::error_ctype, m_position - m_base); return false;
            }
            if (0 == this->append_set(char_set)) {
                fail(regex_constants::error_ctype, m_position - m_base);
                return false;
        ...
        regex_constants::syntax_option_type basic_regex_parser<charT, traits>::parse_options()
        {
            ...
            do {
                ...
                case 's':
                    ...
                    break;
                case 'm': f &= ~regex_constants::no_mod_m; break;
                case 'i': f |= regex_constants::icase; break;
                case 'x': f |= regex_constants::mod_x; break;
                default : breakout = true; continue;
                }
                if (++m_position == m_end) {
                    // Rewind to start of (? sequence:
                    --m_position;
            ...
            while ((!m_alt_jumps.empty()) && (m_alt_jumps.back() > last_paren_start)) {
                ...
                if (jmp->type != syntax_element_jump) {
                    ...
                    fail(
                        regex_constants::error_unknown,
                        this->m_position - this->m_base,
                        "Internal logic failed while compiling the expression, probably you added a repeat to "
