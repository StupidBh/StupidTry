...
namespace boost {
    namespace BOOST_REGEX_DETAIL_NS {
        ...
        unsigned find_sort_syntax(const traits* pt, charT* delim)
        {
            ...
            int pos = 0;
            while ((pos <= static_cast<int>(sa.size())) && (pos <= static_cast<int>(sA.size())) &&
                   (sa[pos] == sA[pos])) {
                ++pos;
            }
            --pos;
            ...
            if ((pos != 0) && (count_chars(sa, maybe_delim) == count_chars(sA, maybe_delim)) &&
                (count_chars(sa, maybe_delim) == count_chars(sc, maybe_delim))) {
                *delim = maybe_delim;
                return sort_delim;
            }
            //
