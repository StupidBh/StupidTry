...
namespace boost {
    ...
    namespace BOOST_REGEX_DETAIL_NS {
        ...
        std::basic_streambuf<charT, traits>* parser_buf<charT, traits>::setbuf(char_type* s, streamsize n)
        {
            this->setg(s, s, s + n);
            return this;
        }
        ...
        template<class charT, class traits>
        typename parser_buf<charT, traits>::pos_type parser_buf<charT, traits>::seekoff(
            off_type off,
    ...
    #pragma warning(pop)
        ...
        {
            cpp_regex_traits_base(const std::locale& l) { (void)imbue(l); }
            ...
            bool operator==(const cpp_regex_traits_base& b) const
            {
                return (m_pctype == b.m_pctype) && (m_pmessages == b.m_pmessages) && (m_pcollate == b.m_pcollate);
            }
        };
        ...
        public:
            ...
            regex_constants::escape_syntax_type escape_syntax_type(charT c) const
            {
                map_iterator_type i = m_char_map.find(c);
                if (i == m_char_map.end()) {
                    if (this->m_pctype->is(std::ctype_base::lower, c)) {
                        return regex_constants::escape_type_class;
                    }
                    if (this->m_pctype->is(std::ctype_base::upper, c)) {
        ...
        void cpp_regex_traits_char_layer<charT>::init()
        {
            ...
            if ((!cat_name.empty()) && (this->m_pmessages != 0)) {
                cat = this->m_pmessages->open(cat_name, this->m_locale);
                if ((int)cat < 0) {
        ...
        template<class charT>
        typename cpp_regex_traits_implementation<charT>::char_class_type const
            ...
            try {
                ...
                for (unsigned i = 0; i < result.size(); ++i) {
                    if (static_cast<uchar_type>(result[i]) == (std::numeric_limits<uchar_type>::max)()) {
                        ...
                        result2.append(1, static_cast<charT>(1 + static_cast<uchar_type>(result[i])))
                            .append(1, charT('b') - 1);
                    }
                }
        ...
        template<class charT>
        inline std::shared_ptr<const cpp_regex_traits_implementation<charT>>
    ...
    public:
        ...
        charT translate_nocase(charT c) const { return m_pimpl->m_pctype->tolower(c); }

        charT translate(charT c, bool icase) const { return icase ? m_pimpl->m_pctype->tolower(c) : c; }

        charT tolower(charT c) const { return m_pimpl->m_pctype->tolower(c); }

        charT toupper(charT c) const { return m_pimpl->m_pctype->toupper(c); }

        string_type transform(const charT* p1, const charT* p2) const { return m_pimpl->transform(p1, p2); }
        ...
        bool isctype(charT c, char_class_type f) const
        {
            ...
            static const ctype_mask mask_base = static_cast<ctype_mask>(
                std::ctype<charT>::alnum | std::ctype<charT>::alpha | std::ctype<charT>::cntrl |
                std::ctype<charT>::digit | std::ctype<charT>::graph | std::ctype<charT>::lower |
                std::ctype<charT>::print | std::ctype<charT>::punct | std::ctype<charT>::space |
                std::ctype<charT>::upper | std::ctype<charT>::xdigit);

            if ((f & mask_base) && (m_pimpl->m_pctype->is(static_cast<ctype_mask>(f & mask_base), c))) {
                return true;
            }
            else if (
                (f & BOOST_REGEX_DETAIL_NS::cpp_regex_traits_implementation<charT>::mask_unicode) &&
                ...
                (f & BOOST_REGEX_DETAIL_NS::cpp_regex_traits_implementation<charT>::mask_blank) &&
                m_pimpl->m_pctype->is(std::ctype<charT>::space, c) && !BOOST_REGEX_DETAIL_NS::is_separator(c)) {
                return true;
            }
            else if (
                (f & BOOST_REGEX_DETAIL_NS::cpp_regex_traits_implementation<charT>::mask_vertical) &&
                (::boost::BOOST_REGEX_DETAIL_NS::is_separator(c) || (c == '\v'))) {
                return true;
            }
            else if (
                (f & BOOST_REGEX_DETAIL_NS::cpp_regex_traits_implementation<charT>::mask_horizontal) &&
                this->isctype(c, std::ctype<charT>::space) &&
                !this->isctype(c, BOOST_REGEX_DETAIL_NS::cpp_regex_traits_implementation<charT>::mask_vertical)) {
                return true;
            }
#ifdef __CYGWIN__
            //
    ...
    std::intmax_t cpp_regex_traits<charT>::toi(const charT*& first, const charT* last, int radix) const
    {
        ...
        sbuf.pubsetbuf(
            const_cast<charT*>(static_cast<const charT*>(first)),
            static_cast<std::streamsize>(last - first));
        is.clear();
        if (std::abs(radix) == 16) {
            is >> std::hex;
        }
        else if (std::abs(radix) == 8) {
            is >> std::oct;
        }
        else {
            is >> std::dec;
