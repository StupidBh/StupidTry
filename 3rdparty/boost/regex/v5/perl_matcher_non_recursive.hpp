...
namespace boost {
    namespace BOOST_REGEX_DETAIL_NS {
        ...
        struct saved_matched_paren : public saved_state
        {
            ...
            saved_matched_paren(int i, const sub_match<BidiIterator>& s) :
                saved_state(1),
                index(i),
                sub(s)
            {
        ...
        struct saved_assertion : public saved_position<BidiIterator>
        {
            ...
            saved_assertion(bool p, const re_syntax_base* ps, BidiIterator pos) :
                saved_position<BidiIterator>(ps, pos, saved_type_assertion),
                positive(p)
            {
            }
        };
        ...
        struct saved_repeater : public saved_state
        {
            ...
            saved_repeater(int i, repeater_count<BidiIterator>** s, BidiIterator start, int current_recursion_id) :
                saved_state(saved_state_repeater_count),
                count(i, s, start, current_recursion_id)
            {
            }
        };
        ...
        struct saved_extra_block : public saved_state
        {
            ...
            saved_extra_block(saved_state* b, saved_state* e) :
                saved_state(saved_state_extra_block),
                base(b),
                end(e)
            {
            }
        ...
        bool perl_matcher<BidiIterator, Allocator, traits>::match_all_states()
        {
            static matcher_proc_type const s_match_vtable[34] = {
                ...
                //(::boost::is_random_access_iterator<BidiIterator>::value ? &perl_matcher<BidiIterator, Allocator,
                //traits>::match_dot_repeat_fast : &perl_matcher<BidiIterator, Allocator,
        ...
        void perl_matcher<BidiIterator, Allocator, traits>::extend_stack()
        {
            if (used_block_count) {
                ...
                backup_state =
                    reinterpret_cast<saved_state*>(reinterpret_cast<char*>(stack_base) + BOOST_REGEX_BLOCKSIZE);
        ...
        template<class BidiIterator, class Allocator, class traits>
        inline void perl_matcher<BidiIterator, Allocator, traits>::push_matched_paren(
            int index,
            const sub_match<BidiIterator>& sub)
        {
        ...
        template<class BidiIterator, class Allocator, class traits>
        inline void
            perl_matcher<BidiIterator, Allocator, traits>::push_assertion(const re_syntax_base* ps, bool positive)
        {
        ...
        template<class BidiIterator, class Allocator, class traits>
        inline void
            perl_matcher<BidiIterator, Allocator, traits>::push_repeater_count(int i, repeater_count<BidiIterator>** s)
        {
            ...
            (void)new (pmp) saved_repeater<BidiIterator>(
                i,
                s,
                position,
                this->recursion_stack.empty() ? (INT_MIN + 3) : this->recursion_stack.back().idx);
            m_backup_state = pmp;
        }

        template<class BidiIterator, class Allocator, class traits>
        inline void perl_matcher<BidiIterator, Allocator, traits>::push_single_repeat(
            std::size_t c,
            const re_repeat* r,
            BidiIterator last_position,
            int state_id)
        ...
        template<class BidiIterator, class Allocator, class traits>
        inline void perl_matcher<BidiIterator, Allocator, traits>::push_recursion(
            int idx,
            const re_syntax_base* p,
            results_type* presults,
        ...
        bool perl_matcher<BidiIterator, Allocator, traits>::match_startmark()
        {
            ...
            switch (index) {
            case 0 : pstate = pstate->next.p; break;
            case -1:
            case -2: {
    ...
    #pragma warning(pop)
        ...
        bool perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast()
        {
            ...
            std::size_t count = static_cast<std::size_t>(
                (std::min)(static_cast<std::size_t>(std::distance(position, last)), greedy ? rep->max : rep->min));
            if (rep->min > count) {
                position = last;
    ...
    #pragma option push -w-8008 -w-8066 -w-8004
            ...
            if (::boost::is_random_access_iterator<BidiIterator>::value) {
                ...
                std::size_t len =
                    (desired == (std::numeric_limits<std::size_t>::max)()) ? 0u : std::distance(position, last);
                if (desired >= len) {
                    end = last;
                }
                else {
    ...
    #pragma option push -w-8008 -w-8066 -w-8004
            ...
            if (::boost::is_random_access_iterator<BidiIterator>::value) {
                ...
                while ((count < desired) && (position != last) &&
                       map[static_cast<unsigned char>(traits_inst.translate(*position, icase))]) {
                    ++position;
                    ++count;
                }
            }
    ...
    #pragma option push -w-8008 -w-8066 -w-8004
            ...
            if (::boost::is_random_access_iterator<BidiIterator>::value) {
                ...
                while ((count < desired) && (position != last) &&
                       (position != re_is_set_member(position, last, set, re.get_data(), icase))) {
                    ++position;
                    ++count;
                }
            }
    ...
    #pragma warning(pop)
        ...
        bool perl_matcher<BidiIterator, Allocator, traits>::match_recursion()
        {
            ...
            //
            for (typename std::vector<recursion_info<results_type>>::reverse_iterator i = recursion_stack.rbegin();
                 i != recursion_stack.rend();
                 ++i) {
                ...
                if (!recursion_stack.empty()) {
                    if (index == recursion_stack.back().idx) {
                        ...
                        *m_presult = recursion_stack.back().results;
                        push_recursion(
                            recursion_stack.back().idx,
                            recursion_stack.back().preturn_address,
                            m_presult,
                            &recursion_stack.back().results);
        ...
        bool perl_matcher<BidiIterator, Allocator, traits>::match_commit()
        {
            ...
            case commit_commit: restart = last; break;
            case commit_skip:
                if (base != position) {
                    restart = position;
