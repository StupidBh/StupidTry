...
namespace boost {
    ...
    public:
        ...
        typedef typename std::iterator_traits<BidiIterator>::value_type char_type;
        ...
        // size:
        size_type size() const { return empty() ? 0 : m_subs.size() - 2; }

        size_type max_size() const { return m_subs.max_size(); }

        bool empty() const { return m_subs.size() < 2; }
    ...
    public:
        ...
        string_type str(const char_type* sub) const { return (*this)[sub].str(); }
    ...
    public:
        ...
        const_iterator begin() const { return (m_subs.size() > 2) ? (m_subs.begin() + 2) : m_subs.end(); }
        ...
        OutputIterator format(OutputIterator out, Functor fmt, match_flag_type flags = format_default) const
        {
            ...
            typedef typename BOOST_REGEX_DETAIL_NS::
                compute_functor_type<Functor, match_results<BidiIterator, Allocator>, OutputIterator>::type F;
            F func(fmt);
        ...
        string_type format(Functor fmt, match_flag_type flags = format_default) const
        {
            ...
            typedef typename BOOST_REGEX_DETAIL_NS::compute_functor_type<
                Functor,
                match_results<BidiIterator, Allocator>,
                BOOST_REGEX_DETAIL_NS::string_out_iterator<std::basic_string<char_type>>>::type F;
            F func(fmt);
        ...
        OutputIterator format(OutputIterator out, Functor fmt, match_flag_type flags, const RegexT& re) const
        {
            ...
            typedef typename BOOST_REGEX_DETAIL_NS::
                compute_functor_type<Functor, match_results<BidiIterator, Allocator>, OutputIterator, traits_type>::type
                    F;
            F func(fmt);
        ...
        string_type format(Functor fmt, match_flag_type flags, const RegexT& re) const
        {
            ...
            typedef typename BOOST_REGEX_DETAIL_NS::compute_functor_type<
                Functor,
                match_results<BidiIterator, Allocator>,
                BOOST_REGEX_DETAIL_NS::string_out_iterator<std::basic_string<char_type>>,
                traits_type>::type F;
            F func(fmt);
        ...
        allocator_type get_allocator() const { return m_subs.get_allocator(); }
        ...
        bool operator==(const match_results& that) const
        {
            ...
            return (m_subs == that.m_subs) && (m_base == that.m_base) &&
                   (m_last_closed_paren == that.m_last_closed_paren);
        }

        bool operator!=(const match_results& that) const { return !(*this == that); }
        ...
        void set_base(BidiIterator pos) { m_base = pos; }

        BidiIterator base() const { return m_base; }
        ...
        void set_named_subs(std::shared_ptr<named_sub_type> subs) { m_named_subs = std::move(subs); }
