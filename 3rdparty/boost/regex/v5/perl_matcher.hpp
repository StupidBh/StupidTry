    ...
    namespace BOOST_REGEX_DETAIL_NS {
        ...
        inline void verify_options(boost::regex_constants::syntax_option_type, match_flag_type mf)
        {
            ...
            if ((mf & match_extra) && is_posix) {
                BOOST_REGEX_DETAIL_THROW(
                    std::logic_error("Usage Error: Can't mix regular expression captures with POSIX matching rules"));
            }
        }
        ...
        {
            return (
                (c < static_cast<charT>(0)) ? true : ((c >= static_cast<charT>(1 << CHAR_BIT)) ? true : map[c] & mask));
        }
...
#define STR_COMP(s, p) string_compare(s, p)
    ...
    #pragma warning(pop)
        ...
        public:
            ...
            void setf(match_flag_type f) { m_match_flags |= f; }

            void unsetf(match_flag_type f) { m_match_flags &= ~f; }
        ...
        private:
            ...
            {
                return ::boost::is_random_access_iterator<BidiIterator>::value ? match_dot_repeat_fast()
        ...
        private:
            ...
            bool m_unwound_alt;
            // We are unwinding a commit - used by independent subs to determine whether to stop there or carry on
            // unwinding:
    ...
    #pragma warning(push)
        ...
        #pragma warning(disable: 26495)
            ...
            perl_matcher& operator=(const perl_matcher&) { return *this; }

            perl_matcher(const perl_matcher& that) :
                m_result(that.m_result),
                re(that.re),
                traits_inst(that.traits_inst),
                rep_obj(0)
            {
            }
#ifdef BOOST_REGEX_MSVC
    #pragma warning(pop)
        ...
        bool
            factory_match(perl_matcher<const char*, match_results<const char*>::allocator_type, regex::traits_type>& m);
        bool factory_match(
            perl_matcher<const wchar_t*, match_results<const wchar_t*>::allocator_type, wregex::traits_type>& m);
        bool factory_match(
            perl_matcher<
                std::string::const_iterator,
                match_results<std::string::const_iterator>::allocator_type,
                regex::traits_type>& m);
        bool factory_match(
            perl_matcher<
                std::wstring::const_iterator,
                match_results<std::wstring::const_iterator>::allocator_type,
                wregex::traits_type>& m);
        bool factory_match(
            perl_matcher<
                std::string::iterator,
                match_results<std::string::iterator>::allocator_type,
                regex::traits_type>& m);
        bool factory_match(
            perl_matcher<
                std::wstring::iterator,
                match_results<std::wstring::iterator>::allocator_type,
                wregex::traits_type>& m);
