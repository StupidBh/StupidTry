...
namespace boost {
        ...
        match_flag_type flags = match_default)
    {
        ...
        BOOST_REGEX_DETAIL_NS::perl_matcher<BidiIterator, match_alloc_type, traits>
            matcher(first, last, m, e, flags | regex_constants::match_any, first);
        return BOOST_REGEX_DETAIL_NS::factory_find(matcher);
    }

    BOOST_REGEX_MODULE_EXPORT template<class charT, class traits>
    inline bool
        regex_search(const charT* str, const basic_regex<charT, traits>& e, match_flag_type flags = match_default)
    {
