...
namespace boost {
    ...
    namespace BOOST_REGEX_DETAIL_NS {
        ...
        public:
            ...
            // accessor functions:
            //
            bool has_digraphs() const { return m_has_digraphs; }

            bool is_negated() const { return m_negate; }
            ...
            set_iterator singles_begin() const { return m_singles.begin(); }

            set_iterator singles_end() const { return m_singles.end(); }

            list_iterator ranges_begin() const { return m_ranges.begin(); }

            list_iterator ranges_end() const { return m_ranges.end(); }

            set_iterator equivalents_begin() const { return m_equivalents.begin(); }

            set_iterator equivalents_end() const { return m_equivalents.end(); }

            m_type classes() const { return m_classes; }

            m_type negated_classes() const { return m_negated_classes; }
        ...
        public:
            ...
            std::ptrdiff_t getoffset(void* addr) { return getoffset(addr, m_pdata->m_data.data()); }
            ...
            re_syntax_base* getaddress(std::ptrdiff_t off) { return getaddress(off, m_pdata->m_data.data()); }
            ...
            regbase::flag_type flags() { return m_pdata->m_flags; }
            ...
            re_syntax_base* insert_state(std::ptrdiff_t pos, syntax_element_type t)
            {
                return insert_state(pos, t, sizeof(re_syntax_base));
            }

            re_literal* append_literal(charT c);
            re_syntax_base* append_set(const basic_char_set<charT, traits>& char_set);
            re_syntax_base*
                append_set(const basic_char_set<charT, traits>& char_set, std::integral_constant<bool, false>*);
            re_syntax_base*
                append_set(const basic_char_set<charT, traits>& char_set, std::integral_constant<bool, true>*);
        ...
        protected:
            ...
            const ::boost::regex_traits_wrapper<traits>& m_traits; // convenience reference to traits class
            ...
            std::vector<unsigned char>
                m_recursion_checks; // notes which recursions we've followed while analysing this expression
            typename traits::char_class_type m_word_mask;  // mask used to determine if a character is a word character
            typename traits::char_class_type m_mask_space; // mask used to determine if a character is a word character
            typename traits::char_class_type
                m_lower_mask; // mask used to determine if a character is a lowercase character
            typename traits::char_class_type
                m_upper_mask; // mask used to determine if a character is an uppercase character
            typename traits::char_class_type
                m_alpha_mask; // mask used to determine if a character is an alphabetic character
        private:
        ...
        basic_regex_creator<charT, traits>::basic_regex_creator(regex_data<charT, traits>* data) :
            m_pdata(data),
            m_traits(*(data->m_ptraits)),
            m_last_state(0),
            m_icase(false),
            m_repeater_id(0),
            m_has_backrefs(false),
            m_bad_repeats(0),
            m_has_recursions(false),
            m_word_mask(0),
            m_mask_space(0),
            m_lower_mask(0),
            m_upper_mask(0),
            m_alpha_mask(0)
        {
            ...
            static const charT l[5] = {
                'l', 'o', 'w', 'e', 'r',
            };
            static const charT u[5] = {
                'u', 'p', 'p', 'e', 'r',
            };
            static const charT a[5] = {
                'a', 'l', 'p', 'h', 'a',
            };
        ...
        template<class charT, class traits>
        re_syntax_base*
            basic_regex_creator<charT, traits>::insert_state(std::ptrdiff_t pos, syntax_element_type t, std::size_t s)
        {
            // append a new state, start by aligning our last one:
            m_pdata->m_data.align();
            // set the offset to the next state in our last one:
        ...
        re_literal* basic_regex_creator<charT, traits>::append_literal(charT c)
        {
            ...
            if ((0 == m_last_state) || (m_last_state->type != syntax_element_literal)) {
                ...
                result =
                    static_cast<re_literal*>(append_state(syntax_element_literal, sizeof(re_literal) + sizeof(charT)));
                result->length = 1;
        ...
        template<class charT, class traits>
        inline re_syntax_base*
            basic_regex_creator<charT, traits>::append_set(const basic_char_set<charT, traits>& char_set)
        {
            typedef std::integral_constant<bool, (sizeof(charT) == 1)> truth_type;
            return char_set.has_digraphs() ? append_set(char_set, static_cast<std::integral_constant<bool, false>*>(0))
            ...
            std::integral_constant<bool, false>*)
        {
            ...
            re_set_long<m_type>* result =
                static_cast<re_set_long<m_type>*>(append_state(syntax_element_long_set, sizeof(re_set_long<m_type>)));
            //
            // fill in the basics:
            //
            result->csingles =
                static_cast<unsigned int>(std::distance(char_set.singles_begin(), char_set.singles_end()));
            result->cranges =
                static_cast<unsigned int>(std::distance(char_set.ranges_begin(), char_set.ranges_end())) / 2;
            result->cequivalents =
                static_cast<unsigned int>(std::distance(char_set.equivalents_begin(), char_set.equivalents_end()));
            result->cclasses = char_set.classes();
            result->cnclasses = char_set.negated_classes();
            if (flags() & regbase::icase) {
                // adjust classes as needed:
                if (((result->cclasses & m_lower_mask) == m_lower_mask) ||
                    ((result->cclasses & m_upper_mask) == m_upper_mask)) {
                    result->cclasses |= m_alpha_mask;
                }
                if (((result->cnclasses & m_lower_mask) == m_lower_mask) ||
                    ((result->cnclasses & m_upper_mask) == m_upper_mask)) {
            ...
            while (sfirst != slast) {
                charT* p = static_cast<charT*>(this->m_pdata->m_data.extend(
                    sizeof(charT) * (sfirst->first == static_cast<charT>(0) ? 1
                                     : sfirst->second                       ? 3
                                                                            : 2)));
            ...
            while (first != last) {
                ...
                charT* p =
                    static_cast<charT*>(this->m_pdata->m_data.extend(sizeof(charT) * (s1.size() + s2.size() + 2)));
            ...
            std::integral_constant<bool, true>*)
        {
            ...
            while (first != last) {
                ...
                if (flags() & regex_constants::collate) {
                    ...
                    std::memset(
                        result->_map + static_cast<unsigned char>(c1),
                        true,
                        ...
                        if (idx >= hash_value_mask) {
                            ...
                            if (idx <= 0) {
                                ...
                                if (0 == (this->flags() & regex_constants::no_except)) {
                                    std::string message =
                                        "Encountered a forward reference to a marked sub-expression that does not "
                                        "exist.";
        ...
        void basic_regex_creator<charT, traits>::create_startmaps(re_syntax_base* state)
        {
            ...
            while (!v.empty()) {
                ...
                create_startmap(
                    state->next.p,
                    static_cast<re_alt*>(state)->_map,
                    &static_cast<re_alt*>(state)->can_be_null,
                    mask_take);
                m_bad_repeats = 0;
                ...
                create_startmap(
                    static_cast<re_alt*>(state)->alt.p,
                    static_cast<re_alt*>(state)->_map,
                    &static_cast<re_alt*>(state)->can_be_null,
                    mask_skip);
        ...
        int basic_regex_creator<charT, traits>::calculate_backstep(re_syntax_base* state)
        {
            ...
            while (state) {
                ...
                case syntax_element_startmark:
                    if ((static_cast<re_brace*>(state)->index == -1) || (static_cast<re_brace*>(state)->index == -2)) {
                ...
                case syntax_element_endmark:
                    ...
                    break;
                case syntax_element_literal      : result += static_cast<re_literal*>(state)->length; break;
                case syntax_element_wild         :
                case syntax_element_set          : result += 1; break;
                case syntax_element_dot_rep      :
                case syntax_element_char_rep     :
                case syntax_element_short_set_rep:
                ...
                case syntax_element_backstep     : {
                    ...
                    // adjust the type of the state to allow for faster matching:
                    state->type = this->get_repeat_type(state);
                    if ((state->type == syntax_element_dot_rep) || (state->type == syntax_element_char_rep) ||
                        (state->type == syntax_element_short_set_rep)) {
                        if (rep->max != rep->min) {
                            return -1;
                        }
                        if (static_cast<std::size_t>((std::numeric_limits<int>::max)() - result) < rep->min) {
                            return -1; // protection against overflow, we can't calculate a backstep in this case and
                                       // the expression is probably ill-formed.
                        }
                ...
                case syntax_element_long_set:
                    ...
                    result += 1;
                    break;
                case syntax_element_jump: state = static_cast<re_jump*>(state)->alt.p; continue;
                case syntax_element_alt : {
        ...
        struct recursion_saver
        {
            ...
            recursion_saver(std::vector<unsigned char>* p) :
                saved_state(*p),
                state(p)
            {
            }

            ~recursion_saver() { state->swap(saved_state); }
        };

        template<class charT, class traits>
        void basic_regex_creator<charT, traits>::create_startmap(
            re_syntax_base* state,
            unsigned char* l_map,
            unsigned int* pnull,
            unsigned char mask,
            ...
            while (state) {
                ...
                case syntax_element_literal: {
                    ...
                    if (l_map) {
                        l_map[0] |= mask_init;
                        charT first_char =
                            *static_cast<charT*>(static_cast<void*>(static_cast<re_literal*>(state) + 1));
                    ...
                    if (l_map) {
                        ...
                        if (static_cast<re_set_long<m_type>*>(state)->singleton) {
                            ...
                            for (unsigned int i = 0; i < (1u << CHAR_BIT); ++i) {
                                ...
                                if (&c != re_is_set_member(
                                              &c,
                                              &c + 1,
                                              static_cast<re_set_long<m_type>*>(state),
                                              *m_pdata,
                                              l_icase)) {
                                    l_map[i] |= mask;
                                }
                ...
                case syntax_element_set:
                    if (l_map) {
                        l_map[0] |= mask_init;
                        for (unsigned int i = 0; i < (1u << CHAR_BIT); ++i) {
                            if (static_cast<re_set*>(state)->_map[static_cast<unsigned char>(
                                    m_traits.translate(static_cast<charT>(i), l_icase))]) {
                                l_map[i] |= mask;
                            }
                        }
                    }
                ...
                case syntax_element_long_set_rep : {
                    ...
                    if (rep->_map[0] & mask_init) {
                        ...
                        create_startmap(state->next.p, l_map, pnull, mask, ++recursion_count);
                        if ((state->type == syntax_element_alt) || (static_cast<re_repeat*>(state)->min == 0) ||
                            (not_last_jump == 0)) {
                ...
                case syntax_element_endmark:
                    ...
                    if (static_cast<re_brace*>(state)->index < 0) {
                        ...
                        recursion_start && (recursion_sub != 0) &&
                    ...
                    if (m_pdata->m_has_recursions && static_cast<re_brace*>(state)->index) {
                        ...
                        while (p) {
                            if (p->type == syntax_element_recurse) {
                                ...
                                if ((p2->type == syntax_element_startmark) &&
                                    (p2->index == static_cast<re_brace*>(state)->index)) {
                                    ok = true;
                                    break;
                                }
                            }
        ...
        unsigned basic_regex_creator<charT, traits>::get_restart_type(re_syntax_base* state)
        {
            ...
            while (state) {
                switch (state->type) {
                case syntax_element_startmark:
                case syntax_element_endmark         : state = state->next.p; continue;
                case syntax_element_start_line      : return regbase::restart_line;
                case syntax_element_word_start      : return regbase::restart_word;
                case syntax_element_buffer_start    : return regbase::restart_buf;
                case syntax_element_restart_continue: return regbase::restart_continue;
    ...
    #pragma warning(pop)
                ...
                case syntax_element_buffer_end:
                case syntax_element_restart_continue: state = state->next.p; break;
                case syntax_element_dot_rep:
                case syntax_element_char_rep:
                case syntax_element_short_set_rep:
                case syntax_element_long_set_rep:
