...
namespace boost {
    ...
    BOOST_REGEX_MODULE_EXPORT template<
        class BidiIterator,
        class Allocator = typename std::vector<sub_match<BidiIterator>>::allocator_type>
    class match_results;
    ...
    namespace BOOST_REGEX_DETAIL_NS {
        ...
        struct trivial_format_traits
        {
            ...
            static std::ptrdiff_t length(const charT* p) { return global_length(p); }

            static charT tolower(charT c) { return ::boost::BOOST_REGEX_DETAIL_NS::global_lower(c); }

            static charT toupper(charT c) { return ::boost::BOOST_REGEX_DETAIL_NS::global_upper(c); }
    ...
    #pragma warning(disable: 26812)
        ...
        public:
            ...
            OutputIterator format(ForwardIter p1, match_flag_type f) { return format(p1, p1 + m_traits.length(p1), f); }
        ...
        private:
            ...
            inline typename Results::value_type const&
                get_named_sub(ForwardIter i, ForwardIter j, const std::integral_constant<bool, false>&)
            {
                std::vector<char_type> v(i, j);
                return (i != j) ? this->m_results.named_subexpression(&v[0], &v[0] + v.size())
                                : this->m_results.named_subexpression(
                                      static_cast<const char_type*>(0),
                                      static_cast<const char_type*>(0));
            }

            inline typename Results::value_type const&
                get_named_sub(ForwardIter i, ForwardIter j, const std::integral_constant<bool, true>&)
            {
            ...
            inline int get_named_sub_index(ForwardIter i, ForwardIter j, const std::integral_constant<bool, false>&)
            {
                ...
                return (i != j) ? this->m_results.named_subexpression_index(&v[0], &v[0] + v.size())
                                : this->m_results.named_subexpression_index(
                                      static_cast<const char_type*>(0),
                                      static_cast<const char_type*>(0));
            }
    ...
    #pragma warning(pop)
        ...
        template<class OutputIterator, class Results, class traits, class ForwardIter>
        OutputIterator basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format(
            ForwardIter p1,
            ForwardIter p2,
            match_flag_type f)
        {
        ...
        void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_all(unsigned recursion_count)
        {
            ...
            while (m_position != m_end) {
                ...
                case '&':
                    ...
                    put(*m_position++);
                    break;
                case '\\': format_escape(); break;
                case '(':
                    if ((m_flags & boost::regex_constants::format_all) &&
                        (recursion_count < BOOST_REGEX_MAX_RECURSION_DEPTH)) {
                        ++m_position;
        ...
        bool basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::handle_perl_verb(bool have_brace)
        {
            ...
            static const char_type LAST_PAREN_MATCH[] = { 'L', 'A', 'S', 'T', '_', 'P', 'A', 'R',
                                                          'E', 'N', '_', 'M', 'A', 'T', 'C', 'H' };
            static const char_type LAST_SUBMATCH_RESULT[] = { 'L', 'A', 'S', 'T', '_', 'S', 'U', 'B', 'M', 'A',
                                                              'T', 'C', 'H', '_', 'R', 'E', 'S', 'U', 'L', 'T' };
        ...
        void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::put(char_type c)
        {
            ...
            case output_next_upper:
                ...
                this->m_state = m_restore_state;
                break;
            case output_lower: c = m_traits.tolower(c); break;
            case output_upper: c = m_traits.toupper(c); break;
            default          : break;
            }
            *m_out = c;
            ++m_out;
            ...
            const traits& t)
        {
            ...
            BOOST_REGEX_DETAIL_NS::
                basic_regex_formatter<OutputIterator, match_results<Iterator, Alloc>, traits, ForwardIter>
                    f(out, m, t);
        ...
        template<class T>
        T& do_unwrap_ref(std::reference_wrapper<T> const& r)
        {
            return r.get();
        }
        ...
        private:
            ...
            //
            static_assert(
                (::std::is_pointer<F>::value || ::std::is_function<F>::value || ::std::is_class<F>::value),
                "The functor must be a pointer or a class with a function call operator");
        ...
        public:
            ...
            typedef typename std::conditional<
                ...
                typename std::conditional<
                    ...
                    std::integral_constant<int, format_traits_imp<F, M, O>::value>>::type>::type type;
            //
            // This static assertion will fail if the functor passed does not accept
            // the same type of arguments passed.
            //
            static_assert(
                std::is_class<F>::value && !has_const_iterator<F>::value ? (type::value > 1) : true,
                "Argument mismatch in Functor type");
        };

        template<class Base, class Match>
        struct format_functor3
        {
            format_functor3(Base b) :
                func(b)
            {
            }
            ...
            OutputIter
                operator()(const Match& m, OutputIter i, boost::regex_constants::match_flag_type f, const Traits&)
            {
                return (*this)(m, i, f);
            }
        ...
        {
            format_functor_c_string(const charT* ps) :
                func(ps)
            {
            }
        ...
        template<
            ...
            class Traits = BOOST_REGEX_DETAIL_NS::trivial_format_traits<typename Match::char_type>>
        struct compute_functor_type
        {
            ...
            typedef typename std::conditional<
                tag::value == 0,
                format_functor_c_string<maybe_char_type, Match, Traits>,
                typename std::conditional<
                    tag::value == 1,
                    format_functor_container<Func, Match, Traits>,
                    typename std::conditional<
                        tag::value == 2,
                        format_functor1<Func, Match>,
                        typename std::
                            conditional<tag::value == 3, format_functor2<Func, Match>, format_functor3<Func, Match>>::
    ...
    inline OutputIterator regex_format(
        OutputIterator out,
    ...
    inline std::basic_string<typename match_results<Iterator, Allocator>::char_type>
