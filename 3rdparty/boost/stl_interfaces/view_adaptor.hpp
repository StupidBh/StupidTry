...
#include <type_traits>
    ...
    #if !BOOST_STL_INTERFACES_USE_CPP23_STD_RANGE_ADAPTOR_CLOSURE && BOOST_STL_INTERFACES_USE_CONCEPTS && \
        defined(BOOST_GCC) && 14 <= __GNUC__
        #define BOOST_STL_INTERFACES_USE_LIBSTDCPP_GCC14_RANGE_ADAPTOR_CLOSURE 1
    #elif !BOOST_STL_INTERFACES_USE_CPP23_STD_RANGE_ADAPTOR_CLOSURE && BOOST_STL_INTERFACES_USE_CONCEPTS && \
        defined(BOOST_GCC) && 12 <= __GNUC__
    ...
    #if !BOOST_STL_INTERFACES_USE_CPP23_STD_RANGE_ADAPTOR_CLOSURE && defined(_MSC_VER) && _MSC_VER <= 1929
...
namespace boost {
    namespace stl_interfaces {
        namespace detail {
            ...
            using invocable_expr = decltype(std::declval<F>()(std::declval<Args>()...));
            template<typename F, typename... Args>
            constexpr bool is_invocable_v = is_detected_v<invocable_expr, F, Args...>;
            ...
            private:
                ...
                static constexpr decltype(auto) call_impl(T&& this_, std::index_sequence<I...>, Args&&... args)
                {
                    return ((T&&)this_).f_((Args&&)args..., std::get<I>(((T&&)this_).bound_args_)...);
                }
        ...
        {
            return detail::bind_back_result<Func, Args...>(0, (Func&&)f, (Args&&)args...);
        }

#if BOOST_STL_INTERFACES_DEFINE_CUSTOM_RANGE_ADAPTOR_CLOSURE || defined(BOOST_STL_INTERFACES_DOXYGEN)
        ...
        template<
            typename D,
            typename Enable = std::enable_if_t<std::is_class<D>::value && std::is_same<D, std::remove_cv_t<D>>::value>>
    #endif
    ...
    #if BOOST_STL_INTERFACES_USE_CONCEPTS
            template<typename T>
            concept range_adaptor_closure_ =
                std::derived_from<std::remove_cvref_t<T>, range_adaptor_closure<std::remove_cvref_t<T>>>;
    #else
            template<typename T>
            using range_adaptor_closure_tag_expr =
                typename range_adaptor_closure<T>::inheritance_tag_with_an_unlikely_name_;
            template<typename T>
            constexpr bool range_adaptor_closure_ = is_detected_v<range_adaptor_closure_tag_expr, remove_cvref_t<T>>;
    #endif
        }
    ...
    #else
            template<typename T, typename Enable = std::enable_if_t<detail::is_invocable_v<D, T>>>
    #endif
    ...
    #else
            template<typename T, typename Enable = std::enable_if_t<detail::is_invocable_v<D const&, T>>>
    #endif
            [[nodiscard]] friend constexpr decltype(auto) operator|(T&& t, D const& d)
            {
                return d((T&&)t);
            }
...
#else
            template<typename T, typename Enable = std::enable_if_t<detail::is_invocable_v<F const&, T>>>
#endif
...
#else
            template<typename T, typename Enable = std::enable_if_t<detail::is_invocable_v<F&&, T>>>
#endif
        ...
        namespace detail {
            ...
            struct adaptor_impl
            {
                static constexpr decltype(auto) call(F const& f, Args&&... args) { return f((Args&&)args...); }
            };
            ...
            struct adaptor_impl<F, false, Args...>
            {
                ...
                {
                    using closure_func = std::decay_t<decltype(stl_interfaces::bind_back(f, (Args&&)args...))>;
                    return closure<closure_func>(stl_interfaces::bind_back(f, (Args&&)args...));
                }
            };
#endif
        }
...
#else
                return detail::adaptor_impl<F const&, detail::is_invocable_v<F const&, Args...>, Args...>::call(
                    f_,
                    (Args&&)args...);
#endif
            }
