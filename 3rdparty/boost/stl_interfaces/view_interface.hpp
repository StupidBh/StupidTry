    ...
    namespace stl_interfaces {
        BOOST_STL_INTERFACES_NAMESPACE_V1
        {
            ...
            public:
                ...
                template<typename D = Derived, typename R = decltype(std::declval<D&>().empty())>
                constexpr explicit operator bool() noexcept(noexcept(std::declval<D&>().empty()))
                {
                    return !derived().empty();
                }

                template<typename D = Derived, typename R = decltype(std::declval<D const&>().empty())>
                constexpr explicit operator bool() const noexcept(noexcept(std::declval<D const&>().empty()))
                {
                    return !derived().empty();
                }
                ...
                template<typename D = Derived>
                constexpr auto size() noexcept(noexcept(std::declval<D&>().end() - std::declval<D&>().begin()))
            ...
            constexpr auto operator!=(ViewInterface lhs, ViewInterface rhs) noexcept(noexcept(lhs == rhs))
