...
namespace boost {
    namespace stl_interfaces {
        ...
        struct access
        {
            ...
            template<typename D>
            static constexpr auto base(D const& d) noexcept -> decltype(d.base_reference())
            {
                return d.base_reference();
            }
    ...
    struct proxy_arrow_result
        ...
        namespace detail {
            ...
            auto make_pointer(
                T&& value,
                std::enable_if_t<
                    !std::is_pointer<Pointer>::value && !std::is_same<Pointer, void>::value &&
            ...
            template<typename T, typename U>
            constexpr auto common_diff(T lhs, U rhs) noexcept(
                noexcept(static_cast<common_t<T, U>>(lhs) - static_cast<common_t<T, U>>(rhs)))
    ...
    namespace stl_interfaces {
        BOOST_STL_INTERFACES_NAMESPACE_V1
        {
            ...
            public:
                ...
                template<typename D = Derived>
                constexpr auto operator*() const noexcept(noexcept(*access::base(std::declval<D const&>())))
                ...
                template<typename D = Derived>
                constexpr auto
                    operator->() noexcept(noexcept(detail::make_pointer<pointer, reference>(*std::declval<D&>())))
                ...
                constexpr auto operator->() const
                    noexcept(noexcept(detail::make_pointer<pointer, reference>(*std::declval<D const&>())))
                        -> decltype(detail::make_pointer<pointer, reference>(*std::declval<D const&>()))
                {
                    return detail::make_pointer<pointer, reference>(*derived());
                }
                ...
                constexpr auto operator[](difference_type i) const
                    noexcept(noexcept(D(std::declval<D const&>()), std::declval<D&>() += i, *std::declval<D&>()))
                ...
                constexpr auto operator++(int) noexcept(noexcept(D(std::declval<D&>()), ++std::declval<D&>()))
                    -> std::remove_reference_t<
                        decltype(D(std::declval<D&>()), ++std::declval<D&>(), std::declval<D&>())>
                {
                    D retval = derived();
                    ++derived();
                    return retval;
                ...
                template<typename D = Derived>
                constexpr auto operator+(difference_type i) const
                    noexcept(noexcept(D(std::declval<D&>()), std::declval<D&>() += i)) -> std::remove_reference_t<
                        decltype(D(std::declval<D&>()), std::declval<D&>() += i, std::declval<D&>())>
                {
                    D retval = derived();
                    retval += i;
                    return retval;
                ...
                constexpr auto operator-(D other) const
                    noexcept(noexcept(access::base(std::declval<D const&>()) - access::base(other)))
