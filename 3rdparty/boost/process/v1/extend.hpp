    ...
    namespace process {
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            ...
            namespace extend {
                ...
                /// This handler is invoked before the process in launched, to setup parameters. The required signature
                /// is `void(Exec &)`, where `Exec` is a template parameter.
                constexpr boost::process::v1::detail::make_handler_t<boost::process::v1::detail::on_setup_> on_setup;
                /// This handler is invoked if an error occurred. The required signature is `void(auto & exec, const
                /// std::error_code&)`, where `Exec` is a template parameter.
                constexpr boost::process::v1::detail::make_handler_t<boost::process::v1::detail::on_error_> on_error;
                /// This handler is invoked if launching the process has succeeded. The required signature is `void(auto
                /// & exec)`, where `Exec` is a template parameter.
                constexpr boost::process::v1::detail::make_handler_t<boost::process::v1::detail::on_success_>
                    on_success;

#if defined(BOOST_POSIX_API) || defined(BOOST_PROCESS_DOXYGEN)
                /// This handler is invoked if the fork failed. The required signature is `void(auto & exec)`, where
                /// `Exec` is a template parameter. \note Only available on posix.
                constexpr ::boost::process::v1::detail::make_handler_t<
                    ::boost::process::v1::detail::posix::on_fork_error_>
                    on_fork_error;
                /// This handler is invoked if the fork succeeded. The required signature is `void(Exec &)`, where
                /// `Exec` is a template parameter. \note Only available on posix.
                constexpr ::boost::process::v1::detail::make_handler_t<
                    ::boost::process::v1::detail::posix::on_exec_setup_>
                    on_exec_setup;
                /// This handler is invoked if the exec call errored. The required signature is `void(auto & exec)`,
                /// where `Exec` is a template parameter. \note Only available on posix.
                constexpr ::boost::process::v1::detail::make_handler_t<
                    ::boost::process::v1::detail::posix::on_exec_error_>
                    on_exec_error;
#endif
                 ...
                 *
                 * \tparam Sequence The used initializer-sequence, it is fulfills the boost.fusion
                [sequence](http://www.boost.org/doc/libs/master/libs/fusion/doc/html/fusion/sequence.html) concept.


                \xmlonly
                As information for extension development, here is the structure of the process launching (in pseudo-code
                and uml) <xi:include href="posix_pseudocode.xml" xmlns:xi="http://www.w3.org/2001/XInclude"/>
                ...
                struct posix_executor
                {
                    ...
                    /// A pointer to the environment variables, as default it is set to
                    /// [environ](http://pubs.opengroup.org/onlinepubs/009695399/basedefs/xbd_chap08.html)
                    char** env = environ;
                    /// The pid of the process - it will be -1 before invoking
                    /// [fork](http://pubs.opengroup.org/onlinepubs/009695399/functions/fork.html), and after forking
                    /// either 0 for the new process or a positive value if in the current process. */
                    pid_t pid = -1;
                    /// This shared-pointer holds the exit code. It's done this way, so it can be shared between an
                    /// `asio::io_context` and \ref child.
                    std::shared_ptr<std::atomic<int>> exit_status = std::make_shared<std::atomic<int>>(still_active);

                    /// This function returns a const reference to the error state of the executor.
                    const std::error_code& error() const;

                    /// This function can be used to report an error to the executor. This will be handled according to
                    /// the configuration of the executor, i.e. it
                ...
                \xmlonly
                As information for extension development, here is the structure of the process launching (in pseudo-code
                and uml)<xi:include href="windows_pseudocode.xml" xmlns:xi="http://www.w3.org/2001/XInclude"/>
                <mediaobject>
                <caption>
                ...
                struct windows_executor
                {
                    ...
                    /// A pointer to the process-attributes of type
                    /// [SECURITY_ATTRIBUTES](https://msdn.microsoft.com/en-us/library/windows/desktop/aa379560.aspx).
                    /// It's null by default.
                    ::boost::detail::winapi::LPSECURITY_ATTRIBUTES_ proc_attrs = nullptr;
                    /// A pointer to the thread-attributes of type
                    /// [SECURITY_ATTRIBUTES](https://msdn.microsoft.com/en-us/library/windows/desktop/aa379560.aspx).
                    /// It' null by default.
                    ...
                    /// The element holding the process-information after process creation. The type is
                    /// [PROCESS_INFORMATION](https://msdn.microsoft.com/en-us/library/windows/desktop/ms684873.aspx)
                    ...
                    /// The type of the
                    /// [startup-info](https://msdn.microsoft.com/en-us/library/windows/desktop/ms686331.aspx),
                    /// depending on the char-type.
                    typedef typename detail::startup_info<Char>::type startup_info_t;
                    /// The type of the [extended
                    /// startup-info](https://msdn.microsoft.com/de-de/library/windows/desktop/ms686329.aspx), depending
                    /// the char-type; only defined with winapi-version equal or higher than 6.
                    ...
                    /// defined with winapi-version equal or higher than 6.
                    void set_startup_info_ex();
                    /// This element is an instance or a reference (if \ref startup_info_ex exists) to the
                    /// [startup-info](https://msdn.microsoft.com/en-us/library/windows/desktop/ms686331.aspx) for the
                    /// process.
                    startup_info_t startup_info;
                    /// This element is the instance of the  [extended
                    /// startup-info](https://msdn.microsoft.com/de-de/library/windows/desktop/ms686329.aspx). It is
                    /// only available with a winapi-version equal or higher than 6.
