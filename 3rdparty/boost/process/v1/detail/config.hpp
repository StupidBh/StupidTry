...
namespace boost {
    ...
    // copied from linux spec.
        ...
        #define BOOST_POSIX_HAS_SIGTIMEDWAIT 1
                ...
                inline void
                    throw_last_error(const char* msg, boost::source_location const& loc = boost::source_location())
                {
                    boost::throw_exception(process_error(get_last_error(), msg), loc);
                }
                ...
                inline void
                    throw_error(const std::error_code& ec, boost::source_location const& loc = boost::source_location())
                {
                    if (ec) {
                ...
                template<>
                constexpr wchar_t null_char<wchar_t>()
                {
                    return L'\0';
                }
                ...
                template<>
                constexpr char equal_sign<char>()
                {
                    return '=';
                }

                template<>
                constexpr wchar_t equal_sign<wchar_t>()
                {
                    return L'=';
                }

                template<typename Char>
                constexpr Char quote_sign();

                template<>
                constexpr char quote_sign<char>()
                {
                    return '"';
                }

                template<>
                constexpr wchar_t quote_sign<wchar_t>()
                {
                    return L'"';
                }
                ...
                template<>
                constexpr char space_sign<char>()
                {
                    return ' ';
                }

                template<>
                constexpr wchar_t space_sign<wchar_t>()
                {
                    return L' ';
                }
