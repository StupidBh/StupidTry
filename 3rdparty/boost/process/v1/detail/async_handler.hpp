        ...
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            namespace detail {
                ...
                struct has_io_context<T, Args...>
                {
                    ...
                    typedef
                        typename std::is_same<typename std::remove_reference<T>::type, boost::asio::io_context>::type
                            is_ios;
                    typedef typename std::conditional<is_ios::value, std::true_type, next>::type type;
                };
