        ...
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            namespace detail {
                namespace posix {
                    ...
                    inline int
                        apply_out_handles(int handle, std::integral_constant<int, 1>, std::integral_constant<int, -1>)
                    {
                        return ::dup2(handle, STDOUT_FILENO);
                    }

                    inline int
                        apply_out_handles(int handle, std::integral_constant<int, 2>, std::integral_constant<int, -1>)
                    {
                        return ::dup2(handle, STDERR_FILENO);
                    }

                    inline int
                        apply_out_handles(int handle, std::integral_constant<int, 1>, std::integral_constant<int, 2>)
                    {
                    ...
                    struct async_out_buffer :
                        ::boost::process::v1::detail::posix::handler_base_ext,
                        ...
                        ::boost::process::v1::detail::uses_handles
                    {
                        ...
                        std::array<int, 4> get_used_handles()
                        {
                            ...
                            else { // if pipe is not constructed, limit_ds is invoked before -> this also means
                                   // on_exec_setup gets invoked before.
                                return { pp1, pp2, pp1, pp2 };
                            }
                        }

                        async_out_buffer(Buffer& buf) :
                            buf(buf)
                        {
                        }
                        ...
                        {
                            auto pipe = this->pipe;
                            boost::asio::async_read(*pipe, buf, [pipe](const boost::system::error_code&, std::size_t) {
                            });
                        ...
                        {
                            int res = apply_out_handles(
                                pipe->native_sink(),
                                std::integral_constant<int, p1>(),
                                std::integral_constant<int, p2>());
                            if (res == -1) {
                        ...
                        ::boost::process::v1::detail::posix::require_io_context
                    {
                        ...
                        async_out_future(std::future<Type>& fut) { fut = promise->get_future(); }
