            ...
            namespace detail {
                namespace posix {
                    ...
                    struct pipe_in : handler_base_ext, ::boost::process::v1::detail::uses_handles
                    {
                        ...
                        pipe_in(int sink, int source) :
                            source(source),
                            sink(sink)
                        {
                        }

                        std::array<int, 3> get_used_handles() { return { { STDIN_FILENO, source, sink } }; }

                        template<typename T>
                        pipe_in(T& p) :
                            source(p.native_source()),
                            sink(p.native_sink())
                        {
                            p.assign_source(-1);
                        }
                    ...
                    struct async_pipe_in : public pipe_in
                    {
                        ...
                        async_pipe_in(AsyncPipe& p) :
                            pipe_in(p.native_sink(), p.native_source()),
                            pipe(p)
                        {
                        }
