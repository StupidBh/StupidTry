            ...
            namespace detail {
                namespace posix {
                    ...
                    struct on_error_t
                    {
                        ...
                        on_error_t(Executor& exec, const std::error_code& error) :
                            exec(exec),
                            error(error) {};
                    ...
                    struct on_fork_error_t
                    {
                        Executor& exec;
                        const std::error_code& error;
                        on_fork_error_t(Executor& exec, const std::error_code& error) :
                            exec(exec),
                            error(error) {};
                    ...
                    struct on_exec_error_t
                    {
                        ...
                        on_exec_error_t(Executor& exec, const std::error_code& error) :
                            exec(exec),
                            ec(error) {};
                    ...
                    {
                        Executor& exec;
                        on_fork_success_t(Executor& exec) :
                            exec(exec) {};
                    ...
                    template<typename Executor>
                    on_setup_t<Executor> call_on_setup(Executor& exec)
                    {
                        return exec;
                    }

                    template<typename Executor>
                    on_error_t<Executor> call_on_error(Executor& exec, const std::error_code& ec)
                    {
                        return on_error_t<Executor>(exec, ec);
                    }

                    template<typename Executor>
                    on_success_t<Executor> call_on_success(Executor& exec)
                    {
                        return exec;
                    }

                    template<typename Executor>
                    on_fork_error_t<Executor> call_on_fork_error(Executor& exec, const std::error_code& ec)
                    {
                        return on_fork_error_t<Executor>(exec, ec);
                    }

                    template<typename Executor>
                    on_exec_setup_t<Executor> call_on_exec_setup(Executor& exec)
                    {
                        return exec;
                    }

                    template<typename Executor>
                    on_exec_error_t<Executor> call_on_exec_error(Executor& exec, const std::error_code& ec)
                    {
                    ...
                    class executor {
                        ...
                        typedef
                            typename ::boost::process::v1::detail::has_error_handler<Sequence>::type has_error_handler;
                        typedef
                            typename ::boost::process::v1::detail::has_ignore_error<Sequence>::type has_ignore_error;
                        typedef typename ::boost::process::v1::detail::posix::shall_use_vfork<Sequence>::type
                            shall_use_vfork;
                        ...
                        inline void prepare_cmd_style() // this does what execvpe does - but we execute it in the father
                                                        // process, to avoid allocations.
                        {
                    ...
                    public:
                        executor(Sequence& seq) :
                            seq(seq)
                        {
                        }

                        child operator()() { return invoke(has_ignore_error(), shall_use_vfork()); }
                    ...
                    {
                        {
                            ...
                            if (::fcntl(p.p[1], F_SETFD, FD_CLOEXEC) == -1) {
                                ...
                                set_error(
                                    err,
                                    "fcntl(2) failed"); // this might throw, so we need to be sure our pipe is safe.
                                return child();
                            }
