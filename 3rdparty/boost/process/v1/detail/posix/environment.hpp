                ...
                namespace posix {
                    ...
                    class native_environment_impl {
                        ...
                        static std::vector<Char*> _load_var(std::vector<std::basic_string<Char>>& vec)
                        {
                            ...
                            std::transform(vec.begin(), vec.end(), val.begin(), [](std::basic_string<Char>& str) {
                                return &str.front();
                            });
                            val.back() = nullptr;
                            return val;
                    ...
                    {
                        auto itr = std::find_if(_data.begin(), _data.end(), [&](const string_type& st) -> bool {
                            if (st.size() <= id.size()) {
                                return false;
                            }
                            return std::equal(id.begin(), id.end(), st.begin()) &&
                                   (st[id.size()] == equal_sign<Char>());
                        });
                        ...
                        if (itr == _data.end()) {
                            ...
                            return itr->data() + id.size(); // id=Thingy -> +2 points to T
                        }
                    }
                    ...
                    template<typename Char>
                    std::vector<Char*>
                        basic_environment_impl<Char>::_load_var(std::vector<std::basic_string<Char>>& data)
                    {
                        std::vector<Char*> ret;
