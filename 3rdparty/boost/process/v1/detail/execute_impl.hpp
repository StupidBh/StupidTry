...
namespace boost {
        ...
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            ...
            namespace detail {
                ...
                struct has_wchar<First, Args...>
                {
                    ...
                    typedef typename std::remove_cv<typename std::remove_reference<First>::type>::type res_type;
    ...
    // everything needs to be wchar_t
                ...
                template<>
                struct required_char_type<true>
                {
                    typedef wchar_t type;
                };

                template<>
                struct required_char_type<false>
                {
                    typedef char type;
                };
    #endif
                ...
                template<typename... Args>
                using required_char_type_t = typename required_char_type<has_wchar<Args...>::value>::type;
                ...
                struct make_builders_from_view
                {
                    ...
                    typedef typename std::conditional<
                        ...
                        typename make_builders_from_view<next_itr, End, Args..., builder_type>::type>::type type;
                };
                ...
                {
                    typedef
                        typename invoke_get_initializer_collect_keys<boost::fusion::vector<Args...>, Stack..., First>::
                            type next;
                    typedef helper_vector<Stack...> stack_t;

                    typedef
                        typename std::conditional<std::is_same<boost::fusion::void_, First>::value, stack_t, next>::type
                            type;
                };
                ...
                inline child basic_execute_impl(Args&&... args)
                {
                    ...
                    auto inits = boost::fusion::filter_if<boost::process::v1::detail::is_initializer<
                        typename std::remove_reference<boost::mpl::_>::type>>(tup);

                    auto others = boost::fusion::filter_if<boost::mpl::not_<boost::process::v1::detail::is_initializer<
                        typename std::remove_reference<boost::mpl::_>::type>>>(tup);
                    ...
                    boost::fusion::joint_view<decltype(other_inits), decltype(inits)> complete_inits(
                        other_inits,
                        inits);
