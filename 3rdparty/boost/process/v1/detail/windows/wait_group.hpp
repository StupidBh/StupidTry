        ...
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            namespace detail {
                namespace windows {
                    ...
                    inline bool
                        wait_impl(const group_handle& p, std::error_code& ec, std::chrono::system_clock::rep wait_time)
                    {
                        ...
                        while (workaround::get_queued_completion_status(
                            ...
                            &completion_key,
                            &overlapped,
                            static_cast<::boost::winapi::DWORD_>(wait_time))) {
                        ...
                        std::error_code& ec)
                    {
                        std::chrono::milliseconds ms =
                            std::chrono::duration_cast<std::chrono::milliseconds>(timeout_time - Clock::now());
                    ...
                    template<class Clock, class Duration>
                    inline bool
                        wait_until(const group_handle& p, const std::chrono::time_point<Clock, Duration>& timeout_time)
                    {
                        std::error_code ec;
                    ...
                    template<class Rep, class Period>
                    inline bool wait_for(const group_handle& p, const std::chrono::duration<Rep, Period>& rel_time)
                    {
                        std::error_code ec;
