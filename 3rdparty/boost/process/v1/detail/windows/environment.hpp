            ...
            namespace detail {
                namespace windows {
                    ...
                    class native_environment_impl {
                        ...
                        std::unique_ptr<Char[], void (*)(Char*)> _buf { boost::winapi::get_environment_strings<Char>(),
                    ...
                    inline auto native_environment_impl<Char>::get(const pointer_type id) -> string_type
                    {
                        ...
                        if (size == 0) // failed
                        {
                            ...
                            if (err == ::boost::winapi::ERROR_ENVVAR_NOT_FOUND_) { // well, then we consider that an
                                                                                   // empty value
                                return string_type();
                            }
                            else {
                        ...
                        if (size ==
                            sizeof(buf)) // the return size gives the size without the null, so I know this went wrong
                        {
                            /* limit defined here
                             * https://msdn.microsoft.com/en-us/library/windows/desktop/ms683188(v=vs.85).aspx but I
                             * used 32768, so it is a multiple of 4096.
                             */
                            ...
                            while (buf_size <= max_size) {
                                ...
                                if (size == buf_size) { // buffer to small
                                    ...
                                    return std::basic_string<Char>(buf.data(), buf.data() + size);
                                }
                            }
                        }
                    ...
                    inline auto basic_environment_impl<Char>::get(const string_type& id) -> string_type
                    {
                        ...
                        if (std::equal(id.begin(), id.end(), _data.begin()) &&
                            (_data[id.size()] == equal_sign<Char>())) {
                            return string_type(_data.data()); // null-char is handled by the string.
                        }

                        std::vector<Char> seq = {
                            '\0'
                        }; // using a vector, because strings might cause problems with nullchars
                        ...
                        itr += seq.size(); // advance to the value behind the '='; the std::string will take care of
                                           // finding the null-char.
                    ...
                    template<typename T>
                    constexpr T env_seperator();

                    template<>
                    constexpr char env_seperator()
                    {
                        return ';';
                    }

                    template<>
                    constexpr wchar_t env_seperator()
                    {
                        return L';';
                    }
                    ...
                    inline int get_id()
                    {
                        return boost::winapi::GetCurrentProcessId();
                    }

                    inline void* native_handle()
                    {
