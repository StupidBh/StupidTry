            ...
            namespace detail {
                ...
                namespace windows {
                    ...
                    inline std::vector<native_handle_type> get_handles(std::error_code& ec)
                    {
                        ...
                        constexpr static auto STATUS_INFO_LENGTH_MISMATCH_ =
                            static_cast<::boost::winapi::NTSTATUS_>(0xC0000004l);
                        ...
                        for (; nt_status == STATUS_INFO_LENGTH_MISMATCH_;
                             nt_status = workaround::nt_system_query_information(
                                 workaround::SystemHandleInformation_,
                                 info_pointer,
                                 static_cast<::boost::winapi::ULONG_>(buffer.size()),
                                 nullptr)) {
                        ...
                        for (auto itr = info_pointer->Handle; itr != (info_pointer->Handle + info_pointer->Count);
                             itr++) {
                            if (itr->OwnerPid == pid) {
                                res.push_back(
                                    reinterpret_cast<native_handle_type>(
                                        static_cast<std::uintptr_t>(itr->HandleValue)));
                            }
                        }
                    ...
                    inline bool is_stream_handle(native_handle_type handle, std::error_code& ec)
                    {
                        ...
                        workaround::OBJECT_TYPE_INFORMATION_* type_info_p =
                            reinterpret_cast<workaround::OBJECT_TYPE_INFORMATION_*>(vec.data());
                        ...
                        return type_info_p->TypeName.Length >= 5 && nm[0] == L'F' && nm[1] == L'i' && nm[2] == L'l' &&
                               nm[3] == L'e' && nm[4] == L'\0';
                    }
                    ...
                    struct limit_handles_ : handler_base_ext
                    {
                        ...
                        void on_setup(Executor& exec) const
                        {
                            ...
                            foreach_used_handle(exec, [&](::boost::winapi::HANDLE_ handle) {
                                auto itr = std::find(all_handles.begin(), all_handles.end(), handle);
                                ::boost::winapi::DWORD_ flags = 0u;
                                if (itr != all_handles.end()) {
                                    *itr = ::boost::winapi::INVALID_HANDLE_VALUE_; // the handle is used mark it as
                                                                                   // invalid, so it doesn't get reset
                                }
                            });

                            auto part_itr = std::partition(
                                all_handles.begin(),
                                all_handles.end(),
                                [](::boost::winapi::HANDLE_ handle) {
                                    return handle != ::boost::winapi::INVALID_HANDLE_VALUE_;
                                });
