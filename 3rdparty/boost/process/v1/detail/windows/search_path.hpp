                ...
                namespace windows {
                        ...
                        const std::vector<boost::process::v1::filesystem::path>& path)
                    {
                        ...
                        auto itr = std::find_if(ne.cbegin(), ne.cend(), [&](const value_type& e) {
                            return id ==
                                   ::boost::to_upper_copy(e.get_name(), ::boost::process::v1::detail::process_locale());
                        });
                        ...
                        it_ex = std::transform(
                            ...
                            [](const std::wstring& ws) {
                                return boost::to_lower_copy(ws, ::boost::process::v1::detail::process_locale());
                            });

                        std::transform(extensions_in.begin(), extensions_in.end(), it_ex, [](const std::wstring& ws) {
                            return boost::to_upper_copy(ws, ::boost::process::v1::detail::process_locale());
                        });

                        std::copy(
                            std::make_move_iterator(extensions_in.begin()),
                            std::make_move_iterator(extensions_in.end()),
                            extensions.begin() + 1);
                        ...
                        for (const boost::process::v1::filesystem::path& pp_ : path) {
                            ...
                            for (boost::process::v1::filesystem::path ext : extensions) {
                                ...
                                if (!ec && file &&
                                    ::boost::winapi::sh_get_file_info(
                                        pp_ext.native().c_str(),
                                        0,
                                        0,
                                        0,
                                        ::boost::winapi::SHGFI_EXETYPE_)) {
                                    return pp_ext;
                                }
                            }
                        }
