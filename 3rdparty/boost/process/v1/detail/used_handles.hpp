    ...
    namespace process {
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            namespace detail {
                ...
                struct uses_handles
                {
                    // If you get an error here, you must add a `get_handles` function that returns a range or a single
                    // handle value
                ...
                struct foreach_handle_invocator
                {
                    ...
                    foreach_handle_invocator(Func& func) :
                        func(func)
                    {
                    }
                ...
                void foreach_used_handle(Executor& exec, Function&& func)
                {
                    boost::fusion::for_each(
                        boost::fusion::filter_if<does_use_handle<boost::mpl::_>>(exec.seq),
                        foreach_handle_invocator<Function>(func));
                }

                template<typename Executor>
                std::vector<::boost::process::v1::detail::api::native_handle_type> get_used_handles(Executor& exec)
                {
                    std::vector<::boost::process::v1::detail::api::native_handle_type> res = exec.get_used_handles();
                    foreach_used_handle(exec, [&](::boost::process::v1::detail::api::native_handle_type handle) {
