...
namespace boost {
            ...
            namespace detail {
                ...
                public:
                    ...
                    std::string message(int ev) const override
                    {
                        std::string str;
                        switch (ev) {
                        case std::codecvt_base::ok     : str = "ok"; break;
                        case std::codecvt_base::partial: str = "partial"; break;
                        case std::codecvt_base::error  : str = "error"; break;
                        case std::codecvt_base::noconv : str = "noconv"; break;
    ...
    defined(__OpenBSD__) || defined(__HAIKU__)
            ...
            {
                return std::use_facet<std::codecvt<wchar_t, char, std::mbstate_t>>(detail::process_locale());
            }
            ...
            namespace detail {
                ...
                inline std::size_t convert(
                    ...
                    wchar_t* to,
                    wchar_t* to_end,
                    const ::boost::process::v1::codecvt_type& cvt = ::boost::process::v1::codecvt())
                {
                    std::mbstate_t state = std::mbstate_t(); // perhaps unneeded, but cuts bug reports
                    const char* from_next;
                    wchar_t* to_next;

                    auto res = cvt.in(state, from, from_end, from_next, to, to_end, to_next);

                    if (res != std::codecvt_base::ok) {
                        throw process_error(
                            res,
                            ::boost::process::v1::codecvt_category(),
                    ...
                    const ::boost::process::v1::codecvt_type& cvt = ::boost::process::v1::codecvt())
                {
                    ...
                    if ((res = cvt.out(state, from, from_end, from_next, to, to_end, to_next)) !=
                        std::codecvt_base::ok) {
                        throw process_error(
                            res,
                            ::boost::process::v1::codecvt_category(),
                    ...
                    const ::boost::process::v1::codecvt_type& cvt = ::boost::process::v1::codecvt())
                {
                    ...
                    auto sz = convert(st.c_str(), st.c_str() + st.size(), &out.front(), &out.back(), cvt);
                ...
                inline std::vector<wchar_t> convert(
                    const std::vector<char>& st,
                    const ::boost::process::v1::codecvt_type& cvt = ::boost::process::v1::codecvt())
                {
                    std::vector<wchar_t> out(st.size() + 10); // just to be sure
                    auto sz = convert(st.data(), st.data() + st.size(), &out.front(), &out.back(), cvt);
                    ...
                    const ::boost::process::v1::codecvt_type& cvt = ::boost::process::v1::codecvt())
                {
                    ...
                    using namespace std;
                    auto sz = convert(begin, end, &out.front(), &out.back(), cvt);
                    out.resize(sz);
                    return out;
                }
