...
namespace boost {
    ...
    namespace process {
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            ...
            /** The main class to hold a child process. It is similar to
             * [std::thread](http://en.cppreference.com/w/cpp/thread/thread), in that it has a join and detach function.
             *
             * @attention The destructor will call terminate on the process if not joined or detached without any
             * warning.
             *
             */
            ...
            class child {
                 ...
                 *
                 * @attention There is no guarantee that this will work. The process need the right access rights, which
                ...
                /** Get the exit_code. The return value is without any meaning if the child wasn't waited for or if it
                 * was terminated. */
                int exit_code() const;
                /** Get the Process Identifier. */
                pid_t id() const;

                /** Get the native, uninterpreted exit code. The return value is without any meaning if the child wasn't
                 * waited for or if it was terminated. */
                 ...
                 * @note That does not mean, that the process is still running. It only means, that the handle does or
                 * did exist.
                 */
                bool valid() const;
                 ...
                 *  It is implement with [SIGKILL](http://pubs.opengroup.org/onlinepubs/009695399/functions/kill.html)
                 * on posix and [TerminateProcess](https://technet.microsoft.com/en-us/library/ms686714.aspx) on
                 * windows.
                 *
                 */
                void terminate();
