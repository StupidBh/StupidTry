    ...
    namespace process {
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            namespace detail {
                ...
                template<>
                struct is_wchar_t<env_append<wchar_t>> : std::true_type
                {
                };

                template<>
                struct is_wchar_t<env_reset<wchar_t>> : std::true_type
                {
                };

                template<>
                struct is_wchar_t<basic_environment<wchar_t>> : std::true_type
                {
                };
                ...
                struct env_proxy
                {
                    ...
                    env_set<Char> operator=(const string_type& value) { return { std::move(key), value }; }
                    ...
                    env_append<Char> operator+=(const string_type& value) { return { std::move(key), value }; }
                    ...
                    env_reset<Char> operator=(boost::none_t) { return { std::move(key) }; }
                };
                ...
                struct env_
                {
                    ...
                    template<typename Char>
                    env_set<Char>
                        operator()(const std::basic_string<Char>& key, const std::basic_string<Char>& value) const
                    {
                        return { key, value };
                    }

                    template<typename Char>
                    env_set<Char> operator()(
                    ...
                    template<typename Char>
                    env_set<Char>
                        operator()(const std::basic_string<Char>& key, const std::initializer_list<Char*>& value) const
                    {
                        return { key, make_env_string<Char>(value) };
                    }
                ...
                struct env_builder
                {
                    ...
                    env_builder() :
                        env { basic_native_environment<Char>() }
                    {
                    }

                    void operator()(const basic_environment<Char>& e) { env = e; }

                    void operator()(env_init<Char>& ei) { env = std::move(ei.env); }

                    void operator()(env_set<Char>& es) { env[es.key] = es.value; }

                    void operator()(env_reset<Char>& es) { env.erase(es.key); }
                    ...
                    api::env_init<Char> get_initializer() { return api::env_init<Char>(std::move(env)); }
                };
