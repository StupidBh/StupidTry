    ...
    namespace process {
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            namespace detail {
                ...
                template<typename T>
                using is_streambuf = typename std::is_same<T, boost::asio::streambuf>::type;
                template<typename T>
                using is_const_buffer = std::integral_constant<
                    bool,
                    std::is_same<boost::asio::const_buffer, T>::value |
                        std::is_base_of<boost::asio::const_buffer, T>::value>;
                template<typename T>
                using is_mutable_buffer = std::integral_constant<
                    bool,
                ...
                struct std_in_
                {
                    ...
                    api::pipe_in operator=(basic_pipe<Char, Traits>& p) const
                    {
                        return p;
                    }
                    ...
                    template<typename Char, typename Traits>
                    api::pipe_in operator=(basic_opstream<Char, Traits>& p) const
                    {
                        return p.pipe();
                    }
                    ...
                    template<typename Char, typename Traits>
                    api::pipe_in operator=(basic_pstream<Char, Traits>& p) const
                    {
                        return p.pipe();
                    }
                    ...
                    template<
                        typename T,
                        typename =
                            typename std::enable_if<is_const_buffer<T>::value || is_mutable_buffer<T>::value>::type>
                ...
                struct std_out_
                {
                    ...
                    api::file_out<p1, p2> operator=(const boost::process::v1::filesystem::path& p) const
                    {
                    ...
                    template<typename Char, typename Traits>
                    api::pipe_out<p1, p2> operator=(basic_pipe<Char, Traits>& p) const
                    {
                        return p;
                    }
                    ...
                    template<typename Char, typename Traits>
                    api::pipe_out<p1, p2> operator=(basic_ipstream<Char, Traits>& p) const
                    {
                        return p.pipe();
                    }
                    ...
                    template<typename Char, typename Traits>
                    api::pipe_out<p1, p2> operator=(basic_pstream<Char, Traits>& p) const
                    {
                        return p.pipe();
                    }
                    ...
                    api::async_out_buffer<p1, p2, const asio::mutable_buffer>
                        operator=(const asio::mutable_buffer& buf) const
                    {
                        return buf;
                    }

                    api::async_out_buffer<p1, p2, const asio::mutable_buffers_1>
                        operator=(const asio::mutable_buffers_1& buf) const
                    {
                        return buf;
                    }
                    ...
                    api::async_out_future<p1, p2, std::vector<char>>
                        operator=(std::future<std::vector<char>>& fut) const
                    {
                        return fut;
                    }
                    ...
                    template<
                        ...
                        typename = typename std::enable_if<
                            (((p1 == 1) && (pin == 2)) || ((p1 == 2) && (pin == 1))) && (p2 == -1)>::type>
            ...
            Valid types for pipe I/O are the following:
             ...
             - `boost::asio::const_buffer`   \xmlonly <footnote><para> Constructed with
            <code>boost::asio::buffer</code></para></footnote> \endxmlonly
             - `boost::asio::mutable_buffer` \xmlonly <footnote><para> Constructed with
            <code>boost::asio::buffer</code></para></footnote> \endxmlonly
            ...
            \warning This feature requires `boost/process/async.hpp` to be included and a reference to
            `boost::asio::io_context` to be passed to the launching function.
            ...
            */
            ...
            \note The Semantic is the same as for \xmlonly <globalname
            alt="boost::process::v1::std_err">std_err</globalname> \endxmlonly

            \note `std_err` and `std_out` can be combined into one stream, with the `operator &`, i.e. `std_out &
            std_err`.
