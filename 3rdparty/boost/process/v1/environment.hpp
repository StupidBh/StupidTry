...
namespace boost {
        ...
        BOOST_PROCESS_V1_INLINE namespace v1
        {
            namespace detail {
                ...
                struct make_const_entry
                {
                    ...
                    Environment* env;
                    make_const_entry(Environment& env) :
                        env(&env) {};
            ...
            public:
                ...
                using const_iterator =
                    boost::transform_iterator<const_entry_maker, Char**, const_entry_type, const_entry_type>;
                ...
                iterator find(const string_type& key)
                {
                    ...
                    while (*p != nullptr) {
                        ...
                        if ((std::distance(st1.begin(), st1.end()) < len) && std::equal(st1.begin(), st1.end(), *p)) {
                            break;
                        }
                        p++;
                    }
                ...
                bool empty() { return *this->_env_impl == nullptr; }

                std::size_t size() const { return (_get_end() - this->_env_impl); }

                void clear()
                {
                    std::vector<string_type> names;
                    names.resize(size());
                    std::transform(cbegin(), cend(), names.begin(), [](const const_entry_type& cet) {
                        return cet.get_name();
                    });
            ...
            public:
                ...
                struct const_entry_type
                {
                    ...
                    {
                        return string_type(_name.begin(), _name.end());
                    }
    ...
    namespace this_process {
        ...
        inline int get_id()
        {
            return ::boost::process::v1::detail::api::get_id();
        }

        /// Get the native handle of the current process.
        inline native_handle_type native_handle()
        {
            return ::boost::process::v1::detail::api::native_handle();
        }
#if !defined(BOOST_NO_ANSI_APIS)
        /// Get the environment of the current process.
        inline native_environment environment()
        {
            return ::boost::process::v1::native_environment();
        }
#endif
        /// Get the environment of the current process.
        inline wnative_environment wenvironment()
        {
            return ::boost::process::v1::wnative_environment();
        }
        ...
        inline std::vector<boost::process::v1::filesystem::path> path()
        {
            ...
            auto itr = std::find_if(ne.cbegin(), ne.cend(), [&](const value_type& e) {
