...
namespace detail {
    ...
    struct handle_closer
    {
        ...
        handle_closer(bool close) :
            close(close)
        {
        }

        handle_closer(DWORD flags) :
            close(false),
            flags { flags }
        {
        }
    ...
    struct process_io_binding
    {
        ...
        process_io_binding(FILE* f) :
            process_io_binding(reinterpret_cast<HANDLE>(::_get_osfhandle(_fileno(f))))
        {
        }

        process_io_binding(HANDLE h) :
            h { h, get_flags(h) }
        {
        }

        process_io_binding(std::nullptr_t) :
            process_io_binding(filesystem::path("NUL"))
        {
        }
        ...
        process_io_binding(
            ReadablePipe& pipe,
            typename std::enable_if<is_readable_pipe<ReadablePipe>::value && Target != STD_INPUT_HANDLE>::type* =
                nullptr)
        {
        ...
        process_io_binding(
            WritablePipe& pipe,
            typename std::enable_if<is_writable_pipe<WritablePipe>::value && Target == STD_INPUT_HANDLE>::type* =
                nullptr)
        {
            net::detail::native_pipe_handle p[2];
            error_code ec;
    ...
    struct process_io_binding
    {
        ...
        process_io_binding(FILE* f) :
            process_io_binding(fileno(f))
        {
        }

        process_io_binding(int fd) :
            fd(fd)
        {
        }

        process_io_binding(std::nullptr_t) :
            process_io_binding(filesystem::path("/dev/null"))
        {
        }

        process_io_binding(const filesystem::path& pth) :
            fd(::open(pth.c_str(), Target == STDIN_FILENO ? O_RDONLY : (O_WRONLY | O_CREAT), 0660)),
        ...
        error_code on_setup(posix::default_launcher&, const filesystem::path&, const char* const*) { return ec; }

        error_code on_exec_setup(posix::default_launcher&, const filesystem::path&, const char* const*)
        {
            if (::dup2(fd, target) == -1) {
