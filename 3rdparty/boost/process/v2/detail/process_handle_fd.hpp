...
namespace detail {
    ...
    struct basic_process_handle_fd
    {
        ...
        executor_type get_executor() { return descriptor_.get_executor(); }
        ...
        basic_process_handle_fd(executor_type executor) :
            pid_(-1),
            descriptor_(executor)
        {
        }

        basic_process_handle_fd(executor_type executor, pid_type pid) :
            pid_(pid),
            descriptor_(executor, syscall(SYS_pidfd_open, pid, 0))
        {
        ...
        basic_process_handle_fd(executor_type executor, pid_type pid, native_handle_type process_handle) :
            pid_(pid),
            descriptor_(executor, process_handle)
        {
        }
    ...
    private:
        template<typename>
        friend struct basic_process_handle_fd;
        pid_type pid_ = -1;
        ...
        struct async_wait_op_
        {
            ...
            void operator()(Self&& self)
            {
                ...
                struct completer
                {
                    ...
                    void operator()() { self.complete(ec); }
                };
    ...
    public:
        ...
        auto async_wait(
            ...
            -> decltype(net::async_compose<WaitHandler, void(error_code)>(
                async_wait_op_ { descriptor_, pid_, exit_code },
                handler,
                descriptor_))
        {
