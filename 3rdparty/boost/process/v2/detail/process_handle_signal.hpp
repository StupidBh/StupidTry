...
namespace detail {
    ...
    struct basic_process_handle_signal
    {
        ...
        executor_type get_executor() { return signal_set_.get_executor(); }
        ...
        basic_process_handle_signal(basic_process_handle_signal&& handle) :
            pid_(handle.pid_),
            signal_set_(handle.signal_set_.get_executor(), SIGCHLD)
        {
            handle.pid_ = -1;
        }
        ...
        basic_process_handle_signal(basic_process_handle_signal<Executor1>&& handle) :
            pid_(handle.pid_),
            signal_set_(Executor1(handle.signal_set_.get_executor()), SIGCHLD)
        {
            handle.pid_ = -1;
        }
        ...
        void terminate_if_running(error_code&) { terminate_if_running(); }
        ...
        bool is_open() const { return pid_ != -1; }
    ...
    private:
        ...
        struct signal_set_dummy_
        {
            ...
            signal_set_dummy_(Executor executor, int) :
        ...
        struct async_wait_op_
        {
            ...
            void operator()(Self&& self, error_code ec, int /*sig*/)
            {
                if (ec == net::error::operation_aborted &&
                    self.get_cancellation_state().cancelled() == net::cancellation_type::none) {
                    ec.clear();
                }
    ...
    public:
        ...
        auto async_wait(
            ...
            -> decltype(net::async_compose<WaitHandler, void(error_code)>(
                async_wait_op_ { signal_set_, pid_, exit_code },
                handler,
                signal_set_))
        {
