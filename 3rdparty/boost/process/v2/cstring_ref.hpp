...
namespace detail {
    ...
    BOOST_CONSTEXPR static const char* null_char_(char)
    {
        return "";
    }

    BOOST_CONSTEXPR static const wchar_t* null_char_(wchar_t)
    {
        return L"";
    }

    BOOST_CONSTEXPR static const char16_t* null_char_(char16_t)
    {
        return u"";
    }

    BOOST_CONSTEXPR static const char32_t* null_char_(char32_t)
    {
        return U"";
    }

#if defined(BOOST_PROCESS_V2_HAS_CHAR8_T)
    BOOST_CONSTEXPR static const char8_t* null_char_(char8_t)
    {
        return u8"";
    }
#endif
...
struct basic_cstring_ref
{
    ...
    BOOST_CONSTEXPR basic_cstring_ref() noexcept :
        view_(detail::null_char_(value_type {}))
    {
    }

    BOOST_CONSTEXPR basic_cstring_ref(std::nullptr_t) = delete;

    BOOST_CONSTEXPR basic_cstring_ref(const value_type* s) :
        view_(s)
    {
    }
    ...
    template<
        ...
        typename = typename std::enable_if<std::is_same<
            const value_type,
            typename std::remove_pointer<decltype(std::declval<Source>().c_str())>::type>::value>::type>
    BOOST_CONSTEXPR basic_cstring_ref(Source&& src) :
        view_(src.c_str())
    {
    }
    ...
    {
        return reverse_iterator(view_ + length());
    };
    ...
    BOOST_ATTRIBUTE_NODISCARD BOOST_CONSTEXPR bool empty() const BOOST_NOEXCEPT
    {
        return *view_ == *detail::null_char_(CharT {});
    }
    ...
    BOOST_CONSTEXPR basic_cstring_ref substr(size_type pos = 0) const { return basic_cstring_ref(view_ + pos); }
    ...
    BOOST_CONSTEXPR bool starts_with(value_type x) const BOOST_NOEXCEPT { return traits_type::eq(view_[0], x); }
...
inline std::basic_ostream<charT, traits>&
    operator<<(std::basic_ostream<charT, traits>& os, const basic_cstring_ref<charT, traits>& str)
{
