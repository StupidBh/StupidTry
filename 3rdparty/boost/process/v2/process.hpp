...
struct basic_process
{
    ...
    explicit basic_process(executor_type executor, const filesystem::path& exe, Args&& args, Inits&&... inits) :
        basic_process(
    ...
    explicit basic_process(
        ...
        Inits&&... inits) :
        basic_process(
            default_process_launcher()(executor_type(context.get_executor()), exe, args, std::forward<Inits>(inits)...))
    {
    }
    ...
    explicit basic_process(
        ...
        basic_process(
            default_process_launcher()(
                executor_type(context.get_executor()),
                exe,
                std::forward<Args>(args),
                std::forward<Inits>(inits)...))
    {
    ...
    explicit basic_process(executor_type exec, pid_type pid) :
        process_handle_(std::move(exec), pid)
    {
    }
    ...
    explicit basic_process(
        ExecutionContext& context,
        pid_type pid,
        typename std::enable_if<std::is_convertible<ExecutionContext&, net::execution_context&>::value, void*>::type =
            nullptr) :
    ...
    explicit basic_process(
        ExecutionContext& context,
        typename std::enable_if<is_convertible<ExecutionContext&, net::execution_context&>::value, void*>::type =
            nullptr) :
    ...
    ~basic_process() { process_handle_.terminate_if_running(); }
    ...
    void interrupt(error_code& ec) { process_handle_.interrupt(ec); }
    ...
    void request_exit(error_code& ec) { process_handle_.request_exit(ec); }

    /// Send the process a signal requesting it to stop. This may rely on undocumented functions.
    void suspend(error_code& ec) { process_handle_.suspend(ec); }
    ...
    void resume(error_code& ec) { process_handle_.resume(ec); }
    ...
    void terminate(error_code& ec) { process_handle_.terminate(exit_status_, ec); }
    ...
    int exit_code() const { return evaluate_exit_code(exit_status_); }
    ...
    native_exit_code_type native_exit_code() const { return exit_status_; }
        ...
        void operator()(Self&& self)
        {
            if (!process_is_running(res)) {
                struct completer
                {
                    ...
                    void operator()() { self.complete(error_code {}, evaluate_exit_code(code)); }
                };
...
public:
    ...
    auto async_wait(WaitHandler&& handler = net::default_completion_token_t<executor_type>())
        -> decltype(net::async_compose<WaitHandler, void(error_code, int)>(
            async_wait_op_ { process_handle_, exit_status_ },
            handler,
            process_handle_))
    {
