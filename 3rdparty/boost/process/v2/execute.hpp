...
namespace detail {
    ...
    struct execute_op
    {
        ...
        struct cancel
        {
            ...
            cancel(basic_process<Executor>* proc) :
                proc(proc)
            {
            }
        ...
        void operator()(Self&& self)
        {
            ...
            auto pro_ = proc.get();
            pro_->async_wait(net::bind_cancellation_slot(net::cancellation_slot(), std::move(self)));
        }
 ...
 */
template<
    typename Executor = net::any_io_executor,
...
inline auto
    async_execute(basic_process<Executor> proc, WaitHandler&& handler = net::default_completion_token_t<Executor>())
        -> decltype(net::async_compose<WaitHandler, void(error_code, int)>(
            detail::execute_op<Executor> { nullptr },
            handler,
            std::declval<Executor>()))
{
    ...
    return net::async_compose<WaitHandler, void(error_code, int)>(
        detail::execute_op<Executor> { std::move(pro_) },
        handler,
        exec);
}
