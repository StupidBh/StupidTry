...
namespace boost {
    ...
    public:
        ...
        // construct/copy
        BOOST_CONSTEXPR basic_string_view() BOOST_NOEXCEPT : ptr_(NULL), len_(0) {}
        ...
        {
            return ~static_cast<size_type>(0) / (sizeof(value_type) * 2u);
        }
        ...
        {
            return pos >= len_ ? BOOST_THROW_EXCEPTION(std::out_of_range("boost::string_view::at")),
                   ptr_[0]     : ptr_[pos];
        }
        ...
        BOOST_CXX14_CONSTEXPR basic_string_view substr() const { return basic_string_view(data(), size()); }
        ...
        BOOST_CXX14_CONSTEXPR int compare(size_type pos1, size_type n1, basic_string_view x) const
        {
            return substr(pos1, n1).compare(x);
        }

        BOOST_CXX14_CONSTEXPR int
            compare(size_type pos1, size_type n1, basic_string_view x, size_type pos2, size_type n2) const
        {
            return substr(pos1, n1).compare(x.substr(pos2, n2));
        }

        BOOST_CXX14_CONSTEXPR int compare(const charT* x) const { return compare(basic_string_view(x)); }
        ...
        BOOST_CXX14_CONSTEXPR bool contains(basic_string_view s) const BOOST_NOEXCEPT { return find(s) != npos; }

        BOOST_CXX14_CONSTEXPR bool contains(charT c) const BOOST_NOEXCEPT { return find(c) != npos; }

        BOOST_CXX14_CONSTEXPR bool contains(const charT* s) const BOOST_NOEXCEPT { return find(s) != npos; }
        ...
        BOOST_CXX14_CONSTEXPR size_type
            find_first_not_of(const charT* s, size_type pos, size_type n) const BOOST_NOEXCEPT
        {
        ...
        BOOST_CXX14_CONSTEXPR size_type
            find_last_not_of(const charT* s, size_type pos, size_type n) const BOOST_NOEXCEPT
        {
    ...
    inline BOOST_CXX14_CONSTEXPR bool
        operator==(basic_string_view<charT, traits> x, basic_string_view<charT, traits> y) BOOST_NOEXCEPT
    {
        if (x.size() != y.size()) {
            return false;
        }
    ...
    inline BOOST_CXX14_CONSTEXPR bool
        operator==(basic_string_view<charT, traits> x, const std::basic_string<charT, traits, Allocator>& y)
            BOOST_NOEXCEPT
    {
    ...
    template<typename charT, typename traits, typename Allocator>
    inline BOOST_CXX14_CONSTEXPR bool
        operator==(const std::basic_string<charT, traits, Allocator>& x, basic_string_view<charT, traits> y)
            BOOST_NOEXCEPT
    {
    ...
    template<typename charT, typename traits>
    inline BOOST_CXX14_CONSTEXPR bool operator==(basic_string_view<charT, traits> x, const charT* y) BOOST_NOEXCEPT
    {
        return x == basic_string_view<charT, traits>(y);
    }

    template<typename charT, typename traits>
    inline BOOST_CXX14_CONSTEXPR bool operator==(const charT* x, basic_string_view<charT, traits> y) BOOST_NOEXCEPT
    {
        return basic_string_view<charT, traits>(x) == y;
    }
