...
namespace boost {
    namespace wave {
        namespace impl {
            ...
            template<class AdaptableUnaryFunctionT, class IteratorT>
            inline typename ref_transform_iterator_generator<AdaptableUnaryFunctionT, IteratorT>::type
                make_ref_transform_iterator(IteratorT base, AdaptableUnaryFunctionT const& f)
            {
                typedef
                    typename ref_transform_iterator_generator<AdaptableUnaryFunctionT, IteratorT>::type iterator_type;
            ...
            struct get_token_value
            {
                ...
                {
                    BOOST_ASSERT(1 == std::distance(node.value.begin(), node.value.end()));
