    ...
    namespace wave {
        namespace util {
            ...
            namespace impl {
                ...
                inline bool is_special_macroname(ContextT const& ctx, StringT const& name)
                {
                    if (name.size() < 7) {
                        return false;
                    }
                    ...
                    if (boost::wave::need_has_include(ctx.get_language()) && ("__has_include" == name)) {
                        return true;
                    }
    #endif
                    ...
                    if ('_' == name[0] && '_' == name[1]) {
                        ...
                        if (str == "cplusplus" || str == "STDC__" || str == "TIME__" || str == "DATE__" ||
                            str == "LINE__" || str == "FILE__" || str == "INCLUDE_LEVEL__") {
                            return true;
                        }
                    }
                    return false;
                ...
                inline bool token_equals(TokenT const& left, TokenT const& right)
                {
                    ...
                    if (IS_CATEGORY(left, ParameterTokenType)) {
                        ...
                        return (T_IDENTIFIER == id || IS_CATEGORY(id, KeywordTokenType) ||
                    ...
                    return token_id(left) == token_id(right) &&
                           (IS_CATEGORY(left, WhiteSpaceTokenType) || left.get_value() == right.get_value());
                }
                ...
                inline bool definition_equals(ContainerT const& definition, ContainerT const& new_definition)
                {
                    ...
                    while (first1 != last1 && first2 != last2 && token_equals(*first1, *first2)) {
                        ...
                        if (IS_CATEGORY(id1, WhiteSpaceTokenType) && IS_CATEGORY(id2, WhiteSpaceTokenType)) {
                        ...
                        else if (!IS_CATEGORY(id1, WhiteSpaceTokenType) && !IS_CATEGORY(id2, WhiteSpaceTokenType)) {
                            ++first1;
                            ++first2;
                        }
                        else {
                ...
                inline bool is_whitespace_only(ContainerT const& argument)
                {
                    ...
                    for (typename ContainerT::const_iterator it = argument.begin(); it != end; ++it) {
                ...
                ///////////////////////////////////////////////////////////////////////////////
                template<typename ContainerT>
                inline bool is_blank_only(ContainerT const& argument)
                {
                    typename ContainerT::const_iterator end = argument.end();
                    for (typename ContainerT::const_iterator it = argument.begin(); it != end; ++it) {
                        if (!IS_CATEGORY(*it, WhiteSpaceTokenType) && (T_PLACEMARKER != token_id(*it))) {
                            return false;
                        }
                    }
                    return true;
                ...
                inline void trim_sequence_left(ContainerT& argument)
                {
                    ...
                    if (argument.size() > 0 && IS_CATEGORY(argument.front(), WhiteSpaceTokenType)) {
                        argument.pop_front();
                    }
                }
                ...
                inline void trim_sequence_right(ContainerT& argument)
                {
                    ...
                    if (argument.size() > 0 && IS_CATEGORY(argument.back(), WhiteSpaceTokenType)) {
                        argument.pop_back();
                    }
                }
                ...
                inline bool
                    skip_to_token(ContextT& ctx, IteratorT& it, IteratorT const& end, token_id id, bool& seen_newline)
                {
                    using namespace boost::wave;
                    if (token_id(*it) == id) {
                        return true;
                    ...
                    while (IS_CATEGORY(*it, WhiteSpaceTokenType) || T_NEWLINE == token_id(*it)) {
                ...
                public:
                    find_concat_operator(bool& found_) :
                        found_concat(found_)
                    {
                    }
                ...
                struct to_string_helper
                {
                    ...
                    static Target call(Src const& str) { return Target(str.c_str()); }
                };
                ...
                struct to_string_helper<Src, Src>
                {
                    ...
                    static Src const& call(Src const& str) { return str; }
                };
                ...
                struct to_string_helper<Target, char const*>
                {
                    ...
                    static Target call(char const* str) { return Target(str); }
                };
