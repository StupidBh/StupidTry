    ...
    namespace wave {
        namespace util {
            ...
            public:
                ...
                IteratorT& get_base_iterator() { return base_type::base_reference(); }

                IteratorT const& get_base_iterator() const { return base_type::base_reference(); }
                ...
                bool equal(boost::iterator_adaptor<OtherDerivedT, OtherIteratorT, V, C, R, D> const& x) const
                {
                    ...
                    return ((unput_queue.empty() && rhs.unput_queue.empty()) ||
                            (&unput_queue == &rhs.unput_queue && unput_queue.begin() == rhs.unput_queue.begin())) &&
            ...
            namespace impl {
                ...
                struct gen_unput_queue_iterator
                {
                    ...
                    static return_type generate(iterator_type const& it) { return return_type(it, last); }
                ...
                template<typename IteratorT, typename TokenT, typename ContainerT>
                typename gen_unput_queue_iterator<IteratorT, TokenT, ContainerT>::container_type
                ...
                struct gen_unput_queue_iterator<unput_queue_iterator<IteratorT, TokenT, ContainerT>, TokenT, ContainerT>
                {
                    ...
                    static return_type generate(iterator_type& it) { return return_type(it.base(), last); }
                ...
                {
                    static void do_(IteratorT& dest, IteratorT const& src) { dest = src; }
                };
                ...
                struct next_token
                {
                    static boost::wave::token_id peek(IteratorT it, IteratorT end, bool skip_whitespace = true)
                    {
                        ...
                        if (skip_whitespace) {
                            for (++it; it != end; ++it) {
                                if (!IS_CATEGORY(*it, WhiteSpaceTokenType) && T_NEWLINE != token_id(*it)) {
                    ...
                    static boost::wave::token_id peek(iterator_type it, iterator_type end, bool skip_whitespace = true)
                    {
                        ...
                        if (0 != queue.size()) {
                            ...
                            if (skip_whitespace) {
                                for (++cit; cit != cend; ++cit) {
                                    if (!IS_CATEGORY(*cit, WhiteSpaceTokenType) && T_NEWLINE != token_id(*cit)) {
                                        break; // stop at the first non-whitespace token
                                    }
                                }
                            }
                        ...
                        if (skip_whitespace) {
                            for (/**/; base_it != base_end; ++base_it) {
                                if (!IS_CATEGORY(*base_it, WhiteSpaceTokenType) && T_NEWLINE != token_id(*base_it)) {
                                    break; // stop at the first non-whitespace token
                                }
                            }
                        }
