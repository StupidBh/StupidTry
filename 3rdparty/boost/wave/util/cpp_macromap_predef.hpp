    ...
    namespace wave {
        namespace util {
            ...
            private:
                ...
                static string_type strconv(std::string const& ss) { return string_type(ss.c_str()); }
            ...
            protected:
                void reset_datestr()
                {
                    ...
                    if (tt != (time_t)-1) {
                        tb = localtime(&tt);
                        datestr_ = strconv((boost::format("\"%s %2d %4d\"") % monthnames[tb->tm_mon] % tb->tm_mday %
                                            (tb->tm_year + 1900))
                                               .str());
                    }
                    else {
                ...
                void reset_timestr()
                {
                    ...
                    if (tt != (time_t)-1) {
                        tb = localtime(&tt);
                        timestr_ = strconv(
                            (boost::format("\"%02d:%02d:%02d\"") % tb->tm_hour % tb->tm_min % tb->tm_sec).str());
                    }
                    else {
                        timestr_ = "\"??:??:??\"";
                    }
                ...
                void reset_version()
                {
                    ...
                    version_ =
                        strconv((boost::format("0x%02d%1d%1d%04ld") % BOOST_WAVE_VERSION_MAJOR %
                                 BOOST_WAVE_VERSION_MINOR % BOOST_WAVE_VERSION_SUBMINOR % (seconds / (3600 * 24)))
                                    .str());
                }
                ...
                void reset_versionstr()
                {
                    ...
                    versionstr_ = strconv((boost::format("\"%d.%d.%d.%ld [%s/%s]\"") % BOOST_WAVE_VERSION_MAJOR %
                ...
                {
                    return strconv((boost::format("0x%02d%1d%1d") % BOOST_WAVE_VERSION_MAJOR %
                                    BOOST_WAVE_VERSION_MINOR % BOOST_WAVE_VERSION_SUBMINOR)
                                       .str());
                }

                // __WAVE_CONFIG__
                string_type get_config() const { return strconv((boost::format("0x%08x") % BOOST_WAVE_CONFIG).str()); }
            ...
            public:
                ...
                {
                    static static_macros data[] = { { "__STDC__", T_INTLIT, "1" },
