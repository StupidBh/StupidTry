...
namespace boost {
        ...
        namespace util {
            ...
            public:
                macromap(ContextT& ctx_) :
                    current_macros(0),
                    defined_macros(new defined_macros_type(1)),
                    main_pos("", 0),
                    ctx(ctx_),
                    macro_uid(1)
                {
                ...
                // expects a token sequence as its parameters
                template<typename IteratorT>
                bool has_include(IteratorT const& begin, IteratorT const& end, bool is_quoted_filename, bool is_system)
                    const;
    #endif
                ...
                token_type const& expand_tokensequence(
                    IteratorT& first,
                ...
                name_iterator begin() { return defined_macros_type::make_iterator(current_macros->begin()); }

                name_iterator end() { return defined_macros_type::make_iterator(current_macros->end()); }
                ...
                const_name_iterator end() const { return defined_macros_type::make_iterator(current_macros->end()); }

            protected:
                //  Helper functions for expanding all macros in token sequences
                template<typename IteratorT, typename ContainerT>
                token_type const& expand_tokensequence_worker(
                    ContainerT& pending,
                ...
                template<typename IteratorT, typename ContainerT>
                token_type const& resolve_has_include(IteratorT& first, IteratorT const& last, ContainerT& expanded);
    #endif

                //  Resolve operator _Pragma or the #pragma directive
                template<typename IteratorT, typename ContainerT>
                bool resolve_operator_pragma(
                    IteratorT& first,
                    IteratorT const& last,
                    ContainerT& expanded,
                    bool& seen_newline);
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline bool macromap<ContextT>::add_macro(
                token_type const& name,
                bool has_parameters,
                parameter_container_type& parameters,
                definition_container_type& definition,
                ...
                defined_macros_type* scope)
            {
                ...
                if (boost::wave::need_variadics(ctx.get_language()) && "__VA_OPT__" == name.get_value()) {
    ...
    #if BOOST_WAVE_SUPPORT_HAS_INCLUDE != 0
                if (boost::wave::need_has_include(ctx.get_language()) && "__has_include" == name.get_value()) {
                ...
                std::pair<typename defined_macros_type::iterator, bool> p = current_scope->insert(
                    typename defined_macros_type::value_type(
                        name.get_value(),
                        macro_ref_type(new macro_definition_type(name, has_parameters, is_predefined, ++macro_uid))));
                ...
                ctx.get_hooks().defined_macro(
                    ctx.derived(),
                    name,
                    has_parameters,
                    (*p.first).second->macroparameters,
                    (*p.first).second->macrodefinition,
                    is_predefined);
                return true;
            }
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline bool macromap<ContextT>::is_defined(
                typename token_type::string_type const& name,
                typename defined_macros_type::iterator& it,
                defined_macros_type* scope) const
            {
                ...
                if (name == "__LINE__" || name == "__FILE__" || name == "__INCLUDE_LEVEL__") {
                    return true;
                }

    #if BOOST_WAVE_SUPPORT_HAS_INCLUDE != 0
                return (boost::wave::need_has_include(ctx.get_language()) && (name == "__has_include"));
    #else
                return false;
    #endif
            }
            ...
            inline bool macromap<ContextT>::is_defined(IteratorT const& begin, IteratorT const& end) const
            {
                ...
                if (T_IDENTIFIER != id && !IS_CATEGORY(id, KeywordTokenType) &&
                    ...
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, invalid_macroname, msg.c_str(), main_pos);
                    return false;
                }
            ...
            template<typename ContextT>
            template<typename IteratorT>
            inline bool macromap<ContextT>::has_include(
                ...
                bool is_system) const
            {
                ...
                return ctx.get_hooks().locate_include_file(ctx, fn, is_system, 0, dir_path, native_path);
            }
    #endif
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline bool macromap<ContextT>::get_macro(
                string_type const& name,
                bool& has_parameters,
                bool& is_predefined,
                position_type& pos,
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline bool macromap<ContextT>::remove_macro(
                string_type const& name,
                ...
                bool even_predefined)
            {
                ...
                if (it != current_macros->end()) {
                    ...
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, bad_undefine_statement, name.c_str(), pos);
                }
            ...
            template<typename ContextT>
            template<typename IteratorT, typename ContainerT>
            inline typename ContextT::token_type const& macromap<ContextT>::expand_tokensequence(
                IteratorT& first,
                IteratorT const& last,
                ContainerT& pending,
                ContainerT& expanded,
            ...
            template<typename ContextT>
            template<typename IteratorT, typename ContainerT>
            inline typename ContextT::token_type const& macromap<ContextT>::expand_tokensequence_worker(
                ContainerT& pending,
                unput_queue_iterator<IteratorT, token_type, ContainerT>& first,
                unput_queue_iterator<IteratorT, token_type, ContainerT> const& last,
                bool& seen_newline,
                ...
                if (first != last) {
                    ...
                    if (T_IDENTIFIER == id || IS_CATEGORY(id, KeywordTokenType) ||
                        ...
                        if (expand_operator_defined && (*first).get_value() == "defined") {
                            ...
                            boost::wave::need_has_include(ctx.get_language()) && expand_operator_has_include &&
                        ...
                        if (is_defined(name_token.get_value(), it)) {
                            ...
                            typename ContextT::token_type const& result = expand_tokensequence_worker(
                                pending,
                                first,
                                last,
                                seen_newline,
            ...
            template<typename ContextT>
            template<typename IteratorT, typename ContainerT, typename SizeT>
            inline typename std::vector<ContainerT>::size_type macromap<ContextT>::collect_arguments(
                token_type const curr_token,
                std::vector<ContainerT>& arguments,
                IteratorT& next,
                IteratorT& endparen,
                ...
                bool& seen_newline)
            {
                ...
                while (++next != end && nested_parenthesis_level) {
                    ...
                    if (0 == parameter_count && !IS_CATEGORY((*next), WhiteSpaceTokenType) && id != T_NEWLINE &&
                    ...
                    case T_PLACEHOLDER: break; // ignore placeholder
            ...
            template<typename ContextT>
            template<typename IteratorT, typename ContainerT>
            inline void macromap<ContextT>::expand_whole_tokensequence(
                ContainerT& expanded,
                IteratorT& first,
                IteratorT const& last,
                bool expand_operator_defined,
                bool expand_operator_has_include)
            {
                ...
                while (!pending_queue.empty() || first_it != last_it) {
                    expanded.push_back(expand_tokensequence_worker(
                        pending_queue,
                        first_it,
                        last_it,
                        seen_newline,
            ...
            template<typename ContextT>
            template<typename ContainerT>
            inline void macromap<ContextT>::expand_argument(
                typename std::vector<ContainerT>::size_type arg,
                std::vector<ContainerT>& arguments,
                std::vector<ContainerT>& expanded_args,
                bool expand_operator_defined,
            ...
            template<typename ContextT>
            template<typename ContainerT>
            inline void macromap<ContextT>::expand_replacement_list(
                ...
                ContainerT& expanded)
            {
                ...
                for (; cit != cend; ++cit) {
                    ...
                    if (T_POUND_POUND == base_id) {
                        ...
                        if (adjacent_stringize || adjacent_concat ||
                            T_POUND_POUND == impl::next_token<macro_definition_iter_t>::peek(cit, cend)) {
                    ...
                    if (IS_CATEGORY((*cit), ParameterTokenType)) {
                        ...
                        if (use_replaced_arg) {
                            ...
                            if (is_ellipsis) {
                                ...
                                for (typename vector<ContainerT>::size_type arg = i; arg < expanded_args.size();
                                     ++arg) {
                                    expand_argument(
                                        arg,
                                        arguments,
                        ...
                        else if (adjacent_stringize && !IS_CATEGORY(*cit, WhiteSpaceTokenType)) {
    ...
    #endif
                            {
                                ...
                                if (is_ellipsis && boost::wave::need_variadics(ctx.get_language())) {
                                    ...
                                    expanded.push_back(
                                        token_type(T_STRINGLIT, impl::as_stringlit(arguments, i, pos), pos));
                                }
                                else
    #endif
                                {
                ...
                if (adjacent_stringize) {
                    ...
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, ill_formed_operator, "stringize ('#')", main_pos);
                    return;
                }
            ...
            template<typename ContextT>
            template<typename IteratorT, typename ContainerT>
            inline void macromap<ContextT>::rescan_replacement_list(
                token_type const& curr_token,
                macro_definition_type& macro_def,
                ContainerT& replacement_list,
                ContainerT& expanded,
                bool expand_operator_defined,
                bool expand_operator_has_include,
            ...
            template<typename ContextT>
            template<typename IteratorT, typename ContainerT>
            inline bool macromap<ContextT>::expand_macro(
                ContainerT& expanded,
                token_type const& curr_token,
                typename defined_macros_type::iterator it,
                IteratorT& first,
                ...
                ContainerT* queue_symbol)
            {
                ...
                BOOST_ASSERT(
                    T_IDENTIFIER == token_id(curr_token) || IS_CATEGORY(token_id(curr_token), KeywordTokenType) ||
                ...
                if (!macro_def.is_available_for_replacement) {
                    ...
                    if (0 != queue_symbol) {
                        queue_symbol->push_back(
                            token_type(T_NONREPLACABLE_IDENTIFIER, curr_token.get_value(), curr_token.get_position()));
                        expanded.splice(expanded.end(), *queue_symbol);
                    }
                    else {
                        expanded.push_back(
                ...
                if (T_LEFTPAREN == impl::next_token<IteratorT>::peek(first, last)) {
                    ...
                    if (macro_def.is_functionlike) {
                        ...
                        std::vector<ContainerT> arguments;
                        typename std::vector<ContainerT>::size_type count_args = collect_arguments(
                            curr_token,
                            arguments,
                            first,
                            seqend,
                        ...
                        if (count_args < parm_count_required || arguments.size() < parm_count_required) {
                        ...
                        expand_replacement_list(
                            macro_def.macrodefinition.begin(),
            ...
            inline bool macromap<ContextT>::expand_predefined_macro(token_type const& curr_token, ContainerT& expanded)
            {
                ...
                if (ctx.get_hooks().expanding_object_like_macro(ctx.derived(), deftoken, ContainerT(), curr_token)) {
                    // do not expand this macro, just copy the whole sequence
                    expanded.push_back(curr_token);
                    return false; // no further preprocessing required
                }
                ...
                if (value == "__LINE__") {
                    ...
                    std::string file("\"");
                    fs::path filename(wave::util::create_path(curr_token.get_expand_position().get_file().c_str()));

                    using boost::wave::util::impl::escape_lit;
                    file += escape_lit(wave::util::native_file_string(filename)) + "\"";
                    replacement = token_type(T_STRINGLIT, file.c_str(), curr_token.get_position());
                }
            ...
            inline typename ContextT::token_type const&
                macromap<ContextT>::resolve_defined(IteratorT& first, IteratorT const& last, ContainerT& pending)
            {
                using namespace boost::wave;
                ...
                boost::spirit::classic::parse_info<IteratorT> hit =
                    defined_grammar_gen<typename ContextT::lexer_type>::parse_operator_defined(start, last, result);

                if (!hit.hit) {
                    string_type msg("defined(): ");
                    msg = msg + util::impl::as_string<string_type>(first, last);
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, ill_formed_expression, msg.c_str(), main_pos);
                    ...
                    pending.push_back(
                        token_type(T_INTLIT, is_defined(result.begin(), result.end()) ? "1" : "0", main_pos));
                }
            ...
            template<typename IteratorT, typename ContainerT>
            inline typename ContextT::token_type const&
                macromap<ContextT>::resolve_has_include(IteratorT& first, IteratorT const& last, ContainerT& pending)
            {
                using namespace boost::wave;
                using namespace boost::wave::grammars;
                ...
                boost::spirit::classic::parse_info<IteratorT> hit =
                    has_include_grammar_gen<typename ContextT::lexer_type>::parse_operator_has_include(
                        start,
                        last,
                        result,
                        is_quoted_filename,
                ...
                if (!hit.hit) {
                    ...
                    // insert a token, which reflects the outcome
                    pending.push_back(token_type(
                        T_INTLIT,
                        has_include(result.begin(), result.end(), is_quoted_filename, is_system) ? "1" : "0",
                        main_pos));
                }
            ...
            template<typename ContextT>
            template<typename IteratorT, typename ContainerT>
            inline bool macromap<ContextT>::resolve_operator_pragma(
                IteratorT& first,
                IteratorT const& last,
                ContainerT& pending,
                bool& seen_newline)
            {
                ...
                typename std::vector<ContainerT>::size_type count_args =
                    collect_arguments(pragma_token, arguments, first, endparen, last, 1, seen_newline);
                ...
                for (typename ContainerT::const_iterator it_exp = expanded.begin(); it_exp != end_exp; ++it_exp) {
                ...
                pending.push_front(
                    token_type(T_STRINGLIT, string_type("\"") + pragma_cmd + "\"", pragma_token.get_position()));
            ...
            template<typename ContextT>
            template<typename ContainerT>
            inline bool macromap<ContextT>::is_valid_concat(
                string_type new_value,
                position_type const& pos,
            ...
            void macromap<ContextT>::set_expand_positions(ContainerT& tokens, position_type pos)
            {
                ...
                for (typename ContainerT::iterator it = tokens.begin(); it != ex_end; ++it) {
            ...
            inline void report_invalid_concatenation(
                Context& ctx,
                ...
                typename Context::position_type const& main_pos)
            {
                ...
                error_string += "\"";
                BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, invalid_concat, error_string.c_str(), main_pos);
            }
            ...
            inline bool macromap<ContextT>::concat_tokensequence(ContainerT& expanded)
            {
                ...
                for (iterator_type it = expanded.begin(); it != end; /**/) {
                    if (T_POUND_POUND == BASE_TOKEN(token_id(*it))) {
                        ...
                        if (!is_valid_concat(concat_result, (*prev).get_position(), rescanned) &&
                            !IS_CATEGORY(*prev, WhiteSpaceTokenType) && !IS_CATEGORY(*next, WhiteSpaceTokenType)) {
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline void macromap<ContextT>::predefine_macro(
                defined_macros_type* scope,
                string_type const& name,
                token_type const& t)
            {
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline void macromap<ContextT>::init_predefined_macros(
                char const* fname,
                defined_macros_type* scope,
                bool at_global_scope)
            {
                ...
                if (string_type(fname) != "<Unknown>") {
                    ...
                    predefine_macro(
                        ...
                        token_type(
                            T_STRINGLIT,
                            string_type("\"") + escape_lit(native_file_string(filename)).c_str() + "\"",
                            pos));
                    base_name = fname;
                }
