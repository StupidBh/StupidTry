...
namespace boost {
    namespace wave {
        namespace util {
            ...
            inline bool retrieve_macroname(
                ContextT& ctx,
            ...
            inline bool retrieve_macrodefinition(
                ParseNodeT const& node,
                boost::spirit::classic::parser_id id,
                ...
                if (get_node_range(node, id, nodes)) {
                    ...
                    for (const_tree_iterator cit = nodes.first; cit != end; ++cit) {
                        if ((*cit).value.begin() != (*cit).value.end()) {
                            typename ContainerT::iterator inserted =
                                macrodefinition.insert(macrodefinition.end(), *(*cit).value.begin());
                    ...
                    if (last_nonwhite != macrodefinition.end()) {
                        if (update_position) {
                            act_pos.set_column(
                                (*last_nonwhite).get_position().get_column() + (*last_nonwhite).get_value().size());
                        }
                ...
                boost::wave::language_support language)
            {
                ...
                boost::spirit::classic::tree_parse_info<lexer_type> hit = predef_macros_type::parse_predefined_macro(
                    lexer_type(begin, end, position_type(), language),
                    lexer_type());

                if (!hit.match || (!hit.full && T_EOF != token_id(*hit.stop))) {
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, bad_macro_definition, macrostring.c_str(), act_pos);
                    return false;
                }
                ...
                if (!boost::wave::util::retrieve_macroname(
                        ctx,
                        *hit.trees.begin(),
                ...
                has_parameters = boost::wave::util::retrieve_macrodefinition(
                    *hit.trees.begin(),
                    BOOST_WAVE_MACRO_PARAMETERS_ID,
                    macroparameters,
                    act_pos,
                    true);
                boost::wave::util::retrieve_macrodefinition(
                    *hit.trees.begin(),
                    BOOST_WAVE_MACRO_DEFINITION_ID,
                    macrodefinition,
                    act_pos,
                    true);
                ...
                return ctx
                    .add_macro_definition(macroname, has_parameters, macroparameters, macrodefinition, is_predefined);
            }
        ...
        namespace impl {
            ...
            public:
                ...
                pp_iterator_functor(
                    ...
                    iter_ctx(new base_iteration_context_type(
                        ctx,
                        lexer_type(first_, last_, pos_, boost::wave::enable_prefer_pp_numbers(ctx.get_language())),
                        lexer_type(),
                        pos_.get_file().c_str())),
                    seen_newline(true),
            ...
            protected:
                ...
                bool dispatch_directive(
                    tree_parse_info_type const& hit,
            ...
            inline bool pp_iterator_functor<ContextT>::returned_from_include()
            {
                if (iter_ctx->first == iter_ctx->last && ctx.get_iteration_depth() > 0) {
                    ...
                    if (iter_ctx->if_block_depth != ctx.get_if_block_depth()) {
                        ...
                        BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, unbalanced_if_endif, msg.c_str(), old_pos);
                    }
                    return true;
                }
                return false;
            ...
            namespace impl {
                ...
                bool consider_emitting_line_directive(ContextT const& ctx, token_id id)
                {
                    ...
                    if (!IS_CATEGORY(id, WhiteSpaceTokenType) && !IS_CATEGORY(id, EOLTokenType) &&
                ...
                pp_iterator_functor<ContextT>::operator()()
            {
                ...
                if ((must_emit_line_directive || (was_seen_newline && skipped_newline)) &&
                    ...
                    if (need_emit_line_directives(ctx.get_language()) && emit_line_directive()) {
                        skipped_newline = false;
                        ctx.get_hooks().may_skip_whitespace(
                            ctx.derived(),
                            act_token,
                            skipped_newline); // feed ws eater FSM
                        id = token_id(act_token);
                    }
                }
                ...
                seen_newline = false;
                switch (id) {
                case T_NONREPLACABLE_IDENTIFIER: act_token.set_token_id(id = T_IDENTIFIER); break;
                ...
                case T_RAWSTRINGLIT:
                    iter_ctx->emitted_lines += context_policies::util::rawstring_count_newlines(act_token);
                    break;
    #endif

                case T_CCOMMENT: // will come here only if whitespace is preserved
                    iter_ctx->emitted_lines += context_policies::util::ccomment_count_newlines(act_token);
                    break;
                ...
                case T_PP_NUMBER: // re-tokenize the pp-number
                {
                    ...
                    lexer_type it =
                        lexer_type(pp_number.begin(), pp_number.end(), act_token.get_position(), ctx.get_language());
                ...
                case T_EOF: seen_newline = true; break;
                ...
                if (token_is_valid(act_token) && whitespace.must_insert(id, act_token.get_value())) {
                    ...
                    return act_token =
                               result_type(T_SPACE, typename result_type::string_type(" "), act_token.get_position());
                }
                ...
                pp_iterator_functor<ContextT>::get_next_token()
            {
                ...
                if (iter_ctx->first != iter_ctx->last) {
                    ...
                    if (ctx.get_if_block_depth() > 0 && !need_single_line(ctx.get_language())) {
                        ...
                        BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, missing_matching_endif, "", act_pos);
                    }
                }
                else {
            ...
            inline typename pp_iterator_functor<ContextT>::result_type const& pp_iterator_functor<ContextT>::pp_token()
            {
                ...
                do {
                    if (!pending_queue.empty()) {
                        ...
                        !unput_queue.empty() || T_IDENTIFIER == id || IS_CATEGORY(id, KeywordTokenType) ||
                        ...
                        act_token = ctx.expand_tokensequence(
                            iter_ctx->first,
                            iter_ctx->last,
                            pending_queue,
                            unput_queue,
            ...
            namespace impl {
                ...
                bool next_token_is_pp_directive(ContextT& ctx, IteratorT& it, IteratorT const& end)
                {
                    ...
                    for (/**/; it != end; ++it) {
                        ...
                        if (T_CPPCOMMENT == id || context_policies::util::ccomment_has_newline(*it)) {
                            break;
                        }
                ...
                inline void remove_leading_whitespace(ContextT& ctx, ContainerT& c, bool call_hook = true)
                {
            ...
            inline bool pp_iterator_functor<ContextT>::extract_identifier(IteratorT& it)
            {
                ...
                if (T_IDENTIFIER == id || IS_CATEGORY(id, KeywordTokenType) ||
                    IS_EXTCATEGORY(id, OperatorTokenType | AltExtTokenType) || IS_CATEGORY(id, BoolLiteralTokenType)) {
                    IteratorT save = it;
            ...
            template<typename ContextT>
            template<typename IteratorT>
            inline bool pp_iterator_functor<ContextT>::ensure_is_last_on_line(IteratorT& it, bool call_hook)
            {
                ...
                if (it == iter_ctx->last && !need_single_line(ctx.get_language())) {
                    ...
                    if (!need_no_newline_at_end_of_file(ctx.get_language())) {
                        ...
                        // newline.
                        BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, last_line_not_terminated, "", act_pos);
                    }
            ...
            template<typename ContextT>
            template<typename IteratorT>
            inline bool pp_iterator_functor<ContextT>::skip_to_eol_with_check(IteratorT& it, bool call_hook)
            {
                typename ContextT::string_type value((*it).get_value());
                if (!impl::skip_to_eol(ctx, it, iter_ctx->last, call_hook) && !need_single_line(ctx.get_language())) {
                    // The line doesn't end with an eol but eof token.
                    seen_newline = true; // allow to resume after warning
                    iter_ctx->first = it;
            ...
            inline bool pp_iterator_functor<ContextT>::handle_pp_directive(IteratorT& it)
            {
                ...
                if (!ctx.get_if_block_status()) {
                    ...
                    case T_PP_UNDEF: // #undef
                        if (!impl::call_found_directive_hook(ctx, *it) && extract_identifier(it)) {
                            on_undefine(it);
                        }
    ...
    //             break;
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline bool pp_iterator_functor<ContextT>::dispatch_directive(
                tree_parse_info_type const& hit,
                result_type const& found_directive,
                token_sequence_type const& found_eoltokens)
            {
                ...
                case T_MSEXT_PP_ENDREGION: break; // ignore these
    #endif
                ...
                default: // #something else
                    ...
                    // we simply copy the whole construct to the output
                    {
                        ...
                        std::copy(
                            make_ref_transform_iterator(begin_child_it, get_value),
            ...
            template<typename ContextT>
            inline void
                pp_iterator_functor<ContextT>::on_include(string_type const& s, bool is_system, bool include_next)
            {
                ...
                if (string_type::npos == pos_end) {
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, bad_include_statement, s.c_str(), act_pos);
                    return;
                }
            ...
            template<typename ContextT>
            inline bool pp_iterator_functor<ContextT>::on_include_helper(
                char const* f,
                char const* s,
                bool is_system,
                bool include_next)
            {
                ...
                if (!ctx.get_hooks()
                         .locate_include_file(ctx, file_path, is_system, current_name, dir_path, native_path_str)) {
    ...
    // test, if this file is known through a #pragma once directive
    ...
    #endif
                {
                    ...
                    boost::shared_ptr<base_iteration_context_type> new_iter_ctx(new iteration_context_type(
                        ctx,
                        native_path_str.c_str(),
                        act_pos,
            ...
            template<typename ContextT>
            inline void pp_iterator_functor<ContextT>::on_include(
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end,
                bool include_next)
            {
                ...
                ctx.expand_whole_tokensequence(begin2, toexpand.end(), expanded, false, false);
            ...
            inline void pp_iterator_functor<ContextT>::on_define(parse_node_type const& node)
            {
                ...
                if (has_parameters) {
                    ...
                    if (boost::wave::need_variadics(ctx.get_language())) {
                        ...
                        for (parameter_iterator_t pit = macroparameters.begin(); pit != end; ++pit) {
                            if (seen_ellipses) {
                            ...
                            if (boost::wave::need_va_opt(ctx.get_language()) && ("__VA_OPT__" == (*pit).get_value())) {
                                BOOST_WAVE_THROW_CTX(
                                    ctx,
                                    preprocess_exception,
                        ...
                        if (!seen_ellipses) {
                            ...
                            for (definition_iterator_t dit = macrodefinition.begin(); dit != pend; ++dit) {
                                if (T_IDENTIFIER == token_id(*dit) && "__VA_ARGS__" == (*dit).get_value()) {
                                    seen_va_args = true;
                                }
        #if BOOST_WAVE_SUPPORT_VA_OPT != 0
                                if (T_IDENTIFIER == token_id(*dit) && "__VA_OPT__" == (*dit).get_value()) {
                                    seen_va_opt = true;
                                }
        #endif
                            }
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline void pp_iterator_functor<ContextT>::on_ifdef(
                result_type const& found_directive,
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end)
            {
                ...
                do {
                    is_defined = ctx.is_defined_macro(toexpand.begin(), toexpand.end());
                             ...
                             .evaluated_conditional_expression(ctx.derived(), found_directive, toexpand, is_defined));
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline void pp_iterator_functor<ContextT>::on_ifndef(
                result_type const& found_directive,
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end)
            {
            ...
            inline void pp_iterator_functor<ContextT>::on_else()
            {
                if (!ctx.enter_else_block()) {
                    // #else without matching #if
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, missing_matching_if, "#else", act_pos);
                }
            }
            ...
            inline void pp_iterator_functor<ContextT>::on_endif()
            {
                if (!ctx.exit_if_block()) {
                    ...
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, missing_matching_if, "#endif", act_pos);
                }
            }
            ...
            inline void pp_iterator_functor<ContextT>::replace_undefined_identifiers(token_sequence_type& expanded)
            {
                ...
                for (typename token_sequence_type::iterator exp_it = expanded.begin(); exp_it != exp_end; ++exp_it) {
                    using namespace boost::wave;

                    token_id id = token_id(*exp_it);
                    if (IS_CATEGORY(id, IdentifierTokenType) || IS_CATEGORY(id, KeywordTokenType)) {
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline void pp_iterator_functor<ContextT>::on_if(
                result_type const& found_directive,
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end)
            {
                ...
                do {
    ...
    #if BOOST_WAVE_DUMP_CONDITIONAL_EXPRESSIONS != 0
                    {
                        ...
                        BOOST_WAVE_DUMP_CONDITIONAL_EXPRESSIONS_OUT << "#if " << outstr << std::endl;
                    }
    #endif
                    try {
                        // parse the expression and enter the #if block
                        if_status = grammars::expression_grammar_gen<result_type>::evaluate(
                            expanded.begin(),
                            expanded.end(),
                            act_pos,
                        ...
                        break;
                             ...
                             .evaluated_conditional_expression(ctx.derived(), found_directive, toexpand, if_status) &&
                ...
                if (grammars::error_noerror != status) {
                    ...
                    if (grammars::error_division_by_zero & status) {
                        BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, division_by_zero, expression.c_str(), act_pos);
                    }
                    else if (grammars::error_integer_overflow & status) {
                        BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, integer_overflow, expression.c_str(), act_pos);
                    }
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline void pp_iterator_functor<ContextT>::on_elif(
                result_type const& found_directive,
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end)
            {
                ...
                if (ctx.get_if_block_some_part_status()) {
                    if (!ctx.enter_elif_block(false)) {
                        // #else without matching #if
                        BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, missing_matching_if, "#elif", act_pos);
                        // fall through...
                    }
            ...
            inline void pp_iterator_functor<ContextT>::on_illformed(typename result_type::string_type s)
            {
                ...
                // throw the exception
                BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, ill_formed_directive, s.c_str(), act_pos);
            }
            ...
            template<typename ContextT>
            inline void pp_iterator_functor<ContextT>::on_line(
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end)
            {
                BOOST_ASSERT(ctx.get_if_block_status());
                ...
                typedef typename ref_transform_iterator_generator<
                    ...
                    typename parse_tree_type::const_iterator>::type const_tree_iterator_t;
                ...
                if (!impl::retrieve_line_info(first, last, line, file_name, error)) {
                    ...
                    if (!impl::retrieve_line_info(expanded.begin(), expanded.end(), line, file_name, error)) {
                        typename ContextT::string_type msg(boost::wave::util::impl::as_string(expanded));
                        BOOST_WAVE_THROW_VAR_CTX(ctx, preprocess_exception, error, msg.c_str(), act_pos);
                        return;
                    }
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline void pp_iterator_functor<ContextT>::on_error(
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end)
            {
                BOOST_ASSERT(ctx.get_if_block_status());
                ...
                typename ref_transform_iterator_generator<
                    get_token_value<result_type, parse_node_type>,
                    typename parse_tree_type::const_iterator>::type first =
                    make_ref_transform_iterator(begin, get_value);
                ...
                // message
                std::copy(first, make_ref_transform_iterator(end, get_value), std::inserter(expanded, expanded.end()));
    ...
    #endif
                {
                    ...
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, error_directive, msg.c_str(), act_pos);
                }
            }
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline void pp_iterator_functor<ContextT>::on_warning(
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end)
            {
                BOOST_ASSERT(ctx.get_if_block_status());
        ...
        #endif
                {
                    // report the corresponding error
                    BOOST_WAVE_STRINGTYPE msg(boost::wave::util::impl::as_string(expanded));
                    BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, warning_directive, msg.c_str(), act_pos);
                }
            }
            ...
            ///////////////////////////////////////////////////////////////////////////////
            template<typename ContextT>
            inline bool pp_iterator_functor<ContextT>::on_pragma(
                typename parse_tree_type::const_iterator const& begin,
                typename parse_tree_type::const_iterator const& end)
            {
                using namespace boost::wave;
                ...
                if (first != last) {
                    if (T_IDENTIFIER == token_id(*first) && boost::wave::need_c99(ctx.get_language()) &&
            ...
            template<typename ContextT>
            inline bool pp_iterator_functor<ContextT>::interpret_pragma(
                token_sequence_type const& pragma_body,
        ...
        template<typename ContextT>
        class pp_iterator :
            public boost::spirit::classic::
                multi_pass<boost::wave::impl::pp_iterator_functor<ContextT>, boost::wave::util::functor_input> {
        public:
