    ...
    namespace wave {
        namespace util {
            ...
            public:
                ...
                void set_max_include_nesting_depth(size_type new_depth) { max_include_nesting_depth = new_depth; }

                size_type get_max_include_nesting_depth() const { return max_include_nesting_depth; }
                ...
                void push(Context& ctx, PositionT const& pos, typename base_type::value_type const& val)
                {
                    if (iter_ctx.size() == max_include_nesting_depth) {
                        ...
                        BOOST_WAVE_THROW_CTX(ctx, preprocess_exception, include_nesting_too_deep, buffer.c_str(), pos);
                    }
                    iter_ctx.push(val);
                }
