...
namespace boost {
    namespace wave {
        namespace util {
            ...
            public:
                ...
                bool add_include_path(char const* path_, bool is_system = false)
                {
                    return add_include_path(
                        path_,
                        (is_system || was_sys_include_path) ? system_include_paths : user_include_paths);
                }

                void set_sys_include_delimiter() { was_sys_include_path = true; }

                bool
                    find_include_file(std::string& s, std::string& dir, bool is_system, char const* current_file) const;
                void set_current_directory(char const* path_);

                boost::filesystem::path get_current_directory() const { return current_dir; }

            protected:
                bool find_include_file(std::string& s, std::string& dir, include_list_type const& pathes, char const*)
                    const;
                ...
                void load(Archive& ar, const unsigned int loaded_version)
                {
                    ...
                    if (version != (loaded_version & ~version_mask)) {
                        BOOST_WAVE_THROW(
                            ...
                            incompatible_config,
                            "cpp_include_path state version",
                            load_filepos());
                        return;
                    }
            ...
            inline bool include_paths::find_include_file(
                std::string& s,
                std::string& dir,
...
namespace boost {
    namespace serialization {
        ...
        inline void save(Archive& ar, boost::filesystem::path const& p, const unsigned int /* file_version */)
        {
            using namespace boost::serialization;
        ...
        template<class Archive>
        inline void load(Archive& ar, boost::filesystem::path& p, const unsigned int /* file_version */)
        {
            using namespace boost::serialization;
            std::string path_str;
            ar& make_nvp("filepath", path_str);
        ...
        template<class Archive>
        inline void serialize(Archive& ar, boost::filesystem::path& p, const unsigned int file_version)
        {
        ...
        inline void save(
            Archive& ar,
            const typename boost::wave::util::bidirectional_map<std::string, std::string>::type& t,
            const unsigned int /* file_version */)
        {
            boost::serialization::stl::save_collection<
                Archive,
                typename boost::wave::util::bidirectional_map<std::string, std::string>::type>(ar, t);
        }
        ...
        inline void load(
            ...
            const unsigned int /* file_version */)
        {
            typedef typename boost::wave::util::bidirectional_map<std::string, std::string>::type map_type;
            boost::serialization::stl::load_collection<
                Archive,
                map_type,
                boost::serialization::stl::archive_input_unique<Archive, map_type>,
                boost::serialization::stl::no_reserve_imp<map_type>>(ar, t);
        }
...
BOOST_CLASS_VERSION(boost::wave::util::include_paths, boost::wave::util::include_paths::version);
