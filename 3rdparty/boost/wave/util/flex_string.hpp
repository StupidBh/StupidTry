...
namespace boost {
    ...
    // gcc 11.2 fails to deduce below that pData_ never points to emptyString_ if capacity() == 0 and emits warnings
    // like these: 'operator delete(void*, unsigned long)' called on unallocated object
    // 'boost::wave::util::SimpleStringStorage<char, std::allocator<char> >::emptyString_' [-Wfree-nonheap-object]
    // Unfortunately, suppressing this warning doesn't work, so we have to use __builtin_unreachable() to assert that
    // this never happens.
    ...
    #define BOOST_WAVE_COMPILE_TIME_ASSERT(x)
            ...
            public:
                ...
                size_type max_size() const { return std::size_t(-1) / sizeof(E) - sizeof(Data) - 1; }
            ...
            template<typename E, class A>
            const typename SimpleStringStorage<E, A>::Data SimpleStringStorage<E, A>::emptyString_ =
            ...
            class AllocatorStringStorage : public A {
                ...
                {
                    return boost::allocator_allocate(
                        static_cast<A&>(*this),
                        1 + (sz - 1) / sizeof(E),
            ...
            public:
                ...
                size_type max_size() const { return boost::allocator_max_size(static_cast<const A&>(*this)); }

                size_type capacity() const { return size_type(pData_->pEndOfMem_ - pData_->buffer_); }
            ...
            private:
                enum
                {
                    temp1 = threshold * sizeof(value_type) > sizeof(Storage) ? threshold * sizeof(value_type)
                                                                             : sizeof(Storage)
                };
            ...
            public:
                ...
                SmallStringOpt(const allocator_type&) { buf_[maxSmallString] = maxSmallString; }
                ...
                size_type max_size() const { return boost::allocator_max_size(get_allocator()); }

                size_type capacity() const { return Small() ? maxSmallString : GetStorage().capacity(); }
                ...
                const value_type* data() const { return Small() ? buf_ : GetStorage().data(); }
            ...
            public:
                ...
                CowString(const allocator_type& a) { new (buf_) Storage(1, 1, a); }
                ...
                void swap(CowString& rhs) { Data().swap(rhs.Data()); }
            ...
            class flex_string : private Storage {
                ...
                {
                    if (!condition) {
                        boost::throw_exception(Exception(msg));
                    }
                }
#else
                template<typename Exception>
                ...
                struct Invariant
                {
                    Invariant(const flex_string& s) :
                        s_(s)
                    {
                        BOOST_ASSERT(s_.Sane());
                    }

                    ~Invariant() { BOOST_ASSERT(s_.Sane()); }
            ...
            private:
                static size_type Min(size_type lhs, size_type rhs) { return lhs < rhs ? lhs : rhs; }
            ...
            public:
                ...
                flex_string& append(const flex_string& str) { return append(str, 0, npos); }
                ...
                flex_string& append(const value_type* s) { return append(s, traits_type::length(s)); }
                ...
                flex_string& assign(const value_type* s) { return assign(s, traits_type::length(s)); }
            ...
            private:
                ...
                bool IsAliasedRange(Iterator beginIterator, Iterator endIterator)
                {
                    if (!empty() && beginIterator != endIterator) {
                        typedef const typename std::iterator_traits<Iterator>::value_type* value_pointer;
            ...
            public:
                ...
                std::ptrdiff_t
                    compare(size_type pos1, size_type n1, const flex_string& str, size_type pos2, size_type n2) const
                {
            ...
            template<typename E, class T, class A, class S>
            flex_string<E, T, A, S>
                operator+(const typename flex_string<E, T, A, S>::value_type* lhs, const flex_string<E, T, A, S>& rhs)
            {
                flex_string<E, T, A, S> result;
            ...
            template<typename E, class T, class A, class S>
            flex_string<E, T, A, S>
                operator+(typename flex_string<E, T, A, S>::value_type lhs, const flex_string<E, T, A, S>& rhs)
            {
                flex_string<E, T, A, S> result;
            ...
            template<typename E, class T, class A, class S>
            flex_string<E, T, A, S>
                operator+(const flex_string<E, T, A, S>& lhs, const typename flex_string<E, T, A, S>::value_type* rhs)
            {
            ...
            template<typename E, class T, class A, class S>
            flex_string<E, T, A, S>
                operator+(const flex_string<E, T, A, S>& lhs, typename flex_string<E, T, A, S>::value_type rhs)
            {
                flex_string<E, T, A, S> result;
            ...
            inline std::basic_istream<
                typename flex_string<E, T, A, S>::value_type,
            ...
            const typename flex_string<E1, T, A, S>::size_type flex_string<E1, T, A, S>::npos =
                (typename flex_string<E1, T, A, S>::size_type)(-1);
...
namespace boost {
    namespace serialization {
        ...
        inline void
            save(Archive& ar, boost::wave::util::flex_string<E, T, A, S> const& t, const unsigned int file_version)
        {
            boost::serialization::stl::save_collection<Archive, wave::util::flex_string<E, T, A, S>>(ar, t);
        }

        template<class Archive, typename E, class T, class A, class S>
        inline void load(Archive& ar, boost::wave::util::flex_string<E, T, A, S>& t, const unsigned int file_version)
        {
            boost::serialization::stl::load_collection<
                Archive,
                boost::wave::util::flex_string<E, T, A, S>,
                boost::serialization::stl::archive_input_seq<Archive, boost::wave::util::flex_string<E, T, A, S>>,
                boost::serialization::stl::reserve_imp<boost::wave::util::flex_string<E, T, A, S>>>(ar, t);
        }
        ...
        template<class Archive, typename E, class T, class A, class S>
        inline void
            serialize(Archive& ar, boost::wave::util::flex_string<E, T, A, S>& t, const unsigned int file_version)
        {
