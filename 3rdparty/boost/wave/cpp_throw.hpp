    ...
    namespace wave {
        namespace util {
            ...
            void throw_(typename Exception::error_code code, S1 msg, Pos const& pos)
            {
                ...
                stream << Exception::severity_text(code) << ": " << Exception::error_text(code);
                if (msg[0] != 0) {
                    stream << ": " << msg;
                }
                stream << std::ends;
                ...
                boost::throw_exception(
                    Exception(throwmsg.c_str(), code, pos.get_line(), pos.get_column(), pos.get_file().c_str()));
            }
            ...
            void throw_(typename Exception::error_code code, S1 msg, Pos const& pos, S2 name)
            {
                ...
                stream.freeze(false);
                boost::throw_exception(
                    Exception(throwmsg.c_str(), code, pos.get_line(), pos.get_column(), pos.get_file().c_str(), name));
            }

            template<typename Exception, typename Context, typename S1, typename Pos, typename S2>
            void throw_(Context& ctx, typename Exception::error_code code, S1 msg, Pos const& pos, S2 name)
            {
                std::strstream stream;
                stream << Exception::severity_text(code) << ": " << Exception::error_text(code);
                if (msg[0] != 0) {
