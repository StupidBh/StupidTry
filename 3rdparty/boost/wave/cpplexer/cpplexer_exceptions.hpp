    ...
    // helper macro for throwing exceptions
            ...
            #define BOOST_WAVE_LEXER_THROW(cls, code, msg, line, column, name)                     \
                {                                                                                  \
                    ...
                    stream << cls::severity_text(cls::code) << ": " << cls::error_text(cls::code); \
                    if ((msg)[0] != 0)                                                             \
                        stream << ": " << (msg);                                                   \
                    stream << std::ends;                                                           \
                    std::string throwmsg = stream.str();                                           \
                    stream.freeze(false);                                                          \
                    boost::throw_exception(cls(throwmsg.c_str(), cls::code, line, column, name));  \
                }                                                                                  \
                /**/
        #else
            #include <sstream>
            #define BOOST_WAVE_LEXER_THROW(cls, code, msg, line, column, name)                        \
                {                                                                                     \
                    ...
                    if ((msg)[0] != 0)                                                                \
                        stream << ": " << (msg);                                                      \
                    stream << std::ends;                                                              \
                    boost::throw_exception(cls(stream.str().c_str(), cls::code, line, column, name)); \
                }                                                                                     \
                /**/
            ...
            #define BOOST_WAVE_LEXER_THROW_VAR(cls, codearg, msg, line, column, name)        \
                {                                                                            \
                    ...
                    std::string throwmsg = stream.str();                                     \
                    stream.freeze(false);                                                    \
                    boost::throw_exception(cls(throwmsg.c_str(), code, line, column, name)); \
                }                                                                            \
                /**/
        #else
            #include <sstream>
            #define BOOST_WAVE_LEXER_THROW_VAR(cls, codearg, msg, line, column, name)            \
                {                                                                                \
                    ...
                    if ((msg)[0] != 0)                                                           \
                        stream << ": " << (msg);                                                 \
                    stream << std::ends;                                                         \
                    boost::throw_exception(cls(stream.str().c_str(), code, line, column, name)); \
                }                                                                                \
                /**/
        #endif // BOOST_NO_STRINGSTREAM
    #endif     // BOOST_WAVE_LEXER_THROW
...
namespace boost {
    namespace wave {
        namespace cpplexer {
            ...
            class BOOST_SYMBOL_VISIBLE cpplexer_exception : public std::exception {
            public:
            ...
            public:
                ...
                char const* what() const throw() BOOST_OVERRIDE { return "boost::wave::lexing_exception"; }

                char const* description() const throw() BOOST_OVERRIDE { return buffer; }

                int get_severity() const throw() BOOST_OVERRIDE { return level; }
