...
namespace boost {
    namespace wave {
        namespace cpplexer {
            ...
            public:
                include_guards() :
                    state(&include_guards::state_0),
                    detected_guards(false),
                    current_state(true),
                    if_depth(0)
                {
                }

                Token& detect_guard(Token& t) { return current_state ? (this->*state)(t) : t; }
            ...
            private:
                ...
                {
                    return (
                        T_POUND == BASE_TOKEN(id) || IS_CATEGORY(id, WhiteSpaceTokenType) ||
