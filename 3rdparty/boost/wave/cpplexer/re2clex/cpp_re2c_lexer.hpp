...
namespace boost {
    namespace wave {
            ...
            namespace re2clex {
    ...
    #endif
                {
                    ...
                    scanner.act_in_cpp0x_mode =
                        boost::wave::need_cpp2a(language_) || boost::wave::need_cpp0x(language_);
    #else
                ...
                inline TokenT& lexer<IteratorT, PositionT, TokenT>::get(TokenT& result)
                {
                    ...
                    case T_SPACE2:
                    case T_ANY:
                    case T_PP_NUMBER    : value = string_type((char const*)scanner.tok, scanner.cur - scanner.tok); break;
                    ...
                    case T_ANY_TRIGRAPH:
                        if (boost::wave::need_convert_trigraphs(language)) {
                            value = impl::convert_trigraph(
                                string_type((char const*)scanner.tok, scanner.cur - scanner.tok));
                        }
                        else {
                            value = string_type((char const*)scanner.tok, scanner.cur - scanner.tok);
                        }
                ...
                template<typename IteratorT, typename PositionT, typename TokenT>
                inline int lexer<IteratorT, PositionT, TokenT>::report_error(
                    Scanner<IteratorT> const* s,
                    int errcode,
                    char const* msg,
                    ...)
                {
                    ...
                    BOOST_WAVE_LEXER_THROW_VAR(lexing_exception, errcode, buffer, s->line, s->column, s->file_name);
        ...
        #define BOOST_WAVE_RE2C_NEW_LEXER_INLINE inline
            ...
            template<typename IteratorT, typename PositionT, typename TokenT>
            BOOST_WAVE_RE2C_NEW_LEXER_INLINE lex_input_interface<TokenT>*
