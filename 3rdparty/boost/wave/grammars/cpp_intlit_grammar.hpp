...
namespace boost {
    namespace wave {
        namespace grammars {
            ...
            namespace closures {
                ...
                struct intlit_closure : boost::spirit::classic::closure<intlit_closure, uint_literal_type>
                {
                    member1 val;
                };
            }
    ...
    #define TRACE_INTLIT_GRAMMAR bool(BOOST_SPIRIT_DEBUG_FLAGS_CPP & BOOST_SPIRIT_DEBUG_FLAGS_INTLIT_GRAMMAR) /**/

            struct intlit_grammar : boost::spirit::classic::grammar<intlit_grammar, closures::intlit_closure::context_t>
            {
                intlit_grammar(bool& is_unsigned_) :
                    is_unsigned(is_unsigned_)
                {
                    BOOST_SPIRIT_DEBUG_TRACE_GRAMMAR_NAME(*this, "intlit_grammar", TRACE_INTLIT_GRAMMAR);
                }
                ...
                struct definition
                {
                    ...
                    definition(intlit_grammar const& self)
                    {
                        ...
                        int_lit =
                            (sub_int_lit = (ch_p('0')[self.val = 0] >> (hex_lit | oct_lit) | dec_lit) >>
                                           !as_lower_d
                                               [(ch_p('u')[phx::var(self.is_unsigned) = true] || ch_p('l')) |
                                                (ch_p('l') || ch_p('u')[phx::var(self.is_unsigned) = true])],

                             hex_lit = (ch_p('X') | ch_p('x')) >> uint_parser<uint_literal_type, 16>()[(
                                                                      self.val = phx::arg1,
                                                                      phx::var(self.is_unsigned) = true)],

                             oct_lit = !uint_parser<uint_literal_type, 8>()
                                           [(self.val = phx::arg1, phx::var(self.is_unsigned) = true)],

                             dec_lit = uint_parser<uint_literal_type, 10>()[self.val = phx::arg1]);
                    ...
                    rule_t const& start() const { return int_lit; }
                };
    ...
    #undef TRACE_INTLIT_GRAMMAR
        ...
        #define BOOST_WAVE_INTLITGRAMMAR_GEN_INLINE inline
            ...
            BOOST_WAVE_INTLITGRAMMAR_GEN_INLINE
                uint_literal_type intlit_grammar_gen<TokenT>::evaluate(TokenT const& token, bool& is_unsigned)
            {
