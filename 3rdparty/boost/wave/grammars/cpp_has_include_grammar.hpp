...
namespace boost {
    ...
    #define TRACE_CPP_HAS_INCLUDE_GRAMMAR \
        bool(BOOST_SPIRIT_DEBUG_FLAGS_CPP & BOOST_SPIRIT_DEBUG_FLAGS_HAS_INCLUDE_GRAMMAR) /**/
            ...
            struct has_include_grammar : public boost::spirit::classic::grammar<has_include_grammar<ContainerT>>
            {
                has_include_grammar(ContainerT& tokens_seq_, bool& is_quoted_filename_, bool& is_system_) :
                    tokens_seq(tokens_seq_),
                    is_quoted_filename(is_quoted_filename_),
                    is_system(is_system_),
                    true_(true)
                {
                    BOOST_SPIRIT_DEBUG_TRACE_GRAMMAR_NAME(*this, "has_include_grammar", TRACE_CPP_HAS_INCLUDE_GRAMMAR);
                ...
                struct definition
                {
                    ...
                    definition(has_include_grammar const& self)
                    {
                        ...
                        system_include = ch_p(T_LESS)[spirit_append_actor(self.tokens_seq)] >>
                                         *(~ch_p(T_GREATER))[spirit_append_actor(self.tokens_seq)] >>
                                         ch_p(T_GREATER)[spirit_append_actor(self.tokens_seq)]
                                                        [assign_a(self.is_quoted_filename, self.true_)]
                                                        [assign_a(self.is_system, self.true_)];

                        nonsystem_include = ch_p(T_STRINGLIT)[spirit_append_actor(self.tokens_seq)]
                                                             [assign_a(self.is_quoted_filename, self.true_)];

                        // an action to store a sequence of parsed tokens
                        auto append_seq = [&](typename ScannerT::iterator_t first, typename ScannerT::iterator_t last) {
                        ...
                        parenthesized_exp = ch_p(T_LEFTPAREN)[spirit_append_actor(self.tokens_seq)] >>
                                            computed_include >>
                    ...
                    rule_t const& start() const { return has_include_op; }
                };
    ...
    #undef TRACE_CPP_HAS_INCLUDE_GRAMMAR
        ...
        #define BOOST_WAVE_HAS_INCLUDE_GRAMMAR_GEN_INLINE inline
            ...
            BOOST_WAVE_HAS_INCLUDE_GRAMMAR_GEN_INLINE
                boost::spirit::classic::parse_info<typename has_include_grammar_gen<LexIteratorT>::iterator1_type>
                has_include_grammar_gen<LexIteratorT>::parse_operator_has_include(
                    iterator1_type const& first,
                    ...
                    bool& is_system)
            {
                ...
                has_include_grammar<token_sequence_type> g(tokens, is_quoted_filename, is_system);
                return boost::spirit::classic::parse(first, last, g, ch_p(T_SPACE) | ch_p(T_CCOMMENT));
            }

            template<typename LexIteratorT>
            BOOST_WAVE_HAS_INCLUDE_GRAMMAR_GEN_INLINE
                boost::spirit::classic::parse_info<typename has_include_grammar_gen<LexIteratorT>::iterator2_type>
                has_include_grammar_gen<LexIteratorT>::parse_operator_has_include(
                    iterator2_type const& first,
                    ...
                    bool& is_system)
            {
                ...
                has_include_grammar<token_sequence_type> g(found_qualified_name, is_quoted_filename, is_system);
                return boost::spirit::classic::parse(first, last, g, ch_p(T_SPACE) | ch_p(T_CCOMMENT));
            }
