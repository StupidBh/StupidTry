...
namespace boost {
    namespace wave {
        namespace grammars {
            ...
            namespace closures {
                ...
                struct chlit_closure : boost::spirit::classic::closure<chlit_closure, boost::uint32_t, bool>
                {
                    member1 value;
                    member2 long_lit;
                };
            ...
            namespace impl {
                ...
                struct compose_character_literal
                {
                    ...
                    void operator()(boost::uint32_t& value, bool long_lit, bool& overflow, boost::uint32_t character)
                        const
                    {
                        // The following assumes that wchar_t is max. 32 Bit
                        BOOST_STATIC_ASSERT(sizeof(wchar_t) <= 4);

                        static boost::uint32_t masks[] = { 0x000000ff, 0x0000ffff, 0x00ffffff, 0xffffffff };
                        static boost::uint32_t overflow_masks[] = { 0xff000000, 0xffff0000, 0xffffff00, 0xffffffff };
    ...
    #define TRACE_CHLIT_GRAMMAR bool(BOOST_SPIRIT_DEBUG_FLAGS_CPP & BOOST_SPIRIT_DEBUG_FLAGS_CHLIT_GRAMMAR) /**/

            struct chlit_grammar :
                public boost::spirit::classic::grammar<chlit_grammar, closures::chlit_closure::context_t>
            {
                chlit_grammar() :
                    overflow(false)
                {
                    BOOST_SPIRIT_DEBUG_TRACE_GRAMMAR_NAME(*this, "chlit_grammar", TRACE_CHLIT_GRAMMAR);
                }
                ...
                struct definition
                {
                    typedef boost::spirit::classic::rule<ScannerT, closures::chlit_closure::context_t> rule_t;
                    ...
                    definition(chlit_grammar const& self)
                    {
                        ...
                        typedef uint_parser<unsigned int, 16, 1, 2 * sizeof(char)> hex_char_parser_type;
                        typedef uint_parser<unsigned int, 16, 1, 2 * sizeof(wchar_t)> hex_wchar_parser_type;

                        // the rule for a character literal
                        ch_lit =
                            eps_p[(self.value = phx::val(0), self.long_lit = phx::val(false))] >>
                            !ch_p('L')[self.long_lit = phx::val(true)] >> ch_p('\'') >>
                            +((ch_p('\\') >>
                               (ch_p('a') // BEL
                                    [impl::
                                         compose(self.value, self.long_lit, phx::var(self.overflow), phx::val(0x07))] |
                                ch_p('b') // BS
                                    [impl::
                                         compose(self.value, self.long_lit, phx::var(self.overflow), phx::val(0x08))] |
                                ch_p('t') // HT
                                    [impl::
                                         compose(self.value, self.long_lit, phx::var(self.overflow), phx::val(0x09))] |
                                ch_p('n') // NL
                                    [impl::
                                         compose(self.value, self.long_lit, phx::var(self.overflow), phx::val(0x0a))] |
                                ch_p('v') // VT
                                    [impl::
                                         compose(self.value, self.long_lit, phx::var(self.overflow), phx::val(0x0b))] |
                                (ch_p('e') | ch_p('E')) // ESC
                                    [impl::
                                         compose(self.value, self.long_lit, phx::var(self.overflow), phx::val(0x1b))] |
                                ch_p('f') // FF
                                    [impl::
                                         compose(self.value, self.long_lit, phx::var(self.overflow), phx::val(0x0c))] |
                                ch_p('r') // CR
                                    [impl::
                                         compose(self.value, self.long_lit, phx::var(self.overflow), phx::val(0x0d))] |
                                ch_p('?')
                                    [impl::compose(self.value, self.long_lit, phx::var(self.overflow), phx::val('?'))] |
                                ch_p('\'')[impl::compose(
                                    self.value,
                                    self.long_lit,
                                    phx::var(self.overflow),
                                    phx::val('\''))] |
                                ch_p('\"')[impl::compose(
                                    self.value,
                                    self.long_lit,
                                ...
                                ch_p('\\')[impl::compose(
                                    ...
                                    phx::var(self.overflow),
                                    phx::val('\\'))] |
                                ch_p('x') >> if_p(self.long_lit)[hex_wchar_parser_type()[impl::compose(
                                                                     self.value,
                                                                     self.long_lit,
                                                                     phx::var(self.overflow),
                                                                     phx::arg1)]]
                                                 .else_p[hex_char_parser_type()[impl::compose(
                                                     self.value,
                                                     self.long_lit,
                                                     phx::var(self.overflow),
                                                     phx::arg1)]] |
                                ch_p('u') >>
                                    uint_parser<unsigned int, 16, 4, 4>()
                                        [impl::compose(self.value, self.long_lit, phx::var(self.overflow), phx::arg1)] |
                                ch_p('U') >>
                    ...
                    rule_t const& start() const { return ch_lit; }
                };
    ...
    #undef TRACE_CHLIT_GRAMMAR
        ...
        #define BOOST_WAVE_CHLITGRAMMAR_GEN_INLINE inline
            ...
            BOOST_WAVE_CHLITGRAMMAR_GEN_INLINE IntegralResult
                chlit_grammar_gen<IntegralResult, TokenT>::evaluate(TokenT const& token, value_error& status)
            {
                ...
                if (!hit.hit) {
                    ...
                    if ('L' == token_val[0]) {
                        ...
                        if (g.overflow || result > (IntegralResult)(std::numeric_limits<wchar_t>::max)()) {
                            // out of range
                        ...
                        if (g.overflow || result > (IntegralResult)(std::numeric_limits<unsigned char>::max)()) {
                            // out of range
                            status = error_character_overflow;
                        }
                    }
