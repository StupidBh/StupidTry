...
namespace boost {
    namespace proto {
        ...
        namespace detail {
            ...
            struct array_matches : mpl::false_
            {
            };

            template<typename T, std::size_t M>
            struct array_matches<T[M], T*> : mpl::true_
            {
            };

            template<typename T, std::size_t M>
            struct array_matches<T[M], T const*> : mpl::true_
            {
            };

            template<typename T, std::size_t M>
            struct array_matches<T[M], T[proto::N]> : mpl::true_
            {
            };

            template<
                typename T,
                typename U BOOST_PROTO_TEMPLATE_ARITY_PARAM(long Arity = detail::template_arity<U>::value)>
            struct lambda_matches : mpl::false_
            {
            };

            template<typename T>
            struct lambda_matches<T, proto::_ BOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)> : mpl::true_
            {
            };

            template<typename T>
            struct lambda_matches<T, T BOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)> : mpl::true_
            {
            };

            template<typename T, std::size_t M, typename U>
            struct lambda_matches<T[M], U BOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)> : array_matches<T[M], U>
            {
            };

            template<typename T, std::size_t M>
            struct lambda_matches<T[M], _ BOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)> : mpl::true_
            {
            };

            template<typename T, std::size_t M>
            struct lambda_matches<T[M], T[M] BOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)> : mpl::true_
            {
            };
            ...
            template<
                ...
                bool Zero,
                typename Void = void>
            struct vararg_matches : mpl::false_
            {
            };
            ...
            struct vararg_matches<Expr, Args1, Args2, Back, true, false, typename Back::proto_is_vararg_> :
                and_2<
                    matches_<
                        ...
                        proto::basic_expr<ignore, Args2, Args2::arity>>::value,
            ...
            template<typename T, typename U>
            struct is_cv_ref_compatible : mpl::true_
            {
            };

            template<typename T, typename U>
            struct is_cv_ref_compatible<T, U&> : mpl::false_
            {
            };

            template<typename T, typename U>
            struct is_cv_ref_compatible<T&, U&> : mpl::bool_<is_const<T>::value == is_const<U>::value>
            {
            };
            ...
            template<typename T, typename U, bool B = is_array<BOOST_PROTO_UNCVREF(T)>::value>
            struct terminal_array_matches : mpl::false_
            {
            };

            template<typename T, typename U, std::size_t M>
            struct terminal_array_matches<T, U (&)[M], true> : is_convertible<T, U (&)[M]>
            {
            };

            template<typename T, typename U>
            struct terminal_array_matches<T, U (&)[proto::N], true> : is_convertible<T, U*>
            {
            };

            template<typename T, typename U>
            struct terminal_array_matches<T, U*, true> : is_convertible<T, U*>
            {
            };
            ...
            struct terminal_matches :
                mpl::or_<
                    mpl::and_<
                        ...
                        lambda_matches<BOOST_PROTO_UNCVREF(T), BOOST_PROTO_UNCVREF(U)>>,
            ...
            template<typename T, typename U>
            struct terminal_matches :
                mpl::and_<is_cv_ref_compatible<T, U>, lambda_matches<BOOST_PROTO_UNCVREF(T), BOOST_PROTO_UNCVREF(U)>>
            {
            };

            template<typename T, std::size_t M>
            struct terminal_matches<T (&)[M], T (&)[proto::N]> : mpl::true_
            {
            };

            template<typename T, std::size_t M>
            struct terminal_matches<T (&)[M], T*> : mpl::true_
            {
            };
    ...
    // Avoid ambiguity errors on MSVC
            ...
            template<typename T, std::size_t M>
            struct terminal_matches<T const (&)[M], T const[M]> : mpl::true_
            {
            };
    #endif
#endif
            ...
            template<typename T>
            struct terminal_matches<T, T> : mpl::true_
            {
            };
            ...
            template<typename T>
            struct terminal_matches<T const&, T> : mpl::true_
            {
            };

            template<typename T>
            struct terminal_matches<T, proto::_> : mpl::true_
            {
            };

            template<typename T>
            struct terminal_matches<T, exact<T>> : mpl::true_
            {
            };

            template<typename T, typename U>
            struct terminal_matches<T, proto::convertible_to<U>> : is_convertible<T, U>
            {
            };

            // matches_
            template<typename Expr, typename BasicExpr, typename Grammar>
            struct matches_ : mpl::false_
            {
            };

            template<typename Expr, typename BasicExpr>
            struct matches_<Expr, BasicExpr, proto::_> : mpl::true_
            {
            };
...
#include <boost/proto/detail/lambda_matches.hpp>
            ...
            struct matches_<Expr, proto::basic_expr<Tag, Args, Arity>, proto::if_<If, Then, Else>> :
                mpl::eval_if_c<
                    static_cast<bool>(remove_reference<
                                      typename when<_, If>::template impl<Expr, int, int>::result_type>::type::value),
                    matches_<Expr, proto::basic_expr<Tag, Args, Arity>, typename Then::proto_grammar>,
                    matches_<Expr, proto::basic_expr<Tag, Args, Arity>, typename Else::proto_grammar>>::type
            {
            ...
            template<typename Expr, typename BasicExpr>
            struct matches_<Expr, BasicExpr, or_<>> : mpl::false_
            {
                typedef not_<_> which;
            };

            template<typename Expr, typename BasicExpr, typename G0>
            struct matches_<Expr, BasicExpr, or_<G0>> : matches_<Expr, BasicExpr, typename G0::proto_grammar>
            {
                typedef G0 which;
            };
            ...
            template<typename Expr, typename BasicExpr>
            struct matches_<Expr, BasicExpr, and_<>> : mpl::true_
            {
            };

            template<typename Expr, typename BasicExpr, typename G0>
            struct matches_<Expr, BasicExpr, and_<G0>> : matches_<Expr, BasicExpr, typename G0::proto_grammar>
            {
            };
            ...
            struct matches_<Expr, proto::basic_expr<Tag, Args, Arity>, switch_<Cases, Transform>> :
                matches_<
                    ...
                    typename Cases::template case_<
                        typename when<_, Transform>::template impl<Expr, int, int>::result_type>::proto_grammar>
            {
                typedef typename Cases::template case_<
            ...
            struct matches_<Expr, proto::basic_expr<Tag, Args, Arity>, switch_<Cases>> :
                matches_<Expr, proto::basic_expr<Tag, Args, Arity>, typename Cases::template case_<Tag>::proto_grammar>
            {
        ...
        namespace detail {
            template<typename Expr, typename State, typename Data>
            struct _and_impl<proto::and_<>, Expr, State, Data> : proto::_::impl<Expr, State, Data>
            {
            };
        ...
        struct if_ : transform<if_<If, Then, Else>>
        {
            ...
            struct impl : transform_impl<Expr, State, Data>
            {
                typedef typename when<_, If>::template impl<Expr, State, Data>::result_type condition;

                typedef typename mpl::if_c<
                    static_cast<bool>(remove_reference<condition>::type::value),
                    when<_, Then>,
                    when<_, Else>>::type which;
        ...
        struct or_ : transform<or_<BOOST_PROTO_LOGICAL_G>>
        {
            ...
            template<typename Expr, typename State, typename Data>
            struct impl :
                detail::matches_<typename Expr::proto_derived_expr, typename Expr::proto_grammar, or_>::which::
        ...
        struct and_ : transform<and_<BOOST_PROTO_LOGICAL_G>>
        {
            ...
            template<typename Expr, typename State, typename Data>
            struct impl : detail::_and_impl<and_, Expr, State, Data>
            {
            };
        };
        ...
        struct switch_<Cases> : transform<switch_<Cases>>
        {
            ...
            template<typename Expr, typename State, typename Data>
            struct impl : Cases::template case_<typename Expr::proto_tag>::template impl<Expr, State, Data>
            {
            };
        ...
        ///
        template<BOOST_PROTO_LOGICAL_typename_G>
        struct is_callable<or_<BOOST_PROTO_LOGICAL_G>> : mpl::true_
        {
        };
        ...
        ///
        template<BOOST_PROTO_LOGICAL_typename_G>
        struct is_callable<and_<BOOST_PROTO_LOGICAL_G>> : mpl::true_
        {
        };
        ...
        ///
        template<typename Grammar>
        struct is_callable<not_<Grammar>> : mpl::true_
        {
        };
        ...
        struct is_callable<if_<If, Then, Else>> : mpl::true_
        {
        };
        ...
        ///
        template<typename Grammar>
        struct is_callable<vararg<Grammar>> : mpl::true_
        {
        };
        ...
        ///
        template<typename Cases, typename Transform>
        struct is_callable<switch_<Cases, Transform>> : mpl::true_
        {
        };
