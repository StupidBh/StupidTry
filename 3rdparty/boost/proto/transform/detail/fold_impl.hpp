    ...
    #define BOOST_PROTO_FOLD_STATE_TYPE(Z, N, DATA)                                                              \
        typedef typename when<_, Fun>::                                                                          \
            template impl<typename result_of::child_c<Expr, N>::type, BOOST_PP_CAT(state, N), Data>::result_type \
                BOOST_PP_CAT(state, BOOST_PP_INC(N));                                                            \
        /**/

    #define BOOST_PROTO_FOLD_STATE(Z, N, DATA)                                                         \
        BOOST_PP_CAT(state, BOOST_PP_INC(N))                                                           \
        BOOST_PP_CAT(s, BOOST_PP_INC(N)) = typename when<_, Fun>::                                     \
            template impl<typename result_of::child_c<Expr, N>::type, BOOST_PP_CAT(state, N), Data>()( \
                proto::child_c<N>(e),                                                                  \
                BOOST_PP_CAT(s, N),                                                                    \
                d);                                                                                    \
        /**/
    ...
    #define BOOST_PROTO_REVERSE_FOLD_STATE_TYPE(Z, N, DATA)                               \
        typedef typename when<_, Fun>::template impl<                                     \
            typename result_of::child_c<Expr, BOOST_PP_SUB(DATA, BOOST_PP_INC(N))>::type, \
            BOOST_PP_CAT(state, BOOST_PP_SUB(DATA, N)),                                   \
            Data>::result_type BOOST_PP_CAT(state, BOOST_PP_SUB(DATA, BOOST_PP_INC(N)));  \
        /**/

    #define BOOST_PROTO_REVERSE_FOLD_STATE(Z, N, DATA)                                               \
        BOOST_PP_CAT(state, BOOST_PP_SUB(DATA, BOOST_PP_INC(N)))                                     \
        BOOST_PP_CAT(s, BOOST_PP_SUB(DATA, BOOST_PP_INC(N))) = typename when<_, Fun>::template impl< \
            typename result_of::child_c<Expr, BOOST_PP_SUB(DATA, BOOST_PP_INC(N))>::type,            \
            BOOST_PP_CAT(state, BOOST_PP_SUB(DATA, N)),                                              \
            Data>()(                                                                                 \
...
struct fold_impl<State0, Fun, Expr, State, Data, N> : transform_impl<Expr, State, Data>
{
...
template<typename State0, typename Fun, typename Expr, typename State, typename Data>
struct reverse_fold_impl<State0, Fun, Expr, State, Data, N> : transform_impl<Expr, State, Data>
{
        ...
        typename reverse_fold_impl::data_param d) const
    {
        ...
        BOOST_PP_CAT(s, N) = typename when<_, State0>::template impl<Expr, State, Data>()(e, s, d);
