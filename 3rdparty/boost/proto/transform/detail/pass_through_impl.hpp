    ...
    #define BOOST_PROTO_DEFINE_TRANSFORM_TYPE(Z, N, DATA) \
        typename Grammar::BOOST_PP_CAT(                   \
            proto_child,                                  \
            N)::template impl<typename result_of::child_c<Expr, N>::type, State, Data>::result_type /**/

    #define BOOST_PROTO_DEFINE_TRANSFORM(Z, N, DATA)                                  \
        typename Grammar::BOOST_PP_CAT(proto_child, N)::                              \
            template impl<typename result_of::child_c<Expr, N>::type, State, Data>()( \
                e.proto_base().BOOST_PP_CAT(child, N),                                \
                s,                                                                    \
                d) /**/
...
//  LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
    ...
    #define BOOST_PP_ITERATION_PARAMS_1 \
        (3, (1, BOOST_PROTO_MAX_ARITY, <boost / proto / transform / detail / pass_through_impl.hpp>))
...
struct pass_through_impl<Grammar, Domain, Expr, State, Data, N> : transform_impl<Expr, State, Data>
{
    ...
    typedef typename base_expr<
        ...
        BOOST_PP_CAT(list, N) < BOOST_PP_ENUM(N, BOOST_PROTO_DEFINE_TRANSFORM_TYPE, ~)> > ::type expr_type;
    ...
    {
        expr_type const that = { BOOST_PP_ENUM(N, BOOST_PROTO_DEFINE_TRANSFORM, ~) };
