    ...
    #define BOOST_PROTO_MAKE_IF(Z, M, DATA) make_if_<BOOST_PP_CAT(A, M), Expr, State, Data>     /**/

    #define BOOST_PROTO_MAKE_IF_TYPE(Z, M, DATA) typename BOOST_PROTO_MAKE_IF(Z, M, DATA)::type /**/

    #define BOOST_PROTO_MAKE_IF_APPLIED(Z, M, DATA) BOOST_PROTO_MAKE_IF(Z, M, DATA)::applied || /**/

    #define BOOST_PROTO_CONSTRUCT_ARG(Z, M, DATA) \
        detail::as_lvalue(typename when<_, BOOST_PP_CAT(A, M)>::template impl<Expr, State, Data>()(e, s, d)) /**/
...
//  LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
    ...
    #define BOOST_PP_ITERATION_PARAMS_1 (3, (0, BOOST_PROTO_MAX_ARITY, <boost / proto / transform / detail / make.hpp>))
...
namespace detail {
    ...
    template<
        ...
        typename State,
        typename Data>
    struct make_<R<BOOST_PP_ENUM_PARAMS(N, A)>, Expr, State, Data BOOST_PROTO_TEMPLATE_ARITY_PARAM(N)> :
        nested_type_if<
    ...
    template<
        ...
        typename State,
        typename Data>
    struct make_<noinvoke<R<BOOST_PP_ENUM_PARAMS(N, A)>>, Expr, State, Data BOOST_PROTO_TEMPLATE_ARITY_PARAM(1)>
    {
    ...
    struct is_applyable<R(BOOST_PP_ENUM_PARAMS(N, A))> : mpl::true_
    {
    };
    ...
    struct construct_<proto::expr<T, A, N>, true>
    {
        ...
        BOOST_FORCEINLINE result_type operator()(BOOST_PP_ENUM_BINARY_PARAMS(BOOST_PP_MAX(N, 1), A, &a)) const
        {
    ...
    BOOST_FORCEINLINE Type construct(BOOST_PP_ENUM_BINARY_PARAMS(N, A, &a))
    {
...
struct make<Object(BOOST_PP_ENUM_PARAMS(N, A))> : transform<make<Object(BOOST_PP_ENUM_PARAMS(N, A))>>
{
    ...
    template<typename Expr, typename State, typename Data>
        ...
        make<typename detail::expand_pattern<
            ...
            detail::BOOST_PP_CAT(expand_pattern_rest_, BOOST_PP_DEC(N)) <
                Object BOOST_PP_ENUM_TRAILING_PARAMS(BOOST_PP_DEC(N), A)>>::type > ::template impl<Expr, State, Data>
    {
    };
};
    #endif
