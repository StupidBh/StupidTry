...
struct expand_pattern<BOOST_PP_INC(N), Fun, Cont> : Cont::template cat<BOOST_PP_ENUM(BOOST_PP_INC(N), M0, ~)>
{
...
struct BOOST_PP_CAT(expand_pattern_rest_, N)
{
    ...
    struct cat;
        ...
        #define BOOST_PP_ITERATION_PARAMS_2 (3, (1, M, <boost / proto / transform / detail / pack_impl.hpp>))
        #include BOOST_PP_ITERATE()
};
