    ...
    #define BOOST_PROTO_CHILD_N(Z, N, expr) proto::child_c<N>(expr) /**/
    ...
    struct is_expr_handled<Expr, Context, N>
    {
        ...
        static const bool value =
            sizeof(yes_type) == sizeof(detail::check_is_expr_handled(
                                    (sctx_(stag_ BOOST_PP_ENUM_TRAILING(N, BOOST_PROTO_CHILD_N, sexpr_)), 0)));
...
namespace context {
    ...
    struct callable_eval<Expr, Context, N>
    {
        ...
        typedef typename BOOST_PROTO_RESULT_OF<Context(
            typename Expr::proto_tag BOOST_PP_ENUM_TRAILING_PARAMS(N, child))>::type result_type;
        ...
        {
            return context(typename Expr::proto_tag() BOOST_PP_ENUM_TRAILING(N, BOOST_PROTO_CHILD_N, expr));
        }
    };
}
