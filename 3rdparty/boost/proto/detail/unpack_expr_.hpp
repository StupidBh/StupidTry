    ...
    #define BOOST_PROTO_FUSION_NEXT_ITERATOR_TYPE(Z, N, DATA)                                          \
        typedef typename fusion::result_of::next<BOOST_PP_CAT(fusion_iterator, N)>::type BOOST_PP_CAT( \
            fusion_iterator,                                                                           \
            BOOST_PP_INC(N));                                                                          \
        /**/
    ...
    #define BOOST_PROTO_FUSION_AT_TYPE(Z, N, DATA) \
        typename add_const<typename fusion::result_of::value_of<BOOST_PP_CAT(fusion_iterator, N)>::type>::type /**/

    /// INTERNAL ONLY
    ///
    #define BOOST_PROTO_FUSION_NEXT_ITERATOR(Z, N, DATA)                       \
        BOOST_PP_CAT(fusion_iterator, BOOST_PP_INC(N))                         \
        BOOST_PP_CAT(it, BOOST_PP_INC(N)) = fusion::next(BOOST_PP_CAT(it, N)); \
        /**/
    ...
    /// INTERNAL ONLY
    ///
    #define BOOST_PROTO_FUSION_AT(Z, N, DATA) *BOOST_PP_CAT(it, N)                                            /**/

    /// INTERNAL ONLY
    ///
    #define BOOST_PROTO_FUSION_AS_CHILD_AT_TYPE(Z, N, DATA) \
        typename detail::protoify<BOOST_PROTO_FUSION_AT_TYPE(Z, N, DATA), Domain>::result_type                /**/

    /// INTERNAL ONLY
    ///
    #define BOOST_PROTO_FUSION_AS_CHILD_AT(Z, N, DATA) \
        detail::protoify<BOOST_PROTO_FUSION_AT_TYPE(Z, N, DATA), Domain>()(BOOST_PROTO_FUSION_AT(Z, N, DATA)) /**/
    ...
    unpack_expr_<tag::terminal, default_domain, Sequence, 1u>
{
    ...
    #define BOOST_PP_ITERATION_PARAMS_1 (3, (1, BOOST_PROTO_MAX_ARITY, <boost / proto / detail / unpack_expr_.hpp>))
...
struct unpack_expr_<Tag, Domain, Sequence, N>
{
    ...
    typedef BOOST_PP_CAT(list, N)<BOOST_PP_ENUM(N, BOOST_PROTO_FUSION_AS_CHILD_AT_TYPE, ~)> proto_args;
    ...
    static type const call(Sequence const& sequence)
    {
        ...
        expr_type const that = { BOOST_PP_ENUM(N, BOOST_PROTO_FUSION_AS_CHILD_AT, ~) };
...
struct unpack_expr_<Tag, deduce_domain, Sequence, N>
{
    ...
    typedef unpack_expr_<
        Tag,
        typename BOOST_PP_CAT(deduce_domain, N) < BOOST_PP_ENUM(N, BOOST_PROTO_FUSION_AT_TYPE, ~)>::type,
        Sequence, N > other;
