...
//  LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
    ...
    #define BOOST_PP_ITERATION_PARAMS_1                                       \
        (3,                                                                   \
         (2,                                                                  \
          BOOST_PP_MAX(BOOST_PROTO_MAX_ARITY, BOOST_PROTO_MAX_LOGICAL_ARITY), \
...
{};
    ...
    BOOST_PP_CAT(
        and_,
        BOOST_PP_DEC(
            N))<P0::value BOOST_PP_COMMA_IF(BOOST_PP_SUB(N, 2)) BOOST_PP_ENUM_SHIFTED_PARAMS(BOOST_PP_DEC(N), P)>
{
};
    #endif

template<BOOST_PP_ENUM_PARAMS(BOOST_PP_DEC(N), typename P)>
struct BOOST_PP_CAT(and_, N)<false, BOOST_PP_ENUM_PARAMS(BOOST_PP_DEC(N), P)> : mpl::false_
{
};
...
struct _and_impl<proto::and_<BOOST_PP_ENUM_PARAMS(N, G)>, Expr, State, Data> : proto::transform_impl<Expr, State, Data>
{
        #define M0(Z, N, DATA)                                                                                         \
            typedef typename proto::when<proto::_, BOOST_PP_CAT(G, N)>::template impl<Expr, State, Data> BOOST_PP_CAT( \
                Gimpl,                                                                                                 \
                N);                                                                                                    \
        /**/
