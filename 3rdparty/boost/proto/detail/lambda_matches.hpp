    ...
    #define BOOST_PROTO_DEFINE_LAMBDA_MATCHES(Z, N, DATA) \
        lambda_matches<BOOST_PP_CAT(Expr, N), BOOST_PP_CAT(Grammar, N)>
...
//  LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
    ...
    #define BOOST_PP_ITERATION_PARAMS_1 (3, (2, BOOST_PROTO_MAX_ARITY, <boost / proto / detail / lambda_matches.hpp>))
...
template<template<BOOST_PP_ENUM_PARAMS(N, typename BOOST_PP_INTERCEPT)> class T
             BOOST_PP_ENUM_TRAILING_PARAMS(N, typename Expr) BOOST_PP_ENUM_TRAILING_PARAMS(N, typename Grammar)>
struct lambda_matches<
    T<BOOST_PP_ENUM_PARAMS(N, Expr)>,
    T<BOOST_PP_ENUM_PARAMS(N, Grammar)> BOOST_PROTO_TEMPLATE_ARITY_PARAM(N)> :
    BOOST_PP_CAT(and_, N)<
