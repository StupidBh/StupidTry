...
#include <boost/utility/result_of.hpp>
    ...
    #if 10 < BOOST_PROTO_MAX_ARITY || 10 < BOOST_PROTO_MAX_LOGICAL_ARITY || 10 < BOOST_PROTO_MAX_FUNCTION_CALL_ARITY
        #define BOOST_PROTO_DONT_USE_PREPROCESSED_FILES
    #endif
#endif

#ifndef BOOST_PROTO_BROKEN_CONST_OVERLOADS
    #if BOOST_WORKAROUND(__GNUC__, == 3) || BOOST_WORKAROUND(__EDG_VERSION__, BOOST_TESTED_AT(310))
...
// stricter about the return types of some functions.
    ...
    #define BOOST_PROTO_DISABLE_MSVC_C4522 \
        __pragma(warning(disable: 4522)) // 'class' : multiple assignment operators specified
    #define BOOST_PROTO_DISABLE_MSVC_C4714 \
        __pragma(warning(disable: 4714)) // function 'xxx' marked as __forceinline not inlined
#else
...
#define M0(Z, N, DATA)                                   \
    template<BOOST_PP_ENUM_PARAMS_Z(Z, N, typename Arg)> \
...
#undef M0
        ...
        namespace domainns_ {
            ...
            template<
                ...
                typename Tag,
                typename Args,
                bool WantsBasicExpr = wants_basic_expr<typename Domain::proto_generator>::value>
            struct base_expr;
        }
...
#define M0(Z, N, DATA) typedef _child_c<N> BOOST_PP_CAT(_child, N);
        BOOST_PP_REPEAT_FROM_TO(2, BOOST_PP_DEC(BOOST_PROTO_MAX_ARITY), M0, ~)
#undef M0
