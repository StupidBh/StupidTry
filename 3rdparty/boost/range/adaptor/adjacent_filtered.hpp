...
namespace boost {
    namespace range_detail {
        ...
        public:
            adjacent_filtered_range(const P& p, R& r) :
                base_range(skip_iter(boost::begin(r), boost::end(r), p), skip_iter(boost::end(r), boost::end(r), p))
            {
            }
        };
            ...
            operator|(const ForwardRng& r, const adjacent_holder<BinPredicate>& f)
        {
            ...
            return adjacent_filtered_range<BinPredicate, const ForwardRng, true>(f.val, r);
        }
        ...
        {
            BOOST_RANGE_CONCEPT_ASSERT((ForwardRangeConcept<const ForwardRng>));
            return adjacent_filtered_range<BinPredicate, const ForwardRng, false>(f.val, r);
        }
    ...
    namespace adaptors {
        namespace {
            const range_detail::forwarder<range_detail::adjacent_holder> adjacent_filtered =
                range_detail::forwarder<range_detail::adjacent_holder>();

            const range_detail::forwarder<range_detail::adjacent_excl_holder> adjacent_filtered_excl =
