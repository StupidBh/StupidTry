...
namespace boost {
    namespace range_detail {
        ...
        public:
            ...
            const Value& operator()(const Value& x) const { return (x == m_impl->m_from) ? m_impl->m_to : x; }
        ...
        class replaced_range :
            public boost::iterator_range<boost::transform_iterator<
        ...
        public:
            ...
            replaced_range(R& r, value_type from, value_type to) :
                base_t(
                    make_transform_iterator(boost::begin(r), Fn(from, to)),
        ...
        inline replaced_range<SinglePassRange> operator|(SinglePassRange& r, const replace_holder<Value>& f)
        {
        ...
        template<class SinglePassRange, class Value>
        inline replaced_range<const SinglePassRange> operator|(const SinglePassRange& r, const replace_holder<Value>& f)
        {
    ...
    namespace adaptors {
        namespace {
            const range_detail::forwarder2<range_detail::replace_holder> replaced =
                range_detail::forwarder2<range_detail::replace_holder>();
        }

        template<class SinglePassRange, class Value>
        inline replaced_range<SinglePassRange> replace(SinglePassRange& rng, Value from, Value to)
        {
            BOOST_RANGE_CONCEPT_ASSERT((SinglePassRangeConcept<SinglePassRange>));
        ...
        template<class SinglePassRange, class Value>
        inline replaced_range<const SinglePassRange> replace(const SinglePassRange& rng, Value from, Value to)
        {
            BOOST_RANGE_CONCEPT_ASSERT((SinglePassRangeConcept<const SinglePassRange>));
