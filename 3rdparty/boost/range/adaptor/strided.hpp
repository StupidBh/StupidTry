...
namespace boost {
    namespace range_detail {
        ...
        public:
            ...
            strided_iterator(
                ...
                typename iterators::enable_if_convertible<OtherIterator, base_iterator>::type* = 0) :
                m_it(other.base()),
        ...
        private:
            void increment()
            {
                for (difference_type i = 0; (m_it != m_last) && (i < m_stride); ++i) {
                    ++m_it;
                }
            }
            ...
            bool equal(
                const strided_iterator<OtherIterator, Category>& other,
                typename iterators::enable_if_convertible<OtherIterator, base_iterator>::type* = 0) const
            {
        ...
        public:
            ...
            template<class OtherIterator>
            strided_iterator(
                const strided_iterator<OtherIterator, bidirectional_traversal_tag>& other,
                typename iterators::enable_if_convertible<OtherIterator, base_iterator>::type* = 0) :
                m_it(other.base()),
            ...
            difference_type get_offset() const { return m_offset; }
        ...
        private:
            void increment() { m_offset += m_stride; }

            void decrement() { m_offset -= m_stride; }
            ...
            bool equal(
                ...
                typename iterators::enable_if_convertible<OtherIterator, base_iterator>::type* = 0) const
            {
                return (m_index + m_offset) == (other.get_index() + other.get_offset());
            }
        ...
        public:
            ...
            template<class OtherIterator>
            strided_iterator(
                const strided_iterator<OtherIterator, random_access_traversal_tag>& other,
                typename iterators::enable_if_convertible<OtherIterator, base_iterator>::type* = 0) :
                m_it(other.base()),
        ...
        private:
            void increment() { m_index += m_stride; }

            void decrement() { m_index -= m_stride; }

            void advance(difference_type offset) { m_index += (m_stride * offset); }
            ...
            void update() const { m_it = m_first + m_index; }
        ...
        inline strided_iterator<typename range_iterator<Rng>::type, forward_traversal_tag>
            make_begin_strided_iterator(Rng& rng, Difference stride, forward_traversal_tag)
        {
            return strided_iterator<typename range_iterator<Rng>::type, forward_traversal_tag>(
                boost::begin(rng),
                boost::end(rng),
                stride);
        }
        ...
        template<class Rng, class Difference>
        inline strided_iterator<typename range_iterator<const Rng>::type, forward_traversal_tag>
            make_begin_strided_iterator(const Rng& rng, Difference stride, forward_traversal_tag)
        {
            return strided_iterator<typename range_iterator<const Rng>::type, forward_traversal_tag>(
                boost::begin(rng),
                boost::end(rng),
                stride);
        }
            ...
            make_begin_strided_iterator(Rng& rng, Difference stride, bidirectional_traversal_tag)
        {
            ...
            return strided_iterator<typename range_iterator<Rng>::type, bidirectional_traversal_tag>(
                boost::begin(rng),
                difference_type(),
                stride);
        }
            ...
            make_begin_strided_iterator(const Rng& rng, Difference stride, bidirectional_traversal_tag)
        {
            ...
            return strided_iterator<typename range_iterator<const Rng>::type, bidirectional_traversal_tag>(
                boost::begin(rng),
                difference_type(),
                stride);
        }
        ...
        {
            return strided_iterator<typename range_iterator<Rng>::type, random_access_traversal_tag>(
                boost::begin(rng),
                boost::begin(rng),
                stride);
        }
        ...
        {
            return strided_iterator<typename range_iterator<const Rng>::type, random_access_traversal_tag>(
                boost::begin(rng),
                boost::begin(rng),
                stride);
        }
        ...
        template<
            class Rng,
            class Category = typename iterators::pure_iterator_traversal<typename range_iterator<Rng>::type>::type>
        class strided_range :
            public iterator_range<range_detail::strided_iterator<typename range_iterator<Rng>::type, Category>> {
            typedef range_detail::strided_iterator<typename range_iterator<Rng>::type, Category> iter_type;
            ...
            strided_range(Difference stride, Rng& rng) :
                super_t(
                    range_detail::make_begin_strided_iterator(
                        rng,
                        stride,
                        typename iterator_traversal<typename range_iterator<Rng>::type>::type()),
        ...
        template<class Rng, class Difference>
        inline strided_range<Rng> operator|(Rng& rng, const strided_holder<Difference>& stride)
        {
            return strided_range<Rng>(stride.val, rng);
        }

        template<class Rng, class Difference>
        inline strided_range<const Rng> operator|(const Rng& rng, const strided_holder<Difference>& stride)
        {
    ...
    namespace adaptors {
        ...
        namespace {
            const range_detail::forwarder<range_detail::strided_holder> strided =
                range_detail::forwarder<range_detail::strided_holder>();
        }

        template<class Range, class Difference>
        inline strided_range<Range> stride(Range& rng, Difference step)
        {
            return strided_range<Range>(step, rng);
        }

        template<class Range, class Difference>
        inline strided_range<const Range> stride(const Range& rng, Difference step)
        {
