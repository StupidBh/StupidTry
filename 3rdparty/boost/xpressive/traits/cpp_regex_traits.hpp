...
namespace boost {
    ...
    // Reserve some bits for the implementation
            ...
            umaskex_t const all_ctype_masks = std_ctype_reserved | std_ctype_alnum | std_ctype_alpha | std_ctype_cntrl |
                                              std_ctype_digit | std_ctype_graph | std_ctype_lower | std_ctype_print |
                                              std_ctype_punct | std_ctype_space | std_ctype_upper | std_ctype_xdigit;
            ...
            umaskex_t const non_std_ctype_newline =
                unused_mask<all_ctype_masks | non_std_ctype_underscore | non_std_ctype_blank>::value;
            ...
            {
                return "\0\0\1\0\2\0\0\0\3"[i & 0xf] + "\0\4\5\0\6\0\0\0\7"[(i & 0xf0) >> 04] +
                       "\0\10\11\0\12\0\0\0\13"[(i & 0xf00) >> 010];
            }
#endif

            // convenient constant for the extra masks
            umaskex_t const non_std_ctype_masks =
                non_std_ctype_underscore | non_std_ctype_blank | non_std_ctype_newline;
    ...
    // HACKHACK Cygwin and mingw have buggy ctype facets for wchar_t
        ...
        template<typename Char>
        struct cpp_regex_traits : detail::cpp_regex_traits_base<Char>
        {
            ...
            bool operator==(cpp_regex_traits<char_type> const& that) const { return this->loc_ == that.loc_; }
            ...
            char_type widen(char ch) const { return this->ctype_->widen(ch); }
            ...
            /// \param ch The source character.
            /// \return ch
            static char_type translate(char_type ch) { return ch; }

            /// Converts a character to lower-case using the internally-stored std::locale.
            ///
            /// \param ch The source character.
            /// \return std::tolower(ch, this->getloc()).
            char_type translate_nocase(char_type ch) const { return this->ctype_->tolower(ch); }

            /// Converts a character to lower-case using the internally-stored std::locale.
            ///
            /// \param ch The source character.
            /// \return std::tolower(ch, this->getloc()).
            char_type tolower(char_type ch) const { return this->ctype_->tolower(ch); }

            /// Converts a character to upper-case using the internally-stored std::locale.
            ///
            /// \param ch The source character.
            /// \return std::toupper(ch, this->getloc()).
            char_type toupper(char_type ch) const { return this->ctype_->toupper(ch); }
            ...
            string_type fold_case(char_type ch) const
            {
                ...
                char_type ntcs[] = { this->ctype_->tolower(ch), this->ctype_->toupper(ch), 0 };
                if (ntcs[1] == ntcs[0]) {
                    ntcs[1] = 0;
                }
            ...
            static bool in_range(char_type first, char_type last, char_type ch) { return first <= ch && ch <= last; }
            ...
            bool in_range_nocase(char_type first, char_type last, char_type ch) const
            {
                ...
                // case folding, but this is the best we can do with the standard
                // std::ctype facet.
                return this->in_range(first, last, ch) || this->in_range(first, last, this->ctype_->toupper(ch)) ||
            ...
            {
                static detail::umaskex_t const icase_masks = detail::std_ctype_lower | detail::std_ctype_upper;
            ...
            ///
            locale_type getloc() const { return this->loc_; }
        ...
        template<>
        struct has_fold_case<cpp_regex_traits<char>> : mpl::true_
        {
        };
