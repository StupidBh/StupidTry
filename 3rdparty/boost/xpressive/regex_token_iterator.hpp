...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            struct regex_token_iterator_impl : counted_base<regex_token_iterator_impl<BidiIter>>
            {
                ...
                bool next()
                {
                    if (-1 != this->n_) {
                        ...
                        if (0 != (++this->n_ %= (int)this->subs_.size()) || this->iter_.next()) {
                            this->result_ = (-1 == this->subs_[this->n_]) ? this->iter_.what_.prefix()
        ...
        struct regex_token_iterator
        {
            ...
            /// \param flags Optional match flags, used to control how the expression is matched against the sequence.
            /// (See match_flag_type.)
            /// \pre \c [begin,end) is a valid range.
            ...
            friend bool
                operator==(regex_token_iterator<BidiIter> const& left, regex_token_iterator<BidiIter> const& right)
            {
            ...
            value_type const& operator*() const { return this->impl_->result_; }
