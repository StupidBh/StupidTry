...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            // matchable_ex
            template<typename BidiIter>
            struct matchable_ex : matchable<BidiIter>, counted_base<matchable_ex<BidiIter>>
            {
                typedef BidiIter iterator_type;
                typedef typename iterator_value<iterator_type>::type char_type;
                ...
                bool skip_match(match_state<BidiIter>& state) const { return this->match(state); }
            };
            ...
            struct shared_matchable
            {
                ...
                bool operator!() const { return !this->xpr_; }
                ...
                matchable_ptr const& matchable() const { return this->xpr_; }

                bool match(match_state<BidiIter>& state) const { return this->xpr_->match(state); }

                void link(xpression_linker<char_type>& linker) const { this->xpr_->link(linker); }

                void peek(xpression_peeker<char_type>& peeker) const { this->xpr_->peek(peeker); }
