    ...
    namespace xpressive {
        namespace detail {
            ...
            struct xpression_visitor_base
            {
                ...
                void swap(xpression_visitor_base<BidiIter>& that) { this->self_.swap(that.self_); }

                int get_hidden_mark() { return -(int)(++this->self_->hidden_mark_count_); }

                void mark_number(int mark_nbr)
                {
                    if (0 < mark_nbr) {
                        this->self_->mark_count_ = (std::max)(this->self_->mark_count_, (std::size_t)mark_nbr);
                    }
                }

                shared_ptr<regex_impl<BidiIter>>& self() { return this->self_; }
            ...
            struct xpression_visitor : xpression_visitor_base<BidiIter>
            {
                ...
                template<typename Matcher>
                typename apply<Matcher>::type call(Matcher const& matcher)
                {
                    this->visit_(matcher);
