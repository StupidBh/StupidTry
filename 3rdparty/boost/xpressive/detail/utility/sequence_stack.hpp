        ...
        namespace detail {
            ...
            private:
                ...
                struct chunk
                {
                    ...
                    ~chunk() { deallocate(this->begin_, this->size()); }

                    std::size_t size() const { return static_cast<std::size_t>(this->end_ - this->begin_); }
                ...
                T* grow_(std::size_t count, T const& t)
                {
                    if (this->current_chunk_) {
                        ...
                        // grow exponentially
                        std::size_t new_size = (std::max)(count,
                                                          static_cast<std::size_t>(
                                                              static_cast<double>(this->current_chunk_->size()) * 1.5));

                        // Create a new expr and insert it into the list
                        this->current_chunk_ =
                            new chunk(new_size, t, count, this->current_chunk_, this->current_chunk_->next_);
                    }
                    else {
                        // first chunk is 256
