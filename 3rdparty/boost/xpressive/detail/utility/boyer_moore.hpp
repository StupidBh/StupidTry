...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            struct boyer_moore : noncopyable
            {
                ...
                boyer_moore(char_type const* begin, char_type const* end, Traits const& tr, bool icase) :
                    ...
                    fold_(),
                    find_fun_(
                        icase ? (case_fold() ? &boyer_moore::find_nocase_fold_ : &boyer_moore::find_nocase_)
                              : &boyer_moore::find_)
                {
            ...
            private:
                ...
                void init_(Traits const& tr, mpl::true_)
                {
                    ...
                    for (unsigned char offset = this->length_; offset; --offset, ++this->last_) {
                        ...
                        for (typename string_type::const_iterator beg = this->fold_.back().begin(),
                                                                  end = this->fold_.back().end();
                             beg != end;
                             ++beg) {
