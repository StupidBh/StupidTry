...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            struct simple_repeat_matcher : quant_style_variable_width
            {
                ...
                bool match_(match_state<BidiIter>& state, Next const& next, greedy_slow_tag) const
                {
                    ...
                    if (this->leading_) {
                        state.next_search_ = (matches && matches < this->max_) ? state.cur_
                                             : (tmp == state.end_)             ? tmp
                                                                               : boost::next(tmp);
                    }
                ...
                bool match_(match_state<BidiIter>& state, Next const& next, greedy_fast_tag) const
                {
                    ...
                    if (this->leading_) {
                        state.next_search_ = (diff_to_end && diff_to_end < this->max_) ? state.cur_
                                             : (tmp == state.end_)                     ? tmp
                                                                                       : boost::next(tmp);
                    }
