...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            struct optional_matcher : quant_style<quant_variable_width, unknown_width::value, Xpr::pure>
            {
                Xpr xpr_;
            ...
            private:
                ...
                {
                    return this->xpr_.BOOST_NESTED_TEMPLATE push_match<Next>(state) || next.match(state);
                }

                template<typename BidiIter, typename Next>
                bool match_(match_state<BidiIter>& state, Next const& next, mpl::false_) const // Non-greedy
                {
                    return next.match(state) || this->xpr_.BOOST_NESTED_TEMPLATE push_match<Next>(state);
                }
            ...
            // optional_mark_matcher
            template<typename Xpr, typename Greedy>
            struct optional_mark_matcher : quant_style<quant_variable_width, unknown_width::value, Xpr::pure>
            {
                Xpr xpr_;
                int mark_number_;
