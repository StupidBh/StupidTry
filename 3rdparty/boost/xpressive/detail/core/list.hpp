    ...
    namespace xpressive {
        namespace detail {
            ...
            private:
                ...
                {
                    list_iterator(list_iterator<> const& it) :
                        _node(it._node)
                    {
                    }

                    explicit list_iterator(node_base* n = 0) :
                        _node(n)
                    {
                    }
            ...
            public:
                ...
                list() { _sentry._next = _sentry._prev = &_sentry; }
                ...
                bool empty() const { return _sentry._next == &_sentry; }
                ...
                iterator begin() { return iterator(_sentry._next); }

                const_iterator begin() const { return const_iterator(_sentry._next); }

                iterator end() { return iterator(&_sentry); }

                const_iterator end() const { return const_iterator(const_cast<node_base*>(&_sentry)); }

                size_type size() const { return static_cast<size_type>(std::distance(begin(), end())); }
            };
