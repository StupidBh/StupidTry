...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            struct boyer_moore_finder : finder<BidiIter>
            {
                ...
                bool ok_for_partial_matches() const { return false; }
            ...
            //
            template<typename BidiIter, typename Traits>
            struct hash_peek_finder : finder<BidiIter>
            {
                typedef typename iterator_value<BidiIter>::type char_type;
                ...
                bool operator()(match_state<BidiIter>& state) const
                {
                    Traits const& tr = traits_cast<Traits>(state);
                    state.cur_ =
                        (this->bset_.icase() ? this->find_(state.cur_, state.end_, tr, mpl::true_())
            ...
            template<
                typename BidiIter,
                typename Traits,
                std::size_t Size = sizeof(typename iterator_value<BidiIter>::type)>
            struct line_start_finder : finder<BidiIter>
            {
                typedef typename iterator_value<BidiIter>::type char_type;
            ...
            //
            template<typename BidiIter, typename Traits>
            struct line_start_finder<BidiIter, Traits, 1u> : finder<BidiIter>
            {
                typedef typename iterator_value<BidiIter>::type char_type;
                typedef typename iterator_difference<BidiIter>::type diff_type;
                typedef typename Traits::char_class_type char_class_type;
            ...
            struct leading_simple_repeat_finder : finder<BidiIter>
            {
                leading_simple_repeat_finder() :
                    finder<BidiIter>()
                {
