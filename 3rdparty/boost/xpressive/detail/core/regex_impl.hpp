...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            struct finder : counted_base<finder<BidiIter>>
            {
                virtual ~finder() {}
            ...
            struct traits : counted_base<traits<Char>>
            {
                virtual ~traits() {}
            ...
            struct traits_holder : traits<typename Traits::char_type>
            {
                ...
                Traits const& traits() const { return this->traits_; }

                char_type tolower(char_type ch) const { return this->tolower_(ch, typename Traits::version_tag()); }

                char_type toupper(char_type ch) const { return this->toupper_(ch, typename Traits::version_tag()); }

                int value(char_type ch, int radix) const { return this->traits_.value(ch, radix); }
            ...
            private:
                char_type tolower_(char_type ch, regex_traits_version_1_tag) const { return ch; }

                char_type toupper_(char_type ch, regex_traits_version_1_tag) const { return ch; }

                char_type tolower_(char_type ch, regex_traits_version_2_tag) const { return this->traits_.tolower(ch); }

                char_type toupper_(char_type ch, regex_traits_version_2_tag) const { return this->traits_.toupper(ch); }
            ...
            //
            template<typename BidiIter>
            struct regex_impl : enable_reference_tracking<regex_impl<BidiIter>>
            {
