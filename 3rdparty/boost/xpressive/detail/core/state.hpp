...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            //
            template<typename BidiIter>
            struct match_state : noncopyable
            {
                ...
                // beginning of buffer
                bool bos() const { return this->cur_ == this->begin_; }

                // end of buffer
                bool eos() { return this->cur_ == this->end_ && this->found_partial_match(); }
                ...
                sub_match_impl& sub_match(int n) { return this->sub_matches_[n]; }
            ...
            private:
                void init_(regex_impl const& impl, match_results& what)
                {
                    ...
                    this->sub_matches_ = this->extras_->sub_match_stack_.push_sequence(
                        total_mark_count,
                        sub_match_impl(begin_),
                        detail::fill);
                    this->sub_matches_ += impl.hidden_mark_count_;

                    // initialize the match_results struct
                    access::init_match_results(
                        what,
                        id,
                        impl.traits_,
                        this->sub_matches_,
            ...
            {
                memento<BidiIter> mem = { state.extras_->sub_match_stack_.push_sequence(
                                              state.mark_count_,
                                              sub_match_impl<BidiIter>(state.begin_)),
