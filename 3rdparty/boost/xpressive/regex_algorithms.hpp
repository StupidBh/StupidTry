    ...
    namespace xpressive {
        ...
        namespace detail {
                ...
                bool not_initial_null = false)
            {
                ...
                save_restore<bool> not_null(
                    state.flags_.match_not_null_,
                    state.flags_.match_not_null_ || not_initial_null);
        ...
        /// occur within the sequence <tt>[begin, end)</tt>. If no such matches are found and <tt>!(flags \&
        /// format_no_copy)</tt> then calls <tt>std::copy(begin, end, out)</tt>. Otherwise, for each match found, if
        /// <tt>!(flags \& format_no_copy)</tt> calls <tt>std::copy(m.prefix().first, m.prefix().second, out)</tt>, and
        /// then calls <tt>m.format(out, format, flags)</tt>. Finally if <tt>!(flags \& format_no_copy)</tt> calls
        /// <tt>std::copy(last_m.suffix().first, last_m.suffix().second, out)</tt> where \c last_m is a copy of the last
        /// match found.
        ///
        ...
        inline BidiContainer regex_replace(
            ...
            Formatter const& format,
            regex_constants::match_flag_type flags = regex_constants::match_default,
            typename disable_if<mpl::or_<detail::is_char_ptr<BidiContainer>, detail::is_char_ptr<Formatter>>>::type* =
                0)
        {
