...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            struct regex_iterator_impl : counted_base<regex_iterator_impl<BidiIter>>
            {
                ...
                {
                    return this->rex_.regex_id() == that.rex_.regex_id() && this->state_.begin_ == that.state_.begin_ &&
                           this->state_.cur_ == that.state_.cur_ && this->state_.end_ == that.state_.end_ &&
        ...
        struct regex_iterator
        {
            ...
            value_type const& operator*() const { return this->impl_->what_; }
            ...
            /// \post (**this)[0].matched == true if a full match was found, and false if it was a partial match (found
            /// as a result of the match_partial flag being set).
            /// \post (**this)[n].first == For all integers n \< (*this)-\>size(), the start of the sequence that
            /// matched sub-expression n. Alternatively, if sub-expression n did not participate in the match, then end.
            /// \post (**this)[n].second == For all integers n \< (*this)-\>size(), the end of the sequence that matched
            /// sub-expression n. Alternatively, if sub-expression n did not participate in the match, then end.
            /// \post (**this)[n].matched == For all integers n \< (*this)-\>size(), true if sub-expression n
            /// participated in the match, false otherwise.
            /// \post (*this)-\>position() == The distance from the start of the original sequence being iterated, to
