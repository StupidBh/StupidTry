...
namespace boost {
    namespace xpressive {
        ...
        /// denote the range of characters <tt>[first,second)</tt> which formed that match. Otherwise \c matched is \c
        /// false, and members \c first and \c second contained undefined values.
        ///
        ...
        struct sub_match : std::pair<BidiIter, BidiIter>
        {
        private:
            /// INTERNAL ONLY
            ///
        ...
        public:
            ...
            string_type str() const { return this->matched ? string_type(this->first, this->second) : string_type(); }
            ...
            difference_type length() const { return this->matched ? std::distance(this->first, this->second) : 0; }

            operator bool_type() const { return this->matched ? &dummy::i_ : 0; }

            bool operator!() const { return !this->matched; }
            ...
            int compare(string_type const& str) const { return this->str().compare(str); }

            /// \overload
            ///
            int compare(sub_match const& sub) const { return this->str().compare(sub.str()); }

            /// \overload
            ///
            int compare(value_type const* ptr) const { return this->str().compare(ptr); }
