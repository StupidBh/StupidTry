...
namespace boost {
    namespace xpressive {
        namespace detail {
            ...
            struct results_extras : counted_base<results_extras<BidiIter>>
            {
            ...
            struct char_overflow_handler_
            {
                void operator()(numeric::range_check_result result) const // throw(regex_error)
                {
                    if (numeric::cInRange != result) {
                        BOOST_THROW_EXCEPTION(regex_error(
            ...
            struct case_converting_iterator
            {
                ...
                OutputIterator base() const { return this->out_; }
                ...
                case_converting_iterator& operator*() { return *this; }
                ...
                case_converting_iterator& operator=(Char ch)
                {
                    ...
                    case op_lower: ch = this->traits_->tolower(ch); break;

                    case op_upper: ch = this->traits_->toupper(ch); break;
            ...
            struct noop_output_iterator
            {
                ...
                noop_output_iterator& operator++() { return *this; }

                noop_output_iterator& operator++(int) { return *this; }

                noop_output_iterator& operator*() { return *this; }

                noop_output_iterator& operator=(Char const&) { return *this; }
            };
            ...
            struct formatter_wrapper<Formatter, true> : unary_binary_ternary
            {
                operator Formatter*();
            };

            template<typename Formatter>
            struct formatter_wrapper<Formatter*, false> : unary_binary_ternary
            {
                operator Formatter*();
            };
            ...
            struct formatter_arity<Formatter, What, Out, typename Formatter::proto_is_expr_> : mpl::size_t<4>
            {
            };

            template<typename T>
            struct is_char_ptr : mpl::false_
            {
            };

            template<typename T>
            struct is_char_ptr<T*> : mpl::not_<is_function<T>>
            {
            };
            ...
            template<typename T>
            typename mpl::if_<is_function<T>, T*, T const&>::type as_callable(T const& t)
            {
                return t;
            }
#endif
        ...
        public:
                ...
                named_marks_(that.named_marks_)
            {
                if (that) {
                    ...
                    detail::sub_match_impl<BidiIter>* sub_matches = extras.sub_match_stack_.push_sequence(
                        size,
                        detail::sub_match_impl<BidiIter>(*that.base_),
                        detail::fill);
                    detail::core_access<BidiIter>::init_sub_match_vector(
                        this->sub_matches_,
                        sub_matches,
                        size,
                        that.sub_matches_);
            ...
            size_type size() const { return this->sub_matches_.size(); }
            ...
            ///
            difference_type length(size_type sub = 0) const { return this->sub_matches_[sub].length(); }
            ...
            {
                return this->sub_matches_[sub].matched ? std::distance(*this->base_, this->sub_matches_[sub].first)
                                                       : -1;
            }

            /// Returns (*this)[sub].str().
            ///
            string_type str(size_type sub = 0) const { return this->sub_matches_[sub].str(); }
            ...
            /// stored in *this.
            ///
            const_iterator begin() const { return this->sub_matches_.begin(); }

            /// Returns a terminating iterator that enumerates over all the marked sub-expression
            /// matches stored in *this.
            ///
            const_iterator end() const { return this->sub_matches_.end(); }

            /// Returns a true value if (*this)[0].matched, else returns a false value.
            ///
            operator bool_type() const { return (!this->empty() && this->sub_matches_[0].matched) ? &dummy::i_ : 0; }

            /// Returns true if empty() || !(*this)[0].matched, else returns false.
            ///
            bool operator!() const { return this->empty() || !this->sub_matches_[0].matched; }

            /// Returns the id of the basic_regex object most recently used with this match_results object.
            ///
            regex_id_type regex_id() const { return this->regex_id_; }

            /// Returns a Sequence of nested match_results elements.
            ///
            nested_results_type const& nested_results() const { return this->nested_results_; }
            ...
            /// format used by ECMA-262, ECMAScript Language Specification, Chapter 15 part 5.4.11
            /// String.prototype.replace.
            ///
            /// Otherwise, if \c Format models <tt>Callable\<match_results\<BidiIter\>, OutputIterator,
            /// regex_constants::match_flag_type\></tt>, this function returns <tt>fmt(*this, out, flags)</tt>.
            ///
            /// Otherwise, if \c Format models <tt>Callable\<match_results\<BidiIter\>, OutputIterator\></tt>, this
            /// function returns <tt>fmt(*this, out)</tt>.
            ///
                ...
                typename disable_if<detail::is_char_ptr<Format>>::type* = 0) const
            {
                ...
                typedef typename detail::formatter_arity<Format, match_results<BidiIter>, OutputIterator>::type arity;
            ...
            ///
            /// Otherwise, if \c Format models <tt>Callable\<match_results\<BidiIter\>, OutputIterator,
            /// regex_constants::match_flag_type\></tt>, this function returns a \c string_type object \c x populated by
            /// calling <tt>fmt(*this, out, flags)</tt>, where \c out is a \c back_insert_iterator into \c x.
            ///
            /// Otherwise, if \c Format models <tt>Callable\<match_results\<BidiIter\>, OutputIterator\></tt>, this
            /// function returns a \c string_type object \c x populated by calling <tt>fmt(*this, out)</tt>, where \c
            /// out is a \c back_insert_iterator into \c x.
            ///
            /// Otherwise, if \c Format models <tt>Callable\<match_results\<BidiIter\> \></tt>, this function
            ...
            match_results<BidiIter> const& operator()(regex_id_type regex_id, size_type index = 0) const
            {
                ...
                typename nested_results_type::const_iterator begin = this->nested_results_.begin(),
        ...
        private:
            ...
            void set_prefix_suffix_(BidiIter begin, BidiIter end)
            {
                this->base_ = begin;
                this->prefix_ =
                    sub_match<BidiIter>(begin, this->sub_matches_[0].first, begin != this->sub_matches_[0].first);
                this->suffix_ =
                    sub_match<BidiIter>(this->sub_matches_[0].second, end, this->sub_matches_[0].second != end);
            ...
            /// INTERNAL ONLY
            ///
            void reset_() { detail::core_access<BidiIter>::init_sub_match_vector(this->sub_matches_, 0, 0); }
            ...
            /// INTERNAL ONLY
            ///
            const_reference at_(size_type sub) const { return this->sub_matches_[sub]; }
            ...
            const_reference at_(char_type const* name) const
            {
                ...
                BOOST_THROW_EXCEPTION(regex_error(regex_constants::error_badmark, "invalid named back-reference"));
            ...
            /// INTERNAL ONLY
            ///
            const_reference at_(string_type const& name) const { return (*this)[name.c_str()]; }
            ...
            OutputIterator
                format_(OutputIterator out, Callable1 const& format, regex_constants::match_flag_type, mpl::size_t<1>)
                    const
            {
#if BOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ == 0)
            ...
            OutputIterator
                format_(OutputIterator out, Callable2 const& format, regex_constants::match_flag_type, mpl::size_t<2>)
                    const
            {
#if BOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ == 0)
            ...
            OutputIterator
                format_(OutputIterator out, Expr const& format, regex_constants::match_flag_type, mpl::size_t<4>) const
            {
            ...
            OutputIterator format_ecma_262_(ForwardIterator cur, ForwardIterator end, OutputIterator out) const
            {
                while (cur != end) {
                    switch (*cur) {
                    case BOOST_XPR_CHAR_(char_type, '$'): out = this->format_backref_(++cur, end, out); break;

                    default: *out++ = *cur++; break;
                    }
                }
            ...
            OutputIterator format_sed_(ForwardIterator cur, ForwardIterator end, OutputIterator out) const
            {
                while (cur != end) {
                    ...
                    case BOOST_XPR_CHAR_(char_type, '\\'): out = this->format_escape_(++cur, end, out); break;
            ...
            OutputIterator format_perl_(ForwardIterator cur, ForwardIterator end, OutputIterator out) const
            {
                ...
                while (cur != end) {
                    switch (*cur) {
                    case BOOST_XPR_CHAR_(char_type, '$'): iout = this->format_backref_(++cur, end, iout); break;
            ...
            template<typename ForwardIterator, typename OutputIterator>
            OutputIterator
                format_all_impl_(ForwardIterator& cur, ForwardIterator end, OutputIterator out, bool metacolon = false)
                    const
            {
                int max = 0, sub = 0;
                ...
                while (cur != end) {
                    ...
                    case BOOST_XPR_CHAR_(char_type, '('):
                        ...
                        BOOST_XPR_ENSURE_(
                            BOOST_XPR_CHAR_(char_type, ')') == *(cur - 1),
                    ...
                    case BOOST_XPR_CHAR_(char_type, ':'):
                        if (metacolon) {
                            BOOST_FALLTHROUGH;
                        case BOOST_XPR_CHAR_(char_type, ')'): ++cur; return out;
                        }
                        BOOST_FALLTHROUGH;
            ...
            ///
            template<typename ForwardIterator, typename OutputIterator>
            OutputIterator format_backref_(ForwardIterator& cur, ForwardIterator end, OutputIterator out) const
            {
                if (cur == end) {
            ...
            ///
            template<typename ForwardIterator, typename OutputIterator>
            OutputIterator format_escape_(ForwardIterator& cur, ForwardIterator end, OutputIterator out) const
            {
                ...
                char_type ch = *cur++;
                switch (ch) {
                case BOOST_XPR_CHAR_(char_type, 'a'): *out++ = BOOST_XPR_CHAR_(char_type, '\a'); break;

                case BOOST_XPR_CHAR_(char_type, 'e'): *out++ = converter(27); break;

                case BOOST_XPR_CHAR_(char_type, 'f'): *out++ = BOOST_XPR_CHAR_(char_type, '\f'); break;

                case BOOST_XPR_CHAR_(char_type, 'n'): *out++ = BOOST_XPR_CHAR_(char_type, '\n'); break;

                case BOOST_XPR_CHAR_(char_type, 'r'): *out++ = BOOST_XPR_CHAR_(char_type, '\r'); break;

                case BOOST_XPR_CHAR_(char_type, 't'): *out++ = BOOST_XPR_CHAR_(char_type, '\t'); break;

                case BOOST_XPR_CHAR_(char_type, 'v'): *out++ = BOOST_XPR_CHAR_(char_type, '\v'); break;
                ...
                case BOOST_XPR_CHAR_(char_type, 'x'):
                    ...
                    if (BOOST_XPR_CHAR_(char_type, '{') == *cur) {
                        ...
                        *out++ = converter(detail::toi(cur, end, *this->traits_, 16, 0xffff));
                        BOOST_XPR_ENSURE_(
                            4 == std::distance(tmp, cur) && cur != end && BOOST_XPR_CHAR_(char_type, '}') == *cur++,
                            error_escape,
                            "invalid hex escape : must be \\x { HexDigit HexDigit HexDigit HexDigit }");
                    }
                    else {
                        tmp = cur;
                        *out++ = converter(detail::toi(cur, end, *this->traits_, 16, 0xff));
                        BOOST_XPR_ENSURE_(
                            2 == std::distance(tmp, cur),
                            error_escape,
                ...
                case BOOST_XPR_CHAR_(char_type, 'c'):
                    ...
                    BOOST_XPR_ENSURE_(
                        this->traits_
                                ->in_range(BOOST_XPR_CHAR_(char_type, 'a'), BOOST_XPR_CHAR_(char_type, 'z'), *cur) ||
                            this->traits_
                                ->in_range(BOOST_XPR_CHAR_(char_type, 'A'), BOOST_XPR_CHAR_(char_type, 'Z'), *cur),
                        error_escape,
            ...
            ///
            template<typename ForwardIterator, typename OutputIterator>
            OutputIterator format_named_backref_(ForwardIterator& cur, ForwardIterator end, OutputIterator out) const
            {
                using namespace regex_constants;
                BOOST_XPR_ENSURE_(
                    cur != end && BOOST_XPR_CHAR_(char_type, '<') == *cur++,
                    error_badmark,
                ...
                BOOST_XPR_ENSURE_(
                    cur != begin && cur != end && BOOST_XPR_CHAR_(char_type, '>') == *cur,
                    error_badmark,
                    "invalid named back-reference");
        ...
        struct regex_id_filter_predicate
        {
            ...
            bool operator()(match_results<BidiIter> const& res) const { return this->regex_id_ == res.regex_id(); }
...
namespace std {
    ...
    concept_map OutputIterator<boost::xpressive::detail::case_converting_iterator<Iter_, Char_>, Char_> {};

    template<typename Char_>
    concept_map OutputIterator<boost::xpressive::detail::noop_output_iterator<Char_>, Char_> {};
}
#endif
