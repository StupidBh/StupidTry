...
namespace boost {
    namespace xpressive {
        ...
        struct regex_compiler
        {
            ...
            locale_type getloc() const { return this->traits_.getloc(); }
        ...
        private:
            ...
            // regex_traits
            /// INTERNAL ONLY
            traits_type& rxtraits() { return this->traits_.traits(); }

            ///////////////////////////////////////////////////////////////////////////
            // regex_traits
            /// INTERNAL ONLY
            traits_type const& rxtraits() const { return this->traits_.traits(); }
            ...
            detail::sequence<BidiIter> parse_alternates(FwdIter& begin, FwdIter end)
            {
                ...
                do {
                    switch (++count) {
                    case 1 : seq = this->parse_sequence(tmp, end); break;
                    case 2 : seq = detail::make_dynamic<BidiIter>(alternate_matcher()) | seq; BOOST_FALLTHROUGH;
                    default: seq |= this->parse_sequence(tmp, end);
                    }
            ...
            detail::sequence<BidiIter> parse_group(FwdIter& begin, FwdIter end)
            {
                ...
                case token_negative_lookahead: negative = true; BOOST_FALLTHROUGH;
                case token_positive_lookahead: lookahead = true; break;

                case token_negative_lookbehind: negative = true; BOOST_FALLTHROUGH;
                case token_positive_lookbehind: lookbehind = true; break;

                case token_independent_sub_expression: keeper = true; break;
                ...
                case token_comment:
                    while (BOOST_XPR_ENSURE_(begin != end, error_paren, "mismatched parenthesis")) {
                        ...
                        case token_escape:
                            ...
                            BOOST_FALLTHROUGH;
                        case token_literal: ++begin; break;
                        default           : break;
                        }
                    }
                    break;
                ...
                case token_named_mark:
                    ...
                    for (std::size_t i = 0; i < this->self_->named_marks_.size(); ++i) {
                        BOOST_XPR_ENSURE_(
                            this->self_->named_marks_[i].name_ != name,
                            error_badmark,
            ...
            detail::sequence<BidiIter> parse_charset(FwdIter& begin, FwdIter end)
            {
                ...
                return detail::make_charset_xpression<BidiIter>(chset, this->rxtraits(), this->traits_.flags());
            }
            ...
            detail::sequence<BidiIter> parse_atom(FwdIter& begin, FwdIter end)
            {
                ...
                case token_literal:
                    return detail::make_literal_xpression<BidiIter>(
                        this->parse_literal(begin, end),
                        this->traits_.flags(),
                        this->rxtraits());

                case token_any: return detail::make_any_xpression<BidiIter>(this->traits_.flags(), this->rxtraits());

                case token_assert_begin_sequence: return detail::make_dynamic<BidiIter>(detail::assert_bos_matcher());

                case token_assert_end_sequence: return detail::make_dynamic<BidiIter>(detail::assert_eos_matcher());
                ...
                case token_escape:
                    ...
                    case detail::escape_char:
                        return detail::make_char_xpression<BidiIter>(esc.ch_, this->traits_.flags(), this->rxtraits());
                ...
                case token_group_begin: return this->parse_group(begin, end);

                case token_charset_begin: return this->parse_charset(begin, end);
                ...
                case token_quote_meta_begin:
                    return detail::make_literal_xpression<BidiIter>(
                        this->parse_quote_meta(begin, end),
                        this->traits_.flags(),
                        this->rxtraits());

                case token_quote_meta_end:
                    BOOST_THROW_EXCEPTION(
                        regex_error(error_escape, "found quote-meta end without corresponding quote-meta begin"));
                    break;

                case token_end_of_pattern: break;

                default: begin = old_begin; break;
                }
            ...
            string_type parse_literal(FwdIter& begin, FwdIter end)
            {
                ...
                for (FwdIter prev = begin, tmp = ++begin; begin != end; prev = begin, begin = tmp) {
                    ...
                    if (this->traits_.get_quant_spec(tmp, end, spec)) {
                        ...
                        switch (this->traits_.get_token(tmp, end)) {
                        case token_escape:
                            esc = this->parse_escape(tmp, end);
                            if (detail::escape_char != esc.type_) {
                                return literal;
                            }
                            literal.insert(literal.end(), esc.ch_);
                            break;
                        case token_literal: literal.insert(literal.end(), *tmp++); break;
