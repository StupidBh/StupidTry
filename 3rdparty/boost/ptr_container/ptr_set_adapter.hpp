...
namespace boost {
    namespace ptr_container_detail {
        ...
        struct set_config
        {
            ...
            typedef BOOST_DEDUCED_TYPENAME VoidPtrSet::allocator_type allocator_type;
            ...
            typedef BOOST_DEDUCED_TYPENAME
                mpl::eval_if_c<Ordered, select_value_compare<VoidPtrSet>, mpl::identity<void>>::type value_compare;

            typedef value_compare key_compare;

            typedef BOOST_DEDUCED_TYPENAME mpl::eval_if_c<Ordered, mpl::identity<void>, select_hasher<VoidPtrSet>>::type
                hasher;

            typedef BOOST_DEDUCED_TYPENAME
                mpl::eval_if_c<Ordered, mpl::identity<void>, select_key_equal<VoidPtrSet>>::type key_equal;

            typedef BOOST_DEDUCED_TYPENAME
                mpl::if_c<Ordered, ordered_associative_container_tag, unordered_associative_container_tag>::type
                    container_type;

            typedef void_ptr_iterator<BOOST_DEDUCED_TYPENAME VoidPtrSet::iterator, Key> iterator;

            typedef void_ptr_iterator<BOOST_DEDUCED_TYPENAME VoidPtrSet::const_iterator, const Key> const_iterator;

            typedef void_ptr_iterator<
                BOOST_DEDUCED_TYPENAME
                    mpl::eval_if_c<Ordered, select_iterator<VoidPtrSet>, select_local_iterator<VoidPtrSet>>::type,
                Key>
                local_iterator;

            typedef void_ptr_iterator<
                BOOST_DEDUCED_TYPENAME
                    mpl::eval_if_c<Ordered, select_iterator<VoidPtrSet>, select_const_local_iterator<VoidPtrSet>>::type,
                const Key>
        ...
        public:
            ...
            iterator find(const key_type& x) { return iterator(this->base().find(const_cast<key_type*>(&x))); }

            const_iterator find(const key_type& x) const
            {
                return const_iterator(this->base().find(const_cast<key_type*>(&x)));
            }

            size_type count(const key_type& x) const { return this->base().count(const_cast<key_type*>(&x)); }

            iterator lower_bound(const key_type& x)
            {
                return iterator(this->base().lower_bound(const_cast<key_type*>(&x)));
            }

            const_iterator lower_bound(const key_type& x) const
            {
                return const_iterator(this->base().lower_bound(const_cast<key_type*>(&x)));
            }

            iterator upper_bound(const key_type& x)
            {
                return iterator(this->base().upper_bound(const_cast<key_type*>(&x)));
            }

            const_iterator upper_bound(const key_type& x) const
            {
                return const_iterator(this->base().upper_bound(const_cast<key_type*>(&x)));
            }
            ...
            iterator_range<iterator> equal_range(const key_type& x)
            {
                std::
                    pair<BOOST_DEDUCED_TYPENAME base_type::ptr_iterator, BOOST_DEDUCED_TYPENAME base_type::ptr_iterator>
                        p = this->base().equal_range(const_cast<key_type*>(&x));
        ...
        protected:
            size_type bucket(const key_type& key) const { return this->base().bucket(const_cast<key_type*>(&key)); }
        };
    ...
    class ptr_set_adapter :
        ...
        typedef ptr_container_detail::ptr_set_adapter_base<Key, VoidPtrSet, CloneAllocator, Ordered> base_type;
    ...
    public:
        ...
        template<class Comp>
        explicit ptr_set_adapter(const Comp& comp, const allocator_type& a) :
            base_type(comp, a)
        {
        ...
        ptr_set_adapter(SizeType n, const Hash& hash, const Pred& pred, const Allocator& a) :
        ...
        std::pair<iterator, bool> insert(key_type* x) // strong
        {
            ...
            std::pair<BOOST_DEDUCED_TYPENAME base_type::ptr_iterator, bool> res = this->base().insert(x);
            if (res.second) {
                ptr.release();
            }
        ...
        iterator insert(iterator where, key_type* x) // strong
        {
            ...
            auto_type ptr(x, *this);
            BOOST_DEDUCED_TYPENAME base_type::ptr_iterator res = this->base().insert(where.base(), x);
            if (*res == x) {
                ptr.release();
            }
            return iterator(res);
        ...
        BOOST_DEDUCED_TYPENAME boost::disable_if<ptr_container_detail::is_pointer_or_integral<Range>>::type
        ...
        bool transfer(
            BOOST_DEDUCED_TYPENAME PtrSetAdapter::iterator object,
        ...
        template<class PtrSetAdapter>
        size_type transfer(
            BOOST_DEDUCED_TYPENAME PtrSetAdapter::iterator first,
        ...
        BOOST_DEDUCED_TYPENAME
            boost::disable_if<boost::is_same<Range, BOOST_DEDUCED_TYPENAME PtrSetAdapter::iterator>, size_type>::type
    ...
    template<class Key, class VoidPtrMultiSet, class CloneAllocator = heap_clone_allocator, bool Ordered = true>
    ...
    public: // typedefs
        ...
        typedef Key key_type;
        typedef BOOST_DEDUCED_TYPENAME base_type::auto_type auto_type;
        typedef BOOST_DEDUCED_TYPENAME VoidPtrMultiSet::allocator_type allocator_type;
    ...
    public:
        ...
        template<class Comp>
        explicit ptr_multiset_adapter(const Comp& comp, const allocator_type& a) :
            base_type(comp, a)
        {
        }
        ...
        iterator insert(key_type* x) // strong
        {
            ...
            auto_type ptr(x, *this);
            BOOST_DEDUCED_TYPENAME base_type::ptr_iterator res = this->base().insert(x);
            ptr.release();
            return iterator(res);
        }
