...
namespace boost {
    ...
    namespace ptr_container_detail {
        ...
        struct dynamic_clone_deleter
        {
            ...
            dynamic_clone_deleter(Container& cont) :
                cont(&cont)
            {
            }
        ...
        public: // typedefs
            ...
            typedef BOOST_DEDUCED_TYPENAME container_type::difference_type difference_type;
            typedef BOOST_DEDUCED_TYPENAME container_type::size_type size_type;
            typedef BOOST_DEDUCED_TYPENAME Config::allocator_type allocator_type;
            typedef CloneAllocator clone_allocator_type;
            typedef ptr_container_detail::static_move_ptr<
                Ty_,
                BOOST_DEDUCED_TYPENAME
                    boost::mpl::if_c<is_clone_allocator_empty, static_deleter_type, dynamic_deleter_type>::type>
                auto_type;

        protected:
            typedef ptr_container_detail::scoped_deleter<reversible_ptr_container> scoped_deleter;
            typedef BOOST_DEDUCED_TYPENAME container_type::iterator ptr_iterator;
            typedef BOOST_DEDUCED_TYPENAME container_type::const_iterator ptr_const_iterator;
        ...
        private:
            ...
            void copy(const reversible_ptr_container& r) { this->copy(r.begin(), r.end()); }
            ...
            template<class ForwardIterator>
            void clone_back_insert(ForwardIterator first, ForwardIterator last)
            {
                BOOST_ASSERT(first != last);
            ...
            void remove_all() { this->remove(begin(), end()); }
        ...
        protected:
            ...
            {
                if (!allow_null) {
                    BOOST_PTR_CONTAINER_THROW_EXCEPTION(0 == x && "null not allowed", bad_pointer, msg);
                }
            }
        ...
        public: // foundation: should be protected, but public for poor compilers' sake.
            ...
            template<class SizeType>
            reversible_ptr_container(SizeType n, const allocator_type& a, fixed_length_sequence_tag) :
                c_(n, a)
            {
            }
            ...
            reversible_ptr_container(const Compare& comp, const allocator_type& a) :
                c_(comp, a)
            {
            }

            template<class ForwardIterator>
            reversible_ptr_container(ForwardIterator first, ForwardIterator last, fixed_length_sequence_tag) :
                c_(std::distance(first, last))
            {
                constructor_impl(first, last, std::forward_iterator_tag());
            }

            template<class SizeType, class InputIterator>
            reversible_ptr_container(SizeType n, InputIterator first, InputIterator last, fixed_length_sequence_tag) :
                c_(n)
            {
                constructor_impl(
                    first,
            ...
            template<class Compare>
            reversible_ptr_container(const Compare& comp, const allocator_type& a, associative_container_tag) :
                c_(comp, a)
            {
            }

            template<class InputIterator>
            reversible_ptr_container(InputIterator first, InputIterator last, associative_container_tag)
            {
            ...
            reversible_ptr_container(const Hash& h, const Pred& pred, const allocator_type& a) :
                c_(h, pred, a)
            {
            }
        ...
        public:
            ...
            clone_allocator_type& get_clone_allocator() { return static_cast<clone_allocator_type&>(*this); }
        ...
        public: // container requirements
            ...
            const_iterator cend() const { return const_iterator(c_.end()); }
        ...
        public: // access interface
            ...
            auto_type replace(size_type idx, Ty_* x) // strong
            {
                ...
                auto_type ptr(x, *this);
                BOOST_PTR_CONTAINER_THROW_EXCEPTION(idx >= size(), bad_index, "'replace()' out of bounds");
...
#else
    #define BOOST_PTR_CONTAINER_DEFINE_RELEASE(base_type) using base_type::release;
#endif
...
#define BOOST_PTR_CONTAINER_DEFINE_CONSTRUCTORS(PC, base_type)                 \
    ...
    explicit PC(const allocator_type& a) :                                     \
        base_type(a)                                                           \
    {                                                                          \
    }                                                                          \
    template<class InputIterator>                                              \
    ...
    template<class InputIterator>                                              \
    PC(InputIterator first, InputIterator last, const allocator_type& a) :     \
