...
namespace boost {
    ...
    class ptr_sequence_adapter :
        public ptr_container_detail::
            reversible_ptr_container<ptr_container_detail::sequence_config<T, VoidPtrSeq>, CloneAllocator> {
    ...
    public:
        ...
        template<class U>
        ptr_sequence_adapter(
            const ptr_sequence_adapter<U, VoidPtrSeq, CloneAllocator>& r,
            ptr_container_detail::fixed_length_sequence_tag tag) :
            base_type(r, tag)
        {
        }
        ...
        auto_type pop_back()
        {
            BOOST_ASSERT(!this->empty() && "'pop_back()' on empty container");
        ...
        auto_type pop_front()
        {
            BOOST_ASSERT(!this->empty() && "'pop_front()' on empty container");
        ...
        reference front()
        {
            BOOST_ASSERT(!this->empty() && "accessing 'front()' on empty container");

            BOOST_ASSERT(!::boost::is_null(this->begin()));
            return *this->begin();
        }

        const_reference front() const { return const_cast<ptr_sequence_adapter*>(this)->front(); }

        reference back()
        {
            BOOST_ASSERT(!this->empty() && "accessing 'back()' on empty container");
            BOOST_ASSERT(!::boost::is_null(--this->end()));
            return *--this->end();
        }

        const_reference back() const { return const_cast<ptr_sequence_adapter*>(this)->back(); }
    ...
    public:                               // deque/vector inerface
        ...
        {
            BOOST_PTR_CONTAINER_THROW_EXCEPTION(n >= this->size(), bad_index, "'at()' out of bounds");
            BOOST_ASSERT(!this->is_null(n));
            return (*this)[n];
        }
    ...
    public: // vector interface
        ...
        void reserve(size_type n) { this->base().reserve(n); }

        void reverse() { this->base().reverse(); }
    ...
    public:
        ...
        {
            insert_impl(before, first, last, BOOST_DEDUCED_TYPENAME iterator_category<InputIterator>::type());
        }
            ...
            PtrSeqAdapter& from) // strong
        {
            ...
            this->base().insert(before.base(), first.base(), last.base()); // strong
        ...
        BOOST_DEDUCED_TYPENAME
            boost::disable_if<boost::is_same<Range, BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator>>::type
        ...
        void transfer(iterator before, PtrSeqAdapter& from) // strong
        {
            ...
            this->base().insert(before.base(), from.begin().base(), from.end().base()); // strong
            from.base().clear();                                                        // nothrow
        }

    public:                                                                                       // C-array support
        void transfer(iterator before, value_type* from, size_type size, bool delete_from = true) // strong
        {
            BOOST_ASSERT(from != 0);
            if (delete_from) {
                BOOST_DEDUCED_TYPENAME base_type::scoped_deleter deleter(*this, from, size); // nothrow
    ...
    public:                         // resize
        void resize(size_type size) // basic
        {
            ...
            if (old_size > size) {
                ...
                for (; old_size != size; ++old_size) {
                    this->push_back(new BOOST_DEDUCED_TYPENAME boost::remove_pointer<value_type>::type());
                }
            }
        ...
        void rresize(size_type size) // basic
        {
            size_type old_size = this->size();
            if (old_size > size) {
                this->erase(this->begin(), boost::next(this->begin(), old_size - size));
            }
            else if (size > old_size) {
                for (; old_size != size; ++old_size) {
                    this->push_front(new BOOST_DEDUCED_TYPENAME boost::remove_pointer<value_type>::type());
                }
            }
    ...
    public: // algorithms
        void sort(iterator first, iterator last) { sort(first, last, std::less<T>()); }
        ...
        void sort(iterator first, iterator last, Compare comp)
        {
            ...
            std::sort(first.base(), last.base(), void_ptr_indirect_fun<Compare, T>(comp));
        }
        ...
        void unique(iterator first, iterator last) { unique(first, last, std::equal_to<T>()); }

        void unique() { unique(this->begin(), this->end()); }
    ...
    private:
        ...
        {
            typename base_type::ptr_iterator p = std::stable_partition(first.base(), last.base(), is_not_zero_ptr());
            this->base().erase(p, this->end().base());
        }

        void range_check_impl(iterator, iterator, std::bidirectional_iterator_tag) { /* do nothing */ }

        void range_check_impl(iterator first, iterator last, std::random_access_iterator_tag)
        {
        ...
        {
            range_check_impl(first, last, BOOST_DEDUCED_TYPENAME iterator_category<iterator>::type());
        }
    ...
    public:
        ...
        void erase_if(iterator first, iterator last, Pred pred)
        {
            range_check(first, last);
            this->base().erase(
                std::remove_if(first.base(), last.base(), void_ptr_delete_if<Pred, value_type>(pred)),
                last.base());
        }
        ...
        void merge(iterator first, iterator last, ptr_sequence_adapter& from, BinPred pred)
        {
            ...
            std::inplace_merge(this->begin().base(), middle, this->end().base(), bin_pred);
        }
