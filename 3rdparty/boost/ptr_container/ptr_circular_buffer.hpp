...
namespace boost {
    ...
    class ptr_circular_buffer :
        public ptr_sequence_adapter<
            T,
            boost::circular_buffer<typename ptr_container_detail::void_ptr<T>::type, Allocator>,
            CloneAllocator> {
        typedef ptr_sequence_adapter<
            T,
            boost::circular_buffer<typename ptr_container_detail::void_ptr<T>::type, Allocator>,
            CloneAllocator>
            base_type;

        typedef boost::circular_buffer<typename ptr_container_detail::void_ptr<T>::type, Allocator>
    ...
    public: // constructors
        ...
        ptr_circular_buffer(const ptr_circular_buffer& r) :
            base_type(r.size(), r.begin(), r.end(), ptr_container_detail::fixed_length_sequence_tag())
        {
        }
        ...
        BOOST_PTR_CONTAINER_DEFINE_RELEASE_AND_CLONE(ptr_circular_buffer, base_type, this_type)

    public: // allocators
        allocator_type& get_allocator() { return this->base().get_allocator(); }
    ...
    public:                     // circular buffer functions
        ...
        {
            if (this->size() > new_capacity) {
                this->erase(this->begin(), this->begin() + (this->size() - new_capacity));
            }
        ...
        template<class InputIterator>
        void assign(capacity_type capacity, InputIterator first, InputIterator last) // basic
        {
        ...
        iterator rerase(iterator first, iterator last) // nothrow
        {
            ...
            return iterator(this->base().rerase(first.base(), last.base()));
        }
    ...
    public: // transfer
            ...
            PtrSeqAdapter& from) // nothrow
        {
            ...
            for (BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator begin = first; begin != last; ++begin, ++before) {
    ...
    public:                                                                                       // C-array support
        void transfer(iterator before, value_type* from, size_type size, bool delete_from = true) // nothrow
        {
            BOOST_ASSERT(from != 0);
            if (delete_from) {
