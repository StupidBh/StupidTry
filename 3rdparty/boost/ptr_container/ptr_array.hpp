...
namespace boost {
    ...
    namespace ptr_container_detail {
        ...
        public:
            ...
            ptr_array_impl(Allocator /*a*/ = Allocator()) { this->assign(0); }

            ptr_array_impl(size_t, T*, Allocator /*a*/ = Allocator()) { this->assign(0); }
        };
    }
    ...
    class ptr_array :
        public ptr_sequence_adapter<
            T,
            ptr_container_detail::ptr_array_impl<typename ptr_container_detail::void_ptr<T>::type, N>,
            CloneAllocator> {
    private:
        ...
        typedef ptr_array<T, N, CloneAllocator> this_type;
    ...
    public: // constructors
        ...
        ptr_array(const ptr_array& r)
        {
            size_t i = 0;
            for (; i != N; ++i) {
                this->base()[i] = this->null_policy_allocate_clone(static_cast<const U*>(&r[i]));
            }
        }
        ...
        ptr_array(const ptr_array<U, N>& r)
        {
            size_t i = 0;
            for (; i != N; ++i) {
                this->base()[i] = this->null_policy_allocate_clone(static_cast<const T*>(&r[i]));
            }
        }
    ...
    public:                     // compile-time interface
        ...
        auto_type replace(size_t idx, U* r) // strong
        {
            ...
            BOOST_PTR_CONTAINER_THROW_EXCEPTION(idx >= N, bad_index, "'replace()' aout of bounds");
        ...
        bool is_null(size_t idx) const { return base_class::is_null(idx); }
