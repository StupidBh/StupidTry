...
namespace boost {
    namespace ptr_container_detail {
        ...
        struct map_config
        {
            ...
            typedef BOOST_DEDUCED_TYPENAME
                mpl::eval_if_c<Ordered, select_value_compare<VoidPtrMap>, mpl::identity<void>>::type value_compare;

            typedef BOOST_DEDUCED_TYPENAME
                mpl::eval_if_c<Ordered, select_key_compare<VoidPtrMap>, mpl::identity<void>>::type key_compare;

            typedef BOOST_DEDUCED_TYPENAME mpl::eval_if_c<Ordered, mpl::identity<void>, select_hasher<VoidPtrMap>>::type
                hasher;

            typedef BOOST_DEDUCED_TYPENAME
                mpl::eval_if_c<Ordered, mpl::identity<void>, select_key_equal<VoidPtrMap>>::type key_equal;

            typedef BOOST_DEDUCED_TYPENAME mpl::if_c<
                Ordered,
                ptr_container_detail::ordered_associative_container_tag,
                ptr_container_detail::unordered_associative_container_tag>::type container_type;
            ...
            typedef ptr_map_iterator<
                BOOST_DEDUCED_TYPENAME
                    mpl::eval_if_c<Ordered, select_iterator<VoidPtrMap>, select_local_iterator<VoidPtrMap>>::type,
                key_type,
                U* const>
                local_iterator;
            ...
            typedef ptr_map_iterator<
                BOOST_DEDUCED_TYPENAME
                    mpl::eval_if_c<Ordered, select_iterator<VoidPtrMap>, select_const_local_iterator<VoidPtrMap>>::type,
                key_type,
                const U* const>
        ...
        class ptr_map_adapter_base :
            public ptr_container_detail::associative_ptr_container<map_config<T, VoidPtrMap, Ordered>, CloneAllocator> {
            typedef ptr_container_detail::associative_ptr_container<map_config<T, VoidPtrMap, Ordered>, CloneAllocator>
                base_type;
        ...
        public:
            ...
            explicit ptr_map_adapter_base(SizeType n, ptr_container_detail::unordered_associative_container_tag tag) :
                base_type(n, tag)
            {
            }
            ...
            iterator find(const key_type& x) { return iterator(this->base().find(x)); }

            const_iterator find(const key_type& x) const { return const_iterator(this->base().find(x)); }

            size_type count(const key_type& x) const { return this->base().count(x); }

            iterator lower_bound(const key_type& x) { return iterator(this->base().lower_bound(x)); }

            const_iterator lower_bound(const key_type& x) const { return const_iterator(this->base().lower_bound(x)); }

            iterator upper_bound(const key_type& x) { return iterator(this->base().upper_bound(x)); }

            const_iterator upper_bound(const key_type& x) const { return const_iterator(this->base().upper_bound(x)); }

            iterator_range<iterator> equal_range(const key_type& x)
            {
                std::
                    pair<BOOST_DEDUCED_TYPENAME base_type::ptr_iterator, BOOST_DEDUCED_TYPENAME base_type::ptr_iterator>
            ...
            mapped_reference at(const key_type& key) { return const_cast<mapped_reference>(lookup(key)); }

            const_mapped_reference at(const key_type& key) const { return lookup(key); }

            mapped_reference operator[](const key_type& key) { return insert_lookup(key); }
        ...
        protected:
            size_type bucket(const key_type& key) const { return this->base().bucket(key); }
        };
    ...
    template<class T, class VoidPtrMap, class CloneAllocator = heap_clone_allocator, bool Ordered = true>
    class ptr_map_adapter : public ptr_container_detail::ptr_map_adapter_base<T, VoidPtrMap, CloneAllocator, Ordered> {
        typedef ptr_container_detail::ptr_map_adapter_base<T, VoidPtrMap, CloneAllocator, Ordered> base_type;
    ...
    private:
        ...
        {
            std::pair<BOOST_DEDUCED_TYPENAME base_type::ptr_iterator, bool> res =
        ...
        void map_basic_clone_and_insert(II first, II last)
        {
            while (first != last) {
                if (this->find(first->first) == this->end()) {
                    ...
                    // strong
                    this->safe_insert(p.first, boost::ptr_container::move(ptr));
                    // strong, commit
                }
                ++first;
            }
    ...
    public:
        ...
        template<class Comp>
        explicit ptr_map_adapter(const Comp& comp, const allocator_type& a) :
            base_type(comp, a)
        {
        }

        template<class SizeType>
        explicit ptr_map_adapter(SizeType n, ptr_container_detail::unordered_associative_container_tag tag) :
            base_type(n, tag)
        {
        }

        template<class Hash, class Pred, class Allocator>
        ptr_map_adapter(const Hash& hash, const Pred& pred, const Allocator& a) :
            base_type(hash, pred, a)
        {
        }
        ...
        ptr_map_adapter(const ptr_map_adapter& r) { map_basic_clone_and_insert(r.begin(), r.end()); }
...
#ifndef BOOST_NO_AUTO_PTR
        template<class U>
        ptr_map_adapter(std::auto_ptr<U> r) :
            base_type(r)
        {
        }
#endif
#ifndef BOOST_NO_CXX11_SMART_PTR
        template<class U>
        ptr_map_adapter(std::unique_ptr<U> r) :
    ...
    private:
        std::pair<iterator, bool> insert_impl(const key_type& key, mapped_type x) // strong
        {
            ...
            std::pair<BOOST_DEDUCED_TYPENAME base_type::ptr_iterator, bool> res =
                this->base().insert(std::make_pair(key, x));        // strong, commit
            if (res.second) {                                       // nothrow
                ptr.release();                                      // nothrow
            }
        ...
        iterator insert_impl(iterator before, const key_type& key, mapped_type x) // strong
        {
            ...
            auto_type ptr(x, *this); // nothrow
            BOOST_DEDUCED_TYPENAME base_type::ptr_iterator res =
                this->base().insert(before.base(), std::make_pair(key, x));
            // strong, commit
            ptr.release(); // notrow
            return iterator(res);
        }
    ...
    public:
        std::pair<iterator, bool> insert(key_type& key, mapped_type x) { return insert_impl(key, x); }
        ...
        {
            this->enforce_null_policy(p.second, "Null pointer in 'ptr_map_adapter::insert()'");

            auto_type ptr(this->null_policy_allocate_clone(p.second), *this);
            BOOST_DEDUCED_TYPENAME base_type::ptr_iterator result =
                this->base().insert(before.base(), std::make_pair(p.first, ptr.get())); // strong
        ...
        bool transfer(
            BOOST_DEDUCED_TYPENAME PtrMapAdapter::iterator object,
        ...
        template<class PtrMapAdapter>
        size_type transfer(
            BOOST_DEDUCED_TYPENAME PtrMapAdapter::iterator first,
        ...
        BOOST_DEDUCED_TYPENAME
            boost::disable_if<boost::is_same<Range, BOOST_DEDUCED_TYPENAME PtrMapAdapter::iterator>, size_type>::type
    ...
    class ptr_multimap_adapter :
        ...
        typedef ptr_container_detail::ptr_map_adapter_base<T, VoidPtrMultiMap, CloneAllocator, Ordered> base_type;
    ...
    private:
        ...
        void map_basic_clone_and_insert(II first, II last)
        {
            while (first != last) {
                ...
                // strong
                safe_insert(pair.first, boost::ptr_container::move(ptr));
                // strong, commit
                ++first;
            }
        }
    ...
    public:
        ...
        template<class Comp>
        explicit ptr_multimap_adapter(const Comp& comp, const allocator_type& a) :
            base_type(comp, a)
        {
        }
        ...
        ptr_multimap_adapter(const ptr_multimap_adapter& r) { map_basic_clone_and_insert(r.begin(), r.end()); }
...
#ifndef BOOST_NO_AUTO_PTR
        template<class U>
        explicit ptr_multimap_adapter(std::auto_ptr<U> r) :
            base_type(r)
        {
        }
#endif
#ifndef BOOST_NO_CXX11_SMART_PTR
        template<class U>
        explicit ptr_multimap_adapter(std::unique_ptr<U> r) :
            base_type(std::move(r))
        {
        }
#endif
    ...
    private:
        iterator insert_impl(const key_type& key, mapped_type x) // strong
        {
            ...
            auto_type ptr(x, *this); // nothrow
            BOOST_DEDUCED_TYPENAME base_type::ptr_iterator res = this->base().insert(std::make_pair(key, x));
            // strong, commit
            ptr.release(); // notrow
            return iterator(res);
        }
    ...
    public:
        ...
        iterator insert(iterator before, ptr_container_detail::ref_pair<F, S> p) // strong
        {
            this->enforce_null_policy(p.second, "Null pointer in 'ptr_multimap_adapter::insert()'");
            iterator res = insert_impl(before, p.first, this->null_policy_allocate_clone(p.second));
            return res;
        }
