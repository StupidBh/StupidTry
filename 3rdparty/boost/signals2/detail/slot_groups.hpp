...
namespace boost {
    namespace signals2 {
        namespace detail {
            enum slot_meta_group
            {
                front_ungrouped_slots,
                grouped_slots,
                back_ungrouped_slots
            };
            ...
            public:
                ...
                group_key_less(const GroupCompare& group_compare) :
                    _group_compare(group_compare)
                {
                }

                bool operator()(
                    const typename group_key<Group>::type& key1,
                    const typename group_key<Group>::type& key2) const
                {
            ...
            private:
                ...
                typedef std::map<typename group_key<Group>::type, typename list_type::iterator, group_key_compare_type>
                    map_type;
            ...
            public:
                ...
                grouped_list(const grouped_list& other) :
                    _list(other._list),
                    _group_map(other._group_map),
                    _group_key_compare(other._group_key_compare)
                {
                    ...
                    for (other_map_it = other._group_map.begin(); other_map_it != other._group_map.end();
                        ...
                        ++other_next_map_it;
                        typename list_type::const_iterator other_next_list_it =
                            other.get_list_iterator(other_next_map_it);
                ...
                {
                    map_iterator map_it;
                    if (key.first == front_ungrouped_slots) { // optimization
                ...
                {
                    map_iterator map_it;
                    if (key.first == back_ungrouped_slots) { // optimization
            ...
            private:
                ...
                {
                    if (_group_key_compare(arg1, arg2)) {
                        return false;
                    }
                    if (_group_key_compare(arg2, arg1)) {
                        return false;
                    }
                    return true;
                }
                ...
                void m_insert(const map_iterator& map_it, const group_key_type& key, const ValueType& value)
                {
                    ...
                    if (lower_bound_it == _group_map.end() || weakly_equivalent(lower_bound_it->first, key) == false) {
