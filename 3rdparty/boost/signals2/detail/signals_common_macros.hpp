...
#include <boost/config.hpp>
    ...
    #define BOOST_SIGNALS2_SIGNATURE_FULL_ARGS(arity) BOOST_PP_ENUM(arity, BOOST_SIGNALS2_SIGNATURE_FULL_ARG, ~)
    // arg1, arg2, ..., argn
    #define BOOST_SIGNALS2_SIGNATURE_ARG_NAMES(arity) BOOST_PP_ENUM(arity, BOOST_SIGNALS2_SIGNATURE_ARG_NAME, ~)
    // T1, T2, ..., TN
    #define BOOST_SIGNALS2_ARGS_TEMPLATE_INSTANTIATION(arity) BOOST_PP_ENUM_SHIFTED_PARAMS(BOOST_PP_INC(arity), T)
    // R (T1, T2, ..., TN)
    #define BOOST_SIGNALS2_SIGNATURE_FUNCTION_TYPE(arity) R(BOOST_SIGNALS2_ARGS_TEMPLATE_INSTANTIATION(arity))
    ...
    #define BOOST_SIGNALS2_ARGS_TEMPLATE_DECL(arity) BOOST_PP_ENUM_SHIFTED_PARAMS(BOOST_PP_INC(arity), typename T)
    // prefixR, prefixT1, prefixT2, ..., prefixTN
    #define BOOST_SIGNALS2_PREFIXED_SIGNATURE_TEMPLATE_INSTANTIATION(arity, prefix) \
        BOOST_PP_CAT(prefix, R)                                                     \
        BOOST_PP_COMMA_IF(arity) BOOST_PP_ENUM_SHIFTED_PARAMS(BOOST_PP_INC(arity), BOOST_PP_CAT(prefix, T))
    // R, T1, T2, ..., TN
    ...
    #define BOOST_SIGNALS2_FUNCTION_N_DECL(arity) \
        BOOST_PP_CAT(boost::function, arity)<BOOST_SIGNALS2_SIGNATURE_TEMPLATE_INSTANTIATION(arity)>
    ...
    #define BOOST_SIGNALS2_EXT_FUNCTION_N_DECL(arity) \
        BOOST_PP_CAT(boost::function, BOOST_PP_INC(arity))<BOOST_SIGNALS2_EXT_SLOT_TEMPLATE_INSTANTIATION(arity)>
    // slotN
    ...
    #define BOOST_SIGNALS2_BOUND_EXTENDED_SLOT_FUNCTION_INVOKER_N(arity) \
        BOOST_PP_CAT(bound_extended_slot_function_invoker, arity)
    ...
    #define BOOST_SIGNALS2_PORTABLE_SIGNATURE(arity, Signature)                   \
        typename function_traits<Signature>::result_type BOOST_PP_COMMA_IF(arity) \
    ...
    #define BOOST_SIGNALS2_FULL_FORWARD_ARGS(arity) BOOST_PP_ENUM(arity, BOOST_SIGNALS2_FULL_CREF_ARG, ~)
    #define BOOST_SIGNALS2_FORWARDED_ARGS(arity)    BOOST_SIGNALS2_SIGNATURE_ARG_NAMES(arity)
    ...
    #define BOOST_SIGNALS2_SLOT_TEMPLATE_SPECIALIZATION_DECL(arity) \
        BOOST_SIGNALS2_SIGNATURE_TEMPLATE_DECL(arity), typename SlotFunction
    ...
    #define BOOST_SIGNALS2_SIGNAL_TEMPLATE_DECL(arity)                                                           \
        BOOST_SIGNALS2_SIGNATURE_TEMPLATE_DECL(arity), typename Combiner, typename Group, typename GroupCompare, \
    ...
    #define BOOST_SIGNALS2_SIGNAL_TEMPLATE_DEFAULTED_DECL(arity)                                \
        BOOST_SIGNALS2_SIGNATURE_TEMPLATE_DECL(arity),                                          \
            typename Combiner = optional_last_value<R>, typename Group = int,                   \
    ...
    #define BOOST_SIGNALS2_SLOT_TEMPLATE_SPECIALIZATION_DECL(arity) typename SlotFunction, typename R, typename... Args
    #define BOOST_SIGNALS2_SLOT_TEMPLATE_SPECIALIZATION             <R (Args...), SlotFunction>

    #define BOOST_SIGNALS2_SIGNAL_TEMPLATE_DECL(arity)                                                       \
        typename Signature, typename Combiner, typename Group, typename GroupCompare, typename SlotFunction, \
        ...
        typename Signature,                                                                                   \
            typename Combiner = optional_last_value<typename boost::function_traits<Signature>::result_type>, \
                     ...
                     typename ExtendedSlotFunction =                                                          \
                         typename detail::variadic_extended_signature<Signature>::function_type,              \
                     typename Mutex = signals2::mutex
    #define BOOST_SIGNALS2_SIGNAL_TEMPLATE_SPECIALIZATION_DECL(arity)                    \
        typename Combiner, typename Group, typename GroupCompare, typename SlotFunction, \
            typename ExtendedSlotFunction, typename Mutex, typename R, typename... Args
    #define BOOST_SIGNALS2_SIGNAL_TEMPLATE_SPECIALIZATION \
        <\
  R (Args...), \
  Combiner, \
  ...
  Mutex>
    ...
    #define BOOST_SIGNALS2_STD_FUNCTIONAL_BASE std_functional_base<Args...>
