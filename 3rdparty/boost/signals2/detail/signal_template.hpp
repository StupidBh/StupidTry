...
#define BOOST_SIGNALS2_SIGNAL_TEMPLATE_INSTANTIATION                                                         \
    BOOST_SIGNALS2_SIGNATURE_TEMPLATE_INSTANTIATION(BOOST_SIGNALS2_NUM_ARGS), Combiner, Group, GroupCompare, \
...
namespace boost {
        ...
        namespace detail {
            ...
            public:
                ...
                result_type operator()(
                    ExtendedSlotFunction& func,
                    const connection& conn BOOST_SIGNALS2_PP_COMMA_IF(BOOST_SIGNALS2_NUM_ARGS)
                        BOOST_SIGNALS2_FULL_FORWARD_ARGS(BOOST_SIGNALS2_NUM_ARGS)) const
                {
                    return func(
                        conn BOOST_SIGNALS2_PP_COMMA_IF(BOOST_SIGNALS2_NUM_ARGS)
            ...
            public:
                ...
                BOOST_SIGNALS2_BOUND_EXTENDED_SLOT_FUNCTION_N(BOOST_SIGNALS2_NUM_ARGS)(
                    const ExtendedSlotFunction& fun) :
                    _fun(fun),
                ...
                {
                    return BOOST_SIGNALS2_BOUND_EXTENDED_SLOT_FUNCTION_INVOKER_N(
                        BOOST_SIGNALS2_NUM_ARGS)<typename ExtendedSlotFunction::result_type>()(
                        _fun,
                        *_connection BOOST_SIGNALS2_PP_COMMA_IF(BOOST_SIGNALS2_NUM_ARGS)
                            BOOST_SIGNALS2_FORWARDED_ARGS(BOOST_SIGNALS2_NUM_ARGS));
                }
            ...
            public:
                ...
                // typedef slotN+1<R, const connection &, T1, T2, ..., TN, extended_slot_function_type>
                // extended_slot_type;
            ...
            private:
                ...
                typedef BOOST_SIGNALS2_BOUND_EXTENDED_SLOT_FUNCTION_N(
                    BOOST_SIGNALS2_NUM_ARGS)<extended_slot_function_type> bound_extended_slot_function_type;
            ...
            public:
                ...
                typedef typename detail::slot_call_iterator_t<
                    slot_invoker,
                    typename connection_list_type::iterator,
                    connection_body<group_key_type, slot_type, Mutex>>
                    slot_call_iterator;
                typedef detail::BOOST_SIGNALS2_WEAK_SIGNAL_CLASS_NAME(
                    BOOST_SIGNALS2_NUM_ARGS)<BOOST_SIGNALS2_SIGNAL_TEMPLATE_INSTANTIATION> weak_signal_type;

                BOOST_SIGNALS2_SIGNAL_IMPL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS)(
                    const combiner_type& combiner_arg,
                    const group_compare_type& group_compare) :
                    _shared_state(
                        boost::make_shared<invocation_state>(connection_list_type(group_compare), combiner_arg)),
                ...
                void disconnect_all_slots()
                {
                    ...
                    for (it = local_state->connection_bodies().begin(); it != local_state->connection_bodies().end();
                         ++it) {
                        (*it)->disconnect();
                    }
                }
                ...
                void disconnect(const group_type& group)
                {
                    ...
                    for (it = local_state->connection_bodies().lower_bound(group_key); it != end_it; ++it) {
                        (*it)->disconnect();
                    }
                }
                ...
                result_type operator()(BOOST_SIGNALS2_SIGNATURE_FULL_ARGS(BOOST_SIGNALS2_NUM_ARGS))
                {
                    ...
                    return detail::combiner_invoker<typename combiner_type::result_type>()(
                        ...
                        slot_call_iterator(
                            local_state->connection_bodies().begin(),
                            local_state->connection_bodies().end(),
                            cache),
                        slot_call_iterator(
                            local_state->connection_bodies().end(),
                            local_state->connection_bodies().end(),
    ...
    #define BOOST_SIGNALS2_ADD_REF_TYPE(z, n, data) typename add_reference<BOOST_PP_CAT(T, BOOST_PP_INC(n))>::type
    ...
    // typename add_reference<T1>::type arg1, typename add_reference<T2>::type arg2, ..., typename
    // add_reference<Tn>::type argn
    #define BOOST_SIGNALS2_ADD_REF_ARGS(arity) BOOST_PP_ENUM(arity, BOOST_SIGNALS2_ADD_REF_ARG, ~)
                    slot_invoker(BOOST_SIGNALS2_ADD_REF_ARGS(BOOST_SIGNALS2_NUM_ARGS))
                        BOOST_PP_EXPR_IF(BOOST_SIGNALS2_NUM_ARGS, :)
    ...
    #define BOOST_SIGNALS2_M_ARG_NAMES(arity) BOOST_PP_ENUM(arity, BOOST_SIGNALS2_M_ARG_NAME, ~)
                    ...
                    result_type m_invoke(
                        const connection_body_type& connectionBody,
                    ...
                    {
                        return connectionBody->slot().slot_function()(
                            BOOST_SIGNALS2_M_ARG_NAMES(BOOST_SIGNALS2_NUM_ARGS));
                    }
                };
    ...
    #undef BOOST_SIGNALS2_M_ARG_NAME
       ...
       // inside operator()
                ...
                public:
                    invocation_state(const connection_list_type& connections_in, const combiner_type& combiner_in) :
                ...
                public:
                    ...
                    invocation_janitor(
                        ...
                        _cache(cache),
                        _sig(sig),
                        _connection_bodies(connection_bodies)
                    {
                    }
                ...
                void nolock_cleanup_connections_from(
                    garbage_collecting_lock<mutex_type>& lock,
                    bool grab_tracked,
                    const typename connection_list_type::iterator& begin,
                    unsigned count = 0) const
                {
                    BOOST_ASSERT(_shared_state.unique());
                    typename connection_list_type::iterator it;
                    unsigned i;
                    for (it = begin, i = 0; it != _shared_state->connection_bodies().end() && (count == 0 || i < count);
                         ++i) {
                        bool connected;
                        if (grab_tracked) {
                ...
                void nolock_cleanup_connections(
                    garbage_collecting_lock<mutex_type>& lock,
                    bool grab_tracked,
                ...
                void nolock_force_unique_connection_list(garbage_collecting_lock<mutex_type>& lock)
                {
                    if (_shared_state.unique() == false) {
                        _shared_state =
                            boost::make_shared<invocation_state>(*_shared_state, _shared_state->connection_bodies());
                ...
                connection_body_type
                    create_new_connection(garbage_collecting_lock<mutex_type>& lock, const slot_type& slot)
                {
                ...
                // connect slot
                connection nolock_connect(
                    garbage_collecting_lock<mutex_type>& lock,
                    const slot_type& slot,
                    connect_position position)
                {
                    connection_body_type newConnectionBody = create_new_connection(lock, slot);
        ...
        template<BOOST_SIGNALS2_SIGNAL_TEMPLATE_SPECIALIZATION_DECL(BOOST_SIGNALS2_NUM_ARGS)>
        class BOOST_SIGNALS2_SIGNAL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS) BOOST_SIGNALS2_SIGNAL_TEMPLATE_SPECIALIZATION :
            public signal_base,
            public detail::BOOST_SIGNALS2_STD_FUNCTIONAL_BASE {
            typedef detail::BOOST_SIGNALS2_SIGNAL_IMPL_CLASS_NAME(
                BOOST_SIGNALS2_NUM_ARGS)<BOOST_SIGNALS2_SIGNAL_TEMPLATE_INSTANTIATION> impl_class;

        public:
            typedef typename impl_class::weak_signal_type weak_signal_type;
            friend class detail::BOOST_SIGNALS2_WEAK_SIGNAL_CLASS_NAME(
            ...
            typedef Group group_type;
            typedef GroupCompare group_compare_type;
            typedef typename impl_class::slot_call_iterator slot_call_iterator;
            typedef typename mpl::identity<BOOST_SIGNALS2_SIGNATURE_FUNCTION_TYPE(BOOST_SIGNALS2_NUM_ARGS)>::type
                signature_type;
    ...
    #undef BOOST_SIGNALS2_MISC_STATEMENT
            ...
            template<unsigned n>
            class arg :
                public detail::BOOST_SIGNALS2_PREPROCESSED_ARG_N_TYPE_CLASS_NAME(
                    BOOST_SIGNALS2_NUM_ARGS)<n BOOST_SIGNALS2_PP_COMMA_IF(BOOST_SIGNALS2_NUM_ARGS)
            ...
            BOOST_SIGNALS2_SIGNAL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS)(
                const combiner_type& combiner_arg = combiner_type(),
        ...
        template<BOOST_SIGNALS2_SIGNAL_TEMPLATE_DECL(BOOST_SIGNALS2_NUM_ARGS)>
        void swap(
            BOOST_SIGNALS2_SIGNAL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS) < BOOST_SIGNALS2_SIGNAL_TEMPLATE_INSTANTIATION >
                &sig1,
            BOOST_SIGNALS2_SIGNAL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS) < BOOST_SIGNALS2_SIGNAL_TEMPLATE_INSTANTIATION >
                &sig2) BOOST_NOEXCEPT
        {
        ...
        namespace detail {
            ...
            class BOOST_SIGNALS2_WEAK_SIGNAL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS)
                BOOST_SIGNALS2_SIGNAL_TEMPLATE_SPECIALIZATION {
            public:
                typedef typename BOOST_SIGNALS2_SIGNAL_CLASS_NAME(
                    BOOST_SIGNALS2_NUM_ARGS)<BOOST_SIGNALS2_SIGNAL_TEMPLATE_INSTANTIATION>::result_type result_type;
                ...
                (const BOOST_SIGNALS2_SIGNAL_CLASS_NAME(BOOST_SIGNALS2_NUM_ARGS) <
                 BOOST_SIGNALS2_SIGNAL_TEMPLATE_INSTANTIATION > &signal) :
                ...
                result_type operator()(BOOST_SIGNALS2_SIGNATURE_FULL_ARGS(BOOST_SIGNALS2_NUM_ARGS))
                {
                    shared_ptr<
                        ...
                        BOOST_SIGNALS2_SIGNAL_TEMPLATE_INSTANTIATION> > shared_pimpl(_weak_pimpl.lock());
