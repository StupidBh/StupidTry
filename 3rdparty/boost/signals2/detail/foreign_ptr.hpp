...
namespace boost {
    ...
    namespace signals2 {
        ...
        template<typename T>
        struct weak_ptr_traits<boost::weak_ptr<T>>
        {
            typedef boost::shared_ptr<T> shared_type;
        };
#ifndef BOOST_NO_CXX11_SMART_PTR
        template<typename T>
        struct weak_ptr_traits<std::weak_ptr<T>>
        {
        ...
        template<typename T>
        struct shared_ptr_traits<boost::shared_ptr<T>>
        {
        ...
        namespace detail {
            ...
            public:
                foreign_shared_ptr_impl(const FSP& p) :
                    _p(p)
                {
                }

                virtual foreign_shared_ptr_impl* clone() const { return new foreign_shared_ptr_impl(*this); }
            ...
            public:
                ...
                ~foreign_void_shared_ptr() { delete _p; }
                ...
                void swap(foreign_void_shared_ptr& other) { boost::core::invoke_swap(_p, other._p); }
            ...
            public:
                foreign_weak_ptr_impl(const FWP& p) :
                    _p(p)
                {
                }

                virtual foreign_void_shared_ptr lock() const { return foreign_void_shared_ptr(_p.lock()); }

                virtual bool expired() const { return _p.expired(); }

                virtual foreign_weak_ptr_impl* clone() const { return new foreign_weak_ptr_impl(*this); }
            ...
            public:
                ...
                void swap(foreign_void_weak_ptr& other) { boost::core::invoke_swap(_p, other._p); }
