...
namespace boost {
    namespace signals2 {
        namespace detail {
            ...
            template<class T, class StackBufferPolicy, class GrowPolicy, class Allocator>
            class auto_buffer : Allocator {
            private:
                enum
                {
                    N = auto_buffer_detail::compute_buffer_objects < StackBufferPolicy,
                    T > ::value
                };

                BOOST_STATIC_CONSTANT(bool, is_stack_buffer_empty = N == 0u);

                typedef auto_buffer<T, store_n_objects<0>, GrowPolicy, Allocator> local_buffer;
            ...
            public:
                ...
                typedef typename boost::mpl::if_c<
                    boost::has_trivial_assign<T>::value && sizeof(T) <= sizeof(long double),
                    const value_type,
                    const_reference>::type optimized_const_reference;
            ...
            private:
                ...
                static void copy_rai(const T* begin, const T* end, pointer where, const boost::true_type&)
                {
                    std::memcpy(where, begin, sizeof(T) * std::distance(begin, end));
                }

                template<class I, bool b>
                static void copy_rai(I begin, I end, pointer where, const boost::integral_constant<bool, b>&)
                {
                ...
                {
                    copy_impl(begin, end, where, typename std::iterator_traits<I>::iterator_category());
                }
                ...
                void auto_buffer_destroy(pointer where, const boost::false_type&) { (*where).~T(); }
                ...
                void destroy_back_n(size_type n) { destroy_back_n(n, boost::has_trivial_destructor<T>()); }
                ...
                void auto_buffer_destroy(const boost::true_type&) { deallocate(buffer_, members_.capacity_); }

                pointer move_to_new_buffer(size_type new_capacity, const boost::false_type&)
                {
                    pointer new_buffer = allocate(new_capacity); // strong
                    scope_guard guard = make_obj_guard(*this, &auto_buffer::deallocate, new_buffer, new_capacity);
                ...
                {
                    pointer new_buffer = move_to_new_buffer(new_capacity, boost::has_nothrow_copy<T>());
                    auto_buffer_destroy();
                    buffer_ = new_buffer;
                ...
                static void swap_helper(auto_buffer& l, auto_buffer& r, const boost::true_type&)
                {
                ...
                static void swap_helper(auto_buffer& l, auto_buffer& r, const boost::false_type&)
                {
                    BOOST_ASSERT(l.is_on_stack() && r.is_on_stack());
                ...
                template<class I>
                void insert_impl(const_iterator before, I begin_arg, I end_arg, std::input_iterator_tag)
                {
                ...
                void grow_back(size_type n, const boost::false_type&) { unchecked_push_back_n(n); }

                void grow_back(size_type n) { grow_back(n, boost::has_trivial_constructor<T>()); }
                ...
                void grow_back_one(const boost::false_type&) { unchecked_push_back(); }

                void grow_back_one() { grow_back_one(boost::has_trivial_constructor<T>()); }

                template<class I>
                void insert_impl(const_iterator before, I begin_arg, I end_arg, std::forward_iterator_tag)
                {
            ...
            public:
                ...
                auto_buffer& operator=(const auto_buffer& r) // basic
                {
                    ...
                    if (diff >= 0) {
                        ...
                        if (members_.capacity_ >= r.size()) {
                            ...
                            scope_guard guard = make_obj_guard(*this, &auto_buffer::deallocate, new_buffer, r.size());
                ...
                auto_buffer(size_type size_arg, optimized_const_reference init_value, const allocator_type& a) :
                    allocator_type(a),
                    members_((std::max)(size_arg, size_type(N))),
                    buffer_(allocate(members_.capacity_)),
                    size_(0)
                ...
                template<class ForwardIterator>
                auto_buffer(ForwardIterator begin_arg, ForwardIterator end_arg, const allocator_type& a) :
                    allocator_type(a),
                    members_(std::distance(begin_arg, end_arg)),
                    buffer_(allocate(members_.capacity_)),
                    size_(0)
                ...
                ~auto_buffer() { auto_buffer_destroy(); }

            public:
                bool empty() const { return size_ == 0; }

                bool full() const { return size_ == members_.capacity_; }

                bool is_on_stack() const { return members_.capacity_ <= N; }

                size_type size() const { return size_; }

                size_type capacity() const { return members_.capacity_; }
            ...
            public:
                ...
                const_iterator cbegin() const { return const_cast<const auto_buffer*>(this)->begin(); }

                const_iterator cend() const { return const_cast<const auto_buffer*>(this)->end(); }

                const_reverse_iterator crbegin() const { return const_cast<const auto_buffer*>(this)->rbegin(); }

                const_reverse_iterator crend() const { return const_cast<const auto_buffer*>(this)->rend(); }

            public:
                reference front() { return buffer_[0]; }

                optimized_const_reference front() const { return buffer_[0]; }

                reference back() { return buffer_[size_ - 1]; }

                optimized_const_reference back() const { return buffer_[size_ - 1]; }
                ...
                void insert(const_iterator before, size_type n, optimized_const_reference x)
                {
                ...
                template<class ForwardIterator>
                void insert(const_iterator before, ForwardIterator begin_arg, ForwardIterator end_arg) // basic
                {
                ...
                {
                    BOOST_ASSERT(!(std::distance(from, to) > 0) || !empty());
                ...
                void swap(auto_buffer& r)
                {
                    ...
                    bool exactly_one_on_stack = (on_stack && !r_on_stack) || (!on_stack && r_on_stack);
            ...
            private:
                typedef boost::aligned_storage<N * sizeof(T), boost::alignment_of<T>::value> storage;
                ...
                struct members_type : storage /* to enable EBO */
                {
                    ...
                    void* address() const { return const_cast<storage&>(static_cast<const storage&>(*this)).address(); }
                };
            ...
            template<class T, class SBP, class GP, class A>
            inline bool operator==(const auto_buffer<T, SBP, GP, A>& l, const auto_buffer<T, SBP, GP, A>& r)
            {
                if (l.size() != r.size()) {
                    return false;
                }
