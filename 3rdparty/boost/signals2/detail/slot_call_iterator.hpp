    ...
    namespace signals2 {
        namespace detail {
            ...
            public:
                ...
                template<typename M>
                void set_active_slot(garbage_collecting_lock<M>& lock, connection_body_base* active_slot)
                {
                    if (m_active_slot) {
                        m_active_slot->dec_slot_refcount(lock);
                    }
            ...
            class slot_call_iterator_t :
                public boost::iterator_facade<
                    slot_call_iterator_t<Function, Iterator, ConnectionBody>,
            ...
            public:
                slot_call_iterator_t(Iterator iter_in, Iterator end_in, cache_type& c) :
                    iter(iter_in),
                    end(end_in),
                    cache(&c),
                    callable_iter(end_in)
                {
                    lock_next_callable();
                }
                ...
                bool equal(const slot_call_iterator_t& other) const { return iter == other.iter; }
