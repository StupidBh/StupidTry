...
namespace boost {
    namespace sort {
        namespace spin_detail {
            ...
            static void insert_partial_sort(
                Iter1_t first,
                Iter1_t mid,
                Iter1_t last,
                Compare comp,
            ...
            static bool check_stable_sort(const range<Iter1_t>& rng_data, const range<Iter2_t>& rng_aux, Compare comp);

            template<class Iter1_t, class Iter2_t, class Compare>
            static void
                range_sort(const range<Iter1_t>& range1, const range<Iter2_t>& range2, Compare comp, uint32_t level);

            template<class Iter1_t, class Iter2_t, class Compare>
            static void sort_range_sort(const range<Iter1_t>& rng_data, const range<Iter2_t>& rng_aux, Compare comp);
            ...
            static bool check_stable_sort(const range<Iter1_t>& rng_data, const range<Iter2_t>& rng_aux, Compare comp)
            {
                ...
                const size_t min_insert_partial_sort = ((ndata >> 3) < 33) ? 32 : (ndata >> 3);
                if (ndata < 2) {
                    return true;
                }
                ...
                for (Iter1_t it1 = rng_data.first; it2 != rng_data.last && (sw = !comp(*it2, *it1)); it1 = it2++)
                    ;
                if (sw) {
                    return true;
                }
                ...
                range_sort(const range<Iter1_t>& range1, const range<Iter2_t>& range2, Compare comp, uint32_t level)
            {
                ...
                if (range1.size() > 1024) {
                    if ((level & 1) == 0) {
                        if (check_stable_sort(range2, range1, comp)) {
                            return;
                        }
                    }
                    else {
                ...
                if (level < 2) {
                    ...
                    range_sort(range_it2(range2.first, range2.first + nelem1), range_input1, comp, level - 1);

                    range_sort(range_it2(range2.first + nelem1, range2.last), range_input2, comp, level - 1);
                };
            ...
            static void sort_range_sort(const range<Iter1_t>& rng_data, const range<Iter2_t>& rng_aux, Compare comp)
            {
                ...
                uint32_t nlevel = nbits64(((rng_data.size() + sort_min - 1) / sort_min) - 1);
                // assert (nlevel != 0);
            ...
            spinsort<Iter_t, Compare>::spinsort(Iter_t first, Iter_t last, Compare comp, value_t* paux, size_t naux) :
                ptr(paux),
                nptr(naux),
                construct(false),
                owner(false)
            {
                ...
                //------------------- check if sort ---------------------------------
                bool sw = true;
                for (Iter_t it1 = first, it2 = first + 1; it2 != last && (sw = !comp(*it2, *it1)); it1 = it2++)
                    ;
                if (sw) {
                    return;
                }
                ...
                if (ptr == nullptr) {
                    ptr = reinterpret_cast<value_t*>(std::malloc(nptr * sizeof(value_t)));
                ...
                if ((nlevel & 1) == 1) {
                    ...
                    //  parameter
                    //----------------------------------------------------------------
                    range_it range_1(first, first + nelem_1), range_2(first + nelem_1, last);
        ...
        template<class Iter_t, class Compare = compare_iter<Iter_t>>
        inline void indirect_spinsort(Iter_t first, Iter_t last, Compare comp = Compare())
        {
