    ...
    namespace sort {
        namespace spreadsort {
            namespace detail {
                ...
                template<typename T>
                inline unsigned rough_log_2_size(const T& input)
                {
                    unsigned result = 0;
                ...
                inline size_t get_min_count(unsigned log_range)
                {
                    ...
                    BOOST_STATIC_ASSERT(
                        max_finishing_splits >= max_splits && max_finishing_splits < (8 * sizeof(unsigned)));
                    ...
                    if (log_range < base_range) {
                        ...
                        for (unsigned offset = min_size; offset < log_range; offset += ++result)
                            ;
                    ...
                    unsigned bit_length = ((((max_splits - 1) + remainder) / max_splits) + base_iterations + min_size);
                    // Preventing overflow; this situation shouldn't occur
                ...
                inline RandomAccessIter* size_bins(
                    size_t* bin_sizes,
