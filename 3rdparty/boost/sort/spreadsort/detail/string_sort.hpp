    ...
    namespace sort {
        namespace spreadsort {
            namespace detail {
                ...
                inline void update_offset(RandomAccessIter first, RandomAccessIter finish, size_t& char_offset)
                {
                    ...
                    while (true) {
                        ...
                        do {
                            ...
                            if ((*curr).size() > char_offset) {
                                ...
                                if (memcmp(curr->data() + nextOffset, first->data() + nextOffset, step_byte_size) !=
                                    0) {
                                    if (step_size == 1) {
                                        char_offset = nextOffset;
                                        return;
                    ...
                    Get_length length)
                {
                    ...
                    while (true) {
                        ...
                        do {
                            // ignore empties, but if the nextOffset would exceed the length or
                            // not match, exit; we've found the last matching character
                            if (length(*curr) > char_offset &&
                                (length(*curr) <= (nextOffset + 1) ||
                ...
                struct offset_less_than
                {
                    ...
                    inline bool operator()(const Data_type& x, const Data_type& y) const
                    {
                        ...
                        for (size_t u = fchar_offset; u < minSize; ++u) {
                            ...
                            if (static_cast<Unsigned_char_type>(x[u]) != static_cast<Unsigned_char_type>(y[u])) {
                                return static_cast<Unsigned_char_type>(x[u]) < static_cast<Unsigned_char_type>(y[u]);
                            }
                        }
                    ...
                    size_t* bin_sizes)
                {
                    ...
                    update_offset<RandomAccessIter, Unsigned_char_type>(first, finish, char_offset);
                    ...
                    for (RandomAccessIter current = first; current != last; ++current) {
                        if ((*current).size() <= char_offset) {
                            ...
                            bin_sizes[static_cast<Unsigned_char_type>((*current)[char_offset]) + 1]++;
                        }
                    }
                    ...
                    for (RandomAccessIter current = *local_bin; current < next_bin_start; ++current) {
                        // empties belong in this bin
                        while ((*current).size() > char_offset) {
                            target_bin = bins + static_cast<Unsigned_char_type>((*current)[char_offset]);
                    ...
                    *local_bin = next_bin_start;
                    ...
                    for (unsigned u = 0; u < last_bin; ++u) {
                        ...
                        for (RandomAccessIter current = *local_bin; current < next_bin_start; ++current) {
                            ...
                            for (target_bin = bins + static_cast<Unsigned_char_type>((*current)[char_offset]);
                ...
                template<class RandomAccessIter, class Unsigned_char_type>
                inline void reverse_string_sort_rec(
                    RandomAccessIter first,
                    RandomAccessIter last,
                    size_t char_offset,
                    std::vector<RandomAccessIter>& bin_cache,
                    ...
                    size_t* bin_sizes)
                {
                    ...
                    // Offsetting on identical characters.  This section works
                    // a few characters at a time for optimal worst-case performance.
                    update_offset<RandomAccessIter, Unsigned_char_type>(curr, last, char_offset);
                    RandomAccessIter* target_bin;
                    ...
                    for (RandomAccessIter current = first; current != last; ++current) {
                        if ((*current).size() <= char_offset) {
                            ...
                            bin_sizes[max_bin - static_cast<Unsigned_char_type>((*current)[char_offset])]++;
                        }
                    }
                    // Assign the bin positions
                    bin_cache[cache_offset] = first;
                    ...
                    for (RandomAccessIter current = *local_bin; current < next_bin_start; ++current) {
                        // empties belong in this bin
                        while ((*current).size() > char_offset) {
                            target_bin = end_bin - static_cast<Unsigned_char_type>((*current)[char_offset]);
                            iter_swap(current, (*target_bin)++);
                        }
                    }
                    *local_bin = next_bin_start;
                    ...
                    for (unsigned u = 0; u < last_bin; ++u) {
                        ...
                        for (RandomAccessIter current = *local_bin; current < next_bin_start; ++current) {
                            // Swapping into place until the correct element has been swapped in
                            for (target_bin = end_bin - static_cast<Unsigned_char_type>((*current)[char_offset]);
                                 target_bin != local_bin;
                                 target_bin = end_bin - static_cast<Unsigned_char_type>((*current)[char_offset])) {
                                iter_swap(current, (*target_bin)++);
                            }
                    ...
                    Get_length length)
                {
                    ...
                    *local_bin = next_bin_start;
                    ...
                    for (unsigned ii = 0; ii < last_bin; ++ii) {
                        ...
                        for (RandomAccessIter current = *local_bin; current < next_bin_start; ++current) {
                            // Swapping into place until the correct element has been swapped in
                            for (target_bin = bins + get_character((*current), char_offset); target_bin != local_bin;
                                 target_bin = bins + get_character((*current), char_offset)) {
                                iter_swap(current, (*target_bin)++);
                            }
                        }
                    ...
                    for (unsigned u = cache_offset + 1; u < cache_offset + last_bin + 2; lastPos = bin_cache[u], ++u) {
                        ...
                        if (count < max_size) {
                            boost::sort::pdqsort(
                                lastPos,
                                bin_cache[u],
                                offset_char_less_than<Data_type, Get_char, Get_length>(char_offset + 1));
                        }
                        else {
                            string_sort_rec<RandomAccessIter, Unsigned_char_type, Get_char, Get_length>(
                                lastPos,
                                bin_cache[u],
                                char_offset + 1,
                                bin_cache,
                    ...
                    *local_bin = next_bin_start;
                    ...
                    for (unsigned u = cache_offset + 1; u < cache_offset + last_bin + 2; lastPos = bin_cache[u], ++u) {
                        ...
                        if (count < max_size) {
                            ...
                            string_sort_rec<RandomAccessIter, Unsigned_char_type, Get_char, Get_length, Compare>(
                                lastPos,
                                bin_cache[u],
                                char_offset + 1,
                                bin_cache,
                    ...
                    Compare comp)
                {
                    ...
                    *local_bin = next_bin_start;
                    ...
                    for (unsigned u = 0; u < last_bin; ++u) {
                        ...
                        for (RandomAccessIter current = *local_bin; current < next_bin_start; ++current) {
                            // Swapping into place until the correct element has been swapped in
                            for (target_bin = end_bin - get_character((*current), char_offset); target_bin != local_bin;
                                 target_bin = end_bin - get_character((*current), char_offset)) {
                                iter_swap(current, (*target_bin)++);
                            }
                        }
                ...
                inline typename boost::enable_if_c<sizeof(Unsigned_char_type) <= 2, void>::type
                    string_sort(RandomAccessIter first, RandomAccessIter last, Unsigned_char_type)
                {
                    size_t bin_sizes[(1 << (8 * sizeof(Unsigned_char_type))) + 1];
                    std::vector<RandomAccessIter> bin_cache;
                    string_sort_rec<RandomAccessIter, Unsigned_char_type>(first, last, 0, bin_cache, 0, bin_sizes);
                }
                ...
                template<class RandomAccessIter, class Unsigned_char_type>
                inline typename boost::enable_if_c<sizeof(Unsigned_char_type) <= 2, void>::type
                    reverse_string_sort(RandomAccessIter first, RandomAccessIter last, Unsigned_char_type)
                {
                    size_t bin_sizes[(1 << (8 * sizeof(Unsigned_char_type))) + 1];
                    std::vector<RandomAccessIter> bin_cache;
                    reverse_string_sort_rec<RandomAccessIter, Unsigned_char_type>(
                ...
                // Holds the bin vector and makes the initial recursive call
                template<class RandomAccessIter, class Get_char, class Get_length, class Unsigned_char_type>
                inline typename boost::enable_if_c<sizeof(Unsigned_char_type) <= 2, void>::type string_sort(
                    RandomAccessIter first,
                    RandomAccessIter last,
                    Get_char get_character,
                    Get_length length,
                ...
                template<class RandomAccessIter, class Get_char, class Get_length, class Unsigned_char_type>
                inline typename boost::disable_if_c<sizeof(Unsigned_char_type) <= 2, void>::type string_sort(
                    RandomAccessIter first,
                    RandomAccessIter last,
                    Get_char /* get_character */,
                    Get_length /* length */,
                ...
                template<
                    ...
                    class Compare,
                    class Unsigned_char_type>
                inline typename boost::enable_if_c<sizeof(Unsigned_char_type) <= 2, void>::type reverse_string_sort(
                    RandomAccessIter first,
                    RandomAccessIter last,
                    Get_char get_character,
                    Get_length length,
                    ...
                    Unsigned_char_type)
                {
                    size_t bin_sizes[(1 << (8 * sizeof(Unsigned_char_type))) + 1];
                    std::vector<RandomAccessIter> bin_cache;
                    reverse_string_sort_rec<RandomAccessIter, Unsigned_char_type, Get_char, Get_length, Compare>(
                        first,
                        last,
                        0,
                        bin_cache,
                ...
                template<
                    ...
                    class Compare,
                    class Unsigned_char_type>
                inline typename boost::disable_if_c<sizeof(Unsigned_char_type) <= 2, void>::type reverse_string_sort(
                    RandomAccessIter first,
                    RandomAccessIter last,
                    Get_char /* get_character */,
                    Get_length /* length */,
