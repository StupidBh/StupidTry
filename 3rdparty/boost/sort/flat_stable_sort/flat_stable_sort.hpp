...
namespace boost {
    namespace sort {
        namespace flat_internal {
            ...
            //----------------------------------------------------------------------------
            template<class Iter_t, typename Compare = bscu::compare_iter<Iter_t>, uint32_t Power2 = 10>
            ...
            public:
                ...
                //                   PUBLIC FUNCTIONS
                //-------------------------------------------------------------------------
                flat_stable_sort(Iter_t first, Iter_t last, Compare comp, circular_t* ptr_circ) :
            ...
            void flat_stable_sort<Iter_t, Compare, Power2>::divide(it_index itx_first, it_index itx_last)
            {
                ...
                if (nblock > 7) {
                    if (is_sorted_forward(itx_first, itx_last)) {
                        return;
                    }
                    if (is_sorted_backward(itx_first, itx_last)) {
                        return;
                    }
                };
            ...
            //------------------------------------------------------------------------
            template<class Iter_t, typename Compare, uint32_t Power2>
            void flat_stable_sort<Iter_t, Compare, Power2>::sort_small(it_index itx_first, it_index itx_last)
            {
                size_t nblock = size_t(itx_last - itx_first);
            ...
            //------------------------------------------------------------------------
            template<class Iter_t, typename Compare, uint32_t Power2>
            bool flat_stable_sort<Iter_t, Compare, Power2>::is_sorted_forward(it_index itx_first, it_index itx_last)
            {
                size_t nblock = size_t(itx_last - itx_first);
                ...
                size_t nsorted1 = bsc::number_stable_sorted_forward(rng.first, rng.last, min_process, cmp);
                if (nsorted1 == nelem) {
                    return true;
                }
                if (nsorted1 == 0) {
                ...
                if (nsorted2 <= (BLOCK_SIZE << 1)) {
                    ...
                    bscu::insert_sorted(rng.first, itaux, rng.last, cmp, ptr_circ->get_buffer());
                }
                else { // Adjust the size of the sorted data to a number of blocks
                    size_t mask = ~(BLOCK_SIZE - 1);
                    size_t nsorted1_adjust = nsorted1 & mask;
                    flat_stable_sort(rng.first + nsorted1_adjust, rng.last, cmp, ptr_circ);
            ...
            //------------------------------------------------------------------------
            template<class Iter_t, typename Compare, uint32_t Power2>
            bool flat_stable_sort<Iter_t, Compare, Power2>::is_sorted_backward(it_index itx_first, it_index itx_last)
            {
                size_t nblock = size_t(itx_last - itx_first);
                range_it rng = get_group_range(*itx_first, nblock);
                ...
                size_t nsorted2 = bsc::number_stable_sorted_backward(rng.first, rng.last, min_process, cmp);
                if (nsorted2 == nelem) {
                    return true;
                }
                if (nsorted2 == 0) {
                ...
                if (nsorted1 <= (BLOCK_SIZE << 1)) {
                    ...
                    bscu::insert_sorted_backward(rng.first, itaux, rng.last, cmp, ptr_circ->get_buffer());
                }
                else { // Adjust the size of nsorted2 for to be a number of blocks
                    size_t nblock1 = (nsorted1 + BLOCK_SIZE - 1) >> Power2;
                    size_t nsorted1_adjust = (nblock1 << Power2);
                    flat_stable_sort(rng.first, rng.first + nsorted1_adjust, cmp, ptr_circ);
                    merge_range_pos(itx_first, itx_first + nblock1, itx_last);
                };
                return true;
            }
        ...
        template<
            class Iter_t,
            class Compare = bscu::compare_iter<Iter_t>,
            bscu::enable_if_string<value_iter<Iter_t>>* = nullptr>
        inline void flat_stable_sort(Iter_t first, Iter_t last, Compare cmp = Compare())
        {
        ...
        {
            static constexpr const uint32_t BitsSize = (Size == 0) ? 0 : (Size > 128) ? 7 : bscu::tmsb[Size - 1];
            static constexpr const uint32_t sz[10] = { 10, 10, 10, 9, 8, 7, 6, 6 };
        ...
        inline void flat_stable_sort(Iter_t first, Iter_t last, Compare cmp = Compare())
        {
            flat::flat_stable_sort<Iter_t, Compare, block_size_fss<sizeof(value_iter<Iter_t>)>::data>(first, last, cmp);
        }

        template<class Iter_t, class Compare = compare_iter<Iter_t>>
        inline void indirect_flat_stable_sort(Iter_t first, Iter_t last, Compare comp = Compare())
        {
            typedef typename std::vector<Iter_t>::iterator itx_iter;
            typedef common::less_ptr_no_null<Iter_t, Compare> itx_comp;
            common::indirect_sort(flat_stable_sort<itx_iter, itx_comp>, first, last, comp);
        }
