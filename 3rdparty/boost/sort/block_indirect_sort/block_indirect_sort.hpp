...
namespace boost {
    namespace sort {
        namespace blk_detail {
            ...
            ///
            //----------------------------------------------------------------------------
            template<uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare = compare_iter<Iter_t>>
            struct block_indirect_sort
            {
                //------------------------------------------------------------------------
                ...
                block_indirect_sort(Iter_t first, Iter_t last) :
                    block_indirect_sort(first, last, Compare(), std::thread::hardware_concurrency())
                {
                }
            ...
            block_indirect_sort<Block_size, Group_size, Iter_t, Compare>::block_indirect_sort(
                Iter_t first,
                Iter_t last,
                Compare cmp,
                uint32_t nthr) :
                ...
                nthread(nthr)
            {
                try {
                    ...
                    //------------------- check if sort -----------------------------------
                    bool sorted = true;
                    for (Iter_t it1 = first, it2 = first + 1; it2 != last && (sorted = !bk.cmp(*it2, *it1));
                         it1 = it2++)
                        ;
                    if (sorted) {
                        return;
                    ...
                    sorted = true;
                    for (Iter_t it1 = first, it2 = first + 1; it2 != last && (sorted = !bk.cmp(*it1, *it2));
                         it1 = it2++)
                        ;
                    ...
                    if (nthread > nthreadmax) {
                        nthread = (uint32_t)nthreadmax;
                    }
                    ...
                    ptr = reinterpret_cast<value_t*>(std::malloc(Block_size * nthread * sizeof(value_t)));
                    if (ptr == nullptr) {
                        bk.error = true;
                        throw std::bad_alloc();
                    }
                ...
                uint32_t level_thread)
            {
                ...
                //-------------------------------------------------------------------------
                if (level_thread != 0) {
                    auto f1 = [this, &son_counter, pos_index_mid, pos_index2, level_thread]() {
            ...
            //-----------------------------------------------------------------------------
            template<uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare>
            void block_indirect_sort<Block_size, Group_size, Iter_t, Compare>::start_function(void)
            {
            ...
            ///
            //----------------------------------------------------------------------------
            template<class Iter_t, class Compare, enable_if_string<value_iter<Iter_t>>* = nullptr>
            inline void block_indirect_sort_call(Iter_t first, Iter_t last, Compare cmp, uint32_t nthr)
            {
            ...
            struct block_size
            {
                static constexpr const uint32_t BitsSize = (Size == 0) ? 0 : (Size > 256) ? 9 : tmsb[Size - 1];
                static constexpr const uint32_t sz[10] = { 4096, 4096, 4096, 4096, 2048, 1024, 768, 512, 256, 128 };
            ...
            ///
            //----------------------------------------------------------------------------
            template<class Iter_t, class Compare, enable_if_not_string<value_iter<Iter_t>>* = nullptr>
            inline void block_indirect_sort_call(Iter_t first, Iter_t last, Compare cmp, uint32_t nthr)
            {
                block_indirect_sort<block_size<sizeof(value_iter<Iter_t>)>::data, 64, Iter_t, Compare>(
                    first,
                    last,
                    cmp,
                    nthr);
        ...
        void block_indirect_sort(Iter_t first, Iter_t last)
        {
            ...
            blk_detail::block_indirect_sort_call(first, last, Compare(), std::thread::hardware_concurrency());
        }
        ...
        ///               iterators
        //-----------------------------------------------------------------------------
        template<class Iter_t, class Compare, bscu::enable_if_not_integral<Compare>* = nullptr>
        void block_indirect_sort(Iter_t first, Iter_t last, Compare comp)
        {
            blk_detail::block_indirect_sort_call(first, last, comp, std::thread::hardware_concurrency());
        }
        ...
        void block_indirect_sort(Iter_t first, Iter_t last, Compare comp, uint32_t nthread)
        {
