    ...
    namespace sort {
        namespace blk_detail {
            ...
            struct merge_blocks
            {
                ...
                //                F U N C T I O N S
                //------------------------------------------------------------------------
                merge_blocks(backbone_t& bkb, size_t pos_index1, size_t pos_index2, size_t pos_index3);
                ...
                ///
                //------------------------------------------------------------------------
                void function_merge_range_pos(const range_pos& rng_input, atomic_t& counter, bool& error)
                {
                ...
                /// @param error : global indicator of error.
                //------------------------------------------------------------------------
                void function_cut_range(const range_pos& rng_input, atomic_t& counter, bool& error)
                {
                    bscu::atomic_add(counter, 1);
                    function_t f1 = [this, rng_input, &counter, &error]() -> void {
                        if (!error) {
            ...
            merge_blocks<Block_size, Group_size, Iter_t, Compare>::merge_blocks(
                backbone_t& bkb,
                const size_t pos_index1,
                const size_t pos_index2,
                const size_t pos_index3) :
                bk(bkb)
            {
                size_t nblock1 = pos_index2 - pos_index1;
                size_t nblock2 = pos_index3 - pos_index2;
                if (nblock1 == 0 || nblock2 == 0) {
                    return;
                }
                ...
                //  tail process
                //-------------------------------------------------------------------
                if (vpos2.back().pos() == (bk.nblock - 1) && bk.range_tail.first != bk.range_tail.last) {
            ...
            {
                if (vblkpos1.size() == 0 || vblkpos2.size() == 0) {
                    return;
                }
                ...
                if (bsc::is_mergeable(range_back1, bk.range_tail, bk.cmp)) {
                    bsc::merge_uncontiguous(range_back1, bk.range_tail, bk.get_range_buf(), bk.cmp);
                    if (vblkpos1.size() > 1) {
            ...
            void merge_blocks<Block_size, Group_size, Iter_t, Compare>::cut_range(range_pos rng_input)
            {
                ...
                while (pos_ini < pos_last) {
                    ...
                    while (pos < pos_last && bk.index[pos - 1].side() == bk.index[pos].side()) {
                        ++pos;
                    }
                    if (pos < pos_last) {
                        merge_uncontiguous(
            ...
            void merge_blocks<Block_size, Group_size, Iter_t, Compare>::extract_ranges(range_pos range_input)
            {
                ...
                for (size_t posx = posx_ini + 1; posx <= range_input.last; ++posx) {
                    ...
                    if (!final) {
                        ...
                        mergeable = (side_max != side_posx && is_mergeable(rng_max, rng_posx, bk.cmp));
                    }
                    if (bk.error) {
                        return;
                    }
