    ...
    namespace sort {
        namespace blk_detail {
            ...
            struct parallel_sort
            {
                ...
                //------------------------------------------------------------------------
                void function_divide_sort(Iter_t first, Iter_t last, uint32_t level, atomic_t& counter, bool& error)
                {
                ...
                counter(0)
            {
                ...
                //------------------- check if sort --------------------------------------
                bool sorted = true;
                for (Iter_t it1 = first, it2 = first + 1; it2 != last && (sorted = !bk.cmp(*it2, *it1)); it1 = it2++)
                    ;
                if (sorted) {
                    return;
                }
                ...
                sorted = true;
                for (Iter_t it1 = first, it2 = first + 1; it2 != last && (sorted = !bk.cmp(*it1, *it2)); it1 = it2++)
                    ;
