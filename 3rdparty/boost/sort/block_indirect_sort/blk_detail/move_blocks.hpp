    ...
    namespace sort {
        namespace blk_detail {
            ...
            struct move_blocks
            {
                ...
                //------------------------------------------------------------------------
                void function_move_sequence(std::vector<size_t>& sequence, atomic_t& counter, bool& error)
                {
                ...
                /// @param error : global indicator of error.
                //------------------------------------------------------------------------
                void function_move_long_sequence(std::vector<size_t>& sequence, atomic_t& counter, bool& error)
                {
                    bscu::atomic_add(counter, 1);
                    function_t f1 = [this, sequence, &counter, &error]() -> void {
                        if (!error) {
            ...
            //-------------------------------------------------------------------------
            template<uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare>
            move_blocks<Block_size, Group_size, Iter_t, Compare>::move_blocks(backbone_t& bkb) :
                bk(bkb)
            {
                ...
                while (pos_index_ini < bk.index.size()) {
                    while (pos_index_ini < bk.index.size() && bk.index[pos_index_ini].pos() == pos_index_ini) {
                        ++pos_index_ini;
                    }

                    if (pos_index_ini == bk.index.size()) {
                        break;
                    }
            ...
            //-------------------------------------------------------------------------
            template<uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare>
            void move_blocks<Block_size, Group_size, Iter_t, Compare>::move_sequence(
                const std::vector<size_t>& init_sequence)
            {
            ...
            //-------------------------------------------------------------------------
            template<uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare>
            void move_blocks<Block_size, Group_size, Iter_t, Compare>::move_long_sequence(
                const std::vector<size_t>& init_sequence)
            {
