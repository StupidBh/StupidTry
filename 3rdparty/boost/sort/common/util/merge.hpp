            ...
            namespace util {
                    ...
                    Compare comp)
                {
                    ...
                    if (size_t((end_buf1 - buf1) + (end_buf2 - buf2)) >= MIN_CHECK) {
                        if (buf1 == end_buf1) {
                            return move_forward(buf_out, buf2, end_buf2);
                        }
                        if (buf2 == end_buf2) {
                            return move_forward(buf_out, buf1, end_buf1);
                        }
                    ...
                    while ((buf1 != end_buf1) && (buf2 != end_buf2)) {
                        *(buf_out++) = (!comp(*buf2, *buf1)) ? std::move(*(buf1++)) : std::move(*(buf2++));
                    }

                    return (buf1 == end_buf1) ? move_forward(buf_out, buf2, end_buf2)
                ...
                static Value_t* merge_construct(
                    Iter1_t first1,
                    const Iter1_t last1,
                    Iter2_t first2,
                    const Iter2_t last2,
                    ...
                    Compare comp)
                {
                    ...
                    if (size_t((last1 - first1) + (last2 - first2)) >= MIN_CHECK) {
                        if (first1 == last1) {
                            return move_construct(it_out, first2, last2);
                        }
                        if (first2 == last2) {
                            return move_construct(it_out, first1, last1);
                        }
                    ...
                    while (first1 != last1 && first2 != last2) {
                        construct_object(
                            (it_out++),
                            (!comp(*first2, *first1)) ? std::move(*(first1++)) : std::move(*(first2++)));
                    }
                    return (first1 == last1) ? move_construct(it_out, first2, last2)
                ...
                //---------------------------------------------------------------------------
                template<class Iter1_t, class Iter2_t, class Compare>
                static Iter2_t merge_half_backward(
                    Iter1_t buf1,
                    Iter1_t end_buf1,
                    Iter2_t buf2,
                    Iter2_t end_buf2,
                    ...
                    Compare comp)
                {
                    ...
                    while ((buf1 != end_buf1) && (buf2 != end_buf2)) {
                        *(--end_buf_out) = (!comp(*(end_buf2 - 1), *(end_buf1 - 1))) ? std::move(*(--end_buf2))
                                                                                     : std::move(*(--end_buf1));
                    }
                    return (buf1 == end_buf1) ? here::move_backward(end_buf_out, buf2, end_buf2) : buf1;
                }
                ...
                //-----------------------------------------------------------------------------
                template<class Iter1_t, class Iter2_t, class Iter3_t, class Compare>
                static bool merge_uncontiguous(
                    Iter1_t src1,
                ...
                //-----------------------------------------------------------------------------
                template<class Iter1_t, class Iter2_t, class Compare>
                static bool merge_contiguous(Iter1_t src1, Iter1_t src2, Iter1_t end_src2, Iter2_t buf, Compare comp)
                {
                    //-------------------------------------------------------------------------
                    //                      Metaprogramming
                    //-------------------------------------------------------------------------
                    ...
                    Iter2_t& it2_out)
                {
                    ...
                    static_assert(std::is_same<type1, type3>::value, "Incompatible iterators\n");
                    ...
                    while (buf1 != end_buf1 && buf2 != end_buf2) {
                        circ.push_back(comp(*buf2, *buf1) ? std::move(*(buf2++)) : std::move(*(buf1++)));
                    }
                    it2_out = buf2;
                    it1_out = buf1;
