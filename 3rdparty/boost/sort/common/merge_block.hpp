    ...
    namespace sort {
        namespace common {
            ...
            struct merge_block
            {
                ...
                ///               iterators
                //------------------------------------------------------------------------
                merge_block(Iter_t first, Iter_t last, Compare comp, circular_t* pcirc_buffer) :
                    global_range(first, last),
                    cmp(comp),
                    ptr_circ(pcirc_buffer),
                ...
                range_it get_group_range(size_t pos, size_t nrange) const
                {
                    ...
                    Iter_t it2 = ((pos + nrange) == nblock) ? global_range.last
                                                            : global_range.first + ((pos + nrange) << LOG_BLOCK);
                ...
                //-------------------------------------------------------------------------
                bool is_tail(size_t pos) const { return (pos == (nblock - 1) && ntail != 0); };
                ...
                /// @return
                //-------------------------------------------------------------------------
                void merge_range_pos(it_index itx_first, it_index itx_mid, it_index itx_last);
                ...
                /// @return
                //-------------------------------------------------------------------------
                void move_range_pos_backward(it_index itx_first, it_index itx_last, size_t npos);
            ...
            void merge_block<Iter_t, Compare, Power2>::merge_range_pos(
                it_index itx_first,
                it_index itx_mid,
                it_index itx_last)
            {
                ...
                while (itxA != indexA.end() &&
                        ...
                        if (cmp(*rngB.back(), *rngA.front())) {
                            if (!is_tail(*itxB)) {
                                *(itx_out++) = *itxB;
                                ...
                                ptr_circ->push_move_back(rngB.first, rngB.size());
                            }
                            ++itxB;
                            validB = false;
                            continue;
                    ...
                    // Normal merge
                    //----------------------------------------------------------------
                    bool side = util::merge_circular(itA, rngA.last, itB, rngB.last, *ptr_circ, cmp, itA, itB);
                    if (side) { // rngA is finished
                        ptr_circ->pop_move_front(rngA.first, rngA.size());
                        *(itx_out++) = *(itxA++);
                        validA = false;
                    }
                    else { // rngB is finished
                        if (!is_tail(*itxB)) {
                ...
                if (itxA == indexA.end()) { // the index A is finished
                    rngB = get_range(*itxB);
            ...
            //-------------------------------------------------------------------------
            template<class Iter_t, class Compare, uint32_t Power2>
            void merge_block<Iter_t, Compare, Power2>::move_range_pos_backward(
                it_index itx_first,
                it_index itx_last,
                size_t npos)
            {
            ...
            //-------------------------------------------------------------------------
            template<class Iter_t, class Compare, uint32_t Power2>
            void merge_block<Iter_t, Compare, Power2>::rearrange_with_index(void)
            { //--------------------------------------------------------------------
                //                     Code
                //--------------------------------------------------------------------
