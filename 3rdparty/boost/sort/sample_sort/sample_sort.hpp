...
namespace boost {
    namespace sort {
        namespace sample_detail {
            ...
            struct sample_sort
            {
                ...
                void execute_first(void)
                {
                    ...
                    while ((job = atomic_add(njob, 1)) < ninterval) {
                        uninit_merge_level4(vrange_buf_ini[job], vv_range_it[job], vv_range_buf[job], comp);
                    }
                }
            ...
            sample_sort<Iter_t, Compare>::sample_sort(
                Iter_t first,
                Iter_t last,
                Compare cmp,
                uint32_t num_thread,
                ...
                error(false)
            {
                ...
                if (paux != nullptr) {
                    ...
                    value_t* ptr = reinterpret_cast<value_t*>(std::malloc(nelem * sizeof(value_t)));
            ...
            void sample_sort<Iter_t, Compare>::initial_configuration(void)
            {
                ...
                bss::spinsort<it_to_it, compare_ptr>(vsample.begin(), vsample.end(), compare_ptr(comp));
                ...
                for (uint32_t i = 0; i < nthread; ++i) {
                    ...
                    for (uint32_t k = 0; k < (ninterval - 1); ++k) {
                        Iter_t it2 = std::upper_bound(itaux, vmem_thread[i].last, *vmilestone[k], comp);
