...
namespace boost {
    ...
    public:
         ...
         * __Throws
         */
        ~strict_lock() { mtx_.unlock(); } /*< unlocks on destruction >*/
         ...
         */
        bool owns_lock() const BOOST_NOEXCEPT { return true; }
    ...
    public:
            ...
            lk_(lk) /*< Store reference to lk >*/
        {
            ...
            BOOST_THREAD_ASSERT_PRECONDITION(lk.mutex() != 0, lock_error());
            if (!lk.owns_lock()) {
                lk.lock();      /*< ensures it is locked >*/
            }
        ...
        {
            /*< Define BOOST_THREAD_DONT_CHECK_PRECONDITIONS if you don't want to check lk ownership >*/
            BOOST_THREAD_ASSERT_PRECONDITION(lk_.mutex() != 0, lock_error());
            if (!lk_.owns_lock()) {
                lk_.lock();      /*< ensures it is locked >*/
            }
         ...
         */
        ~nested_strict_lock() BOOST_NOEXCEPT { lk_ = move(tmp_lk_); /*< Move ownership to nesting lock >*/ }

        // observers
        /**
         * return @c the owned mutex.
         */
        mutex_type* mutex() const BOOST_NOEXCEPT { return tmp_lk_.mutex(); }
         ...
         */
        bool owns_lock(mutex_type const* l) const BOOST_NOEXCEPT { return l == mutex(); }
