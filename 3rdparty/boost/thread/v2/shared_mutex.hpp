...
namespace boost {
    namespace thread_v2 {
...
#ifdef BOOST_THREAD_USES_CHRONO
        template<class Clock, class Duration>
        bool shared_mutex::try_lock_until(const boost::chrono::time_point<Clock, Duration>& abs_time)
        {
            boost::unique_lock<mutex_t> lk(mut_);
            if (!gate1_.wait_until(lk, abs_time, boost::bind(&shared_mutex::no_writer, boost::ref(*this)))) {
                return false;
            }
            state_ |= write_entered_;
            if (!gate2_.wait_until(lk, abs_time, boost::bind(&shared_mutex::no_readers, boost::ref(*this)))) {
                state_ &= ~write_entered_;
                return false;
            }
            return true;
        ...
        {
            boost::unique_lock<mutex_t> lk(mut_);
            if (!gate1_.timed_wait(lk, abs_or_rel_time, boost::bind(&shared_mutex::no_writer, boost::ref(*this)))) {
                return false;
            }
            state_ |= write_entered_;
            if (!gate2_.timed_wait(lk, abs_or_rel_time, boost::bind(&shared_mutex::no_readers, boost::ref(*this)))) {
                state_ &= ~write_entered_;
                return false;
            }
            return true;
...
#ifdef BOOST_THREAD_USES_CHRONO
        template<class Clock, class Duration>
        bool shared_mutex::try_lock_shared_until(const boost::chrono::time_point<Clock, Duration>& abs_time)
        {
            boost::unique_lock<mutex_t> lk(mut_);
            if (!gate1_.wait_until(
                    lk,
                    abs_time,
                    boost::bind(&shared_mutex::no_writer_no_max_readers, boost::ref(*this)))) {
                return false;
            }
            count_t num_readers = (state_ & n_readers_) + 1;
            state_ &= ~n_readers_;
...
namespace boost {
    namespace thread_v2 {
        ...
        class upgrade_mutex {
            ...
            bool no_writer_no_upgrader() const { return (state_ & (write_entered_ | upgradable_entered_)) == 0; }
            ...
            {
                return (state_ & (write_entered_ | upgradable_entered_)) == 0 && (state_ & n_readers_) != n_readers_;
            }

            bool no_upgrader() const { return (state_ & upgradable_entered_) == 0; }

            bool one_upgrader() const { return (state_ & upgradable_entered_) != 0; }

            bool no_readers() const { return (state_ & n_readers_) == 0; }

            bool one_reader() const { return (state_ & n_readers_) == 1; }
...
#ifdef BOOST_THREAD_USES_CHRONO
        template<class Clock, class Duration>
        bool upgrade_mutex::try_lock_until(const boost::chrono::time_point<Clock, Duration>& abs_time)
        {
            boost::unique_lock<mutex_t> lk(mut_);
            if (!gate1_
                     .wait_until(lk, abs_time, boost::bind(&upgrade_mutex::no_writer_no_upgrader, boost::ref(*this)))) {
                return false;
            }
            state_ |= write_entered_;
            if (!gate2_.wait_until(lk, abs_time, boost::bind(&upgrade_mutex::no_readers, boost::ref(*this)))) {
                state_ &= ~write_entered_;
                return false;
            }
            return true;
        ...
        bool upgrade_mutex::timed_lock(T const& abs_or_rel_time)
        {
            ...
            if (!gate1_.timed_wait(
                    lk,
                    abs_or_rel_time,
                    boost::bind(&upgrade_mutex::no_writer_no_upgrader, boost::ref(*this)))) {
                return false;
            }
            state_ |= write_entered_;
            if (!gate2_.timed_wait(lk, abs_or_rel_time, boost::bind(&upgrade_mutex::no_readers, boost::ref(*this)))) {
                state_ &= ~write_entered_;
                return false;
            }
            return true;
...
#ifdef BOOST_THREAD_USES_CHRONO
        template<class Clock, class Duration>
        bool upgrade_mutex::try_lock_shared_until(const boost::chrono::time_point<Clock, Duration>& abs_time)
        {
            boost::unique_lock<mutex_t> lk(mut_);
            if (!gate1_.wait_until(
                    lk,
                    abs_time,
                    boost::bind(&upgrade_mutex::no_writer_no_max_readers, boost::ref(*this)))) {
                return false;
            }
            count_t num_readers = (state_ & n_readers_) + 1;
            state_ &= ~n_readers_;
...
#ifdef BOOST_THREAD_USES_CHRONO
        template<class Clock, class Duration>
        bool upgrade_mutex::try_lock_upgrade_until(const boost::chrono::time_point<Clock, Duration>& abs_time)
        {
            boost::unique_lock<mutex_t> lk(mut_);
            if (!gate1_.wait_until(
                    lk,
                    abs_time,
                    boost::bind(&upgrade_mutex::no_writer_no_upgrader_no_max_readers, boost::ref(*this)))) {
                return false;
            }
            count_t num_readers = (state_ & n_readers_) + 1;
            state_ &= ~n_readers_;
    ...
    #ifdef BOOST_THREAD_USES_CHRONO
        template<class Clock, class Duration>
        bool upgrade_mutex::try_unlock_shared_and_lock_until(const boost::chrono::time_point<Clock, Duration>& abs_time)
        {
            boost::unique_lock<mutex_t> lk(mut_);
            BOOST_ASSERT(one_or_more_readers());
            if (!gate1_
        ...
        template<class Clock, class Duration>
        bool
            upgrade_mutex::try_unlock_upgrade_and_lock_until(const boost::chrono::time_point<Clock, Duration>& abs_time)
        {
            boost::unique_lock<mutex_t> lk(mut_);
            BOOST_ASSERT(no_writer());
            BOOST_ASSERT(one_upgrader());
