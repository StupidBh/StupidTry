...
namespace boost {
    ...
    namespace threads {
        ...
        namespace chrono_details {
            ...
            inline time_max_t steady_simplified()
            {
    ...
    namespace detail {
        ...
        {
            boost::time_max_t s = ns / 1'000'000'000l;
            timespec ts;
            ts.tv_sec = static_cast<long>(s);
            ts.tv_nsec = static_cast<long>(ns - s * 1'000'000'000l);
            return ts;
        }

        inline boost::time_max_t timespec_to_ns(timespec const& ts)
        {
            return static_cast<boost::time_max_t>(ts.tv_sec) * 1'000'000'000l + ts.tv_nsec;
        }
#endif
        ...
        {
#if defined BOOST_THREAD_CHRONO_POSIX_API || defined BOOST_THREAD_CHRONO_MAC_API
            explicit platform_duration(timespec const& v) :
                ts_val(v)
            {
            }
            ...
            explicit platform_duration(boost::time_max_t const& ns = 0) :
                ts_val(ns_to_timespec(ns))
            {
            }
            ...
            platform_duration(boost::posix_time::time_duration const& rel_time)
            {
                ...
                ts_val.tv_nsec =
                    static_cast<long>(rel_time.fractional_seconds() * (1'000'000'000l / rel_time.ticks_per_second()));
    #else
                ns_val = static_cast<boost::time_max_t>(rel_time.total_seconds()) * 1'000'000'000l;
                ns_val += rel_time.fractional_seconds() * (1'000'000'000l / rel_time.ticks_per_second());
    #endif
            }
#endif
            ...
            static platform_duration zero() { return platform_duration(0); }
        ...
        {
            return platform_duration(ms * 1'000'000l);
        }

        struct real_platform_timepoint
        {
#if defined BOOST_THREAD_CHRONO_POSIX_API || defined BOOST_THREAD_CHRONO_MAC_API
            explicit real_platform_timepoint(timespec const& v) :
                dur(v)
            {
            }
            ...
            explicit real_platform_timepoint(boost::time_max_t const& ns) :
                dur(ns)
            {
            }
        ...
        struct real_platform_clock
        {
            static real_platform_timepoint now()
            {
                ...
                boost::time_max_t ns =
                    ((((static_cast<boost::time_max_t>(ft.dwHighDateTime) << 32) | ft.dwLowDateTime) -
        ...
        struct mono_platform_timepoint
        {
            ...
            explicit mono_platform_timepoint(timespec const& v) :
                dur(v)
            {
            }
            ...
            explicit mono_platform_timepoint(boost::time_max_t const& ns) :
                dur(ns)
            {
            }
    ...
    #if defined BOOST_THREAD_USES_CHRONO
            // This conversion assumes that chrono::steady_clock::time_point and mono_platform_timepoint share the same
            // epoch.
            template<class Duration>
        ...
        struct mono_platform_clock
        {
            static mono_platform_timepoint now()
            {
                ...
                return mono_platform_timepoint(msec * 1'000'000);
        #endif
    #elif defined(BOOST_THREAD_CHRONO_MAC_API)
                kern_return_t err;
