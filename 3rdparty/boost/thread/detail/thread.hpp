...
namespace boost {
    ...
    namespace detail {
        ...
        public:
            ...
            {
                typedef
                    typename make_tuple_indices<std::tuple_size<std::tuple<F, ArgTypes...>>::value, 1>::type index_type;
    ...
    //            {}
        ...
        template<typename F>
        class thread_data<boost::reference_wrapper<F>> : public detail::thread_data_base {
        private:
            F& f;
        ...
        template<typename F>
        class thread_data<const boost::reference_wrapper<F>> : public detail::thread_data_base {
        private:
            F& f;
    ...
    private:
        ...
        static inline detail::thread_data_ptr
            make_thread_info(BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_RV_REF(ArgTypes)... args)
        {
            return detail::thread_data_ptr(
                detail::heap_new<detail::thread_data<typename boost::remove_reference<F>::type, ArgTypes...>>(
                    boost::forward<F>(f),
        ...
        {
            return detail::thread_data_ptr(
                detail::heap_new<detail::thread_data<typename boost::remove_reference<F>::type>>(boost::forward<F>(f)));
        }
    #endif
        static inline detail::thread_data_ptr make_thread_info(void (*f)())
        {
            return detail::thread_data_ptr(
                detail::heap_new<detail::thread_data<void (*)()>>(boost::forward<void (*)()>(f)));
        }
#else
        template<typename F>
        static inline detail::thread_data_ptr make_thread_info(
            F f,
...
#endif
        void swap(thread& x) BOOST_NOEXCEPT { thread_info.swap(x.thread_info); }
...
#define BOOST_THREAD_DEFINES_THREAD_NATIVE_HANDLE
...
#endif
#if defined BOOST_THREAD_USES_DATETIME
        static inline void yield() BOOST_NOEXCEPT { this_thread::yield(); }

        static inline void sleep(const system_time& xt) { this_thread::sleep(xt); }
#endif
    ...
    private:
        ...
        friend inline std::size_t hash_value(const thread::id& v)
        {
        ...
        friend std::size_t hash_value(const thread::id& v);
    ...
    public:
        ...
        bool operator==(const id& y) const BOOST_NOEXCEPT { return thread_data == y.thread_data; }

        bool operator!=(const id& y) const BOOST_NOEXCEPT { return thread_data != y.thread_data; }

        bool operator<(const id& y) const BOOST_NOEXCEPT { return thread_data < y.thread_data; }

        bool operator>(const id& y) const BOOST_NOEXCEPT { return y.thread_data < thread_data; }
        ...
        friend BOOST_SYMBOL_VISIBLE std::basic_ostream<charT, traits>&
    ...
    #else
        template<class charT, class traits>
        BOOST_SYMBOL_VISIBLE std::basic_ostream<charT, traits>& print(std::basic_ostream<charT, traits>& os) const
        {
            if (thread_data) {
                io::ios_flags_saver ifs(os);
    ...
    inline std::size_t hash_value(const thread::id& v)
    {
    #if defined BOOST_THREAD_PROVIDES_BASIC_THREAD_ID
        return hash_value(v.thread_data);
    #else
    ...
    inline void thread::join()
    {
        ...
        BOOST_THREAD_VERIFY_PRECONDITION(
            join_noexcept(),
            thread_resource_error(
                static_cast<int>(system::errc::invalid_argument),
    ...
    template<class charT, class traits>
    BOOST_SYMBOL_VISIBLE std::basic_ostream<charT, traits>&
    ...
    namespace detail {
        ...
        template<typename F>
        struct thread_exit_function : thread_exit_function_base
        {
            F f;
    ...
    namespace this_thread {
        ...
        void at_thread_exit(F f)
        {
            detail::thread_exit_function_base* const thread_exit_func =
                detail::heap_new<detail::thread_exit_function<F>>(f);
