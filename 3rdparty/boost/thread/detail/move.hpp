...
namespace boost {
    ...
    #ifndef BOOST_NO_SFINAE
    template<typename T>
    typename enable_if<boost::is_convertible<T&, boost::detail::thread_move_t<T>>, boost::detail::thread_move_t<T>>::
        type move(T& t)
    {
...
namespace boost {
    namespace detail {
        ...
        {
            return (
                BOOST_THREAD_RV_REF(typename ::boost::remove_cv<typename ::boost::remove_reference<T>::type>::type))(v);
        }
        ...
        //  make_rv_ref(T &v)  BOOST_NOEXCEPT
        //  {
        //    return (BOOST_THREAD_RV_REF(typename ::boost::remove_cv<typename
        //    ::boost::remove_reference<T>::type>::type))(v);
        //  }
        //  template <typename T>
        ...
        //  {
        //    return (const BOOST_THREAD_RV_REF(typename ::boost::remove_cv<typename
        //    ::boost::remove_reference<T>::type>::type))(v);
        //  }
    }
}
...
// #define BOOST_THREAD_MAKE_RV_REF(RVALUE) boost::detail::make_rv_ref(RVALUE)
    ...
    #define BOOST_THREAD_COPYABLE(TYPE)
        ...
        #define BOOST_THREAD_COPYABLE(TYPE)                                                         \
            ...
            {                                                                                       \
                this->operator=(static_cast<const ::boost::rv<TYPE>&>(const_cast<const TYPE&>(t))); \
                return *this;                                                                       \
            }
...
namespace boost {
    namespace thread_detail {
        ...
        template<class T>
        struct is_rv : ::boost::move_detail::is_rv<T>
        {
        };

#else
        template<class T>
        struct is_rv : ::boost::integral_constant<bool, false>
        {
        };

        template<class T>
        struct is_rv<::boost::detail::thread_move_t<T>> : ::boost::integral_constant<bool, true>
        {
        };

        template<class T>
        struct is_rv<const ::boost::detail::thread_move_t<T>> : ::boost::integral_constant<bool, true>
        {
        };
#endif
        ...
        public:
            typedef typename conditional<
                ...
                typename conditional<
                    ...
                    typename remove_cv<Up>::type>::type>::type type;
        };
#endif

#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
        template<class T>
        typename decay<T>::type decay_copy(T&& t)
        {
...
#else
        template<class T>
        typename decay<T>::type decay_copy(BOOST_THREAD_FWD_REF(T) t)
        {
            return boost::forward<T>(t);
        }
#endif
