...
namespace boost {
    namespace concurrent {
        ...
        private:
            ...
            inline size_type inc(size_type idx) const BOOST_NOEXCEPT { return (idx + 1) % capacity_; }

            inline bool empty(unique_lock<mutex>&) const BOOST_NOEXCEPT { return in_ == out_; }

            inline bool empty(lock_guard<mutex>&) const BOOST_NOEXCEPT { return in_ == out_; }

            inline bool full(unique_lock<mutex>&) const BOOST_NOEXCEPT { return (inc(in_) == out_); }

            inline bool full(lock_guard<mutex>&) const BOOST_NOEXCEPT { return (inc(in_) == out_); }

            inline size_type capacity(lock_guard<mutex>&) const BOOST_NOEXCEPT { return capacity_ - 1; }
        ...
        sync_bounded_queue<ValueType>::sync_bounded_queue(typename sync_bounded_queue<ValueType>::size_type max_elems) :
            waiting_full_(0),
            waiting_empty_(0),
            data_(new value_type[max_elems + 1]),
            in_(0),
            out_(0),
        ...
        //    waiting_full_(0), waiting_empty_(0), data_(new value_type[max_elems + 1]), in_(0), out_(0),
        //    capacity_(max_elems + 1),
        //        closed_(false)
        //  {
        //    BOOST_ASSERT_MSG(max_elems >= 1, "number of elements must be > 1");
        ...
        template<typename ValueType>
        typename sync_bounded_queue<ValueType>::size_type
            sync_bounded_queue<ValueType>::wait_until_not_full(unique_lock<mutex>& lk)
        {
            for (;;) {
                throw_if_closed(lk);
                size_type in_p_1 = inc(in_);
        ...
        template<typename ValueType>
        queue_op_status
            sync_bounded_queue<ValueType>::try_push_back(BOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
        {
            if (closed(lk)) {
                return queue_op_status::closed;
            }
        ...
        template<typename ValueType>
        queue_op_status
            sync_bounded_queue<ValueType>::wait_push_back(BOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
        {
            if (closed(lk)) {
                return queue_op_status::closed;
            }
        ...
        template<typename ValueType>
        sync_bounded_queue<ValueType>&
            operator<<(sync_bounded_queue<ValueType>& sbq, BOOST_THREAD_RV_REF(ValueType) elem)
        {
