    ...
    namespace concurrent {
        namespace detail {
            ...
            struct scheduled_type
            {
                ...
                scheduled_type(T const& pdata, time_point tp) :
                    data(pdata),
                    time(tp)
                {
                }

                scheduled_type(BOOST_THREAD_RV_REF(T) pdata, time_point tp) :
                    data(boost::move(pdata)),
                    time(tp)
                {
                }

                scheduled_type(scheduled_type const& other) :
                    data(other.data),
                    time(other.time)
                {
                }
                ...
                scheduled_type(BOOST_THREAD_RV_REF(scheduled_type) other) :
                ...
                bool operator<(const scheduled_type& other) const { return this->time > other.time; }
            }; // end struct
            ...
            chrono::time_point<Clock, Duration> limit_timepoint(chrono::time_point<Clock, Duration> const& tp)
            {
                // Clock != chrono::steady_clock
                ...
                const chrono::time_point<Clock, Duration> tpmax(
                    chrono::time_point_cast<Duration>(
                        Clock::now() + chrono::milliseconds(BOOST_THREAD_POLL_INTERVAL_MILLISECONDS)));
                ...
                convert_to_steady_clock_timepoint(chrono::time_point<Clock, Duration> const& tp)
            {
                ...
                // The system time may jump while wait_until() is waiting. To compensate for this and time out near
                // the correct time, we limit how long wait_until() can wait before going around the loop again.
                const chrono::steady_clock::duration dura(
                    chrono::duration_cast<chrono::steady_clock::duration>(tp - Clock::now()));
                const chrono::steady_clock::duration duramax(
                    chrono::milliseconds(BOOST_THREAD_POLL_INTERVAL_MILLISECONDS));
        ...
        template<class T, class Clock = chrono::steady_clock, class TimePoint = typename Clock::time_point>
        class sync_timed_queue : private sync_priority_queue<detail::scheduled_type<T, Clock, TimePoint>> {
        ...
        void sync_timed_queue<T, Clock, TimePoint>::push(
            BOOST_THREAD_RV_REF(T) elem,
            chrono::time_point<clock, Duration> const& tp)
        {
        ...
        void sync_timed_queue<T, Clock, TimePoint>::push(
            BOOST_THREAD_RV_REF(T) elem,
            chrono::duration<Rep, Period> const& dura)
        {
        ...
        template<class T, class Clock, class TimePoint>
        template<class Duration>
        queue_op_status sync_timed_queue<T, Clock, TimePoint>::try_push(
            const T& elem,
            chrono::time_point<clock, Duration> const& tp)
        {
        ...
        template<class Rep, class Period>
        queue_op_status
            sync_timed_queue<T, Clock, TimePoint>::try_push(const T& elem, chrono::duration<Rep, Period> const& dura)
        {
        ...
        template<class T, class Clock, class TimePoint>
        queue_op_status
            sync_timed_queue<T, Clock, TimePoint>::wait_to_pull_until(unique_lock<mutex>& lk, TimePoint const& tp)
        {
            for (;;) {
                if (not_empty_and_time_reached(lk)) {
                ...
                if (clock::now() >= tp) {
                    return super::empty(lk) ? queue_op_status::timeout : queue_op_status::not_ready;
                }
        ...
        template<class T, class Clock, class TimePoint>
        template<class Rep, class Period>
        queue_op_status sync_timed_queue<T, Clock, TimePoint>::wait_to_pull_for(
            unique_lock<mutex>& lk,
            chrono::duration<Rep, Period> const& dura)
        {
            const chrono::steady_clock::time_point tp(
                chrono::steady_clock::now() + chrono::duration_cast<chrono::steady_clock::duration>(dura));
            for (;;) {
                if (not_empty_and_time_reached(lk)) {
                    return queue_op_status::success;
                }
                ...
                const chrono::steady_clock::time_point tpmin(
                    (std::min)(tp, detail::convert_to_steady_clock_timepoint(super::data_.top().time)));
                super::cond_.wait_until(lk, tpmin);
            }
        }
        ...
        {
            unique_lock<mutex> lk(super::mtx_);
            const queue_op_status rc =
                wait_to_pull_until(lk, chrono::time_point_cast<typename time_point::duration>(tp));
            if (rc == queue_op_status::success) {
                pull(lk, elem);
            }
            return rc;
        }
