...
namespace boost {
        ...
        namespace parallel {
            BOOST_THREAD_INLINE_NAMESPACE(v2)
            {
                ...
                public:
                    ...
                    virtual const char* what() const BOOST_NOEXCEPT_OR_NOTHROW { return "task_canceled_exception"; }
                };
                ...
                namespace detail {
                    ...
                    struct wrapped
                    {
                        ...
                        wrapped(TRH& tr, BOOST_THREAD_RV_REF(F) f) :
                            tr(tr),
                            f(move(f))
                        {
                        }
    ...
    #if defined BOOST_THREAD_PROVIDES_EXECUTORS
                        group.push_back(
                            async(*ex, detail::wrapped<task_region_handle_gen<Executor>, F>(*this, forward<F>(f))));
    #else
                        group.push_back(
                            async(detail::wrapped<task_region_handle_gen<Executor>, F>(*this, forward<F>(f))));
    #endif
#else
    #if defined BOOST_THREAD_PROVIDES_EXECUTORS
...
#endif
                class task_region_handle : public task_region_handle_gen<default_executor> {
                    default_executor tp;
                    template<typename F>
                    friend void task_region(BOOST_THREAD_FWD_REF(F) f);
                    template<typename F>
