...
namespace boost {
    namespace csbl {
        ...
        public:
            ...
            devector(devector const& x) BOOST_NOEXCEPT : data_(x.data_), front_index_(x.front_index_) {}
            ...
            bool empty() const BOOST_NOEXCEPT { return data_.size() == front_index_; }

            size_type size() const BOOST_NOEXCEPT { return data_.size() - front_index_; }

            reference front() BOOST_NOEXCEPT { return data_[front_index_]; }

            const_reference front() const BOOST_NOEXCEPT { return data_[front_index_]; }

            reference back() BOOST_NOEXCEPT { return data_.back(); }

            const_reference back() const BOOST_NOEXCEPT { return data_.back(); }
