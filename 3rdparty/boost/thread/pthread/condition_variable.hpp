...
namespace boost {
    ...
    inline void condition_variable::wait(unique_lock<mutex>& m)
    {
        ...
        if (!m.owns_lock()) {
            boost::throw_exception(
                condition_error(-1, "boost::condition_variable::wait() failed precondition mutex not owned"));
        }
#endif
        int res = 0;
        {
    ...
    inline bool
        condition_variable::do_wait_until(unique_lock<mutex>& m, detail::internal_platform_timepoint const& timeout)
    {
#if defined BOOST_THREAD_THROW_IF_PRECONDITION_NOT_SATISFIED
        if (!m.owns_lock()) {
    ...
    public:
        ...
        condition_variable_any()
        {
            ...
            if (res2) {
                ...
                boost::throw_exception(thread_resource_error(
                    res2,
                    "boost::condition_variable_any::condition_variable_any() failed in pthread_cond_init"));
            }
        }
        ...
        void wait(lock_type& m)
        {
            ...
            if (res) {
                boost::throw_exception(
                    condition_error(res, "boost::condition_variable_any::wait() failed in pthread_cond_wait"));
            }
        }
    ...
    private:
        ...
        bool do_wait_until(lock_type& m, detail::internal_platform_timepoint const& timeout)
        {
            int res = 0;
            {
                thread_cv_detail::lock_on_exit<lock_type> guard;
