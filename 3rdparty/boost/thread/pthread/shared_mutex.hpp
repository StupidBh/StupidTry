...
namespace boost {
    ...
    private:
        ...
        public:
            ...
            void assert_lock_shared() const
            {
                ...
                //  except when unlock_upgrade_and_lock has decreased the number of readers but has not taken yet
                //  exclusive ownership.
            }
            ...
            void assert_lock_not_upgraded() const { BOOST_ASSERT(!upgrade); }

            bool can_lock() const { return !(shared_count || exclusive); }

            void lock() { exclusive = true; }
            ...
            bool can_lock_shared() const { return !(exclusive || exclusive_waiting_blocked); }

            bool no_shared() const { return shared_count == 0; }

            bool one_shared() const { return shared_count == 1; }

            void lock_shared() { ++shared_count; }

            void unlock_shared() { --shared_count; }
            ...
            bool can_lock_upgrade() const { return !(exclusive || exclusive_waiting_blocked || upgrade); }
    ...
    public:
        ...
        bool timed_lock_shared(TimeDuration const& relative_time)
        {
            ...
            boost::unique_lock<boost::mutex> lk(state_change);
            if (!shared_cond
                     .timed_wait(lk, relative_time, boost::bind(&state_data::can_lock_shared, boost::ref(state)))) {
                return false;
            }
            state.lock_shared();
            return true;
        ...
        bool timed_lock_upgrade(TimeDuration const& relative_time)
        {
            ...
            boost::unique_lock<boost::mutex> lk(state_change);
            if (!shared_cond
                     .timed_wait(lk, relative_time, boost::bind(&state_data::can_lock_upgrade, boost::ref(state)))) {
                return false;
            }
            state.lock_shared();
            state.upgrade = true;
        ...
        bool try_unlock_upgrade_and_lock()
        {
            boost::unique_lock<boost::mutex> lk(state_change);
            state.assert_lock_upgraded();
            if (!state.exclusive && !state.exclusive_waiting_blocked && state.upgrade && state.shared_count == 1) {
        ...
        bool try_unlock_shared_and_lock_upgrade_until(const chrono::time_point<Clock, Duration>& abs_time)
        {
            ...
            state.assert_lock_shared();
            if (!exclusive_cond
                     .wait_until(lk, abs_time, boost::bind(&state_data::can_lock_upgrade, boost::ref(state)))) {
                return false;
            }
            state.upgrade = true;
            return true;
