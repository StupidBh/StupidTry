...
namespace boost {
     ...
     * When we acquire a lock, we set the thread id; and when we release a lock, we reset it back to its default no id
     * state.
     *
     */
    ...
    public:
        ...
        bool is_locked_by_this_thread() const { return this_thread::get_id() == id_; }

        bool is_locked() const { return !(thread::id() == id_); }
