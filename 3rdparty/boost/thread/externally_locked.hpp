...
namespace boost {
    ...
    public:
         ...
         *
         * Throws: lock_error if BOOST_THREAD_THROW_IF_PRECONDITION_NOT_SATISFIED is not defined and the lk parameter
        ...
        T& get(strict_lock<mutex_type>& lk)
        {
            BOOST_THREAD_ASSERT_PRECONDITION(
                lk.owns_lock(mtx_),
                lock_error()); /*< run time check throw if not locks the same >*/
            return obj_;
        }
        ...
        {
            BOOST_STATIC_ASSERT(
                (is_same<mutex_type, typename Lock::mutex_type>::value)); /*< that locks the same type >*/
            BOOST_THREAD_ASSERT_PRECONDITION(
                lk.owns_lock(mtx_),
                lock_error()); /*< run time check throw if not locks the same >*/
            return obj_;
        ...
        mutex_type* mutex() const BOOST_NOEXCEPT { return mtx_; }
    ...
    public:
         ...
         */
        externally_locked(T& obj, mutex_type& mtx) BOOST_NOEXCEPT : obj_(&obj), mtx_(&mtx) {}

        /// copy constructor
        externally_locked(externally_locked const& rhs) BOOST_NOEXCEPT : obj_(rhs.obj_), mtx_(rhs.mtx_) {}

        /// move constructor
        externally_locked(BOOST_THREAD_RV_REF(externally_locked) rhs) BOOST_NOEXCEPT : obj_(rhs.obj_), mtx_(rhs.mtx_) {}
