...
namespace boost {
    ...
    struct SharedLockable
    {
        ...
        BOOST_CONCEPT_USAGE(SharedLockable)
        {
            ...
            if (l.try_lock_shared_until(t)) {
                return;
            }
            if (l.try_lock_shared_for(d)) {
                return;
            }
        }
    ...
    struct UpgradeLockable
    {
        ...
        BOOST_CONCEPT_USAGE(UpgradeLockable)
        {
            l.lock_upgrade();
            l.unlock_upgrade();
            if (l.try_lock_upgrade()) {
                return;
            }
            if (l.try_lock_upgrade_until(t)) {
                return;
            }
            if (l.try_lock_upgrade_for(d)) {
                return;
            }
            if (l.try_unlock_shared_and_lock()) {
                return;
            }
            if (l.try_unlock_shared_and_lock_until(t)) {
                return;
            }
            if (l.try_unlock_shared_and_lock_for(d)) {
                return;
            }
            ...
            if (l.try_unlock_shared_and_lock_upgrade_until(t)) {
                return;
            }
            if (l.try_unlock_shared_and_lock_upgrade_for(d)) {
                return;
            }
            l.unlock_and_lock_upgrade();
            l.unlock_upgrade_and_lock();
            if (l.try_unlock_upgrade_and_lock()) {
                return;
            }
            if (l.try_unlock_upgrade_and_lock_until(t)) {
                return;
            }
            if (l.try_unlock_upgrade_and_lock_for(d)) {
                return;
            }
