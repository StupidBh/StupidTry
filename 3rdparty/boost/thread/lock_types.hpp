...
namespace boost {
    ...
    unique_lock(const volatile unique_lock&);
    #endif
#endif
        unique_lock() BOOST_NOEXCEPT : m(0), is_locked(false) {}
        ...
        unique_lock(Mutex& m_, defer_lock_t) BOOST_NOEXCEPT : m(&m_), is_locked(false) {}
        ...
        unique_lock(Mutex& mtx, const chrono::time_point<Clock, Duration>& t) :
            m(&mtx),
            is_locked(mtx.try_lock_until(t))
        {
        }
        ...
        unique_lock(Mutex& mtx, const chrono::duration<Rep, Period>& d) :
            m(&mtx),
            is_locked(mtx.try_lock_for(d))
        {
        }
#endif

        unique_lock(BOOST_THREAD_RV_REF(unique_lock) other) BOOST_NOEXCEPT :
            m(BOOST_THREAD_RV(other).m),
            is_locked(BOOST_THREAD_RV(other).is_locked)
        {
        ...
        BOOST_THREAD_EXPLICIT_LOCK_CONVERSION
            unique_lock(BOOST_THREAD_RV_REF_BEG upgrade_lock<Mutex> BOOST_THREAD_RV_REF_END other);

#ifndef BOOST_THREAD_PROVIDES_EXPLICIT_LOCK_CONVERSION
        // std-2104 unique_lock move-assignment should not be noexcept
        unique_lock&
            operator=(BOOST_THREAD_RV_REF_BEG upgrade_lock<Mutex> BOOST_THREAD_RV_REF_END other) // BOOST_NOEXCEPT
        {
    ...
    unique_lock& operator=(unique_lock<Mutex> other)
    {
      ...
      return *this;
        ...
        template<class Clock, class Duration>
        unique_lock(
            BOOST_THREAD_RV_REF_BEG upgrade_lock<mutex_type> BOOST_THREAD_RV_REF_END ul,
        ...
        template<class Clock, class Duration>
        unique_lock(
            BOOST_THREAD_RV_REF_BEG shared_lock<mutex_type> BOOST_THREAD_RV_REF_END sl,
            const chrono::time_point<Clock, Duration>& abs_time) :
            m(0),
            is_locked(false)
        {
        ...
        void lock()
        {
            if (m == 0) {
                boost::throw_exception(
                    boost::lock_error(
                        static_cast<int>(system::errc::operation_not_permitted),
        ...
        operator bool_type() const BOOST_NOEXCEPT { return is_locked ? &unique_lock::lock : 0; }

        bool operator!() const BOOST_NOEXCEPT { return !owns_lock(); }
#else
        explicit operator bool() const BOOST_NOEXCEPT { return owns_lock(); }
#endif
        bool owns_lock() const BOOST_NOEXCEPT { return is_locked; }

        Mutex* mutex() const BOOST_NOEXCEPT { return m; }
    ...
    BOOST_THREAD_DCL_MOVABLE_BEG(Mutex)
    unique_lock<Mutex> BOOST_THREAD_DCL_MOVABLE_END
    ...
    public:
        ...
        shared_lock() BOOST_NOEXCEPT : m(0), is_locked(false) {}
        ...
        shared_lock(Mutex& m_, defer_lock_t) BOOST_NOEXCEPT : m(&m_), is_locked(false) {}
        ...
        // std-2104 unique_lock move-assignment should not be noexcept
        shared_lock&
            operator=(BOOST_THREAD_RV_REF_BEG shared_lock<Mutex> BOOST_THREAD_RV_REF_END other) // BOOST_NOEXCEPT
        {
        ...
        void unlock()
        {
            ...
            if (!owns_lock()) {
                boost::throw_exception(
                    boost::lock_error(
                        static_cast<int>(system::errc::operation_not_permitted),
                        "boost shared_lock doesn't own the mutex"));
            }
            m->unlock_shared();
            is_locked = false;
        }
        ...
        operator bool_type() const BOOST_NOEXCEPT { return is_locked ? &shared_lock::lock : 0; }
    ...
    BOOST_THREAD_DCL_MOVABLE_BEG(Mutex)
    shared_lock<Mutex> BOOST_THREAD_DCL_MOVABLE_END
    ...
    public:
        ...
        upgrade_lock() BOOST_NOEXCEPT : m(0), is_locked(false) {}
        ...
        upgrade_lock(Mutex& m_, defer_lock_t) BOOST_NOEXCEPT : m(&m_), is_locked(false) {}
        ...
        upgrade_lock(Mutex& mtx, const chrono::time_point<Clock, Duration>& t) :
            m(&mtx),
            is_locked(mtx.try_lock_upgrade_until(t))
        {
        }
        ...
        upgrade_lock(Mutex& mtx, const chrono::duration<Rep, Period>& d) :
            m(&mtx),
            is_locked(mtx.try_lock_upgrade_for(d))
        {
        }
#endif
        ...
        void lock()
        {
            if (m == 0) {
                boost::throw_exception(
                    boost::lock_error(
                        static_cast<int>(system::errc::operation_not_permitted),
                        "boost upgrade_lock has no mutex"));
            }
            if (owns_lock()) {
                boost::throw_exception(
                    boost::lock_error(
                        static_cast<int>(system::errc::resource_deadlock_would_occur),
                        "boost upgrade_lock owns already the mutex"));
            }
            m->lock_upgrade();
            is_locked = true;
        }
        ...
        void unlock()
        {
            ...
            if (!owns_lock()) {
                boost::throw_exception(
                    boost::lock_error(
                        static_cast<int>(system::errc::operation_not_permitted),
                        "boost upgrade_lock doesn't own the mutex"));
            }
            m->unlock_upgrade();
            is_locked = false;
        }
        ...
        operator bool_type() const BOOST_NOEXCEPT { return is_locked ? &upgrade_lock::lock : 0; }
    ...
    BOOST_THREAD_DCL_MOVABLE_BEG(Mutex)
    upgrade_lock<Mutex> BOOST_THREAD_DCL_MOVABLE_END
    ...
    public:
        ...
        upgrade_to_unique_lock(BOOST_THREAD_RV_REF_BEG upgrade_to_unique_lock<Mutex> BOOST_THREAD_RV_REF_END other)
            BOOST_NOEXCEPT :
            source(BOOST_THREAD_RV(other).source),
            exclusive(::boost::move(BOOST_THREAD_RV(other).exclusive))
        {
        ...
        upgrade_to_unique_lock& operator=(
            BOOST_THREAD_RV_REF_BEG upgrade_to_unique_lock<Mutex> BOOST_THREAD_RV_REF_END other) // BOOST_NOEXCEPT
        {
        ...
        operator bool_type() const BOOST_NOEXCEPT { return exclusive.owns_lock() ? &upgrade_to_unique_lock::swap : 0; }
        ...
        bool owns_lock() const BOOST_NOEXCEPT { return exclusive.owns_lock(); }

        Mutex* mutex() const BOOST_NOEXCEPT { return exclusive.mutex(); }
    };

    BOOST_THREAD_DCL_MOVABLE_BEG(Mutex)
    upgrade_to_unique_lock<Mutex> BOOST_THREAD_DCL_MOVABLE_END

        namespace detail
    {
        template<typename Mutex>
        class try_lock_wrapper : private unique_lock<Mutex> {
        ...
        public:
            ...
            void swap(try_lock_wrapper& other) { base::swap(other); }
            ...
            bool owns_lock() const { return base::owns_lock(); }

            Mutex* mutex() const BOOST_NOEXCEPT { return base::mutex(); }

            Mutex* release() { return base::release(); }

#if defined(BOOST_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS)
            typedef typename base::bool_type bool_type;

            operator bool_type() const { return base::operator bool_type(); }

            bool operator!() const { return !this->owns_lock(); }
#else
