...
namespace boost {
    ...
    public:
        ...
        const_strict_lock_ptr(T const& val, Lockable& mtx, adopt_lock_t tag) BOOST_NOEXCEPT : lk_(mtx, tag), value_(val)
        {
        }

        /**
         * Move constructor.
         * @effects takes ownership of the mutex owned by @c other, stores a reference to the mutex and the value type
         * of @c other.
         */
        const_strict_lock_ptr(BOOST_THREAD_RV_REF(const_strict_lock_ptr) other) BOOST_NOEXCEPT :
            lk_(boost::move(BOOST_THREAD_RV(other).lk_)),
            value_(BOOST_THREAD_RV(other).value_)
        {
        }
    ...
    public:
         ...
         */
        T* operator->() { return const_cast<T*>(&this->value_); }
     ...
     *
     * An object of type const_unique_lock_ptr is a unique_lock that provides a const pointer access to the synchronized
     * value type. As unique_lock controls the ownership of a lockable object within a scope. Ownership of the lockable
     * object may be acquired at construction or after construction, and may be transferred, after acquisition, to
     * another const_unique_lock_ptr object. Objects of type const_unique_lock_ptr are not copyable but are movable. The
     * behavior of a program is undefined if the mutex and the value type pointed do not exist for the entire remaining
     * lifetime of the const_unique_lock_ptr object. The supplied Mutex type shall meet the BasicLockable requirements.
     *
    ...
    public:
         ...
         * @effects stores a reference to it and to the value type @c value taking ownership.
         */
        unique_lock_ptr(T& value, Lockable& mtx, adopt_lock_t) BOOST_NOEXCEPT : base_type(value, mtx, adopt_lock) {}
         ...
         * @effects stores a reference to it and to the value type @c value c.
         */
        unique_lock_ptr(T& value, Lockable& mtx, defer_lock_t) BOOST_NOEXCEPT : base_type(value, mtx, defer_lock) {}
         ...
         * @effects try to lock the mutex @c mtx, stores a reference to it and to the value type @c value.
         */
        unique_lock_ptr(T& value, Lockable& mtx, try_to_lock_t) BOOST_NOEXCEPT : base_type(value, mtx, try_to_lock) {}
    ...
    class synchronized_value {
        ...
        friend std::tuple<
            ...
            typename synchronized_value_strict_lock_ptr<SV2>::type> synchronize(SV1& sv1, SV2& sv2);
    ...
    public:
         ...
         *
         */
        mutex_type const& mutex() const { return mtx_; }
         ...
         */
        template<typename F>
        inline typename boost::result_of<F(value_type&)>::type operator()(BOOST_THREAD_RV_REF(F) fct)
        {
            strict_lock<mutex_type> lk(mtx_);
            return fct(value_);
        }
        ...
        template<typename F>
        inline typename boost::result_of<F(value_type const&)>::type operator()(BOOST_THREAD_RV_REF(F) fct) const
        {
            strict_lock<mutex_type> lk(mtx_);
            return fct(value_);
        }
        ...
        template<typename F>
        inline typename boost::result_of<F(value_type&)>::type operator()(F const& fct)
        {
            strict_lock<mutex_type> lk(mtx_);
            return fct(value_);
        }
        ...
        template<typename F>
        inline typename boost::result_of<F(value_type const&)>::type operator()(F const& fct) const
        {
            strict_lock<mutex_type> lk(mtx_);
            return fct(value_);
        }
        ...
        template<typename R>
        inline R operator()(R (*fct)(value_type&))
        {
            strict_lock<mutex_type> lk(mtx_);
            return fct(value_);
        }
        ...
        template<typename R>
        inline R operator()(R (*fct)(value_type const&)) const
        {
            strict_lock<mutex_type> lk(mtx_);
            return fct(value_);
        }
         ...
         * As discussed, operator-> can only lock over the duration of a call, so it is insufficient for complex
         * operations. With synchronize() you get to lock the object in a scoped and to directly access the object
         * inside that scope.
         *
         * Example
         *   void fun(synchronized_value<vector<int>> & v) {
    ...
    public:
        deref_value operator*() { return BOOST_THREAD_MAKE_RV_REF(deref_value(*this)); }

        const_deref_value operator*() const { return BOOST_THREAD_MAKE_RV_REF(const_deref_value(*this)); }
