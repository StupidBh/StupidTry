    ...
    namespace detail {
        ...
        class waiter_for_any_in_seq {
            ...
            struct registered_waiter
            {
                ...
                registered_waiter(future_type& a_future, notify_when_ready_handle handle_, count_type index_) :
                    future_(&a_future),
                    handle(handle_),
                    index(index_)
                {
            ...
            struct all_futures_lock
            {
                    ...
                    locks(new boost::unique_lock<boost::mutex>[count])
                {
                    for (count_type_portable i = 0; i < count; ++i) {
                        locks[i] =
                            BOOST_THREAD_MAKE_RV_REF(boost::unique_lock<boost::mutex>(waiters[i].future_->mutex()));
                    }
                }
