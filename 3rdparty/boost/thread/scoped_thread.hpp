...
namespace boost {
    ...
    public:
        ...
        template<
            class F,
            class... Args,
            typename = typename disable_if<is_same<typename decay<F>::type, Thread>, void*>::type>
         ...
         */
        explicit strict_scoped_thread(BOOST_THREAD_RV_REF(Thread) t) BOOST_NOEXCEPT : t_(boost::move(t)) {}

        /**
         * Destructor
         * Effects: Call the CallableThread functor before destroying the owned thread.
         * Remark: The CallableThread should not throw when joining the thread as the scoped variable is on a scope
     ...
     * Having a wrapper that can join the thread before destroying it seems a natural need.
     *
     * Remark: @c scoped_thread is not a @c thread as @c thread is not designed to be derived from as a polymorphic
     * type. Anyway @c scoped_thread can be used in most of the contexts a @c thread could be used as it has the same
     * non-deprecated interface with the exception of the construction.
     *
     * Example:
     *
    ...
    public:
         ...
         */
        scoped_thread() BOOST_NOEXCEPT : t_() {}
         ...
         * Effects: move the thread to own @c t.
         */
        explicit scoped_thread(BOOST_THREAD_RV_REF(Thread) t) BOOST_NOEXCEPT : t_(boost::move(t)) {}
         ...
         * Move constructor.
         */
        scoped_thread(BOOST_RV_REF(scoped_thread) x) BOOST_NOEXCEPT : t_(boost::move(BOOST_THREAD_RV(x).t_)) {}
         ...
         *
         */
        void swap(scoped_thread& x) BOOST_NOEXCEPT { t_.swap(x.t_); }

        // forwarded thread functions
        inline id get_id() const BOOST_NOEXCEPT { return t_.get_id(); }
        ...
        native_handle_type native_handle() BOOST_NOEXCEPT { return t_.native_handle(); }

        bool joinable() const BOOST_NOEXCEPT { return t_.joinable(); }

#if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
        void interrupt() { t_.interrupt(); }

        bool interruption_requested() const BOOST_NOEXCEPT { return t_.interruption_requested(); }
#endif

        static unsigned hardware_concurrency() BOOST_NOEXCEPT { return Thread::hardware_concurrency(); }

#ifdef BOOST_THREAD_PROVIDES_PHYSICAL_CONCURRENCY
        static unsigned physical_concurrency() BOOST_NOEXCEPT { return Thread::physical_concurrency(); }
#endif
    };
    ...
    void swap(scoped_thread<Destroyer, Thread>& lhs, scoped_thread<Destroyer, Thread>& rhs) BOOST_NOEXCEPT
    {
        return lhs.swap(rhs);
    }
