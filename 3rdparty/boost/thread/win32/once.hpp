...
namespace boost {
    ...
    struct once_flag
    {
        ...
        BOOST_CONSTEXPR once_flag() BOOST_NOEXCEPT : status(0), count(0) {}
    ...
    namespace detail {
        ...
        unsigned const once_mutex_name_length =
            once_mutex_name_fixed_length + sizeof(void*) * 2 + sizeof(unsigned long) * 2 + 1;
...
#endif
            BOOST_STATIC_ASSERT(
                sizeof(fixed_mutex_name) == (sizeof(once_char_type) * (once_mutex_name_fixed_length + 1)));

            std::memcpy(mutex_name, fixed_mutex_name, sizeof(fixed_mutex_name));
            detail::int_to_string(
                reinterpret_cast<std::ptrdiff_t>(flag_address),
...
#endif
                ::boost::detail::win32::synchronize | ::boost::detail::win32::event_modify_state,
                false,
                mutex_name);
        }
        ...
        inline void commit_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
            ...
            if (!ctx.event_handle && (::boost::detail::interlocked_read_acquire(&flag.count) > 1)) {
    ...
    inline void call_once(once_flag& flag, void (*f)())
    {
        ...
        while (::boost::detail::interlocked_read_acquire(&flag.status) != ctx.function_complete_flag_value) {
            ...
            BOOST_VERIFY(
                !::boost::winapi::WaitForSingleObjectEx(ctx.event_handle, ::boost::detail::win32::infinite, 0));
        }
    }
    ...
    inline void call_once(
        once_flag& flag,
        BOOST_THREAD_RV_REF(Function) f,
        BOOST_THREAD_RV_REF(A) a,
    ...
    void call_once(once_flag& flag, void (*f)(BOOST_THREAD_RV_REF(T1)), BOOST_THREAD_RV_REF(T1) p1)
    {
        ...
        while (::boost::detail::interlocked_read_acquire(&flag.status) != ctx.function_complete_flag_value) {
            if (detail::enter_once_region(flag, ctx)) {
                BOOST_TRY
                {
                    f(thread_detail::decay_copy(boost::forward<T1>(p1)));
                }
                BOOST_CATCH(...)
                {
                    detail::rollback_once_region(flag, ctx);
    ...
    void call_once(
        once_flag& flag,
        void (*f)(BOOST_THREAD_RV_REF(T1), BOOST_THREAD_RV_REF(T2)),
        BOOST_THREAD_RV_REF(T1) p1,
        BOOST_THREAD_RV_REF(T2) p2)
    {
        // Try for a quick win: if the procedure has already been called
        // just skip through:
        detail::once_context ctx;
