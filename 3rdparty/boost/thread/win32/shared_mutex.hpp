...
namespace boost {
    ...
    private:
        struct state_data
        {
            unsigned long shared_count: 11, shared_waiting: 11, exclusive: 1, upgrade: 1, exclusive_waiting: 7,
        ...
        static state_data interlocked_compare_exchange(state_data* target, state_data new_value, state_data comparand)
        {
            ...
            long const res = BOOST_INTERLOCKED_COMPARE_EXCHANGE(reinterpret_cast<long*>(target), new_val, comp);
            state_data result;
            ...
            if (old_state.shared_waiting || old_state.exclusive_waiting) {
                BOOST_VERIFY(
                    winapi::ReleaseSemaphore(
                        semaphores[unlock_sem],
                        old_state.shared_waiting + (old_state.exclusive_waiting ? 1 : 0),
                        0) != 0);
            }
        }
    ...
    public:
        ...
        void lock_shared()
        {
            for (;;) {
                ...
                BOOST_VERIFY(
                    winapi::WaitForSingleObjectEx(semaphores[unlock_sem], ::boost::detail::win32::infinite, 0) == 0);
            }
        }

    private:
        unsigned long getMs(detail::platform_duration const& d) { return static_cast<unsigned long>(d.getMs()); }
    ...
    public:
        ...
        {
            const detail::mono_platform_timepoint t(
                detail::mono_platform_clock::now() + detail::platform_duration(relative_time));
        ...
        bool timed_lock_shared(boost::system_time const& wait_until)
        {
            ...
            return do_lock_shared_until<detail::real_platform_clock>(
                t,
                detail::platform_milliseconds(BOOST_THREAD_POLL_INTERVAL_MILLISECONDS));
        }
#endif
        ...
        bool try_lock_shared_until(const chrono::time_point<Clock, Duration>& t)
        {
            ...
            return do_lock_shared_until<Clock>(
                t,
...
#endif
                BOOST_VERIFY(
                    winapi::WaitForMultipleObjectsEx(2, semaphores, wait_all, ::boost::detail::win32::infinite, 0) < 2);
            }
        }
    ...
    public:
        ...
        {
            const detail::real_platform_timepoint t(wait_until);
            return do_lock_until<detail::real_platform_clock>(
                t,
                detail::platform_milliseconds(BOOST_THREAD_POLL_INTERVAL_MILLISECONDS));
        }
