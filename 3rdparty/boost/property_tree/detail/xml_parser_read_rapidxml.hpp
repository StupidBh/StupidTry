    ...
    namespace property_tree {
        namespace xml_parser {
            ...
            void read_xml_node(detail::rapidxml::xml_node<Ch>* node, Ptree& pt, int flags)
            {
                ...
                case node_element: {
                    ...
                    if (node->first_attribute()) {
                        Ptree& pt_attr_root =
                            pt_node.push_back(std::make_pair(xmlattr<typename Ptree::key_type>(), Ptree()))->second;
                ...
                case node_cdata: {
                    if (flags & no_concat_text) {
                        pt.push_back(std::make_pair(xmltext<typename Ptree::key_type>(), Ptree(node->value())));
                    }
                    else {
                ...
                case node_comment: {
                    if (!(flags & no_comments)) {
                        pt.push_back(
                            std::make_pair(
                                xmlcomment<typename Ptree::key_type>(),
                ...
                const std::string& filename)
            {
                ...
                std::vector<Ch> v(std::istreambuf_iterator<Ch>(stream.rdbuf()), std::istreambuf_iterator<Ch>());
                if (!stream.good()) {
                ...
                try {
                    ...
                    const int f_tws = parse_normalize_whitespace | parse_trim_whitespace;
                    const int f_c = parse_comment_nodes;
                    // Some compilers don't like the bitwise or in the template arg.
                    const int f_tws_c = parse_normalize_whitespace | parse_trim_whitespace | parse_comment_nodes;
                    xml_document<Ch> doc;
                    if (flags & no_comments) {
                    ...
                    Ptree local;
                    for (xml_node<Ch>* child = doc.first_node(); child; child = child->next_sibling()) {
