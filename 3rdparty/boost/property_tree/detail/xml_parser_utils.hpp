            ...
            Str decode_char_entities(const Str& s)
            {
                ...
                for (typename Str::const_iterator it = s.begin(); it != end; ++it) {
                    if (*it == Ch('&')) {
                        ...
                        if (ent == detail::widen<Str>("lt")) {
                            r += Ch('<');
                        }
                        else if (ent == detail::widen<Str>("gt")) {
                            r += Ch('>');
                        }
                        else if (ent == detail::widen<Str>("amp")) {
                            r += Ch('&');
                        }
                        else if (ent == detail::widen<Str>("quot")) {
                            r += Ch('"');
                        }
                        else if (ent == detail::widen<Str>("apos")) {
                            r += Ch('\'');
                        }
                        else {
                            BOOST_PROPERTY_TREE_THROW(xml_parser_error("invalid character entity", "", 0));
