...
namespace boost {
    namespace property_tree {
        namespace xml_parser {
            template<class Str>
            void write_xml_indent(
                std::basic_ostream<typename Str::value_type>& stream,
                int indent,
                const xml_writer_settings<Str>& settings)
            {
                stream << std::basic_string<typename Str::value_type>(
                    indent * settings.indent_count,
                    settings.indent_char);
            }

            template<class Str>
            void write_xml_comment(
                std::basic_ostream<typename Str::value_type>& stream,
                const Str& s,
                int indent,
                bool separate_line,
                        ...
                        if (optional<const Ptree&> attribs = pt.get_child_optional(xmlattr<Str>())) {
                            for (It it = attribs.get().begin(); it != attribs.get().end(); ++it) {
                                stream << Ch(' ') << it->first << Ch('=') << Ch('"')
                                       << encode_char_entities(it->second.template get_value<Str>()) << Ch('"');
                            }
                        }
            ...
            {
                typedef typename Ptree::key_type Str;
                stream << detail::widen<Str>("<?xml version=\"1.0\" encoding=\"") << settings.encoding
                       << detail::widen<Str>("\"?>\n");
