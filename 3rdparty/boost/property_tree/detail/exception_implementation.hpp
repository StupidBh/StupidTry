...
namespace boost {
    namespace property_tree {
        ...
        namespace detail {
            ...
            template<class P>
            inline std::string prepare_bad_path_what(const std::string& what, const P& path)
            {
        ...
        template<class D>
        inline ptree_bad_data::ptree_bad_data(const std::string& w, const D& d) :
            ptree_error(w),
            m_data(d)
        {
        }
        ...
        template<class D>
        inline D ptree_bad_data::data() const
        {
        ...
        template<class P>
        inline ptree_bad_path::ptree_bad_path(const std::string& w, const P& p) :
            ptree_error(detail::prepare_bad_path_what(w, p)),
            m_path(p)
        {
        }

        template<class P>
        inline P ptree_bad_path::path() const
        {
