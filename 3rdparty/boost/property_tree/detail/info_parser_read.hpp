...
namespace boost {
    namespace property_tree {
        namespace info_parser {
            ...
            template<class It>
            std::basic_string<typename std::iterator_traits<It>::value_type> expand_escapes(It b, It e)
            {
                ...
                try {
                    ...
                    while (stream.good()) {
                        ...
                        if (!stream.good() && !stream.eof()) {
                            BOOST_PROPERTY_TREE_THROW(info_parser_error("read error", filename, line_no));
                        }
                            ...
                            if (directive == convert_chtype<Ch, char>("include")) {
                                ...
                                if (!inc_stream.good()) {
                                    BOOST_PROPERTY_TREE_THROW(
                                        info_parser_error("cannot open include file " + inc_name, filename, line_no));
                                }
                                read_info_internal(inc_stream, *stack.top(), inc_name, include_depth + 1);
                            }
                            else { // Unknown directive
                                BOOST_PROPERTY_TREE_THROW(info_parser_error("unknown directive", filename, line_no));
                            }

                            // Directive must be followed by end of line
                            skip_whitespace(text);
                            if (*text != Ch('\0')) {
                                BOOST_PROPERTY_TREE_THROW(info_parser_error("expected end of line", filename, line_no));
                            }
                        ...
                        while (1) {
                            ...
                            case s_data_cont: {
                                ...
                                if (*text == Ch('\"')) // Continuation must start with "
                                {
                                    ...
                                    BOOST_PROPERTY_TREE_THROW(
                                        info_parser_error("expected \" after \\ in previous line", "", 0));
                                }
