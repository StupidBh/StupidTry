...
namespace boost {
    namespace property_tree {
        ...
        inline void save(Archive& ar, const basic_ptree<K, D, C>& t, const unsigned int /*file_version*/)
        {
        ...
        namespace detail {
            template<class Archive, class K, class D, class C>
            inline void load_children(Archive& ar, basic_ptree<K, D, C>& t)
            {
                ...
                const bsl::library_version_type library_version(ar.get_library_version());
                ...
                t.clear();
                while (count-- > 0) {
                    bsl::detail::stack_construct<Archive, value_type> u(ar, item_version);
         ...
         */
        template<class Archive, class K, class D, class C>
        inline void load(Archive& ar, basic_ptree<K, D, C>& t, const unsigned int /*file_version*/)
        {
            namespace bsl = boost::serialization;
         ...
         */
        template<class Archive, class K, class D, class C>
        inline void serialize(Archive& ar, basic_ptree<K, D, C>& t, const unsigned int file_version)
        {
            using namespace boost::serialization;
