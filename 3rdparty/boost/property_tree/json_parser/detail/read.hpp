    ...
    namespace property_tree {
        namespace json_parser {
            namespace detail {
                ...
                class minirange {
                public:
                    minirange(Iterator first, Sentinel last) :
                        first(first),
                        last(last)
                    {
                    }
                ...
                template<typename Iterator, typename Sentinel, typename Encoding, typename Callbacks>
                    ...
                    const std::string& filename)
                {
                    ...
                    BOOST_STATIC_ASSERT_MSG(
                        (boost::is_same<typename Callbacks::char_type, typename Encoding::internal_char>::value),
                        "Encoding is not capable of producing the needed character type.");

                    detail::parser<Callbacks, Encoding, Iterator, Sentinel> parser(callbacks, encoding);
                ...
                template<>
                struct encoding<char> : utf8_utf8_encoding
                {
                };

                template<>
                struct encoding<wchar_t> : wide_wide_encoding
                {
                };
                    ...
                    const std::string& filename)
                {
                    ...
                    read_json_internal(iterator(stream), iterator(), encoding, callbacks, filename);
