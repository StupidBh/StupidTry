        ...
        namespace ini_parser {
                ...
                while (stream.good()) {
                    ...
                    if (!line.empty()) {
                        ...
                        if (line[0] == semicolon || line[0] == hash) {
                            ...
                            Str key = property_tree::detail::trim(line.substr(1, end - 1), stream.getloc());
                            ...
                            if (eqpos == Str::npos) {
                                BOOST_PROPERTY_TREE_THROW(
                                    ini_parser_error("'=' character not found in line", "", line_no));
                            }
                            if (eqpos == 0) {
            ...
            namespace detail {
                ...
                void check_dupes(const Ptree& pt)
                {
                    ...
                    typename Ptree::const_assoc_iterator it = pt.ordered_begin(), end = pt.not_found();
                    lastkey = &it->first;
                    for (++it; it != end; ++it) {
                        if (*lastkey == it->first) {
                            BOOST_PROPERTY_TREE_THROW(ini_parser_error("duplicate key", "", 0));
                        }
                        lastkey = &it->first;
                    }
                }
                    ...
                    bool throw_on_children)
                {
                    ...
                    for (typename Ptree::const_iterator it = pt.begin(), end = pt.end(); it != end; ++it) {
                        ...
                        stream << it->first << Ch('=') << it->second.template get_value<std::basic_string<Ch>>()
                               << Ch('\n');
                    }
                }

                template<typename Ptree>
                void write_top_level_keys(
                    std::basic_ostream<typename Ptree::key_type::value_type>& stream,
                    const Ptree& pt)
                {
                    ...
                    for (typename Ptree::const_iterator it = pt.begin(), end = pt.end(); it != end; ++it) {
                        if (!it->second.empty()) {
                            ...
                            if (!it->second.data().empty()) {
                                BOOST_PROPERTY_TREE_THROW(ini_parser_error("mixed data and children", "", 0));
                            }
                ...
                int flags = 0)
            {
                ...
                if (!pt.data().empty()) {
                    BOOST_PROPERTY_TREE_THROW(ini_parser_error("ptree has data on root", "", 0));
                }
                ...
                const std::locale& loc = std::locale())
            {
                ...
                stream.flush();
                if (!stream) {
                    BOOST_PROPERTY_TREE_THROW(ini_parser_error("write error", filename, 0));
                }
            }
