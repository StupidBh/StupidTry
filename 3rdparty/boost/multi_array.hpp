...
namespace boost {
        ...
        namespace multi_array {
            ...
            struct populate_index_ranges
            {
                multi_array_types::index_range
                    ...
                    operator()(multi_array_types::index base, multi_array_types::size_type extent_)
                {
            ...
            template<class T>
            struct disable_multi_array_impl : disable_multi_array_impl_impl<is_multi_array_impl<T>::value>
            {
            };
    ...
    class multi_array : public multi_array_ref<T, NumDims>, private boost::empty_value<Allocator> {
    ...
    public:
...
#ifdef BOOST_NO_FUNCTION_TEMPLATE_ORDERING
                ,
            typename mpl::if_<detail::multi_array::is_multi_array_impl<ExtentList>, int&, int>::type* = 0
#endif
            ) :
        ...
        template<class ExtentList>
        explicit multi_array(ExtentList const& extents, const general_storage_order<NumDims>& so) :
        ...
        multi_array(const const_multi_array_ref<T, NumDims>& rhs, const Allocator& alloc = Allocator()) :
        ...
        multi_array(
            const const_multi_array_ref<T, NumDims>& rhs,
            const general_storage_order<NumDims>& so,
            const Allocator& alloc = Allocator()) :
            super_type(0, so, rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        ...
        multi_array(const detail::multi_array::const_sub_array<T, NumDims>& rhs, const Allocator& alloc = Allocator()) :
            super_type(0, c_storage_order(), rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        {
            allocate_space();
        ...
        multi_array(
            const detail::multi_array::const_sub_array<T, NumDims>& rhs,
            const general_storage_order<NumDims>& so,
            const Allocator& alloc = Allocator()) :
            super_type(0, so, rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        ...
        multi_array(
            const detail::multi_array::const_multi_array_view<T, NumDims>& rhs,
            const Allocator& alloc = Allocator()) :
            super_type(0, c_storage_order(), rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        {
        ...
        multi_array(const multi_array_ref<T, NumDims>& rhs, const Allocator& alloc = Allocator()) :
            super_type(0, c_storage_order(), rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        {
            allocate_space();
        ...
        multi_array(
            const multi_array_ref<T, NumDims>& rhs,
            const general_storage_order<NumDims>& so,
            const Allocator& alloc = Allocator()) :
            super_type(0, so, rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        ...
        multi_array(const detail::multi_array::sub_array<T, NumDims>& rhs, const Allocator& alloc = Allocator()) :
            super_type(0, c_storage_order(), rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        {
            allocate_space();
        ...
        multi_array(
            const detail::multi_array::sub_array<T, NumDims>& rhs,
            const general_storage_order<NumDims>& so,
            const Allocator& alloc = Allocator()) :
            super_type(0, so, rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        ...
        multi_array(
            const detail::multi_array::multi_array_view<T, NumDims>& rhs,
            const Allocator& alloc = Allocator()) :
            super_type(0, c_storage_order(), rhs.index_bases(), rhs.shape()),
            alloc_base(boost::empty_init_t(), alloc)
        {
        ...
        multi_array& resize(const detail::multi_array ::extent_gen<NumDims>& ranges)
        {
            ...
            const size_type& (*min)(const size_type&, const size_type&) = std::min;
            std::transform(
            ...
            typename multi_array::BOOST_NESTED_TEMPLATE array_view<NumDims>::type view_old = (*this)[old_idxes];
            typename multi_array::BOOST_NESTED_TEMPLATE array_view<NumDims>::type view_new = new_array[new_idxes];
        ...
        ~multi_array() { deallocate_space(); }

    private:
        friend inline bool operator==(const multi_array& a, const multi_array& b) { return a.base() == b.base(); }

        friend inline bool operator!=(const multi_array& a, const multi_array& b) { return !(a == b); }

        const super_type& base() const { return *this; }

        const Allocator& allocator() const { return alloc_base::get(); }

        Allocator& allocator() { return alloc_base::get(); }
