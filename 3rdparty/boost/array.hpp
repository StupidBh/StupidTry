...
#include <boost/config/workaround.hpp>
    ...
    #pragma warning( \
        disable: 4610) // class 'boost::array<T,N>' can never be instantiated - user defined constructor required
...
namespace boost {
    ...
    public:
        ...
        BOOST_CONSTEXPR
#endif
        const_reference operator[](size_type i) const { return BOOST_ASSERT_MSG(i < N, "out of range"), elems[i]; }
        ...
        // front() and back()
        BOOST_CXX14_CONSTEXPR reference front() { return elems[0]; }

        BOOST_CONSTEXPR const_reference front() const { return elems[0]; }

        BOOST_CXX14_CONSTEXPR reference back() { return elems[N - 1]; }

        BOOST_CONSTEXPR const_reference back() const { return elems[N - 1]; }
        ...
        BOOST_CXX14_CONSTEXPR void swap(array<T, N>& y) { std::swap(elems, y.elems); }
    ...
    public:
        struct
        {
        } elems; // enables initialization with = {{}}
    ...
    template<class T, std::size_t N>
    constexpr auto operator<=>(const array<T, N>& x, const array<T, N>& y) -> decltype(x.elems[0] <=> y.elems[0])
    {
        for (std::size_t i = 0; i < N; ++i) {
    ...
    template<class T>
    constexpr auto operator<=>(const array<T, 0>& /*x*/, const array<T, 0>& /*y*/) -> std::strong_ordering
    {
        return std::strong_ordering::equal;
    }
