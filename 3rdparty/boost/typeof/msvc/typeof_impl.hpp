...
namespace boost {
    ...
    #define BOOST_TYPEOF_NEXT_INDEX(next) friend char (*encode_index(encode_counter<next>*))[next];
        ...
        template<typename T>
        typename std::enable_if<!std::is_function<T>::value, typename sizer<T>::type>::type encode_start(T const&);

        template<typename T>
        typename std::enable_if<std::is_function<T>::value, typename sizer<T>::type>::type encode_start(T&);
...
#define BOOST_TYPEOF_NESTED_TYPEDEF_TPL(name, expr)                                           \
    ...
    {                                                                                         \
        enum                                                                                  \
        {                                                                                     \
            _typeof_register_value = sizeof(boost::type_of::typeof_register_type<name>(expr)) \
        };                                                                                    \
