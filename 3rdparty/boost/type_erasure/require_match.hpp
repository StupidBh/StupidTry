...
namespace boost {
    namespace type_erasure {
        ...
        {
            ::boost::type_erasure::is_relaxed<typename ::boost::type_erasure::detail::extract_concept<
        ...
        void require_match(const Op& op, U&&... arg)
        {
            ::boost::type_erasure::is_relaxed<::boost::type_erasure::detail::extract_concept_t<
...
namespace detail {
    ...
    void require_match_impl(
        ::boost::mpl::true_,
        const ::boost::type_erasure::binding<Concept>& table,
        const Op& op BOOST_PP_ENUM_TRAILING_BINARY_PARAMS(N, U, RREF arg))
    {
        if (!::boost::type_erasure::check_match(table, op BOOST_TYPE_ERASURE_FORWARD_ARGS(N, U, arg))) {
            BOOST_THROW_EXCEPTION(::boost::type_erasure::bad_function_call());
        }
    }
    ...
    void require_match_impl(
        ::boost::mpl::false_,
        const ::boost::type_erasure::binding<Concept>&,
        const Op& BOOST_PP_ENUM_TRAILING_BINARY_PARAMS(N, U, RREF BOOST_PP_INTERCEPT))
    {
    }
    ...
    void require_match_impl(::boost::mpl::true_, const Op& op BOOST_PP_ENUM_TRAILING_BINARY_PARAMS(N, U, RREF arg))
    {
        if (!::boost::type_erasure::check_match(op BOOST_TYPE_ERASURE_FORWARD_ARGS(N, U, arg))) {
            BOOST_THROW_EXCEPTION(::boost::type_erasure::bad_function_call());
        }
    }
...
{
    ::boost::type_erasure::is_relaxed<Concept> cond;
    ::boost::type_erasure::detail::require_match_impl(cond, table, op BOOST_TYPE_ERASURE_FORWARD_ARGS(N, U, arg));
}

    #if N != 0

template<class Op BOOST_PP_ENUM_TRAILING_PARAMS(N, class U)>
void require_match(const Op& op BOOST_PP_ENUM_TRAILING_BINARY_PARAMS(N, U, RREF arg))
{
    ::boost::type_erasure::is_relaxed<
        typename ::boost::type_erasure::detail::BOOST_PP_CAT(do_extract_concept, N) <
            typename ::boost::type_erasure::detail::get_signature<Op>::type,
        BOOST_PP_ENUM_PARAMS(N, U)>::type > cond;
    ::boost::type_erasure::detail::require_match_impl(cond, op BOOST_TYPE_ERASURE_FORWARD_ARGS(N, U, arg));
}
