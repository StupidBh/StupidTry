...
namespace boost {
    namespace type_erasure {
        namespace detail {
            ...
            struct get_placeholders_in_argument_impl<T&>
            {
                template<class Out>
                using apply =
                    typename ::boost::type_erasure::detail::get_placeholders_in_argument_impl<T>::template apply<Out>;
            };
            ...
            template<class Out, class... T>
            using get_placeholders_impl = ::boost::mp11::
                mp_fold<::boost::mp11::mp_list<T...>, Out, ::boost::type_erasure::detail::get_placeholders_f>;
            ...
            struct get_placeholders_in_argument
            {
                typedef typename ::boost::mpl::eval_if<
                    ...
                    ::boost::mpl::identity<Out>>::type type;
            };
            ...
            {
                typedef typename ::boost::type_erasure::detail::get_placeholders_in_argument<T, Out>::type type;
            };
            ...
            struct get_placeholders_impl<Out, T0, T...>
            {
                typedef typename ::boost::type_erasure::detail::
                    get_placeholders_in_argument<T0, typename get_placeholders_impl<Out, T...>::type>::type type;
            };
    ...
    #define BOOST_TYPE_ERASURE_GET_PLACEHOLDER(z, n, data)                            \
        typedef typename ::boost::type_erasure::detail::get_placeholders_in_argument< \
            BOOST_PP_CAT(data, n),                                                    \
            BOOST_PP_CAT(type, n)>::type BOOST_PP_CAT(type, BOOST_PP_INC(n));

    #if N != 0

template<template<BOOST_PP_ENUM_PARAMS(N, class T)> class T, BOOST_PP_ENUM_PARAMS(N, class T), class Out>
...
template<class R BOOST_PP_ENUM_TRAILING_PARAMS(N, class T), class Out>
struct get_placeholders_in_argument<R(BOOST_PP_ENUM_PARAMS(N, T)), Out>
{
    typedef typename ::boost::type_erasure::detail::get_placeholders_in_argument<R, Out>::type type0;
    BOOST_PP_REPEAT(N, BOOST_TYPE_ERASURE_GET_PLACEHOLDER, T)
    typedef BOOST_PP_CAT(type, N) type;
};
