    ...
    namespace urls {
        ...
        /** A non-owning reference to a valid authority
            ...
            @see
                @ref parse_authority.
        */
        class BOOST_URL_DECL authority_view : private detail::parts_base {
            detail::url_impl u_;

            friend struct detail::url_impl;

            explicit authority_view(detail::url_impl const& u) noexcept;
        ...
        public:
            ...
            */
            ...
            /** Construct from a string.
                ...
                @see
                    @ref parse_authority.
            */
            explicit authority_view(core::string_view s);
            ...
            /** Assignment
                ...
                Throws nothing.
            */
            authority_view& operator=(authority_view const& other) noexcept;
            ...
            /** Return the number of characters in the authority
                ...
                Throws nothing.
            */
            std::size_t size() const noexcept { return u_.offset(id_end); }
            ...
            */
            ...
            /** Return a pointer to the first character
                ...
                Throws nothing.
            */
            char const* data() const noexcept { return u_.cs_; }
            ...
            */
            ...
            BOOST_URL_STRTOK_RETURN userinfo(BOOST_URL_STRTOK_ARG(token)) const
            {
                encoding_opts opt;
                opt.space_as_plus = false;
                return encoded_userinfo().decode(opt, std::move(token));
            }
            ...
            */
            template<BOOST_URL_STRTOK_TPARAM>
            BOOST_URL_STRTOK_RETURN user(BOOST_URL_STRTOK_ARG(token)) const
            {
                encoding_opts opt;
                opt.space_as_plus = false;
                return encoded_user().decode(opt, std::move(token));
            }
            ...
            */
            template<BOOST_URL_STRTOK_TPARAM>
            BOOST_URL_STRTOK_RETURN password(BOOST_URL_STRTOK_ARG(token)) const
            {
                encoding_opts opt;
                opt.space_as_plus = false;
                return encoded_password().decode(opt, std::move(token));
            }
            ...
            */
            urls::host_type host_type() const noexcept { return u_.host_type_; }
            ...
            */
            template<BOOST_URL_STRTOK_TPARAM>
            BOOST_URL_STRTOK_RETURN host(BOOST_URL_STRTOK_ARG(token)) const
            {
                encoding_opts opt;
                opt.space_as_plus = false;
                return encoded_host().decode(opt, std::move(token));
            }
            ...
            */
            template<BOOST_URL_STRTOK_TPARAM>
            BOOST_URL_STRTOK_RETURN host_address(BOOST_URL_STRTOK_ARG(token)) const
            {
                encoding_opts opt;
                opt.space_as_plus = false;
                return encoded_host_address().decode(opt, std::move(token));
            }
            ...
            */
            template<BOOST_URL_STRTOK_TPARAM>
            BOOST_URL_STRTOK_RETURN host_name(BOOST_URL_STRTOK_ARG(token)) const
            {
                encoding_opts opt;
                opt.space_as_plus = false;
                return encoded_host_name().decode(opt, std::move(token));
            }
            ...
            */
            friend bool operator==(authority_view const& a0, authority_view const& a1) noexcept
            {
            ...
            /** Return the result of comparing two authorities.
                ...
                @return `true` if `a0 != a1`, otherwise `false`
            */
            friend bool operator!=(authority_view const& a0, authority_view const& a1) noexcept { return !(a0 == a1); }
            ...
            */
            friend std::ostream& operator<<(std::ostream& os, authority_view const& a) { return os << a.buffer(); }
        };
        ...
        */
        BOOST_URL_DECL
        system::result<authority_view> parse_authority(core::string_view s) noexcept;
