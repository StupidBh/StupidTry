    ...
    namespace urls {
        namespace grammar {
            ...
            */
            ...
            char const* find_if(char const* const first, char const* const last, CS const& cs) noexcept
            {
                // If you get a compile error here
                // it means your type does not meet
                // the requirements. Please check the
                // documentation.
                static_assert(is_charset<CS>::value, "CharSet requirements not met");

                return detail::find_if(first, last, cs, detail::has_find_if<CS> {});
            }
            ...
            */
            template<BOOST_URL_CONSTRAINT(CharSet) CS>
            char const* find_if_not(char const* const first, char const* const last, CS const& cs) noexcept
            {
                // If you get a compile error here
                // it means your type does not meet
                // the requirements. Please check the
                ...
                return detail::find_if_not(first, last, cs, detail::has_find_if_not<CS> {});
            }
            ...
            namespace implementation_defined {
                ...
                struct charset_ref
                {
                    ...
                    constexpr bool operator()(char ch) const noexcept { return cs_(ch); }

                    char const* find_if(char const* first, char const* last) const noexcept
                    {
                        return grammar::find_if(first, last, cs_);
                    }

                    char const* find_if_not(char const* first, char const* last) const noexcept
                    {
                        return grammar::find_if_not(first, last, cs_);
                    }
                };
            } // namespace implementation_defined
            ...
            */
            ...
            constexpr typename std::enable_if<
                is_charset<CS>::value && !std::is_same<CS, implementation_defined::charset_ref<CS>>::value,
