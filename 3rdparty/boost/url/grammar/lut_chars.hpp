        ...
        namespace grammar {
            ...
            namespace detail {
                ...
                struct is_pred<
                    T,
                    void_t<
                        decltype(std::declval<bool&>() = std::declval<T const&>().operator()(std::declval<char>()))>> :
                    std::true_type
                {
                };
            } // namespace detail
            ...
            */
            class lut_chars {
                ...
                constexpr static std::uint64_t lo(char c) noexcept { return static_cast<unsigned char>(c) & 3; }

                constexpr static std::uint64_t hi(char c) noexcept
                {
                    return 1ULL << (static_cast<unsigned char>(c) >> 2);
                }

                constexpr static lut_chars construct(char const* s) noexcept
                {
                    return *s ? lut_chars(*s) + construct(s + 1) : lut_chars();
                }

                constexpr static lut_chars construct(unsigned char ch, bool b) noexcept
                {
                    return b ? lut_chars(ch) : lut_chars();
                }

                template<class Pred>
                constexpr static lut_chars construct(Pred pred, unsigned char ch) noexcept
                {
                    return ch == 255 ? construct(ch, pred(static_cast<char>(ch)))
                                     : construct(ch, pred(static_cast<char>(ch))) + construct(pred, ch + 1);
                }

                constexpr lut_chars() = default;

                constexpr lut_chars(std::uint64_t m0, std::uint64_t m1, std::uint64_t m2, std::uint64_t m3) noexcept :
                    mask_ { m0, m1, m2, m3 }
                {
                }
            ...
            public:
                ...
                */
                constexpr lut_chars(char const* s) noexcept :
                ...
                */
                template<
                    ...
                    class = typename std::enable_if<
                        detail::is_pred<Pred>::value && !std::is_base_of<lut_chars, Pred>::value>::type
#endif
                    >
                constexpr lut_chars(Pred const& pred) noexcept :
                    lut_chars(construct(pred, 0))
                {
                }
                ...
                */
                constexpr bool operator()(unsigned char ch) const noexcept { return operator()(static_cast<char>(ch)); }

                /// @copydoc operator()(unsigned char) const
                constexpr bool operator()(char ch) const noexcept { return mask_[lo(ch)] & hi(ch); }
                ...
                */
                friend constexpr lut_chars operator+(lut_chars const& cs0, lut_chars const& cs1) noexcept
                {
                    return lut_chars(
                ...
                */
                ...
                {
                    return lut_chars(~mask_[0], ~mask_[1], ~mask_[2], ~mask_[3]);
                }
