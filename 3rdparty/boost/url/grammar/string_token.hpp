            ...
            */
            ...
            namespace implementation_defined {
                ...
                struct is_token<
                    T,
                    void_t<
                        decltype(std::declval<T&>().prepare(std::declval<std::size_t>())),
                        decltype(std::declval<T&>().result())>> :
                    std::integral_constant<
                        bool,
                        std::is_convertible<decltype(std::declval<T&>().result()), typename T::result_type>::value &&
                            std::is_same<decltype(std::declval<T&>().prepare(0)), char*>::value &&
                            std::is_base_of<arg, T>::value &&
                            std::is_convertible<T const volatile*, arg const volatile*>::value>
                {
                };
            } // namespace implementation_defined
             ...
             */
            template<class T>
            concept StringToken = std::derived_from<T, string_token::arg> && requires(T t, std::size_t n) {
            ...
            namespace implementation_defined {
                struct return_string : arg
                {
                    ...
                    result_type result() noexcept { return std::move(s_); }
            ...
            namespace implementation_defined {
                ...
                struct append_to_t : arg
                {
                    ...
                    result_type result() noexcept { return s_; }
            ...
            namespace implementation_defined {
                template<class Alloc>
                struct assign_to_t : arg
                {
                    using string_type = std::basic_string<char, std::char_traits<char>, Alloc>;

                    using result_type = string_type&;

                    explicit assign_to_t(string_type& s) noexcept :
                        s_(s)
                    {
                    }
            ...
            namespace implementation_defined {
                template<class Alloc>
                struct preserve_size_t : arg
                {
                    using result_type = core::string_view;

                    using string_type = std::basic_string<char, std::char_traits<char>, Alloc>;

                    explicit preserve_size_t(string_type& s) noexcept :
                        s_(s)
                    {
                    }
                    ...
                    result_type result() noexcept { return core::string_view(s_.data(), n_); }
