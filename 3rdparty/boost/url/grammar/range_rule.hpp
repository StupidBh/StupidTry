        ...
        namespace grammar {
            ...
            */
            ...
            class range {
                ...
                struct small_buffer
                {
                    alignas(alignof(::max_align_t)) unsigned char buf[BufferSize];

                    void const* addr() const noexcept { return buf; }

                    void* addr() noexcept { return buf; }
                };
                ...
                template<class R0, class R1, bool>
                struct impl2;

                template<class R0, class R1>
                friend struct implementation_defined::range_rule_t;

                any_rule& get() noexcept { return *reinterpret_cast<any_rule*>(sb_.addr()); }

                any_rule const& get() const noexcept { return *reinterpret_cast<any_rule const*>(sb_.addr()); }

                template<class R>
                range(core::string_view s, std::size_t n, R const& r);

                template<class R0, class R1>
                range(core::string_view s, std::size_t n, R0 const& first, R1 const& next);
            ...
            public:
                ...
                */
                ...
                /** Assignment
                    ...
                    @return `*this`
                */
                range& operator=(range const&) noexcept;
                ...
                */
                core::string_view string() const noexcept { return s_; }
            };
            ...
            namespace implementation_defined {
                ...
                {
                    using value_type = range<typename R::value_type>;

                    system::result<value_type> parse(char const*& it, char const* end) const;

                    constexpr range_rule_t(R const& next, std::size_t N, std::size_t M) noexcept :
                        next_(next),
                        N_(N),
                        M_(M)
                    {
            ...
            */
            ...
            constexpr implementation_defined::range_rule_t<R>
                range_rule(R const& next, std::size_t N = 0, std::size_t M = std::size_t(-1)) noexcept
            {
                ...
                return implementation_defined::range_rule_t<R> { next, N, M };
            }
            ...
            namespace implementation_defined {
                ...
                struct range_rule_t
                {
                    using value_type = range<typename R0::value_type>;

                    system::result<value_type> parse(char const*& it, char const* end) const;

                    constexpr range_rule_t(R0 const& first, R1 const& next, std::size_t N, std::size_t M) noexcept :
                        first_(first),
                        next_(next),
                        N_(N),
                        M_(M)
            ...
            /** Match a repeating number of elements
                ...
                @see
                    ...
                    @ref squelch.
            */
            template<BOOST_URL_CONSTRAINT(Rule) R1, BOOST_URL_CONSTRAINT(Rule) R2>
            constexpr auto
                range_rule(R1 const& first, R2 const& next, std::size_t N = 0, std::size_t M = std::size_t(-1)) noexcept
                ->
#if 1
                typename std::enable_if<!std::is_integral<R2>::value, implementation_defined::range_rule_t<R1, R2>>::
                    type
#else
                range_rule_t<R1, R2>
#endif
            {
                ...
                // the type requirements. Please check
                // the documentation.
                static_assert(is_rule<R1>::value, "Rule requirements not met");
                static_assert(is_rule<R2>::value, "Rule requirements not met");
                ...
                return implementation_defined::range_rule_t<R1, R2> { first, next, N, M };
            }
