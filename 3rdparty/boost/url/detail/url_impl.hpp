...
namespace boost {
    namespace urls {
        ...
        namespace detail {
            ...
            {
                static constexpr std::size_t const zero_ = 0;
                ...
                host_type host_type_ = urls::host_type::none;
                scheme scheme_ = urls::scheme::none;

                from from_ = from::string;

                url_impl(from b) noexcept :
                    from_(b)
                {
                }
                ...
                authority_view construct_authority() const noexcept;
                ...
                void apply_userinfo(pct_string_view const&, pct_string_view const*) noexcept;
                void apply_host(host_type, pct_string_view, unsigned char const*) noexcept;
            ...
            class path_ref : private parts_base {
            ...
            public:
                ...
                path_ref(core::string_view, std::size_t, std::size_t) noexcept;
                ...
                bool alias_of(url_impl const& impl) const noexcept { return impl_ == &impl; }

                bool alias_of(path_ref const& ref) const noexcept
                {
                    if (impl_) {
                        return impl_ == ref.impl_;
                    }
                    BOOST_ASSERT(data_ != ref.data_ || (size_ == ref.size_ && nseg_ == ref.nseg_ && dn_ == ref.dn_));
            ...
            class BOOST_URL_DECL query_ref : private parts_base {
                url_impl const* impl_ = nullptr;
                char const* data_ = nullptr;
                std::size_t size_ = 0;
                std::size_t nparam_ = 0;
            ...
            public:
                ...
                bool alias_of(query_ref const& ref) const noexcept
                {
                    if (impl_) {
                        return impl_ == ref.impl_;
                    }
                    BOOST_ASSERT(
                        data_ != ref.data_ || (size_ == ref.size_ && nparam_ == ref.nparam_ && dn_ == ref.dn_));
                    return data_ == ref.data_;
                }
            };
