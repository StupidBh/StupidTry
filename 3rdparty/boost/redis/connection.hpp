...
namespace boost::redis {
    namespace detail {
        ...
        inline std::chrono::steady_clock::time_point compute_expiry(std::chrono::steady_clock::duration timeout)
        {
        ...
        struct connection_impl
        {
            ...
            using receive_channel_type = asio::experimental::channel<Executor, void(system::error_code, std::size_t)>;
            using exec_notifier_type = asio::experimental::channel<Executor, void(system::error_code, std::size_t)>;
            ...
            void cancel(operation op)
            {
                ...
                case operation::reconnection : st_.cfg.reconnect_wait_interval = std::chrono::seconds::zero(); break;
                case operation::run          :
            ...
            auto async_exec(request const& req, any_adapter adapter, CompletionToken&& token)
            {
                ...
                info->set_done_callback([notifier]() { notifier->try_send(std::error_code {}, 0); });
            ...
            void set_receive_adapter(any_adapter adapter) { st_.mpx.set_receive_adapter(std::move(adapter)); }
        };
        ...
        struct writer_op
        {
            ...
            void operator()(Self& self, system::error_code ec = {}, std::size_t bytes_written = 0u)
            {
                ...
                auto act = fsm_.resume(conn->st_, ec, bytes_written, self.get_cancellation_state().cancelled());
                ...
                case writer_action_type::write_some:
                    conn->stream_.async_write_some(
                        ...
                        asio::cancel_at(conn->writer_timer_, compute_expiry(act.timeout()), std::move(self)));
                    return;
                ...
                for (;;) {
                    ...
                    case reader_fsm::action::type::read_some:
                        conn->stream_.async_read_some(
                            ...
                            asio::cancel_at(conn->reader_timer_, compute_expiry(act.timeout()), std::move(self)));
                        return;
        ...
        public:
            ...
            void operator()(Self& self, system::error_code ec = {})
            {
                ...
                case run_action_type::immediate: asio::async_immediate(self.get_io_executor(), std::move(self)); return;
                ...
                case run_action_type::parallel_group:
                    asio::experimental::make_parallel_group(
                        [this](auto token) { return this->reader(token); },
                        [this](auto token) { return this->writer(token); })
        ...
        public:
            ...
            void operator()(asio::cancellation_type_t cancel_type) const
            {
                ...
                constexpr auto mask = asio::cancellation_type_t::terminal | asio::cancellation_type_t::partial;
    ...
    public:
        ...
        basic_connection(executor_type ex, logger lgr) :
            basic_connection(std::move(ex), asio::ssl::context { asio::ssl::context::tlsv12_client }, std::move(lgr))
        {
        }
        ...
        asio::ssl::context const& get_ssl_context() const noexcept { return impl_->stream_.get_ssl_context(); }
    ...
    private:
        ...
        struct run_initiation
        {
            ...
            void operator()(Handler&& handler, config const* cfg)
            {
                ...
                auto token_with_slot =
                    asio::bind_cancellation_slot(self->run_signal_.slot(), std::forward<Handler>(handler));
    ...
    public:
        ...
        connection(executor_type ex, logger lgr) :
            connection(std::move(ex), asio::ssl::context { asio::ssl::context::tlsv12_client }, std::move(lgr))
        {
        }
        ...
        connection(asio::io_context& ioc, logger lgr) :
            connection(ioc.get_executor(), asio::ssl::context { asio::ssl::context::tlsv12_client }, std::move(lgr))
        {
        }
        ...
        asio::ssl::stream<asio::ip::tcp::socket>& next_layer() noexcept { return impl_.impl_->stream_.next_layer(); }
        ...
        asio::ssl::context const& get_ssl_context() const noexcept { return impl_.impl_->stream_.get_ssl_context(); }
    ...
    private:
        ...
        void async_run_impl(config const& cfg, asio::any_completion_handler<void(boost::system::error_code)> token);
