...
namespace boost::redis::detail {
    ...
    inline void
        process_setup_node(connection_state& st, resp3::basic_node<std::string_view> const& nd, system::error_code& ec)
    {
        switch (nd.data_type) {
    ...
    {
        return any_adapter { [&st](any_adapter::parse_event evt, resp3::node_view const& nd, system::error_code& ec) {
    ...
    run_action run_fsm::resume(connection_state& st, system::error_code ec, asio::cancellation_type_t cancel_state)
    {
            ...
            for (;;) {
                ...
                if (!ec) {
                    ...
                    if (st.cfg.setup.get_commands() != 0u) {
                        ...
                        elm->set_done_callback([&elem_ref = *elm, &st] { on_setup_done(elem_ref, st); });
                        st.mpx.add(elm);
                    }
