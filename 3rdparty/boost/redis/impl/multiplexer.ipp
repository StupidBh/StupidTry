...
namespace boost::redis::detail {
    ...
    void multiplexer::cancel_on_conn_lost()
    {
        ...
        std::for_each(std::begin(reqs_), std::end(reqs_), [](auto const& ptr) { return ptr->mark_waiting(); });
    }
    ...
    {
        auto point = std::stable_partition(std::begin(reqs_), std::end(reqs_), [](const std::shared_ptr<elem>& ptr) {
            return !(ptr->is_written() && ptr->get_remaining_responses() == 0u);
        });

        std::for_each(point, std::end(reqs_), [](auto const& ptr) { ptr->notify_done(); });
