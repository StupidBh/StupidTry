...
#if BOOST_WORKAROUND(__MWERKS__, <= 0x3004) /* pro9 reintroduced the bug */ \
    || (BOOST_WORKAROUND(__MWERKS__, > 0x3100) && BOOST_WORKAROUND(__MWERKS__, BOOST_TESTED_AT(0x3201)))
...
namespace boost {
    namespace python {
        ...
        namespace detail {
            ...
            struct is_data_member_pointer :
                mpl::and_<detail::is_member_pointer<T>, mpl::not_<detail::is_member_function_pointer<T>>>
            {
            };
    ...
    #define BOOST_PYTHON_NO_DATA_MEMBER
            ...
            namespace error {
                ...
                template<class C>
                struct assertion : mpl::if_<C, assertion_ok<C>, assertion_failed<C>>::type
                {
                };
                ...
                {
                    template<class Default>
                    static void must_be_derived_class_member(Default const&)
                    {
...
#endif
                        typedef typename assertion<detail::is_member_function_pointer<Fn>>::failed test2
        ...
        private: // types
            ...
            struct id_vector
            {
                ...
                BOOST_STATIC_CONSTANT(std::size_t, size = mpl::size<bases>::value + 1);
                type_info ids[size];
            };
        ...
        public: // member functions
            ...
            {
                this->def_impl(detail::unwrap_wrapper((W*)0), name, f, detail::def_helper<char const*>(0), &f);
                return *this;
            }
            ...
            self& def(char const* name, Fn fn, A1 const& a1, A2 const& a2)
            {
                ...
                this->def_impl(detail::unwrap_wrapper((W*)0), name, fn, detail::def_helper<A1, A2>(a1, a2), &fn);
            ...
            {
                base::add_property(name, this->make_getter(fget), this->make_setter(fset), docstr);
                return *this;
            }
        ...
        private: // helper functions
            ...
            object make_getter(F f)
            {
                ...
                return objects::add_doc(
                    this->make_fn_impl(
                        ...
                        is_obj_or_proxy(),
                        (char*)0,
                        detail::is_data_member_pointer<F>()),
                    NULL);
            }
            ...
            self& def_readonly_impl(char const* name, D B::* pm_, char const* doc BOOST_PYTHON_YES_DATA_MEMBER)
            {
                return this->add_property(name, pm_, doc);
            }

            template<class D, class B>
            self& def_readwrite_impl(char const* name, D B::* pm_, char const* doc BOOST_PYTHON_YES_DATA_MEMBER)
            {
                return this->add_property(name, pm_, pm_, doc);
            }

            template<class D>
            self& def_readonly_impl(char const* name, D& d, char const* BOOST_PYTHON_NO_DATA_MEMBER)
            {
                return this->add_static_property(name, python::make_getter(d));
            }

            template<class D>
            self& def_readwrite_impl(char const* name, D& d, char const* BOOST_PYTHON_NO_DATA_MEMBER)
            {
            ...
            inline void def_impl(T*, char const* name, LeafVisitor, Helper const& helper, def_visitor<Visitor> const* v)
            {
            ...
            inline void def_impl(T*, char const* name, Fn fn, Helper const& helper, ...)
            {
                objects::add_to_namespace(
                    *this,
                    name,
                    make_function(fn, helper.policies(), helper.keywords(), detail::get_signature(fn, (T*)0)),
                    helper.doc());
...

            {
                ...
                detail::define_with_defaults(name, overloads, *this, detail::get_signature(sig));
            }

            template<class Fn, class A1>
            void def_maybe_overloads(char const* name, Fn fn, A1 const& a1, ...)
            {
                this->def_impl(detail::unwrap_wrapper((W*)0), name, fn, detail::def_helper<A1>(a1), &fn);
            }
