    ...
    namespace python {
        namespace numpy {
            ...
            public:
                ...
                /// @brief Return the stride of the nth dimension. raises IndexError if k not in [-get_nd() :
                /// get_nd()-1]
            ...
            namespace detail {
                ...
                BOOST_NUMPY_DECL ndarray from_data_impl(
                    void* data,
                    dtype const& dt,
                ...
                ndarray from_data_impl(
                    ...
                    object const& owner,
                    bool writeable,
                    typename boost::enable_if<boost::python::detail::is_integral<typename Container::value_type>>::type*
                        enabled = NULL)
                {
            ...
            template<typename Container>
            inline ndarray
                from_data(void* data, dtype const& dt, Container shape, Container strides, python::object const& owner)
            {
             ...
             */
            BOOST_NUMPY_DECL ndarray from_object(
                object const& obj,
                dtype const& dt,
                int nd_min,
                int nd_max,
            ...
            BOOST_NUMPY_DECL inline ndarray
                from_object(object const& obj, dtype const& dt, int nd, ndarray::bitflag flags = ndarray::NONE)
            {
                return from_object(obj, dt, nd, nd, flags);
            }

            BOOST_NUMPY_DECL inline ndarray
                from_object(object const& obj, dtype const& dt, ndarray::bitflag flags = ndarray::NONE)
            {
                return from_object(obj, dt, 0, 0, flags);
            }

            BOOST_NUMPY_DECL ndarray
                from_object(object const& obj, int nd_min, int nd_max, ndarray::bitflag flags = ndarray::NONE);

            BOOST_NUMPY_DECL inline ndarray
                from_object(object const& obj, int nd, ndarray::bitflag flags = ndarray::NONE)
            {
