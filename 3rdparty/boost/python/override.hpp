...
namespace boost {
    namespace python {
        ...
        #if BOOST_WORKAROUND(_MSC_FULL_VER, BOOST_TESTED_AT(140'050'215))
                template<class T>
                operator T*()
                {
                    converter::return_from_python<T*> converter;
        ...
        public:
            detail::method_result operator()() const
            {
                detail::method_result x(PyObject_CallFunction(this->ptr(), const_cast<char*>("()")));
                return x;
            }
...
detail::method_result operator()(BOOST_PP_ENUM_BINARY_PARAMS_Z(1, N, A, const& a)) const
{
    detail::method_result x(PyObject_CallFunction(
        this->ptr(),
