        ...
        namespace detail {
            ...
            struct tuple_extract_impl
            {
                ...
                struct apply
                {
                    ...
                    static typename Tuple::head_type extract(Tuple const& x) { return x.get_head(); }
                };
            };
            ...
            struct tuple_extract_impl<false>
            {
                ...
                struct apply
                {
                    ...
                    static result_type extract(Tuple const& x) { return next::extract(x.get_tail()); }
                };
            };
            ...
            struct tuple_extract_base_select
            {
                ...
                typedef typename mpl::apply1<Predicate, typename add_lvalue_reference<head_type>::type>::type match_t;
            ...
            template<class Tuple, class Predicate>
            struct tuple_extract : tuple_extract_base_select<Tuple, typename mpl::lambda<Predicate>::type>::type
            {
            };
            ...
            template<class Tuple>
            struct keyword_extract : tuple_extract<Tuple, is_reference_to_keywords<mpl::_1>>
            {
            };
            ...
            struct default_implementation_extract :
                tuple_extract<Tuple, indirect_traits::is_reference_to_member_function_pointer<mpl::_1>>
            {
            };
            ...
            struct def_helper
            {
                ...
                // shown above.
                def_helper(T1 const& a1) :
                    m_all(a1, m_nil, m_nil, m_nil)
                {
                }

                def_helper(T1 const& a1, T2 const& a2) :
                    m_all(a1, a2, m_nil, m_nil)
                {
                }

                def_helper(T1 const& a1, T2 const& a2, T3 const& a3) :
                    m_all(a1, a2, a3, m_nil)
                {
                }
            ...
            public: // Extractor functions which pull the appropriate value out
                    // of the tuple
                char const* doc() const { return doc_extract<all_t>::extract(m_all); }
