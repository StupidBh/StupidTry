...
#if defined(__linux__) && defined(__GNUC__) && BOOST_WORKAROUND(__GNUC__, == 3) && \
    BOOST_WORKAROUND(__GNUC_MINOR__, == 3) &&                                      \
    (BOOST_WORKAROUND(__GNUC_PATCHLEVEL__, == 1) || BOOST_WORKAROUND(__GNUC_PATCHLEVEL__, == 3))
                typedef typename remove_reference<typename add_const<ExceptionType>::type>::type exception_non_ref;
#else
                typedef typename add_lvalue_reference<typename add_const<ExceptionType>::type>::type exception_cref;
#endif
