...
namespace boost {
    namespace python {
        namespace api {
            ...
            struct enable_binary : boost::enable_if_<is_object_operators<L, R>::value, T>
            {
            };
    ...
    #define BOOST_PYTHON_BINARY_RETURN(T) T
            ...
            template<class U>
            inline bool object_operators<U>::operator!() const
            {
                object_cref2 x = *static_cast<U const*>(this);
                int is_true = PyObject_IsTrue(x.ptr());
                if (is_true < 0) {
...
#define BOOST_PYTHON_COMPARE_OP(op, opid)                                    \
    ...
    BOOST_PYTHON_BINARY_RETURN(object)                                       \
    operator op(L const& l, R const& r)                                      \
    {                                                                        \
        return PyObject_RichCompare(object(l).ptr(), object(r).ptr(), opid); \
    }
