    ...
    namespace python {
        ...
        namespace detail {
            namespace error {
                ...
                template<bool>
                struct multiple_functions_passed_to_def;

                template<>
                struct multiple_functions_passed_to_def<false>
                {
                    typedef char type;
                };
            }
            ...
            void def_from_helper(char const* name, F const& fn, Helper const& helper)
            {
                ...
                typedef
                    typename error::multiple_functions_passed_to_def<Helper::has_default_implementation>::type assertion
                        BOOST_ATTRIBUTE_UNUSED;

                detail::scope_setattr_doc(
                    name,
                    boost::python::make_function(fn, helper.policies(), helper.keywords()),
                    helper.doc());
            }
            ...
            void def_maybe_overloads(char const* name, SigT sig, StubsT const& stubs, detail::overloads_base const*)
            {
                ...
                detail::define_with_defaults(name, stubs, current, detail::get_signature(sig));
            }
            ...
            inline object make_function1(object const& x, object const*)
            {
                return x;
            }
        }
