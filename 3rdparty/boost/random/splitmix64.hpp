...
namespace boost {
    namespace random {
        ...
        public:
            ...
            /** Seeds the generator with a user provided seed. */
            explicit splitmix64(std::uint64_t state = 0) noexcept { seed(state); }
            ...
            inline void discard(std::uint64_t z) noexcept { state_ += z * UINT64_C(0x9E3779B97F4A7C15); }
             ...
             * can produce.
             */
            static constexpr result_type(min)() noexcept { return (std::numeric_limits<std::uint64_t>::min)(); }
        };
