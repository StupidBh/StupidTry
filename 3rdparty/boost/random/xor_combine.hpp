...
namespace boost {
    namespace random {
        ...
        public:
             ...
             * @endxmlwarning
             */
            BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(xor_combine_engine, result_type, v) { seed(v); }

            /**
             * Constructs a @c xor_combine_engine, seeding both base generators
             * with values produced by @c seq.
             */
            BOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(xor_combine_engine, SeedSeq, seq) { seed(seq); }
            ...
            xor_combine_engine(It& first, It last) :
                _rng1(first, last),
                _rng2(/* advanced by other call */ first, last)
            {
            }
            ...
            result_type operator()() { return (_rng1() << s1) ^ (_rng2() << s2); }
        ...
        template<class URNG1, int s1, class URNG2, int s2, typename URNG1::result_type v = 0>
        ...
        public:
            ...
            xor_combine(result_type val) :
                base_type(val)
            {
            }

            template<class It>
            xor_combine(It& first, It last) :
            ...
            {
                return (std::min)((this->base1().min)(), (this->base2().min)());
            }

            result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() const
            {
                return (std::max)((this->base1().min)(), (this->base2().max)());
            }
        };
