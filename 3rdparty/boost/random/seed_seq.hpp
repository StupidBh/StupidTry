        ...
        public:
            ...
            void generate(Iter first, Iter last) const
            {
                ...
                for (std::size_t k = 0; k < m; ++k) {
                    value_type r1 =
                        static_cast<value_type>(*(first + k % n) ^ *(first + (k + p) % n) ^ *(first + (k + n - 1) % n));
                    r1 = r1 ^ (r1 >> 27);
                    r1 = (r1 * 1'664'525u) & mask;
                    value_type r2 = static_cast<value_type>(
                        r1 + ((k == 0)   ? s
                              : (k <= s) ? k % n + v[k - 1]
                                         : (k % n)));
                ...
                for (std::size_t k = m; k < m + n; ++k) {
                    value_type r3 = static_cast<value_type>(
                        (*(first + k % n) + *(first + (k + p) % n) + *(first + (k + n - 1) % n)) & mask);
                    r3 = r3 ^ (r3 >> 27);
                    r3 = (r3 * 1'566'083'941u) & mask;
