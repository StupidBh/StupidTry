...
namespace boost {
    namespace random {
        ...
        public:
             ...
             */
            BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(linear_congruential_engine, IntType, x0) { seed(x0); }

            /**
             * Constructs a @c linear_congruential_engine, seeding it with values
             * produced by a call to @c seq.generate().
             */
            BOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(linear_congruential_engine, SeedSeq, seq) { seed(seq); }
             ...
             * can produce.
             */
            static BOOST_CONSTEXPR result_type min BOOST_PREVENT_MACRO_SUBSTITUTION() { return c == 0 ? 1 : 0; }

            /**
             * Returns the largest value that the @c linear_congruential_engine
             * can produce.
             */
            static BOOST_CONSTEXPR result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() { return modulus - 1; }
            ...
            void discard(boost::uintmax_t z)
            {
                ...
                if (b_gcd == 1) {
                    ...
                    _x = mod_type::mult_add(a_z, _x, mod_type::mult(mod_type::mult(c, b_inv), a_z - 1));
                }
                else {
        ...
        public:
            linear_congruential(IntType x0 = 1) :
                base_type(x0)
            {
            }
         ...
         *  @endblockquote
         */
        typedef linear_congruential_engine<uint32_t, 16807, 0, 2'147'483'647> minstd_rand0;
         ...
         *  @endblockquote
         */
        typedef linear_congruential_engine<uint32_t, 48271, 0, 2'147'483'647> minstd_rand;
        ...
        public:
             ...
             */
            static BOOST_CONSTEXPR uint32_t max BOOST_PREVENT_MACRO_SUBSTITUTION() { return 0x7FFFFFFF; }

            /** Seeds the generator with the default seed. */
            rand48() :
                lcf(cnv(static_cast<uint32_t>(1)))
            {
            }
             ...
             */
            BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(rand48, result_type, x0) { seed(x0); }

            /**
             * Seeds the generator with values produced by @c seq.generate().
             */
            BOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(rand48, SeedSeq, seq) { seed(seq); }
             ...
             */
            BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(rand48, result_type, x0) { lcf.seed(cnv(x0)); }
             ...
             * Seeds the generator with values produced by @c seq.generate().
             */
            BOOST_RANDOM_DETAIL_SEED_SEQ_SEED(rand48, SeedSeq, seq) { lcf.seed(seq); }
            ...
            template<class CharT, class Traits>
            friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& os, const rand48& r)
            {
                os << r.lcf;
                return os;
            }
            ...
            template<class CharT, class Traits>
            friend std::basic_istream<CharT, Traits>& operator>>(std::basic_istream<CharT, Traits>& is, rand48& r)
            {
                is >> r.lcf;
                return is;
            }
             ...
             * sequences of values.
             */
            friend bool operator==(const rand48& x, const rand48& y) { return x.lcf == y.lcf; }

            /**
             * Returns true if the two generators will produce different
             * sequences of values.
             */
            friend bool operator!=(const rand48& x, const rand48& y) { return !(x == y); }

        private:
            /// \cond show_private
            typedef random::linear_congruential_engine<
                uint64_t,
            ...
            static boost::uint64_t cnv(boost::uint32_t x) { return (static_cast<uint64_t>(x) << 16) | 0x330e; }
