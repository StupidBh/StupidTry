...
namespace boost {
    namespace random {
        ...
        public:
             ...
             * a >= 0 && b > 0 if p < 0
             */
            explicit generalized_inverse_gaussian_distribution(
                RealType p_arg = RealType(1.0),
                RealType a_arg = RealType(1.0),
                RealType b_arg = RealType(1.0)) :
                _p(p_arg),
            ...
            RealType operator()(URNG& urng) const
            {
                ...
                if (log_concave >= result_type(.5) && log_concave <= result_type(2)) {
                    ...
                    s =
                        min(result_type(1) / _abs_p,
                            log(result_type(1) + result_type(1) / _alpha +
                                sqrt(result_type(1) / (_alpha * _alpha) + result_type(2) / _alpha)));
                }
                RealType eta = -psi(t);
        ...
        private:
            ...
            static result_type
                chi(const RealType& x,
                    const RealType& s,
                    const RealType& t,
                    const RealType& s_deriv,
                    const RealType& t_deriv,
                    const RealType& eta,
                    const RealType& zeta,
