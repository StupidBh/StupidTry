...
namespace boost {
    namespace random {
        ...
        public:
             ...
             */
            static BOOST_CONSTEXPR result_type min BOOST_PREVENT_MACRO_SUBSTITUTION() { return 1; }

            /**
             * Returns the largest value that the generator can produce
             */
            static BOOST_CONSTEXPR result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() { return MLCG1::modulus - 1; }
        ...
        public:
            ...
            template<class T>
            additive_combine(T& arg) :
                base_t(arg)
            {
            }

            template<class T>
            additive_combine(const T& arg) :
                base_t(arg)
            {
            }

            template<class It>
            additive_combine(It& first, It last) :
                base_t(first, last)
            {
            }
        };
        ...
        typedef additive_combine_engine<
            linear_congruential_engine<uint32_t, 40014, 0, 2'147'483'563>,
            linear_congruential_engine<uint32_t, 40692, 0, 2'147'483'399>>
            ecuyer1988;
