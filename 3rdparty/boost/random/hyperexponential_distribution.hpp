    ...
    namespace random {
        ...
        namespace hyperexp_detail {
            ...
            std::vector<T>& normalize(std::vector<T>& v)
            {
                ...
                for (typename std::vector<T>::iterator it = v.begin(); it != end; ++it) {
                    *it /= sum;
                    final_sum += *it;
                }
            ...
            bool check_probabilities(std::vector<RealT> const& probabilities)
            {
                ...
                for (std::size_t i = 0; i < n; ++i) {
                    if (probabilities[i] < 0 || probabilities[i] > 1 || !(boost::core::isfinite)(probabilities[i])) {
                        return false;
                    }
                    sum += probabilities[i];
                }
            ...
            bool check_rates(std::vector<RealT> const& rates)
            {
                ...
                for (std::size_t i = 0; i < n; ++i) {
                    if (rates[i] <= 0 || !(boost::core::isfinite)(rates[i])) {
                        return false;
                    }
                }
                return true;
            ...
            bool check_params(std::vector<RealT> const& probabilities, std::vector<RealT> const& rates)
            {
                ...
                return check_probabilities(probabilities) && check_rates(rates);
            }
         ...
         * parameters, namely a <em>phase probability vector</em> \f$\mathbf{\alpha}=(\alpha_1,\ldots,\alpha_k)\f$ and a
         * <em>rate vector</em> \f$\mathbf{\lambda}=(\lambda_1,\ldots,\lambda_k)\f$.
         *
         ...
         *
         * References:
         * -# A.O. Allen, <em>Probability, Statistics, and Queuing Theory with Computer Science Applications, Second
         * Edition</em>, Academic Press, 1990.
         * -# Boost C++ Libraries, <em>Boost.Math / Statistical Distributions: Hyperexponential Distribution</em>,
         * Online: http://www.boost.org/doc/libs/release/libs/math/doc/html/dist.html , 2014.
         * -# D.G. Feitelson, <em>Workload Modeling for Computer Systems Performance Evaluation</em>, Cambridge
         * University Press, 2014
         * -# A. Feldmann and W. Whitt, <em>Fitting mixtures of exponentials to long-tail distributions to analyze
         * network performance models</em>, Performance Evaluation 31(3-4):245, doi:10.1016/S0166-5316(97)00003-5, 1998.
         * -# H.T. Papadopolous, C. Heavey and J. Browne, <em>Queueing Theory in Manufacturing Systems Analysis and
         * Design</em>, Chapman & Hall/CRC, 1993, p. 35.
         * -# R.F. Rosin, <em>Determining a computing center environment</em>, Communications of the ACM 8(7):463-468,
         * 1965.
         * -# K.S. Trivedi, <em>Probability and Statistics with Reliability, Queueing, and Computer Science
         * Applications</em>, John Wiley & Sons, Inc., 2002.
         * -# J. Tyszer, <em>Object-Oriented Computer Simulation of Discrete-Event Systems</em>, Springer, 1999.
         * -# Wikipedia, <em>Hyperexponential Distribution</em>, Online:
         * http://en.wikipedia.org/wiki/Hyperexponential_distribution , 2014.
         * -# Wolfram Mathematica, <em>Hyperexponential Distribution</em>, Online:
         * http://reference.wolfram.com/language/ref/HyperexponentialDistribution.html , 2014.
         * .
         *
         * \author Marco Guazzone (marco.guazzone@gmail.com)
         */
        ...
        public:
            ...
            public:
                 ...
                 *
                 * \tparam ProbIterT Must meet the requirements of \c InputIterator concept (ISO,2014,sec. 24.2.3
                 * [input.iterators]).
                 * \tparam RateIterT Must meet the requirements of \c InputIterator concept (ISO,2014,sec. 24.2.3
                 * [input.iterators]).
                 *
                 * \param prob_first The iterator to the beginning of the range of non-negative real elements
                 * representing the phase probabilities; if elements don't sum to 1, they are normalized.
                 * \param prob_last The iterator to the ending of the range of non-negative real elements representing
                 * the phase probabilities; if elements don't sum to 1, they are normalized.
                 * \param rate_first The iterator to the beginning of the range of non-negative real elements
                 * representing the rates.
                 * \param rate_last The iterator to the ending of the range of non-negative real elements representing
                 * the rates.
                 *
                 * References:
            ...
            public:
                ...
                param_type(ProbIterT prob_first, ProbIterT prob_last, RateIterT rate_first, RateIterT rate_last) :
                 ...
                 *
                 * \tparam ProbRangeT Must meet the requirements of <a
                 * href="boost:/libs/range/doc/html/range/concepts.html">Range</a> concept.
                 * \tparam RateRangeT Must meet the requirements of <a
                 * href="boost:/libs/range/doc/html/range/concepts.html">Range</a> concept.
                 *
                 * \param prob_range The range of non-negative real elements representing the phase probabilities; if
            ...
            public:
                ...
                param_type(
                    ProbRangeT const& prob_range,
                    RateRangeT const& rate_range,
                    typename boost::disable_if_c<
                        boost::has_pre_increment<ProbRangeT>::value ||
                        boost::has_pre_increment<RateRangeT>::value>::type* = 0) :
                 ...
                 * \tparam RateIterT Must meet the requirements of \c InputIterator concept (ISO,2014,sec. 24.2.3
                 * [input.iterators]).
                 * \tparam RateIterT2 Must meet the requirements of \c InputIterator concept (ISO,2014,sec. 24.2.3
                 * [input.iterators]).
                 *
                 * \param rate_first The iterator to the beginning of the range of non-negative real elements
                 * representing the rates.
            ...
            public:
                 ...
                 */
            public:
                std::vector<RealT> probabilities() const { return probs_; }
                 ...
                 */
            public:
                std::vector<RealT> rates() const { return rates_; }
            ...
            public:
                BOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, param_type, param)
                {
                    ...
                    if (param.probs_.size() != param.rates_.size() || param.probs_.size() == 0) {
            ...
            public:
                ...
                {
                    return lhs.probs_ == rhs.probs_ && lhs.rates_ == rhs.rates_;
                }
            ...
            private:
                std::vector<RealT> probs_; ///< The <em>phase probability vector</em> parameter of the distribution
            private:
                std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
            }; // param_type
        ...
        public:
             ...
             * [input.iterators]).
             *
             * \param prob_first The iterator to the beginning of the range of non-negative real elements representing
             * the phase probabilities; if elements don't sum to 1, they are normalized.
             * \param prob_last The iterator to the ending of the range of non-negative real elements representing the
             * phase probabilities; if elements don't sum to 1, they are normalized.
             * \param rate_first The iterator to the beginning of the range of non-negative real elements representing
             * the rates.
             * \param rate_last The iterator to the ending of the range of non-negative real elements representing the
             * rates.
             *
             * References:
             * -# ISO, <em>ISO/IEC 14882-2014: Information technology - Programming languages - C++</em>, 2014
        ...
        public:
            ...
            hyperexponential_distribution(
                ...
                RateRangeT const& rate_range,
                typename boost::disable_if_c<
                    boost::has_pre_increment<ProbRangeT>::value || boost::has_pre_increment<RateRangeT>::value>::type* =
                    0) :
                dd_(prob_range),
                rates_(std::begin(rate_range), std::end(rate_range))
            {
        ...
        public:
            hyperexponential_distribution(
                std::initializer_list<RealT> const& l1,
        ...
        public:
            std::size_t num_phases() const { return rates_.size(); }

            /** Returns the <em>phase probability vector</em> parameter of the distribution. */
        public:
            std::vector<RealT> probabilities() const { return dd_.probabilities(); }
        ...
        public:
            RealT min BOOST_PREVENT_MACRO_SUBSTITUTION() const { return 0; }

            /** Returns the largest value that the distribution can produce. */
        public:
            RealT max BOOST_PREVENT_MACRO_SUBSTITUTION() const { return std::numeric_limits<RealT>::infinity(); }
        ...
        public:
            BOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, hyperexponential_distribution, hd)
            {
                os << hd.param();
                return os;
            }
        ...
        public:
            BOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, hyperexponential_distribution, hd)
            {
                param_type param;
                if (is >> param) {
                    hd.param(param);
             ...
             */
        public:
            BOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(hyperexponential_distribution, lhs, rhs)
            {
                return lhs.dd_ == rhs.dd_ && lhs.rates_ == rhs.rates_;
            }
             ...
             */
        public:
            BOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(hyperexponential_distribution)

        private:
            boost::random::discrete_distribution<int, RealT>
                dd_; ///< The \c discrete_distribution used to sample the phase probability and choose the rate
        private:
            std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
        }; // hyperexponential_distribution
