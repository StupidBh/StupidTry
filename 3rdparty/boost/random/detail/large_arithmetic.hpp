...
namespace boost {
    namespace random {
        namespace detail {
            ...
            {
                const int bits = ::std::numeric_limits<::boost::uintmax_t>::digits / 2;
                ...
                for (int i = 0; i < 2; ++i) {
                    digit_t carry = 0;
                    for (int j = 0; j < 2; ++j) {
                        ::boost::uint64_t temp = ::boost::uintmax_t(a_[i]) * b_[j] + carry + product[i + j];
                ...
                // divide product / m
                for (int i = 3; i >= 2; --i) {
                    ::boost::uintmax_t temp = ::boost::uintmax_t(product[i]) << bits | product[i - 1];

                    digit_t q = digit_t((product[i] == m_[1]) ? mask : temp / m_[1]);

                    ::boost::uintmax_t rem = ((temp - ::boost::uintmax_t(q) * m_[1]) << bits) + product[i - 2];
