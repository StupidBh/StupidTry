...
namespace boost {
    namespace random {
        namespace detail {
            ...
            struct seed_type
            {
                typedef typename boost::conditional<boost::is_integral<T>::value, T, boost::uint32_t>::type type;
            };
            ...
            struct const_pow_impl
            {
                ...
                {
                    return const_pow_impl<N / 2>::call(T(arg * arg), n / 2, n % 2 == 0 ? result : T(result * arg));
                }
            };
            ...
            inline T const_pow(T arg, int n)
            {
                return const_pow_impl<N>::call(arg, n, T(1));
            }
                ...
                while (true) {
                    ...
                    if (Bits < 32 && available_bits < 32 - remaining_bits) {
                        ...
                        if (Bits < 32 || remaining_bits != 0) {
                            boost::uint_least32_t divisor = (boost::uint_least32_t(1) << (32 - remaining_bits));
            ...
            void generate_from_int(Engine& eng, Iter begin, Iter end)
            {
                ...
                int bits = (range == (std::numeric_limits<unsigned_type>::max)())
                ...
                while (true) {
                    ...
                    if (available_bits == 32) {
                        *begin++ = static_cast<boost::uint_least32_t>(val) & 0xFFFFFFFFu;
                        ...
                        if (bits < 32 || remaining_bits != 0) {
                            boost::uint_least32_t extra_bits =
                                boost::uint_least32_t(val) & ((boost::uint_least32_t(1) << (32 - remaining_bits)) - 1);
            ...
            {
                return detail::generate_impl(
                    eng,
                    first,
                    last,
                    boost::random::traits::is_integral<typename Engine::result_type>());
            }

            template<class IntType, IntType m, class SeedSeq>
            IntType seed_one_int(SeedSeq& seq)
            {
                static const int log = ::boost::conditional<
                    (m == 0),
                    ::boost::integral_constant<int, (::std::numeric_limits<IntType>::digits)>,
                    ::boost::static_log2<m>>::type::value;
                static const int k = (log + ((~(static_cast<IntType>(2) << (log - 1)) & m) ? 32 : 31)) / 32;
            ...
            inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], boost::true_type)
            {
                BOOST_STATIC_ASSERT_MSG(
                    ...
                    "Sorry but this routine has not been ported to non built-in integers as it relies on a "
                    "reinterpret_cast.");
