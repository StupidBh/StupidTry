    ...
    namespace random {
        namespace detail {
            ...
            template<
                typename Derived,
                std::size_t N,
                typename OutputType = std::uint64_t,
                typename BlockType = std::uint64_t>
            class xoshiro_base {
            protected:
            ...
            private:
                ...
                {
                    constexpr std::array<std::uint64_t, 4U> jump = { { UINT64_C(0x180ec6d33cfd0aba),
                ...
                {
                    constexpr std::array<std::uint64_t, 8U> jump = { { UINT64_C(0x33ed89b6e7a353f9),
                ...
                {
                    constexpr std::array<std::uint64_t, 4> long_jump = { { UINT64_C(0x76e15d3efefdcbbf),
                ...
                {
                    constexpr std::array<std::uint64_t, 8U> long_jump = { { UINT64_C(0x11467fef8f921d28),
                ...
                {
                    constexpr std::array<std::uint32_t, 4> jump = {
                        { UINT32_C(0x8764000b), UINT32_C(0xf542d2d3), UINT32_C(0x6fa035c3), UINT32_C(0x77f2db5b) }
                    };
                ...
                {
                    constexpr std::array<std::uint32_t, 4> jump = {
                        { UINT32_C(0xb523952e), UINT32_C(0x0b6f099f), UINT32_C(0xccf5a0ef), UINT32_C(0x1c580662) }
                    };
            ...
            public:
                ...
                template<
                    typename Sseq,
                    typename std::enable_if<!std::is_convertible<Sseq, seed_type>::value, bool>::type = true>
                void seed(Sseq& seq)
                {
                ...
                {
                    static_assert(
                        std::is_integral<typename std::iterator_traits<FIter>::value_type>::value,
                ...
                /** Seeds the generator with a user provided seed. */
                explicit xoshiro_base(const seed_type value) { seed(value); }
                ...
                template<
                    typename Sseq,
                    typename std::enable_if<!std::is_convertible<Sseq, xoshiro_base>::value, bool>::type = true>
                ...
                xoshiro_base& operator=(const xoshiro_base& other) noexcept
                {
                    state_ = other.state();
                    return *this;
                }
                ...
                xoshiro_base& operator=(xoshiro_base&& other) noexcept
                {
                    state_ = other.state();
                    return *this;
                }

                inline result_type next() noexcept { return static_cast<Derived*>(this)->next(); }

                /** This is the jump function for the generator. It is equivalent
                 *  to 2^128 calls to next(); it can be used to generate 2^128
                 *  non-overlapping subsequences for parallel computations. */
                inline void jump() noexcept { jump_impl(xoshiro_type()); }

                /** This is the long-jump function for the generator. It is equivalent to
                 *  2^192 calls to next(); it can be used to generate 2^64 starting points,
                 *  from each of which jump() will generate 2^64 non-overlapping
                 *  subsequences for parallel distributed computations. */
                inline void long_jump() noexcept { long_jump_impl(xoshiro_type()); }
                ...
                inline friend std::basic_ostream<CharT, Traits>&
                    operator<<(std::basic_ostream<CharT, Traits>& ost, const xoshiro_base& e)
                {
                ...
                template<typename CharT, typename Traits>
                inline friend std::basic_istream<CharT, Traits>&
                    operator>>(std::basic_istream<CharT, Traits>& ist, xoshiro_base& e)
                {
                    for (std::size_t i {}; i < e.state_.size(); ++i) {
                ...
                inline std::array<BlockType, N> state() const noexcept { return state_; }
            };
