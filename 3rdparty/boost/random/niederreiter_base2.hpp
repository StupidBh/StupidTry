    ...
    namespace random {
        ...
        namespace qrng_detail {
            namespace nb2 {
                ...
                inline typename DynamicBitset::size_type bitset_log2(const DynamicBitset& v)
                {
                    if (v.none()) {
                ...
                template<typename DynamicBitset>
                inline void calculate_v(
                    const DynamicBitset& pb,
            ...
            public:
                explicit niederreiter_base2_lattice(std::size_t dimension) { resize(dimension); }
                    ...
                    for (std::size_t dim = 0; dim != dimension; ++dim) {
                        ...
                        if (poly > (std::numeric_limits<value_type>::max)()) {
                            boost::throw_exception(
                                std::range_error(
                                    "niederreiter_base2: polynomial value outside the given value type range"));
                        }
                        ...
                        // powers of PX.
                        pb.resize(space_required);
                        tmp.resize(space_required);
        ...
        class niederreiter_base2_engine :
            public qrng_detail::gray_coded_qrng<qrng_detail::niederreiter_base2_lattice<UIntType, w, Nb2Table>> {
         ...
         *
         * However, it is possible to provide your own table to \niederreiter_base2_engine should the default one be
         * insufficient.
         */
        typedef niederreiter_base2_engine<boost::uint_least64_t, 64u, default_niederreiter_base2_table>
            niederreiter_base2;
