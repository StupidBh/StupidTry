...
namespace boost {
    namespace random {
        ...
        public:
            ...
            public:
                    ...
                    _intervals(il.begin(), il.end())
                {
                    if (_intervals.size() < 2) {
                        ...
                        _weights.reserve(_intervals.size());
                        for (typename std::vector<RealType>::const_iterator iter = _intervals.begin(),
                                                                            end = _intervals.end();
                             iter != end;
                             ++iter) {
                ...
                std::vector<RealType> densities() const
                {
                    ...
                    for (typename std::vector<RealType>::const_iterator iter = _weights.begin(), end = _weights.end();
                         iter != end;
                         ++iter) {
             ...
             */
            piecewise_linear_distribution() { default_init(); }
            ...
            result_type min BOOST_PREVENT_MACRO_SUBSTITUTION() const { return _intervals.front(); }

            /** Returns the largest value that the distribution can produce. */
            result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() const { return _intervals.back(); }
            ...
            param_type param() const { return param_type(_intervals, _weights); }
        ...
        private:
            ...
            void init(const std::vector<RealType>& intervals_arg, const std::vector<RealType>& weights_arg)
            {
                using std::abs;
            ...
            void init() { init(_intervals, _weights); }
