...
namespace boost {
    namespace random {
        ...
        namespace detail {
            ...
            template<class RealType>
            RealType poisson_table<RealType>::value[10] = { 0.0,
                                                            0.0,
                                                            0.69314718055994529,
                                                            1.7917594692280550,
                                                            3.1780538303479458,
        ...
        public:
            ...
            public:
                ...
                friend bool operator==(const param_type& lhs, const param_type& rhs) { return lhs._mean == rhs._mean; }
            ...
            IntType max BOOST_PREVENT_MACRO_SUBSTITUTION() const { return (std::numeric_limits<IntType>::max)(); }
        ...
        private:
            ...
            bool use_inversion() const { return _mean < 10; }
                ...
                while (true) {
                    ...
                    if (v <= 0.86 * _u._ptrd.v_r) {
                        ...
                        return static_cast<IntType>(
                            floor((2 * _u._ptrd.a / (0.5 - abs(u)) + _u._ptrd.b) * u + _mean + 0.445));
                    }
                    ...
                    if (k >= 10) {
                        if (log(v * _u._ptrd.smu) <= (k + 0.5) * log(_mean / k) - _mean - log_sqrt_2pi + k -
                                                         (1 / 12. - (1 / 360. - 1 / (1260. * k * k)) / (k * k)) / k) {
                            return static_cast<IntType>(k);
                        }
                    }
                    else if (k >= 0) {
                        if (log(v) <= k * log(_mean) - _mean - log_factorial(static_cast<IntType>(k))) {
                            return static_cast<IntType>(k);
                        }
                    }
                }
