...
namespace boost {
    namespace random {
        ...
        namespace qrng_detail {
            ...
            public:
                explicit sobol_lattice(std::size_t dimension) { resize(dimension); }
                    ...
                    for (std::size_t dim = 1; dim < dimension; ++dim) {
                        ...
                        if (poly > (std::numeric_limits<value_type>::max)()) {
                            boost::throw_exception(
                                std::range_error("sobol: polynomial value outside the given value type range"));
                        }
        ...
        //! any more values. The length of the low discrepancy sequence is given by \f$L=Dimension \times (2^{w} -
        //! 1)\f$.
        template<typename UIntType, unsigned w, typename SobolTables = default_sobol_table>
        class sobol_engine : public qrng_detail::gray_coded_qrng<qrng_detail::sobol_lattice<UIntType, w, SobolTables>> {
        ...
        public:
            ...
            static BOOST_CONSTEXPR result_type min BOOST_PREVENT_MACRO_SUBSTITUTION() { return (base_t::min)(); }

            /** @copydoc boost::random::niederreiter_base2_engine::max() */
            static BOOST_CONSTEXPR result_type max BOOST_PREVENT_MACRO_SUBSTITUTION() { return (base_t::max)(); }
            ...
            void seed() { base_t::seed(); }

            /** @copydoc boost::random::niederreiter_base2_engine::seed(UIntType) */
            void seed(UIntType init) { base_t::seed(init); }
            ...
            void discard(boost::uintmax_t z) { base_t::discard(z); }

            /** Returns true if the two generators will produce identical sequences of outputs. */
            BOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(sobol_engine, x, y) { return static_cast<const base_t&>(x) == y; }

            /** Returns true if the two generators will produce different sequences of outputs. */
            BOOST_RANDOM_DETAIL_INEQUALITY_OPERATOR(sobol_engine)

            /** Writes the textual representation of the generator to a @c std::ostream. */
            BOOST_RANDOM_DETAIL_OSTREAM_OPERATOR(os, sobol_engine, s) { return os << static_cast<const base_t&>(s); }

            /** Reads the textual representation of the generator from a @c std::istream. */
            BOOST_RANDOM_DETAIL_ISTREAM_OPERATOR(is, sobol_engine, s) { return is >> static_cast<base_t&>(s); }
         ...
         * https://web.archive.org/web/20170802022909/http://web.maths.unsw.edu.au/~fkuo/sobol/new-joe-kuo-6.21201
         *
