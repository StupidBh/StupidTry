    ...
    namespace random {
        ...
        public:
            ...
            explicit mixmax_engine(
                ...
                uint32_t machineID,
                uint32_t runID,
                uint32_t streamID); ///< Constructor, four 32-bit seeds for 128-bit seeding flexibility

            void seed(std::uint64_t seedval = default_seed)
            {
        ...
        public: // SEEDING FUNCTIONS
            template<class It>
            mixmax_engine(It& first, It last)
            {
                seed(first, last);
            }
            ...
            void discard(std::uint64_t nsteps)
            {
                for (std::uint64_t j = 0; j < nsteps; ++j) {
                    (*this)();
                }
                ...
                operator>>(std::basic_istream<CharT, Traits>& in, mixmax_engine& me)
            {
                ...
                if (sum == savedsum && counter > 0 && counter < Ndim) {
                    me.S.V = vec;
                    me.S.counter = counter;
                    me.S.sumtot = savedsum;
                }
                else {
            ...
            friend bool operator==(const mixmax_engine& x, const mixmax_engine& y)
            {
                return x.S.counter == y.S.counter && x.S.sumtot == y.S.sumtot && x.S.V == y.S.V;
            }

            friend bool operator!=(const mixmax_engine& x, const mixmax_engine& y) { return !operator==(x, y); }

        private:
            BOOST_STATIC_CONSTANT(int, BITS = 61);
            BOOST_STATIC_CONSTANT(std::uint64_t, M61 = 2'305'843'009'213'693'951ULL);
            ...
            inline void seed_vielbein(
                rng_state_t* X,
                unsigned int i); // seeds with the i-th unit vector, i = 0..Ndim-1,  for testing only
            inline void seed_uniquestream(
                rng_state_t* Xin,
                uint32_t clusterID,
                uint32_t machineID,
                uint32_t runID,
            ...
            inline std::uint64_t apply_bigskip(
                std::uint64_t* Vout,
                std::uint64_t* Vin,
                uint32_t clusterID,
                uint32_t machineID,
        ...
        mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::mixmax_engine()
        ...
        template<int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL>
        mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::mixmax_engine(std::uint64_t seedval)
        {
            ///< constructor, one uint64_t seed, random numbers are statistically independent from any two distinct
            ///< seeds, e.g. consecutive seeds are ok
            seed_uniquestream(&S, 0, 0, (uint32_t)(seedval >> 32), (uint32_t)seedval);
        }
        ...
        template<int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL>
        uint64_t mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::MULWU(uint64_t k)
        {
        ...
        template<int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL>
        std::uint64_t
            mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::iterate_raw_vec(std::uint64_t* Y, std::uint64_t sumtotOld)
        {
        ...
        template<int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL>
        void mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::seed_vielbein(rng_state_t* X, unsigned int index)
        {
        ...
        template<int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL>
        void mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::seed_uniquestream(
            rng_state_t* Xin,
            uint32_t clusterID,
            uint32_t machineID,
            uint32_t runID,
        ...
        template<int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL>
        std::uint64_t mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::apply_bigskip(
            std::uint64_t* Vout,
            std::uint64_t* Vin,
            uint32_t clusterID,
            uint32_t machineID,
            ...
            uint32_t streamID)
        {
            /*
             ...
             collide provided 1) at least one bit of ID is different 2) less than 10^100 numbers are drawn from the
             stream (this is good enough : a single CPU will not exceed this in the lifetime of the universe, 10^19 sec,
             ...
             vector by seed_vielbein(X,0), and use it in all your runs, just change runID to get completely
             nonoverlapping streams of random numbers on a different day.

             clusterID and machineID are provided for the benefit of large organizations who wish to ensure that a
             simulation which is running in parallel on a large number of  clusters and machines will have non-colliding
...
#include "boost/random/detail/mixmax_skip_N17.ipp"
        ...
        template<int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL>
        inline std::uint64_t
            mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::fmodmulM61(std::uint64_t cum, std::uint64_t s, std::uint64_t a)
        {
        ...
        template<int Ndim, unsigned int SPECIALMUL, std::int64_t SPECIAL>
        std::uint64_t mixmax_engine<Ndim, SPECIALMUL, SPECIAL>::modadd(std::uint64_t foo, std::uint64_t bar)
        {
