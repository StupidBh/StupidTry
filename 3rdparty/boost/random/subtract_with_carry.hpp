...
namespace boost {
    namespace random {
        ...
        public:
            ...
            BOOST_STATIC_CONSTANT(uint32_t, default_seed = 19'780'503u);
             ...
             * it with @c value.
             */
            BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(subtract_with_carry_engine, IntType, value) { seed(value); }

            /**
             * Constructs a new @c subtract_with_carry_engine and seeds
             * it with values produced by @c seq.generate().
             */
            BOOST_RANDOM_DETAIL_SEED_SEQ_CONSTRUCTOR(subtract_with_carry_engine, SeedSeq, seq) { seed(seq); }
            ...
            {
                typedef linear_congruential_engine<uint32_t, 40014, 0, 2'147'483'563> gen_t;
            ...
            {
                std::size_t short_index = (k < short_lag) ? (k + long_lag - short_lag) : (k - short_lag);
            ...
            void discard(boost::uintmax_t z) { detail::subtract_with_carry_discard::apply(*this, z); }
        ...
        private:
            ...
            IntType compute(unsigned int index) const { return x[(k + index) % long_lag]; }
        ...
        public:
            ...
            BOOST_STATIC_CONSTANT(std::size_t, long_lag = r);
            BOOST_STATIC_CONSTANT(std::size_t, short_lag = s);
            BOOST_STATIC_CONSTANT(boost::uint32_t, default_seed = 19'780'503u);
        ...
        private:
            /// \cond show_private
            RealType compute(unsigned int index) const { return x[(k + index) % long_lag]; }
        ...
        class subtract_with_carry : public subtract_with_carry_engine<IntType, boost::static_log2<m>::value, s, r> {
            typedef subtract_with_carry_engine<IntType, boost::static_log2<m>::value, s, r> base_type;

        public:
            subtract_with_carry() {}

            BOOST_RANDOM_DETAIL_GENERATOR_CONSTRUCTOR(subtract_with_carry, Gen, gen) { seed(gen); }

            BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(subtract_with_carry, IntType, val) { seed(val); }

            template<class It>
            subtract_with_carry(It& first, It last) :
            ...
            BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(subtract_with_carry, IntType, val) { base_type::seed(val); }

            template<class It>
            void seed(It& first, It last)
            {
                base_type::seed(first, last);
            }
        };

        template<class RealType, int w, unsigned s, unsigned r, int v = 0>
        class subtract_with_carry_01 : public subtract_with_carry_01_engine<RealType, w, s, r> {
            typedef subtract_with_carry_01_engine<RealType, w, s, r> base_type;

        public:
            subtract_with_carry_01() {}

            BOOST_RANDOM_DETAIL_GENERATOR_CONSTRUCTOR(subtract_with_carry_01, Gen, gen) { seed(gen); }

            BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(subtract_with_carry_01, uint32_t, val) { seed(val); }

            template<class It>
            subtract_with_carry_01(It& first, It last) :
                base_type(first, last)
            {
            }
            ...
            BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(subtract_with_carry_01, uint32_t, val) { base_type::seed(val); }
