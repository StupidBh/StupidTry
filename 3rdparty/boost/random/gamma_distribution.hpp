    ...
    namespace random {
        ...
        public:
            ...
            explicit gamma_distribution(
                const result_type& alpha_arg = result_type(1.0),
            ...
            result_type operator()(Engine& eng) const
            {
                ...
                if (_alpha == result_type(1)) {
                    ...
                    for (;;) {
                        ...
                        result_type x = sqrt(result_type(2) * _alpha - result_type(1)) * y + _alpha - result_type(1);
                        if (x <= result_type(0)) {
                            continue;
                        }
                        if (uniform_01<RealType>()(eng) >
                            (result_type(1) + y * y) *
                                exp((_alpha - result_type(1)) * log(x / (_alpha - result_type(1))) -
                                    sqrt(result_type(2) * _alpha - result_type(1)) * y)) {
                            continue;
                        }
                        return x * _beta;
                    }
             ...
             */
            friend bool operator==(const gamma_distribution& lhs, const gamma_distribution& rhs)
            {
                return lhs._alpha == rhs._alpha && lhs._beta == rhs._beta && lhs._exp == rhs._exp;
            }
