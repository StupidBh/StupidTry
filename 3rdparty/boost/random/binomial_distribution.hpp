...
namespace boost {
    namespace random {
        ...
        namespace detail {
            ...
            template<class RealType>
            const RealType binomial_table<RealType>::table[10] = { 0.08106146679532726,  0.04134069595540929,
        ...
        public:
             ...
             * Requires: t >=0 && 0 <= p <= 1
             */
            explicit binomial_distribution(IntType t_arg = 1, RealType p_arg = RealType(0.5)) :
                _t(t_arg),
                _p(p_arg)
            {
                init();
            ...
            friend bool operator==(const binomial_distribution& lhs, const binomial_distribution& rhs)
            {
                return lhs._t == rhs._t && lhs._p == rhs._p;
            }
        ...
        private:
            ...
            {
                if (k < 10) {
                    return detail::binomial_table<RealType>::table[k];
                }
                else {
                    RealType ikp1 = RealType(1) / (k + 1);
                    return (RealType(1) / 12 -
                            (RealType(1) / 360 - (RealType(1) / 1260) * (ikp1 * ikp1)) * (ikp1 * ikp1)) *
                           ikp1;
                }
            }
                ...
                while (true) {
                    ...
                    if (v <= _u.btrd.u_rv_r) {
                        ...
                        return static_cast<IntType>(
                            floor((2 * _u.btrd.a / (0.5 - abs(u)) + _u.btrd.b) * u + _u.btrd.c));
                    }
                    ...
                    if (km <= 15) {
                        ...
                        v = log(v);
                        RealType rho = (km / _u.btrd.npq) * (((km / 3. + 0.625) * km + 1. / 6) / _u.btrd.npq + 0.5);
                        ...
                        IntType nm = _t - m + 1;
                        RealType h = (m + 0.5) * log((m + 1) / (_u.btrd.r * nm)) + fc(m) + fc(_t - m);

                        IntType nk = _t - k + 1;
                        if (v <= h + (_t + 1) * log(static_cast<RealType>(nm) / nk) +
                                     (k + 0.5) * log(nk * _u.btrd.r / (k + 1)) - fc(k) - fc(_t - k)) {
                            return k;
                        }
                        else {
                            continue;
