    ...
    namespace random {
        namespace detail {
            ...
            struct integer_alias_table
            {
                ...
                {
                    _alias_table.clear();
                    _alias_table.push_back(std::make_pair(static_cast<WeightType>(1), static_cast<IntType>(0)));
                ...
                {
                    return _alias_table == other._alias_table && _average == other._average && _excess == other._excess;
                }

                static WeightType normalize(WeightType val, WeightType /* average */) { return val; }
                ...
                bool accept(IntType result, WeightType val) const { return result < _excess || val < _average; }

                static WeightType try_get_sum(const std::vector<WeightType>& weights)
                {
                    WeightType result = static_cast<WeightType>(0);
                    for (typename std::vector<WeightType>::const_iterator iter = weights.begin(), end = weights.end();
                         iter != end;
                         ++iter) {
                ...
                {
                    return uniform_int_distribution<WeightType>(static_cast<WeightType>(0), max - 1)(urng);
                }
            ...
            {
                WeightType get_weight(IntType) const { return WeightType(1.0); }

                template<class Iter>
                WeightType init_average(Iter first, Iter last)
                {
                    std::size_t size = std::distance(first, last);
                    WeightType weight_sum = std::accumulate(first, last, static_cast<WeightType>(0));
                ...
                bool operator==(const real_alias_table& other) const { return _alias_table == other._alias_table; }

                static WeightType normalize(WeightType val, WeightType average) { return val / average; }

                static void normalize(std::vector<WeightType>& weights)
                {
                    WeightType sum = std::accumulate(weights.begin(), weights.end(), static_cast<WeightType>(0));
                    for (typename std::vector<WeightType>::iterator iter = weights.begin(), end = weights.end();
                         iter != end;
                         ++iter) {
                        *iter /= sum;
                    }
                ...
                bool accept(IntType, WeightType) const { return true; }
        ...
        public:
            ...
            public:
                 ...
                 */
                param_type() :
                    _probabilities(1, static_cast<WeightType>(1))
                {
                }
                 ...
                 */
                std::vector<WeightType> probabilities() const { return _probabilities; }
            ...
            private:
                ...
                void normalize() { impl_type::normalize(_probabilities); }
             ...
             */
            discrete_distribution() { _impl.init_empty(); }
             ...
             */
            discrete_distribution(std::initializer_list<WeightType> wl) { init(wl.begin(), wl.end()); }
#endif
            /**
             ...
             */
            explicit discrete_distribution(const param_type& parm) { param(parm); }
            ...
            IntType operator()(URNG& urng, const param_type& parm) const
            {
                if (WeightType limit = impl_type::try_get_sum(parm._probabilities)) {
                    ...
                    WeightType sum = 0;
                    std::size_t result = 0;
                    for (typename std::vector<WeightType>::const_iterator iter = parm._probabilities.begin(),
            ...
            std::vector<WeightType> probabilities() const
            {
                ...
                std::size_t i = 0;
                for (typename impl_type::alias_table_t::const_iterator iter = _impl._alias_table.begin(),
            ...
            param_type param() const { return param_type(*this); }

            /** Sets the parameters of the distribution. */
            void param(const param_type& parm) { init(parm._probabilities.begin(), parm._probabilities.end()); }
             ...
             */
            BOOST_RANDOM_DETAIL_EQUALITY_OPERATOR(discrete_distribution, lhs, rhs) { return lhs._impl == rhs._impl; }
            /**
        ...
        private:
            ...
            void init(Iter first, Iter last, std::forward_iterator_tag)
            {
                ...
                typename impl_type::alias_table_t::iterator b_iter = below_average.begin(), b_end = below_average.end(),
                                                            a_iter = above_average.begin(), a_end = above_average.end();
            ...
            typedef typename detail::select_alias_table<(
                ::boost::is_integral<WeightType>::value)>::template apply<IntType, WeightType>::type impl_type;
            impl_type _impl;
            /// @endcond
        };
