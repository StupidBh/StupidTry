...
namespace boost {
    namespace random {
         ...
         * \f$\displaystyle P(x) = \frac{1}{2} e^{-(x+\lambda)/2} \left( \frac{x}{\lambda} \right)^{k/4-1/2} I_{k/2-1}(
         * \sqrt{\lambda x} )\f$. where  \f$\displaystyle I_\nu(z)\f$ is a modified Bessel function of the first kind.
         *
        ...
        public:
            ...
            public:
                 ...
                 */
                explicit param_type(RealType k_arg = RealType(1), RealType lambda_arg = RealType(1)) :
                    _k(k_arg),
                    _lambda(lambda_arg)
                {
             ...
             * Requires: k > 0 && lambda > 0
             */
            explicit non_central_chi_squared_distribution(
                RealType k_arg = RealType(1),
                RealType lambda_arg = RealType(1)) :
             ...
             */
            explicit non_central_chi_squared_distribution(const param_type& parm) :
                _param(parm)
            {
            }
            ...
            RealType min BOOST_PREVENT_MACRO_SUBSTITUTION() const { return RealType(0); }
