...
namespace boost {
    namespace mpi {
        template<>
        struct is_mpi_datatype<void*> : mpl::true_
        {
        };
    }
} // namespace boost
...
namespace boost {
    typedef mpl::if_c<
        ...
        mpl::if_c<
            ...
            long,
            mpl::if_c<(sizeof(void*) <= sizeof(boost::intmax_t)), boost::intmax_t, void>::type>::type>::type
        ptr_serialize_type;

    BOOST_STATIC_ASSERT((!boost::is_void<ptr_serialize_type>::value));

    template<typename T>
    inline T& unsafe_serialize(T& x)
    {
        return x;
    }
    ...
    namespace mpi {
        template<>
        inline MPI_Datatype get_mpi_datatype<void*>(void* const& x)
        {
    ...
    {
        return std::make_pair(x.first, x.second) < std::make_pair(y.first, y.second);
    }
