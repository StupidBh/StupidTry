...
namespace boost {
    namespace parallel {
        ...
        namespace detail {
            ...
            struct maybe_put_in_lvalue_pm
            {
                ...
                static inline void do_put(PropertyMap, const Key&, const Value&)
                {
                    BOOST_ASSERT(false);
                }
            };
            ...
            {
                template<typename PropertyMap, typename Key, typename Value>
                static inline void do_put(PropertyMap pm, const Key& key, const Value& value)
                {
                    using boost::put;
            ...
            template<typename PropertyMap, typename Key, typename Value>
            inline void maybe_put_impl(PropertyMap pm, const Key& key, const Value& value, writable_property_map_tag)
            {
                using boost::put;
            ...
            template<typename PropertyMap, typename Key, typename Value>
            inline void maybe_put_impl(PropertyMap pm, const Key& key, const Value& value, lvalue_property_map_tag)
            {
            ...
            template<typename PropertyMap, typename Key, typename Value>
            inline void maybe_put_impl(PropertyMap, const Key&, const Value&, ...)
            {
                BOOST_ASSERT(false);
            }

            template<typename PropertyMap, typename Key, typename Value>
            inline void maybe_put(PropertyMap pm, const Key& key, const Value& value)
            {
                maybe_put_impl(pm, key, value, typename property_traits<PropertyMap>::category());
            }
        ...
        public:
            ...
            typedef multi_index::multi_index_container<
                ...
                multi_index::indexed_by<
                    ...
                    multi_index::hashed_unique<pair_first_extractor<key_type, value_type>>>>
                ghost_cells_type;
            ...
            typedef typename ghost_cells_type::template nth_index<1>::type ghost_cells_key_index_type;
            ...
            typedef typename detail::make_nonlvalue_property_map<(
                ...
                is_same<lvalue_property_map_tag, local_category>::value)>::template apply<local_category>::type
                category;
                ...
                data(new data_t(pg, global, pm, basic_reduce<value_type>(), false))
            {
                ...
                data->process_group.template get_receiver<Handler>()->setup_triggers(data->process_group);
            }
            ...
            process_group_type process_group() const { return data->process_group.base(); }
            ...
            {
                send(data->process_group, p, property_map_put, boost::parallel::detail::make_untracked_pair(k, v));
            }
            ...
            struct data_t
            {
                data_t(
                    ...
                    storage(pm),
                    ghost_cells(),
                    max_ghost_cells(1'000'000),
                    get_default_value(dv),
                    has_default_resolver(has_default_resolver),
                    model(cm_forward)
                {
                }
        ...
        private:
            ...
            {
                explicit handle_message(const shared_ptr<data_t>& data, const Reduce& reduce = Reduce()) :
                    data_ptr(data),
                    reduce(reduce)
                {
                }
                ...
                value_type handle_get(int source, int tag, const key_type& data, trigger_receive_context);

                void handle_multiget(int source, int tag, const std::vector<key_type>& data, trigger_receive_context);

                void handle_multiget_reply(
                    int source,
                    int tag,
                    const std::vector<boost::parallel::detail::untracked_pair<key_type, value_type>>& msg,
                    trigger_receive_context);

                void handle_multiput(
                    int source,
                    int tag,
            ...
            {
                explicit on_synchronize(const shared_ptr<data_t>& data) :
                    data_ptr(data)
                {
                }
    ...
    #define PBGL_DISTRIB_PMAP distributed_property_map<ProcessGroup, GlobalMap, StorageMap>
        ...
        template<typename ProcessGroup, typename GlobalMap, typename StorageMap>
        inline void request(const PBGL_DISTRIB_PMAP& pm, typename PBGL_DISTRIB_PMAP::key_type const& key)
        {
         ...
         */
        template<typename ProcessGroup, typename GlobalMap, typename StorageMap>
        inline typename PBGL_DISTRIB_PMAP::value_type
            get(const PBGL_DISTRIB_PMAP& pm, typename PBGL_DISTRIB_PMAP::key_type const& key)
        {
            using boost::get;

            typename property_traits<GlobalMap>::value_type p = get(pm.data->global, key);
            ...
            typename PBGL_DISTRIB_PMAP::value_type const& value)
        {
            ...
            if (id != process_id(pm.data->process_group)) {
                pm.cell(key, false) = value;
            }
        }
        ...
        template<typename ProcessGroup, typename GlobalMap, typename StorageMap>
        void synchronize(PBGL_DISTRIB_PMAP& pm)
        {
            pm.do_synchronize();
        }
        ...
        {
            typedef distributed_property_map<ProcessGroup, GlobalMap, StorageMap> result_type;
