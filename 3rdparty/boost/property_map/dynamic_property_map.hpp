...
namespace boost {
    ...
    struct dynamic_const_put_error : public dynamic_property_exception
    {
        ...
        const char* what() const throw() BOOST_OVERRIDE { return "Attempt to put a value into a const property map: "; }
    };
    ...
    namespace detail {
        ...
        typename boost::property_traits<PMap>::reference get_wrapper_xxx(const PMap& pmap, const Key& key)
        {
            using boost::get;
            return get(pmap, key);
        }
        ...
        class dynamic_property_map_adaptor : public dynamic_property_map {
            ...
            void do_put(const any&, const any&, mpl::bool_<false>) { BOOST_THROW_EXCEPTION(dynamic_const_put_error()); }
        ...
        public:
            ...
            {
                return get_wrapper_xxx(
                    property_map_,
                    any_cast<typename boost::property_traits<PropertyMap>::key_type>(key_));
            }
            ...
            {
                do_put(in_key, in_value, mpl::bool_<(is_convertible<category*, writable_property_map_tag*>::value)>());
            }
    ...
    struct dynamic_properties
    {
        ...
        typedef boost::
            function3<boost::shared_ptr<dynamic_property_map>, const std::string&, const boost::any&, const boost::any&>
                generate_fn_type;
    ...
    public:
        ...
        iterator lower_bound(const std::string& name) { return property_maps.lower_bound(name); }

        const_iterator lower_bound(const std::string& name) const { return property_maps.lower_bound(name); }
    ...
    {
        for (dynamic_properties::iterator i = dp.lower_bound(name); i != dp.end() && i->first == name; ++i) {
    ...
    {
        for (dynamic_properties::const_iterator i = dp.lower_bound(name); i != dp.end() && i->first == name; ++i) {
            if (i->second->key() == typeid(key)) {
    ...
    inline boost::shared_ptr<boost::dynamic_property_map>
