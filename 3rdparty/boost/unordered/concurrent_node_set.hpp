...
namespace boost {
    namespace unordered {
        ...
        private:
            ...
            using type_policy =
                detail::foa::node_set_types<Key, typename boost::allocator_void_pointer<Allocator>::type>;
            ...
            bool friend
                operator==(concurrent_node_set<K, H, KE, A> const& lhs, concurrent_node_set<K, H, KE, A> const& rhs);

            template<class K, class H, class KE, class A, class Predicate>
            friend typename concurrent_node_set<K, H, KE, A>::size_type
                erase_if(concurrent_node_set<K, H, KE, A>& set, Predicate pred);

            template<class Archive, class K, class H, class KE, class A>
            friend void serialize(Archive& ar, concurrent_node_set<K, H, KE, A>& c, unsigned int version);
        ...
        public:
            ...
            template<class InputIterator>
            concurrent_node_set(InputIterator f, InputIterator l, allocator_type const& a) :
            ...
            concurrent_node_set(concurrent_node_set const& rhs, allocator_type const& a) :
                table_(rhs.table_, a)
            {
            }
            ...
            concurrent_node_set(std::initializer_list<value_type> il, const allocator_type& a) :
                concurrent_node_set(il, detail::foa::default_bucket_count, hasher(), key_equal(), a)
            {
            }
            ...
            concurrent_node_set& operator=(concurrent_node_set&& rhs) noexcept(
                boost::allocator_is_always_equal<Allocator>::type::value ||
                boost::allocator_propagate_on_container_move_assignment<Allocator>::type::value)
            {
                table_ = std::move(rhs.table_);
                return *this;
            }
        ...
        void swap(
            concurrent_node_set<Key, Hash, Pred, Alloc>& x,
            concurrent_node_set<Key, Hash, Pred, Alloc>& y) noexcept(noexcept(x.swap(y)))
        {
            x.swap(y);
        }

        template<class K, class H, class P, class A, class Predicate>
        typename concurrent_node_set<K, H, P, A>::size_type erase_if(concurrent_node_set<K, H, P, A>& c, Predicate pred)
        {
            return c.table_.erase_if(pred);
        }

        template<class Archive, class K, class H, class KE, class A>
        void serialize(Archive& ar, concurrent_node_set<K, H, KE, A>& c, unsigned int)
        {
        ...
        concurrent_node_set(
            ...
            Pred = Pred(),
            Allocator = Allocator())
            -> concurrent_node_set<typename std::iterator_traits<InputIterator>::value_type, Hash, Pred, Allocator>;
        ...
        concurrent_node_set(
            ...
            Hash = Hash(),
            Pred = Pred(),
            Allocator = Allocator()) -> concurrent_node_set<T, Hash, Pred, Allocator>;
        ...
        template<
            ...
            class = std::enable_if_t<detail::is_input_iterator_v<InputIterator>>,
            class = std::enable_if_t<detail::is_allocator_v<Allocator>>>
        concurrent_node_set(InputIterator, InputIterator, std::size_t, Allocator) -> concurrent_node_set<
        ...
        template<
            ...
            class = std::enable_if_t<detail::is_input_iterator_v<InputIterator>>,
            class = std::enable_if_t<detail::is_allocator_v<Allocator>>>
        concurrent_node_set(InputIterator, InputIterator, Allocator) -> concurrent_node_set<
            typename std::iterator_traits<InputIterator>::value_type,
            boost::hash<typename std::iterator_traits<InputIterator>::value_type>,
            std::equal_to<typename std::iterator_traits<InputIterator>::value_type>,
            Allocator>;
        ...
        template<
            ...
            class = std::enable_if_t<detail::is_input_iterator_v<InputIterator>>,
            class = std::enable_if_t<detail::is_allocator_v<Allocator>>>
        concurrent_node_set(InputIterator, InputIterator, std::size_t, Hash, Allocator) -> concurrent_node_set<
            typename std::iterator_traits<InputIterator>::value_type,
            Hash,
            std::equal_to<typename std::iterator_traits<InputIterator>::value_type>,
            Allocator>;
