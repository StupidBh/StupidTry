        ...
        namespace detail {
            namespace foa {
                ...
                class table_iterator {
                    ...
                    using table_element_pointer = typename group_pointer_traits::template rebind<table_element_type>;
                ...
                public:
                    ...
                    inline pointer operator->() const noexcept { return std::addressof(type_policy::value_from(*p())); }
                ...
                private:
                    ...
                    {
                        using diff_type = typename boost::pointer_traits<char_pointer>::difference_type;
                ...
                private:
                    ...
                    table_erase_return_type(const_iterator pos_) :
                        pos { pos_ }
                    {
                    }
                ...
                using table_core_impl = table_core<
                    TypePolicy,
                    group15<plain_integral>,
                    table_arrays,
                    plain_size_control,
                    Hash,
                    Pred,
    ...
    #pragma warning(disable: 4714) /* marked as __forceinline not inlined */
                ...
                class table : table_core_impl<TypePolicy, Hash, Pred, Allocator> {
                    ...
                    using group_type_pointer = typename boost::pointer_traits<
                        typename boost::allocator_pointer<Allocator>::type>::template rebind<group_type>;
                ...
                public:
                    ...
                    BOOST_FORCEINLINE
                        typename std::enable_if<is_emplace_kv_able<table, K>::value, std::pair<iterator, bool>>::type
                        emplace(K&& k, V&& v)
                    {
                    ...
                    BOOST_FORCEINLINE std::pair<iterator, bool> insert(const init_type& x) { return emplace_impl(x); }
                    ...
                    BOOST_FORCEINLINE
                        typename std::enable_if<!std::is_same<T, value_type>::value, std::pair<iterator, bool>>::type
                    ...
                    BOOST_FORCEINLINE auto erase(Key&& x) -> typename std::enable_if<
                        !std::is_convertible<Key, iterator>::value && !std::is_convertible<Key, const_iterator>::value,
                        std::size_t>::type
                    {
                        auto it = find(x);
                        if (it != end()) {
                    ...
                    {
                        using value_reference = typename std::
                            conditional<std::is_same<key_type, value_type>::value, const_reference, reference>::type;
                ...
                private:
                    ...
                    table(compatible_concurrent_table&& x, arrays_holder<ArraysType, Allocator>&& ah) :
                        super { std::move(x.h()),
                                ...
                                std::move(x.al()),
                                [&x] {
                                    return arrays_type { x.arrays.groups_size_index,
                    ...
                    {
                        erase_on_exit(table& x_, const_iterator it_) :
                            x(x_),
                            it(it_)
                        {
                        }
                    ...
                    BOOST_FORCEINLINE std::pair<iterator, bool> emplace_impl(Args&&... args)
                    {
                        ...
                        if (BOOST_LIKELY(this->size_ctrl.size < this->size_ctrl.ml)) {
                            return { make_iterator(this->unchecked_emplace_at(pos0, hash, std::forward<Args>(args)...)),
                                     true };
                        }
                        else {
