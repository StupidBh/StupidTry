...
#include <utility>
    ...
    #if defined(BOOST_UNORDERED_ENABLE_PARALLEL_ALGORITHMS) || !defined(BOOST_NO_CXX17_HDR_EXECUTION)
    ...
    namespace unordered {
        namespace detail {
            ...
            namespace foa {
                ...
                private:
                    ...
                    T* data(std::size_t pos) noexcept
                    {
                        return reinterpret_cast<T*>(
                            (reinterpret_cast<uintptr_t>(&buf) + cacheline_size - 1) / cacheline_size * cacheline_size +
                            pos * element_offset);
                    }
                ...
                {
                    using group_access_allocator_type = typename boost::allocator_rebind<Allocator, group_access>::type;
                    using group_access_pointer = typename boost::allocator_pointer<group_access_allocator_type>::type;
                    ...
                    group_access* group_accesses() const noexcept { return boost::to_address(group_accesses_); }
                    ...
                    {
                        arrays.group_accesses_ = boost::allocator_allocate(al, arrays.groups_size_mask + 1);
                    ...
                    static void
                        delete_group_access(group_access_allocator_type al, concurrent_table_arrays& arrays) noexcept
                    {
                        if (arrays.elements()) {
                            boost::allocator_deallocate(al, arrays.group_accesses_, arrays.groups_size_mask + 1);
                        }
                    }
...
#include <boost/unordered/detail/foa/ignore_wshadow.hpp>
    ...
    #pragma warning(disable: 4714) /* marked as __forceinline not inlined */
                ...
                private:
                    ...
                    using enable_if_is_value_type = typename std::enable_if<
                        !std::is_same<init_type, value_type>::value && std::is_same<Value, value_type>::value,
                        T>::type;
                ...
                public:
                    ...
                    concurrent_table(compatible_nonconcurrent_table&& x, arrays_holder<ArraysType, Allocator>&& ah) :
                        super { std::move(x.h()),
                                std::move(x.pred()),
                                std::move(x.al()),
                                [&x] {
                                    return arrays_type::new_group_access(
                                        ...
                                        typename arrays_type::super {
                                            x.arrays.groups_size_index,
                                            x.arrays.groups_size_mask,
                    ...
                    {
                        visit_all_impl(group_exclusive {}, std::forward<ExecutionPolicy>(policy), std::forward<F>(f));
                    }
                    ...
                    BOOST_FORCEINLINE auto emplace(Value&& x) -> typename std::
                        enable_if<detail::is_similar_to_any<Value, value_type, init_type>::value, bool>::type
                    {
                    ...
                    {
                        return emplace_impl(try_emplace_args_t {}, std::forward<Key>(x), std::forward<Args>(args)...);
                    }
                    ...
                    {
                        return construct_and_emplace_and_visit_flast(group_exclusive {}, std::forward<Args>(args)...);
                    }
                    ...
                    {
                        return insert_and_visit(std::forward<Value>(x), [](const value_type&) {}, std::forward<F>(f));
                    }

                    template<typename Value, typename F>
                    BOOST_FORCEINLINE bool insert_or_cvisit(Value&& x, F&& f)
                    {
                        return insert_and_cvisit(std::forward<Value>(x), [](const value_type&) {}, std::forward<F>(f));
                    }
                    ...
                    {
                        return emplace_and_visit_impl(group_shared {}, std::forward<F1>(f1), std::forward<F2>(f2), x);
                    }
                    ...
                    friend bool operator!=(const concurrent_table& x, const concurrent_table& y) { return !(x == y); }
                ...
                private:
                    ...
                    static inline exclusive_bilock_guard
                        exclusive_access(const concurrent_table& x, const concurrent_table& y)
                    {
                    ...
                    static inline typename std::
                        conditional<std::is_same<key_type, value_type>::value, const value_type&, value_type&>::type
                    ...
                    BOOST_FORCEINLINE std::size_t visit_impl(GroupAccessMode access_mode, const Key& x, F&& f) const
                    {
                        auto lck = shared_access();
                        auto hash = this->hash_for(x);
                        return unprotected_visit(access_mode, x, this->position_for(hash), hash, std::forward<F>(f));
                    }

                    template<typename GroupAccessMode, typename FwdIterator, typename F>
                    BOOST_FORCEINLINE std::size_t
                        bulk_visit_impl(GroupAccessMode access_mode, FwdIterator first, FwdIterator last, F&& f) const
                    {
                        auto lck = shared_access();
                        std::size_t res = 0;
                        ...
                        while (n) {
                            ...
                            std::advance(
                                first,
                                static_cast<typename std::iterator_traits<FwdIterator>::difference_type>(m));
                        }
                        return res;
                    }
    ...
    #pragma warning(disable: 4800)
                    ...
                    template<typename GroupAccessMode, typename FwdIterator, typename F>
                    BOOST_FORCEINLINE std::size_t
                        unprotected_bulk_visit(GroupAccessMode access_mode, FwdIterator first, std::size_t m, F&& f)
                            const
                    {
...
next_key:;
    ...
    #pragma warning(pop) /* C4800 */
                    ...
                    BOOST_FORCEINLINE bool
                        construct_and_emplace_or_visit_flast(GroupAccessMode access_mode, Args&&... args)
                    {
                    ...
                    template<typename GroupAccessMode, typename... Args>
                    BOOST_FORCEINLINE bool
                        construct_and_emplace_and_visit_flast(GroupAccessMode access_mode, Args&&... args)
                    {
                        return mp11::tuple_apply(
                            call_construct_and_emplace_and_visit {},
                            std::tuple_cat(
                                std::make_tuple(this, access_mode),
                                tuple_rotate_right<2>(std::forward_as_tuple(std::forward<Args>(args)...))));
                    }

                    template<typename GroupAccessMode, typename F, typename... Args>
                    BOOST_FORCEINLINE bool
                        construct_and_emplace_or_visit(GroupAccessMode access_mode, F&& f, Args&&... args)
                    {
                    ...
                    BOOST_FORCEINLINE bool
                        construct_and_emplace_and_visit(GroupAccessMode access_mode, F1&& f1, F2&& f2, Args&&... args)
                    {
                        auto lck = shared_access();
                    ...
                    template<typename GroupAccessMode, typename F, typename... Args>
                    BOOST_NOINLINE bool noinline_emplace_or_visit(GroupAccessMode access_mode, F&& f, Args&&... args)
                    {
                    ...
                    template<typename GroupAccessMode, typename F1, typename F2, typename... Args>
                    BOOST_NOINLINE bool
                        noinline_emplace_and_visit(GroupAccessMode access_mode, F1&& f1, F2&& f2, Args&&... args)
                    {
                        return emplace_and_visit_impl(
                            access_mode,
                            std::forward<F1>(f1),
                    ...
                    template<typename GroupAccessMode, typename F, typename... Args>
                    BOOST_FORCEINLINE bool emplace_or_visit_impl(GroupAccessMode access_mode, F&& f, Args&&... args)
                    {
                        return emplace_and_visit_impl(
                            access_mode,
                            [](const value_type&) {},
                    ...
                    template<typename GroupAccessMode, typename F1, typename F2, typename... Args>
                    BOOST_FORCEINLINE bool
                        emplace_and_visit_impl(GroupAccessMode access_mode, F1&& f1, F2&& f2, Args&&... args)
                    {
                        for (;;) {
                            {
                                auto lck = shared_access();
                    ...
                    BOOST_FORCEINLINE int
                        unprotected_norehash_emplace_or_visit(GroupAccessMode access_mode, F&& f, Args&&... args)
                    {
                    ...
                    struct reserve_size
                    {
                        reserve_size(concurrent_table& x_) :
                            x(x_)
                        {
                    ...
                    template<typename GroupAccessMode, typename F1, typename F2, typename... Args>
                    BOOST_FORCEINLINE int unprotected_norehash_emplace_and_visit(
                        GroupAccessMode access_mode,
                        F1&& f1,
                        F2&& f2,
                        Args&&... args)
...
startover:
                    ...
                    template<typename GroupAccessMode, typename F>
                    auto for_all_elements_while(GroupAccessMode access_mode, F f) const -> decltype(f(nullptr), bool())
                    {
                    ...
                    {
                        save(ar, version, std::integral_constant<bool, std::is_same<key_type, value_type>::value> {});
                    }
                    ...
                    {
                        load(ar, version, std::integral_constant<bool, std::is_same<key_type, value_type>::value> {});
                    }
                    ...
                    void load(Archive& ar, unsigned int, std::true_type /* set */)
                    {
                        ...
                        for (std::size_t n = 0; n < s; ++n) {
                            ...
                            ar.reset_object_address(std::addressof(type_policy::value_from(*loc.p)), std::addressof(x));
                        }
                    }
