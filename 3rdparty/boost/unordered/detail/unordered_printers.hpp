...
__asm__(
    ...
    ".ascii \"        else:\\n\"\n"
    ".ascii \"            self.to_address = lambda ptr: ptr if (ptr.type.code == gdb.TYPE_CODE_PTR) else "
    "type(vis).boost_to_address(ptr)\\n\"\n"
    ...
    ".ascii \"        self.cpo = "
    "BoostUnorderedPointerCustomizationPoint(self.table[\\\"buckets_\\\"][\\\"buckets\\\"])\\n\"\n"
    ...
    ".ascii \"        self.is_map = self.name.endswith(\\\"map\\\")\\n\"\n"
    ".ascii \"        self.cpo = "
    "BoostUnorderedPointerCustomizationPoint(self.table[\\\"arrays\\\"][\\\"groups_\\\"])\\n\"\n"
    ...
    ".ascii \"    def to_string(self):\\n\"\n"
    ".ascii \"        size = "
    "BoostUnorderedHelpers.maybe_unwrap_atomic(self.table[\\\"size_ctrl\\\"][\\\"size\\\"])\\n\"\n"
    ".ascii \"        return f\\\"{self.name} with {size} elements\\\"\\n\"\n"
    ...
    ".ascii \"        else:\\n\"\n"
    ".ascii \"            return pos == self.N-1 and (at(0) & 0x4000400040004000) == 0x4000 and (at(1) & "
    ...
    "self.groups.dereference().type.sizeof\\n\"\n"
    ".ascii \"                pc_ = self.cpo.next(pc_, -n0)\\n\"\n"

    ".ascii \"                mask = (self.match_occupied(pc_.cast(self.groups.type).dereference()) >> (n0+1)) << "
    "(n0+1)\\n\"\n"
    ".ascii \"                while mask == 0:\\n\"\n"
    ...
    ".ascii \"    add_template_printer = lambda name, printer: pp.add_printer(name, f\\\"^{name}<.*>$\\\", "
    "printer)\\n\"\n"
    ...
    ".ascii \"    add_template_printer(\\\"boost::unordered::detail::iterator_detail::iterator\\\", "
    "BoostUnorderedFcaIteratorPrinter)\\n\"\n"
    ".ascii \"    add_template_printer(\\\"boost::unordered::detail::iterator_detail::c_iterator\\\", "
    "BoostUnorderedFcaIteratorPrinter)\\n\"\n"
    ...
    ".ascii \"    add_concrete_printer(\\\"boost::unordered::detail::foa::table_core_cumulative_stats\\\", "
    ...
    ".ascii \"    add_template_printer(\\\"boost::unordered::detail::foa::concurrent_cumulative_stats\\\", "
    "BoostUnorderedFoaCumulativeStatsPrinter)\\n\"\n"
    ...
    ".ascii \"    * Returns a `gdb.Value` with the raw pointer equivalent to your fancy pointer, as if you did the "
    ...
    ".ascii \"    * Note, you will not actually do these operations as stated. You will do equivalent lower-level "
    "operations that emulate having done the above\\n\"\n"
    ".ascii \"        * Ultimately, it will be as if you called `operator+()` on your fancy pointer in C++, but using "
    "only raw pointers\\n\"\n"
