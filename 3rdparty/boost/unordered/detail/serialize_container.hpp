...
namespace boost {
    namespace unordered {
        namespace detail {
            ...
            struct load_or_save_unordered_set<Set, true> /* save */
            {
            ...
            template<typename Set>
            struct load_or_save_unordered_set<Set, false> /* load */
            {
                template<typename Archive>
                void operator()(Archive& ar, Set& x, unsigned int) const
                {
                    ...
                    for (std::size_t n = 0; n < s; ++n) {
                        ...
                        std::pair<iterator, bool> p = adapt_insert_return_type(x.insert(std::move(value.get())));
                        if (!p.second) {
            ...
            struct load_or_save_unordered_map<Map, true> /* save */
            {
                template<typename Archive>
            ...
            template<typename Map>
            struct load_or_save_unordered_map<Map, false> /* load */
            {
                template<typename Archive>
                void operator()(Archive& ar, Map& x, unsigned int) const
                {
                    ...
                    for (std::size_t n = 0; n < s; ++n) {
                        ...
                        std::pair<iterator, bool> p =
                            adapt_insert_return_type(x.emplace(std::move(key.get()), std::move(mapped.get())));
                        if (!p.second) {
                            throw_exception(bad_archive_exception());
                        }
