...
#define BOOST_UNORDERED_STATIC_ASSERT_INVOCABLE(F)                     \
    static_assert(                                                     \
        boost::unordered::detail::is_invocable<F, value_type&>::value, \
...
#define BOOST_UNORDERED_DETAIL_LAST_ARG(Arg, Args) mp11::mp_back<mp11::mp_list<Arg BOOST_UNORDERED_DETAIL_COMMA Args>>
...
#define BOOST_UNORDERED_DETAIL_PENULTIMATE_ARG(Arg1, Arg2, Args)                                              \
    mp11::mp_at_c<                                                                                            \
        mp11::mp_list<Arg1 BOOST_UNORDERED_DETAIL_COMMA Arg2 BOOST_UNORDERED_DETAIL_COMMA Args>,              \
        mp11::mp_size<                                                                                        \
            mp11::mp_list<Arg1 BOOST_UNORDERED_DETAIL_COMMA Arg2 BOOST_UNORDERED_DETAIL_COMMA Args>>::value - \
            2>

#define BOOST_UNORDERED_STATIC_ASSERT_PENULTIMATE_ARG_INVOCABLE(Arg1, Arg2, Args) \
    BOOST_UNORDERED_STATIC_ASSERT_INVOCABLE(BOOST_UNORDERED_DETAIL_PENULTIMATE_ARG(Arg1, Arg2, Args))

#define BOOST_UNORDERED_STATIC_ASSERT_PENULTIMATE_ARG_CONST_INVOCABLE(Arg1, Arg2, Args) \
    BOOST_UNORDERED_STATIC_ASSERT_CONST_INVOCABLE(BOOST_UNORDERED_DETAIL_PENULTIMATE_ARG(Arg1, Arg2, Args))
...
namespace boost {
    ...
    #define BOOST_UNORDERED_STATIC_ASSERT_FWD_ITERATOR(Iterator)                                                     \
        static_assert(                                                                                               \
            std::is_base_of<std::forward_iterator_tag, typename std::iterator_traits<Iterator>::iterator_category>:: \
                value,                                                                                               \
            "The provided iterator must be at least forward");
#else
    #define BOOST_UNORDERED_STATIC_ASSERT_FWD_ITERATOR(Iterator) \
        static_assert(std::forward_iterator<Iterator>, "The provided iterator must be at least forward");
...
#define BOOST_UNORDERED_STATIC_ASSERT_KEY_COMPATIBLE_ITERATOR(Iterator)                                             \
    static_assert(                                                                                                  \
        std::is_same<typename std::iterator_traits<Iterator>::value_type, key_type>::value ||                       \
            detail::are_transparent<typename std::iterator_traits<Iterator>::value_type, hasher, key_equal>::value, \
