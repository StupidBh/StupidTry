...
#include <boost/detail/workaround.hpp>
    ...
    !BOOST_WORKAROUND(BOOST_MSVC, < 1800) && !BOOST_WORKAROUND(BOOST_GCC_VERSION, < 40900)
...
namespace boost {
    ...
    template<class T>
    struct is_copy_constructible : public boost::is_constructible<T, const T&>
    {
    };
...
namespace boost {
    ...
    namespace detail {
        ...
        struct is_copy_constructible_imp : public boost::is_constructible<T, const T&>
        {
        };

        template<class T>
        struct is_copy_constructible_imp<T, true> : public false_type
        {
        };
    ...
    template<class T>
    struct is_copy_constructible : public detail::is_copy_constructible_imp<T, is_noncopyable<T>::value>
    {
    };
    ...
    namespace detail {
        ...
        !(defined(BOOST_MSVC) && _MSC_VER == 1800)
            ...
            static boost::type_traits::yes_type
                test(const T1&, boost::mpl::int_<sizeof(T1(boost::declval<const T1&>()))>* = 0);
        #else
            template<class T1>
            ...
            static boost::type_traits::no_type
                test(const T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
            ...
            BOOST_STATIC_CONSTANT(
                bool,
                value =
                    (sizeof(test(boost::declval<BOOST_DEDUCED_TYPENAME boost::add_reference<T const>::type>())) ==
                         sizeof(boost::type_traits::yes_type) &&
                     !boost::is_rvalue_reference<T>::value && !boost::is_array<T>::value));
        };
        ...
        struct is_copy_constructible_impl
        {
            BOOST_STATIC_CONSTANT(
                bool,
                value = (boost::detail::is_copy_constructible_impl2<boost::is_noncopyable<T>::value, T>::value));
        };

    } // namespace detail

    template<class T>
    struct is_copy_constructible : public integral_constant<bool, ::boost::detail::is_copy_constructible_impl<T>::value>
    {
    };
