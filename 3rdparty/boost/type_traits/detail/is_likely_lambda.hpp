...
#elif !defined(BOOST_NO_CXX11_LAMBDAS) && !defined(BOOST_NO_CXX11_DECLTYPE) &&            \
    !defined(BOOST_NO_CXX11_TEMPLATE_ALIASES) && !BOOST_WORKAROUND(BOOST_MSVC, < 1900) && \
...
namespace boost {
    ...
    namespace type_traits_detail {
        ...
        {
            template<typename Q>
            static boost::true_type test(decltype(&Q::operator())*);
            template<typename>
        ...
        template<typename T>
        struct is_likely_stateless_lambda<T, typename boost::enable_if_<has_one_operator_call<T>::value>::type> :
