...
#if (                                                                                                                \
    BOOST_WORKAROUND(__MWERKS__, < 0x3000) || BOOST_WORKAROUND(__IBMCPP__, < 600) ||                                 \
    BOOST_WORKAROUND(BOOST_BORLANDC, < 0x5A0) || defined(__ghs) || BOOST_WORKAROUND(__HP_aCC, < 60700) ||            \
    BOOST_WORKAROUND(MPW_CPLUS, BOOST_TESTED_AT(0x890)) || BOOST_WORKAROUND(__SUNPRO_CC, BOOST_TESTED_AT(0x580))) && \
...
//
    ...
    defined(BOOST_TT_HAS_ACCURATE_BINARY_OPERATOR_DETECTION)
...
// set, then don't treat noexcept functions as seperate types.  This is a fix for msvc with the /Zc:noexceptTypes- flag
// set.
//
#if defined(__cpp_rvalue_references) && !defined(__cpp_noexcept_function_type) && \
    !defined(BOOST_TT_NO_NOEXCEPT_SEPARATE_TYPE)
    #define BOOST_TT_NO_NOEXCEPT_SEPARATE_TYPE
#endif
//
...
//
#if defined(BOOST_MSVC) && !defined(__cpp_rvalue_references) && !defined(BOOST_TT_NO_NOEXCEPT_SEPARATE_TYPE) && \
    !defined(_NOEXCEPT_TYPES_SUPPORTED)
    #define BOOST_TT_NO_NOEXCEPT_SEPARATE_TYPE
#endif
#if defined(__cpp_rvalue_references) && defined(__NVCC__) && defined(__CUDACC__) && \
    !defined(BOOST_TT_NO_NOEXCEPT_SEPARATE_TYPE)
    #define BOOST_TT_NO_NOEXCEPT_SEPARATE_TYPE
#endif
