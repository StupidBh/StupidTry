...
namespace boost {
    ...
    namespace detail {
...
#ifndef BOOST_IS_BASE_OF
    #if !BOOST_WORKAROUND(BOOST_BORLANDC, BOOST_TESTED_AT(0x581)) && !BOOST_WORKAROUND(__SUNPRO_CC, <= 0x540) && \
        !BOOST_WORKAROUND(__EDG_VERSION__, <= 243) && !BOOST_WORKAROUND(__DMC__, BOOST_TESTED_AT(0x840))
        ...
        *************************************************************************/
        ...
        struct is_base_and_derived_impl2
        {
            BOOST_STATIC_CONSTANT(bool, value = (::boost::is_convertible<D*, B*>::value));
        };
