...
#define BOOST_TT_INTRINSICS_HPP_INCLUDED
    ...
    //
    ...
    #if (defined(BOOST_MSVC) && defined(BOOST_MSVC_FULL_VER) && (BOOST_MSVC_FULL_VER >= 140'050'215)) || \
        ...
        #define BOOST_HAS_NOTHROW_CONSTRUCTOR(T) \
            (__has_nothrow_constructor(T) || ::boost::has_trivial_constructor<T>::value)
        #if !defined(BOOST_INTEL)
            #define BOOST_HAS_NOTHROW_COPY(T) \
                ((__has_nothrow_copy(T) || ::boost::has_trivial_copy<T>::value) && !is_array<T>::value)
            #define BOOST_HAS_TRIVIAL_COPY(T) (__has_trivial_copy(T) || ::boost::is_pod<T>::value)
        #elif (_MSC_VER >= 1900)
            #define BOOST_HAS_NOTHROW_COPY(T)                                                                         \
                ((__is_nothrow_constructible(T, typename add_lvalue_reference<typename add_const<T>::type>::type)) && \
                 !is_array<T>::value)
            #define BOOST_HAS_TRIVIAL_COPY(T) \
                (__is_trivially_constructible(T, typename add_lvalue_reference<typename add_const<T>::type>::type))
        #endif
        ...
        #define BOOST_IS_CONVERTIBLE(T, U) \
            ((__is_convertible_to(T, U) || (is_same<T, U>::value && !is_function<U>::value)) && !__is_abstract(U))
        #define BOOST_IS_ENUM(T) __is_enum(T)
        //  This one fails if the default alignment has been changed with /Zp:
        //  #   define BOOST_ALIGNMENT_OF(T) __alignof(T)

        #if defined(_MSC_VER) && (_MSC_VER >= 1800)
            #define BOOST_HAS_TRIVIAL_MOVE_CONSTRUCTOR(T)                             \
                ((__is_trivially_constructible(T, T&&) || boost::is_pod<T>::value) && \
                 !::boost::is_volatile<T>::value && !::boost::is_reference<T>::value)
            #define BOOST_HAS_TRIVIAL_MOVE_ASSIGN(T)                                                               \
                ((__is_trivially_assignable(T, T&&) || boost::is_pod<T>::value) && !::boost::is_const<T>::value && \
                 !::boost::is_volatile<T>::value && !::boost::is_reference<T>::value)
        #elif defined(_MSC_VER) && (_MSC_VER >= 1700)
            #define BOOST_HAS_TRIVIAL_MOVE_CONSTRUCTOR(T)                                                             \
                ((__has_trivial_move_constructor(T) || boost::is_pod<T>::value) && !::boost::is_volatile<T>::value && \
                 !::boost::is_reference<T>::value)
            #define BOOST_HAS_TRIVIAL_MOVE_ASSIGN(T)                                                          \
                ((__has_trivial_move_assign(T) || boost::is_pod<T>::value) && !::boost::is_const<T>::value && \
                 !::boost::is_volatile<T>::value && !::boost::is_reference<T>::value)
        #endif
    ...
    #if defined(BOOST_CLANG) && defined(__has_feature) && defined(__has_builtin) && \
        (!(defined(__CUDACC__) && (__CUDACC_VER_MAJOR__ < 11)) || defined(__CUDA__))
        //
        ...
        #include <cstddef>
            ...
            #define BOOST_IS_UNION(T) __is_union(T)
        #endif
        #if (!defined(__GLIBCXX__) || (__GLIBCXX__ >= 20'080'306 && __GLIBCXX__ != 20'080'519)) && __has_feature(is_pod)
            #define BOOST_IS_POD(T) __is_pod(T)
        #endif
        #if (!defined(__GLIBCXX__) || (__GLIBCXX__ >= 20'080'306 && __GLIBCXX__ != 20'080'519)) && \
            __has_feature(is_empty)
            #define BOOST_IS_EMPTY(T) __is_empty(T)
        #endif
            ...
            #define BOOST_HAS_TRIVIAL_ASSIGN(T)                                       \
                (__is_trivially_assignable(T&, const T&) && !is_volatile<T>::value && \
                 is_assignable<T&, const T&>::value)
        #elif __has_feature(has_trivial_assign)
            #define BOOST_HAS_TRIVIAL_ASSIGN(T) \
                (__has_trivial_assign(T) && !is_volatile<T>::value && is_assignable<T&, const T&>::value)
        #endif
            ...
            #define BOOST_HAS_NOTHROW_CONSTRUCTOR(T) \
                (__is_nothrow_constructible(T) && is_default_constructible<T>::value)
        ...
        #if __has_builtin(__is_nothrow_constructible)
            #define BOOST_HAS_NOTHROW_COPY(T)                                                                    \
                (__is_nothrow_constructible(T, const T&) && !is_volatile<T>::value && !is_reference<T>::value && \
                 is_copy_constructible<T>::value)
        #elif __has_feature(has_nothrow_copy)
            #define BOOST_HAS_NOTHROW_COPY(T)                                                  \
                (__has_nothrow_copy(T) && !is_volatile<T>::value && !is_reference<T>::value && \
                 is_copy_constructible<T>::value)
        #endif
        #if __has_builtin(__is_nothrow_assignable)
            #define BOOST_HAS_NOTHROW_ASSIGN(T) \
            ...
            #define BOOST_IS_POLYMORPHIC(T) __is_polymorphic(T)
                ...
                #define BOOST_HAS_TRIVIAL_MOVE_CONSTRUCTOR(T)                                   \
                    (__is_trivially_constructible(T, T&&) && is_constructible<T, T&&>::value && \
                     !::boost::is_volatile<T>::value)
            #endif
            #if __has_extension(is_trivially_assignable)
                #define BOOST_HAS_TRIVIAL_MOVE_ASSIGN(T)                                    \
                    (__is_trivially_assignable(T&, T&&) && is_assignable<T&, T&&>::value && \
                     !::boost::is_volatile<T>::value)
            #endif
        #endif
        ...
        #define BOOST_IS_EMPTY(T) __is_empty(T)
        #define BOOST_HAS_TRIVIAL_CONSTRUCTOR(T) \
            ((__has_trivial_constructor(T) BOOST_INTEL_TT_OPTS) && !::boost::is_volatile<T>::value)
        #define BOOST_HAS_TRIVIAL_COPY(T) ((__has_trivial_copy(T) BOOST_INTEL_TT_OPTS) && !is_reference<T>::value)
        #if (__GNUC__ * 100 + __GNUC_MINOR__) >= 409
            #define BOOST_HAS_TRIVIAL_ASSIGN(T)                                                      \
                ((__has_trivial_assign(T) BOOST_INTEL_TT_OPTS) && !::boost::is_volatile<T>::value && \
                 !::boost::is_const<T>::value && is_assignable<T&, const T&>::value)
            #define BOOST_HAS_TRIVIAL_DESTRUCTOR(T) \
                (__has_trivial_destructor(T) BOOST_INTEL_TT_OPTS && is_destructible<T>::value)
            #define BOOST_HAS_NOTHROW_CONSTRUCTOR(T) \
                (__has_nothrow_constructor(T) && is_default_constructible<T>::value BOOST_INTEL_TT_OPTS)
            #define BOOST_HAS_NOTHROW_COPY(T)                                                                        \
                ((__has_nothrow_copy(T) BOOST_INTEL_TT_OPTS) && !is_volatile<T>::value && !is_reference<T>::value && \
                 is_copy_constructible<T>::value)
            #define BOOST_HAS_NOTHROW_ASSIGN(T)                                                                    \
                ((__has_nothrow_assign(T) BOOST_INTEL_TT_OPTS) && !is_volatile<T>::value && !is_const<T>::value && \
                 is_assignable<T&, const T&>::value)
        #else
            #define BOOST_HAS_TRIVIAL_ASSIGN(T)                                                      \
                ((__has_trivial_assign(T) BOOST_INTEL_TT_OPTS) && !::boost::is_volatile<T>::value && \
            ...
            #define BOOST_HAS_NOTHROW_CONSTRUCTOR(T) (__has_nothrow_constructor(T) BOOST_INTEL_TT_OPTS)
                ...
                #define BOOST_HAS_NOTHROW_COPY(T)                                             \
                    ((__has_nothrow_copy(T) BOOST_INTEL_TT_OPTS) && !is_volatile<T>::value && \
                     !is_reference<T>::value && !is_array<T>::value)
            #endif
            #define BOOST_HAS_NOTHROW_ASSIGN(T)                                                                    \
                ((__has_nothrow_assign(T) BOOST_INTEL_TT_OPTS) && !is_volatile<T>::value && !is_const<T>::value && \
                 !is_array<T>::value)
        ...
        #define BOOST_IS_ENUM(T)        __is_enum(T)
        #define BOOST_IS_POLYMORPHIC(T) __is_polymorphic(T)
        #if (!defined(unix) && !defined(__unix__) && !(defined(__VXWORKS__) && defined(__i386__))) || defined(__LP64__)
            // GCC sometimes lies about alignment requirements
            // of type double on 32-bit unix platforms, use the
            // old implementation instead in that case:
        ...
        #define BOOST_HAS_TRIVIAL_CONSTRUCTOR(T) \
            (__oracle_has_trivial_constructor(T) && !::boost::is_volatile<T>::value)
        #define BOOST_HAS_TRIVIAL_COPY(T) (__oracle_has_trivial_copy(T) && !is_reference<T>::value)
        #define BOOST_HAS_TRIVIAL_ASSIGN(T)                                                                   \
            ((__oracle_has_trivial_assign(T) || __oracle_is_trivial(T)) && !::boost::is_volatile<T>::value && \
             !::boost::is_const<T>::value && is_assignable<T&, const T&>::value)
        #define BOOST_HAS_TRIVIAL_DESTRUCTOR(T) (__oracle_has_trivial_destructor(T) && is_destructible<T>::value)
        #define BOOST_HAS_NOTHROW_CONSTRUCTOR(T)                                                                       \
            ((__oracle_has_nothrow_constructor(T) || __oracle_has_trivial_constructor(T) || __oracle_is_trivial(T)) && \
             is_default_constructible<T>::value)
        //  __oracle_has_nothrow_copy appears to behave the same as __oracle_has_nothrow_assign, disabled for now:
        // #   define BOOST_HAS_NOTHROW_COPY(T) ((__oracle_has_nothrow_copy(T) || __oracle_has_trivial_copy(T) ||
        // __oracle_is_trivial(T)) && !is_volatile<T>::value && !is_reference<T>::value &&
        // is_copy_constructible<T>::value)
        #define BOOST_HAS_NOTHROW_ASSIGN(T)                                                                  \
            ((__oracle_has_nothrow_assign(T) || __oracle_has_trivial_assign(T) || __oracle_is_trivial(T)) && \
             !is_volatile<T>::value && !is_const<T>::value && is_assignable<T&, const T&>::value)
        ...
        #define BOOST_HAS_NOTHROW_COPY(T) \
            (__has_nothrow_copy_constructor(T) && !is_volatile<T>::value && !is_reference<T>::value)
        #define BOOST_HAS_NOTHROW_ASSIGN(T)     (__has_nothrow_assign(T) && !is_volatile<T>::value)
        #define BOOST_HAS_VIRTUAL_DESTRUCTOR(T) __has_virtual_destructor(T)
