...
//  mimomorin@...     - For a patch to use rvalue refs on supporting compilers
    ...
    // Some compilers let us detect even const-qualified rvalues at compile-time
    #if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES) || defined(BOOST_MSVC) && !defined(_PREFAST_) || \
        (BOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ <= 5) && !defined(BOOST_INTEL) &&      \
         !defined(BOOST_CLANG)) ||                                                                  \
        (BOOST_WORKAROUND(__GNUC__, == 3) && (__GNUC_MINOR__ >= 4) && !defined(BOOST_INTEL) && !defined(BOOST_CLANG))
        ...
        // temporaries and avoid reevaluation of the collection expression.
            ...
            BOOST_WORKAROUND(__SUNPRO_CC, < 0x5100) || BOOST_WORKAROUND(__DECCXX_VER, <= 60'590'042)
            #define BOOST_FOREACH_NO_RVALUE_DETECTION
        #endif
        // Some compilers do not correctly implement the lvalue/rvalue conversion
        // rules of the ternary conditional operator.
        #if defined(BOOST_FOREACH_NO_RVALUE_DETECTION) || defined(BOOST_NO_SFINAE) ||                            \
            ...
            BOOST_WORKAROUND(__MWERKS__, BOOST_TESTED_AT(0x3206)) || BOOST_WORKAROUND(__SUNPRO_CC, >= 0x5100) || \
...
namespace boost {
    ...
    namespace foreach {
        ...
        template<typename T>
        struct is_lightweight_proxy : boost::mpl::false_
        {
        };
...
template<typename T>
inline boost::foreach::is_lightweight_proxy<T>* boost_foreach_is_lightweight_proxy(T*&, BOOST_FOREACH_TAG_DEFAULT)
{
    return 0;
}

template<typename T>
inline boost::mpl::true_* boost_foreach_is_lightweight_proxy(std::pair<T, T>*&, boost::foreach::tag)
{
    return 0;
}

template<typename T>
inline boost::mpl::true_* boost_foreach_is_lightweight_proxy(boost::iterator_range<T>*&, boost::foreach::tag)
{
    return 0;
}

template<typename T>
inline boost::mpl::true_* boost_foreach_is_lightweight_proxy(boost::sub_range<T>*&, boost::foreach::tag)
{
    return 0;
}

template<typename T>
inline boost::mpl::true_* boost_foreach_is_lightweight_proxy(T**&, boost::foreach::tag)
{
    return 0;
}
...
//   at the global namespace for your type.
template<typename T>
inline boost::foreach::is_noncopyable<T>* boost_foreach_is_noncopyable(T*&, BOOST_FOREACH_TAG_DEFAULT)
{
    return 0;
}
...
namespace boost {
    ...
    namespace foreach_detail_ {
        ...
        struct type2type : boost::mpl::if_<C, T const, T>
        {
        };
        ...
        struct is_char_array :
            mpl::and_<is_array<T>, mpl::or_<is_convertible<T, char const*>, is_convertible<T, wchar_t const*>>>
        {
        };
        ...
        struct foreach_iterator
        {
            ...
            typedef BOOST_DEDUCED_TYPENAME
                boost::mpl::eval_if<C, range_const_iterator<container>, range_mutable_iterator<container>>::type type;
        };
        ...
        struct foreach_reverse_iterator
        {
            ...
            typedef BOOST_DEDUCED_TYPENAME
                boost::mpl::eval_if<C, range_reverse_iterator<container const>, range_reverse_iterator<container>>::type
                    type;
        };

        template<typename T, typename C = boost::mpl::false_>
        struct foreach_reference : iterator_reference<BOOST_DEDUCED_TYPENAME foreach_iterator<T, C>::type>
        {
        };
        ...
        //
        template<typename T>
        inline type2type<T>* encode_type(T&, boost::false_type*)
        {
            return 0;
        }

        template<typename T>
        inline type2type<T, const_>* encode_type(T const&, boost::true_type*)
        {
            return 0;
        }
    ...
    // Borland needs a little extra help with arrays
    ...
    #if defined(BOOST_FOREACH_COMPILE_TIME_CONST_RVALUE_DETECTION) && !defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
            ///////////////////////////////////////////////////////////////////////////////
    ...
    #elif defined(BOOST_FOREACH_COMPILE_TIME_CONST_RVALUE_DETECTION) && defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
        ///////////////////////////////////////////////////////////////////////////////
        ...
        struct rvalue_probe
        {
            ...
            typedef BOOST_DEDUCED_TYPENAME
                boost::mpl::if_<boost::mpl::or_<boost::is_abstract<T>, boost::is_array<T>>, private_type_, T>::type
                    value_type;
        ...
        #define BOOST_FOREACH_IS_RVALUE(COL)                                          \
            boost::foreach_detail_::and_(                                             \
                ...
                (true ? 0                                                             \
                      : boost::foreach_detail_::is_rvalue_(                           \
                            (true ? boost::foreach_detail_::make_probe(COL) : (COL)), \
                            0)))
        ...
        // can't ever return an array or an abstract type by value
            ...
            typedef BOOST_DEDUCED_TYPENAME boost::mpl::if_<boost::is_array<T>, private_type_, T>::type value_type;
        #else
            typedef BOOST_DEDUCED_TYPENAME
                boost::mpl::if_<boost::mpl::or_<boost::is_abstract<T>, boost::is_array<T>>, private_type_, T>::type
                    value_type;
        ...
        private:
            enum size_type
            {
                size = sizeof(T) > sizeof(T*) ? sizeof(T) : sizeof(T*)
            };
        ...
        inline bool* should_copy_impl(boost::mpl::false_*, boost::mpl::false_*, bool* is_rvalue)
        {
            return is_rvalue;
        }
    ...
    // Cannot seem to get sunpro to handle addressof() with array types.
    ...
    #ifdef BOOST_FOREACH_RUN_TIME_CONST_RVALUE_DETECTION
        template<typename T>
        inline auto_any<simple_variant<T>> contain(T const& t, bool* rvalue)
        {
        ...
        {
            return auto_any<BOOST_DEDUCED_TYPENAME foreach_iterator<T, C>::type>(boost::end(auto_any_cast<T, C>(col)));
        }
    ...
    #ifndef BOOST_NO_FUNCTION_TEMPLATE_ORDERING
        template<typename T, typename C>
        inline auto_any<int> end(auto_any_t, type2type<T*, C>*, boost::mpl::true_*) // null-terminated C-style strings
        {
        ...
        //
        template<typename T, typename C>
        inline BOOST_DEDUCED_TYPENAME foreach_reference<T, C>::type deref(auto_any_t cur, type2type<T, C>*)
        {
            typedef BOOST_DEDUCED_TYPENAME foreach_iterator<T, C>::type iter_t;
        ...
        {
            return auto_any<reverse_iterator<T*>>(reverse_iterator<T*>(auto_any_cast<T*, boost::mpl::false_>(col)));
        }
    #endif
        ...
        //
        template<typename T, typename C>
        inline BOOST_DEDUCED_TYPENAME foreach_reference<T, C>::type rderef(auto_any_t cur, type2type<T, C>*)
        {
            typedef BOOST_DEDUCED_TYPENAME foreach_reverse_iterator<T, C>::type iter_t;
            return *auto_any_cast<iter_t, boost::mpl::false_>(cur);
        }
    ...
    #define BOOST_FOREACH_IS_LIGHTWEIGHT_PROXY(COL)                          \
        ...
        #define BOOST_FOREACH_SHOULD_COPY(COL) \
            (true                              \
                 ? BOOST_FOREACH_NULL          \
                 : boost::foreach_detail_::or_(BOOST_FOREACH_IS_RVALUE(COL), BOOST_FOREACH_IS_LIGHTWEIGHT_PROXY(COL)))
        ...
        #define BOOST_FOREACH_PREAMBLE()                             \
            ...
            if (bool BOOST_FOREACH_ID(_foreach_is_rvalue) = false) { \
            }                                                        \
            else
        ...
        #define BOOST_FOREACH_SHOULD_COPY(COL) (true ? BOOST_FOREACH_NULL : BOOST_FOREACH_IS_LIGHTWEIGHT_PROXY(COL))

    #endif

    #define BOOST_FOREACH_CONTAIN(COL) \
        boost::foreach_detail_::contain(BOOST_FOREACH_EVALUATE(COL), BOOST_FOREACH_SHOULD_COPY(COL))
    ...
    #define BOOST_FOREACH_NEXT(COL) \
        boost::foreach_detail_::next(BOOST_FOREACH_ID(_foreach_cur), BOOST_FOREACH_TYPEOF(COL))

    #define BOOST_FOREACH_DEREF(COL) \
        boost::foreach_detail_::deref(BOOST_FOREACH_ID(_foreach_cur), BOOST_FOREACH_TYPEOF(COL))
    ...
    #define BOOST_FOREACH_RNEXT(COL) \
        boost::foreach_detail_::rnext(BOOST_FOREACH_ID(_foreach_cur), BOOST_FOREACH_TYPEOF(COL))

    #define BOOST_FOREACH_RDEREF(COL) \
        boost::foreach_detail_::rderef(BOOST_FOREACH_ID(_foreach_cur), BOOST_FOREACH_TYPEOF(COL))
        ...
        if (boost::foreach_detail_::auto_any_t BOOST_FOREACH_ID(_foreach_col) = BOOST_FOREACH_CONTAIN(COL)) {    \
            ...
            for (bool BOOST_FOREACH_ID(_foreach_continue) = true;                                                \
                ...
                if (boost::foreach_detail_::set_false(BOOST_FOREACH_ID(_foreach_continue))) {                    \
                    ...
                    for (VAR = BOOST_FOREACH_DEREF(COL); !BOOST_FOREACH_ID(_foreach_continue);                   \
