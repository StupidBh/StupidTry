...
namespace boost {
    namespace typeindex {
        namespace detail {
            ...
            constexpr ctti_skip make_ctti_skip(
                std::size_t size_at_begin,
                std::size_t size_at_end,
            ...
            template<std::size_t N>
            constexpr ctti_skip
                make_ctti_skip(std::size_t size_at_begin, std::size_t size_at_end, const char (&until_runtime)[N])
            {
            ...
            constexpr ctti_skip skip() noexcept
            {
                return detail::make_ctti_skip BOOST_TYPE_INDEX_CTTI_USER_DEFINED_PARSING;
            }
            ...
            // sizeof("static const char *boost::detail::ctti<") - 1, sizeof("]") - 1, true, "???????????>::n() [T =
            // int"
            constexpr ctti_skip skip() noexcept
            {
                return detail::make_ctti_skip(39, 1, "T = ");
            }
            ...
            constexpr ctti_skip skip() noexcept
            {
                return detail::make_ctti_skip(39, 6, "");
            }
            ...
            constexpr ctti_skip skip() noexcept
            {
                return detail::make_ctti_skip(58, 1, "");
            }
#elif defined(__EDG__) && defined(BOOST_NO_CXX14_CONSTEXPR)
            ...
            constexpr ctti_skip skip() noexcept
            {
                return detail::make_ctti_skip(57, 1, "");
            }
#elif defined(__GNUC__) && (__GNUC__ < 7) && !defined(BOOST_NO_CXX14_CONSTEXPR)
            // sizeof("static constexpr char boost::detail::ctti<T>::s() [with unsigned int I = 0u; } T = ") - 1,
            // sizeof("]") - 1
            constexpr ctti_skip skip() noexcept
            {
                return detail::make_ctti_skip(81, 1, "");
            }
#elif defined(__GNUC__) && (__GNUC__ >= 7) && !defined(BOOST_NO_CXX14_CONSTEXPR)
            // sizeof("static constexpr char boost::detail::ctti<T>::s() [with unsigned int I = 0; } T = ") - 1,
            // sizeof("]") - 1
            constexpr ctti_skip skip() noexcept
            {
                return detail::make_ctti_skip(80, 1, "");
            }
            ...
            constexpr ctti_skip skip() noexcept
            {
                return detail::make_ctti_skip(0, 0, "");
            }
#endif
            ...
            {
#if !defined(BOOST_NO_CXX14_CONSTEXPR) && defined(BOOST_TYPE_INDEX_DETAIL_IS_CONSTANT) && \
    defined(BOOST_TYPE_INDEX_DETAIL_BUILTIN_STRCMP)
                if (boost::typeindex::detail::is_constant_string(v1) &&
                    boost::typeindex::detail::is_constant_string(v2)) {
            ...
            BOOST_CXX14_CONSTEXPR inline const char* skip_begining_runtime(const char* begin) noexcept
            {
                ...
                const char* const it = detail::constexpr_search(
                    begin,
                    begin + ArrayLength,
                    skip_value.until_runtime,
                    skip_value.until_runtime + skip_value.until_runtime_length);
            ...
            BOOST_CXX14_CONSTEXPR inline const char* skip_begining(const char* begin) noexcept
            {
                ...
                return skip().until_runtime_length ? detail::skip_begining_runtime<ArrayLength - skip().size_at_begin>(
            ...
            struct make_index_seq_impl
            {
                typedef typename make_index_sequence_join<
                    ...
                    typename make_index_seq_impl<C + D / 2, (D + 1) / 2>::type>::type type;
            };
...
namespace boost {
    namespace detail {
        ...
        struct ctti
        {
            ...
            // helper functions
            template<unsigned int I>
            constexpr static char s() noexcept
            { // step
                constexpr unsigned int offset = (I >= 10u ? 1u : 0u) + (I >= 100u ? 1u : 0u) + (I >= 1000u ? 1u : 0u) +
                                                (I >= 10000u ? 1u : 0u) + (I >= 100000u ? 1u : 0u) +
                                                (I >= 1'000'000u ? 1u : 0u);
    ...
    #elif defined(__PRETTY_FUNCTION__) || defined(__GNUC__) || (defined(__SUNPRO_CC) && (__SUNPRO_CC >= 0x5130)) ||  \
        (defined(__MWERKS__) && (__MWERKS__ >= 0x3000)) || (defined(__ICC) && (__ICC >= 600)) || defined(__ghs__) || \
        defined(__DMC__)
                ...
                boost::typeindex::detail::assert_compile_time_legths<(
                    size > boost::typeindex::detail::skip().size_at_begin +
                               boost::typeindex::detail::skip().size_at_end + sizeof("const *") - 1)>();
                static_assert(
                    !boost::typeindex::detail::skip().until_runtime_length,
                    "Skipping for GCC in C++14 mode is unsupported");

                typedef typename boost::typeindex::detail::make_index_seq_impl<
                    boost::typeindex::detail::skip().size_at_begin,
                    size - sizeof("const *") + 1 - boost::typeindex::detail::skip().size_at_begin>::type idx_seq;
                return impl(idx_seq());
            }
#else
            ...
            {
    #if defined(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE)
                return boost::typeindex::detail::skip_begining<sizeof(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE)>(
                    BOOST_TYPE_INDEX_FUNCTION_SIGNATURE);
    #elif defined(__FUNCSIG__)
                return boost::typeindex::detail::skip_begining<sizeof(__FUNCSIG__)>(__FUNCSIG__);
    #elif defined(__PRETTY_FUNCTION__) || defined(__GNUC__) || (defined(__SUNPRO_CC) && (__SUNPRO_CC >= 0x5130)) ||  \
        (defined(__MWERKS__) && (__MWERKS__ >= 0x3000)) || (defined(__ICC) && (__ICC >= 600)) || defined(__ghs__) || \
        defined(__DMC__) || defined(__clang__)
