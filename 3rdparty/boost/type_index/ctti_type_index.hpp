...
/// \brief Contains boost::typeindex::ctti_type_index class that is constexpr if C++14 constexpr is supported by
/// compiler.
///
...
namespace boost {
    namespace typeindex {
        ...
        namespace detail {
            ...
            //      1) we do not want to give user ability to manually construct and compare
            //      `struct-that-represents-type` 2) we need to distinguish between `struct-that-represents-type` and
            //      `const char*` 3) we need a thread-safe way to have references to instances
            //      `struct-that-represents-type` 4) we need a compile-time control to make sure that user does not copy
            //      or
        ...
        inline const detail::ctti_data& ctti_construct() noexcept
        {
            ...
            // requirements of T2 are no stricter than those of T1, or if either type is void. Converting a prvalue of
            // type "pointer to T1" to the type "pointer to T2" (where T1 and T2 are object types and where the
            // alignment requirements of T2 are no stricter than those of T1) and back to its original type yields the
