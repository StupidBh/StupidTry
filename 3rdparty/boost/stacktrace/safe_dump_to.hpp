...
namespace boost {
    namespace stacktrace {
        ...
        namespace detail {
            ...
            BOOST_STACKTRACE_FUNCTION std::size_t
                dump(const char* file, const native_frame_ptr_t* frames, std::size_t frames_count) noexcept;
#if defined(BOOST_WINDOWS)
            BOOST_STACKTRACE_FUNCTION std::size_t
                dump(void* fd, const native_frame_ptr_t* frames, std::size_t frames_count) noexcept;
#else
            // POSIX
            BOOST_STACKTRACE_FUNCTION std::size_t
                dump(int fd, const native_frame_ptr_t* frames, std::size_t frames_count) noexcept;
#endif

            struct this_thread_frames
            { // struct is required to avoid warning about usage of inline+BOOST_NOINLINE
                BOOST_NOINLINE BOOST_STACKTRACE_FUNCTION static std::size_t
                    collect(native_frame_ptr_t* out_frames, std::size_t max_frames_count, std::size_t skip) noexcept;

                BOOST_NOINLINE static std::size_t
                    safe_dump_to_impl(void* memory, std::size_t size, std::size_t skip) noexcept
                {
                    ...
                    const std::size_t frames_count = boost::stacktrace::detail::this_thread_frames::collect(
                        mem,
                        size / sizeof(native_frame_ptr_t) - 1,
                        skip + 1);
                    mem[frames_count] = 0;
                ...
                template<class T>
                BOOST_NOINLINE static std::size_t
                    safe_dump_to_impl(T file, std::size_t skip, std::size_t max_depth) noexcept
                {
                    using boost::stacktrace::detail::native_frame_ptr_t;
                    ...
                    const std::size_t frames_count =
                        boost::stacktrace::detail::this_thread_frames::collect(buffer, max_depth, skip + 1);
        ...
        ///
        /// @returns Stored call sequence depth including terminating zero frame. To get the actually consumed bytes
        /// multiply this value by the sizeof(boost::stacktrace::frame::native_frame_ptr_t)
        ///
        ...
        /// @brief Opens a file and rewrites its content with current function call sequence if such operations are
        /// async signal safe.
        ///
        /// @b Complexity: O(N) where N is call sequence length, O(1) if BOOST_STACKTRACE_USE_NOOP is defined.
        ///
        ...
        {
            return boost::stacktrace::detail::this_thread_frames::safe_dump_to_impl(
                file,
                0,
                boost::stacktrace::detail::max_frames_dump);
        }
        ...
        /// @brief Writes into the provided file descriptor the current function call sequence if such operation is
        /// async signal safe.
        ///
        /// @b Complexity: O(N) where N is call sequence length, O(1) if BOOST_STACKTRACE_USE_NOOP is defined.
        ///
        ...
        BOOST_FORCEINLINE std::size_t
            safe_dump_to(std::size_t skip, std::size_t max_depth, platform_specific_descriptor fd) noexcept;
...
#include <boost/stacktrace/detail/pop_options.h>
        ...
        #endif
        #if defined(BOOST_WINDOWS) && \
            !defined(BOOST_WINAPI_IS_MINGW) // MinGW does not provide RtlCaptureStackBackTrace. MinGW-w64 does.
