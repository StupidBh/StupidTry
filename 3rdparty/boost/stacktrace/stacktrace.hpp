...
extern "C"
{
    ...
    bool* boost_stacktrace_impl_ref_capture_stacktraces_at_throw();
        ...
        #pragma comment( \
            linker,      \
            "/ALTERNATENAME:_boost_stacktrace_impl_current_exception_stacktrace=_boost_stacktrace_impl_return_nullptr")
        #pragma comment( \
            linker,      \
            "/ALTERNATENAME:_boost_stacktrace_impl_ref_capture_stacktraces_at_throw=_boost_stacktrace_impl_return_nullptr")
    #else
        #pragma comment( \
            linker,      \
            "/ALTERNATENAME:boost_stacktrace_impl_current_exception_stacktrace=boost_stacktrace_impl_return_nullptr")
        #pragma comment( \
            linker,      \
            "/ALTERNATENAME:boost_stacktrace_impl_ref_capture_stacktraces_at_throw=boost_stacktrace_impl_return_nullptr")
    #endif
...
namespace boost {
    namespace stacktrace {
        ...
        namespace impl {
            ...
            BOOST_NOINLINE BOOST_SYMBOL_VISIBLE __attribute__((weak)) bool& ref_capture_stacktraces_at_throw() noexcept;

#endif

        } // namespace impl

        /// Class that on construction copies minimal information about call stack into its internals and provides
        ...
        class basic_stacktrace {
            ...
            static std::size_t frames_count_from_buffer_size(std::size_t buffer_size) noexcept
            {
                const std::size_t ret =
                    (buffer_size > sizeof(native_frame_ptr_t) ? buffer_size / sizeof(native_frame_ptr_t) : 0);
            ...
            BOOST_NOINLINE void init(std::size_t frames_to_skip, std::size_t max_depth)
            {
                ...
                BOOST_TRY
                {
                    { // Fast path without additional allocations
                        ...
                        const std::size_t frames_count = boost::stacktrace::detail::this_thread_frames::collect(
                            buffer,
                            buffer_size < max_depth ? buffer_size : max_depth,
                            frames_to_skip + 1);
...
#else
                    typedef typename std::allocator_traits<Allocator>::template rebind_alloc<native_frame_ptr_t>
                        allocator_void_t;
#endif
                    ...
                    do {
                        const std::size_t frames_count = boost::stacktrace::detail::this_thread_frames::collect(
                            &buf[0],
                            buf.size() < max_depth ? buf.size() : max_depth,
                            frames_to_skip + 1);
                        ...
                        buf.size() <
                        buf.max_size()); // close to `true`, but suppresses `C4127: conditional expression is constant`.
                }
                BOOST_CATCH(...)
                {
                    // ignore exception
        ...
        public:
            ...
            typedef typename std::vector<boost::stacktrace::frame, Allocator>::const_reverse_iterator
                const_reverse_iterator;

            /// @brief Stores the current function call sequence inside *this without any decoding or any other heavy
            /// platform specific operations.
            ///
            /// @b Complexity: O(N) where N is call sequence length, O(1) if BOOST_STACKTRACE_USE_NOOP is defined.
            ///
            /// @b Async-Handler-Safety: \asyncsafe if Allocator construction, copying, Allocator::allocate and
            /// Allocator::deallocate are async signal safe.
            ...
            /// @brief Stores [skip, skip + max_depth) of the current function call sequence inside *this without any
            /// decoding or any other heavy platform specific operations.
            ///
            /// @b Complexity: O(N) where N is call sequence length, O(1) if BOOST_STACKTRACE_USE_NOOP is defined.
            ///
            /// @b Async-Handler-Safety: \asyncsafe if Allocator construction, copying, Allocator::allocate and
            ...
            /// performed outside the constructor and exception in `allocator_type()` would not result in calling
            /// `std::terminate`.
            BOOST_FORCEINLINE basic_stacktrace(
                std::size_t skip,
            ...
            ///
            /// @b Async-Handler-Safety: \asyncsafe.
            const_reference operator[](std::size_t frame_no) const noexcept { return impl_[frame_no]; }
            ...
            const std::vector<boost::stacktrace::frame, Allocator>& as_vector() const noexcept { return impl_; }

            /// Constructs stacktrace from basic_istreamable that references the dumped stacktrace. Terminating zero
            /// frame is discarded.
            ///
            /// @b Complexity: O(N)
            template<class Char, class Trait>
            static basic_stacktrace
                from_dump(std::basic_istream<Char, Trait>& in, const allocator_type& a = allocator_type())
            {
            ...
            ///
            /// @param begin Beginning of the memory where the stacktrace was saved using the
            /// boost::stacktrace::safe_dump_to
            ///
            /// @param buffer_size_in_bytes Size of the memory. Usually the same value that was passed to the
            /// boost::stacktrace::safe_dump_to
            ///
            /// @b Complexity: O(size) in worst case
            static basic_stacktrace from_dump(
                const void* begin,
        ...
        /// Comparison operators that provide platform dependant ordering and have amortized O(1) complexity; O(size())
        /// worst case complexity; are Async-Handler-Safe.
        template<class Allocator1, class Allocator2>
        bool operator>(const basic_stacktrace<Allocator1>& lhs, const basic_stacktrace<Allocator2>& rhs) noexcept
        {
            return rhs < lhs;
        ...
        std::basic_ostream<CharT, TraitsT>&
            operator<<(std::basic_ostream<CharT, TraitsT>& os, const basic_stacktrace<Allocator>& bt)
        {
            return os << boost::stacktrace::to_string(bt);
        }

        /// This is the typedef to use unless you'd like to provide a specific allocator to
