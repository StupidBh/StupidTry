...
namespace boost {
    namespace stacktrace {
        namespace detail {
            ...
            inline void libbacktrace_syminfo_callback(
                void* data,
                uintptr_t /*pc*/,
                const char* symname,
                uintptr_t /*symval*/,
            ...
            inline int libbacktrace_full_callback(
                void* data,
                uintptr_t /*pc*/,
                const char* filename,
                int lineno,
            ...
            //
            // Currently `backtrace_create_state` can not detect file name on Windows
            // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=82543 That's why we provide a `prog_location` here.
            BOOST_SYMBOL_VISIBLE inline ::backtrace_state*
                construct_state(const program_location& prog_location) noexcept
            {
                // [dcl.inline]: A static local variable in an inline function with external linkage always refers to
                // the same object.
