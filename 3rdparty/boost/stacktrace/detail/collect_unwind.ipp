...
#if defined(BOOST_OS_IOS_AVAILABLE) && defined(BOOST_ARCH_ARM_AVAILABLE) && \
    BOOST_VERSION_NUMBER_MAJOR(BOOST_ARCH_ARM) < 8
...
#include <cstdio>
    ...
    #error \
        "Boost.Stacktrace requires `_Unwind_Backtrace` function. Define `_GNU_SOURCE` macro or `BOOST_STACKTRACE_GNU_SOURCE_NOT_REQUIRED` if _Unwind_Backtrace is available without `_GNU_SOURCE`."
#endif
    ...
    namespace stacktrace {
        namespace detail {
                ...
                std::size_t skip) noexcept
            {
                ...
                frames_count = static_cast<size_t>(
                    ::backtrace(const_cast<void**>(out_frames), static_cast<int>(max_frames_count)));

                // NOTE: There is no way to pass "skip" count to backtrace function so we need to perform left shift
                // operation. If number of elements in result backtrace is >= max_frames_count then "skip" elements are
                // wasted.
