...
namespace boost {
    namespace stacktrace {
        namespace detail {
            ...
            {
                return *path != '\0' && (*path == ':' || *path == '/' || is_abs_path(path + 1));
            }
            ...
            public:
                ...
                operator ::FILE*() const noexcept { return p; }
            ...
            inline std::string source_location(const void* addr, bool position_independent)
            {
                ...
                std::string source_line =
                    boost::stacktrace::detail::addr2line("-Cpe", reinterpret_cast<const void*>(offset));
            ...
            struct to_string_using_addr2line
            {
                ...
                void prepare_function_name(const void* addr) { res = boost::stacktrace::frame(addr).name(); }
                ...
                bool prepare_source_location(const void* addr)
                {
                    ...
                    //  - in pie binaries just passing an address to addr2line won't work (it needs an offset in this
                    //  case)
