...
#include <mutex>
    ...
    #if !defined(__MINGW32__) || (!defined(__clang__) && __GNUC__ < 12) || (defined(__clang__) && __clang_major__ < 16)
    ...
    namespace stacktrace {
        namespace detail {
            ...
            public:
                ...
                void** to_void_ptr_ptr() noexcept { return reinterpret_cast<void**>(&holder_); }
            ...
            class debugging_symbols : boost::noncopyable {
                static void try_init_com(com_holder<::IDebugSymbols>& idebug) noexcept
                {
                    ...
                    const bool res0 =
                        (S_OK == iclient->QueryInterface(__uuidof(IDebugControl), icontrol.to_void_ptr_ptr()));
                    if (!res0) {
                        return;
                    }

                    const bool res1 =
                        (S_OK == iclient->AttachProcess(
                                     0,
                                     ::GetCurrentProcessId(),
        ...
        #   error Your compiler does not support C++11 thread_local storage. It`s impossible to build with BOOST_STACKTRACE_USE_WINDBG_CACHED.
            ...
            public:
                ...
                std::string get_name_impl(const void* addr, std::string* module_name = 0) const
                {
                    ...
                    name[0] = '\0';
                    ULONG size = 0;
                    bool res = (S_OK == idebug_->GetNameByOffset(offset, name, sizeof(name), &size, 0));

                    if (!res && size != 0) {
                        result.resize(size);
                        res =
                            (S_OK ==
                             idebug_->GetNameByOffset(offset, &result[0], static_cast<ULONG>(result.size()), &size, 0));

                        // According to
                        // https://docs.microsoft.com/en-us/windows-hardware/drivers/ddi/dbgeng/nf-dbgeng-idebugsymbols-getnamebyoffset
                    ...
                    if (module_name) {
                        *module_name = result.substr(0, delimiter);
                        if (!module_name->empty()) {
                            ULONG64 base = 0;
                            res = (S_OK == idebug_->GetModuleByOffset(offset, 0, nullptr, &base));

                            if (res) {
                                name[0] = '\0';
                                size = 0;
                                res =
                                    (S_OK == idebug_->GetModuleNames(
                                                 DEBUG_ANY_ID,
                                                 base,
                                                 name,
                                                 sizeof(name),
                    ...
                    result = mingw_demangling_workaround(result.substr(delimiter + 1));
                ...
                std::size_t get_line_impl(const void* addr) const noexcept
                {
                    ...
                    const bool is_ok =
                        (S_OK == idebug_->GetLineByOffset(reinterpret_cast<ULONG64>(addr), &result, 0, 0, 0, 0));
                ...
                std::pair<std::string, std::size_t> get_source_file_line_impl(const void* addr) const
                {
                    ...
                    ULONG size = 0;
                    ULONG line_num = 0;
                    bool res = (S_OK == idebug_->GetLineByOffset(offset, &line_num, name, sizeof(name), &size, 0));
