        ...
        private:
            ...
            struct result_type
            {
                using type = typename std::conditional<
                    ...
                    defer_stored_unsigned_lazily>::type::template type<Min, Max>;
            };
        ...
        public:
            ...
            struct addition_result
            {
                ...
                using type = typename result_type<
                    temp_base_type,
                    rl.exception() ? std::numeric_limits<temp_base_type>::min() : static_cast<temp_base_type>(rl),
                    ru.exception() ? std::numeric_limits<temp_base_type>::max()
                                   : static_cast<temp_base_type>(ru)>::type;
            };
            ...
            struct division_result
            {
                ...
                {
                    if (u_interval.u < r_type(0) || u_interval.l > r_type(0)) {
...
// workaround some microsoft problem
       ...
       constexpr static r_type max(const r_type & t, const r_type & u){
            ...
            return static_cast<bool>(t < u) ? u : t;
                ...
                constexpr static r_interval_type union_interval(const r_interval_type& t, const r_interval_type& u)
                {
                ...
                constexpr static const r_interval_type r_interval = union_interval(t_interval, u_interval);
            ...
            {
                using temp_base_type =
                    typename std::conditional<std::numeric_limits<T>::is_signed, std::intmax_t, std::uintmax_t>::type;
            ...
            struct right_shift_result
            {
                ...
                constexpr static const r_interval_type u_interval { u_min.exception() ? r_type(0) : u_min,
                                                                    checked::cast<temp_base_type>(
            ...
            {
                using type = decltype(typename base_type<T>::type() & typename base_type<U>::type());
            };
