...
namespace boost {
    namespace safe_numerics {
        ...
        {
            switch (c.m_e) {
            case safe_numerics_error::success: std::terminate();
        ...
        struct sum_value_type
        {
            ...
            constexpr flag to_flag(const checked_result<T>& t) const
            {
                ...
                case safe_numerics_error::success: return known_value;
            ...
            constexpr operator std::uint8_t() const { return static_cast<std::uint8_t>(m_flag); }
        };

        // integers addition
        template<class T>
        typename std::enable_if<std::is_integral<T>::value, checked_result<T>>::type constexpr inline
            operator+(const checked_result<T>& t, const checked_result<T>& u)
        {
        ...
        template<class T>
        typename std::enable_if<std::is_integral<T>::value, checked_result<T>>::type constexpr inline
            operator+(const checked_result<T>& t)
        {
            return t;
        }
        ...
        struct product_value_type
        {
            ...
            constexpr flag to_flag(const checked_result<T>& t) const
            {
                ...
                case safe_numerics_error::success:
                    return (t < checked_result<T>(0))   ? less_than_zero
                           : (t > checked_result<T>(0)) ? greater_than_zero
                                                        : zero;
                case safe_numerics_error::negative_overflow_error:
                    // result is below representational minimum
                    return less_than_min;
            ...
            operator*(const checked_result<T>& t, const checked_result<T>& u)
        {
            ...
            case value_type::less_than_min    : return safe_numerics_error::negative_overflow_error;
            case value_type::zero             : return T(0);
            case value_type::greater_than_max : return safe_numerics_error::positive_overflow_error;
            case value_type::less_than_zero   :
            case value_type::greater_than_zero: return checked::multiply<T>(t, u);
            case value_type::indeterminate    : return safe_numerics_error::range_error;
            ...
            operator/(const checked_result<T>& t, const checked_result<T>& u)
        {
            ...
            case value_type::greater_than_max : return safe_numerics_error::positive_overflow_error;
            case value_type::less_than_zero   :
            case value_type::greater_than_zero: return checked::divide<T>(t, u);
            case value_type::indeterminate    : return safe_numerics_error::range_error;
            default                           : assert(false);
            }
            return checked_result<T>(0); // to suppress msvc warning
            ...
            operator%(const checked_result<T>& t, const checked_result<T>& u)
        {
            ...
            case value_type::zero             : return 0;
            case value_type::less_than_zero   :
            case value_type::greater_than_zero: return checked::modulus<T>(t, u);
            case value_type::indeterminate    : return safe_numerics_error::range_error;
            case value_type::t_value          : return t;
            case value_type::u_value          : return checked::subtract<T>(u, 1);
            case value_type::z_value          : return checked::subtract<T>(1, u);
        ...
        template<class T>
        constexpr boost::logic::tribool operator<(const checked_result<T>& t, const checked_result<T>& u)
        {
            using value_type = sum_value_type;
            constexpr const std::uint8_t order = static_cast<std::uint8_t>(value_type::count);
            ...
            case result_type::runtime      : return static_cast<const T&>(t) < static_cast<const T&>(u);
            case result_type::false_value  : return false;
            case result_type::true_value   : return true;
            case result_type::indeterminate: return boost::logic::indeterminate;
            default                        : assert(false);
            }
            return true;
        }
            ...
            operator>>(const checked_result<T>& t, const checked_result<T>& u)
        {
            ...
            if (1 == i) {
                ...
                return (r.m_e == safe_numerics_error::shift_too_large) ? checked_result<T>(0) : r;
            }
            else if (9 == i) {
                return safe_numerics_error::positive_overflow_error;
            }
        ...
        typename std::enable_if<std::is_integral<T>::value, checked_result<T>>::type constexpr inline
            operator|(const checked_result<T>& t, const checked_result<T>& u)
        {
            return t.exception() || u.exception() ? checked_result<T>(safe_numerics_error::range_error)
                                                  : checked::bitwise_or<T>(static_cast<T>(t), static_cast<T>(u));
        }
        ...
        {
            return t.exception() || u.exception() ? checked_result<T>(safe_numerics_error::range_error)
                                                  : checked::bitwise_xor<T>(static_cast<T>(t), static_cast<T>(u));
        }
        ...
        {
            return t.exception() || u.exception() ? checked_result<T>(safe_numerics_error::range_error)
                                                  : checked::bitwise_and<T>(static_cast<T>(t), static_cast<T>(u));
        }
...
namespace std {
    ...
    inline std::basic_ostream<CharT, Traits>&
        operator<<(std::basic_ostream<CharT, Traits>& os, const boost::safe_numerics::checked_result<R>& r)
    {
        bool e = r.exception();
        os << e;
        if (!e) {
    ...
    inline std::basic_ostream<CharT, Traits>&
        operator<<(std::basic_ostream<CharT, Traits>& os, const boost::safe_numerics::checked_result<signed char>& r)
    {
        bool e = r.exception();
        os << e;
        if (!e) {
    ...
    inline std::basic_istream<CharT, Traits>&
        operator>>(std::basic_istream<CharT, Traits>& is, boost::safe_numerics::checked_result<R>& r)
    {
        bool e;
        is >> e;
        if (!e) {
    ...
    template<typename CharT, typename Traits>
    inline std::basic_istream<CharT, Traits>&
        operator>>(std::basic_istream<CharT, Traits>& is, boost::safe_numerics::checked_result<signed char>& r)
    {
        bool e;
        is >> e;
        if (!e) {
...
namespace std {
    ...
    template<class R>
    class numeric_limits<boost::safe_numerics::checked_result<R>> : public std::numeric_limits<R> {
        using this_type = boost::safe_numerics::checked_result<R>;

    public:
        constexpr static this_type min() noexcept { return this_type(std::numeric_limits<R>::min()); }

        constexpr static this_type max() noexcept { return this_type(std::numeric_limits<R>::max()); }
    };
