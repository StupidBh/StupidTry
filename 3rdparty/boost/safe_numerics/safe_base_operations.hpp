...
namespace boost {
    namespace safe_numerics {
        ...
        template<class T>
        constexpr inline Stored safe_base<Stored, Min, Max, P, E>::validated_cast(const T& t) const
        {
        ...
        constexpr inline /*explicit*/ safe_base<Stored, Min, Max, P, E>::safe_base()
        {
            ...
            dispatch<E, safe_numerics_error::uninitialized_value>("safe values must be initialized");
        }

        // construct an instance of a safe type from an instance of a convertible underlying type.
        template<class Stored, Stored Min, Stored Max, class P, class E>
        constexpr inline /*explicit*/ safe_base<Stored, Min, Max, P, E>::safe_base(const Stored& rhs, skip_validation) :
            m_t(rhs)
        {
            static_assert(
                std::is_arithmetic<Stored>(),
        ...
        template<class Stored, Stored Min, Stored Max, class P, class E>
        template<class T, typename std::enable_if<std::is_convertible<T, Stored>::value, bool>::type>
        ...
        template<class Stored, Stored Min, Stored Max, class P, class E>
        template<class R, typename std::enable_if<!boost::safe_numerics::is_safe<R>::value, int>::type>
        constexpr inline safe_base<Stored, Min, Max, P, E>::operator R() const
        {
            ...
            static_assert(!r_interval.excludes(this_interval), "safe type cannot be constructed with this type");
            return validate_detail<R, std::numeric_limits<R>::min(), std::numeric_limits<R>::max(), E>::return_value(
                m_t);
        }
        ...
        {
            static_assert(is_safe<T>::value || is_safe<U>::value, "at least one type must be a safe type");
            ...
            static_assert(
                std::is_same<t_exception_policy, u_exception_policy>::value ||
                    std::is_same<t_exception_policy, void>::value || std::is_same<void, u_exception_policy>::value,
        ...
        struct common_promotion_policy
        {
            ...
            static_assert(
                std::is_same<t_promotion_policy, u_promotion_policy>::value ||
                    std::is_same<t_promotion_policy, void>::value || std::is_same<void, u_promotion_policy>::value,
        ...
        namespace {
            ...
            using legal_overload = boost::mp11::mp_and<
                boost::mp11::mp_or<is_safe<T>, is_safe<U>>,
                boost::mp11::mp_valid<F, typename base_type<T>::type, typename base_type<U>::type>>;
        } // namespace
        ...
        private:
            ...
            using result_base_type = typename promotion_policy::template addition_result<T, U>::type;

            // if exception not possible
            constexpr static result_base_type return_value(const T& t, const U& u, std::false_type)
            {
                return static_cast<result_base_type>(base_value(t)) + static_cast<result_base_type>(base_value(u));
            }
            ...
            constexpr static result_base_type return_value(const T& t, const U& u, std::true_type)
            {
                const std::pair<result_base_type, result_base_type> r =
                    casting_helper<exception_policy, result_base_type>(t, u);

                const r_type rx =
                    checked_operation<result_base_type, dispatch_and_return<exception_policy, result_base_type>>::add(
                        r.first,
                        r.second);
            ...
            constexpr static const interval<result_base_type> return_interval {
                r_type_interval.l.exception() ? std::numeric_limits<result_base_type>::min()
                                              : static_cast<result_base_type>(r_type_interval.l),
                r_type_interval.u.exception() ? std::numeric_limits<result_base_type>::max()
        ...
        public:
            using type = safe_base<result_base_type, rl, ru, promotion_policy, exception_policy>;

            constexpr static type return_value(const T& t, const U& u)
            {
                return type(
                    return_value(t, u, std::integral_constant<bool, exception_possible()>()),
                    typename type::skip_validation());
            }
        };

        template<class T, class U>
        using addition_operator = decltype(std::declval<T const&>() + std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<legal_overload<addition_operator, T, U>::value, addition_result<T, U>>::
            type constexpr inline
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<addition_operator, T, U>::value, T>::type constexpr inline
        ...
        private:
            using promotion_policy = typename common_promotion_policy<T, U>::type;
            using result_base_type = typename promotion_policy::template subtraction_result<T, U>::type;
            ...
            constexpr static result_base_type return_value(const T& t, const U& u, std::true_type)
            {
                ...
                const r_type rx =
                    checked_operation<result_base_type, dispatch_and_return<exception_policy, result_base_type>>::
        ...
        template<class T, class U>
        using subtraction_operator = decltype(std::declval<T const&>() - std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<legal_overload<subtraction_operator, T, U>::value, subtraction_result<T, U>>::
            type constexpr inline
            operator-(const T& t, const U& u)
        {
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<subtraction_operator, T, U>::value, T>::type constexpr inline
            operator-=(T& t, const U& u)
        {
            t = static_cast<T>(t - u);
            return t;
        ...
        private:
            using promotion_policy = typename common_promotion_policy<T, U>::type;
            using result_base_type = typename promotion_policy::template multiplication_result<T, U>::type;
        ...
        template<class T, class U>
        using multiplication_operator = decltype(std::declval<T const&>() * std::declval<U const&>());
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<multiplication_operator, T, U>::value, T>::type constexpr inline
            operator*=(T& t, const U& u)
        {
            t = static_cast<T>(t * u);
            return t;
        ...
        private:
            ...
            constexpr static const int bits = std::min(
                ...
                std::max(
                    std::initializer_list<int> { std::numeric_limits<result_base_type>::digits,
            ...
            constexpr static result_base_type return_value(const T& t, const U& u, std::true_type)
            {
                ...
                const std::pair<t_type, t_type> r = casting_helper<exception_policy, temp_base>(t, u);

                const t_type rx =
                    checked_operation<temp_base, dispatch_and_return<exception_policy, temp_base>>::divide(
                        r.first,
                        r.second);
            ...
            {
                return r_type_interval_t { checked::cast<result_base_type>(base_value(std::numeric_limits<T>::min())),
                                           checked::cast<result_base_type>(base_value(std::numeric_limits<T>::max())) };
            };

            constexpr static r_type_interval_t u_interval()
            {
                return r_type_interval_t { checked::cast<result_base_type>(base_value(std::numeric_limits<U>::min())),
                                           checked::cast<result_base_type>(base_value(std::numeric_limits<U>::max())) };
            };
            ...
            constexpr static bool exception_possible()
            {
                ...
                return static_cast<bool>(ui.includes(r_type(0))) || ri.l.exception() || ri.u.exception();
            }
        ...
        template<class T, class U>
        using division_operator = decltype(std::declval<T const&>() / std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<legal_overload<division_operator, T, U>::value, division_result<T, U>>::
            type constexpr inline
            operator/(const T& t, const U& u)
        {
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<division_operator, T, U>::value, T>::type constexpr inline
            operator/=(T& t, const U& u)
        {
            t = static_cast<T>(t / u);
            return t;
        ...
        private:
            ...
            constexpr static result_base_type return_value(const T& t, const U& u, std::true_type)
            {
                ...
                const t_type rx =
                    checked_operation<temp_base, dispatch_and_return<exception_policy, temp_base>>::modulus(
                        r.first,
                        r.second);
        ...
        template<class T, class U>
        using modulus_operator = decltype(std::declval<T const&>() % std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<legal_overload<modulus_operator, T, U>::value, modulus_result<T, U>>::
            type constexpr inline
            operator%(const T& t, const U& u)
        {
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<modulus_operator, T, U>::value, T>::type constexpr inline
            operator%=(T& t, const U& u)
        {
            t = static_cast<T>(t % u);
            return t;
        ...
        private:
            ...
            using result_base_type = typename promotion_policy::template comparison_result<T, U>::type;
        ...
        public:
            constexpr static bool return_value(const T& t, const U& u)
            {
                ...
                constexpr bool exception_possible = interval_open(t_interval) || interval_open(u_interval);

                return return_value(t, u, std::integral_constant<bool, exception_possible>());
            }
        };

        template<class T, class U>
        using less_than_operator = decltype(std::declval<T const&>() < std::declval<U const&>());
        template<class T, class U>
        using greater_than_operator = decltype(std::declval<T const&>() > std::declval<U const&>());
        template<class T, class U>
        using less_than_or_equal_operator = decltype(std::declval<T const&>() <= std::declval<U const&>());
        template<class T, class U>
        using greater_than_or_equal_operator = decltype(std::declval<T const&>() >= std::declval<U const&>());

        template<class T, class U>
        typename std::enable_if<legal_overload<less_than_operator, T, U>::value, bool>::type constexpr inline
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<greater_than_operator, T, U>::value, bool>::type constexpr inline
            operator>(const T& lhs, const U& rhs)
        {
            return rhs < lhs;
        }
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<greater_than_or_equal_operator, T, U>::value, bool>::
            type constexpr inline
            operator>=(const T& lhs, const U& rhs)
        {
            return !(lhs < rhs);
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<less_than_or_equal_operator, T, U>::value, bool>::type constexpr inline
            operator<=(const T& lhs, const U& rhs)
        {
            return !(lhs > rhs);
        }
        ...
        private:
            ...
            constexpr static bool interval_open(const r_type_interval& t) { return t.l.exception() || t.u.exception(); }
        ...
        template<class T, class U>
        using equal_to_operator = decltype(std::declval<T const&>() == std::declval<U const&>());
        template<class T, class U>
        using not_equal_to_operator = decltype(std::declval<T const&>() != std::declval<U const&>());

        template<class T, class U>
        typename std::enable_if<legal_overload<equal_to_operator, T, U>::value, bool>::type constexpr inline
            operator==(const T& lhs, const U& rhs)
        {
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<not_equal_to_operator, T, U>::value, bool>::type constexpr inline
            operator!=(const T& lhs, const U& rhs)
        {
            return !(lhs == rhs);
        }
        ...
        private:
            using promotion_policy = typename common_promotion_policy<T, U>::type;
            using result_base_type = typename promotion_policy::template left_shift_result<T, U>::type;
        ...
        template<class T, class U>
        using left_shift_operator = decltype(std::declval<T const&>() << std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<
            // exclude usage of << for file input here
            boost::safe_numerics::Numeric<T>() && legal_overload<left_shift_operator, T, U>::value,
        ...
        {
            using promotion_policy = typename common_promotion_policy<T, U>::type;
            using result_base_type = typename promotion_policy::template right_shift_result<T, U>::type;
            ...
            constexpr static bool exception_possible()
            {
                ...
                return static_cast<bool>(
                    ...
                    ui.u > checked_result<result_base_type>(std::numeric_limits<result_base_type>::digits) ||
                    ti.l < checked_result<result_base_type>(0) || ui.l < checked_result<result_base_type>(0) ||
                    ri.l.exception() || ri.u.exception());
            }
        ...
        template<class T, class U>
        using right_shift_operator = decltype(std::declval<T const&>() >> std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<
            // exclude usage of >> for file input here
            boost::safe_numerics::Numeric<T>() && legal_overload<right_shift_operator, T, U>::value,
        ...
        private:
            using promotion_policy = typename common_promotion_policy<T, U>::type;
            using result_base_type = typename promotion_policy::template bitwise_or_result<T, U>::type;
        ...
        public:
            ...
            {
                return type(
                    static_cast<result_base_type>(base_value(t)) | static_cast<result_base_type>(base_value(u)),
                    typename type::skip_validation());
            }
        };

        template<class T, class U>
        using bitwise_or_operator = decltype(std::declval<T const&>() | std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<legal_overload<bitwise_or_operator, T, U>::value, bitwise_or_result<T, U>>::
            type constexpr inline
            operator|(const T& t, const U& u)
        {
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<bitwise_or_operator, T, U>::value, T>::type constexpr inline
            operator|=(T& t, const U& u)
        {
            t = static_cast<T>(t | u);
            return t;
        ...
        private:
            using promotion_policy = typename common_promotion_policy<T, U>::type;
            using result_base_type = typename promotion_policy::template bitwise_and_result<T, U>::type;
        ...
        template<class T, class U>
        using bitwise_and_operator = decltype(std::declval<T const&>() & std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<legal_overload<bitwise_and_operator, T, U>::value, bitwise_and_result<T, U>>::
            type constexpr inline
            operator&(const T& t, const U& u)
        {
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<bitwise_and_operator, T, U>::value, T>::type constexpr inline
            operator&=(T& t, const U& u)
        {
            t = static_cast<T>(t & u);
            return t;
        ...
        {
            using promotion_policy = typename common_promotion_policy<T, U>::type;
            using result_base_type = typename promotion_policy::template bitwise_xor_result<T, U>::type;
        ...
        template<class T, class U>
        using bitwise_xor_operator = decltype(std::declval<T const&>() ^ std::declval<U const&>());

        template<class T, class U>
        typename boost::lazy_enable_if_c<legal_overload<bitwise_xor_operator, T, U>::value, bitwise_xor_result<T, U>>::
            type constexpr inline
            operator^(const T& t, const U& u)
        {
        ...
        template<class T, class U>
        typename std::enable_if<legal_overload<bitwise_xor_operator, T, U>::value, T>::type constexpr inline
            operator^=(T& t, const U& u)
        {
            t = static_cast<T>(t ^ u);
            return t;
        ...
        inline void safe_base<T, Min, Max, P, E>::output(std::basic_ostream<CharT, Traits>& os) const
        {
            os
        ...
        template<
            ...
            >
        template<class CharT, class Traits>
        inline void safe_base<T, Min, Max, P, E>::input(std::basic_istream<CharT, Traits>& is)
        {
            if (std::is_same<T, signed char>::value || std::is_same<T, unsigned char>::value ||
                ...
                is >> m_t;
                if (is.fail()) {
                    boost::safe_numerics::dispatch<E, boost::safe_numerics::safe_numerics_error::domain_error>(
                        "error in file input");
                }
                else {
                    validated_cast(m_t);
