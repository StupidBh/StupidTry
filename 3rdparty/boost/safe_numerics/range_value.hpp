...
std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& os, const range_value<T>& t)
{
    return os << boost::safe_numerics::make_interval<T>() << t.m_t;
}
...
inline std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& os, const result_display<T>& r)
{
