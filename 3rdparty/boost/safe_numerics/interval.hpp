...
namespace boost {
    namespace safe_numerics {
        ...
        struct interval
        {
            ...
            constexpr tribool includes(const interval<R>& t) const { return u >= t.u && l <= t.l; }
            ...
            // otherwise
            //  return false or indeterminate
            constexpr tribool excludes(const interval<R>& t) const { return t.u < l || u < t.l; }
        };
        ...
        {
            // adapted from https://en.wikipedia.org/wiki/Interval_arithmetic
            return utility::minmax<T>(std::initializer_list<T> { t.l * u.l, t.l * u.u, t.u * u.l, t.u * u.u });
        }
        ...
        template<typename T>
        constexpr inline boost::logic::tribool operator<(const interval<T>& t, const interval<T>& u)
        {
            return
...
namespace std {
    ...
    inline std::basic_ostream<CharT, Traits>&
        operator<<(std::basic_ostream<CharT, Traits>& os, const boost::safe_numerics::interval<T>& i)
    {
        return os << '[' << i.l << ',' << i.u << ']';
    }
    ...
    inline std::basic_ostream<CharT, Traits>&
        operator<<(std::basic_ostream<CharT, Traits>& os, const boost::safe_numerics::interval<unsigned char>& i)
    {
    ...
    template<typename CharT, typename Traits>
    inline std::basic_ostream<CharT, Traits>&
        operator<<(std::basic_ostream<CharT, Traits>& os, const boost::safe_numerics::interval<signed char>& i)
    {
        os << "[" << (int)i.l << "," << (int)i.u << "]";
        return os;
    }
