...
namespace boost {
    namespace safe_numerics {
        ...
        template<class T, T Min, T Max, class P, class E>
        constexpr T base_value(const safe_base<T, Min, Max, P, E>& st)
        {
    ...
    #pragma GCC diagnostic ignored "-Wmismatched-tags"
        ...
        private:
            ...
            friend std::basic_ostream<CharT, Traits>&
                operator<<(std::basic_ostream<CharT, Traits>& os, const safe_base& t)
            {
                t.output(os);
                return os;
            }
            ...
            friend inline std::basic_istream<CharT, Traits>&
                operator>>(std::basic_istream<CharT, Traits>& is, safe_base& t)
            {
                t.input(is);
                return is;
            }
        ...
        public:
            ...
            template<class T, typename std::enable_if<std::is_convertible<T, Stored>::value, bool>::type = 0>
...
namespace std {
    ...
    template<class T, T Min, T Max, class P, class E>
    class numeric_limits<boost::safe_numerics::safe_base<T, Min, Max, P, E>> : public std::numeric_limits<T> {
        using SB = boost::safe_numerics::safe_base<T, Min, Max, P, E>;

    public:
        constexpr static SB lowest() noexcept { return SB(Min, typename SB::skip_validation()); }

        constexpr static SB min() noexcept { return SB(Min, typename SB::skip_validation()); }

        constexpr static SB max() noexcept { return SB(Max, typename SB::skip_validation()); }
    };
