    ...
    namespace safe_numerics {
        namespace utility {
            ...
            */
            ...
            template<typename T>
            using bits_type = std::
                integral_constant<int, std::numeric_limits<T>::digits + (std::numeric_limits<T>::is_signed ? 1 : 0)>;
            ...
            */
            ...
            namespace ilog2_detail {
                ...
                {
#define LT(n) n, n, n, n, n, n, n, n, n, n, n, n, n, n, n, n
                    const char LogTable256[256] = { static_cast<char>(-1),
                                                    0,
                                                    1,
                                                    1,
                                                    2,
            ...
            constexpr inline unsigned int ilog2(const T& t)
            {
                ...
                return ilog2_detail::ilog2<bits_type<T>::value>(
                    static_cast<typename boost::uint_t<bits_type<T>::value>::least>(t));
            }
            ...
            */
            ...
            using signed_stored_type =
                typename boost::int_t<max(significant_bits(Min), significant_bits(Max)) + 1>::least;

            // given an unsigned range, return type required to hold all the values
            // in the range
            template<std::uintmax_t Min, std::uintmax_t Max>
            // unsigned range
            using unsigned_stored_type = typename boost::uint_t<significant_bits(Max)>::least;
            ...
            template<typename T>
            constexpr inline std::pair<T, T> minmax(const std::initializer_list<T>& l)
            {
                assert(l.size() > 0);
            ...
            constexpr inline T round_out(const T& t)
            {
                if (t >= 0) {
                    ...
                    return (sb < sizeof(T) * 8) ? ((T)1 << sb) - 1 : std::numeric_limits<T>::max();
                }
                else {
                    const std::uint8_t sb = utility::significant_bits(~t);
                    return (sb < sizeof(T) * 8) ? ~(((T)1 << sb) - 1) : std::numeric_limits<T>::min();
                }
            }
