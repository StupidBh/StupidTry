...
namespace boost {
    namespace safe_numerics {
        ...
        template<typename R, R Min, R Max, typename T, class F>
        struct heterogeneous_checked_operation<
            ...
            T,
            F,
            typename std::enable_if<std::is_integral<R>::value && std::is_integral<T>::value>::type>
        {
            ////////////////////////////////////////////////////
            ...
            {
                constexpr static checked_result<R> cast_impl(
                    const T& t,
                ...
                )
                {
                    ...
                    return boost::safe_numerics::safe_compare::greater_than(t, Max)
                ...
                {
                    return boost::safe_numerics::safe_compare::less_than(t, Min)
            ...
            {
                return cast_impl_detail::cast_impl(t, std::is_signed<R>(), std::is_signed<T>());
            }
        ...
        struct heterogeneous_checked_operation<
            ...
            T,
            F,
            typename std::enable_if<std::is_integral<R>::value && std::is_floating_point<T>::value>::type>
        {
            constexpr static checked_result<R> cast(const T& t) { return static_cast<R>(t); }
        }; // heterogeneous_checked_operation
        ...
        struct heterogeneous_checked_operation<
            ...
            T,
            F,
            typename std::enable_if<std::is_floating_point<R>::value && std::is_integral<T>::value>::type>
        {
            constexpr static checked_result<R> cast(const T& t)
            {
                if (std::numeric_limits<R>::digits < std::numeric_limits<T>::digits) {
                    if (utility::significant_bits(t) > std::numeric_limits<R>::digits) {
                        return F::invoke(safe_numerics_error::precision_overflow_error, "keep precision");
                    }
                }
                return t;
            }
        ...
        struct checked_operation<R, F, typename std::enable_if<std::is_integral<R>::value>::type>
        {
            ////////////////////////////////////////////////////
            ...
            constexpr static checked_result<R> add(const R& t, const R& u)
            {
            ...
            struct subtract_impl_detail
            {
                ...
                {
                    // INT30-C. Ensure that unsigned integer operations do not wrap
                    return t < u ? F::template invoke<safe_numerics_error::negative_overflow_error>(
            ...
            struct multiply_impl_detail
            {
                ...
                )
                {
                    ...
                    return static_cast<i_type>(t) * static_cast<i_type>(u) > std::numeric_limits<R>::max()
                               ? F::template invoke<safe_numerics_error::positive_overflow_error>(
                ...
                {
                    // INT30-C
                    return u > 0 && t > std::numeric_limits<R>::max() / u
                               ? F::template invoke<safe_numerics_error::positive_overflow_error>(
                                     "multiplication overflow")
                               : checked_result<R>(t * u);
                }
                ...
                )
                {
                    ...
                    return (static_cast<i_type>(t) * static_cast<i_type>(u) >
                ...
                {                   // INT32-C
                    return t > 0 ? u > 0 ? t > std::numeric_limits<R>::max() / u
                                               ? F::template invoke<safe_numerics_error::positive_overflow_error>(
                                                     "multiplication overflow")
                                               : checked_result<R>(t * u)
                                         : // u <= 0
            ...
            constexpr static checked_result<R> multiply(const R& t, const R& u)
            {
                return multiply_impl_detail::multiply(
                    ...
                    u,
                    std::is_signed<R>(),
                    std::integral_constant<bool, (sizeof(R) > sizeof(std::uintmax_t) / 2)>());
            }
            ...
            struct divide_impl_detail
            {
                ...
                {
                    return (u == -1 && t == std::numeric_limits<R>::min())
                               ? F::template invoke<safe_numerics_error::positive_overflow_error>(
            ...
            {
                if (u == 0) {
                    return F::template invoke<safe_numerics_error::domain_error>("divide by zero");
                }
            ...
            {
                if (0 == u) {
                    return F::template invoke<safe_numerics_error::domain_error>("denominator is zero");
                }
        ...
        constexpr unsigned int leading_zeros(const T & t){
            if(0 == t)
                ...
                )
                {
                    ...
                    if (safe_compare::greater_than(u, std::numeric_limits<R>::digits - utility::significant_bits(t))) {
                ...
                )
                {
                    ...
                    return F::template invoke<safe_numerics_error::negative_shift>("shifting a negative value");
                }

            }; // left_shift_integer_detail

            constexpr static checked_result<R> left_shift(const R& t, const R& u)
            {
                ...
                if (u < 0) {
                    return F::template invoke<safe_numerics_error::negative_shift>("shifting negative amount");
                }
            ...
            constexpr static checked_result<R> right_shift(const R& t, const R& u)
            {
                // INT34-C - Do not shift an expression by a negative number of bits
            ...
            constexpr static checked_result<R> bitwise_or(const R& t, const R& u)
            {
                ...
                const unsigned int result_size = std::max(significant_bits(t), significant_bits(u));
            ...
            {
                using namespace boost::safe_numerics::utility;
                const unsigned int result_size = std::min(significant_bits(t), significant_bits(u));
