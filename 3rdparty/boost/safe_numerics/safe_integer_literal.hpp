...
namespace boost {
    namespace safe_numerics {
        ...
        template<typename T, T N, class P, class E>
        constexpr T base_value(const safe_literal_impl<T, N, P, E>&)
        {
            return N;
        }
        ...
        inline std::basic_ostream<CharT, Traits>&
            operator<<(std::basic_ostream<CharT, Traits>& os, const safe_literal_impl<T, N, P, E>&)
        {
            return os
        ...
        class safe_literal_impl {
            ...
            friend std::basic_ostream<CharT, Traits>&
                operator<<(std::basic_ostream<CharT, Traits>& os, const safe_literal_impl&)
            {
                return os
                       << ((::std::is_same<T, signed char>::value || ::std::is_same<T, unsigned char>::value ||
        ...
        public:
            ...
            template<class R, typename std::enable_if<!boost::safe_numerics::is_safe<R>::value, int>::type = 0>
            constexpr operator R() const
            {
// if static values don't overlap, the program can never function
#if 1
                constexpr const interval<R> r_interval;
                static_assert(!r_interval.excludes(N), "safe type cannot be constructed with this type");
#endif

                return validate_detail<R, std::numeric_limits<R>::min(), std::numeric_limits<R>::max(), E>::
                    return_value(*this);
            }
            ...
            */
            template<typename Tx, Tx Nx, typename = std::enable_if_t<!checked::minus(Nx).exception()>>
            ...
            */
            template<typename Tx, Tx Nx, typename = std::enable_if_t<!checked::bitwise_not(Nx).exception()>>
        ...
        template<std::intmax_t N, class P = void, class E = void>
        using safe_signed_literal = safe_literal_impl<typename utility::signed_stored_type<N, N>, N, P, E>;

        template<std::uintmax_t N, class P = void, class E = void>
        using safe_unsigned_literal = safe_literal_impl<typename utility::unsigned_stored_type<N, N>, N, P, E>;
...
#define make_safe_literal(n, P, E) boost::safe_numerics::make_safe_literal_impl<decltype(n), n, P, E>()
...
namespace std {
    ...
    template<typename T, T N, class P, class E>
    class numeric_limits<boost::safe_numerics::safe_literal_impl<T, N, P, E>> : public std::numeric_limits<T> {
        using SL = boost::safe_numerics::safe_literal_impl<T, N, P, E>;

    public:
        constexpr static SL lowest() noexcept { return SL(); }

        constexpr static SL min() noexcept { return SL(); }

        constexpr static SL max() noexcept { return SL(); }
    };
