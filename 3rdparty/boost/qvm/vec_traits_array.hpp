...
namespace boost {
    namespace qvm {
        ...
        struct vec_traits<std::array<std::array<T, M>, N>>
        {
        ...
        struct vec_traits<std::array<T, Dim>>
        {
        ...
        template<class T, std::size_t Dim>
        struct vec_traits<std::array<T, Dim> const>
        {
            ...
            template<int I>
            static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL scalar_type read_element(this_vector& x)
            {
                BOOST_QVM_STATIC_ASSERT(I >= 0);
                BOOST_QVM_STATIC_ASSERT(I < int(Dim));
                return x[I];
            ...
            static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL scalar_type read_element_idx(int i, this_vector& x)
            {
                BOOST_QVM_ASSERT(i >= 0);
                BOOST_QVM_ASSERT(i < int(Dim));
                return x[i];
        ...
        struct deduce_vec<std::array<T, Dim>, D>
        {
            typedef vec<T, D> type;
        };

        template<class T, std::size_t Dim, int D>
        struct deduce_vec<std::array<T, Dim> const, D>
        {
            typedef vec<T, D> type;
        };

        template<class T1, class T2, std::size_t Dim, int D>
        struct deduce_vec2<std::array<T1, Dim>, std::array<T2, Dim>, D>
        {
            typedef vec<typename deduce_scalar<T1, T2>::type, D> type;
        };

        template<class T1, class T2, std::size_t Dim, int D>
        struct deduce_vec2<std::array<T1, Dim> const, std::array<T2, Dim>, D>
        {
            typedef vec<typename deduce_scalar<T1, T2>::type, D> type;
        };

        template<class T1, class T2, std::size_t Dim, int D>
        struct deduce_vec2<std::array<T1, Dim>, std::array<T2, Dim> const, D>
        {
            typedef vec<typename deduce_scalar<T1, T2>::type, D> type;
        };

        template<class T1, class T2, std::size_t Dim, int D>
        struct deduce_vec2<std::array<T1, Dim> const, std::array<T2, Dim> const, D>
        {
            typedef vec<typename deduce_scalar<T1, T2>::type, D> type;
        };
...
namespace boost {
    namespace qvm {
        ...
        struct deduce_vec<T[Dim], D>
        {
            typedef vec<T, D> type;
        };

        template<class T, int Dim, int D>
        struct deduce_vec<T const[Dim], D>
        {
            typedef vec<T, D> type;
        };

        template<class T1, class T2, int Dim, int D>
        struct deduce_vec2<T1[Dim], T2[Dim], D>
        {
            typedef vec<typename deduce_scalar<T1, T2>::type, D> type;
        };
