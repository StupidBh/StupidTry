...
namespace boost {
    namespace qvm {
        ...
        struct is_mat
        {
            static bool const value = is_scalar<typename mat_traits<T>::scalar_type>::value &&
                                      mat_traits<T>::rows > 0 && mat_traits<T>::cols > 0;
        };

        namespace qvm_detail {
            template<class T, T>
            struct mtr_dispatch_yes
            {
                char x, y;
            };
        }
        ...
        template<class T>
        class mat_write_element_ref {
            template<class U>
            static qvm_detail::mtr_dispatch_yes<
                typename mat_traits<U>::scalar_type& (*)(U&),
                &mat_traits<U>::template write_element<0, 0>>
                check(int);
        ...
        BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL typename enable_if_c<mat_write_element_ref<M>::value, void>::type
