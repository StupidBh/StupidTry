...
namespace boost {
    namespace qvm {
        namespace qvm_detail {
            ...
            {
                template<int I>
                static ScalarType read_element(VecType const& v);

                template<int I, int N>
                struct read_element_idx_detail
                {
                    static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL ScalarType impl(int const i, VecType const& v)
                    {
                        return I == i ? read_element<I>(v) : read_element_idx_detail<I + 1, N>::impl(i, v);
                    }
                };

                template<int N>
                struct read_element_idx_detail<N, N>
                {
                ...
                static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL ScalarType
                    read_element_idx(int const i, VecType const& v)
                {
            ...
            {
                template<int I>
                static ScalarType& write_element(VecType& v);

                template<int I, int N>
                struct write_element_idx_detail
                {
                    static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL ScalarType& impl(int const i, VecType& v)
                    {
                        return I == i ? write_element<I>(v) : write_element_idx_detail<I + 1, N>::impl(i, v);
                    }
                };

                template<int N>
                struct write_element_idx_detail<N, N>
                {
                ...
                static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL ScalarType&
                    write_element_idx(int const i, VecType& v)
                {
    ...
    namespace boost {                                                                               \
        namespace qvm {                                                                             \
            namespace qvm_detail {                                                                  \
                template<>                                                                          \
                template<>                                                                          \
                BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL ScalarType                            \
                    vec_register_read<VecType, ScalarType, Dim>::read_element<I>(VecType const& v)  \
                {                                                                                   \
    ...
    namespace boost {                                                                                 \
        namespace qvm {                                                                               \
            namespace qvm_detail {                                                                    \
                ...
                template<>                                                                            \
                BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL ScalarType&                             \
                    vec_register_write<VecType, ScalarType, Dim>::write_element<I>(VecType & v)       \
                {                                                                                     \
                    BOOST_QVM_STATIC_ASSERT(I >= 0);                                                  \
                    BOOST_QVM_STATIC_ASSERT(I < Dim);                                                 \
                    return v.Write;                                                                   \
