...
namespace boost {
    namespace qvm {
        ...
        namespace qvm_detail {
            ...
            {
                template<class A>
                static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL typename mat_traits<A>::scalar_type&
                    write_element_idx(int r, int c, A& a)
                {
                    return (I / mat_traits<A>::cols) == r && (I % mat_traits<A>::cols) == c
                               ? mat_traits<
                                     A>::template write_element<I / mat_traits<A>::cols, I % mat_traits<A>::cols>(a)
            ...
            {
                template<class A>
                static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_TRIVIAL typename mat_traits<A>::scalar_type&
        ...
        struct mat_traits_defaults
        {
            ...
            static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL scalar_type read_element(mat_type const& x)
            {
        ...
        protected:
            static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_TRIVIAL scalar_type&
                write_element_idx(int r, int c, mat_type& m)
            {
                return qvm_detail::matrix_w<0, mat_traits<mat_type>::rows * mat_traits<mat_type>::cols>::
                    write_element_idx(r, c, m);
            }
        };
