...
namespace boost {
    namespace qvm {
        ...
        namespace qvm_detail {
            ...
            {
                template<class A>
                static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL typename vec_traits<A>::scalar_type&
                    write_element_idx(int i, A& a)
                {
                    return I == i ? vec_traits<A>::template write_element<I>(a)
            ...
            {
                template<class A>
                static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_TRIVIAL typename vec_traits<A>::scalar_type&
        ...
        struct vec_traits_defaults
        {
            ...
            template<int I>
            static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL scalar_type read_element(vec_type const& x)
            {
                return vec_traits<vec_type>::template write_element<I>(const_cast<vec_type&>(x));
            }

            static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_CRITICAL scalar_type read_element_idx(int i, vec_type const& x)
            {
                return vec_traits<vec_type>::write_element_idx(i, const_cast<vec_type&>(x));
            }

        protected:
            static BOOST_QVM_CONSTEXPR BOOST_QVM_INLINE_TRIVIAL scalar_type& write_element_idx(int i, vec_type& m)
            {
