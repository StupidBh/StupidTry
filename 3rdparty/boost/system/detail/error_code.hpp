    ...
    namespace system {
        ...
        public:
            ...
            BOOST_SYSTEM_CONSTEXPR error_code(int val, const error_category& cat) noexcept :
                d1_(),
                lc_flags_(2 + detail::failed_impl(val, cat))
            {
                d1_.val_ = val;
                d1_.cat_ = &cat;
            }
            ...
            error_code(int val, const error_category& cat, source_location const* loc) noexcept :
                d1_(),
                lc_flags_((loc ? reinterpret_cast<boost::uintptr_t>(loc) : 2) | +detail::failed_impl(val, cat))
            {
                d1_.val_ = val;
                d1_.cat_ = &cat;
            }
            ...
            BOOST_SYSTEM_CONSTEXPR error_code(
                ErrorCodeEnum e,
                typename detail::enable_if<
                    is_error_code_enum<ErrorCodeEnum>::value || std::is_error_code_enum<ErrorCodeEnum>::value>::type* =
                    0) noexcept :
                d1_(),
                lc_flags_(0)
            {
            ...
            void assign(error_code const& ec, source_location const* loc) noexcept { *this = error_code(ec, loc); }

            template<typename ErrorCodeEnum>
            BOOST_SYSTEM_CONSTEXPR
                typename detail::enable_if<is_error_code_enum<ErrorCodeEnum>::value, error_code>::type&
            ...
            BOOST_SYSTEM_CONSTEXPR void clear() noexcept { *this = error_code(); }
            ...
            BOOST_SYSTEM_CONSTEXPR int value() const noexcept
            {
                if (lc_flags_ != 1) {
                    ...
                    unsigned ch = static_cast<unsigned>(
                        reinterpret_cast<boost::uintptr_t>(&ec.category()) % 2'097'143); // 2^21-9, prime
            ...
            bool has_location() const noexcept { return lc_flags_ >= 4; }

            source_location const& location() const noexcept
            {
                BOOST_STATIC_CONSTEXPR source_location loc;
                return lc_flags_ >= 4
                           ? *reinterpret_cast<source_location const*>(lc_flags_ & ~static_cast<boost::uintptr_t>(1))
                           : loc;
            }
        ...
        public:
            ...
            template<
                class E,
                class N1 = void,
                class N2 = typename detail::enable_if<std::is_error_code_enum<E>::value>::type>
...
// This condition must be the same as the one in error_category_impl.hpp
            ...
            operator std::error_code() { return const_cast<error_code const&>(*this); }

            template<class T, class E = typename detail::enable_if<detail::is_same<T, std::error_code>::value>::type>
            operator T&()
            {
                if (lc_flags_ != 1) {
            ...
            template<class T, class E = typename std::enable_if<std::is_same<T, std::error_code>::value>::type>
            ...
            inline friend std::basic_ostream<Ch, Tr>& operator<<(std::basic_ostream<Ch, Tr>& os, error_code const& ec)
            {
