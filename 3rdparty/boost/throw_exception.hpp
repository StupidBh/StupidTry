...
#if !defined(BOOST_EXCEPTION_DISABLE) && defined(BOOST_BORLANDC) && \
    BOOST_WORKAROUND(BOOST_BORLANDC, BOOST_TESTED_AT(0x593))
...
namespace boost {
    ...
    namespace detail {
        ...
        template<class E, class B, std::size_t I = sizeof(wrapexcept_is_convertible<B>(static_cast<E*>(BOOST_NULLPTR)))>
    ...
    public:
        explicit wrapexcept(E const& e) :
            E(e)
        {
            copy_from(&e);
        }

        explicit wrapexcept(E const& e, boost::source_location const& loc) :
            E(e)
        {
            copy_from(&e);
...
namespace boost {
    ...
    namespace detail {
        ...
        struct BOOST_SYMBOL_VISIBLE throw_location
        {
            ...
            explicit throw_location(boost::source_location const& loc) :
                location_(loc)
            {
            }
        };
    ...
    template<class E>
    BOOST_NORETURN void throw_with_location(E&& e, boost::source_location const& loc = BOOST_CURRENT_LOCATION)
    {
        throw_exception_assert_compatibility(e);
    ...
    template<class E>
    BOOST_NORETURN void throw_with_location(E const& e, boost::source_location const& loc = BOOST_CURRENT_LOCATION)
    {
        throw_exception_assert_compatibility(e);
