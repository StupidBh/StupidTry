...
namespace boost {
    namespace serialization {
        namespace detail {
            ...
            void save_impl(Archive& ar, const OT& ot)
            {
...
// be done by serialization the optional<T> as optional<T *>
            ...
            // OT is of the form optional<T>
            template<class Archive, class OT>
            void load_impl(Archive& ar, OT& ot, const unsigned int version)
            {
                bool tflag;
                ar >> boost::serialization::make_nvp("initialized", tflag);
                if (!tflag) {
                ...
                if (0 == version) {
                    ...
                    boost::serialization::library_version_type library_version(ar.get_library_version());
        ...
        template<class Archive, class T>
        void load(Archive& ar, boost::optional<T>& ot, const unsigned int version)
        {
...
#ifndef BOOST_NO_CXX17_HDR_OPTIONAL
        template<class Archive, class T>
        void load(Archive& ar, std::optional<T>& ot, const unsigned int version)
        {
            detail::load_impl(ar, ot, version);
        }
#endif
        ...
        template<class Archive, class T>
        void serialize(Archive& ar, boost::optional<T>& ot, const unsigned int version)
        {
...
#ifndef BOOST_NO_CXX17_HDR_OPTIONAL
        template<class Archive, class T>
        void serialize(Archive& ar, std::optional<T>& ot, const unsigned int version)
        {
            boost::serialization::split_free(ar, ot, version);
        }
#endif
