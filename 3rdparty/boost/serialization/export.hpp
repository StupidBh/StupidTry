...
namespace boost {
    namespace archive {
        namespace detail {
            ...
            struct export_impl
            {
                static const basic_pointer_iserializer& enable_load(mpl::true_)
                {
                    return boost::serialization::singleton<
                        pointer_iserializer<Archive, Serializable>>::get_const_instance();
                }

                static const basic_pointer_oserializer& enable_save(mpl::true_)
                {
                    return boost::serialization::singleton<
            ...
            {
                export_impl<Archive, Serializable>::enable_save(typename Archive::is_saving());

                export_impl<Archive, Serializable>::enable_load(typename Archive::is_loading());
            }
            ...
            namespace detail {                                                                                        \
                namespace extra_detail {                                                                              \
                    ...
                    guid_initializer<T> const& init_guid<T>::g =                                                      \
                        ::boost::serialization::singleton<guid_initializer<T>>::get_mutable_instance().export_guid(); \
                }                                                                                                     \
            }                                                                                                         \
        }                                                                                                             \
    }                                                                                                                 \
    ...
    #define BOOST_SERIALIZATION_MWERKS_BASE_AND_DERIVED(Base, Derived)                        \
        namespace {                                                                           \
            ...
            static int BOOST_PP_CAT(boost_serialization_mwerks_init2_, __LINE__) =            \
                (::boost::serialization::void_cast_register((Derived*)0, (Base*)0), 3);       \
        }
...
#define BOOST_CLASS_EXPORT_CHECK(T)                        \
    BOOST_STATIC_WARNING(boost::is_polymorphic<U>::value); \
    /**/
...
#define BOOST_CLASS_EXPORT(T)                          \
    BOOST_CLASS_EXPORT_GUID(T, BOOST_PP_STRINGIZE(T) ) \
    /**/
