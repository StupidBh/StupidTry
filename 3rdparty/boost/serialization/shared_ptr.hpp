...
namespace boost {
    ...
    #define BOOST_SERIALIZATION_SHARED_PTR(T)                                             \
        BOOST_CLASS_VERSION(::boost::shared_ptr<T>, 1)                                    \
        BOOST_CLASS_TRACKING(::boost::shared_ptr<T>, ::boost::serialization::track_never) \
        /**/
#endif
...
namespace boost {
    namespace serialization {
        ...
        inline void load(Archive& ar, boost::shared_ptr<T>& t, const unsigned int file_version)
        {
            ...
            T* r;
            if (file_version < 1) {
                ar.register_type(static_cast<boost_132::detail::sp_counted_base_impl<T*, null_deleter>*>(NULL));
                ...
                boost::serialization::shared_ptr_helper<boost::shared_ptr>& h =
                    ar.template get_helper<shared_ptr_helper<boost::shared_ptr>>(shared_ptr_helper_id);
                h.append(sp);
                r = sp.get();
            }
            else {
