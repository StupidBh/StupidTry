...
namespace boost {
    ...
    namespace rational_detail {
        ...
        struct is_compatible_integer<FromInt, ToInt, typename enable_if_c<!is_array<FromInt>::value>::type>
        {
            BOOST_STATIC_CONSTANT(
                bool,
                value = ((std::numeric_limits<FromInt>::is_specialized && std::numeric_limits<FromInt>::is_integer &&
        ...
        struct is_backward_compatible_integer<FromInt, ToInt, typename enable_if_c<!is_array<FromInt>::value>::type>
        {
            BOOST_STATIC_CONSTANT(
                bool,
                value =
                    (std::numeric_limits<FromInt>::is_specialized && std::numeric_limits<FromInt>::is_integer &&
    ...
    public:
        ...
        explicit bad_rational(char const* what) :
    ...
    public:
        ...
        BOOST_CONSTEXPR rational(
            const T& n,
            typename enable_if_c<rational_detail::is_compatible_integer<T, IntType>::value>::type const* = 0) :
            num(n),
            den(1)
        {
        }
        ...
        BOOST_CXX14_CONSTEXPR rational(
            const T& n,
            const U& d,
            typename enable_if_c<
                rational_detail::is_compatible_integer<T, IntType>::value &&
                rational_detail::is_compatible_integer<U, IntType>::value>::type const* = 0) :
            num(n),
            den(d)
        {
            normalize();
        ...
        BOOST_CONSTEXPR explicit rational(
            rational<NewType> const& r,
            typename enable_if_c<rational_detail::is_compatible_integer<NewType, IntType>::value>::type const* = 0) :
            num(r.numerator()),
        ...
        BOOST_CONSTEXPR explicit rational(
            rational<NewType> const& r,
            typename disable_if_c<rational_detail::is_compatible_integer<NewType, IntType>::value>::type const* = 0) :
            num(r.numerator()),
            den(is_normalized(int_type(r.numerator()), int_type(r.denominator())) &&
                        is_safe_narrowing_conversion(r.denominator()) && is_safe_narrowing_conversion(r.numerator())
                    ? r.denominator()
                    : (BOOST_THROW_EXCEPTION(bad_rational("bad rational: denormalized conversion")), 0))
        {
        }
        ...
        BOOST_CXX14_CONSTEXPR
            typename enable_if_c<rational_detail::is_compatible_integer<T, IntType>::value, rational&>::type
            operator=(const T& n)
        {
        ...
        BOOST_CXX14_CONSTEXPR rational(
            const T& n,
            typename enable_if_c<rational_detail::is_backward_compatible_integer<T, IntType>::value>::type const* = 0)
        {
        ...
        BOOST_CXX14_CONSTEXPR rational(
            ...
            typename enable_if_c<
                ...
                is_convertible<T, IntType>::value && std::numeric_limits<U>::is_specialized &&
                std::numeric_limits<U>::is_integer &&
                (std::numeric_limits<U>::radix == std::numeric_limits<IntType>::radix) &&
                is_convertible<U, IntType>::value>::type const* = 0)
        {
            assign(n, d);
        }
        ...
        template<class T>
        BOOST_CXX14_CONSTEXPR
            typename boost::enable_if_c<rational_detail::is_compatible_integer<T, IntType>::value, rational&>::type
            operator+=(const T& i)
        {
            num += i * den;
            return *this;
        ...
        BOOST_CXX14_CONSTEXPR const rational& operator++()
        {
            num += den;
            return *this;
        }
    ...
    #pragma parse_mfunc_templ reset
        ...
        template<class T>
        BOOST_CXX14_CONSTEXPR
            typename boost::enable_if_c<rational_detail::is_compatible_integer<T, IntType>::value, bool>::type
    ...
    private:
        ...
        // Helper functions
        static BOOST_CONSTEXPR int_type inner_gcd(param_type a, param_type b, int_type const& zero = int_type(0))
        {
            return b == zero ? a : inner_gcd(b, a % b, zero);
        }

        static BOOST_CONSTEXPR int_type inner_abs(param_type x, int_type const& zero = int_type(0))
        {
            return x < zero ? -x : +x;
        }
        ...
        static BOOST_CONSTEXPR bool is_normalized(
            param_type n,
            param_type d,
            int_type const& zero = int_type(0),
            int_type const& one = int_type(1))
        {
            return d > zero && (n != zero || d == one) && inner_abs(inner_gcd(n, d, zero), zero) == one;
        }
        ...
        //
        template<class T>
        BOOST_CONSTEXPR static typename boost::enable_if_c<
        ...
        BOOST_CONSTEXPR static typename boost::enable_if_c<
            (std::numeric_limits<T>::digits > std::numeric_limits<IntType>::digits) &&
                (std::numeric_limits<T>::is_signed == true) && (std::numeric_limits<IntType>::is_signed == true),
            bool>::type
            is_safe_narrowing_conversion(const T& val)
        {
            ...
            return (val < (T(1) << std::numeric_limits<IntType>::digits)) &&
                   (val >= -(T(1) << std::numeric_limits<IntType>::digits));
        }
        ...
        BOOST_CONSTEXPR static typename boost::enable_if_c<
            (std::numeric_limits<T>::digits > std::numeric_limits<IntType>::digits) &&
                (std::numeric_limits<T>::is_signed == true) && (std::numeric_limits<IntType>::is_signed == false),
            bool>::type
            is_safe_narrowing_conversion(const T& val)
        {
        ...
        BOOST_CONSTEXPR static typename boost::enable_if_c<
            (std::numeric_limits<T>::digits <= std::numeric_limits<IntType>::digits) &&
                (std::numeric_limits<T>::is_signed == false) && (std::numeric_limits<IntType>::is_signed == true),
            bool>::type
        ...
        BOOST_CONSTEXPR static typename boost::enable_if_c<
            (std::numeric_limits<T>::digits <= std::numeric_limits<IntType>::digits) &&
                (std::numeric_limits<T>::is_signed == false) && (std::numeric_limits<IntType>::is_signed == false),
            bool>::type
            is_safe_narrowing_conversion(const T&)
        {
            return true;
    ...
    template<typename IntType>
    BOOST_CONSTEXPR inline rational<IntType> operator+(const rational<IntType>& r)
    {
        return r;
    }

    template<typename IntType>
    BOOST_CXX14_CONSTEXPR inline rational<IntType> operator-(const rational<IntType>& r)
    {
    ...
    BOOST_CXX14_CONSTEXPR inline typename boost::enable_if_c<
        rational_detail::is_compatible_integer<Arg, IntType>::value || is_same<rational<IntType>, Arg>::value,
    ...
    BOOST_CXX14_CONSTEXPR inline typename boost::
        enable_if_c<rational_detail::is_compatible_integer<Arg, IntType>::value, rational<IntType>>::type
    ...
    BOOST_CXX14_CONSTEXPR inline
        typename boost::enable_if_c<rational_detail::is_compatible_integer<Arg, IntType>::value, bool>::type
        operator<=(const Arg& b, const rational<IntType>& a)
    {
        return a >= b;
    }
    ...
    template<class IntType, class Arg>
    BOOST_CONSTEXPR inline typename boost::enable_if_c<
        rational_detail::is_compatible_integer<Arg, IntType>::value || is_same<rational<IntType>, Arg>::value,
        bool>::type
        operator!=(const rational<IntType>& a, const Arg& b)
    {
    ...
    // Comparison operators
    template<typename IntType>
    BOOST_CXX14_CONSTEXPR bool rational<IntType>::operator<(const rational<IntType>& r) const
    {
        // Avoid repeated construction
        int_type const zero(0);
        ...
        struct
        {
            int_type n, d, q, r;
                 ...
                 static_cast<int_type>(this->num % this->den) },
          rs = { r.num, r.den, static_cast<int_type>(r.num / r.den), static_cast<int_type>(r.num % r.den) };
    ...
    template<typename IntType>
    BOOST_CONSTEXPR inline bool rational<IntType>::operator==(const rational<IntType>& r) const
    {
    ...
    // Invariant check
    template<typename IntType>
    BOOST_CXX14_CONSTEXPR inline bool rational<IntType>::test_invariant() const
    {
        return (this->den > int_type(0)) && (integer::gcd(this->num, this->den) == int_type(1));
    }
    ...
    std::ostream& operator<<(std::ostream& os, const rational<IntType>& r)
    {
        ...
        std::streamsize const w = os.width() - static_cast<std::streamsize>(tail.size());

        ss.clear();
        ss.str("");
        ss.flags(os.flags());
        ss << std::setw(w < 0 || (os.flags() & std::ios::adjustfield) != std::ios::internal ? 0 : w) << r.numerator();
    ...
    BOOST_CONSTEXPR inline T rational_cast(const rational<IntType>& src)
    {
    ...
    // defined, etc etc).
    template<typename IntType>
    BOOST_CXX14_CONSTEXPR inline rational<IntType> abs(const rational<IntType>& r)
    {
    ...
    namespace integer {
        ...
        {
            typedef rational<IntType> result_type, first_argument_type, second_argument_type;

            result_type operator()(first_argument_type const& a, second_argument_type const& b) const
            {
                return result_type(
                    integer::gcd(a.numerator(), b.numerator()),
        ...
        struct lcm_evaluator<rational<IntType>>
        {
            ...
            {
                return result_type(
                    integer::lcm(a.numerator(), b.numerator()),
