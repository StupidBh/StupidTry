...
#include <boost/predef/platform.h>
    ...
    // Cygwin 64 compiler does not define _WIN64 and instead defines it in a private header. We can't define _WIN64
    // ourselves because the header defines the macro unconditionally and if the user includes both Boost.WinAPI and
    // Cygwin WinAPI headers there will be conflict.
    #include <_cygwin.h>
#endif
#if defined(__MINGW32__)
    // MinGW and MinGW-w64 define __MINGW32_VERSION_MAJOR/MINOR and __MINGW64_VERSION_MAJOR/MINOR macros in a private
    // header.
    #include <_mingw.h>
#endif
...
// BOOST_WINAPI_IS_CYGWIN indicates that the target Windows SDK is provided by MinGW variant from Cygwin
// (https://cygwin.com/).
#if defined(__CYGWIN__)
...
//
    ...
    #define BOOST_WINAPI_PARTITION_APP \
        (BOOST_PLAT_WINDOWS_DESKTOP || BOOST_PLAT_WINDOWS_STORE || BOOST_WINAPI_PARTITION_PHONE)
...
#define BOOST_WINAPI_PARTITION_APP_SYSTEM                                  \
    (((BOOST_PLAT_WINDOWS_SDK_VERSION >= BOOST_WINAPI_WINDOWS_SDK_10_0) && \
      (BOOST_WINAPI_PARTITION_APP || BOOST_WINAPI_PARTITION_SYSTEM)) ||    \
...
#define BOOST_WINAPI_PARTITION_DESKTOP_SYSTEM                               \
    (((BOOST_PLAT_WINDOWS_SDK_VERSION >= BOOST_WINAPI_WINDOWS_SDK_10_0) &&  \
      (BOOST_WINAPI_PARTITION_DESKTOP || BOOST_WINAPI_PARTITION_SYSTEM)) || \
     ((BOOST_PLAT_WINDOWS_SDK_VERSION < BOOST_WINAPI_WINDOWS_SDK_10_0) && BOOST_WINAPI_PARTITION_DESKTOP))
