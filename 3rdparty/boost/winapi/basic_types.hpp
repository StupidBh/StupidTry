...
#if defined(STRICT)
    #define BOOST_WINAPI_DETAIL_DECLARE_HANDLE(x) \
        struct x##__;                             \
...
// the typedef expands to void. In Windows SDK, VOID is a macro which unfolds to void. We use our own macro in such
// cases.
...
namespace boost {
    namespace winapi {
        ...
        // ::NTSTATUS is defined in ntdef.h or winternl.h in different Windows SDKs, and is not included by windows.h by
        // default, so always use LONG_
