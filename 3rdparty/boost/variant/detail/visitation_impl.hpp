    ...
    namespace detail {
        namespace variant {
            ...
            struct visitation_impl_step
            {
                ...
                typedef visitation_impl_step<next_iter, LastIter> next;
            };
            ...
            inline typename Visitor::result_type visitation_impl_invoke_impl(
                int,
                Visitor& visitor,
                VoidPtrCV storage,
                T*,
            ...
            {
                return visitor.internal_visit(cast_storage<T>(storage), 1L);
            }
            ...
            )
            {
                if (internal_which >= 0) {
                    ...
                    return visitor.internal_visit(cast_storage<backup_holder<T>>(storage), 1L);
                }
            }

            template<typename Visitor, typename VoidPtrCV, typename T, typename NoBackupFlag>
            inline typename Visitor::result_type
                visitation_impl_invoke(int internal_which, Visitor& visitor, VoidPtrCV storage, T* t, NoBackupFlag, int)
            {
                typedef typename mpl::or_<NoBackupFlag, is_nothrow_move_constructible<T>, has_nothrow_copy<T>>::type
                    never_uses_backup;

                return (visitation_impl_invoke_impl)(internal_which, visitor, storage, t, never_uses_backup());
            }
            ...
            template<typename W, typename S, typename Visitor, typename VPCV, typename NBF>
            inline typename Visitor::result_type visitation_impl(
                int,
                int,
                Visitor&,
                VPCV,
            ...
            template<typename Which, typename step0, typename Visitor, typename VoidPtrCV, typename NoBackupFlag>
            BOOST_FORCEINLINE typename Visitor::result_type visitation_impl(
                const int internal_which,
                const int logical_which,
                Visitor& visitor,
                VoidPtrCV storage,
...
#define BOOST_VARIANT_AUX_APPLY_VISITOR_STEP_TYPEDEF(z, N, _)                         \
    ...
    typedef typename BOOST_PP_CAT(step, N)::next BOOST_PP_CAT(step, BOOST_PP_INC(N)); \
    /**/
...
case (Which::value + (N)):                                               \
    return (visitation_impl_invoke)(internal_which,                      \
                                    visitor,                             \
                                    storage,                             \
...
#undef BOOST_VARIANT_AUX_APPLY_VISITOR_STEP_CASE
                ...
                typedef mpl::int_<Which::value + (BOOST_VARIANT_VISITATION_UNROLLING_LIMIT)> next_which;

                typedef BOOST_PP_CAT(step, BOOST_VARIANT_VISITATION_UNROLLING_LIMIT) next_step;

                typedef typename next_step::type next_type;
                typedef typename is_same<next_type, apply_visitor_unrolled>::type is_apply_visitor_unrolled;
                ...
                return detail::variant::visitation_impl(
                    ...
                    no_backup_flag,
                    static_cast<next_which*>(0),
                    static_cast<next_step*>(0));
            }
