...
//  http://www.boost.org/LICENSE_1_0.txt).
    ...
    #define BOOST_VMD_DETAIL_IDENTIFIER_MAKE_SPLIT(tuple) \
        (BOOST_PP_TUPLE_ELEM(0, BOOST_PP_TUPLE_ELEM(0, tuple)), BOOST_PP_TUPLE_ELEM(1, tuple)) /**/
    ...
    #define BOOST_VMD_DETAIL_IDENTIFIER_SPLIT_SEQUENCE_CONCAT_DATA(tuple) \
        BOOST_VMD_IDENTITY_RESULT(BOOST_PP_IIF(                           \
            ...
            BOOST_VMD_DETAIL_IDENTIFIER_MAKE_SPLIT)(tuple))               \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_SPLIT_SEQUENCE_CONCAT(vcseq)                                                   \
        BOOST_VMD_DETAIL_IDENTIFIER_SPLIT_SEQUENCE_CONCAT_DATA(BOOST_VMD_DETAIL_PARENS(vcseq, BOOST_VMD_RETURN_AFTER)) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_GETID_TID(tid)                                                   \
        BOOST_VMD_IDENTITY_RESULT(                                                                       \
            BOOST_PP_IIF(BOOST_VMD_IS_EMPTY(tid), BOOST_VMD_IDENTITY(tid), BOOST_PP_TUPLE_ELEM)(0, tid)) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_GETID_SEQUENCE(vseq)                                                          \
        BOOST_VMD_DETAIL_IDENTIFIER_GETID_TID(BOOST_VMD_DETAIL_PARENS(BOOST_VMD_DETAIL_IDENTIFIER_CONCATENATE(vseq))) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_SPLIT_SEQUENCE(vseq)                                             \
        BOOST_VMD_DETAIL_IDENTIFIER_SPLIT_SEQUENCE_CONCAT(BOOST_VMD_DETAIL_IDENTIFIER_CONCATENATE(vseq)) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_KEYS(id, rest, mods)                                             \
        BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_KEYS_TUPLE(id, rest, BOOST_VMD_DETAIL_MODS_RESULT_OTHER(mods), mods) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_KEYS_D(d, id, rest, mods)                                             \
        BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_KEYS_TUPLE_D(d, id, rest, BOOST_VMD_DETAIL_MODS_RESULT_OTHER(mods), mods) \
        /**/
    ...
    #define BOOST_VMD_DETAIL_IDENTIFIER_CHECK_KEYS(id, rest, mods)        \
        BOOST_PP_IIF(                                                     \
            BOOST_VMD_IS_EMPTY(BOOST_VMD_DETAIL_MODS_RESULT_OTHER(mods)), \
    ...
    #define BOOST_VMD_DETAIL_IDENTIFIER_MKEYS(mods)                               \
        BOOST_PP_BITAND(                                                          \
            ...
            BOOST_VMD_DETAIL_NOT_EMPTY(BOOST_VMD_DETAIL_MODS_RESULT_OTHER(mods))) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_SUCCESS_MODS(id, rest, keymatch, mods)                               \
        BOOST_PP_IIF(                                                                                        \
            BOOST_VMD_DETAIL_IDENTIFIER_MKEYS(mods),                                                         \
            BOOST_PP_IIF(BOOST_VMD_DETAIL_MODS_IS_RESULT_AFTER(mods), (id, rest, keymatch), (id, keymatch)), \
            BOOST_PP_IIF(BOOST_VMD_DETAIL_MODS_IS_RESULT_AFTER(mods), (id, rest), id))                       \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_FAILURE_MODS(mods)                           \
        BOOST_PP_IIF(                                                                \
            BOOST_VMD_DETAIL_IDENTIFIER_MKEYS(mods),                                 \
            BOOST_PP_IIF(BOOST_VMD_DETAIL_MODS_IS_RESULT_AFTER(mods), (, , ), (, )), \
            BOOST_PP_EXPR_IIF(BOOST_VMD_DETAIL_MODS_IS_RESULT_AFTER(mods), (, )))    \
        /**/
    ...
    #define BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_DATA(tuple, mods)                                               \
        BOOST_VMD_DETAIL_IDENTIFIER_ID_REST(BOOST_PP_TUPLE_ELEM(0, tuple), BOOST_PP_TUPLE_ELEM(1, tuple), mods) \
        /**/
    ...
    #define BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_DATA_D(d, tuple, mods)                                               \
        BOOST_VMD_DETAIL_IDENTIFIER_ID_REST_D(d, BOOST_PP_TUPLE_ELEM(0, tuple), BOOST_PP_TUPLE_ELEM(1, tuple), mods) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_SEQUENCE_AFTER(vseq, mods)                                       \
        BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_DATA(BOOST_VMD_DETAIL_IDENTIFIER_SPLIT_SEQUENCE(vseq), mods) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_SEQUENCE_AFTER_D(d, vseq, mods)                                       \
        BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_DATA_D(d, BOOST_VMD_DETAIL_IDENTIFIER_SPLIT_SEQUENCE(vseq), mods) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_SEQUENCE_ID(vseq, mods)                                        \
        BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_ID(BOOST_VMD_DETAIL_IDENTIFIER_GETID_SEQUENCE(vseq), mods) \
        /**/

    #define BOOST_VMD_DETAIL_IDENTIFIER_SEQUENCE_ID_D(d, vseq, mods)                                        \
        BOOST_VMD_DETAIL_IDENTIFIER_PROCESS_ID_D(d, BOOST_VMD_DETAIL_IDENTIFIER_GETID_SEQUENCE(vseq), mods) \
        /**/
    ...
    #define BOOST_VMD_DETAIL_IDENTIFIER_EX(vseq, mods)                                \
        BOOST_PP_IIF(                                                                 \
            BOOST_PP_BITOR(BOOST_VMD_IS_EMPTY(vseq), BOOST_PP_IS_BEGIN_PARENS(vseq)), \
    ...
    #define BOOST_VMD_DETAIL_IS_IDENTIFIER_MULTIPLE(...)                                             \
        BOOST_VMD_DETAIL_IS_ENTIRE(BOOST_VMD_DETAIL_IDENTIFIER(__VA_ARGS__, BOOST_VMD_RETURN_AFTER)) \
        /**/

    #define BOOST_VMD_DETAIL_IS_IDENTIFIER_MULTIPLE_D(d, ...)                                             \
        BOOST_VMD_DETAIL_IS_ENTIRE(BOOST_VMD_DETAIL_IDENTIFIER_D(d, __VA_ARGS__, BOOST_VMD_RETURN_AFTER)) \
        /**/
