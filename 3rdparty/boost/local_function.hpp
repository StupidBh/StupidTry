...
#define BOOST_LOCAL_FUNCTION_HPP_
    ...
    // PUBLIC //
        ...
        #define BOOST_LOCAL_FUNCTION_ID(id, declarations) \
            BOOST_LOCAL_FUNCTION_AUX_DECL(                \
                ...
                BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS(BOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST(declarations)))
        #define BOOST_LOCAL_FUNCTION(declarations) \
            BOOST_LOCAL_FUNCTION_ID(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, declarations)
        ...
        #define BOOST_LOCAL_FUNCTION_TPL(declarations) \
            BOOST_LOCAL_FUNCTION_ID_TPL(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, declarations)
    #else // VARIADIC
        #define BOOST_LOCAL_FUNCTION_ID(id, ...) \
            BOOST_LOCAL_FUNCTION_AUX_DECL(       \
                id,                              \
                0 /* not within template */,     \
                BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS(BOOST_LOCAL_FUNCTION_DETAIL_PP_VOID_LIST(__VA_ARGS__)))
        #define BOOST_LOCAL_FUNCTION(...) \
            BOOST_LOCAL_FUNCTION_ID(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, __VA_ARGS__)
        ...
        #define BOOST_LOCAL_FUNCTION_TPL(...) \
            BOOST_LOCAL_FUNCTION_ID_TPL(BOOST_LOCAL_FUNCTION_DETAIL_PP_LINE_COUNTER, __VA_ARGS__)
    #endif // VARIADIC
    ...
    */
    ...
    (Mistakenly using <c>this</c> instead of <c>this_</c> might not always result in a compiler error and will in
    general result in undefined behaviour.)
    ...
    Within the local function body it possible to access the result type using <c>result_type</c>, the type of the first
    parameter using <c>arg1_type</c>, the type of the second parameter using <c>arg2_type</c>, etc. The bound variable
    types can be accessed using @RefMacro{BOOST_LOCAL_FUNCTION_TYPEOF}.
    ...
    */
    ...
    However, inline local functions cannot be passed as template parameters (e.g., to <c>std::for_each</c>) or assigned
    to other functors (e.g., to <c>boost::function</c>). That is true on C++03 compilers but inline local functions can
    instead be passed as template parameters on C++11 compilers. On C++11 compilers, there is no need to declare a local
    function lined because this library will automatically use C++11 specific features to inline the local function
    while always allowing to pass it as a template parameter. This optimization is automatically enabled when the
    Boost.Config macro <c>BOOST_NO_CXX11_LOCAL_CLASS_TEMPLATE_PARAMETERS</c> is not defined but it also be forced using
    @RefMacro{BOOST_LOCAL_FUNCTION_CONFIG_LOCALS_AS_TPARAMS}.
