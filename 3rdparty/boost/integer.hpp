...
namespace boost {
    ...
    (!defined __TI_COMPILER_VERSION__ || (__TI_COMPILER_VERSION__ >= 7'000'000 && !defined __TI_40BIT_LONG__))
        template<>
    ...
    ((defined(ULLONG_MAX) && (ULLONG_MAX != ULONG_MAX)) ||         \
     ...
     (defined(ULONGLONG_MAX) && (ULONGLONG_MAX != ULONG_MAX)) || (defined(_ULLONG_MAX) && (_ULLONG_MAX != ULONG_MAX)))
        template<>
        struct exact_signed_base_helper<sizeof(boost::long_long_type) * CHAR_BIT>
        {
            typedef boost::long_long_type exact;
        };

        template<>
        struct exact_unsigned_base_helper<sizeof(boost::ulong_long_type) * CHAR_BIT>
        {
            typedef boost::ulong_long_type exact;
        };
#endif
    ...
    struct int_t : public boost::detail::exact_signed_base_helper<Bits>
    {
        BOOST_STATIC_ASSERT_MSG(
            Bits <= (int)(sizeof(boost::intmax_t) * CHAR_BIT),
        ...
        typedef typename boost::detail::int_least_helper<
            ...
            1 +
#endif
            (Bits - 1 <= ::std::numeric_limits<long>::digits) + (Bits - 1 <= ::std::numeric_limits<int>::digits) +
    ...
    struct uint_value_t
    {
        ...
        BOOST_STATIC_CONSTANT(
            unsigned,
            which = 1 + (MaxValue <= ::boost::integer_traits<unsigned long>::const_max) +
        ...
        BOOST_STATIC_CONSTANT(
            unsigned,
            which = 1 + (MaxValue <= ::boost::integer_traits<boost::ulong_long_type>::const_max) +
