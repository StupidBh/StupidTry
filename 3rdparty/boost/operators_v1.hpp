...
#define BOOST_OPERATOR2_LEFT(name) name##2##_##left
    ...
    #define BOOST_BINARY_OPERATOR_COMMUTATIVE(NAME, OP)                                     \
        template<class T, class U, class B = ::boost::detail::empty_base<T>>                \
        struct NAME##2 : B { friend T operator OP(const T& lhs, const U& rhs) { T nrv(lhs); \
        nrv OP## = rhs;                                                                     \
        return nrv;                                                                         \
        }                                                                                   \
        ...
        ;                                                                                   \
                                                                                            \
        template<class T, class B = ::boost::detail::empty_base<T>>                         \
        struct NAME##1 : B { friend T operator OP(const T& lhs, const T& rhs) { T nrv(lhs); \
        nrv OP## = rhs;                                                                     \
        return nrv;                                                                         \
        }                                                                                   \
        }                                                                                   \
    ...
    #define BOOST_BINARY_OPERATOR_NON_COMMUTATIVE(NAME, OP)                                 \
        ...
        ;                                                                                   \
                                                                                            \
        template<class T, class U, class B = ::boost::detail::empty_base<T>>                \
        struct BOOST_OPERATOR2_LEFT(NAME) :                                                 \
            B                                                                               \
        {                                                                                   \
            friend T operator OP(const U& lhs, const T& rhs)                                \
            {                                                                               \
...
#undef BOOST_BINARY_OPERATOR
    ...
    {
        friend bool operator<=(const T& x, const U& y) { return static_cast<bool>(x < y) || static_cast<bool>(x == y); }
        ...
        friend bool operator<=(const U& x, const T& y) { return static_cast<bool>(y > x) || static_cast<bool>(y == x); }
    ...
    struct partially_ordered1 : B
    {
        ...
        friend bool operator<=(const T& x, const T& y) { return static_cast<bool>(x < y) || static_cast<bool>(x == y); }

        friend bool operator>=(const T& x, const T& y) { return static_cast<bool>(y < x) || static_cast<bool>(x == y); }
    };
...
#define BOOST_OPERATOR_TEMPLATE(template_name)                                          \
    template<                                                                           \
        ...
        struct template_name : template_name##2 < T,                                    \
        U, B >                                                                          \
    {                                                                                   \
    };                                                                                  \
                                                                                        \
    template<class T, class U, class B>                                                 \
        struct template_name<T, U, B, ::boost::detail::true_t> : template_name##1 < T,  \
        U >                                                                             \
    {                                                                                   \
    };                                                                                  \
                                                                                        \
    template<class T, class B>                                                          \
        struct template_name<T, T, B, ::boost::detail::false_t> : template_name##1 < T, \
        B >                                                                             \
    {                                                                                   \
    };                                                                                  \
                                                                                        \
...
#undef BOOST_IMPORT_TEMPLATE4
    ...
    struct input_iterator_helper : input_iteratable<T, P, std::iterator<std::input_iterator_tag, V, D, P, R>>
    {
    };

    template<class T>
    struct output_iterator_helper :
        output_iteratable<T, std::iterator<std::output_iterator_tag, void, void, void, void>>
    {
    ...
    struct forward_iterator_helper : forward_iteratable<T, P, std::iterator<std::forward_iterator_tag, V, D, P, R>>
    {
    };

    template<class T, class V, class D = std::ptrdiff_t, class P = V*, class R = V&>
    struct bidirectional_iterator_helper :
        bidirectional_iteratable<T, P, std::iterator<std::bidirectional_iterator_tag, V, D, P, R>>
    {
    };

    template<class T, class V, class D = std::ptrdiff_t, class P = V*, class R = V&>
    struct random_access_iterator_helper :
        random_access_iteratable<T, P, D, R, std::iterator<std::random_access_iterator_tag, V, D, P, R>>
    {
