...
namespace std {
    ...
    public:
        ...
        {
            return (quantity_type::from_value((std::numeric_limits<T>::min)()));
        }

        static BOOST_CONSTEXPR quantity_type(max)()
        {
            return (quantity_type::from_value((std::numeric_limits<T>::max)()));
        }
#ifndef BOOST_NO_CXX11_NUMERIC_LIMITS
        static BOOST_CONSTEXPR quantity_type(lowest)()
        {
            return (quantity_type::from_value((std::numeric_limits<T>::lowest)()));
        }
#endif
        ...
        {
            return (quantity_type::from_value(std::numeric_limits<T>::epsilon()));
        }

        static BOOST_CONSTEXPR quantity_type round_error()
        {
            return (quantity_type::from_value(std::numeric_limits<T>::round_error()));
        }
        ...
        {
            return (quantity_type::from_value(std::numeric_limits<T>::infinity()));
        }

        static BOOST_CONSTEXPR quantity_type quiet_NaN()
        {
            return (quantity_type::from_value(std::numeric_limits<T>::quiet_NaN()));
        }

        static BOOST_CONSTEXPR quantity_type signaling_NaN()
        {
            return (quantity_type::from_value(std::numeric_limits<T>::signaling_NaN()));
        }

        static BOOST_CONSTEXPR quantity_type denorm_min()
        {
            return (quantity_type::from_value(std::numeric_limits<T>::denorm_min()));
        }
