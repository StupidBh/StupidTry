...
namespace boost {
    ...
    namespace units {
        ...
        public:
            ...
            BOOST_CONSTEXPR absolute() :
                val_()
            {
            }

            BOOST_CONSTEXPR absolute(const value_type& val) :
                val_(val)
            {
            }

            BOOST_CONSTEXPR absolute(const this_type& source) :
                val_(source.val_)
            {
            }
            ...
            BOOST_CXX14_CONSTEXPR const this_type& operator+=(const value_type& val)
            {
                val_ += val;
                return *this;
            }
...
#define BOOST_UNITS_DEFINE_CONVERSION_OFFSET(From, To, type_, value_)                                             \
    namespace boost {                                                                                             \
        namespace units {                                                                                         \
            template<>                                                                                            \
            struct affine_conversion_helper<reduce_unit<From::unit_type>::type, reduce_unit<To::unit_type>::type> \
            {                                                                                                     \
