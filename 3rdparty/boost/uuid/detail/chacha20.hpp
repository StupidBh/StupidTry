...
namespace boost {
    namespace uuids {
        namespace detail {
            ...
            private:
                static inline std::uint32_t rotl(std::uint32_t x, int n) noexcept { return (x << n) | (x >> (32 - n)); }
            ...
            public:
                ...
                chacha20_12(std::uint32_t const (&key)[8], std::uint32_t const (&nonce)[2]) noexcept :
                    index_(16)
                {
                    state_[0] = 0x61707865;
                    state_[1] = 0x3320646e;
