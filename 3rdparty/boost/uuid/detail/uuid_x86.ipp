...
// MSVC does not always have immintrin.h (at least, not up to MSVC 10), so include the appropriate header for each
// instruction set
#if defined(BOOST_UUID_USE_AVX10_1)
    #include <immintrin.h>
#elif defined(BOOST_UUID_USE_SSE41)
    ...
    namespace uuids {
        namespace detail {
            ...
            BOOST_FORCEINLINE void
                compare(uuid const& lhs, uuid const& rhs, std::uint32_t& cmp, std::uint32_t& rcmp) noexcept
            {
                __m128i mm_left = uuids::detail::load_unaligned_si128(lhs.data);
                __m128i mm_right = uuids::detail::load_unaligned_si128(rhs.data);

                // To emulate lexicographical_compare behavior we have to perform two comparisons - the forward and
                // reverse one. Then we know which bytes are equivalent and which ones are different, and for those
                // different the comparison results will be opposite. Then we'll be able to find the first differing
                // comparison result (for both forward and reverse ways), and depending on which way it is for, this
                // will be the result of the operation. There are a few notes to consider:
                //
                // 1. Due to little endian byte order the first bytes go into the lower part of the xmm registers,
                //    so the comparison results in the least significant bits will actually be the most significant for
                //    the final operation result. This means we have to determine which of the comparison results have
                //    the least significant bit on, and this is achieved with the "(x - 1) ^ x" trick. With BMI, this
                //    will produce a single blsmsk instruction.
                // 2. Because there is only signed byte comparison until AVX-512, we have to invert byte comparison
                // results whenever signs of the
                //    corresponding bytes are different. I.e. in signed comparison it's -1 < 1, but in unsigned it is
                //    the opposite (255 > 1). To do that we XOR left and right, making the most significant bit of each
                //    byte 1 if the signs are different, and later apply this mask with another XOR to the comparison
                //    results.
                // 3. Until AVX-512, there is only pcmpgtb instruction that compares for "greater" relation, so we swap
                // the arguments to get what we need.
