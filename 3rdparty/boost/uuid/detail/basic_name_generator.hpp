    ...
    namespace uuids {
        namespace detail {
            ...
            public:
                ...
                template<class Ch>
                uuid operator()(Ch const* name) const noexcept
                {
                    HashAlgo hash;
            ...
            private:
                ...
                void process_characters(HashAlgo& hash, wchar_t const* p, std::size_t n) const noexcept
                {
                    ...
                    for (std::size_t i = 0; i < n; ++i) {
                        ...
                        unsigned char bytes[4] = { static_cast<unsigned char>((ch >> 0) & 0xFF),
                ...
                void process_utf32_codepoint(HashAlgo& hash, std::uint32_t cp) const noexcept
                {
                    ...
                    if (cp < 0x80) {
                        ...
                        unsigned char bytes[2] = { static_cast<unsigned char>(0xC0 | (cp >> 6)),
                                                   static_cast<unsigned char>(0x80 | (cp & 0x3F)) };

                        hash.process_bytes(bytes, 2);
                    }
                    else if (cp < 0x10000) {
                        unsigned char bytes[3] = { static_cast<unsigned char>(0xE0 | (cp >> 12)),
                        ...
                        unsigned char bytes[4] = { static_cast<unsigned char>(0xF0 | (cp >> 18)),
