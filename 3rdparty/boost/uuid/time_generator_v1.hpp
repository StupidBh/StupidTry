...
// https://gcc.gnu.org/bugzilla/show_bug.cgi?id=114865
        ...
        inline time_generator_v1::time_generator_v1(uuid::node_type const& node, state_type const& state) noexcept :
            node_(node),
            state_(state)
        {
        }
        ...
        inline time_generator_v1::time_generator_v1(
            uuid::node_type const& node,
            std::atomic<state_type>& state) noexcept :
            node_(node),
            ps_(&state)
        {
        }
        ...
        inline time_generator_v1::result_type time_generator_v1::operator()() noexcept
        {
            if (ps_) {
                ...
                for (;;) {
                    ...
                    if (ps_->compare_exchange_strong(
                            oldst,
                            newst,
