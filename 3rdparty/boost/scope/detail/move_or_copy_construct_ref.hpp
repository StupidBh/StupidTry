...
namespace boost {
    namespace scope {
        namespace detail {
            ...
            //! The type trait produces an rvalue reference to \a From if \a To has a non-throwing constructor from a \a
            //! From rvalue and an lvalue reference otherwise.
            template<typename From, typename To = From>
            struct move_or_copy_construct_ref
            {
                using type = typename std::
                    conditional<std::is_nothrow_constructible<To, From>::value, From&&, From const&>::type;
            };
