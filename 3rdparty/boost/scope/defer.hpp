...
namespace boost {
    namespace scope {
        ...
        namespace detail {
            ...
            // Workaround for clang < 5.0 which can't pass defer_guard as a template template parameter from within
            // defer_guard definition
            template<typename T>
        ...
        private:
            struct data
            {
                ...
                template<typename F, typename = typename std::enable_if<std::is_constructible<Func, F>::value>::type>
                explicit data(F&& func, std::false_type)
                try :
                    m_func(static_cast<F&&>(func)) {
                }
                catch (...) {
                ...
                typename = typename std::enable_if<detail::conjunction<
                    std::is_constructible<
                        ...
                        typename std::is_nothrow_constructible<
                            Func,
                            typename detail::move_or_copy_construct_ref<F, Func>::type>::type>,
                    detail::is_not_like_defer_guard<F>>::value>::type
                //! \endcond
                >
            defer_guard(F&& func) noexcept(BOOST_SCOPE_DETAIL_DOC_HIDDEN(
                std::is_nothrow_constructible<
                    data,
                    typename detail::move_or_copy_construct_ref<F, Func>::type,
                    typename std::is_nothrow_constructible<
                        Func,
                        typename detail::move_or_copy_construct_ref<F, Func>::type>::type>::value)) :
                m_data(
