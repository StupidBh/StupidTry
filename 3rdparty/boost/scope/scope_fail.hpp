...
namespace boost {
    namespace scope {
        ...
        namespace detail {
            ...
            // Workaround for clang < 5.0 which can't pass scope_fail as a template template parameter from within
            // scope_fail definition
            template<typename T>
        ...
        class scope_fail : public scope_exit<Func, Cond> {
            //! \cond
        private:
        ...
        public:
            ...
            template<
                ...
                typename = typename std::enable_if<detail::conjunction<
                    ...
                    detail::is_not_like_scope_fail<F>>::value>::type
                //! \endcond
                >
            ...
            //! \endcond
            scope_fail(scope_fail&& that) noexcept(
         ...
         */
        template<typename F>
        inline scope_fail<typename std::decay<F>::type> make_scope_fail(F&& func, bool active = true) noexcept(
            std::is_nothrow_constructible<scope_fail<typename std::decay<F>::type>, F, bool>::value)
        {
        ...
        inline
            ...
            typename std::enable_if<
                ...
                scope_fail<typename std::decay<F>::type, typename std::decay<C>::type>>::type
#else
            scope_fail<typename std::decay<F>::type, typename std::decay<C>::type>
#endif
            make_scope_fail(F&& func, C&& cond, bool active = true) noexcept(
                std::is_nothrow_constructible<
                    ...
                    bool>::value)
        {
            return scope_fail<typename std::decay<F>::type, typename std::decay<C>::type>(
                static_cast<F&&>(func),
                static_cast<C&&>(cond),
                active);
        }
