...
namespace boost {
    namespace scope {
        ...
        namespace detail {
            ...
            // Workaround for clang < 5.0 which can't pass scope_exit as a template template parameter from within
            // scope_exit definition
            template<typename T>
            using is_not_like_scope_exit = detail::is_not_like<T, scope_exit>;

            //! The scope guard used to invoke the condition and action functions in case of exception during scope
            //! guard construction
            template<typename Func, typename Cond>
            class init_guard {
            private:
            ...
            public:
                ...
                ~init_guard() noexcept(detail::conjunction<
                                       detail::is_nothrow_invocable<Func&>,
                                       detail::is_nothrow_invocable<Cond&>>::value)
                {
                    if (m_active && m_cond()) {
                        m_func();
                    }
                ...
                Func&& get_func() noexcept { return static_cast<Func&&>(m_func); }

                Cond&& get_cond() noexcept { return static_cast<Cond&&>(m_cond); }
        ...
        public:
             ...
             * \returns \c true.
             */
            result_type operator()() const noexcept { return true; }
        };
        ...
        class scope_exit {
            //! \cond
        private:
            struct func_holder : public detail::compact_storage<Func>
            {
            ...
            struct cond_holder : public detail::compact_storage<Cond>
            {
                using cond_base = detail::compact_storage<Cond>;

                template<typename C, typename = typename std::enable_if<std::is_constructible<Cond, C>::value>::type>
            ...
            struct data : public func_holder, public cond_holder
            {
                bool m_active;
                ...
                template<
                    ...
                    typename = typename std::enable_if<detail::conjunction<
                        ...
                        std::is_constructible<
                            ...
                            Func&,
                            bool,
                            typename std::is_nothrow_constructible<Cond, C>::type>>::value>::type>
                explicit data(F&& func, C&& cond, bool active) noexcept(
                    detail::conjunction<
                        std::is_nothrow_constructible<Func, F>,
                        std::is_nothrow_constructible<Cond, C>>::value) :
                    func_holder(
                        static_cast<F&&>(func),
                        static_cast<C&&>(cond),
                        active,
                    ...
                    cond_holder(
                        ...
                        func_holder::get(),
                        active,
                        typename std::is_nothrow_constructible<Cond, C>::type()),
                    m_active(active)
                {
                }

                Func& get_func() noexcept { return func_holder::get(); }

                Func const& get_func() const noexcept { return func_holder::get(); }

                Cond& get_cond() noexcept { return cond_holder::get(); }

                Cond const& get_cond() const noexcept { return cond_holder::get(); }
        ...
        public:
            ...
            template<
                ...
                typename = typename std::enable_if<detail::conjunction<
                    ...
                    std::is_constructible<
                        ...
                        bool>,
                    detail::is_not_like_scope_exit<F>>::value>::type
                //! \endcond
                >
            explicit scope_exit(F&& func, bool active = true) noexcept(BOOST_SCOPE_DETAIL_DOC_HIDDEN(
            ...
            template<
                ...
                //! \endcond
                >
            explicit scope_exit(F&& func, C&& cond, bool active = true) noexcept(BOOST_SCOPE_DETAIL_DOC_HIDDEN(
                std::is_nothrow_constructible<
                    data,
                    typename detail::move_or_copy_construct_ref<F, Func>::type,
                    typename detail::move_or_copy_construct_ref<C, Cond>::type,
            ...
            //! \endcond
            scope_exit(scope_exit&& that) noexcept(BOOST_SCOPE_DETAIL_DOC_HIDDEN(
                std::is_nothrow_constructible<
                    data,
                    typename detail::move_or_copy_construct_ref<Func>::type,
                    typename detail::move_or_copy_construct_ref<Cond>::type,
             ...
             */
            ~scope_exit() noexcept(BOOST_SCOPE_DETAIL_DOC_HIDDEN(
                detail::conjunction<detail::is_nothrow_invocable<Func&>, detail::is_nothrow_invocable<Cond&>>::value))
            {
             ...
             * **Throws:** Nothing.
             */
            bool active() const noexcept { return m_data.m_active; }
             ...
             * \post `this->active() == active`
             */
            void set_active(bool active) noexcept { m_data.m_active = active; }
        };
         ...
         */
        template<typename F>
        inline scope_exit<typename std::decay<F>::type> make_scope_exit(F&& func, bool active = true) noexcept(
            std::is_nothrow_constructible<scope_exit<typename std::decay<F>::type>, F, bool>::value)
        {
        ...
        inline
            ...
            typename std::enable_if<
                std::is_constructible<
                    ...
                    C,
                    bool>::value,
                scope_exit<typename std::decay<F>::type, typename std::decay<C>::type>>::type
#else
            scope_exit<typename std::decay<F>::type, typename std::decay<C>::type>
#endif
            make_scope_exit(F&& func, C&& cond, bool active = true) noexcept(
                std::is_nothrow_constructible<
                    scope_exit<typename std::decay<F>::type, typename std::decay<C>::type>,
                    F,
                    C,
                    bool>::value)
        {
            return scope_exit<typename std::decay<F>::type, typename std::decay<C>::type>(
                static_cast<F&&>(func),
                static_cast<C&&>(cond),
                active);
        }
