...
namespace boost {
    ...
    namespace detail {
        ...
        template<class T>
        struct lsp_if_not_array
        {
            typedef boost::local_shared_ptr<T> type;
        };

        template<class T>
        struct lsp_if_not_array<T[]>
        {
        };

        template<class T, std::size_t N>
        struct lsp_if_not_array<T[N]>
        {
        };
        ...
        public:
            explicit lsp_ms_deleter(A const& a) noexcept :
                a_(a),
                initialized_(false)
            {
            }
            ...
            lsp_ms_deleter(lsp_ms_deleter const& r) noexcept :
                a_(r.a_),
                initialized_(false)
            {
            }
    ...
    typename boost::detail::lsp_if_not_array<T>::type allocate_local_shared(A const& a, Args&&... args)
    {
