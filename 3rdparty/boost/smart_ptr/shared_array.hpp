...
namespace boost {
    ...
    public:
        ...
        shared_array(std::nullptr_t) noexcept :
            px(0),
            pn()
        {
        }
        ...
        shared_array(shared_array const& r) noexcept :
            px(r.px),
            pn(r.pn)
        {
        }

        shared_array(shared_array&& r) noexcept :
            px(r.px),
            pn()
        {
            pn.swap(r.pn);
        ...
        template<class Y>
        shared_array(shared_array<Y> const& r, element_type* p) noexcept :
            px(p),
            pn(r.pn)
        {
        }
        ...
        template<class Y>
        void reset(shared_array<Y> const& r, element_type* p) noexcept
        {
    ...
    template<class T>
    inline bool operator==(shared_array<T> const& p, std::nullptr_t) noexcept
    {
        return p.get() == 0;
    }

    template<class T>
    inline bool operator==(std::nullptr_t, shared_array<T> const& p) noexcept
    {
        return p.get() == 0;
    }
