...
#if defined(_MSC_VER)
    #pragma warning(push)
    // This is a bogus MSVC warning, which is flagged by friend declarations of intrusive_ptr_add_ref and
    // intrusive_ptr_release in intrusive_ref_counter: 'name' : the inline specifier cannot be used when a friend
    // declaration refers to a specialization of a function template Note that there is no inline specifier in the
    // declarations.
