...
namespace boost {
    namespace detail {
        ...
        {
            typedef typename std::remove_cv<typename std::remove_extent<T>::type>::type type;
        };
        ...
        public:
            ...
            static sp_alloc_ptr pointer_to(T& v)
            {
                return sp_alloc_ptr(
                    1,
                    std::pointer_traits<P>::pointer_to(const_cast<typename std::remove_cv<T>::type&>(v)));
            }
#endif
        ...
        template<class A>
        inline void sp_alloc_clear(A& a, typename boost::allocator_pointer<A>::type p, std::size_t, std::false_type)
        {
            boost::alloc_destroy(a, boost::to_address(p));
        }

        template<class A>
        inline void sp_alloc_clear(A& a, typename boost::allocator_pointer<A>::type p, std::size_t n, std::true_type)
        {
    ...
    class alloc_deleter : empty_value<typename allocator_rebind<A, typename detail::sp_alloc_value<T>::type>::type> {
        typedef typename allocator_rebind<A, typename detail::sp_alloc_value<T>::type>::type allocator;
    ...
    namespace detail {
        ...
        class sp_alloc_make {
        public:
            typedef typename boost::allocator_rebind<A, typename sp_alloc_value<T>::type>::type allocator;
    ...
    template<class T, class A>
    inline typename std::enable_if<!std::is_array<T>::value, std::unique_ptr<T, alloc_deleter<T, A>>>::type
    ...
    template<class T, class A>
    inline
        typename std::enable_if<!std::is_array<T>::value, std::unique_ptr<T, alloc_deleter<T, noinit_adaptor<A>>>>::type
    ...
    template<class T, class A>
    inline
        typename std::enable_if<detail::sp_is_unbounded_array<T>::value, std::unique_ptr<T, alloc_deleter<T, A>>>::type
        allocate_unique(const A& alloc, std::size_t size)
    {
        detail::sp_alloc_make<T, A> c(alloc, size);
        boost::alloc_construct_n(c.state(), boost::first_scalar(c.get()), size * detail::sp_alloc_size<T>::value);
        return c.release();
    }

    template<class T, class A>
    inline typename std::enable_if<
        detail::sp_is_bounded_array<T>::value,
        ...
        allocate_unique(const A& alloc)
    {
        ...
        boost::alloc_construct_n(c.state(), boost::first_scalar(c.get()), detail::sp_alloc_size<T>::value);
        return c.release();
    }

    template<class T, class A>
    inline typename std::enable_if<
        detail::sp_is_unbounded_array<T>::value,
    ...
    inline typename std::enable_if<
        detail::sp_is_bounded_array<T>::value,
        std::unique_ptr<typename detail::sp_alloc_result<T>::type, alloc_deleter<T, noinit_adaptor<A>>>>::type
        allocate_unique_noinit(const A& alloc)
    {
        return boost::allocate_unique<T, noinit_adaptor<A>>(alloc);
    }
    ...
    inline
        typename std::enable_if<detail::sp_is_unbounded_array<T>::value, std::unique_ptr<T, alloc_deleter<T, A>>>::type
        allocate_unique(const A& alloc, std::size_t size, const typename std::remove_extent<T>::type& value)
    {
        detail::sp_alloc_make<T, A> c(alloc, size);
    ...
    inline typename std::enable_if<
        detail::sp_is_bounded_array<T>::value,
        std::unique_ptr<typename detail::sp_alloc_result<T>::type, alloc_deleter<T, A>>>::type
        allocate_unique(const A& alloc, const typename std::remove_extent<T>::type& value)
    {
        detail::sp_alloc_make<T, A> c(alloc, std::extent<T>::value);
        boost::alloc_construct_n(
            c.state(),
    ...
    inline
        typename allocator_pointer<typename allocator_rebind<A, typename detail::sp_alloc_value<T>::type>::type>::type
