...
namespace boost {
    namespace detail {
        ...
        template<class T>
        constexpr inline std::size_t sp_objects(std::size_t size) noexcept
        {
        ...
        struct sp_array_offset
        {
            enum
            {
                value = sp_align_up < sizeof(T),
                sp_array_alignment < T,
                U > ::value > ::value
            };
        };

        template<class U, class T>
        inline U* sp_array_start(T* base) noexcept
        {
            enum
            {
                size = sp_array_offset < T,
                U > ::value
            };
        ...
        class sp_array_creator {
            ...
            enum
            {
                offset = sp_array_offset < T,
                element > ::value
            };

            typedef typename sp_type_with_alignment<sp_array_alignment<T, element>::value>::type type;
        ...
        template<class T>
        class BOOST_SYMBOL_VISIBLE sp_array_base : public sp_counted_base {
    ...
    template<class T, class A>
    inline typename std::enable_if<detail::sp_is_unbounded_array<T>::value, shared_ptr<T>>::type
        allocate_shared(const A& allocator, std::size_t count, const typename std::remove_extent<T>::type& value)
    {
        typedef typename detail::sp_array_element<T>::type element;
        typedef typename allocator_rebind<A, element>::type other;
        typedef detail::sp_array_state<other> state;
    ...
    template<class T, class A>
    inline typename std::enable_if<detail::sp_is_bounded_array<T>::value, shared_ptr<T>>::type
        allocate_shared(const A& allocator, const typename std::remove_extent<T>::type& value)
    {
        enum
        {
            count = std::extent<T>::value
