...
namespace boost {
    namespace detail {
        ...
        class BOOST_SYMBOL_VISIBLE lsp_array_base : public local_counted_base {
        public:
        ...
        template<class A>
        class lsp_array_state : public sp_array_state<A> {
        public:
            template<class U>
    ...
    template<class T, class A>
    inline typename std::enable_if<detail::sp_is_unbounded_array<T>::value, local_shared_ptr<T>>::type
        allocate_local_shared(const A& allocator, std::size_t count, const typename std::remove_extent<T>::type& value)
    {
        typedef typename detail::sp_array_element<T>::type element;
        typedef typename allocator_rebind<A, element>::type other;
        typedef detail::lsp_array_state<other> state;
    ...
    template<class T, class A>
    inline typename std::enable_if<detail::sp_is_bounded_array<T>::value, local_shared_ptr<T>>::type
        allocate_local_shared(const A& allocator, const typename std::remove_extent<T>::type& value)
    {
        enum
        {
            count = std::extent<T>::value
