...
namespace boost {
    ...
    public:
        ...
        template<class Y>
        weak_ptr(weak_ptr<Y> const& r, element_type* p) noexcept :
            px(p),
            pn(r.pn)
        {
        }
        ...
        template<class Y>
        weak_ptr(weak_ptr<Y>&& r, element_type* p) noexcept :
            px(p),
            pn(std::move(r.pn))
        {
        }
        ...
        shared_ptr<T> lock() const noexcept { return shared_ptr<T>(*this, boost::detail::sp_nothrow_tag()); }
    ...
    template<class T>
    std::size_t hash_value(boost::weak_ptr<T> const& p) noexcept
    {
...
namespace std {
    ...
    template<class T>
    struct hash<::boost::weak_ptr<T>>
    {
        std::size_t operator()(::boost::weak_ptr<T> const& p) const noexcept { return p.owner_hash_value(); }
    };
