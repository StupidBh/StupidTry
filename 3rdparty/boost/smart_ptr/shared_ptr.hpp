...
namespace boost {
    ...
    namespace detail {
        ...
        inline void
            sp_enable_shared_from_this(boost::shared_ptr<X>* ppx, Y const* py, boost::enable_shared_from_raw const* pe);
        ...
        inline void sp_assert_convertible() noexcept
        {
        ...
        inline void sp_pointer_construct(boost::shared_ptr<T[N]>* /*ppx*/, Y* p, boost::detail::shared_count& pn)
        {
    ...
    public:
        ...
        constexpr shared_ptr(
            boost::detail::sp_internal_constructor_tag,
            element_type* px_,
        ...
        template<class Y, class D>
        shared_ptr(boost::movelib::unique_ptr<Y, D> r) :
            px(r.get()),
            pn()
        {
            boost::detail::sp_assert_convertible<Y, T>();
        ...
        shared_ptr(shared_ptr&& r) noexcept :
            px(r.px),
            pn(static_cast<boost::detail::shared_count&&>(r.pn))
        {
            r.px = 0;
        }
        ...
        template<class Y>
        void reset(shared_ptr<Y> const& r, element_type* p) noexcept
        {
            this_type(r, p).swap(*this);
        }

        template<class Y>
        void reset(shared_ptr<Y>&& r, element_type* p) noexcept
        {
        ...
        template<class Y>
        bool owner_equals(shared_ptr<Y> const& rhs) const noexcept
        {
            return pn == rhs.pn;
        }

        template<class Y>
        bool owner_equals(weak_ptr<Y> const& rhs) const noexcept
        {
            return pn == rhs.pn;
        }
        ...
        void* _internal_get_deleter(boost::detail::sp_typeinfo_ const& ti) const noexcept { return pn.get_deleter(ti); }
    ...
    namespace detail {
        ...
        template<class D, class T>
        D* basic_get_local_deleter(D*, shared_ptr<T> const& p) noexcept;
        template<class D, class T>
        ...
        public:
            ...
            template<class T>
            void set_deleter(shared_ptr<T> const& deleter) noexcept
            {
                deleter_ = deleter;
            }
            ...
            template<class T>
            void operator()(T*) BOOST_SP_NOEXCEPT_WITH_ASSERT
            {
    ...
    template<class T>
    inline bool atomic_is_lock_free(shared_ptr<T> const* /*p*/) noexcept
    {
        return false;
    }
    ...
    inline shared_ptr<T> atomic_load_explicit(shared_ptr<T> const* p, /*memory_order mo*/ M) noexcept
    {
    ...
    template<class T, class M>
    inline void atomic_store_explicit(shared_ptr<T>* p, shared_ptr<T> r, /*memory_order mo*/ M) noexcept
    {
    ...
    template<class T, class M>
    shared_ptr<T> inline atomic_exchange_explicit(shared_ptr<T>* p, shared_ptr<T> r, /*memory_order mo*/ M) noexcept
    {
