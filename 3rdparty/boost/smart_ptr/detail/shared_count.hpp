...
namespace boost {
    ...
    namespace detail {
        ...
        template<class T>
        std::size_t sp_hash_pointer(T* p) noexcept
        {
        ...
        public:
            ...
            constexpr explicit shared_count(sp_counted_base* pi) noexcept :
                pi_(pi)
            {
            }
            ...
            template<class Y, class D>
            explicit shared_count(boost::movelib::unique_ptr<Y, D>& r) :
                pi_(0)
            {
                typedef typename sp_convert_reference<D>::type D2;
            ...
            shared_count(shared_count const& r) noexcept :
                pi_(r.pi_)
            {
                if (pi_ != 0) {
                    pi_->add_ref_copy();
            ...
            shared_count(shared_count&& r) noexcept :
                pi_(r.pi_)
            {
                r.pi_ = 0;
            }
            ...
            bool operator<(shared_count const& r) const noexcept { return std::less<sp_counted_base*>()(pi_, r.pi_); }

            bool operator<(weak_count const& r) const noexcept;

            void* get_deleter(sp_typeinfo_ const& ti) const noexcept { return pi_ ? pi_->get_deleter(ti) : 0; }
        ...
        public:
            ...
            weak_count(shared_count const& r) noexcept :
                pi_(r.pi_)
            {
                if (pi_ != 0) {
                    pi_->weak_add_ref();
            ...
            weak_count(weak_count const& r) noexcept :
                pi_(r.pi_)
            {
                if (pi_ != 0) {
                    pi_->weak_add_ref();
            ...
            bool operator<(weak_count const& r) const noexcept { return std::less<sp_counted_base*>()(pi_, r.pi_); }
        ...
        inline shared_count::shared_count(weak_count const& r, sp_nothrow_tag) noexcept :
            pi_(r.pi_)
        {
            if (pi_ != 0 && !pi_->add_ref_lock()) {
                pi_ = 0;
