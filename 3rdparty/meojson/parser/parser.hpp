...
namespace json {
    ...
    {
        return parser<accept_jsonc, string_t, parsing_t, accel_traits>(content.cbegin(), content.cend()).parse();
    }

    template<bool accept_jsonc, typename string_t, typename parsing_t, typename accel_traits>
    inline std::optional<basic_value<string_t>> parser<accept_jsonc, string_t, parsing_t, accel_traits>::parse()
    {
        if (!skip_whitespace()) {
            return std::nullopt;
        }
        ...
        switch (*_cur) {
        case '[': result_value = parse_array(); break;
        case '{': result_value = parse_object(); break;
    ...
    inline basic_value<string_t> parser<accept_jsonc, string_t, parsing_t, accel_traits>::parse_value()
    {
        ...
        default : return invalid_value<string_t>();
        }
    }
    ...
    template<bool accept_jsonc, typename string_t, typename parsing_t, typename accel_traits>
    inline basic_value<string_t> parser<accept_jsonc, string_t, parsing_t, accel_traits>::parse_boolean()
    {
        switch (*_cur) {
        case 't':
    ...
    template<bool accept_jsonc, typename string_t, typename parsing_t, typename accel_traits>
    inline std::optional<string_t> parser<accept_jsonc, string_t, parsing_t, accel_traits>::parse_stdstring()
    {
        if (*_cur == '"') {
            ++_cur;
        }
    ...
    template<bool accept_jsonc, typename string_t, typename parsing_t, typename accel_traits>
    inline bool parser<accept_jsonc, string_t, parsing_t, accel_traits>::skip_string_literal_with_accel()
    {
    ...
    bool json::parser<accept_jsonc, string_t, parsing_t, accel_traits>::skip_comment() noexcept
    {
        ...
        switch (*_cur++) {
        case '/': t = comment_type::line; break;
        case '*': t = comment_type::block; break;
