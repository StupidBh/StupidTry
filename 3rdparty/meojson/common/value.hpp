...
namespace json {
    ...
    public:
        ...
        template<
            ...
            std::enable_if_t<
                _utils::is_collection<collection_t> && std::is_constructible_v<
        ...
        template<typename fixed_array_t, std::enable_if_t<_utils::is_fixed_array<fixed_array_t>, bool> = true>
        ...
        template<
            ...
            std::enable_if_t<
                _utils::is_map<map_t> &&
                    std::is_constructible_v<typename basic_object<string_t>::value_type, _utils::range_value_t<map_t>>,
                bool> = true>
        ...
        template<typename variant_t, std::enable_if_t<_utils::is_variant<std::decay_t<variant_t>>, bool> = true>
        ...
        template<typename value_t, size_t Size, template<typename, size_t> typename fixed_array_t = std::array>
    ...
    inline bool basic_value<string_t>::is() const noexcept
    {
        if constexpr (std::is_same_v<basic_value<string_t>, value_t>) {
            ...
            return is_array() && as_array().size() == 2 && at(0).template is<typename value_t::first_type>() &&
    ...
    template<typename value_t, typename first_key_t, typename... rest_keys_t>
    inline auto
        basic_value<string_t>::get_helper(const value_t& default_value, first_key_t&& first, rest_keys_t&&... rest)
            const
    {
    ...
    inline auto basic_value<string_t>::get_helper(const value_t& default_value, unique_key_t&& first) const
    {
        if constexpr (std::is_constructible_v<string_t, unique_key_t>) {
            return is_object() ? as_object().get_helper(default_value, std::forward<unique_key_t>(first))
                               : default_value;
        }
    ...
    {
        switch (_type) {
        case value_type::null   : return _utils::null_string<string_t>();
        case value_type::boolean:
        case value_type::number : return as_basic_type_str();
        case value_type::string : return char_t('"') + _utils::unescape_string(as_basic_type_str()) + char_t('"');
        case value_type::array  : return as_array().to_string();
        case value_type::object : return as_object().to_string();
        default                 : throw exception("Unknown basic_value Type");
        }
    }
    ...
    inline string_t basic_value<string_t>::format(size_t indent, size_t indent_times) const
    {
        ...
        case value_type::string : return to_string();
        case value_type::array  : return as_array().format(indent, indent_times);
        case value_type::object : return as_object().format(indent, indent_times);
        default                 : throw exception("Unknown basic_value Type");
        }
    }
    ...
    inline bool basic_value<string_t>::operator==(const basic_value<string_t>& rhs) const
    {
        ...
        switch (_type) {
        case value_type::null   : return rhs.is_null();
        case value_type::boolean:
        case value_type::number :
        case value_type::string : return _raw_data == rhs._raw_data;
        case value_type::array  : return as_array() == rhs.as_array();
        case value_type::object : return as_object() == rhs.as_object();
        default                 : throw exception("Unknown basic_value Type");
        }
    }
