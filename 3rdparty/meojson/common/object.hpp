...
namespace json {
    ...
    public:
        ...
        template<
            typename map_t,
            std::enable_if_t<
                _utils::is_map<map_t> && std::is_constructible_v<value_type, _utils::range_value_t<map_t>>,
                bool> = true>
    ...
    template<typename value_t, typename... rest_keys_t>
    inline auto
        basic_object<string_t>::get_helper(const value_t& default_value, const string_t& key, rest_keys_t&&... rest)
            const
    {
