...
namespace bit7z {
    ...
    public:
         ...
         *                            entire file or only at the beginning. The default behavior is to search at the
         * beginning.
         */
        BitInputArchive(
        ...
        inline void extract(std::vector<byte_t>& outBuffer, uint32_t index = 0) const { extractTo(outBuffer, index); }
        ...
        inline void extract(std::ostream& outStream, uint32_t index = 0) const { extractTo(outStream, index); }
        ...
        inline void extract(std::map<tstring, std::vector<byte_t>>& outMap) const { extractTo(outMap); }
    ...
    private:
        ...
        BIT7Z_NODISCARD
        auto openArchiveStream(const fs::path& name, IInStream* inStream, ArchiveStartOffset startOffset)
            -> IInArchive*;
