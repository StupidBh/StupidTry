...
namespace bit7z {
    ...
    public:
         ...
         */
        template<typename T>
        BIT7Z_NODISCARD static auto
            isHeaderEncrypted(const Bit7zLibrary& lib, T&& inArchive, const BitInFormat& format BIT7Z_DEFAULT_FORMAT)
                -> bool
        {
            try {
        ...
        template<typename T>
        BIT7Z_NODISCARD static auto
            isEncrypted(const Bit7zLibrary& lib, T&& inArchive, const BitInFormat& format BIT7Z_DEFAULT_FORMAT) -> bool
        {
            try {
                const BitArchiveReader reader { lib, std::forward<T>(inArchive), format };
