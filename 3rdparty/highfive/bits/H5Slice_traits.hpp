...
namespace HighFive {
    ...
    public:
        DataSpace apply(const DataSpace& space) const { return static_cast<const Impl&>(*this).apply(space); }
    };
    ...
    struct RegularHyperSlab
    {
        ...
        static RegularHyperSlab fromHDF5Sizes(
        ...
        size_t rank() const
        {
            return std::max(std::max(offset.size(), count.size()), std::max(stride.size(), block.size()));
        }
    ...
    public:
        HyperSlab() { selects.emplace_back(RegularHyperSlab {}, Op::None); };

        explicit HyperSlab(const RegularHyperSlab& sel) { selects.emplace_back(sel, Op::Set); }
        ...
        DataSpace apply(const DataSpace& space_) const { return apply_impl(space_); }
    ...
    private:
        ...
        {
            switch (op) {
            case Op::Noop   : return H5S_SELECT_NOOP;
            case Op::Set    : return H5S_SELECT_SET;
            case Op::Or     : return H5S_SELECT_OR;
            case Op::And    : return H5S_SELECT_AND;
            case Op::Xor    : return H5S_SELECT_XOR;
            case Op::NotB   : return H5S_SELECT_NOTB;
            case Op::NotA   : return H5S_SELECT_NOTA;
            case Op::Append : return H5S_SELECT_APPEND;
            case Op::Prepend: return H5S_SELECT_PREPEND;
            case Op::Invalid: return H5S_SELECT_INVALID;
            default         : throw DataSpaceException("Invalid HyperSlab operation.");
            }
        }
    ...
    protected:
        ...
        {
            assert(op == Op::Or);
            auto const* it = std::find_if(begin, end, [op](const Select_& sel) { return sel.op != op; });
            return static_cast<size_t>(it - begin);
        }

        DataSpace combine_selections(const DataSpace& left_space, Op op, const DataSpace& right_space) const
        {
            assert(op == Op::Or);
        ...
        DataSpace reduce_streak(const DataSpace& outer_space, Select_ const* begin, Select_ const* end, Op op) const
        {
            assert(op == Op::Or);
    ...
    public:
        ...
        template<typename T>
        void read_raw(T* array, const DataType& mem_datatype, const DataTransferProps& xfer_props = DataTransferProps())
            const;
