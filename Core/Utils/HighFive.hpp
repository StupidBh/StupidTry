...
requires std::derived_from<std::remove_cvref_t<_Ty>, HighFive::NodeTraits<std::remove_cvref_t<_Ty>>>
HighFive::DataSet WriteDataSet(const std::string& name, ValueType&& input, _Ty&& loc)
{
    ...
    data_set = std::forward<_Ty>(loc).createDataSet<TrueValueType>(name, HighFive::DataSpace::From(input));
