...
int CgnsCore::CG_INFO(int status)
{
    if (status != CG_OK) {
...
bool CgnsCore::OpenCGNS()
{
    ...
    if (CG_INFO(cg_open(this->m_cgns_file_path.c_str(), CG_MODE_READ, &this->m_cg_file_id)) != CG_OK) {
        // cg_open does not guarantee the file id is reset on failure; force it to 0 so the
        // destructor / IsOpen() never act on a stale handle.
        this->m_cg_file_id = 0;
...
void CgnsCore::info() const
{
    ...
    for (int base = 1; base <= nbases; ++base) {
        ...
        CG_INFO(cg_base_read(this->m_cg_file_id, base, base_name, &cell_dim, &phys_dim));
        CG_INFO(cg_simulation_type_read(this->m_cg_file_id, base, &base_simulation_type));
        LOG_INFO("{:>2}:[{}] {}, CellDimension={}, PhysicalDimension={}",
                 base,
        ...
        for (int zone = 1; zone <= nzones; ++zone) {
            ...
            CG_INFO(cg_zone_read(this->m_cg_file_id, base, zone, zone_name, zone_size.data()));
            if (zone_type == CG_ZoneType_t::CG_Structured) {
                ...
                case 1: {
                    LOG_INFO("  {:>3}:[{}] {}, Dim={}", zone, cg_ZoneTypeName(zone_type), zone_name, index_zone_dim);
                } break;
                case 2: {
                    cgsize_t vertex_sum = zone_size[0] * zone_size[1];
                    cgsize_t cell_sum = zone_size[2] * zone_size[3];
                    LOG_INFO("  {:>3}:[{}] {}, Dim={}, NVertex=[{},{}]:{}, NCell=[{},{}]:{}, NBoundVertex=[{},{}]",
                             zone,
                ...
                LOG_INFO("  {:>3}:[{}] {}, NVertex={}, NCell={}, NBoundVertex={}",
                         zone,
                         cg_ZoneTypeName(zone_type),
                         zone_name,
                         zone_size[0],
            ...
            for (int sol = 1; sol <= nsols; ++sol) {
                ...
                CG_INFO(cg_sol_info(this->m_cg_file_id, base, zone, sol, sol_name, &sol_location));
                CG_INFO(cg_sol_size(this->m_cg_file_id, base, zone, sol, &sol_data_dim, sol_dim_vals.data()));
                // A whole-field solution has no point set, so cg_sol_ptset_info legitimately fails
                // there. Don't route it through CG_INFO — that would log a spurious error per sol.
                // On failure the type stays *Null and npnts stays 0.
                cg_sol_ptset_info(this->m_cg_file_id, base, zone, sol, &sol_point_set_type, &sol_npnts);

                sol_dim_vals.resize(sol_data_dim);
                LOG_INFO("    {:>2}:[{}]-[{}] {}, NField={}, DataDim={}, DataVal={}, npnts={}",
                         sol,
