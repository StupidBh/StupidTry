...
namespace HighFive {
    ...
    inline DataSpace::DataSpace(DataSpace::DataspaceType space_type)
    {
        auto to_hdf5 = [](auto _space_type) -> H5S_class_t {
            switch (_space_type) {
            case DataSpace::dataspace_scalar: return H5S_SCALAR;
            case DataSpace::dataspace_null  : return H5S_NULL;
            default:
    ...
    inline std::vector<size_t> DataSpace::getMaxDimensions() const
    {
        ...
        std::replace(maxdims.begin(), maxdims.end(), H5S_UNLIMITED, static_cast<hsize_t>(DataSpace::UNLIMITED));
    ...
    namespace details {
        ...
        inline bool checkDimensions(const DataSpace& mem_space, size_t min_dim_requested, size_t max_dim_requested)
        {
