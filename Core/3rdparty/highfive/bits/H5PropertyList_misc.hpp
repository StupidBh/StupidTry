...
namespace HighFive {
    ...
    namespace {
        inline hid_t convert_plist_type(PropertyType propertyType)
        {
            ...
            default                            : HDF5ErrMapper::ToException<PropertyException>("Unsupported property list type");
            }
        }
...
// Specific options to be added to Property Lists
    ...
    inline FileSpaceStrategy::FileSpaceStrategy(H5F_fspace_strategy_t strategy, hbool_t persist, hsize_t threshold) :
        _strategy(strategy),
        _persist(persist),
    ...
    inline PageBufferSize::PageBufferSize(
    ...
    inline Chunking::Chunking(DataSetCreateProps& plist, size_t max_dims) :
        _dims(max_dims + 1)
    {
        auto n_loaded = detail::h5p_get_chunk(plist.getId(), static_cast<int>(_dims.size()), _dims.data());
