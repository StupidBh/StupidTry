...
namespace HighFive {
    namespace detail {
        ...
        inline herr_t h5s_select_hyperslab(
            hid_t space_id,
            H5S_seloper_t op,
            const hsize_t start[],
        ...
        inline hssize_t h5s_get_select_npoints(hid_t spaceid)
        {
            ...
            if (n_points < 0) {
                HDF5ErrMapper::ToException<DataSpaceException>("Unable to get number of points in selection");
            }
            return n_points;
        }

        inline herr_t h5s_select_elements(hid_t space_id, H5S_seloper_t op, size_t num_elem, const hsize_t* coord)
        {
        ...
        inline int h5s_get_simple_extent_ndims(hid_t space_id)
        {
            ...
            if (ndim < 0) {
                HDF5ErrMapper::ToException<DataSetException>("Unable to get number of dimensions of dataspace");
            }
            return ndim;
        }
        ...
        inline hssize_t h5s_get_simple_extent_npoints(hid_t space_id)
        {
            ...
            if (nelements < 0) {
                HDF5ErrMapper::ToException<DataSetException>("Unable to get number of elements in dataspace");
            }
