...
namespace HighFive {
    ...
    inline FixedLengthStringType::FixedLengthStringType(size_t size, StringPadding padding, CharacterSet character_set)
    {
    ...
    public:
        inline AtomicType() :
            DataType(CompoundType(
                { { "r", create_datatype<T>(), 0 }, { "i", create_datatype<T>(), sizeof(T) } },
    ...
    {
        static_assert(true, "Missing specialization of AtomicType<T>. Therefore, type T is not supported by HighFive.");
    }
    ...
    inline size_t find_first_atomic_member_size(hid_t hid)
    {
        ...
        if (detail::h5t_get_class(hid) == H5T_COMPOUND) {
            ...
            if (number_of_members == -1) {
                throw DataTypeException("Cannot get members of CompoundType with hid: " + std::to_string(hid));
            }
            if (number_of_members == 0) {
                throw DataTypeException("No members defined for CompoundType with hid: " + std::to_string(hid));
            }
    ...
    inline void CompoundType::create(size_t size)
    {
        if (size == 0) {
            ...
            for (auto& member : members) {
                ...
                if (member_size == 0) {
                    throw DataTypeException(
                        "Cannot get size of DataType with hid: " + std::to_string(member.base_type.getId()));
                }
    ...
    {
        detail::h5t_commit2(object.getId(), name.c_str(), getId(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    }
    ...
    inline DataType create_and_check_datatype()
    {
        ...
        if (sizeof(T) != t.getSize()) {
            ...
            ss << "Size of array type " << sizeof(T) << " != that of memory datatype " << t.getSize() << std::endl;
