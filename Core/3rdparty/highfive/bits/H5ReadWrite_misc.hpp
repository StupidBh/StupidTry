...
namespace HighFive {
    ...
    namespace details {
        ...
        BufferInfo<T>::BufferInfo(const DataType& file_data_type, F getName, Operation _op) :
            ...
            ,
            data_type(string_type_checker<char_array_t>::getDataType(create_datatype<elem_type>(), file_data_type)),
            rank_correction((is_fixed_len_string && is_char_array) ? 1 : 0)
        {
            ...
            if (file_data_type.getClass() != data_type.getClass()) {
                HIGHFIVE_LOG_WARN(
                    getName() + "\": data and hdf5 dataset have different types: " + data_type.string() + " -> " +
                ...
                HIGHFIVE_LOG_WARN_IF(
                    ...
                    getName() + "\": data has higher floating point precision than hdf5 dataset on write: " +
