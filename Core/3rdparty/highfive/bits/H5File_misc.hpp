...
namespace HighFive {
    ...
    inline File::File(const std::string& filename, AccessMode openFlags, const FileAccessProps& fileAccessProps) :
        ...
        const FileAccessProps& fileAccessProps)
    {
        ...
        if (!mustCreate) {
            ...
            if (openOrCreate) {
                ...
                HDF5ErrMapper::ToException<FileException>(std::string("Unable to open file " + filename));
            }
        }
    ...
    inline const std::string& File::getName() const
    {
        if (_filename.empty()) {
            _filename = details::get_name(
                [this](char* buffer, size_t length) { return detail::h5f_get_name(getId(), buffer, length); });
        }
        return _filename;
    }
