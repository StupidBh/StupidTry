...
namespace HighFive {
    namespace details {
        ...
        {
            static constexpr bool value = std::is_same<typename inspector<T>::base_type, std::string>::value;
        };
        ...
        struct ShallowCopyBuffer
        {
            ...
            using hdf5_type = typename std::conditional<
                IsReadOnly,
            ...
            hdf5_type* getPointer() const { return ptr; }

            hdf5_type* begin() const { return getPointer(); }

            void unserialize(T& /* val */) const { /* nothing to do. */ }
        ...
        struct DeepCopyBuffer
        {
            ...
            hdf5_type* getPointer() { return buffer.data(); }

            hdf5_type const* getPointer() const { return buffer.data(); }

            hdf5_type* begin() { return getPointer(); }

            hdf5_type const* begin() const { return getPointer(); }

            void unserialize(T& val) const { inspector<type>::unserialize(buffer.data(), dims, val); }
        ...
        struct StringBuffer
        {
            ...
            public:
                ...
                Iterator operator+(size_t n_strings) const { return Iterator(buffer, pos + n_strings); }

                void operator+=(size_t n_strings) { pos += n_strings; }

                StringView operator*() { return StringView(buffer, pos); }

                StringConstView operator*() const { return StringConstView(buffer, pos); }
            ...
            bool isVariableLengthString() const { return file_datatype.isVariableStr(); }

            bool isFixedLengthString() const { return file_datatype.isFixedLenStr(); }

            bool isNullTerminated() const { return file_datatype.getPadding() == StringPadding::NullTerminated; }
            ...
            Iterator begin() { return Iterator(*this, 0ul); }

            void unserialize(T& val) { inspector<type>::unserialize(begin(), dims, val); }
        ...
        private:
            ...
            std::vector<typename std::conditional<buffer_mode == BufferMode::Write, const char, char>::type*>
        ...
        {
        public:
            explicit Reader(const std::vector<size_t>& _dims, const T& /* val */, const DataType& _file_datatype) :
                StringBuffer<T, BufferMode::Write>(_dims, _file_datatype)
            {
            }
        };
        ...
        struct data_converter
        {
            ...
            template<typename T>
            static Reader<T> get_reader(const std::vector<size_t>& dims, T& val, const DataType& file_datatype)
            {
