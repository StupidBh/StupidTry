    ...
    namespace detail {
        ...
        inline hid_t h5d_get_space(hid_t dset_id)
        {
            ...
            if (dset == H5I_INVALID_HID) {
                HDF5ErrMapper::ToException<DataSetException>(std::string("Unable to get dataspace of the dataset"));
            }
        ...
        inline hid_t h5d_get_type(hid_t dset_id)
        {
            ...
            if (type_id == H5I_INVALID_HID) {
                HDF5ErrMapper::ToException<DataSetException>(std::string("Unable to get datatype of the dataset"));
            }

            return type_id;
        }

        inline herr_t h5d_read(
            hid_t dset_id,
            hid_t mem_type_id,
            hid_t mem_space_id,
            hid_t file_space_id,
        ...
        inline hid_t h5d_open2(hid_t loc_id, const char* name, hid_t dapl_id)
        {
            ...
            if (dataset_id == H5I_INVALID_HID) {
                HDF5ErrMapper::ToException<DataSetException>(
                    std::string("Unable to open the dataset \"") + name + "\":");
            }
