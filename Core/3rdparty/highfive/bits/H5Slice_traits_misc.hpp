...
namespace HighFive {
    ...
    namespace detail {
        ...
        public:
            ...
            RegularHyperSlab asSlab() { return RegularHyperSlab(offset, count); }
        ...
        template<class... Slices>
        inline void
            build_hyper_slab(HyperSlab& slab, size_t axis, HyperCube& cube, size_t id, const Slices&... higher_slices)
        {
            auto slice = std::array<size_t, 2> { id, id + 1 };
            build_hyper_slab(slab, axis, cube, slice, higher_slices...);
        }
        ...
        template<class... Slices>
        inline void compute_squashed_shape(
            size_t axis,
    ...
    inline Selection SliceTraits<Derivate>::select(const HyperSlab& hyper_slab, const DataSpace& memspace) const
    {
    ...
    inline void SliceTraits<Derivate>::read(T& array, const DataTransferProps& xfer_props) const
    {
        ...
        if (!details::checkDimensions(mem_space, buffer_info.getMinRank(), buffer_info.getMaxRank())) {
            ...
            ss << "Impossible to read DataSet of dimensions " << mem_space.getNumberDimensions()
               << " into arrays of dimensions: " << buffer_info.getMinRank() << "(min) to " << buffer_info.getMaxRank()
               << "(max)";
        ...
        if (c == DataTypeClass::VarLen || t.isVariableStr()) {
            ...
            (void)detail::h5t_reclaim(t.getId(), mem_space.getId(), xfer_props.getId(), r.getPointer());
#else
            // This one is deprecated since 1.12.0
            (void)detail::h5d_vlen_reclaim(t.getId(), mem_space.getId(), xfer_props.getId(), r.getPointer());
#endif
        }
    }
    ...
    template<typename T>
    inline void
        SliceTraits<Derivate>::read_raw(T* array, const DataType& mem_datatype, const DataTransferProps& xfer_props)
            const
    {
        static_assert(!std::is_const<T>::value, "read() requires a non-const structure to read data into");

        const auto& slice = static_cast<const Derivate&>(*this);

        detail::h5d_read(
            details::get_dataset(slice).getId(),
            mem_datatype.getId(),
    ...
    template<typename Derivate>
    template<typename T>
    inline void SliceTraits<Derivate>::write_raw(
        const T* buffer,
    ...
    inline Selection SliceTraits<Derivate>::squeezeMemSpace(const std::vector<size_t>& axes) const
    {
        ...
        return detail::make_selection(DataSpace(squeezed_dims), slice.getSpace(), detail::getDataSet(slice));
    }
