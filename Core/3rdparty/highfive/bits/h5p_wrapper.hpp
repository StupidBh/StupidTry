...
namespace HighFive {
    namespace detail {
        ...
        inline herr_t h5p_set_file_space_strategy(
            hid_t plist_id,
        ...
        inline herr_t h5p_get_file_space_strategy(
            hid_t plist_id,
            H5F_fspace_strategy_t* strategy,
            hbool_t* persist,
            hsize_t* threshold)
        ...
        inline herr_t
            h5p_set_page_buffer_size(hid_t plist_id, size_t buf_size, unsigned min_meta_per, unsigned min_raw_per)
        {
        ...
        inline herr_t h5p_set_est_link_info(hid_t plist_id, unsigned est_num_entries, unsigned est_name_len)
        {
        ...
        inline herr_t h5p_get_est_link_info(hid_t plist_id, unsigned* est_num_entries, unsigned* est_name_len)
        {
        ...
        inline herr_t h5p_get_chunk_cache(hid_t dapl_id, size_t* rdcc_nslots, size_t* rdcc_nbytes, double* rdcc_w0)
        {
        ...
        inline herr_t h5p_set_chunk_cache(hid_t dapl_id, size_t rdcc_nslots, size_t rdcc_nbytes, double rdcc_w0)
        {
        ...
        inline herr_t h5p_set_create_intermediate_group(hid_t plist_id, unsigned crt_intmd)
        {
            ...
            if (err < 0) {
                HDF5ErrMapper::ToException<PropertyException>("Error setting property for create intermediate groups");
            }
            return err;
        }
        ...
        inline herr_t h5p_get_create_intermediate_group(hid_t plist_id, unsigned* crt_intmd)
        {
            ...
            if (err < 0) {
                HDF5ErrMapper::ToException<PropertyException>("Error getting property for create intermediate groups");
            }
            return err;
        }
        ...
        inline herr_t h5p_get_mpio_no_collective_cause(
            hid_t plist_id,
        ...
        inline herr_t h5p_get_link_creation_order(hid_t plist_id, unsigned* crt_order_flags)
        {
            ...
            if (err < 0) {
                HDF5ErrMapper::ToException<PropertyException>("Error getting property for link creation order");
            }
            return err;
        }

        inline herr_t h5p_get_attr_phase_change(hid_t plist_id, unsigned* max_compact, unsigned* min_dense)
        {
            herr_t err = H5Pget_attr_phase_change(plist_id, max_compact, min_dense);
            if (err < 0) {
                HDF5ErrMapper::ToException<PropertyException>("Error getting property for attribute phase change");
            }
            return err;
        }
