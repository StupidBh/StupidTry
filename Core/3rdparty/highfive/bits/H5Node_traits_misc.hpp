...
namespace HighFive {
    ...
    inline DataSet NodeTraits<Derivate>::createDataSet(
        ...
        bool parents)
    {
        ...
        return DataSet(detail::h5d_create2(
            static_cast<Derivate*>(this)->getId(),
            dataset_name.c_str(),
            dtype.getId(),
            space.getId(),
            lcpl.getId(),
    ...
    inline DataSet NodeTraits<Derivate>::createDataSet(
        ...
        bool parents)
    {
        return createDataSet(dataset_name, space, create_and_check_datatype<T>(), createProps, accessProps, parents);
    }
    ...
    template<typename Derivate>
    inline DataSet
        NodeTraits<Derivate>::getDataSet(const std::string& dataset_name, const DataSetAccessProps& accessProps) const
    {
        return DataSet(
            detail::h5d_open2(static_cast<const Derivate*>(this)->getId(), dataset_name.c_str(), accessProps.getId()));
    }
    ...
    inline Group NodeTraits<Derivate>::createGroup(const std::string& group_name, bool parents)
    {
        LinkCreateProps lcpl;
        lcpl.add(CreateIntermediateGroup(parents));
        return detail::make_group(detail::h5g_create2(
            static_cast<Derivate*>(this)->getId(),
            group_name.c_str(),
            lcpl.getId(),
            H5P_DEFAULT,
    ...
    template<typename Derivate>
    inline Group NodeTraits<Derivate>::createGroup(
    ...
    {
        return detail::make_group(
            detail::h5g_open2(static_cast<const Derivate*>(this)->getId(), group_name.c_str(), H5P_DEFAULT));
    }

    template<typename Derivate>
    inline DataType
        NodeTraits<Derivate>::getDataType(const std::string& type_name, const DataTypeAccessProps& accessProps) const
    {
        return DataType(
            detail::h5t_open2(static_cast<const Derivate*>(this)->getId(), type_name.c_str(), accessProps.getId()));
    }
    ...
    inline std::string NodeTraits<Derivate>::getObjectName(size_t index) const
    {
        return details::get_name([&](char* buffer, size_t length) {
            return detail::h5l_get_name_by_idx(
    ...
    template<typename Derivate>
    inline bool
        NodeTraits<Derivate>::rename(const std::string& src_path, const std::string& dst_path, bool parents) const
    {
        LinkCreateProps lcpl;
        lcpl.add(CreateIntermediateGroup(parents));
        herr_t err = detail::h5l_move(
            static_cast<const Derivate*>(this)->getId(),
            src_path.c_str(),
            static_cast<const Derivate*>(this)->getId(),
            dst_path.c_str(),
    ...
    inline bool NodeTraits<Derivate>::_exist(const std::string& node_path, bool raise_errors) const
    {
        ...
        const auto val =
            detail::nothrow::h5l_exists(static_cast<const Derivate*>(this)->getId(), node_path.c_str(), H5P_DEFAULT);
        if (val < 0) {
            if (raise_errors) {
    ...
    {
        H5L_info_t linkinfo;
        detail::h5l_get_info(static_cast<const Derivate*>(this)->getId(), node_path.c_str(), &linkinfo, H5P_DEFAULT);

        if (linkinfo.type == H5L_TYPE_ERROR) {
            HDF5ErrMapper::ToException<GroupException>(
                std::string("Link type of \"") + node_path + "\" is H5L_TYPE_ERROR");
        }
    ...
    {
        const auto id = detail::h5o_open(static_cast<const Derivate*>(this)->getId(), node_path.c_str(), H5P_DEFAULT);
    ...
    template<typename Derivate>
    inline void NodeTraits<Derivate>::createSoftLink(
    ...
    template<typename Derivate>
    inline void NodeTraits<Derivate>::createExternalLink(
        const std::string& link_name,
    ...
    inline void NodeTraits<Derivate>::createHardLink(
        const std::string& link_name,
        const T& target_obj,
        LinkCreateProps linkCreateProps,
        const LinkAccessProps& linkAccessProps,
        const bool parents)
    {
        static_assert(!std::is_same<T, Attribute>::value, "hdf5 doesn't support hard links to Attributes");
        if (parents) {
            linkCreateProps.add(CreateIntermediateGroup {});
        }
