...
namespace HighFive {
    namespace detail {
        ...
        inline htri_t h5t_is_variable_str(hid_t type_id)
        {
            ...
            if (is_variable < 0) {
                HDF5ErrMapper::ToException<DataTypeException>("Failed to check if string is variable length");
            }
            return is_variable;
        }

        inline herr_t h5t_set_fields(hid_t type_id, size_t spos, size_t epos, size_t esize, size_t mpos, size_t msize)
        {
            herr_t err = H5Tset_fields(type_id, spos, epos, esize, mpos, msize);
            if (err < 0) {
                HDF5ErrMapper::ToException<DataTypeException>("Failed to create custom floating point data type");
            }
            return err;
        }
        ...
        inline herr_t h5t_set_ebias(hid_t type_id, size_t ebias)
        {
            ...
            if (err < 0) {
                HDF5ErrMapper::ToException<DataTypeException>("Failed to exponent bias of floating point data type");
            }
        ...
        inline herr_t
            h5t_commit2(hid_t loc_id, const char* name, hid_t type_id, hid_t lcpl_id, hid_t tcpl_id, hid_t tapl_id)
        {
        ...
        inline herr_t h5t_enum_insert(hid_t type, const char* name, const void* value)
        {
            ...
            if (err < 0) {
                HDF5ErrMapper::ToException<DataTypeException>("Failed to add new member to this enum datatype");
            }
            return err;
        }
