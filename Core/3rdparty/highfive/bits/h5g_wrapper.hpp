...
namespace HighFive {
    namespace detail {
        ...
        inline hid_t h5g_create2(hid_t loc_id, const char* name, hid_t lcpl_id, hid_t gcpl_id, hid_t gapl_id)
        {
            hid_t group_id = H5Gcreate2(loc_id, name, lcpl_id, gcpl_id, gapl_id);
            if (group_id == H5I_INVALID_HID) {
                HDF5ErrMapper::ToException<GroupException>(std::string("Unable to create the group \"") + name + "\":");
            }
        ...
        inline hid_t h5g_open2(hid_t loc_id, const char* name, hid_t gapl_id)
        {
            ...
            if (group_id == H5I_INVALID_HID) {
                HDF5ErrMapper::ToException<GroupException>(std::string("Unable to open the group \"") + name + "\":");
            }
            return group_id;
        }
