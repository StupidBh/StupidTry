...
namespace HighFive {
    namespace detail {
        ...
        inline herr_t h5l_create_external(
            ...
            hid_t lapl_id)
        {
            ...
            if (err < 0) {
                HDF5ErrMapper::ToException<GroupException>(std::string("Unable to move link to \"") + dst_name + "\":");
            }
            return err;
        }

        inline ssize_t h5l_get_name_by_idx(
            hid_t loc_id,
            ...
            hid_t lapl_id)
        {
            ssize_t n_chars = H5Lget_name_by_idx(loc_id, group_name, idx_type, order, n, name, size, lapl_id);

            if (n_chars < 0) {
                HDF5ErrMapper::ToException<GroupException>(std::string("Unable to obtain link name from index."));
            }
