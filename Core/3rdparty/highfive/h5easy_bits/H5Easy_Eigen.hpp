...
namespace H5Easy {
    namespace detail {
        ...
        struct io_impl<T, typename std::enable_if<std::is_base_of<Eigen::DenseBase<T>, T>::value>::type>
        {
            ...
            inline static std::vector<size_t> mem_shape(const T& data) { return inspector<T>::getDimensions(data); }

            // The shape of an Eigen object as used in HighFive core.
            template<class D>
            inline static std::vector<size_t> mem_shape(const File& file, const std::string& path, const D& dataset)
            {
