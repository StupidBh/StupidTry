...
namespace H5Easy {
    namespace detail {
        ...
        inline Exception error(const File& file, const std::string& path, const std::string& message)
        {
            ...
            ss << message << std::endl << "Path: " << path << std::endl << "Filename: " << file.getName() << std::endl;
        ...
        template<class T>
        inline DataSet initScalarDataset(File& file, const std::string& path, const T& data, const DumpOptions& options)
        {
            if (!file.exist(path)) {
        ...
        template<class T>
        inline Attribute initAttribute(
            File& file,
            const std::string& path,
            ...
            const DumpOptions& options)
        {
            auto get_attribute = [&](auto& obj) {
                ...
                throw error(file, path, "H5Easy: Attribute exists, overwrite with H5Easy::DumpMode::Overwrite.");
            };
        ...
        template<class T>
        inline Attribute initScalarAttribute(
            File& file,
            const std::string& path,
            const std::string& key,
            const T& data,
