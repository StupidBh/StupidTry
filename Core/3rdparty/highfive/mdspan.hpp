        ...
        public:
            ...
            static void prepare(type& val, const std::vector<size_t>& dims)
            {
                if (dims.size() < ndim) {
                    ...
                    os << "Impossible to pair DataSet with " << dims.size() << " dimensions into an mdspan with rank "
                       << ndim << ".";
                ...
                for (size_t r = 0; r < ndim; ++r) {
                    if (dims[r] != val.extent(r)) {
                        std::ostringstream os;
                        os << "Mismatching dimensions for mdspan: expected " << val.extent(r) << " for dimension " << r
                           << ", but got " << dims[r] << ".";
                        throw DataSpaceException(os.str());
                    }
                }
            ...
            static hdf5_type* data(type& val) { return data_impl(val); }

            static const hdf5_type* data(const type& val) { return data_impl(val); }
