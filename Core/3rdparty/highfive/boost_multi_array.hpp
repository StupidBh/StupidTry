    ...
    namespace details {
        ...
        struct inspector<boost::multi_array<T, Dims>>
        {
            ...
            static void prepare(type& val, const std::vector<size_t>& dims)
            {
                ...
                std::size_t size =
                    std::accumulate(dims.begin(), dims.begin() + Dims, std::size_t { 1 }, std::multiplies<size_t>());
            ...
            static void unserialize(It vec_align, const std::vector<size_t>& dims, type& val)
            {
                ...
                for (size_t i = 0; i < val.num_elements(); ++i) {
                    inspector<value_type>::unserialize(vec_align + i * subsize, next_dims, *(val.origin() + i));
                }
            }
        };
