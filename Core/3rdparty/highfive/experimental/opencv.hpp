        ...
        struct inspector<cv::Mat_<T>>
        {
            ...
            static size_t getRank(const type& val)
            {
                if (val.empty()) {
                    ...
                    return static_cast<size_t>(val.dims) + inspector<value_type>::getRank(getAnyElement(val));
                }
            }

            static const T& getAnyElement(const type& val) { return *reinterpret_cast<T const*>(val.data); }

            static T& getAnyElement(type& val) { return *reinterpret_cast<T*>(val.data); }

            static size_t getLocalRank(const type& val) { return static_cast<size_t>(val.dims); }
