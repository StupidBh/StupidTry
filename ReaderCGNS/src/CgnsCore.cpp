...
#define CG_INFO(STATUS) CG_INFO(STATUS, __FILE__, __LINE__)
    ...
    bool CgnsCore::OpenCGNS()
    {
        ...
        LOG_INFO("[{}] v{:.2f}, precision={}, file_id{}",
    ...
    void CgnsCore::info()
    {
        if (!this->OpenCGNS() && !this->IsOpen()) {
            LOG_WARN("Open [{}] failed.", this->m_cgns_file_path);
            return;
        }
        ...
        for (int base = 1; base <= nbases; ++base) {
            ...
            for (int zone = 1; zone <= nzones; ++zone) {
                ...
                for (int sol = 1; sol <= nsols; ++sol) {
                    ...
                    CG_INFO(
                        cg_sol_ptset_info(this->m_cg_file_id, base, zone, sol, &sol_point_set_type, sol_npnts.data()));
                ...
                for (int discrete = 1; discrete <= ndiscrete; ++discrete) {
                    ...
                    if (discrete_point_set_type == CG_PointSetType_t::CG_PointSetTypeNull) {
                        ...
                        CG_INFO(
                            cg_discrete_ptset_read(this->m_cg_file_id, base, zone, discrete, discrete_npnts.data()));
                    }
