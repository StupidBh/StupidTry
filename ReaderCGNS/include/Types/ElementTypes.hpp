#pragma once
#include "ReaderAPI/ReaderApiTypes.hpp"
...
namespace ReaderAPI {
    ...
    public:
        ...
        void Replace(std::size_t index, const Elem& elem)
        {
            ...
            if (previous.nodes.size() == replacement.nodes.size() && previous.mid_nodes.size() == replacement.mid_nodes.size() &&
                ...
                this->MetaData(index) = static_cast<ElementMetaData>(replacement);
        ...
        {
            const auto& [meta_data, offsets] = this->m_records[i];
            auto range = [&](std::size_t domain) {
                const std::size_t first = offsets[domain];
                const std::size_t count = offsets[domain + 1] - first;
                return ElementNodeRange(count ? this->m_node_ids.data() + first : nullptr, count);
            };
            return ElementView(meta_data, range(0), range(1), range(2));
        }
    ...
    class ElementRange {
    public:
        explicit ElementRange(const std::vector<Elem>& elems) :
            m_elems(&elems)
        {
        }

        explicit ElementRange(const ElementTable& table) :
            m_table(&table)
        {
        }
