...
namespace {
    ...
    constexpr bool IsValidLogLevel(const ReaderAPI::Logger::LogLevel level) noexcept
    {
        return level >= ReaderAPI::Logger::READER_CGNS_LOG_TRACE && level <= ReaderAPI::Logger::READER_CGNS_LOG_CRITICAL;
    }
...
bool LogDispatcher::SetCallback(const ReaderAPI::Logger::LogCallback callback, void* context) noexcept
{
...
bool LogDispatcher::IsDispatchEnabled(const ReaderAPI::Logger::LogLevel level) const noexcept
{
    return IsValidLogLevel(level) && this->m_enabled.load(std::memory_order_acquire);
}

void LogDispatcher::Dispatch(const ReaderAPI::Logger::LogLevel level, const char* file, const int line, const char* message) noexcept
{
    ...
    ReaderAPI::Logger::LogCallback callback = nullptr;
